...
def get_localedir():
    try:
        ...
        return str(resources.path("flowstrider.localization", ""))
    except Exception:
...
lang_sys = gettext.translation("messages", localedir=get_localedir(), languages=["en"])
# Output language that is being used for the actual content (elicitation results (pdf
# ...and cmd) and metadata usage hints (xlsx))
lang_out = gettext.translation("messages", localedir=get_localedir(), languages=["en"])
...
def init_localization(language: str, position: str):
    ...
    if position == "sys":
        ...
        lang_sys = gettext.translation(
            "messages", localedir=get_localedir(), languages=[language]
        )
        lang_sys.install()
