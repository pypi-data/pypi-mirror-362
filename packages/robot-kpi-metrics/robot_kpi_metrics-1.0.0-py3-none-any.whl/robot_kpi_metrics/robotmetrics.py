...
IGNORE_LIBRARIES = ["SeleniumLibrary", "BuiltIn", "Collections", "DateTime", "Dialogs", "OperatingSystem",
...
def generate_report(opts):
    ...
    if opts.metrics_report_name:
        ...
        result_file_name = 'kpi-metrics-' + mt_time + '.html'
    ...
    logging.info(" Preparing data for KPI dashboard")
    ...
    # Sort keyword times by max duration for KPI display
    if not kw_times.empty:
        kw_times_sorted = kw_times.sort_values('time_max', ascending=False)
    else:
        kw_times_sorted = kw_times
    ...
    with codecs.open(result_file,'w','utf-8') as fh:
        fh.write(template.render(
            ...
            suites = suite_list,
            tests = test_list,
            keyword_times = kw_times_sorted,
            suite_error_stats = suite_error_stats,
            suites_list = details_list,
            execution_stats=execution_stats,
            test_time_group=test_time_group,
        ))
    logging.info(" KPI Results file created successfully and can be found at {}".format(result_file))
