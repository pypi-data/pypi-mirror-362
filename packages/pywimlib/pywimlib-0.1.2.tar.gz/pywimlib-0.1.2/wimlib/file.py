...
class WIMFile(object):
    def __init__(self, path=None, flags=None, compression=0, callback=None, context=None):
        ...
        if not self.path:
            ...
            if not callback:
                ...
                self._wim_struct = self._open_with_progress(
                    path, flags, callback, context)
        ...
        if not self._has_baking_file:
            if fd:
                ...
                ret = _backend.lib.wimlib_write(
                    self._wim_struct, path, image, write_flags, threads)
        else:
    ...
    def extract_xml_data(self, file):
        raise NotImplementedError(
            "Error: wimlib functions with FILE* argument not supported yet")
    ...
    def split(self, swm_name, part_size, flags):
        ret = _backend.lib.wimlib_split(
            self._wim_struct, swm_name, part_size, write_flags)
        if ret:
            raise WIMError(ret)

    def set_output_pack_compression_type(self, compression_type):
        ret = _backend.lib.wimlib_set_output_pack_compression_type(
    ...
    def iterate_lookup_table(self, flags, callback, context):
        ...
        def callback_wrapper(resource_entry, user_context):
            ...
            return ret_val if ret_val is not None else 0
        context = _backend.ffi.new_handle(context)
        ret = _backend.lib.wimlib_iterate_lookup_table(
            self._wim_struct, flags, callback_wrapper, context)
        if ret:
            raise WIMError(ret)
