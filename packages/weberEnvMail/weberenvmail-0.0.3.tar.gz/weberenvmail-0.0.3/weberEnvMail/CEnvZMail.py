...
class CEnvZMail:
    def __init__(self, sFullEnvFN=None):
        # sFullEnvFN 全路径的env文件
        if sFullEnvFN:  # 若存在则加载内容到系统环境变量，否则直接从环境变量中取
            bLoad = load_dotenv(dotenv_path=sFullEnvFN, override=True)  # load environment variables from .env
            PrintTimeMsg(f"CEnvZMail.load_dotenv({sFullEnvFN})={bLoad}")
    ...
    def _get_passwd_by_acct(self, sMailAcct):
        ...
        if not sAcctPasswd:  # Linux环境变量不能有@符号
            sAcctPasswd = os.getenv('ENV_ZMAIL_PWD', '')
            if not sAcctPasswd:
