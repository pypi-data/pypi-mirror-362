...
function logline(line) {
  str = '';
  str += '<li style="list-style: none;margin: 5px 0px;">';
  tmps =  line.match(/\[([^\|]*)\|([^\|]*)\|([^\|]*)\|([^\]]*)\]\s(.*?)$/);
  if (tmps) {
    str += '<span class="text-info">' + tmps[1] + '</span>&nbsp;&nbsp;';
    if (tmps[2] == 'DEBUG') color = 'text-white';
    else if (tmps[2] == 'INFO') color = 'text-success';
    else if (tmps[2] == 'WARNING') color = 'text-warning';
    else color = 'text-danger';
    str += '<span class="' + color + '" style="display:inline-block; width:70px;">' + tmps[2] + '</span>';
