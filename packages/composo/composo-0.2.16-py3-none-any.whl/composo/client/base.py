...
class BaseClient(ABC):
    ...
    def _prepare_evaluation_request(
        ...
        # Convert system message to messages format if provided
        if final_system:
            ...
            if not final_messages or final_messages[0].get("role") != "system":
                ...
                final_messages.insert(0, {"role": "system", "content": final_system})
            else:
                # Replace existing system message
                final_messages[0] = {"role": "system", "content": final_system}

        # Filter out messages with any None fields to avoid API errors
        filtered_messages = []
        for msg in final_messages:
            clean_msg = {}
            for key, value in msg.items():
                if key == "content" and value is None:
                    continue  # 不发 content 字段
        ...
        # No longer need system_message field since it's now in messages
        return EvaluationRequest(
            messages=filtered_messages,
            system_message=None,  # Set to None since system is now in messages
            tools=final_tools,
