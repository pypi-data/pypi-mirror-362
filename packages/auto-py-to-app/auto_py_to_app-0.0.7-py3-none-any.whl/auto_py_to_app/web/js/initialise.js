...
const buildUpOptions = (providedOptions) => {
    return providedOptions.map(option => {
        ...
        if (placement === OPTION_IGNORED){
            inputType = null
        } else if (option.nargs === 0) {
            inputType = OPTION_INPUT_TYPE_SWITCH
        } else if (option.choices !== null) {
            inputType = OPTION_INPUT_TYPE_DROPDOWN
        } else if (options_inputTypeDoubleMultipleInput.indexOf(option.dest) !== -1) {
            inputType = OPTION_INPUT_TYPE_DOUBLE_MULTIPLE_INPUT
        } else if (option.default !== null || options_inputTypeMultipleInput.indexOf(option.dest) !== -1) {
...
window.addEventListener("load", async () => {
    ...
    if (document.getElementById('entry-target').value === '') {
        document.getElementById('entry-target').value = initialisationData.defaultOutputFolder;
        colourInput(document.getElementById('entry-target'),false,false,true)
    }
