...
def __setup_logging_ui_forwarding():
    """ Setup forwarding of logs by cx_Freeze and auto-py-to-app to the ui """
    cxfreeze_logger = logging.getLogger('cx_Freeze')
    ...
    handler.terminator = ""
    cxfreeze_logger.addHandler(handler)

    module_logger = logging.getLogger(__name__.split('.ui')[0]+'.packaging')
    handler = logging.StreamHandler(utils.ForwardToFunctionStream(send_message_to_ui_output))
...
def initialise():
    ...
    return {
        ...
        'options': packaging.get_cxfreeze_options(),
...
def will_packaging_overwrite_existing(file_path, output_folder, target_name):
    ...
    return packaging.will_packaging_overwrite_existing(file_path, output_folder, target_name)


@eel.expose
def package(command, non_cxfreeze_options):
    """ Package the script provided using the options selected by the user """
    packaging_options = {
        'increaseRecursionLimit': non_cxfreeze_options['increaseRecursionLimit'],
        'outputDirectory': non_cxfreeze_options['outputDirectory'],
    }

    packaging_successful = packaging.package(
        cxfreeze_command = command,
