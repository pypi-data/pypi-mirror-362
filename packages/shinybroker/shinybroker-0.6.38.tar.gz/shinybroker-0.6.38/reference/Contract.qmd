...
|-----------------|------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| contract_params | [dict](`dict`)               | Dictionary for which each named element defines a contract property. See IBKR's [Contract Management]( https://www.interactivebrokers.com/campus/ibkr-api-page/contracts/#contract-management ) documentation for more examples. | `None`    |
| contract_params | [str](`str`) or [int](`int`) | If you pass a single string or an integer, it will be interpreted as `conId` in the resultant `Contract` object.                                                                                                                 | `None`    |
...
|-----------|--------|-----------------------------------------------------------------------------------|
| attribute |        | All `Contract` objects possess one attribute for every valid IBKR contract field. |

## "Why not just use ticker"? {.doc-section .doc-section-why-not-just-use-ticker}
...
with `fetch_matching_symbols`. You
will quickly note
...
## Writing & Checking Contract Definitions {.doc-section .doc-section-writing--checking-contract-definitions}
...
## Contract ID is Sufficient On its Own {.doc-section .doc-section-contract-id-is-sufficient-on-its-own}
...
## Three Ways of Creating / Updating Contracts {.doc-section .doc-section-three-ways-of-creating--updating-contracts}
...
## Compact Printing {.doc-section .doc-section-compact-printing}
...
## Use Compact to Copy Contract Objects {.doc-section .doc-section-use-compact-to-copy-contract-objects}
...
## "So why not always specify contracts using contract ID only"? {.doc-section .doc-section-so-why-not-always-specify-contracts-using-contract-id-only}
