...
class SimpleRetryCore:
    ...
    def start_keyword(self, data, result):
        ...
        self.keyword_stack.append(normalized_name)
        # No pending-status logging to avoid unnecessary logs

    def end_keyword(self, data, result):
        normalized_name = self._normalize_keyword_name(data.name)

        # Muting keyword logic (e.g., Run Keyword And Ignore Error)
        if result.status == 'FAIL':
            muted_parent = next(
                (kw for kw in reversed(self.keyword_stack[:-1]) if kw in self.muting_keywords),
                None
            )
            if muted_parent:
                logging.info(f"[Muted] Failure in '{data.name}' suppressed (inside '{muted_parent}')")
                result.status = 'PASS'
                result.message = f"[Muted] Failure in '{data.name}' suppressed due to wrapper: '{muted_parent}'"

                if self.gui_controller and getattr(self.gui_controller, "gui_ready", False):
                    self.gui_controller.update_status(
                        f"Skipped retry for '{data.name}' (inside '{muted_parent}')", "gray"
                    )
                    self.gui_controller._update_failure_display(
                        text=f"'{data.name}' failed, but retry skipped (muted wrapper: '{muted_parent}')",
                        prefix="[Muted]",
                        status="pending"
                    )
        ...
        # Handle user-initiated test skip
        if self.skip_test:
            result.status = 'FAIL'
        ...
        # Handle failure with GUI debugger intervention
        if result.status == 'FAIL':
