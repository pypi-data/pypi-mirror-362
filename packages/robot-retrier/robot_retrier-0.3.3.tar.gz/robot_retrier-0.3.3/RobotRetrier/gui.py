...
class SimpleRetryGUI:
    ...
    def library_imported(self, name):
        """Handle library import event safely for both module and file-based libraries."""
        try:
            builtin = BuiltIn()
            instance = builtin.get_library_instance(name)

            if instance:
                # ✅ Use instance info instead of re-importing
                keywords = []
                for kw in dir(instance):
                    if callable(getattr(instance, kw)) and not kw.startswith("_"):
                        # Capture keyword name and arg spec (if possible)
                        func = getattr(instance, kw)
                        try:
                            argspec = str(inspect.signature(func))
                        except Exception:
                            argspec = "()"
                        keywords.append({"name": kw, "args": argspec, "doc": func.__doc__ or ""})

                self.libraries[name] = keywords

                if getattr(self, "executor_ready", False):
                    self._refresh_library_dropdown()
            else:
                logging.warning(f"[GUI] Library '{name}' not found in BuiltIn context. Skipping.")
        except Exception as e:
            logging.warning(f"[GUI] Could not process library '{name}': {e}")


        except ModuleNotFoundError:
            logging.warning(f"[Debugger GUI] Library '{name}' not found. Skipping.")
        except Exception as e:
