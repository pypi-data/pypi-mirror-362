...
class SimpleRetryCore:
    ...
    def end_keyword(self, data, result):
        ...
        if result.status == 'FAIL' and not self.retry_success:
            ...
            self.failed_stack_snapshot = deepcopy(self.keyword_stack)  # 🔥 capture full stack
            ...
            if self.gui_controller and getattr(self.gui_controller, "gui_ready", False):
                def show_and_wait():
                    self.gui_controller.show_failure(
                        ...
                        call_stack=self.failed_stack_snapshot  # 🎯 now pass full path to GUI
                    )
                ...
                while not self.continue_event.wait(timeout=0.1):
                    try:
