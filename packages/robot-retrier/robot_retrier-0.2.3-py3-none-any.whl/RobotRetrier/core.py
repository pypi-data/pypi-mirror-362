...
class SimpleRetryCore:
    ...
    def end_keyword(self, data, result):
        # Use the top of the stack as actual keyword context
        ...
        # Handle muted wrapper logic
        if result.status == 'FAIL':
            muted_parent = next(
                (kw.name for kw in reversed(self.keyword_stack[:-1]) if
                 self._normalize_keyword_name(kw.name) in self.muting_keywords),
                None
            )
            if muted_parent:
        ...
        # Handle failure that needs GUI intervention
        if result.status == 'FAIL':
            self.failed_keyword = current_kw  # ✅ use inner-most keyword
            if self.gui_controller and getattr(self.gui_controller, "gui_ready", False):
        ...
        # Refresh variable display if needed
        if self.gui_controller and getattr(self.gui_controller, "gui_ready", False):
            try:
