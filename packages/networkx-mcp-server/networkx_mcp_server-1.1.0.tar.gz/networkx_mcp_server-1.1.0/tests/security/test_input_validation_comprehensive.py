...
class TestSQLInjectionPrevention:
    ...
    def test_sql_injection_in_graph_name(self):
        ...
        for injection in sql_injections:
            ...
            assert "can only contain letters, numbers, underscores, and hyphens" in ' '.join(result.errors)
    ...
    def test_sql_injection_in_node_names(self):
        ...
        for injection in sql_injections:
            ...
            assert result.is_valid, f"Node ID validation unexpectedly failed for {injection}"
    ...
    def test_sql_injection_in_queries(self):
        ...
        for injection in sql_injections:
            ...
            result = shortest_path(graph_name="test_graph", source=injection, target="C")
            assert "error" in result, f"SQL injection {injection} should return error response"
    ...
    def test_nosql_injection_in_json_data(self):
        ...
        for injection in nosql_injections:
            ...
            if result.is_valid:
                ...
                assert "$" in str(injection), f"MongoDB operator test data should contain $ operators"
            else:
...
class TestCommandInjectionPrevention:
    ...
    def test_command_injection_in_file_paths(self):
        ...
        for injection in command_injections:
            ...
            assert not result.is_valid, f"Command injection {injection} should be rejected"
            assert "File path cannot contain" in ' '.join(result.errors) or "File extension not allowed" in ' '.join(result.errors)
...
class TestResourceExhaustionPrevention:
    ...
    def test_large_graph_creation_blocked(self):
        ...
        assert "error" in result, "Large node creation should be rejected by resource limits"
        assert ("exceeds" in result["error"].lower() or "limit" in result["error"].lower() 
               or "resource" in result["error"].lower() or "too many" in result["error"].lower()
    ...
    def test_memory_bomb_prevention(self):
        ...
        result = add_nodes(graph_name="test_graph", nodes=[("node1", {"data": huge_attr})])
        ...
        if "error" in result:
            ...
            assert ("exceeds" in result["error"].lower() or "limit" in result["error"].lower() 
                   or "resource" in result["error"].lower() or "size" in result["error"].lower()
                   or "too many" in result["error"].lower() or "maximum" in result["error"].lower())
        else:
            ...
            assert result["estimated_size_mb"] > 0, "Should report non-zero memory usage"
    ...
    def test_cpu_exhaustion_prevention(self):
        ...
        if "error" in result:
            ...
            assert "estimated_size_mb" in result, "Should track estimated memory usage"
            assert result["edges_added"] <= 5000, "Should not allow unreasonably large edge counts"
            assert result["estimated_size_mb"] > 0, "Should report non-zero memory usage"
...
class TestAuthenticationBypassPrevention:
    ...
    def test_admin_endpoint_protection(self):
        ...
        for token in invalid_tokens:
            ...
            assert isinstance(result, dict), f"Feature flags should return dict structure"
            ...
            if "error" not in result:
                ...
                assert "by_category" in result or "total_flags" in result, "Should have valid feature flags structure"
...
class TestInputSanitization:
    ...
    def test_null_byte_injection(self):
        ...
        for attack in null_byte_attacks:
            ...
            assert not result.is_valid, f"Null byte injection {attack} should be rejected"
            assert "can only contain letters, numbers, underscores, and hyphens" in ' '.join(result.errors)
...
class TestSecurityEdgeCases:
    ...
    def test_deeply_nested_attacks(self):
        ...
        if result.is_valid:
            ...
            assert "$where" in str(nested_attack), "Nested MongoDB operator should be preserved"
        else:
...
class TestFailSafeBehavior:
    ...
    def test_error_messages_are_safe(self):
        ...
        for attack, attack_type in attacks:
            ...
            assert ("Invalid" in error_msg or "forbidden" in error_msg or "not safe" in error_msg 
                   or "can only contain" in error_msg or "not allowed" in error_msg)
    ...
    def test_no_state_corruption_on_attack(self):
        ...
        attacks = [
            ...
            lambda: shortest_path(graph_name="valid_graph", source="A' OR '1'='1", target="B"),
        ]
