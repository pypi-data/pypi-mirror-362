...
ROADMAP = {
    "0.1.0": {
        "target_features": ["graph_operations", "stdio_transport", "networkx_integration", "basic_algorithms"],
        "status": "✅ Complete"
    },
    "0.2.0": {
    ...
    "1.0.0": {
        "target_features": ["monitoring", "rate_limiting", "data_validation", "error_recovery"],
        "status": "❌ Future"
    }
}
...
if __name__ == "__main__":
    ...
    print(f"Features: {summary['enabled_count']}/{summary['total_features']} ({summary['enabled_percentage']}%)")
    print()
    ...
    for version, info in ROADMAP.items():
        print(f"  {version}: {info['status']} - {len(info['target_features'])} features")