...
class MCPTestClient:
    ...
    async def call_server(self, method: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    async def initialize(self):
        ...
        response = await self.call_server("initialize", {
            "protocolVersion": "2024-11-05",
            "capabilities": {},
            "clientInfo": {"name": "e2e-test", "version": "1.0.0"}
        })
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = await self.call_server("tools/call", {
            "name": tool_name,
            "arguments": arguments
        })
...
async def test_social_network_analysis():
    ...
    result = await client.call_tool("add_nodes", {
    ...
    result = await client.call_tool("add_edges", {
        "graph_name": "social_network",
        "edges": friendships
    })
    ...
    print(f"   Most influential: {most_influential[0]} (betweenness: {most_influential[1]:.2f})")
    
    # 5. Find communities
    print("\n5. Detecting communities...")
    try:
        result = await client.call_tool("community_detection", {
            "graph_name": "social_network",
            "algorithm": "louvain"
        })
    ...
    for source, target in test_pairs:
        ...
        assert result["success"]
        print(f"   {source} → {target}: {' → '.join(result['path'])} (distance: {result['length']})")
    
    # 7. Get final statistics
    print("\n7. Final network statistics...")
    result = await client.call_tool("graph_info", {
        "graph_name": "social_network"
    })
    assert result["success"]
...
async def test_transportation_network():
    ...
    print("\n3. Adding roads...")
    roads = [
        ["Int_1", "Int_2"], ["Int_2", "Int_3"], ["Int_3", "Int_4"],
        ["Int_1", "Int_5"], ["Int_5", "Int_6"], ["Int_6", "Int_4"],
        ["Int_2", "Int_7"], ["Int_7", "Int_8"], ["Int_8", "Int_9"],
        ["Int_5", "Int_8"], ["Int_6", "Int_9"], ["Int_9", "Int_10"],
        ["Int_4", "Int_10"], ["Int_3", "Int_9"], ["Int_7", "Int_10"]
    ]
    ...
    if result.get("success"):
        if result["is_connected"]:
            ...
            print(f"   ⚠️  Network has {result['num_components']} disconnected components")
...
async def test_batch_operations():
    ...
    assert len(batch_response) == 4, "Should have 4 responses (no response for notification)"
...
async def test_performance_under_load():
    ...
    for i in range(10):
        if i % 3 == 0:
            ...
            coro = client.call_tool("centrality_measures", {
                "graph_name": "load_test",
                "measures": ["degree"]
            })
        tasks.append(coro)
