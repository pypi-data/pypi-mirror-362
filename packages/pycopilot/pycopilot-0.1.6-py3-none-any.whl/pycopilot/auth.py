...
class Authentication:
    ...
    def gh_get_user(self, token: str) -> GithubUserData:
        ...
        with httpx.Client() as client:
            response = client.get(GH_AUTH_TOKEN_URL, headers=headers)
            response.raise_for_status()
    ...
    def gh_copilot_authenticate(self, token: str) -> GithubCopilotAuth:
        ...
        with httpx.Client() as client:
            response = client.get(GH_COPILOT_INTERNAL_AUTH_URL, headers=headers)
            response.raise_for_status()
    ...
    def auth(self) -> GithubAuth:
        ...
        try:
            self.get_token()
            print(f"An existing token was found in `{CONFIG_DIR / 'token.json'}`")
            if not Confirm.ask(
                "Do you want to override it with a new token?", default=False
            ):
                print("Using existing token.")
                return self.cache_auth()
        except FileNotFoundError:
            pass  # No existing token, proceed with new authentication
