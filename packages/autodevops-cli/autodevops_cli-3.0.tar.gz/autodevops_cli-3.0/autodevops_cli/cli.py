...
def run_pipeline(commit_message):
    ...
    # replace with actual image name/tag
...
def trigger_workflow():
    ...
    print("✅ Workflow triggered." if res.status_code ==
          204 else f"❌ Trigger failed: {res.text}")
...
def download_and_extract_logs(run_id):
    ...
    if r.status_code == 200:
        ...
        print(f"❌ Failed to download logs: {r.text}")
        exit(1)
