...
@click.command()
@click.option('--setup', is_flag=True, help="Run project analysis setup")
@click.argument('folder_path', required=False)
def main(setup, folder_path):
    """AutoDevOps Setup: Analyze project structure and generate docs"""
    if setup and folder_path:
        run_analysis(folder_path, max_files=500)
    else:
        console.print(
            "[red]❗ Usage: autodevops-setup --setup /path/to/project[/red]")
