...
logger.add(LOG_DIR / f"autodevops_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log", rotation="1 MB")
...
def run_analysis(folder_path, max_files=500):
    ...
    structure_analysis = gpt_call("Structure Analysis", "Analyze the folder structure and suggest improvements.", summary)
    save_output("structure_analysis.md", structure_analysis, folder_path)
    tech_stack = gpt_call("Tech Stack", "Detect the tech stack from this project structure.", summary)
    save_output("tech_stack.md", tech_stack, folder_path)
    readme = gpt_call("README.md", "Generate a README.md file for this project.", summary)
...
def scan_directory_structure(folder_path, max_files=500):
    ...
    summary += ["\nTop folders by file count:"] + [f"  {folder}/: {count} files" for folder, count in sorted(folder_count.items(), key=lambda x: -x[1])[:10]]
    summary += ["\nSample files:"] + [f"  {file}" for file in important_files[:25]]
    return summary
...
def copy_required_files(folder_path):
    ...
    def safe_copy(src_file, dest_file, label):
        try:
            ...
            console.log(f"[yellow]⚠️ {label} not found in {REQUIREMENTS_DIR}, skipping...[/yellow]")

    safe_copy(REQUIREMENTS_DIR / "Dockerfile", target_path / "Dockerfile", "Dockerfile")
    safe_copy(REQUIREMENTS_DIR / "test_main.py", target_path / "test_main.py", "test_main.py")

    workflow_file = target_path / ".github/workflows/main.yml"
    if not workflow_file.exists():
        workflow_file.parent.mkdir(parents=True, exist_ok=True)
        safe_copy(REQUIREMENTS_DIR / "main.yml", workflow_file, "GitHub Actions workflow")
