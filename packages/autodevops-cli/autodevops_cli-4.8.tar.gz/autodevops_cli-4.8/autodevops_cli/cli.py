...
REQUIREMENTS_DIR = BASE_DIR  # No separate folder, all files nearby
...
def save_output(filename, content, folder_path, copy_files=False):
    ...
    if copy_files:
        copy_required_files(folder_path)  # Now correctly passed


def copy_required_files(folder_path):  # ✅ Fixed: Now accepts an argument
    # Source: current CLI tool directory
    ...
    def safe_copy(src_file, dest_file, label):
        try:
            ...
            console.log(
                f"[yellow]⚠️ {label} not found at {src_file}, skipping...[/yellow]")

    # Perform copies
    safe_copy(current_dir / "Dockerfile",
              docker_target / "Dockerfile", "Dockerfile")
    safe_copy(current_dir / "test_main.py", test_file_target, "test_main.py")
    safe_copy(current_dir / "main.yml", github_workflow_target, "main.yml")
