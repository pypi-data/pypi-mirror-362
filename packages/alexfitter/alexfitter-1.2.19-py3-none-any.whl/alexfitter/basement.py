...
#                print('Using ns_sample=="slice".')
    ...
    def load_data(self):
        ...
        for inst in self.settings['inst_phot']:
            try:
                time, flux, flux_err, custom_series = np.genfromtxt(os.path.join(self.datadir,inst+'.csv'), delimiter=',', dtype=float, unpack=True, skip_header=1)[0:4]
            except:
                time, flux, flux_err = np.genfromtxt(os.path.join(self.datadir,inst+'.csv'), delimiter=',', dtype=float, unpack=True, skip_header=1)[0:3]
...
#                    raise ValueError('User aborted operation.')
        ...
        for inst in self.settings['inst_rv']:
            try:
                time, rv, rv_err, custom_series = np.genfromtxt( os.path.join(self.datadir,inst+'.csv'), delimiter=',', dtype=float, unpack=True, skip_header=1)[0:4]
            except:
                time, rv, rv_err = np.genfromtxt( os.path.join(self.datadir,inst+'.csv'), delimiter=',', dtype=float, unpack=True, skip_header=1)[0:3]
                custom_series = np.zeros_like(time)
