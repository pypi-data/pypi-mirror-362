...
pub(crate) fn isolate_subcircuit_np(
    edge_array: PyReadonlyArray2<u64>,
    sources: HashSet<u64>,
    sinks: HashSet<u64>,
    max_depth: usize,
    max_paths: usize,
    nid_to_cidx_map: Option<HashMap<u64, u64>>,
) -> HashSet<CMatIdx> {
    ...
    for row in edge_array.outer_iter() {
        if row.len() >= 2 {
            let source = row[0] as u64;
            let target = row[1] as u64;
...
pub(crate) fn adjacency_map_from_edge_list_np(
    edge_array: PyReadonlyArray2<u64>,
