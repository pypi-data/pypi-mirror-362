...
#ifdef _WIN32
    #if defined(genericsubtractor_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define GENERICSUBTRACTOR_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define GENERICSUBTRACTOR_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define GENERICSUBTRACTOR_WINDLL
#endif
