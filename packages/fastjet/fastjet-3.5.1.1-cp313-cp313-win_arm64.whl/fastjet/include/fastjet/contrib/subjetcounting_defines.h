...
#ifdef _WIN32
    #if defined(subjetcounting_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define SUBJETCOUNTING_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define SUBJETCOUNTING_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define SUBJETCOUNTING_WINDLL
#endif
