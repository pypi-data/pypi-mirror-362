...
#ifdef _WIN32
    #if defined(centauro_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define CENTAURO_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define CENTAURO_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define CENTAURO_WINDLL
#endif
