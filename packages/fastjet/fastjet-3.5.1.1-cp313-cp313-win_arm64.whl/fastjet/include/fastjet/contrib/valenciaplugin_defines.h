...
#ifdef _WIN32
    #if defined(valenciaplugin_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define VALENCIAPLUGIN_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define VALENCIAPLUGIN_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define VALENCIAPLUGIN_WINDLL
#endif
