...
#ifdef _WIN32
    #if defined(jetcleanser_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define JETCLEANSER_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define JETCLEANSER_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define JETCLEANSER_WINDLL
#endif
