...
#ifdef _WIN32
    #if defined(cmpplugin_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define CMPPLUGIN_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define CMPPLUGIN_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define CMPPLUGIN_WINDLL
#endif
