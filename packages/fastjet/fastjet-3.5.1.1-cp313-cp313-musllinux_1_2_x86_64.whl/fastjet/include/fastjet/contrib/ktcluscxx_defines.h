...
#ifdef _WIN32
    #if defined(ktcluscxx_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define KTCLUSCXX_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define KTCLUSCXX_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define KTCLUSCXX_WINDLL
#endif
