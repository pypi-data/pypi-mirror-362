...
#ifdef _WIN32
    #if defined(variabler_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define VARIABLER_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define VARIABLER_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define VARIABLER_WINDLL
#endif
