...
if test -z ${srcdir}; then
    srcdir="${BASH_SOURCE%/*}"
    echo "setting srcdir to $srcdir"
fi

# check if we have found the relevant file
if test -e ${srcdir}/example/data/single-event.dat ; then
    echo "Checked srcdir = $srcdir contains example/data/single-event.dat"
else
    echo "ERROR: ${srcdir}/example/data/single-event.dat not found"
    exit 1
fi
...
# build the output to be compared with the original one
