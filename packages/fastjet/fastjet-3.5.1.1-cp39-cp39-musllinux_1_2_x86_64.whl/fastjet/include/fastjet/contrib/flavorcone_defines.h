...
#ifdef _WIN32
    #if defined(flavorcone_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define FLAVORCONE_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define FLAVORCONE_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define FLAVORCONE_WINDLL
#endif
