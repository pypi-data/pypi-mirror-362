...
#ifdef _WIN32
    #if defined(energycorrelator_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define ENERGYCORRELATOR_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define ENERGYCORRELATOR_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define ENERGYCORRELATOR_WINDLL
#endif
