...
#ifdef _WIN32
    #if defined(qcdawareplugin_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define QCDAWAREPLUGIN_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define QCDAWAREPLUGIN_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define QCDAWAREPLUGIN_WINDLL
#endif
