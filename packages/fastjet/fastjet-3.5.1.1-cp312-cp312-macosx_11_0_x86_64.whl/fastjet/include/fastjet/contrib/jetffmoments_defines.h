...
#ifdef _WIN32
    #if defined(jetffmoments_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define JETFFMOMENTS_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define JETFFMOMENTS_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define JETFFMOMENTS_WINDLL
#endif
