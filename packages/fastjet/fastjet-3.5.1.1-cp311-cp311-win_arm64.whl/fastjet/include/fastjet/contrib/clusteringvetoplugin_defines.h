...
#ifdef _WIN32
    #if defined(clusteringvetoplugin_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define CLUSTERINGVETOPLUGIN_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define CLUSTERINGVETOPLUGIN_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define CLUSTERINGVETOPLUGIN_WINDLL
#endif
