...
#ifdef _WIN32
    #if defined(nsubjettiness_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define NSUBJETTINESS_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define NSUBJETTINESS_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define NSUBJETTINESS_WINDLL
#endif
