...
#ifdef _WIN32
    #if defined(recursivetools_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define RECURSIVETOOLS_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define RECURSIVETOOLS_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define RECURSIVETOOLS_WINDLL
#endif
