...
#ifdef _WIN32
    #if defined(constituentsubtractor_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define CONSTITUENTSUBTRACTOR_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define CONSTITUENTSUBTRACTOR_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define CONSTITUENTSUBTRACTOR_WINDLL
#endif
