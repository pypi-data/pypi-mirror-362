...
#ifdef _WIN32
    #if defined(signalfreebackgroundestimator_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define SIGNALFREEBACKGROUNDESTIMATOR_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define SIGNALFREEBACKGROUNDESTIMATOR_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define SIGNALFREEBACKGROUNDESTIMATOR_WINDLL
#endif
