...
#ifdef _WIN32
    #if defined(sdfplugin_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define SDFPLUGIN_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define SDFPLUGIN_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define SDFPLUGIN_WINDLL
#endif
