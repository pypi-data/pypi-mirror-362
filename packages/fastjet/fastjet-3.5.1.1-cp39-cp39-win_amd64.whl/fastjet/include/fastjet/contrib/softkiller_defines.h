...
#ifdef _WIN32
    #if defined(softkiller_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define SOFTKILLER_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define SOFTKILLER_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define SOFTKILLER_WINDLL
#endif
