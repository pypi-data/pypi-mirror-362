...
#ifdef _WIN32
    #if defined(lundplane_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define LUNDPLANE_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define LUNDPLANE_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define LUNDPLANE_WINDLL
#endif
