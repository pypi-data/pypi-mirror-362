...
#ifdef _WIN32
    #if defined(ghsalgo_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define GHSALGO_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define GHSALGO_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define GHSALGO_WINDLL
#endif
