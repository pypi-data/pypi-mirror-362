...
#ifdef _WIN32
    #if defined(ifnplugin_EXPORTS) || defined(fastjetcontrib_EXPORTS)
        #define IFNPLUGIN_WINDLL __declspec(dllexport) // Export when building the DLL
    #else
        #define IFNPLUGIN_WINDLL __declspec(dllimport) // Import when using the DLL
    #endif
#else
    // For Linux/macOS
    #define IFNPLUGIN_WINDLL
#endif
