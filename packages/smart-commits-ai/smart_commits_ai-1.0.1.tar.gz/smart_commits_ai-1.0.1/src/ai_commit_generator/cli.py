...
def print_banner():
    ...
    banner.append("Automatically generate conventional commit messages using AI", style="dim")
...
@main.command()
@click.option('--force', '-f', is_flag=True, help='Overwrite existing hook without confirmation')
...
def install(force: bool, config: bool):
    ...
    console.print("   • [green]Groq[/green] (recommended): https://console.groq.com/keys")
    ...
    console.print("   [cyan]echo 'test' > test.txt && git add test.txt && git commit[/cyan]")
...
@click.option('--dry-run', is_flag=True, help='Generate message without writing to file')
@handle_errors
def generate(output: Optional[str], dry_run: bool):
    ...
    try:
        ...
        if message:
            console.print(f"[green]✅ Generated message:[/green] [blue]{message}[/blue]")
            if dry_run:
                console.print("[dim]Note: This was a dry run. No files were modified.[/dim]")
        else:
            console.print("[yellow]⚠️  No staged changes found or merge commit detected[/yellow]")
...
def status(verbose: bool):
    ...
    try:
        ...
        try:
            cfg.api_key
            console.print(f"[green]✅ API key configured[/green] (provider: {cfg.provider})")
        except ConfigError:
            console.print(f"[red]❌ API key not configured[/red] (provider: {cfg.provider})")
