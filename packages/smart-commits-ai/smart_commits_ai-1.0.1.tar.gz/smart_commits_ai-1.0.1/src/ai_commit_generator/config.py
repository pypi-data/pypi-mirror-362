...
class Config:
    ...
    DEFAULT_CONFIG = {
        ...
        "processing": {
            "max_diff_size": 8000,
            "exclude_patterns": ["*.lock", "*.log", "node_modules/*", ".git/*", "dist/*", "build/*", "*.min.js", "*.min.css"],
            "truncate_files": True,
            "max_file_lines": 100
        },
        "fallback": {
    ...
    def api_key(self) -> str:
        ...
        if not api_key:
            raise ConfigError(f"API key not found. Please set {env_var} in your .env file")
    ...
    def debug_enabled(self) -> bool:
        ...
        return self._config["debug"]["enabled"] or os.getenv("DEBUG_ENABLED", "").lower() == "true"
...
Respond with ONLY the commit message, no explanations or additional text."""
    ...
    def validate(self) -> None:
        ...
        if self.provider not in valid_providers:
            raise ConfigError(f"Invalid provider '{self.provider}'. Must be one of: {valid_providers}")
