...
from .api_clients import create_client, APIError
...
class CommitGenerator:
    ...
    def _process_diff(self, diff: str) -> str:
        ...
        if len(filtered_diff) > self.config.max_diff_size:
            logger.debug(f"Diff size ({len(filtered_diff)}) exceeds limit ({self.config.max_diff_size}), truncating")
            filtered_diff = filtered_diff[:self.config.max_diff_size] + "\n... [truncated]"
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if self._validate_message(cleaned_message):
                    ...
                    logger.warning(f"Generated message failed validation: {cleaned_message}")
