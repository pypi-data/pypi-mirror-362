...
from .fluids import advect_density
...
class EPINFTrainer(lightning.LightningModule):
    ...
    class EPINF:
        ...
        export_houdini: bool = dataclasses.field(default=False, metadata={'help': 'whether to export fields for Houdini'})
        ...
        resim: bool = dataclasses.field(default=False, metadata={'help': 'whether to resimulate the fields'})
    ...
    def on_fit_start(self):
        ...
        rich.print(f"[bold green]>>> [Custom]: Renderer resolution set to {dataset.width}x{dataset.height} <<<[/bold green]")
        self.renderer.set_min_near_far(min_near=dataset.near_std, max_far=dataset.far_std)
        rich.print(f"[bold green]>>> [Custom]: Near far std set to {dataset.near_std} - {dataset.far_std} <<<[/bold green]")
    ...
    def training_step(self, batch, batch_idx):
        ...
        for plugin_loss in filter(lambda v: isinstance(v, PluginLoss), self.renderer.plugins):
            p_loss = plugin_loss.weight * plugin_loss.loss
            loss += p_loss
            self.log(f"{plugin_loss.name}", p_loss, on_step=True, on_epoch=False, prog_bar=True)

        if 'rgb_map_static_independent' in result_maps:
            tempo_fading = min(max(self.global_step / self.cfg.fading_step, 0.0), 1.0)
            img_loss_static = 10000 * torch.nn.functional.mse_loss(result_maps['rgb_map_static_independent'], pixels[..., :3])
    ...
    def on_test_end(self):
        ...
        rich.print(f"[bold green]✅ Test video saved at[/bold green] [cyan]{video_path}[/cyan] with {len(frames)} frames")
    ...
    def generate_map(result_maps, pixels, pixels_highlighted, pixels_mask):
        ...
        pixels_highlighted_column = torch.cat([pixel for pixel in pixels_highlighted[..., :3]], dim=1) if pixels_highlighted is not None else torch.zeros_like(rgb_map_column)
        ...
        if N > 1:
            left_side = torch.cat([pixels_highlighted_column, pixels_column, pixels_mask_column.expand_as(pixels_column)], dim=0)
        else:
            left_side = torch.cat([pixels_column, pixels_highlighted_column], dim=1)
    ...
    def resimulate_explicit_fields(self, output_dir, hython_path, total_frames=120):
        ...
        base_res = 128
        resx, resy, resz = int(base_res * weightx), int(base_res * weighty), int(base_res * weightz)
        ...
        source_height = 0.15
        gt_mask = xyz[..., 1] < source_height
        xyz = xyz.to(torch.float16 if self.cfg.dataset.fp16 else torch.float32).to(self.device)
        mask_inside = mask_inside.to(self.device)
        gt_mask = gt_mask.reshape(resx, resy, resz).to(self.device)



        output_dir = os.path.join(output_dir, 'resimulate_fields')
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda', enabled=True):
                ...
                with rich.progress.Progress() as progress:
                    task = progress.add_task("[green]Resimulating...", total=total_frames)
                    dt = 1.0 / float(total_frames - 1)

                    self.model.set_time(time=torch.tensor([0], device=xyz.device, dtype=xyz.dtype))
                    den_sim_t_1 = self.sample_sigma(xyz=xyz, mask=mask_inside).reshape(resx, resy, resz, 1)
                    for frame_idx in range(total_frames):
                        if frame_idx > 0:
                            self.model.set_time(time=torch.tensor([frame_idx - 1 / total_frames], device=xyz.device, dtype=xyz.dtype))
                            vel_sim_t_1 = self.sample_velocity(xyz=xyz, mask=mask_inside).reshape(resx, resy, resz, 3)
                            self.model.set_time(time=torch.tensor([frame_idx / total_frames], device=xyz.device, dtype=xyz.dtype))
                            den_vis_t = self.sample_sigma(xyz=xyz, mask=mask_inside).reshape(resx, resy, resz, 1)
                            den_sim_t = advect_density(den=den_sim_t_1, vel=vel_sim_t_1, coord_3d_sim=xyz.reshape(resx, resy, resz, 3), dt=dt, gt=den_vis_t, gt_mask=gt_mask)
                        else:
                            den_sim_t = den_sim_t_1
                        sigmas_resimulated.append(den_sim_t.detach().cpu().clone())
                        den_sim_t_1 = den_sim_t
                        progress.update(task, advance=1)
                if hython_path is not None:
                    cpu_cores = max(os.cpu_count() - 3, 4)
                    rich.print(f"[bold green][Using multiprocessing with {cpu_cores} cores][/bold green], exporting resimulated fields to Houdini...")
                    with multiprocessing.Pool(processes=cpu_cores) as pool:
                        tasks = [
                            (hython_path, sigma, aabb_std, output_dir, frame_idx)
                            for frame_idx, sigma in enumerate(sigmas_resimulated)
                        ]
                        pool.starmap(self.export_density_houdini, tasks)
                else:
                    for frame_idx, sigma in enumerate(sigmas_resimulated):
                        torch.save(sigma, os.path.join(output_dir, f'sigma_{frame_idx:03d}.pt'))
                        rich.print(f"[bold green]✅ Simulated density field saved at[/bold green] [cyan]{os.path.join(output_dir, f'sigma_{frame_idx:03d}.pt')}[/cyan] with shape [magenta]{sigma.shape}[/magenta]")

        if hython_path is not None:
            gplay_exe = os.path.join(os.path.dirname(hython_path), 'gplay.exe')
            rich.print(f"[bold green]Opening gplay[/bold green]")
            subprocess.run([gplay_exe], check=True, cwd=f"{output_dir}")

    def export_explicit_fields(self, output_dir, hython_path, total_frames=120):
        aabb_std = self.renderer.aabb_std.cpu()  # [low_x, low_y, low_z, high_x, high_y, high_z]
        bound_std = self.renderer.bound_std.cpu()  # [low_x, low_y, low_z, high_x, high_y, high_z]
        ...
        output_dir = os.path.join(output_dir, 'explicit_fields')
        os.makedirs(output_dir, exist_ok=True)
        with torch.no_grad():
            with torch.amp.autocast('cuda', enabled=True):
                self.eval()
                ...
                velocities = []
                with rich.progress.Progress() as progress:
                    task = progress.add_task("[green]Generating fields...", total=total_frames)
                    for frame_idx in range(total_frames):
                        self.model.set_time(time=torch.tensor([frame_idx / total_frames], device=xyz.device, dtype=xyz.dtype))
                        sigma = self.sample_sigma(xyz=xyz, mask=mask_inside).reshape(resx, resy, resz, 1)
                        velocity = self.sample_velocity(xyz=xyz, mask=mask_inside).reshape(resx, resy, resz, 3)
                        sigmas.append(sigma.cpu())
                        velocities.append(velocity.cpu())
                        progress.update(task, advance=1)
                if hython_path is not None:
                    cpu_cores = max(os.cpu_count() - 3, 4)
                    rich.print(f"[bold green][Using multiprocessing with {cpu_cores} cores][/bold green], exporting fields to Houdini...")
                    with multiprocessing.Pool(processes=cpu_cores) as pool:
                        tasks = [
                            (hython_path, sigma, aabb_std, output_dir, frame_idx)
                            for frame_idx, sigma in enumerate(sigmas)
                        ]
                        pool.starmap(self.export_density_houdini, tasks)
                    with multiprocessing.Pool(processes=cpu_cores) as pool:
                        tasks = [
                            (hython_path, velocity, aabb_std, output_dir, frame_idx)
                            for frame_idx, velocity in enumerate(velocities)
                        ]
                        pool.starmap(self.export_velocity_houdini, tasks)
                else:
                    for frame_idx, (sigma, velocity) in enumerate(zip(sigmas, velocities)):
                        torch.save(sigma, os.path.join(output_dir, f'sigma_{frame_idx:03d}.pt'))
                        rich.print(f"[bold green]✅ Density field saved at[/bold green] [cyan]{os.path.join(output_dir, f'sigma_{frame_idx:03d}.pt')}[/cyan] with shape [magenta]{sigma.shape}[/magenta]")
                        torch.save(velocity, os.path.join(output_dir, f'velocity_{frame_idx:03d}.pt'))
                        rich.print(f"[bold green]✅ Velocity field saved at[/bold green] [cyan]{os.path.join(output_dir, f'velocity_{frame_idx:03d}.pt')}[/cyan] with shape [magenta]{velocity.shape}[/magenta]")
    ...
    def sample_sigma(self, xyz, mask, chunk: int = 1024 * 8):
        sigma = []
        for start_idx in range(0, xyz.shape[0], chunk):
            ret = self.model.sigma(xyz[start_idx:start_idx + chunk])
            _sigma = ret[0]
            sigma.append(_sigma.detach())
        sigma = torch.cat(sigma, dim=0)
        if mask is not None:
            sigma[~mask] = 0.0
        return sigma

    def sample_velocity(self, xyz, mask, chunk: int = 1024 * 8):
        velocity = []
        for start_idx in range(0, xyz.shape[0], chunk):
            ret = self.model.vel(xyz[start_idx:start_idx + chunk])
            _velocity = ret
            velocity.append(_velocity.detach())
    ...
    @staticmethod
    def export_density_houdini(hython_path, sigma, aabb_std, output_dir, frame_idx):
        low_x, low_y, low_z, high_x, high_y, high_z = aabb_std[0].item(), aabb_std[1].item(), aabb_std[2].item(), aabb_std[3].item(), aabb_std[4].item(), aabb_std[5].item()
        with tempfile.NamedTemporaryFile(suffix=".npy", delete=False) as f:
            temp_file = f.name
        np.save(temp_file, sigma.numpy())
        out_file = os.path.join(f"{output_dir}", f"sigma_{frame_idx:03d}.bgeo.sc")
        houdini_code = f'''
                        import hou
                        import numpy as np
                        import os
                        ...
                        vol = geo.createVolume(resx, resy, resz, hou.BoundingBox(float({low_x}), float({low_y}), float({low_z}), float({high_x}), float({high_y}), float({high_z})))
                        vol.setAttribValue(name_attrib, "density")
                        vol.setAllVoxels(nparray.flatten().tolist())
                        geo.saveToFile(r"{out_file}")
                        '''
        try:
            EPINFTrainer.execute_template_script(hython_path=hython_path, script_content=houdini_code)
            rich.print(f"[bold green]✅ Exported[/bold green] [cyan]{out_file}[/cyan] with shape [magenta]{sigma.shape}[/magenta]")
    ...
    @staticmethod
    def export_velocity_houdini(hython_path, velocity, aabb_std, output_dir, frame_idx):
        low_x, low_y, low_z, high_x, high_y, high_z = aabb_std[0].item(), aabb_std[1].item(), aabb_std[2].item(), aabb_std[3].item(), aabb_std[4].item(), aabb_std[5].item()
        with tempfile.NamedTemporaryFile(suffix=".npy", delete=False) as f:
            temp_file = f.name
        np.save(temp_file, velocity.numpy())
        out_file = os.path.join(f"{output_dir}", f"velocity_{frame_idx:03d}.bgeo.sc")
        houdini_code = f'''
                        import hou
                        import numpy as np
                        import os
                        ...
                        for i, name in enumerate(['vel.x', 'vel.y', 'vel.z']):
                            ...
                            vol.setAllVoxels(nparray[..., i].flatten().tolist())
                        geo.saveToFile(r"{out_file}")
                        '''
        try:
            EPINFTrainer.execute_template_script(hython_path=hython_path, script_content=houdini_code)
            rich.print(f"[bold green]✅ Exported[/bold green] [cyan]{out_file}[/cyan] with shape [magenta]{velocity.shape}[/magenta]")
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"Failed to execute Houdini script: {e}") from e
        finally:
            os.remove(temp_file)
    ...
    @staticmethod
    def execute_template_script(hython_path, script_content):
