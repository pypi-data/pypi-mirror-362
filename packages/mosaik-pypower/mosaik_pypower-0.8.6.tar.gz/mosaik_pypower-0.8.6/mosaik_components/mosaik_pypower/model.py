...
from mosaik_components.mosaik_pypower import resource_db as rdb
...
def load_case(path, grid_idx, sheetnames):
    ...
    branches = _get_branches(loader, raw_case, entity_map, grid_idx, sheetnames)
...
def make_eid(name: str, grid_idx):
...
def _get_buses(loader, raw_case, entity_map, grid_idx, sheetnames):
    buses = []
    for idx, (bid, btype, base_kv) in enumerate(loader.buses(raw_case, sheetnames)):
...
def _get_branches(loader, raw_case, entity_map, grid_idx, sheetnames):
    branches = []
    for idx, branch in enumerate(loader.branches(raw_case, entity_map, sheetnames)):
...
def _make_ppc(base_mva, bus_data, branch_data):
    ...
    branches = []
    for f, t, l_var, r, x, b, s_max, online, tap in branch_data:
...
class UniqueKeyDict(Dict[str, Any]):
...
class JSON:
    ...
    def branches(raw_case, entity_map, sheetnames):
        if "base_mva" in raw_case:
            ...
            for bid, fbus, tbus, l_var, r, x, c, i_max in raw_case["branch"]:
                info = (r, x, c, i_max)
                yield (False, bid, fbus, tbus, l_var, info, 1, 0)
            ...
            for bid, fbus, tbus, btype, l_var, online in raw_case["branch"]:
                line = lines[btype]
                yield (False, bid, fbus, tbus, l_var, line, int(online), 0)
...
class Excel:
    ...
    def branches(wb, entity_map, sheetnames):
        ...
        for bid, fbus, tbus, btype, l_var, online, tap in Excel._iter(sheet, 7):
            ...
            try:
                ...
                info = lines[btype]
                is_trafo = False
            yield (is_trafo, bid, fbus, tbus, l_var, info, online, tap)
