...
class SemanticKernelOrchestrator:
    ...
    def orchestrate(
        self, user_message: str, chat_history: list[dict], user_info, tenant_id: str, config, **kwargs: dict
    ) -> list[dict]:
        """
        ...
        self.kernel.add_plugin(
            plugin=ChatPlugin(question=user_message, chat_history=chat_history, tenant_id=tenant_id),
            plugin_name="Chat",
        )
        filters.append("Chat")
    ...
    def handle_message(
        ...
        **kwargs: Optional[dict],
        ...
        """
        Handles the user's message, orchestrates the conversation, and logs the interaction.

        Args:
            user_message (str): The message from the user.
            chat_history (List[dict]): The history of the chat.
        ...
        Returns:
            dict: The response to be sent to the user.
        """
    ...
    def call_content_safety_output(
        ...
        """
        Validates the answer for harmful content and replaces it if necessary.
