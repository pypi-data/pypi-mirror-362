...
class AzurePostgresHandler(SearchHandlerBase):
    ...
    def perform_search(self, filename, tenant_id: str = None):
        logger.info(f"Performing search for filename: {filename}")
        return self.azure_postgres_helper.perform_search(filename, tenant_id)
    ...
    def get_files(self, tenant_id: str = None):
        results = self.azure_postgres_helper.get_files(tenant_id)
    ...
    def delete_files(self, files, tenant_id: str = None):
        ...
        self.azure_postgres_helper.delete_documents(ids_to_delete, tenant_id)
    ...
    def get_unique_files(self, tenant_id: str = None):
        results = self.azure_postgres_helper.get_unique_files(tenant_id)
