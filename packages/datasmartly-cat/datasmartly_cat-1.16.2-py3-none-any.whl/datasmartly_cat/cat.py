...
def _initialize_cat_session_if_needed(logging_level = LoggingLevel.INFORMATION):
    """Private function, should not be called byt consumers.
    
    Initializes CAT session, if it was not already initialized. Sets the .NET runtime,
    loads types from JC.Cat.Core namespace and creates a new CAT session if needed.
    """
    global cat_session, cat_was_initialized
    if cat_was_initialized == False:
        set_runtime('coreclr') # set runtime to coreclr
        current_file_directory = pathlib.Path(__file__).parent.resolve()
        dllPath = os.path.join(current_file_directory, 'bin', 'JC.Cat.Core.dll')
