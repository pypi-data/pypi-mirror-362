...
class FreeGpt:
    def gemini(self, prompt: str = None) -> str:
        '''Генерация текста с помощью Gemini-1.5-flash'''
        try:   
            if prompt is None:
    ...
    def deepchat(self, messages: dict = None, model: str = "Qwen/Qwen3-235B-A22B", temperature: float = 0.7, max_tokens: int = None) -> str:
        ...
        try:
            ...
            response = requests.post("https://api.deepinfra.com/v1/openai/chat/completions", json=payload, timeout=(80, 360))
            if response.status_code == 200:
    ...
    def llm7_ai(messages: dict = None, model: str = "gpt-4o") -> str:
        try:
            if messages is None:
                raise ValueError("Забыли указать messages")
    ...
    def only_sq(messages: dict = None, model: str = "gpt-4o") -> str:
        '''Генерация ответа с использованием https://docs.onlysq.ru/'''
        try:
            if messages is None:
                raise ValueError("Забыли указать messages")
            response = requests.post('https://api.onlysq.ru/ai/v2', json={"model": model, "request": {"messages": messages}})
            if response.status_code == 200:
                return response.json()["choices"][0]["message"]["content"]
            else:
                return "Error"
        except Exception as e:
            print(f"FreeAI(only_sq): {e}")
            return "Error"
