                        ...
                        if grauteur is not None:
                            ...
                            for iauteur, auteur in enumerate(grauteur.iter("auteur")):
                                ...
                                if len(prenom_nom) > 0:
                                    ...
                                    auteurs[order] = (auteur, id_auteur)
                            if len(auteurs) > 0:
                                sorted_auteurs = dict(sorted(auteurs.items()))
                                lauteurs, lid_auteurs = zip(*sorted_auteurs.values())
                                metadata["auteur"] = list(lauteurs)
                                metadata["id_auteur"] = list(lid_auteurs)
                                metadata["auteur_12"] = list(lauteurs)[:2]
                                metadata["id_auteur_12"] = list(lid_auteurs)[:2]
