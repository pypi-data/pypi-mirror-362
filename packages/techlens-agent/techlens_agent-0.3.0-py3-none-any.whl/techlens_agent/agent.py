...
class Agent:
    def __init__(self):
        ...
        try:
            self.version = meta_version("techlens-agent")
        except PackageNotFoundError:
            self.version = "dev-build"
    ...
    def parseRepo(self, path: str, repo_name: str, branch: str = None):
        ...
        if self.config.runStats:
            ...
            if not self.config.dry:
                self.log(
                    msg=repo_name,
                    tag="Analyzing repository with techlens-metrics",
                    display=True,
                )
                ...
                techlens_metrics(custom_args=custom_args, license_identifier=None)
            ...
            try:
                ...
                if provider.provider == "aws" and self.checkDependency(
                    ...
                    if aws_cost_file is None:
                        ...
                        self.upload(file=aws_cost_file, route="costs", source="AWS")
                    ...
                    if aws_instance_file is None:
                        ...
                        self.upload(
                            file=aws_instance_file, route="instances", source="AWS"
                        )
                    ...
                    if aws_block_file is None:
                        ...
                        self.upload(file=aws_block_file, route="storage", source="AWS")
                ...
                if provider.provider == "azure" and self.checkDependency(
                    ...
                    if azure_cost_file is None:
                        self.log(
                            msg="Error processing Azure costs", tag=provider.account
                        )
                    else:
                        self.log(msg=azure_cost_file, tag="Azure Costs")
                        self.upload(file=azure_cost_file, route="costs", source="Azure")
                    ...
                    if azure_instance_file is None:
                        self.log(
                            msg="Error processing Azure instances", tag=provider.account
                        )
                    else:
                    ...
                    if azure_block_file is None:
                        self.log(
                            msg="Error processing Azure blocks", tag=provider.account
                        )
                    else:
                        self.log(msg=azure_block_file, tag="Azure Storage")
                        self.upload(
                            file=azure_block_file, route="storage", source="Azure"
                        )
                ...
                if provider.provider == "gcp" and self.checkDependency(
                    ...
                    if gcp_instance_file is None:
                        self.log(
                            msg="Error processing GCP instances", tag=provider.account
                        )
                    else:
                        self.log(msg=gcp_instance_file, tag="GCP instances")
                        self.upload(
                            file=gcp_instance_file, route="instances", source="GCP"
                        )
                    ...
                    if Path(gcp_utilization_file).is_file():
                        self.upload(
                            file=gcp_utilization_file, route="utilization", source="AWS"
                        )
                    ...
                    if gcp_block_file is None:
                        self.log(
                            msg="Error processing GCP blocks", tag=provider.account
                        )
                    else:
                        self.log(msg=gcp_block_file, tag="GCP Storage")
                        self.upload(file=gcp_block_file, route="storage", source="GCP")
            except Exception as e:
...
def main():
    ...
    if not agent.config.shouldUpload and agent.config.is_original_path_remote():
        ...
        print(
            f"techlens-agent -c {agent.config.cfg_path} -o {agent.config.output_dir} --should_upload --dry"
        )
