...
class DecryptLabsRemoteCDM(RemoteCdm):
    def __init__(self, device_type: Union[DeviceTypes, str], system_id: int, security_level: int, host: str,
                 secret: str, device_name: str, service_name: str):
    ...
    def get_license_challenge(self, session_id: bytes, pssh: PSSH, license_type: str = "STREAMING",
                              privacy_mode: bool = True) -> bytes:
        self.pssh = pssh

        res = self.session(self.host + "/get-request",
                           {"init_data": self.pssh.dumps(),
                            "service_certificate": self.req_session.signed_device_certificate,
                            "scheme": "widevine", "service": self.service_name})
    ...
    def parse_license(self, session_id: bytes, license_message: Union[SignedMessage, bytes, str]) -> None:
        ...
        if isinstance(license_message, dict) and 'keys' in license_message:
            session_keys.extend([Key(
                kid=Key.kid_to_uuid(x["kid"]),
                type_=x.get("type", "CONTENT"),
                key=bytes.fromhex(x["key"])
            ) for x in license_message["keys"]])

        else:
            res = self.session(self.host + "/decrypt-response",
                               {"session_id": session_id_api, "init_data": self.pssh.dumps(),
                                "license_request": self.license_request, "license_response": license_message,
                                "scheme": "widevine"})
            ...
            for keys in formatted_keys:
                session_keys.append((Key(kid=UUID(bytes=bytes.fromhex(keys.split(":")[0])), type_="CONTENT",
