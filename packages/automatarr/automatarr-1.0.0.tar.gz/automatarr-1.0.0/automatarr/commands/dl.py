...
from automatarr.core.cdm import DecryptLabsRemoteCDM
from automatarr.core.config import config
from automatarr.core.console import console
from automatarr.core.constants import DOWNLOAD_LICENCE_ONLY, AnyTrack, context_settings
from automatarr.core.credential import Credential
from automatarr.core.drm import DRM_T, PlayReady, Widevine
from automatarr.core.events import events
from automatarr.core.proxies import Basic, Hola, NordVPN
from automatarr.core.service import Service
from automatarr.core.services import Services
from automatarr.core.titles import Movie, Movies, Series, Song, Title_T
from automatarr.core.titles.episode import Episode
from automatarr.core.tracks import Audio, Subtitle, Tracks, Video
from automatarr.core.tracks.attachment import Attachment
from automatarr.core.utilities import get_system_fonts, is_close_match, time_elapsed_since
from automatarr.core.utils import tags
from automatarr.core.utils.click_types import LANGUAGE_RANGE, QUALITY_LIST, SEASON_RANGE, ContextData, MultipleChoice
from automatarr.core.utils.collections import merge_dict
from automatarr.core.utils.subprocess import ffprobe
from automatarr.core.vaults import Vaults


class dl:
    @click.command(short_help="Download, Decrypt, and Mux tracks for titles from a Service.",cls=Services,context_settings=dict(**context_settings, default_map=config.dl, token_normalize_func=Services.get_tag),)
    @click.option("-p", "--profile", type=str, default=None, help="Profile to use for Credentials and Cookies (if available).")
    @click.option("-q","--quality",type=QUALITY_LIST,default=[],help="Download Resolution(s), defaults to the best available resolution.",)
    @click.option("-v","--vcodec",type=click.Choice(Video.Codec, case_sensitive=False),default=None,help="Video Codec to download, defaults to any codec.",)
    @click.option("-a","--acodec",type=click.Choice(Audio.Codec, case_sensitive=False),default=None,help="Audio Codec to download, defaults to any codec.",)
    @click.option("-vb","--vbitrate",type=int,default=None,help="Video Bitrate to download (in kbps), defaults to highest available.",)
    @click.option("-ab","--abitrate",type=int,default=None,help="Audio Bitrate to download (in kbps), defaults to highest available.",)
    @click.option("-r", "--range", "range_", type=MultipleChoice(Video.Range, case_sensitive=False),default=[Video.Range.SDR],help="Video Color Range(s) to download, defaults to SDR.")
    @click.option("-c","--channels",type=float,default=None,help="Audio Channel(s) to download. Matches sub-channel layouts like 5.1 with 6.0 implicitly.",)
    @click.option("-naa","--noatmos","no_atmos",is_flag=True,default=False,help="Exclude Dolby Atmos audio tracks when selecting audio.",)
    @click.option("-w","--wanted",type=SEASON_RANGE,default=None,help="Wanted episodes, e.g. `S01-S05,S07`, `S01E01-S02E03`, `S02-S02E03`, e.t.c, defaults to all.",)
    @click.option("-l", "--lang", type=LANGUAGE_RANGE,default="en",help="Language wanted for Video and Audio.")
    @click.option("-vl","--v-lang",type=LANGUAGE_RANGE,default=[],help="Language wanted for Video, you would use this if the video language doesn't match the audio.",)
    @click.option("-sl", "--s-lang", type=LANGUAGE_RANGE, default=["all"], help="Language wanted for Subtitles.")
    @click.option("--proxy",type=str,default=None,help="Proxy URI to use. If a 2-letter country is provided, it will try get a proxy from the config.",)
    @click.option("--tag", type=str, default=None, help="Set the Group Tag to be used, overriding the one in config if any.")
    @click.option("--tmdb","tmdb_id",type=int,default=None,help="Use this TMDB ID for tagging instead of automatic lookup.",)
    @click.option("--tmdb-name","tmdb_name",is_flag=True,default=False,help="Rename titles using the name returned from TMDB lookup.",)
    @click.option("--tmdb-year","tmdb_year",is_flag=True,default=False,help="Use the release year from TMDB for naming and tagging.",)
    @click.option("--sub-format",type=click.Choice(Subtitle.Codec, case_sensitive=False),default="srt",help="Set Output Subtitle Format, only converting if necessary.",)
    ...
    @click.option("--slow",is_flag=True,default=False,help="Add a 60-120 second delay between each Title download to act more like a real device. ""This is recommended if you are downloading high-risk titles or streams.",)
    @click.option("--list","list_",is_flag=True,default=False,help="Skip downloading and list available tracks and what tracks would have been downloaded.",)
    @click.option("--list-titles",is_flag=True,default=False,help="Skip downloading, only list available titles that would have been downloaded.",)
    @click.option("--skip-dl", is_flag=True, default=False, help="Skip downloading while still retrieving the decryption keys.")
    @click.option("--export", type=Path, help="Export Decryption Keys as you obtain them to a JSON file.")
    @click.option("--cdm-only/--vaults-only",is_flag=True,default=None,help="Only use CDM, or only use Key Vaults for retrieval of Decryption Keys.",)
    ...
    @click.option("--no-source", is_flag=True, default=False, help="Disable the source tag from the output file name and path.")
    @click.option("--workers",type=int,default=None,help="Max workers/threads to download with per-track. Default depends on the downloader.",)
            ...
            if isinstance(title, Episode) and not self.tmdb_searched:
                ...
                if self.tmdb_id:
                    ...
                    if not (
                        self.tmdb_id
                        and tmdb_title
                        and tags.fuzzy_match(tmdb_title, title.title)
                    ):
                        self.tmdb_id = None
                ...
                if isinstance(title, (Movie, Episode)):
                    ...
                    if range_:
                        ...
                        for color_range in missing_ranges:
                            self.log.warning(
                                f"Skipping {color_range.name} video tracks as none are available."
                            )
                    ...
                    if s_lang and "all" not in s_lang:
                        missing_langs = [
                            lang_ for lang_ in s_lang
                            if not any(is_close_match(lang_, [sub.language]) for sub in title.tracks.subtitles)
                        ]
                        if missing_langs:
                            self.log.error(
                                ", ".join(missing_langs) + " not found in tracks"
                            )
                            sys.exit(1)

                        title.tracks.select_subtitles(
                            lambda x: is_close_match(x.language, s_lang)
                        )
                        if not title.tracks.subtitles:
                            self.log.error(
                                f"There's no {s_lang} Subtitle Track..."
                            )
                            sys.exit(1)
