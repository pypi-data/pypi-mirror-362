...
from automatarr.core.downloaders import requests as requests_downloader
from automatarr.core.drm import DRM_T, ClearKey, PlayReady, Widevine
from automatarr.core.events import events
from automatarr.core.tracks import Audio, Subtitle, Tracks, Video
from automatarr.core.utilities import get_extension, is_close_match, try_ensure_utf8
            ...
            tracks.add(
                track_type(
                    ...
                    is_original_lang=bool(
                        language and is_close_match(track_lang, [language])
                    ),
            ...
            if isinstance(session_drm, (Widevine, PlayReady)):
                ...
                try:
                    if not license_widevine:
                        raise ValueError(
                            "license_widevine func must be supplied to use DRM"
                        )
    ...
    def get_supported_key(keys: list[Union[m3u8.model.SessionKey, m3u8.model.Key]]) -> Optional[m3u8.Key]:
        ...
        for key in keys:
            ...
            # TODO: Add support for 'SAMPLE-AES', 'AES-CTR', 'AES-CBC', 'ClearKey'
                ...
                key.keyformat.lower() == PlayReadyCdm
                or "com.microsoft.playready" in key.keyformat.lower()
            ):
                return key
            else:
