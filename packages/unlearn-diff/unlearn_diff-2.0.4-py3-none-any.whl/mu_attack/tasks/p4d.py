...
class P4D_DiffusersPipeline(BaseDiffusersPipeline):
    ...
    def get_loss(self,x0,t,input_ids,input_embeddings,orig_input_ids,orig_input_embeddings,**kwargs):
        ...
        noise_pred_sd = self.unet_sd(noised_latent,t,encoder_hidden_states=encoder_hidden_states_sd).sample
        error = self.criterion(noise_pred_sd,noise_pred)
        return error
    
class P4D_CompvisPipeline(BaseCompvisPipeline):
...
class P4DTask(Task):
    def __init__(
        ...
        save_diffuser=False,         # Flag to trigger conversion
        ...
        **kwargs,
        ...
        if self.backend == "diffusers":
            # For the diffusers backend, initialize the diffusers pipeline directly.
            self.pipe = P4D_DiffusersPipeline(
            ...
            if save_diffuser:
                # First, load the CompVis pipeline to get the original model checkpoint.
                compvis_pipe = P4D_CompvisPipeline(
                ...
                # Convert the CompVis model (using the UNet from the compvis pipeline)
                ...
                # If no conversion is needed, load the CompVis pipeline as usual.
                self.pipe = P4D_CompvisPipeline(
                    config_path=compvis_config_path,
                    ckpt_path=compvis_ckpt_path,
                    device="cuda",
                    cache_path=cache_path,
        ...
        # Setup other properties for sampling
        self.T = 1000
