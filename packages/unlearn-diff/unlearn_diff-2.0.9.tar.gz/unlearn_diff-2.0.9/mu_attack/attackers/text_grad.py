...
class TextGradAttacker(Attacker):
    def __init__(self, lr=1e-2, weight_decay=0.1, rand_init=False, **kwargs):
    ...
    def init_adv(self, task, orig_prompt_len):
        vocab_dict = task.pipe.tokenizer.get_vocab()
        tmp = torch.zeros([1, self.k, len(vocab_dict)]).fill_(1 / len(vocab_dict))
        adv_embedding = torch.nn.Parameter(tmp).to(task.pipe.device)
        if self.rand_init:
    ...
    def init_opt(self):
        self.optimizer = torch.optim.Adam(
            [self.adv_embedding], lr=self.lr, weight_decay=self.weight_decay
        )

    def split_embd(self, input_embed, orig_prompt_len):
        sot_embd, mid_embd, _, eot_embd = torch.split(
            input_embed,
            [1, orig_prompt_len, self.k, 76 - orig_prompt_len - self.k],
            dim=1,
        )
    ...
    def split_id(self, input_ids, orig_prompt_len):
        sot_id, mid_id, _, eot_id = torch.split(
            input_ids,
            [1, orig_prompt_len, self.k, 76 - orig_prompt_len - self.k],
            dim=1,
        )
    ...
    def construct_embd(self, adv_embedding):
        if (
            self.insertion_location == "prefix_k"
        ):  # Prepend k words before the original prompt
            embedding = torch.cat(
                [self.sot_embd, adv_embedding, self.mid_embd, self.eot_embd], dim=1
            )
        elif (
            self.insertion_location == "suffix_k"
        ):  # Append k words after the original prompt
            embedding = torch.cat(
                [self.sot_embd, self.mid_embd, adv_embedding, self.eot_embd], dim=1
            )
        elif (
            self.insertion_location == "mid_k"
        ):  # Insert k words in the middle of the original prompt
            embedding = [
                self.sot_embd,
            ]
            ...
            self.insertion_location == "insert_k"
        ):  # seperate k words into the original prompt with equal intervals
            embedding = [
                self.sot_embd,
            ]
            total_num = self.mid_embd.size(1)
            ...
            for i in range(self.k):
                embedding.append(
                    self.mid_embd[:, internals * i : internals * (i + 1), :]
                )
    ...
    def run(self, task, logger):
        ...
        visualize_embedding = task.pipe.id2embedding(viusalize_prompt_id)
        visualize_orig_prompt_len = (viusalize_prompt_id == 49407).nonzero(
            as_tuple=True
        )[1][0] - 1
        ...
        visualize_sot_id, visualize_mid_id, visualize_eot_id = self.split_id(
            viusalize_prompt_id, visualize_orig_prompt_len
        )

        ### Visualization for the original prompt:
        results = task.evaluate(
            viusalize_prompt_id, prompt, seed=seed, guidance_scale=guidance
        )
        ...
        if not self.universal:
            if seed is None:
                seed = self.eval_seed
            x0 = task.pipe.img2latent(image)
            input_ids = task.pipe.str2id(prompt)
            orig_prompt_len = (input_ids == 49407).nonzero(as_tuple=True)[1][0] - 1
            input_embeddings = task.pipe.id2embedding(input_ids)
            ...
            if self.sequential:
                for t in task.sampled_t:
                    ...
                    for i in range(self.iteration):
                        ...
                        tmp_embeds = adv_one_hot @ task.pipe.all_embeddings
                        ...
                        input_arguments = {
                            ...
                            "orig_input_ids": viusalize_prompt_id,
                            "orig_input_embeddings": visualize_embedding,
                            "seed": seed,
                            "guidance_scale": guidance,
                        }
                        loss = task.pipe.get_loss(**input_arguments)
                        self.adv_embedding.grad = torch.autograd.grad(
                            loss, [self.adv_embedding]
                        )[0]
                        self.optimizer.step()
                        proj_adv_embedding = self.projection(self.adv_embedding)
                        self.adv_embedding.data = proj_adv_embedding.data
                        total_loss += loss.item()
                    _, proj_ids = self.argmax_project(
                        self.adv_embedding, task.all_embeddings, task.pipe.tokenizer
                    )
                    new_visualize_id = self.construct_id(
                        proj_ids, visualize_sot_id, visualize_eot_id, visualize_mid_id
                    )
                    id_list = new_visualize_id[0][1:].tolist()
                    id_list = [
                        id for id in id_list if id != task.pipe.tokenizer.eos_token_id
                    ]
                    new_visualize_prompt = task.pipe.tokenizer.decode(id_list)
                    results = task.evaluate(
                        new_visualize_id,
...
def get(**kwargs):
    return TextGradAttacker(**kwargs)
