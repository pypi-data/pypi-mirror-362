...
class HardPromptAttacker(Attacker):
    ...
    def init_adv(self, task, orig_prompt_len):
        if self.insertion_location != 'per_k_words':
            adv_embedding = torch.nn.Parameter(torch.randn([1, self.k, 768])).to(task.pipe.device)
        else:
            adv_embedding = torch.nn.Parameter(torch.randn([1, np.ceil(orig_prompt_len / self.k).astype(int).item(), 768])).to(task.pipe.device)
        tmp_ids = torch.randint(0,len(task.pipe.tokenizer),(1, self.k)).to(task.pipe.device)
        tmp_embeddings = task.pipe.id2embedding(tmp_ids)
    ...
    def project(self,adv_embedding,task):
        with torch.no_grad():
            ...
            all_embeddings = F.normalize(task.pipe.all_embeddings, p=2, dim=-1)
            sim = F.cosine_similarity(adv_embeddings.unsqueeze(2), all_embeddings, dim=-1)
            most_similar_idx = sim.argmax(dim=-1)
            proj_embeds = task.pipe.all_embeddings[0][most_similar_idx[0]].unsqueeze(0)
        ...
        if not self.universal:
            ...
            if self.sequential:
                for t in task.sampled_t:
                    ...
                    id_list = [id for id in id_list if id!=task.pipe.tokenizer.eos_token_id]
                    new_visualize_prompt = task.pipe.tokenizer.decode(id_list)
                    # print(new_visualize_prompt)
                    results = task.evaluate(new_visualize_id,new_visualize_prompt,seed,guidance_scale=guidance)
