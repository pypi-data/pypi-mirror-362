...
def main():
    ...
    #logger setuup
    log_file = os.path.join(logs_dir, f"Unified_Concept_Editing_evalaute_{config.get('theme')}.log")
    logger = setup_logger(log_file=log_file, level=logging.INFO)
    logger.info("Starting Unified Concept Editing Evalution Framework")
