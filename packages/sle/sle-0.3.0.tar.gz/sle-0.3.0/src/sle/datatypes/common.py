...
class Credentials(univ.Choice):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            "used",
            univ.OctetString()
            .subtype(subtypeSpec=constraint.ValueSizeConstraint(8, 256))
...
class Diagnostics(univ.Integer):
    namedValues = namedval.NamedValues(("duplicateInvokeId", 100), ("otherReason", 127))
...
class SlduStatusNotification(univ.Integer):
    namedValues = namedval.NamedValues(
        ("produceNotification", 0), ("doNotProduceNotification", 1)
    )
