...
class RafUserToProviderPdu(univ.Choice):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            ...
            RafGetParameterInvocation().subtype(
...
class FrameOrNotification(univ.Choice):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType(
            "annotatedFrame",
            RafTransferDataInvocation().subtype(
                implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 0)
            ),
        ),
        namedtype.NamedType(
            "syncNotification",
            RafSyncNotifyInvocation().subtype(
