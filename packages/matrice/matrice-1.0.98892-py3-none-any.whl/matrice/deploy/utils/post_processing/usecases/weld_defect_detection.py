...
class WeldDefectUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(self, counting_summary: Dict, insights: List[str], summary: str, config: WeldDefectConfig, frame_number: Optional[int] = None, stream: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        target_categories = config.target_defect_categories
        ...
        human_text_lines = []
        target_counts = {cat: per_category_count.get(cat, 0) for cat in target_categories}
        total_target_defects = sum(target_counts.values())

        if total_target_defects > 0:
            human_text_lines.append(f"CURRENT FRAME @ {current_timestamp}:")
            category_counts = [f"{count} {cat}" for cat, count in target_counts.items() if count > 0]
        ...
        if total_target_defects > 0:
            ...
            human_text_lines.append(f"ALERTS SINCE @ {start_timestamp}:")
            human_text_lines.append(f"\t- Total Defects Detected: {sum(count for cat, count in total_defect_counts.items() if cat in target_categories)}")
            for cat, count in total_defect_counts.items():
                if count > 0 and cat in target_categories:
                    human_text_lines.append(f"\t- {cat}: {count}")

        human_text = "\n".join(human_text_lines) if human_text_lines else "No critical defects detected"
