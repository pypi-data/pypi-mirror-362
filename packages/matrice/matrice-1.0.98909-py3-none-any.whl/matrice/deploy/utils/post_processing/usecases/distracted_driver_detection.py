...
class DistractedDriverConfig(BaseConfig):
    """Configuration for Distracted Driver Detection use case in driver detection."""
    ...
    driver_categories: List[str] = field(
        default_factory=lambda: ['drinking', 'hair and makeup', 'operating the radio', 'reaching behind', 'safe driving', 'talking on the phone', 'talking to passenger', 'texting']
    )

    target_driver_categories: List[str] = field(
        default_factory=lambda: ['drinking', 'hair and makeup', 'operating the radio', 'reaching behind', 'safe driving', 'talking on the phone', 'talking to passenger', 'texting']
    )
...
class DistractedDriverUseCase(BaseProcessor):
    def __init__(self):
        ...
        # List of driver categories to track
        self.driver_categories = ['drinking', 'hair and makeup', 'operating the radio', 'reaching behind', 'safe driving', 'talking on the phone', 'talking to passenger', 'texting']
        ...
        self._track_merge_iou_threshold: float = 0.2  # IoU ≥ 0.2 → same driver
        self._track_merge_time_window: float = 5.0    # seconds within which to merge

    def _get_track_ids_info(self, detections: list) -> Dict[str, Any]:
        """
        Get detailed information about track IDs for drivers (per frame).
        """
        ...
        total_track_ids = set()
        for s in getattr(self, '_driver_total_track_ids', {}).values():
    ...
    @staticmethod
    def _deduplicate_drivers(detections, iou_thresh=0.7):
        """Suppress duplicate/overlapping driver with same label and high IoU."""
        filtered = []
        ...
        for i, det in enumerate(detections):
            ...
            for j in range(i+1, len(detections)):
                ...
                if det.get("category") == detections[j].get("category"):
                    ...
                    if bbox1 and bbox2:
                        iou = DistractedDriverUseCase._iou(bbox1, bbox2)
                        if iou > iou_thresh:
                            used[j] = True
                            group.append(j)
    ...
    def _update_driver_tracking_state(self, detections: list):
        """
        Track unique driver track_ids per category for total count after tracking.
        ...
        if not hasattr(self, "_driver_total_track_ids"):
            self._driver_total_track_ids = {cat: set() for cat in self.driver_categories}
        self._driver_current_frame_track_ids = {cat: set() for cat in self.driver_categories}

        for det in detections:
            cat = det.get("category")
            raw_track_id = det.get("track_id")
            if cat not in self.driver_categories or raw_track_id is None:
                continue
            ...
            self._driver_total_track_ids.setdefault(cat, set()).add(canonical_id)
            self._driver_current_frame_track_ids[cat].add(canonical_id)

    def get_total_driver_counts(self):
        """
        Return total unique track_id count for each driver category.
        """
        return {cat: len(ids) for cat, ids in getattr(self, '_driver_total_track_ids', {}).items()}
    ...
    def process(self, data: Any, config: ConfigProtocol, context: Optional[ProcessingContext] = None, stream_info: Optional[Dict[str, Any]] = None) -> ProcessingResult:
        """
        Main entry point for Distracted Driver Detection post-processing.
        Applies category mapping, driver smoothing, counting, alerting, and summary generation.
        ...
        if config.target_driver_categories:
                processed_data = [d for d in processed_data if d.get('category') in self.driver_categories]
                self.logger.debug(f"Applied driver category filtering")
        ...
        if config.enable_smoothing:
            ...
            smoothed_drivers = bbox_smoothing(processed_data, self.smoothing_tracker.config, self.smoothing_tracker)
            processed_data = smoothed_drivers
        ...
        try:
            ...
            if self.tracker is None:
                ...
                self.logger.info("Initialized AdvancedTracker for Distracted Driver detection and tracking")
        ...
        # Deduplicate overlapping driver (same label, high IoU)
        processed_data = self._deduplicate_drivers(processed_data, iou_thresh=0.95)

        # Update driver tracking state for total count per label
        self._update_driver_tracking_state(processed_data)
        ...
        # Add total unique driver counts after tracking using only local state
        total_driver_counts = self.get_total_driver_counts()
        counting_summary['total_driver_counts'] = total_driver_counts
        ...
        result = self.create_result(
            data={
                ...
                "alerts": alerts,
                "total_drivers": counting_summary.get("total_count", 0),
                "events": events,
    ...
    def reset_driver_tracking(self) -> None:
        """
        Reset driver tracking state (total counts, track IDs, etc.).
        """
        self._driver_total_track_ids = {cat: set() for cat in self.driver_categories}
        self._total_frame_counter = 0
        self._global_frame_offset = 0
        ...
        self.logger.info("Distracted Driver Detection state reset")
    
    def reset_all_tracking(self) -> None:
        """
        Reset both advanced tracker and driver tracking state.
        """
        self.reset_tracker()
        self.reset_driver_tracking()
        self.logger.info("All driver tracking state reset")
        
    def _generate_events(self, counting_summary: Dict, alerts: List, config: DistractedDriverConfig, frame_number: Optional[int] = None, stream_info: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        if total_drivers > 0:
            ...
            human_text_lines.append(f"\t- {total_drivers} Driver(s) detected [INFO]")
            human_text = "\n".join(human_text_lines)

            event = {
                "type": "distracted_driver_detection",
                "category": "automobile",
                "count": total_drivers,
                "stream_time": datetime.now(timezone.utc).strftime("%Y-%m-%d-%H:%M:%S UTC"),
                "application_name": "Distracted Driver Detection",
        ...
        for alert in alerts:
            ...
            if config.alert_config and config.alert_config.count_thresholds:
                ...
                percentage = (total_drivers / threshold) * 100 if threshold > 0 else 0
                if percentage < 20:
                    intensity_message = "ALERT: Low congestion in the scene"
                elif percentage <= 50:
    ...
    def _generate_tracking_stats(
        ...
        total_drivers = counting_summary.get("total_count", 0)
        total_driver_counts = counting_summary.get("total_driver_counts", {})
        cumulative_total = sum(total_driver_counts.values()) if total_driver_counts else 0
        ...
        if total_drivers > 0:
            ...
            if len(category_counts) == 1:
                drivers_text = f"\t{category_counts[0]} detected"
            elif len(category_counts) == 2:
                drivers_text = f"\t{category_counts[0]} and {category_counts[1]} detected"
            else:
    ...
    def _count_categories(self, detections: list, config: DistractedDriverConfig) -> dict:
        """
    ...
    # Human-friendly display names for driver categories
    CATEGORY_DISPLAY = {
        'Drinking': 'drinking',  
    ...
    def _generate_insights(self, summary: dict, config: DistractedDriverConfig) -> List[str]:
        """
        Generate human-readable insights for each driver category.
        """
        insights = []
        ...
        #     if total_drivers > 15:
        #         insights.append(f"INTENSITY: Heavy congestion in the scene with {total_drivers} drivers")
        #     elif total_drivers == 1:
    ...
    def _check_alerts(self, summary: dict, config: DistractedDriverConfig) -> List[Dict]:
        """
        ...
        if config.alert_config.count_thresholds:
            for category, threshold in config.alert_config.count_thresholds.items():
                if category == "all" and total >= threshold:
                    ...
                    "severity": "warning",
                    "message": f"Total driver count ({total}) exceeds threshold ({threshold})",
                    "category": category,
                    "current_count": total,
                    "threshold": threshold
                    })
    ...
    def _generate_summary(self, summary: dict, alerts: List) -> str:
        ...
        Adds a tab before each driver label for better formatting.
        Also always includes the cumulative driver count so far.
        """
        total = summary.get("total_count", 0)
        per_cat = summary.get("per_category_count", {})
        cumulative = summary.get("total_driver_counts", {}) 
