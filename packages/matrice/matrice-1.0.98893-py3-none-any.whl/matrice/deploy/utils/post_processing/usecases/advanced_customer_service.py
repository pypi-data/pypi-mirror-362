...
class AdvancedCustomerServiceUseCase(BaseProcessor):
    def _generate_per_frame_agg_summary(self, processed_data, analytics_results, config, context, stream_info=None):
        ...
        for frame_id, detections in processed_data.items():
            ...
            # Safely handle frame_id type for total_frames_processed
            if isinstance(frame_id, int):
                total_frames_proc = total_frames_processed + frame_id
            else:
                try:
                    total_frames_proc = total_frames_processed + int(frame_id)
                except Exception:
                    total_frames_proc = total_frames_processed
            tracking_stat = {
                ...
                "total_frames_processed": total_frames_proc,
    ...
    def process(self, data: Any, config: ConfigProtocol,
        ...
        try:
            ...
            # Ensure area attributes are initialized
            if not hasattr(self, 'customer_areas') or not hasattr(self, 'staff_areas') or not hasattr(self, 'service_areas'):
                self._initialize_areas(
                    getattr(config, 'customer_areas', {}),
                    getattr(config, 'staff_areas', {}),
                    getattr(config, 'service_areas', {})
                )
            ...
            # Only include agg_summary (with events and tracking_stats) in result_data
            ...
            # 6. (Skip insights, alerts, summary, predictions for agg_summary-only output)
    ...
    def _extract_predictions(self, data: Any) -> Dict[str, List[Dict[str, Any]]]:
        """Extract predictions from processed data for API compatibility, flat list"""
        predictions = []
        try:
