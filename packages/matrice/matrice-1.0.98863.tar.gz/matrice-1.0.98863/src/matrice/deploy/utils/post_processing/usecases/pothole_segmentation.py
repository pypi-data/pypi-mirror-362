...
class PotholeSegmentationUseCase(BaseProcessor):
    ...
    def _generate_tracking_stats(
        ...
        per_frame_count = summary.get("total_objects", 0)
        total_unique = summary.get("total_pothole_count", 0)

        # Always get track ID info even if no detections
        track_ids_info = self._get_track_ids_info(summary.get("detections", []))
