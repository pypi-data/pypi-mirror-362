...
class CarDamageConfig(BaseConfig):
    ...
    relevant_categories: List[str] = field(default_factory=lambda: ["car damage"])
...
class CarDamageDetectionUseCase(BaseProcessor):
    """
    Car Damage Detection Use Case — structured similarly to License Plate Detection.
    Tracks per-frame counts and total unique car damage detections using track_ids.
    """
    ...
    def __init__(self):
        ...
        # List of categories to track (same as config)
        self.relevant_categories = ["car damage"]
        ...
        self._total_damage_track_ids = set()
        ...
        # Canonical tracking ID resolution
        self._track_aliases: Dict[Any, Any] = {}
        self._canonical_tracks: Dict[Any, Dict[str, Any]] = {}

        # Tunable thresholds for merging fragmented tracks
        self._track_merge_iou_threshold: float = 0.05  # IoU ≥ 0.05 → same object
        self._track_merge_time_window: float = 7.0     # seconds window for merging

    def _update_tracking_state(self, detections: List[Dict[str, Any]]) -> None:
        """
        Track unique car damage track_ids for cumulative and per-frame counts.
        ...
        for det in detections:
            ...
            self._total_damage_track_ids.add(canonical_id)
            self._current_frame_track_ids.add(canonical_id)

    def get_total_car_damage_count(self) -> int:
        """
        Return the total number of unique car damage regions detected so far
        (based on unique track_ids).
        """
        return len(self._total_damage_track_ids)
    ...
    def _get_track_ids_info(self, detections: list) -> dict:
        """
        Collect track ID information for current frame and total unique IDs seen so far.
        Ensures JSON-serializable format for output.
        """
        frame_track_ids = set()
        for det in detections:
        ...
        self._total_damage_track_ids.update(frame_track_ids)

        return {
            "frame_track_ids": list(frame_track_ids),
            "total_unique_track_ids": list(self._total_damage_track_ids),
            "frame_track_ids_count": len(frame_track_ids),
            "total_unique_count": len(self._total_damage_track_ids),
        }
    ...
    def process(
        ...
        """
        Main entry point for Car Damage Detection post-processing.
        Applies category mapping, smoothing, tracking, counting, and summary generation.
        """
        start_time = time.time()

        if not isinstance(config, CarDamageConfig):
        ...
        try:
            ...
            if self.tracker is None:
                ...
                self.logger.info("Initialized AdvancedTracker for Car Damage tracking")
        ...
        total_unique = self.get_total_car_damage_count()
        counting_summary["total_car_damage_count"] = total_unique

        insights = self._generate_insights(counting_summary, config)
        alerts = []  # No alerts for car damage
    ...
    def reset_tracker(self) -> None:
        ...
        if self.tracker is not None:
            self.tracker.reset()
            self.logger.info("AdvancedTracker reset for new car damage session")

    def reset_tracking_state(self) -> None:
        """
        Reset car damage tracking state (total counts, track IDs, etc.).
        """
        self._total_damage_track_ids = set()
        self._total_frame_counter = 0
        self._global_frame_offset = 0
        ...
        self.logger.info("Car damage tracking state reset")
    ...
    def reset_all_tracking(self) -> None:
        ...
        self.logger.info("All car damage tracking state reset")
    ...
    def _generate_events(
        ...
        """Generate structured events for the output format with frame-based keys (no alerts for car damage)."""
        from datetime import datetime, timezone
        ...
        if total_count > 0:
            ...
            human_text_lines.append(f"    - {total_count} car damage(s) detected [INFO]")
            human_text = "\n".join(human_text_lines)

            event = {
                "type": "car_damage_detection",
                "severity": "info",
                "category": "car_damage",
                "count": total_count,
    ...
    def _generate_tracking_stats(
        ...
        total_unique = counting_summary.get("total_car_damage_count", 0)
        ...
        if per_frame_count > 0:
            human_text_lines.append(f"\t- Car Damages Detected: {per_frame_count}")
        else:
            human_text_lines.append("\t- No car damage detected")

        human_text_lines.append("")  # Blank line for spacing

        # TOTAL SINCE section
        human_text_lines.append(f"TOTAL SINCE {start_timestamp}:")
        human_text_lines.append(f"\t- Total Car Damages Detected: {total_unique}")

        human_text = "\n".join(human_text_lines)

        tracking_stat = {
            "type": "car_damage_tracking",
            "category": "car_damage",
    ...
    def _count_categories(self, detections: list, config: CarDamageConfig) -> dict:
        """
        Count the number of car damage detections and return a summary dict.
    ...
    def _generate_insights(self, summary: dict, config: CarDamageConfig) -> List[str]:
        """
        Generate simple human-readable insights for car damage detection.
        """
        insights = []
    ...
    def _check_alerts(self, summary: dict, config: CarDamageConfig) -> List[Dict]:
        """
        No alerts are applicable for Car Damage Detection.
    ...
    def _generate_summary(self, summary: dict, alerts: List) -> str:
        """
        Generate a human_text string for car damage detection.
        Includes per-frame count and cumulative unique count so far.
        """
        total = summary.get("total_count", 0)
        per_cat = summary.get("per_category_count", {})
        cumulative_total = summary.get("total_car_damage_count", 0)

        lines = []

        if total > 0:
            lines.append(f"{total} car damage(s) detected in this frame")
            if per_cat:
            ...
            lines.append("No car damage detected in this frame")

        lines.append(f"Total unique car damages detected: {cumulative_total}")
