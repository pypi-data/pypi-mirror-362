...
class FastLoop:
    ...
    def _get_event_type_value(self, event_class: type[LoopEvent]) -> str:
        type_name = getattr(event_class, "type", None)
        ...
        raise ValueError(
            f"Could not determine event type for {event_class.__name__}. "
            "Use the @app.event decorator or set a default value for the 'type' field."
        )

    def register_events(self, event_types: list[type[LoopEvent]]):
        for event_type in event_types:
            self.register_event(event_type)

    def register_event(self, event_type: type[LoopEvent]):
        type_value = self._get_event_type_value(event_type)
        if type_value in self._event_types:
            logger.warning(
                f"Event type '{type_value}' is already registered. Overwriting."
            )
    ...
    def loop(
        ...
        def _decorator(func: Callable) -> Callable:
            if isinstance(start_event, type) and issubclass(start_event, LoopEvent):
                start_event_key = self._get_event_type_value(start_event)
