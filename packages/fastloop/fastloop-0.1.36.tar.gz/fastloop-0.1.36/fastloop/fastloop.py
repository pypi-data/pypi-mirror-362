...
class FastLoop:
    ...
    def register_events(self, event_classes: list[tuple[str, type[LoopEvent]]]):
        for event_type, event_class in event_classes:
            self.register_event(event_class, event_type)

    def register_event(
        self, event_class: type[LoopEvent], event_type: str | None = None
    ):
        if event_type:
            self._event_types[event_type] = event_class
        else:
            self._event_types[event_class.type] = event_class
