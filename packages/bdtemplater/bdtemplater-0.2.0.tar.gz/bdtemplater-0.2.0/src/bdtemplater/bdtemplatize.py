...
def get_possibly_commented_value(value, strip_quotes=False):
    """
...
def process_dictionary(properties, strip_quotes, only_keys, env_dict, not_keys):
    """
...
def generate_template_from_files(
    ...
    :param properties_files: Path to a file or list of paths to files containing property values.
...
def generate_template(
    ...
    tfdict = process_dictionary(
        properties, strip_quotes, only_keys, env_dict, not_keys
    )
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Generate templates by replacing placeholders with values from properties files"
        ),
...
Examples:
  ...
  %(prog)s --properties-file file1.tfvars --properties-file file2.tfvars # Multiple properties files
  ...
  %(prog)s -f -o output.tf                         # Force overwrite existing output file
  %(prog)s --force --output output.tf              # Force overwrite existing output file
  %(prog)s -e                                      # Inject environment variables
  %(prog)s --env                                   # Inject environment variables
  %(prog)s --env --env-prefix                      # Inject env vars with 'env.' prefix
  %(prog)s --env --env-key-strip MYAPP_            # Strip MYAPP_ prefix from env keys
  %(prog)s --env --env-key-strip AWS_ --env-key-strip K8S_ # Strip multiple prefixes
  %(prog)s --env --env-key-strip-hcdefaults        # Strip HashiCorp tool prefixes
  ...
  %(prog)s --import-module mymodule                # Import external module
    ...
    parser.add_argument(
        ...
        action="append",
        help="Properties file (can be used multiple times, overrides positional argument)",
    )
    ...
    parser.add_argument(
        "--env-prefix",
        action="store_true",
        help="Prefix environment variable keys with 'env.' (requires --env flag)",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(can be used multiple times, requires --env flag)"
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "VAULT_, NOMAD_ (implies --env-key-strip, requires --env flag)"
        ),
    )

    args = parser.parse_args(argv)

    # Validate that --env-prefix requires --env
    if args.env_prefix and not args.env:
        print("Error: --env-prefix flag requires --env flag to be set", file=sys.stderr)
        sys.exit(1)

    # Validate that --env-key-strip requires --env
    ...
    # Validate that --env-key-strip-hcdefaults requires --env
    ...
    # Handle properties files - use list if --properties-file provided, otherwise use positional
    ...
    # Check each properties file exists
    for properties_file in properties_files:
