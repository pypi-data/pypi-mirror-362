...
"""
        ...
        try:
            ...
            try:
                ...
                main([template_path, tfvars_path, "-o", output_path, "-v", "--force"])
...
"""
        ...
        keymap = ["project", "env"]
        result = generate_template(template_content, tfvars_content, strip_quotes=True, only_keys=keymap)
...
"""
        ...
        with pytest.raises(KeyError) as exc_info:
            generate_template(template, tfvars_content, strip_quotes=True, only_keys=keymap)
        assert "Keys in only_keys missing from tfvars: missing_key" in str(exc_info.value)
...
"""
        ...
        result = generate_template(template, tfvars_content, strip_quotes=True, only_keys=keymap)
    ...
    def test_environment_variable_injection(self):
        ...
        result = generate_template(template, tfvars_content, strip_quotes=True, env_dict=test_env)
    ...
    def test_environment_variable_injection_with_keymap(self):
        ...
        result = generate_template(template, tfvars_content, strip_quotes=True, only_keys=keymap, env_dict=test_env)
    ...
    def test_main_with_env_prefix_flag_error_without_env(self, capsys):
        """Test that --env-prefix flag requires --env flag."""
        import tempfile
        ...
        try:
            ...
            captured = capsys.readouterr()
            assert (
                "Error: --env-prefix flag requires --env flag to be set" in captured.err
            )
    ...
    def test_env_key_strip_functionality(self):
        ...
        result = generate_template(template, tfvars_content, strip_quotes=True, env_dict=stripped_env)
    ...
    def test_main_with_env_key_strip_flag_error_without_env(self, capsys):
        """Test that --env-key-strip flag requires --env flag."""
        import tempfile
        ...
        try:
            ...
            captured = capsys.readouterr()
            assert (
                "Error: --env-key-strip flag requires --env flag to be set"
                in captured.err
            )
    ...
    def test_main_with_env_key_strip_hcdefaults_flag_error_without_env(self, capsys):
        """Test that --env-key-strip-hcdefaults flag requires --env flag."""
        import tempfile
        ...
        try:
            ...
            captured = capsys.readouterr()
            assert (
                "Error: --env-key-strip-hcdefaults flag requires --env flag to be set"
                in captured.err
            )
    ...
    def test_env_key_strip_hcdefaults_with_env_prefix(self):
        ...
        result = generate_template(template, tfvars_content, strip_quotes=True, env_dict=prefixed_env)
...
"""
        ...
        not_keys = ["secret"]
        result = generate_template(template_content, tfvars_content, strip_quotes=True, not_keys=not_keys)
...
"""
        ...
        try:
            ...
            try:
                ...
                main([template_path, tfvars_path, "--not-keys", "secret", "--strip-quotes"])
...
"""
        ...
        try:
            ...
            not_keys = ["secret"]
            result = generate_template(template_content, tfvars_content, strip_quotes=True, env_dict=env_dict, not_keys=not_keys)
            assert "Project: test-project" in result
            assert "Secret: @secret@" in result  # Should remain as placeholder
...
"""
        ...
        try:
            ...
            try:
                ...
                main([template_path, tfvars_path, "--not-keys", "secret", "-v"])
    ...
    def test_get_possibly_commented_value_edge_cases(self):
        ...
        assert get_possibly_commented_value(r'"escaped \"quote\""', strip_quotes=True) == r'escaped \"quote\"'
        
        # Test quote handling with mixed quotes
        assert get_possibly_commented_value("'single quoted'", strip_quotes=True) == "single quoted"
        
        # Test comment outside quotes
        assert get_possibly_commented_value('value # comment', strip_quotes=False) == "value"
        
        # Test comment inside quotes (should not be stripped)
        assert get_possibly_commented_value('"value # not a comment"', strip_quotes=True) == "value # not a comment"
        
        # Test ending quote matching
        assert get_possibly_commented_value('"mismatched\'', strip_quotes=True) == '"mismatched\''
    ...
    def test_multiple_replacements_files_verbose_output(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".bdt", delete=False) as template_file:
            template_file.write(template_content)
            template_path = template_file.name
        
        with tempfile.NamedTemporaryFile(mode="w", suffix=".tfvars", delete=False) as tfvars_file1:
            tfvars_file1.write(tfvars_content1)
            tfvars_path1 = tfvars_file1.name
        
        with tempfile.NamedTemporaryFile(mode="w", suffix=".tfvars", delete=False) as tfvars_file2:
        ...
        try:
            ...
            try:
                from bdtemplater.bdtemplatize import main
                main([template_path, "-r", tfvars_path1, "-r", tfvars_path2, "-v"])
                
                stderr_output = captured_stderr.getvalue()
                assert f"Properties files: ['{tfvars_path1}', '{tfvars_path2}']" in stderr_output
...
"""
        ...
        try:
            ...
            try:
                from bdtemplater.bdtemplatize import main
                main([template_path, tfvars_path, "--import-module", module_name, 
                     "--post-process", "custom_post_process", "-v"])
                
                stderr_output = captured_stderr.getvalue()
                assert f"Using post-processing function: custom_post_process from {module_name}" in stderr_output
    ...
    def test_force_flag_without_force_exits_with_code_3(self):
        """Test that script exits with code 3 when output file exists and --force is not used."""
        tfvars_content = 'project = "test-project"'
        template_content = "Project: @project@"
        ...
        try:
            ...
            # Test that script exits with code 3 when file exists and --force is not used
            with pytest.raises(SystemExit) as exc_info:
    ...
    def test_force_flag_verbose_output(self):
        ...
        try:
            ...
            try:
                from bdtemplater.bdtemplatize import main
                main([template_path, tfvars_path, "-o", output_path, "--force", "-v"])
    ...
    def test_dump_flag_with_filters(self):
        ...
        try:
            ...
            try:
                ...
                with pytest.raises(SystemExit) as excinfo:
                    main([template_path, tfvars_path, "--dump", "--only-keys", "project,environment"])
