...
def read_image(path: str) -> np.ndarray:
    try:
        ...
        raise ProcessingError(f"Failed to read image: {path} - {str(e)}")


def write_image(path: str, image: np.ndarray, params: list = None) -> None:
    try:
        ...
        success, buffer = cv2.imencode(f'.{ext}', image, params)
        if not success:
            raise ProcessingError(f"Failed to encode image to format: {ext}")

        with open(path, 'wb') as f:
            f.write(buffer.tobytes())
    except Exception as e:
        raise ProcessingError(f"Failed to save image: {path} - {str(e)}")
...
def enhance_image(
    ...
    try:
        ...
        write_image(output_path, enhanced_image, save_params)
        return True
...
def enhance_images_in_folder(
    ...
    for i, img_path in enumerate(image_files):
        ...
        out_filename = f"{base_name}{suffix}{out_ext}"
