...
def validate_path(path, var_name):
    if not path or not os.path.isabs(path):
        raise ValueError(f"{var_name} must be an absolute path. Provided: '{path}'")
...
def main(pause_to_check=True):
    ...
    if pause_to_check:
        ...
        input(
            f"\nPlease check everything is correct, then press Enter to continue switching to the new user model '{label}.{copy_model}'...\
    ...
    # now edit all the files we found to change the import to import User from the new app, instead of importing {copy_model} as User
    for file in found:
        with open(file, "r") as f:
            lines = f.readlines()
    ...
    if "rename model" not in output.stdout.lower():
        print(
            ...
            Please check the output below. We will stop here to avoid running migrate as something is likely wrong. You may need to manually perform the rename."
        )
    ...
    print(
        f"Successfully switched to the new custom user model '{label}.User'. You may want to perform the following cleanup steps:"
    )
    print(
        f" - Remove any manual references to the old user model in and point to the new model '{label}.User'"
    )
    print(
        f" - Remove the temporary settings file '{temp_settings}' and the import from {installed_apps} and add AUTH_USER_MODEL = '{label}.User' directly to your main settings.py"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="This will pause the script switching to the new user model, so you can check everything is correct before proceeding. \
            You can remove this flag to skip the pause and run straight through.",
    )
