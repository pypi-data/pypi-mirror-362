...
class PySob:
    ...
    def insert(self,
        ...
        if _db_specs[self.__class__][3]:
            # PK is an identity column
            insert_data.pop(_db_specs[self.__class__][1], None)
            return_col = {_db_specs[self.__class__][1]: _db_specs[self.__class__][2]}

        # execute the INSERT statement
        op_errors: list[str] = []
        rec: tuple[Any] = db_insert(errors=op_errors,
                                    insert_stmt=f"INSERT INTO {_db_specs[self.__class__][0]}",
        ...
        if op_errors:
            msg = ("Error INSERTing into table "
                   f"{_db_specs[self.__class__][0]}: {'; '.join(op_errors)}")
    ...
    def update(self,
        ...
        key: int = update_data.pop(_db_specs[self.__class__][1])

        # execute the UPDATE statement
        op_errors: list[str] = []
        db_update(errors=op_errors,
                  update_stmt=f"UPDATE {_db_specs[self.__class__][0]}",
                  update_data=update_data,
                  where_data={_db_specs[self.__class__][1]: key},
                  min_count=1,
                  max_count=1,
                  connection=db_conn,
                  logger=self._logger)
    ...
    def delete(self,
        ...
        if self.id:
            where_data = {_db_specs[self.__class__][1]: self.id}
        else:
            where_data = self.to_columns(omit_nulls=True)
            where_data.pop(_db_specs[self.__class__][1], None)

        # execute the DELETE statement
        op_errors: list[str] = []
        result: int = db_delete(errors=op_errors,
                                delete_stmt=f"DELETE FROM {_db_specs[self.__class__][0]}",
                                where_data=where_data,
                                max_count=1,
                                connection=db_conn,
                                logger=self._logger)
    ...
    def set(self,
        ...
        for key, value in data.items():
            attr: str = (_attrs_map.get(self.__class__) or {}).get(key) or key
    ...
    def in_db(self,
        ...
        return db_exists(errors=errors,
                         table=_db_specs[self.__class__][0],
                         where_data=where_data,
                         connection=db_conn,
                         logger=self._logger)
    ...
    def load(self,
        ...
        recs: list[tuple] = db_select(errors=op_errors,
                                      sel_stmt=f"SELECT {', '.join(attrs)} "
                                               f"FROM {_db_specs[self.__class__][0]}",
                                      where_data=where_data,
                                      limit_count=2,
                                      connection=db_conn,
                                      logger=self._logger)
        ...
        if op_errors:
            ...
            msg = (f"No record found on table "
                   f"{_db_specs[self.__class__][0]} for {dict_stringify(where_data)}")
        elif len(recs) > 1:
        ...
        if msg:
            ...
            for inx, attr in enumerate(attrs):
                ...
                if attr == _db_specs[self.__class__][0]:
    ...
    def get_columns(self) -> list[str]:
        ...
        # PK attribute in DB table might have a different name
        result: list[str] = [_db_specs[self.__class__][1]]
    ...
    def to_columns(self,
        ...
        # PK attribute in DB table might have a different name
        result: dict[str, Any] = {_db_specs[self.__class__][1]: self.__dict__.get("id")}
    ...
    def data_to_params(self,
        ...
        for k, v in data.items():
            if not omit_nulls or v is not None:
                attr: str = dict_get_key(source=_attrs_map.get(self.__class__) or {},
                                         value=k) or k
                result[attr] = v
    ...
    def __load_references(self,
        ...
        op_errors: list[str] = []
        for cls in (_sob_references.get(self.__class__) or []):
    ...
    def initialize(db_specs: tuple[StrEnum | str, StrEnum | str, int | str] |
        ...
        if cls:
            ...
            _db_specs.update({cls: db_specs})
            if attrs_map:
                _attrs_map.update({cls: attrs_map})
            if sob_references:
                _sob_references.update({cls: sob_references})
    ...
    def retrieve(errors: list[str] | None,
        ...
        if not op_errors:
            recs: list[tuple[int | str]] = db_select(errors=op_errors,
                                                     sel_stmt=f"SELECT {_db_specs[cls][1]} "
                                                              f"FROM {_db_specs[cls][0]}",
                                                     where_data=where_data,
                                                     min_count=min_count,
                                                     max_count=max_count,
    ...
    def erase(errors: list[str] | None,
        ...
        result: int = db_delete(errors=op_errors,
                                delete_stmt=f"DELETE FROM {_db_specs[cls][0]}",
                                where_data=where_data,
                                connection=db_conn,
                                logger=logger)
        if op_errors:
