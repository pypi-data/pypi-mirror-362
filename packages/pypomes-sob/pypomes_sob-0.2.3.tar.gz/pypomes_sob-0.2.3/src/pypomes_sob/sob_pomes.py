...
class PySob:
    ...
    def __get_invoking_class(errors: list[str] = None,
        ...
        classname: str = context[:pos]
        filename: str = caller_frame_2.filename
        mark = "." + classname
        ...
        if not result and SOB_PATH_BASE:
            try:
                pos = filename.find(SOB_PATH_BASE)
                module_name: str = filename[pos+1:-3].replace("\\", ".").replace("/", ".")
                module: ModuleType = import_module(name=module_name)
                result = getattr(module,
                                 classname)
            except Exception as e:
