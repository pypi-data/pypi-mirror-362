...
class PySob:
    ...
    def set(self,
        ...
        for key, value in data.items():
            attr: str = PySob._attrs_map[self.__class__].get(key) or key
    ...
    def to_params(self,
        ...
        for key, value in self.__dict__.items():
            if key.islower() and not key.startswith("_") and \
                    (not omit_nulls or value is not None):
                attr: str = dict_get_key(source=PySob._attrs_map[self.__class__],
                                         value=key) or key
                result[attr] = value
    ...
    @staticmethod
    def initialize(db_specs: (StrEnum | str, StrEnum | str),
                   attrs_map: dict[StrEnum | str, StrEnum | str],
