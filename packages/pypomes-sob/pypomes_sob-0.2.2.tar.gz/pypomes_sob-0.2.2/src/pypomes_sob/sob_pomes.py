...
class PySob:
    ...
    def __get_invoking_class(errors: list[str] = None,
        ...
        caller_frame_1: FrameInfo = stack()[1]
        invoking_function: str = caller_frame_1.function
        mark: str = f".{invoking_function}("

        # obtain the invoking class
        caller_frame_2: FrameInfo = stack()[2]
        context: str = caller_frame_2.code_context[0]
        ...
        if not result and SOB_BASE_FOLDER:
            try:
                pos = caller_frame_2.filename.find(SOB_BASE_FOLDER) + len(SOB_BASE_FOLDER) + 1
                module_name: str = caller_frame_2.filename[pos:-3].replace("\\", ".").replace("/", ".")
                module: ModuleType = import_module(name=module_name)
                result = getattr(module,
                                 class_name)
            except Exception as e:
        ...
        if not result:
            msg: str = (f"Unable to obtain class '{class_name}' "
                        f"of invoking function '{invoking_function}'")
            if logger:
                logger.error(msg=f"msg - calling frames: {caller_frame_1} - {caller_frame_2}")
            if isinstance(errors, list):
                errors.append(msg)
