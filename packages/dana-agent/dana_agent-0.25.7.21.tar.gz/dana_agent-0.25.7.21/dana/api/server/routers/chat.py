...
async def chat_with_agent(
    request: schemas.ChatRequest, db: Session = Depends(get_db), chat_service: ChatService = Depends(get_chat_service)
):
    """
    Chat with an agent
    ...
    try:
        ...
        result = await chat_service.chat_with_agent(
            db=db, agent_id=request.agent_id, user_message=request.message, conversation_id=request.conversation_id, context=request.context
        )
