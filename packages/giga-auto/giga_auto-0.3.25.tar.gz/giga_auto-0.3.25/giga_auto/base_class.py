...
class ApiYamlBase(RequestBase,metaclass=SingletonMeta):
    ...
    def __init__(self, service=None, account_key="admin", extra_headers: dict = None, **kwargs):
        ...
        self.kwargs = kwargs
        super().__init__(base_url=self.config["HOST"], expect_code=kwargs.get('expect_code','0000'))
    ...
    def _request(self, method, url, **kwargs):
        ...
        if response.status_code == UNAUTHORIZED and self.kwargs.get('need_login',True):
    ...
    def _apply_prepare(kwargs):
        ...
        if "files" in kwargs:
            ...
            for k, value in files:
                file_metadata.append((k, value[0]))
            allure_data["files"] = file_metadata
        allure.attach(body=json.dumps(allure_data), name="接口请求信息", attachment_type=allure.attachment_type.JSON)
...
class BasicInfo(metaclass=SingletonMeta):
    _data = {}
    _config_key = 'config'  # 提取配置项常量
    ...
    def get_header(self, service: str, role: str) -> dict:
        """
    ...
    def has_config(self) -> bool:
        ...
        检查是否存在配置
        """
        return self._config_key in self._data

    def get_config(self) -> dict:
        """
        获取配置
        """
        return self._data.get(self._config_key, {})

    def set_config(self, value: dict):
        """
        设置配置
        """
        self._data[self._config_key] = value
    ...
    def __repr__(self):
        ...
        返回当前实例的字符串表示，方便调试
        """
        return f"<Basic INFO: {self._data}>"