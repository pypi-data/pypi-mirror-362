...
/// Recursively find all directories to process for cleaning.
fn recursive_collect_dirs(
    current_dir: &Path,
    dir_patterns: &[Pattern],
    dirs_set: &mut HashSet<PathBuf>
) -> anyhow::Result<()> {
    if !current_dir.is_dir() {
        return Ok(());
    }
    // Add the current directory to the set to be processed.
    dirs_set.insert(current_dir.to_path_buf());
    ...
    for entry in fs::read_dir(current_dir)? {
        ...
        // Descend into visible, non-ignored directories.
        if path.is_dir() && !name.starts_with('.') && !dir_patterns.iter().any(|p| p.is_match(&name)) {
            recursive_collect_dirs(&path, dir_patterns, dirs_set)?;
        }
    }
...
fn run_cli<I: IntoIterator<Item = String>>(raw_args: I) -> anyhow::Result<()> {
    ...
    if cli.clean {
        ...
        // 1. Recursively collect all directories that could have had prompts generated for them.
        let mut dirs_to_check = HashSet::new();
        recursive_collect_dirs(start_path, &dir_patterns, &mut dirs_to_check)?;
        
        let mut cleaned_count = 0;
        let outpath = Path::new(&cli.outpath);

        // 2. For each discovered directory, perform a dual check.
        for dir_path in dirs_to_check {
            // Correctly determine the directory's name, handling the special case of ".".
            let dir_name = if dir_path == Path::new(".") {
                env::current_dir()?.file_name().and_then(|s| s.to_str()).map(String::from)
            } else {
                dir_path.file_name().and_then(|s| s.to_str()).map(String::from)
            };

            if let Some(name) = dir_name {
                let prompt_filename = format!("{}_prompt.txt", name);

                // Check 1: In the global output path (for `d2p <dir>`).
                let global_prompt_path = outpath.join(&prompt_filename);
                if global_prompt_path.is_file() {
                    fs::remove_file(&global_prompt_path)?;
                    println!("Removed {}", global_prompt_path.display());
                    cleaned_count += 1;
                }

                // Check 2: Locally inside the directory itself (for `cd <dir> && d2p .`).
                let local_prompt_path = dir_path.join(&prompt_filename);
                // Avoid double-deleting if outpath is the same as the dir being checked
                if global_prompt_path != local_prompt_path && local_prompt_path.is_file() {
                    fs::remove_file(&local_prompt_path)?;
                    println!("Removed {}", local_prompt_path.display());
                    cleaned_count += 1;
                }
            }
        }
