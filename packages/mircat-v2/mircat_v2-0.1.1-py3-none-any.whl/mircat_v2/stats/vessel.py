...
class Vessel:
    ...
    def create_centerline(self, **kwargs):
        ...
        if self.skeleton is None:
            logger.debug("Skeleton not created yet. Creating skeleton first.")
    ...
    def process_centerline(
        self, target_points: int, smoothing_factor: float = 0.5, degree: int = 3
    ):
        """
    ...
    def create_straightened_cpr(self, radius: int = 200):
        """
    ...
    def calculate_straightened_cpr_cross_section_stats(self):
        ...
        stats = {
            idx: self._calculate_cross_section_stats(
                cross_section, pixel_spacing=cross_section_spacing
            )
            for idx, cross_section in enumerate(self.cpr)
        }
        assert len(stats) == len(self.cpr) == len(self.centerline), "Stats length mismatch with CPR or centerline."
        self.cpr_stats = stats
        return self

    @staticmethod
    def _calculate_cross_section_stats(
...
def _create_straightened_cpr(
    ...
    """
    Create a straightened CPR (Curved Planar Reformation) from a centerline and relevant tangent vectors.
