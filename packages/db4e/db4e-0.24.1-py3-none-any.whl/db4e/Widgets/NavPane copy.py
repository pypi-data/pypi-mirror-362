...
class NavPane(Container):
    ...
    def refresh_nav_pane(self) -> None:
        ...
        if self.is_initialized():
            ...
            # The Monero tree
            monero_node = self.depls.root.add(ICON['MON'] + MONEROD_SHORT_LABEL)
            ...
            monero_node.add_leaf(ICON['NEW'] + NEW_LABEL)
            monero_node.expand()

            # The P2Pool tree
            p2pool_node = self.depls.root.add(ICON['P2P'] + P2POOL_SHORT_LABEL)
            ...
            p2pool_node.add_leaf(ICON['NEW'] + NEW_LABEL)
            p2pool_node.expand()

            # Only display XMRig if a P2Pool deployment exists
            xmrig_node = self.depls.root.add(ICON['XMR'] + XMRIG_SHORT_LABEL)
            ...
            if len(p2pool_node.children) > 1:
                xmrig_node.add_leaf(ICON['NEW'] + NEW_LABEL)
            xmrig_node.expand()
