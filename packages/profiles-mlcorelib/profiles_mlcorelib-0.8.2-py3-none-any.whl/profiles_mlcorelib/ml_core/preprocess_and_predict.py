...
def preprocess_and_predict(
    ...
    def predict_helper_sf(df, pkl_model_file_name: str) -> Any:
        df.columns = column_names
        # Df has entity-col too. We split this into index col, and feature df, generate predictions only on feature df, join it back with entity-col
        # Being a pandas dataframe, the order is preserved.
        index_col = df[trainer.entity_column.upper()]
        df = df.drop(trainer.entity_column.upper(), axis=1)
        df = utils.transform_null(
            df, numeric_columns, categorical_columns, timestamp_columns
        )
        ...
        trained_model = load_model(pkl_model_file_name)
        df.columns = [x.upper() for x in df.columns]
        predictions = trainer.predict(trained_model, df)
        predictions = pd.concat([index_col, predictions], axis=1)
        return predictions
    ...
    if creds["type"] == "snowflake":
        ...
        required_features_upper_case.update([trainer.entity_column.upper()])
        ...
        @F.pandas_udtf(
            ...
            output_schema=PandasDataFrameType(
                ...
                [
                    trainer.entity_column.upper(),
                    pycaret_score_column,
        ...
        class predict_scores:
            def end_partition(self, df):
                predictions = predict_helper_sf(df, pkl_model_file_name)

                # Create a new DataFrame with the extracted column names
                prediction_df = pd.DataFrame()
                prediction_df[trainer.entity_column.upper()] = predictions[
                    trainer.entity_column.upper()
                ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project_folder_path", type=str)
    parser.add_argument("--site_config_path", type=str)
    ...
    wh_creds = utils.parse_warehouse_creds(
        args.project_folder_path, args.site_config_path, args.mode
    )
