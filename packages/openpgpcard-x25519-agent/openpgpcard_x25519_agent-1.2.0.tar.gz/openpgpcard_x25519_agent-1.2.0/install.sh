...
repo_url="${REPO_URL:-https://git.sr.ht/~arx10/openpgpcard-x25519-agent/blob/main}"
...
# create pcsc group for pcsc access through polkit
sudo groupadd --system pcsc
sudo useradd --system openpgpcard-x25519-agent --gid pcsc
if [ -d /etc/polkit-1/rules.d ]; then
    wget -O - "$repo_url/etc/polkit-pcsc.rules" |
        sudo tee "/etc/polkit-1/rules.d/99-pcsc.rules" > /dev/null
fi
