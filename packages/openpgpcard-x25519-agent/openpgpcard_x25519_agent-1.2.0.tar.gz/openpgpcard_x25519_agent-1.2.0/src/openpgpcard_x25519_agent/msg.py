    ...
    *other party's* public key)::
    ...
    def format_into_buffer(self):
        ...
        if self.message_type == SUCCESS and self.extension_type:
            ...
            self.format_failure()
        elif self.message_type in (ADD_SMARTCARD_KEY, ADD_SMARTCARD_KEY_CONSTRAINED):
    ...
    def parse_add_smartcard_key_constraint(self, offset):
        ...
        if constraint_type == CONSTRAIN_LIFETIME:
            ...
            return offset + 4
        if constraint_type == CONSTRAIN_CONFIRM:
            self.constrain_confirm = True
            return offset
        if constraint_type == CONSTRAIN_EXTENSION:
            extension_name = self.extract_string(offset)
            raise MessageError(
                f"received unimplemented constraint extension: {extension_name}"
            )
        raise MessageError(f"received unimplemented constraint type: {constraint_type}")
