...
def run_server(socket=None, card_id=None):
    ...
    signal(SIGINT, lambda _number, _frame: server.stop())
    signal(SIGTERM, lambda _number, _frame: server.stop())
    server.start()
...
class Server:
    ...
    def handle_request(self, request, response):
        ...
        elif request.message_type in (ADD_SMARTCARD_KEY, ADD_SMARTCARD_KEY_CONSTRAINED):
    ...
    def derive_shared_secret(self, request, response):
        ...
        seat = self.get_seat(request.reader_id)
        card = seat.get_card()
        pin = seat.derive_pin(card)
        response.shared_secret = calculate_shared_secret(card, request.public_key, pin)
    ...
    def add_smartcard_key(self, request, response):
        ...
        expires = None
        if lifetime:
            expires = datetime.fromtimestamp(_now().timestamp() + lifetime)  # noqa: DTZ006

        seat = self.get_seat(request.reader_id)
        seat.set_pin(request.pin.copy(), expires=expires)
        response.message_type = SUCCESS
    ...
    def get_seat(self, reader_id):
        ...
        try:
            ...
            raise ValueError(f"server does not have requested seat: {reader_id}") from e
...
class Seat:
    ...
    Attributes:
        id (str): Card ID.
        pin (bytearray): PIN.
        derived (bool): True if KDF has been run on PIN.
        expires (datetime): PIN expiration.
    """

    def __init__(self, card_id=None, pin=None, derived=False, expires=None):  # noqa: FBT002
    ...
    def derive_pin(self, card):
        """Runs the specified card's KDF on this seat's PIN, if not already run.

        Replaces the seat's un-derived PIN with the derived PIN, so that the
        PIN cached in memory will now be the derived version.

        Arguments:
            card (OpenPGPcard): Card object.

        Returns:
            bytearray: PIN after running KDF.
        """
        pin = self.get_pin()
        if not pin or self.derived:
            return pin

        self.pin = get_kdf(card).derive(pin)
        self.derived = True
        if pin != self.pin:
    ...
    def get_pin(self):
        ...
        This may return the un-derived or derived PIN; use `derive_pin(card)`
        to always retreive the derived PIN.
    ...
    def set_pin(self, pin, derived=False, expires=None):  # noqa: FBT002
