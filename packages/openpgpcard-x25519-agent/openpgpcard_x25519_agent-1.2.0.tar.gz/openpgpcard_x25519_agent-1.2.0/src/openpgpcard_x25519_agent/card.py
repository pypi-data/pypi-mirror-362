...
def list_all_cards():
    ...
    """
    cards = []
    for i in range(count_all_cards()):
...
def get_kdf(card):
    """Queries card for PIN KDF in use.
    ...
    """
    try:
        return Kdf().parse(card.get_data("F9"))
    except PGPCardException:
        return Kdf()
...
def _log_command(cla, ins, p1, p2, lc):
    s = "sending command to card: %x %x %x %x + %x bytes"
    getLogger(__name__).debug(s, cla, ins, p1, p2, lc)


def _log_response(sw1, sw2, length):
    s = "received response from card: %x %x + %x bytes"
    getLogger(__name__).debug(s, sw1, sw2, length)
...
def verify_pin(card, pin, slot=ENCRYPTION_PIN):
    ...
    try:
        ...
        if e.sw_code & 0xFFF0 == 0x63C0:
            raise PinException(e.sw_code - 0x63C0) from e
        raise
...
def test_card(card_id=None):  # noqa: PT028
