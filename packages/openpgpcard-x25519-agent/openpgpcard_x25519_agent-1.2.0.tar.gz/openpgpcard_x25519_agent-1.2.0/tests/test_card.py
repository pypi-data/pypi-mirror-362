...
from openpgpcard_x25519_agent.kdf import NONE as KDF_NONE
from openpgpcard_x25519_agent.kdf import Kdf
...
@pytest.mark.usefixtures("all_cards_mock")
def test_count_all_cards_when_none():
    assert count_all_cards() == 0


@pytest.mark.usefixtures("five_cards_mock")
def test_count_all_cards_when_multiple():
    assert count_all_cards() == 5


@pytest.mark.usefixtures("all_cards_mock")
def test_list_all_cards_when_none():
    assert list_all_cards() == []


@pytest.mark.usefixtures("five_cards_mock")
def test_list_all_cards_when_multiple():
...
@pytest.mark.usefixtures("card_mock")
def test_get_card_by_index_when_found():
    assert get_card_by_index(63).index == 63


@pytest.mark.usefixtures("card_mock")
def test_get_card_by_index_or_none_when_found():
...
@pytest.mark.usefixtures("all_cards_mock")
def test_get_card_by_id_when_not_found():
...
@pytest.mark.usefixtures("all_cards_mock")
def test_get_default_card_when_none():
    with pytest.raises(ConnectionException):
        get_default_card()


@pytest.mark.usefixtures("five_cards_mock")
def test_get_default_card_when_multiple():
    assert get_default_card().index == 1


def test_get_kdf_when_none():
    card = MagicMock()
    card.get_data.return_value = [0x81, 0x01, 0x00]

    assert get_kdf(card).algorithm == KDF_NONE


def test_get_kdf_when_data_object_not_found():
    card = MagicMock()
    card.get_data.side_effect = PGPCardException(0x6A, 0x88)

    assert get_kdf(card).algorithm == KDF_NONE


def test_get_kdf_when_connection_error():
    card = MagicMock()
    card.get_data.side_effect = ConnectionException("test")
...
def test_cli_test_card_when_success(five_cards_mock, mocker):
    ...
    mocker.patch("openpgpcard_x25519_agent.card.get_kdf", return_value=Kdf())
