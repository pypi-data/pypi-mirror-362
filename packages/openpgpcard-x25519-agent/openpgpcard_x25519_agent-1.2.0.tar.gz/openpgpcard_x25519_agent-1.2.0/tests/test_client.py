...
def test_input_pin_when_no_options(mocker):
    ...
    assert not input_pin(source, "foo", "bar", False, "30m")  # noqa: FBT003
...
def test_print_shared_secret_when_all_options(mocker):
    ...
    assert print_shared_secret(source, "foo", "bar", False, output)  # noqa: FBT003
...
def test_read_public_key_when_prompt(capsys, mocker):
    ...
    assert read_public_key(BytesIO(b"foo"), True) == public_key  # noqa: FBT003
