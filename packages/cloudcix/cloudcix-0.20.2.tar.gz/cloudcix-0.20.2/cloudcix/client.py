...
class Client:
    ...
    def request(self, method, token=None, pk=None, data=None, params=None, files=None, **kwargs):
        ...
        def _dump():
            ...
            def wrapper(**kw):
                return json.loads(response.content, cls=cloudcix.utils.JSONDecoder)
            return wrapper
...
def parse_parameters(params):
    ...
    for k, v in params.items():
        ...
        # Exclude strings from being treated as generic iterables
        elif hasattr(v, '__iter__') and not isinstance(v, str):
            # Process iterables but retain spaces within strings
            params[k] = str([
                item if isinstance(item, str) else str(item).replace(' ', '')
                for item in v
            ])
    return params
