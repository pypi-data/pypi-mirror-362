...
@patch('azpaddypy.mgmt.local_env_manager.LocalDevelopmentSettings')
def test_create_local_env_manager(mock_manager_class, temp_dir):
    ...
    create_local_env_manager(
        file_path=str(dotenv_path),
        settings=dict_settings
    )

    mock_manager.load_from_json.assert_called_once_with(json_path, override=True)
    mock_manager.load_from_dotenv.assert_called_once_with(dotenv_path, override=True)
    mock_manager.apply_settings.assert_called_once_with(dict_settings, override=True)
