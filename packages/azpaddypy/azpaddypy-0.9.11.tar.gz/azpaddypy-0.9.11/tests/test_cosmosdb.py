...
def azure_cosmosdb(mock_azure_identity, mock_cosmos_client):
    ...
    with patch('azpaddypy.resources.cosmosdb.CosmosClient', return_value=mock_cosmos_client):
...
class TestAzureCosmosDBInitialization:
    ...
    @patch('azpaddypy.resources.cosmosdb.CosmosClient')
    def test_init_with_identity(self, mock_client_class, mock_azure_identity):
        ...
        mock_client_class.assert_called_once_with(
            ...
            credential=mock_azure_identity.get_credential.return_value
        )
...
class TestAzureCosmosDBSyncOperations:
    ...
    def test_get_database(self, azure_cosmosdb, mock_cosmos_client):
        ...
        db = azure_cosmosdb.get_database("test_db")
        mock_cosmos_client.get_database_client.assert_called_once_with("test_db")
    ...
    def test_get_container(self, azure_cosmosdb, mock_cosmos_client):
        ...
        container = azure_cosmosdb.get_container("test_db", "test_container")
        mock_cosmos_client.get_database_client.assert_called_once_with("test_db")
        db_proxy = mock_cosmos_client.get_database_client.return_value
        db_proxy.get_container_client.assert_called_once_with("test_container")
        assert isinstance(container, ContainerProxy)
    ...
    def test_query_items_with_caching(self, azure_cosmosdb):
        ...
        azure_cosmosdb.query_items(
            "db", "container", "SELECT * FROM c",
...
class TestAzureCosmosDBAsyncOperations:
    ...
    @patch('azpaddypy.resources.cosmosdb.AsyncCosmosClient')
    async def test_async_client_context(self, mock_async_client_class, azure_cosmosdb, mock_credential):
        """Test the async client context manager."""
        mock_async_client = AsyncMock(spec=AsyncCosmosClient)
...
class TestAzureCosmosDBDeleteItem:
    ...
    def test_delete_item_error(self, azure_cosmosdb):
        ...
        azure_cosmosdb.logger.error.assert_called()
...
class TestFactoryFunction:
    ...
    def test_create_azure_cosmosdb(self, mock_cosmos_class, mock_azure_identity):
        ...
        call_args = mock_cosmos_class.call_args[1]
        assert call_args['endpoint'] == "https://test.documents.azure.com:443/"
        assert call_args['azure_identity'] == mock_azure_identity
        assert call_args['service_name'] == "factory_service" 