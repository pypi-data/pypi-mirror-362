...
from prim_cli.entities.agent_environment import AgentEnvironmentRepository
from prim_cli.entities.voice import VoiceRepository
...
def agent_info(name_or_id: str = typer.Argument(..., help="Agent name or ID.")):
    ...
    if agent.default_voice_id:
        voice = next(
            (v for v in voice_repository.get() if v.id == agent.default_voice_id), None
        )
        if voice:
            voice_name = voice.name or UNNAMED
...
def agent_create():
    ...
    voice = None
    while not voice:
        voice_name_or_id = prompt("Enter voice name or ID", default="", validation=None)
        ...
        voice = next(
            (
                v
                for v in voice_repository.get()
                if v.id == voice_name_or_id
                or v.name.lower() == voice_name_or_id.lower()
            ),
            None,
        )
        if voice:
            agent.default_voice_id = voice.id
...
def agent_update(name_or_id: str = typer.Argument(..., help="Agent name or ID.")):
    ...
    voice_name_or_id = prompt(
        "Enter new voice name or ID", default=current_voice_name, validation=None
    )
    voice = next(
        (
            v
