...
class AgentEnvironmentRepository(BaseRepository[AgentEnvironment]):
    ...
    def get_all(self) -> list[AgentEnvironment]:
        """Get all environments including those with deleted_at set."""
        # Query with a filter to include deleted environments
        query = {"include_deleted": "true"}
        return self.get(query)

    def revive(self, environment: AgentEnvironment) -> AgentEnvironment:
        """Revive a deleted environment by setting deleted_at to None."""
        environment.deleted_at = None
        return self.update_by_id(environment.id, environment)
