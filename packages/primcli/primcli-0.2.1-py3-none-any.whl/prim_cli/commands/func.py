...
def func_create(
    ...
    if not directory:
        ...
        while True:
            ...
            if name not in func_names:
                break
            print_error(
                f"Function name '{name}' is already taken. Please choose a different name."
            )
    ...
    if created_function:
        if name:
            ...
            agent_function_repository.update_by_id(
                created_function.id, created_function
            )
...
def func_update(
    ...
    if (
        ...
        print_error(
            f"Function '{function.name}' is already deployed to environment '{env.name}' and cannot be updated. Create a new function instead."
        )
        return
    ...
    if not directory:
        ...
        while True:
            name = prompt("Enter new function name", default=function.name)
            if name not in func_names:
                break
            print_error(
                f"Function name '{name}' is already taken. Please choose a different name."
...
def func_debug(
    ...
    print_info(
        f"Running function '{function.name}' in environment '{environment.name}'..."
    )
