...
try:
    ...
    print_info("pip install websockets numpy rich")
    sys.exit(1)
...
class PrimVoicesDebugger:
    ...
    async def get_agent_configuration(self) -> Dict[str, Any]:
        ...
        query_params = {"inputType": "mic", "environment": self.config.environment.name}
            ...
            while self.is_listening and self.is_connected:
                ...
                if audio_chunk is not None:
                    ...
                    if not self.is_playing:
                        ...
                        is_interruption, buffered_chunks = self.audio_input.detect_interruption(audio_chunk, echo_buffer)
...
[{TITLE_STYLE}]Monitoring:[/{TITLE_STYLE}]
  ...
  echo        - Show echo alignment state
        ...
        while self.running:
            try:
                ...
                if cmd == "quit" or cmd == "exit" or cmd == "q" or cmd == "x":
                    ...
                    echo_state = self.debugger.audio_input.get_echo_alignment_state()
                    print_info("Echo Alignment State:")
                    for key, value in echo_state.items():
