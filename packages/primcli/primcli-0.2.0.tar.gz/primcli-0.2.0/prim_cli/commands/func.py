...
def func_create(
    ...
    if not directory:
        ...
        while True:
            name = prompt("Enter function name")
            if name not in func_names:
                break
            print_error(f"Function name '{name}' is already taken. Please choose a different name.")
    ...
    if created_function:
        if name:
            created_function.name = name
            agent_function_repository.update_by_id(created_function.id, created_function)
        print_success(f"Function '{created_function.name}' created successfully.")
    else:
...
def func_update(
    ...
    if env and not env.is_debug: # cannot update function if it's deployed to a non-debug environment
        print_error(f"Function '{function.name}' is already deployed to environment '{env.name}' and cannot be updated. Create a new function instead.")
        return

    if not directory:
        agent_function_repository = AgentFunctionRepository(agent.id)
        functions = agent_function_repository.get()
        func_names = [f.name for f in functions if f.id != function.id]
        while True:
            name = prompt(
        ...
        if confirm("View current function code?", default=False):
            ...
            if code:
                ...
                print_warning("No code found for this function.")
        print_info("Provide a directory path for the function code, or hit enter to keep the same code.")
        directory = prompt("Enter directory path", default="", validation=None)
        if not directory:
            print_success(f"Function '{function.name}' updated successfully.")
            return
        directory = Path(directory).expanduser()
        if not directory.exists():
            print_error(
    ...
    if updated_function:
        print_success(f"Function '{updated_function.name}' updated successfully.")
    else:
...
def func_delete(
    ...
    if confirm(f"Are you sure you want to delete function '{function.name}'?"):
        ...
        if is_deleted:
            ...
            print_error(f"Failed to delete function '{function.name}'.")
...
def func_deploy(
    ...
    if confirm(
        f"Deploy function '{function.name}' to environment '{environment.name}'?",
        default=True,
    ):
        ...
        if updated_environment:
            print_success(
                f"Function '{function.name}' deployed to environment '{environment.name}' successfully."
            )
        else:
            print_error(
                f"Failed to deploy function '{function.name}' to environment '{environment.name}'."
            )
    else:
        print_info("Deployment cancelled.")
...
def func_debug(
    ...
    if env_name_or_id:
        ...
        if not environment.function_id == function.id:
            print_error(
                f"Environment '{env_name_or_id}' is not associated with function '{function.name}'. "
                f"Deploy function '{function.name}' to environment '{env_name_or_id}' first, "
                f"or run func debug without --env to use the debug environment."
            )
            return
        print_info(f"Running function '{function.name}' in selected environment '{env_name_or_id}'...")
    else:
        # get debug environment
        debug_id = agent.debug_environment_id
        if not debug_id or not (debug_env := agent_environment_repository.get_by_id(debug_id)):
            # no debug environment set for this agent, create a new one
            debug_env = agent_environment_repository.create(AgentEnvironment(
                agent_id=agent.id,
                name=DEBUG_ENV_NAME,
            ...
            agent.debug_environment_id = debug_env.id
            print_info(f"Running function '{function.name}' in new debug environment '{debug_env.name}'...")
        else:
            print_info(f"Running function '{function.name}' in existing debug environment '{debug_env.name}'...")
