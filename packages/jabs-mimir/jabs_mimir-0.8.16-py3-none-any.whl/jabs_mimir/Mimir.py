...
class Mimir:
    ...
    def renderField(self, typeName, parent, row, label=None, variable=None, **fieldOptions):
        """
        Renders a registered field renderer directly, outside of a block,
        by name. Users can call this to reuse custom field types.

        Args:
            typeName (str): The name of the field type (must be registered).
            parent (Widget): Where to render the field.
            row (int): Grid row.
            label (str): Optional label.
            variable (Any): Optional variable (e.g., StringVar).
            fieldOptions (dict): Additional keys passed into the field dict.

        Returns:
            The rendered widget or frame.
        """
        renderer = self.fieldRenderers.get(typeName)
        if not renderer:
            raise ValueError(f"Unknown field type: {typeName}")
        ...
        return renderer(parent, row, field)
    ...
    def _renderHeading(self, parent, row, field):
        ...
        align = field.get("options", {}).get("align", "w")
    ...
    def addButton(self, parent, row, command, label="OK", validate=True, tooltip=None, column=None, columnspan=None, sticky="EW"):
        """
        ...
        return self._renderButton(parent, row, wrapped, label, tooltip, column, columnspan, sticky)

    def _renderButton(self, parent, row, command, label, tooltip, column, columnspan, sticky):
        """
        ...
        btn.grid(row=row, column=column, columnspan=columnspan, padx=5, pady=5, sticky=sticky)
    ...
    def addLabeledCombobox(self, parent, row, label, variable, values, tooltip=None, state="readonly", column=0, vertical=False, columnspan=1, options=None, sticky="ew"):
        """
        ...
        if vertical:
            ...
            combo.grid(row=row + 1, column=column, columnspan=columnspan, padx=5, pady=(0, 10), sticky=sticky)
        else:
            tb.Label(parent, text=label).grid(row=row, column=column, padx=5, pady=5, sticky="e")
            combo = tb.Combobox(parent, textvariable=variable, values=values, state=state, width=self.widthCombo)
            combo.grid(row=row, column=column + 1, columnspan=columnspan, padx=5, pady=5, sticky=sticky)
    ...
    def addInputGroup(self, parent, row, label, variable, state=None, column=0, vertical=False, columnspan=1, options=None, sticky="ew", labelSticky="ew", button=True, buttonLabel="OK", buttonCommand:Callable = None, buttonOptions=None):
        """
        Add a bootstrap style inputgroup where you have a entry field and a button, also possible to have no button in the group
        Options Wildcard, pass a dict with standard tkinter/ttkbootstrap options, use buttonOptions for wildcard passthrough for button styling
        """
        buttonGrpFrame = tb.Frame(parent)
        buttonGrpFrame.grid(row=row, column=column, columnspan=columnspan, sticky=sticky, padx=5, pady=5)

        if not vertical:
            label = tb.Label(buttonGrpFrame, text=label)
            label.grid(row=row, column=0, padx=10, pady=5, sticky=labelSticky)
            entry = tb.Entry(buttonGrpFrame).grid(row=row, column=1, columnspan=columnspan, sticky=sticky)
            if button:
                if buttonCommand:
                    buttonCommand()
                button = tb.Button(buttonGrpFrame, text=buttonLabel, command=buttonCommand).grid(row=row, column=2)
            buttonGrpFrame.grid_columnconfigure(1, weight=1)
        if vertical:
            label = tb.Label(buttonGrpFrame, text=label)
            label.grid(row=0, column=0, padx=10, pady=5, columnspan=2, sticky=labelSticky)
            entry = tb.Entry(buttonGrpFrame).grid(row=1, column=0, columnspan=columnspan, sticky=sticky)
            if button:
                if buttonCommand:
                    buttonCommand()
                button = tb.Button(buttonGrpFrame, text=buttonLabel, command=buttonCommand).grid(row=1, column=1)
            buttonGrpFrame.grid_columnconfigure(0, weight=1)
    ...
    def addInfoBox(self, parent, row, text=None, textvar=None, label=None, column=0, columnspan=1, border=False, fontsize=None, options=None):
        """
    ...
    def createContextMenu(self, widget, actions: dict):
        def showMenu(event):
            ...
            for label, func in actions.items():
                menu.add_command(label=label, command=(lambda f=func, e=event: lambda: f(e))())
            try:
