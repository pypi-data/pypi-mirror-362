...
# function to check os has apt or yum
def check_os():
    try:
        subprocess.run("which apt".split(), check=True, capture_output=True)
        return "apt"
    except CalledProcessError as e:
        try:
            subprocess.run("which yum".split(), check=True, capture_output=True)
            return "yum"
        except CalledProcessError as e:
            return None
...
def main():
    ...
    def _unzip_if_necessary():
        ...
        # if check_os() == "yum":
        #     os.system("sudo yum install -y --disablerepo=nvidia-ml --disablerepo=cuda --disablerepo=epel p7zip > /dev/null")
        #     os.system("sudo ln -s /usr/bin/7za /usr/bin/7z > /dev/null")
        if os.path.exists("/usr/bin/apt"):
            os.system("sudo apt install -y p7zip-full > /dev/null")
        try:
            dirname = os.path.dirname(pkg_installed_path)
            if "PASSWORD_7Z" in os.environ:
                cmd = f"sudo env LD_LIBRARY_PATH=/usr/local/gcc/lib64 7z x {zip_file} -aoa -p{os.environ['PASSWORD_7Z']} ".split() + [f"-o{dirname}"]
            else:
                cmd = f"sudo env LD_LIBRARY_PATH=/usr/local/gcc/lib64 7z x {zip_file} -aoa ".split() + [f"-o{dirname}"]
    ...
    def install_useful_tools():
        ...
        if check_os() == "yum":
            script_path = os.path.join(
                pkg_installed_path, "scripts/install_useful_tools_centos.sh"
            )
