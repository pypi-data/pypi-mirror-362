...
def init(file, interactive, backend_host, frontend_host):
    ...
    while attempt <= max_attempts:
        ...
        try:
            ...
            if attempt == max_attempts:
                click.secho("✗ Max retry attempts reached.", fg="red", bold=True)
                return
            attempt += 1
...
def new_version(pipeline_id):
    ...
    try:
        ...
        # Optional Git commit
        if Path('.git').exists() and click.confirm(click.style("Auto-commit this version to Git?", fg="bright_blue"), default=True):
            repo = git.Repo()
            repo.index.add([str(schema_path)])
            repo.index.commit(f"Add schema version {new_version} for {pipeline_id}")
            click.secho(f"✓ Committed to Git: {new_version}", fg="bright_green", bold=True)
