...
def update_current_schema(client, db_name, pipeline_id, version=None):
    ...
    if schema:
        ...
        if not current_schema_path.exists() or os.access(current_schema_path, os.W_OK):
            ...
            current_schema_path.chmod(stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)  # Add write permission for user
...
def copy_schema_file(pipeline_id, old_version, new_version):
    ...
    if old_path.exists():
        ...
        schema["metadata"]["parent_version"] = old_version
        schema["metadata"]["status"] = "draft"
        schema["metadata"]["changelog"] = [f"Auto-generated new version {new_version}"]
        new_path.parent.mkdir(parents=True, exist_ok=True)
        with new_path.open("w") as f:
            json.dump(schema, f, indent=2)
        new_path.chmod(stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)  # Add write permission for user
...
def init(file, interactive, backend_host, frontend_host):
    ...
    mongod_process = None  # Initialize mongod_process locally
    show_banner()
    ...
    schema_dir = Path(f"schemas/{pipeline_id}")
    schema_dir.mkdir(parents=True, exist_ok=True)
    schema_path = Path(file) if file else schema_dir / f"v0.1.0.json"
    with schema_path.open("w") as f:
        json.dump(schema, f, indent=2)
    schema_path.chmod(stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IROTH)  # Add write permission for user
...
def new_version(pipeline_id):
    ...
    try:
        ...
        # Do not push to MongoDB here; user must use schema-push manually
        # update_current_schema(client, db_name, pipeline_id, new_version)  # Commented out to avoid read-only issue
