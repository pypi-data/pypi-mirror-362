...
import datetime
from contextchain.engine.validator import validate_schema  # Import for schema validation
import json
...
class VersionManager:
    ...
    def push_schema(self, pipeline_id: str, schema: Dict[str, Any], increment: bool = False, is_initial: bool = False) -> str:
        ...
        # Disable automatic increment; rely on schema's specified version
        if increment:
            logger.warning("Automatic version incrementation is disabled. Use schema_version field manually.")
        timestamp = datetime.datetime.utcnow().isoformat() + "Z"
        # Check for duplicate version only for non-initial pushes
        if not is_initial:
