...
def copy_schema_file(pipeline_id, old_version, new_version):
    ...
    old_path = Path(f"schemas/{pipeline_id}_{old_version}.json")
    new_path = Path(f"schemas/{pipeline_id}_{new_version}.json")
    if old_path.exists():
...
def configure_task(i, interactive, allowed_task_types, tasks):
    ...
    if task_type == "LLM" and interactive:
        ...
        # Add granularity parameter option for LOCAL tasks
...
def init(file, interactive, backend_host, frontend_host):
    ...
    if any(task["task_type"] == "LLM" for task in tasks) and interactive:
        if click.confirm(click.style("Configure LLM settings for this pipeline?", fg="bright_blue"), default=False):
            ...
            llm_config["api_key_env"] = click.prompt(click.style("Environment variable for API key (e.g., OPENROUTER_API_KEY)", fg="bright_blue"), default="OPENROUTER_API_KEY")
            llm_config["api_key"] = click.prompt(click.style("Direct API key (optional, leave empty if using env variable)", fg="bright_blue"), default="") if not llm_config["api_key_env"] else ""
    ...
    schema_path = Path(file) if file else Path(f"schemas/{pipeline_id}_v0.1.0.json")
...
def schema_push(file):
    """Push a schema to MongoDB with automatic versioning."""
    ...
    try:
        ...
        db_name = config["db_name"]
        validate_schema(schema)
        old_version = schema["schema_version"]
        vm = VersionManager(client, db_name)
        new_version = vm.increment_version(schema["pipeline_id"])
        schema["schema_version"] = new_version
        push_schema(client, db_name, schema)
        click.secho(f"✓ Schema {schema['pipeline_id']} pushed to MongoDB with version {new_version}.", fg="bright_green", bold=True)
        update_current_schema(client, db_name, schema["pipeline_id"])
        copy_schema_file(schema["pipeline_id"], old_version, new_version)
...
def schema_version(pipeline_id, type):
    ...
    try:
        ...
        if schema_path.exists():
            ...
            update_current_schema(client, db_name, pipeline_id)
            copy_schema_file(pipeline_id, latest_version["schema_version"], new_version)
        else:
