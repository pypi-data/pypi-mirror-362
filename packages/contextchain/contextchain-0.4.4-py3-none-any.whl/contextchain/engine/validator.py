...
def validate_schema(schema: Dict[str, Any], is_initial: bool = False) -> None:
    ...
    # Skip duplicate version check for initial schema creation
    if not is_initial:
        from contextchain.registry.version_manager import VersionManager  # Import here to avoid circular import
        vm = VersionManager()
        existing_versions = vm.list_versions(schema["pipeline_id"])
        current_version = schema["schema_version"]
        if any(v["schema_version"] == current_version for v in existing_versions):
            logging.error(f"Version {current_version} already exists for pipeline {schema['pipeline_id']}")
            raise ValueError(f"Duplicate version {current_version} detected for pipeline {schema['pipeline_id']}. Please use a unique version number.")
    ...
    for task in tasks:
        ...
        if "parameters" in task and task["parameters"]:
            ...
            for key, value in task["parameters"].items():
                ...
                elif not isinstance(value, (int, str, bool)):  # Allow bool for flags like include_reports
