...
class BaseContextEngineer:
    ...
    def analyze(
        self, file_path: str, code: str, existing_context: Dict[str, Any] = None
    ) -> ContextResult:
        ...
        try:
            ...
            return {
                ...
                "complexity_score": min(
                    cyclomatic_complexity / max(node_counts.get("FunctionDef", 1), 1),
                    10,
                ),
            }
        except:
...
class RequirementsEngineerContext(BaseContextEngineer):
        ...
        try:
            ...
            return ContextResult(
                ...
                data={
                    ...
                    "coverage_areas": list(
                        set([req.requirement_type for req in requirements])
                    ),
                },
                metadata={
                    "file_path": file_path,
    ...
    def _generate_functional_requirement(
        self, func_info: Dict, docstrings: List[str], comments: List[str]
        ...
        if params:
            ...
            requirement_text = (
                f"The function {func_name} should execute its intended functionality"
            )
        ...
        for docstring in docstrings:
            if func_name in docstring.lower():
                ...
                for line in lines:
                    if any(
                        word in line.lower()
                        for word in ["should", "must", "will", "returns"]
                    ):
    ...
    def _generate_class_requirement(
        self, class_info: Dict, docstrings: List[str], comments: List[str]
    ) -> Optional[RequirementContext]:
...
class ArchitectureAnalyzerContext(BaseContextEngineer):
    ...
    def _identify_patterns(self, code: str, analysis: Dict) -> List[str]:
        ...
        if any(
            ...
            for name in [cls["name"] for cls in analysis.get("classes", [])]
        ):
    ...
    def _extract_data_models(self, analysis: Dict) -> List[Dict[str, Any]]:
        ...
        for cls in analysis.get("classes", []):
            if any(
                ...
                for keyword in ["model", "data", "entity"]
            ):
                data_models.append(
                    {
                        "name": cls["name"],
...
class ContextEngineeringManager:
    ...
    def analyze_context(
        self, file_path: str, code: str, enabled_contexts: Set[ContextEngineering]
    ) -> Dict[ContextEngineering, ContextResult]:
