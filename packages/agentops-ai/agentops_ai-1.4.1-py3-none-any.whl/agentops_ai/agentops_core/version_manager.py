...
class VersionManager:
    ...
    def create_version_snapshot(
        self, description: str, trigger: str = "manual"
    ) -> Optional[str]:
        ...
        try:
                    ...
                    for file_path in export_result["exported_files"]:
                        if os.path.exists(file_path):
                            dest_path = os.path.join(
                                version_dir, os.path.basename(file_path)
                            )
            ...
            version_info = VersionInfo(
                ...
                requirements_count=len(
                    workflow.requirement_store.get_all_requirements()
                ),
                tests_count=(
                    len([f for f in os.listdir(".agentops/tests") if f.endswith(".py")])
                    if os.path.exists(".agentops/tests")
                    else 0
                ),
    ...
    def compare_versions(self, version1_id: str, version2_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "differences": {
                "requirements_count": version2.requirements_count
                - version1.requirements_count,
                "tests_count": version2.tests_count - version1.tests_count,
                "clarifications_count": version2.clarifications_count
                - version1.clarifications_count,
                "new_files": list(
                    set(version2.files_included) - set(version1.files_included)
                ),
                "removed_files": list(
                    set(version1.files_included) - set(version2.files_included)
                ),
            },
        }

    def auto_create_version(
        self, trigger: str, description: Optional[str] = None
    ) -> Optional[str]:
        ...
        if trigger == "clarification":
            ...
            return self.create_version_snapshot(
                description or "New tests generated", trigger="test_generation"
            )
        elif trigger == "requirements_update":
            # Create version when requirements are updated
            return self.create_version_snapshot(
                description or "Requirements updated", trigger="requirements_update"
            )
