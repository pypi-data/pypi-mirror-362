...
class TierFeatures:
    ...
    # Usage quotas
    usage_quota: UsageQuota = field(
        default_factory=lambda: UsageQuota(0, 0, 0, 0, 0, 1, 10)
    )
    ...
    def _initialize_tiers(self) -> Dict[PricingTier, TierFeatures]:
        ...
        return {
            PricingTier.DEVELOPER: TierFeatures(
                ...
                context_engineering={ContextEngineering.REQUIREMENTS_GENERATION},
                export_formats={ExportFormat.GHERKIN, ExportFormat.MARKDOWN},
                model_providers={ModelProvider.BYOK},
            ...
            PricingTier.PROFESSIONAL: TierFeatures(
                ...
                model_providers={ModelProvider.BYOK, ModelProvider.AGENTOPS_OPTIMIZED},
                usage_quota=UsageQuota(
                    files_per_month=500,
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            if not pricing_manager.check_agent_access(agent_type):
                available_tiers = [
                    tier.value
                    for tier, features in pricing_manager.tiers.items()
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            if not pricing_manager.consume_usage(metric, amount):
                ...
                limit = getattr(
                    current_features.usage_quota, f"{metric}_per_month", "unlimited"
                )
                raise PermissionError(
