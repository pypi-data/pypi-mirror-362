...
from .pricing import ExportFormat, pricing_manager
...
class BaseExporter:
    ...
    def _filter_requirements(
        self, requirements: List[Requirement], config: ExportConfig
    ) -> List[Requirement]:
        ...
        if config.file_filter:
            filtered = [
                r
                for r in filtered
                if any(pattern in r.file_path for pattern in config.file_filter)
            ]
    ...
    def _prepare_template_data(
        ...
        template_data = {
            ...
            "project_name": config.template_variables.get(
...
class GherkinExporter(BaseExporter):
    ...
    def export(
        ...
        try:
            ...
            return ExportResult(
                success=False, error=str(e), format_type=self.format_type
            )
    ...
    def export(
        ...
        try:
            ...
            template_data = self._prepare_template_data(
                filtered_requirements, config, context_data
            )
            ...
            return ExportResult(
                ...
                metadata={
                    "files_covered": len(
                        set(req.file_path for req in filtered_requirements)
                    ),
            ...
            return ExportResult(
                format=self.format_type, success=False, file_path="", errors=[str(e)]
            )
...
class JSONExporter(BaseExporter):
    ...
    def export(
        ...
        try:
            ...
            export_data = {
                "project": {
                    "name": config.template_variables.get(
                        "project_name", "AgentOps Project"
                    ),
                    "version": config.template_variables.get("version", "1.0.0"),
                    "export_timestamp": datetime.now().isoformat(),
            ...
            export_data["statistics"] = {
                ...
                "average_confidence": (
                    sum(req.confidence for req in filtered_requirements)
    ...
    def export(
        ...
        try:
            ...
            return ExportResult(
                ...
                size_bytes=output_path.stat().st_size,
                record_count=len(filtered_requirements),
                metadata={"files_count": len(files_data), "structure": "hierarchical"},
            )
    ...
    def export(
        ...
        try:
            ...
            return ExportResult(
                ...
                size_bytes=output_path.stat().st_size,
                record_count=len(filtered_requirements),
                metadata={"columns": len(headers), "format": "tabular"},
            )
...
class ExportManager:
        ...
        for export_format in config.formats:
            ...
            if not pricing_manager.check_export_format(export_format):
                results[export_format] = ExportResult(
                    ...
                    file_path="",
                    errors=[
                        f"Export format {export_format.value} not available in current tier"
                    ],
                )
                continue
            ...
            try:
                ...
                results[export_format] = ExportResult(
                    format=export_format, success=False, file_path="", errors=[str(e)]
                )
    ...
    def create_export_config(self, formats: List[str], **kwargs) -> ExportConfig:
        ...
        return ExportConfig(formats=format_enums, **kwargs)

    def batch_export(
        self,
        requirements_list: List[List[Requirement]],
        configs: List[ExportConfig],
        context_data_list: List[Dict[str, Any]] = None,
    ) -> List[Dict[ExportFormat, ExportResult]]:
