...
class DashboardData:
    ...
    def add_user(self, user_id: str, user_data: Dict[str, Any]):
        ...
        self.users[user_id] = {**user_data, "last_updated": datetime.now().isoformat()}

    def add_audit_log(self, event: Dict[str, Any]):
        """Add audit log entry."""
        self.audit_logs.append({**event, "timestamp": datetime.now().isoformat()})
    ...
    def add_real_time_event(self, event: Dict[str, Any]):
        """Add real-time event."""
        self.real_time_events.append({**event, "timestamp": datetime.now().isoformat()})
    ...
    def _setup_routes(self):
        ...
        async def get_dashboard_overview():
            if DashboardFeature.PROJECT_OVERVIEW not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Project overview not available"
                )
        ...
        async def get_projects():
            if DashboardFeature.PROJECT_OVERVIEW not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Projects view not available"
                )
        ...
        async def get_project(project_id: str):
            if DashboardFeature.PROJECT_OVERVIEW not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Project details not available"
                )
        ...
        async def get_monitoring_events():
            if DashboardFeature.REAL_TIME_MONITORING not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Real-time monitoring not available"
                )
        ...
        async def get_project_progress(project_id: str):
            if DashboardFeature.PROGRESS_TRACKING not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Progress tracking not available"
                )
        ...
        async def get_code_analysis(project_id: str):
            if DashboardFeature.CODE_ANALYSIS_VIEW not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Code analysis view not available"
                )
        ...
        async def get_requirements(project_id: str):
            if DashboardFeature.REQUIREMENTS_MANAGEMENT not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Requirements management not available"
                )
        ...
        async def get_test_status(project_id: str):
            if DashboardFeature.TEST_GENERATION_STATUS not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Test generation status not available"
                )
        ...
        async def get_analytics():
            if DashboardFeature.ANALYTICS_DASHBOARD not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Analytics dashboard not available"
                )
        ...
        async def get_team_info():
            if DashboardFeature.TEAM_COLLABORATION not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Team collaboration not available"
                )
        ...
        async def get_audit_trails():
            if DashboardFeature.AUDIT_TRAILS not in self.enabled_features:
                raise HTTPException(
                    status_code=403, detail="Audit trails not available"
                )
        ...
        async def websocket_endpoint(websocket: WebSocket):
            if DashboardFeature.REAL_TIME_MONITORING not in self.enabled_features:
                await websocket.close(
                    code=1008, reason="Real-time monitoring not available"
                )
                return
