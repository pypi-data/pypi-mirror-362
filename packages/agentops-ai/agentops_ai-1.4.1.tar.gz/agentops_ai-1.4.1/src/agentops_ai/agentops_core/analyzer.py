...
class ProjectAnalyzer:
    ...
    def _get_module_name(self, file_path: str) -> str:
        ...
        try:
            ...
            try:
                ...
                if str(file_path_resolved).startswith("/var/folders") and str(
                    project_root_resolved
                ).startswith("/private/var/folders"):
                    # Handle macOS temporary directory path differences
                    alt_path = str(file_path_resolved).replace(
    ...
    def get_import_path(self, from_module: str, to_module: str) -> Optional[str]:
        ...
        if common_prefix_len > 0:
            if (
                len(from_parts) == common_prefix_len
                and len(to_parts) == common_prefix_len
            ):
                return None
    ...
    def validate_imports(
        ...
        for import_name in test_imports:
            if import_name == source_module or import_name.endswith(
                f'.{source_module.split(".")[-1]}'
            ):
                source_imported = True
                break

        for module, names in test_from_imports:
            if module == source_module or module.endswith(
                f'.{source_module.split(".")[-1]}'
            ):
                source_imported = True
                break
