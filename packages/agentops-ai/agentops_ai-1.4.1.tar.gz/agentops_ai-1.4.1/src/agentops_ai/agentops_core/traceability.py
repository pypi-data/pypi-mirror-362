...
class TraceabilityManager:
    ...
    def add_relationship(
        self, project_id: str, relationship: TraceabilityRelationship
    ) -> bool:
        ...
        if relationship.source_id in matrix.items:
            matrix.items[relationship.source_id].relationships.add(
                relationship.target_id
            )

        if relationship.target_id in matrix.items:
            matrix.items[relationship.target_id].relationships.add(
    ...
    def _validate_relationship(
        self, matrix: TraceabilityMatrix, relationship: TraceabilityRelationship
    ) -> bool:
    ...
    def calculate_coverage(self, project_id: str) -> Dict[str, Any]:
        ...
        requirements = [
            item for item in matrix.items.values() if item.type == "requirement"
        ]
    ...
    def _calculate_item_coverage(
        self, matrix: TraceabilityMatrix, items: List[TraceabilityItem]
    ) -> Dict[str, Any]:
        ...
        return {"covered": covered, "coverage_percentage": coverage_percentage}
    ...
    def export_matrix(
        ...
        if format_type == "html" and current_tier in [
    ...
    def _export_excel(self, matrix: TraceabilityMatrix, output_path: str) -> str:
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            ...
            pd.DataFrame(relationships_data).to_excel(
            ...
            summary_data = [
                {
                    "Metric": "Total Items",
                    "Value": coverage_stats.get("requirements", {}).get("total", 0)
                    + coverage_stats.get("tests", {}).get("total", 0)
                    + coverage_stats.get("code_items", {}).get("total", 0),
                },
                {
                    "Metric": "Total Relationships",
                    "Value": coverage_stats.get("relationships", {}).get("total", 0),
                },
                {
                    "Metric": "Overall Coverage %",
                    "Value": coverage_stats.get("overall_coverage", 0),
                },
                {
                    "Metric": "Requirements Coverage %",
                    "Value": coverage_stats.get("requirements", {}).get(
                        "coverage_percentage", 0
                    ),
                },
                {
                    "Metric": "Tests Coverage %",
                    "Value": coverage_stats.get("tests", {}).get(
                        "coverage_percentage", 0
                    ),
                },
                {
                    "Metric": "Code Coverage %",
                    "Value": coverage_stats.get("code_items", {}).get(
                        "coverage_percentage", 0
                    ),
                },
            ]
    ...
    def _export_html(self, matrix: TraceabilityMatrix, output_path: str) -> str:
        ...
        for item in matrix.items.values():
            coverage_class = (
                "coverage-high"
                if item.coverage_percentage >= 80
                else (
                    "coverage-medium"
                    if item.coverage_percentage >= 50
                    else "coverage-low"
                )
            )
...
def add_traceability_relationship(
    project_id: str, relationship: TraceabilityRelationship
) -> bool:
    """Add a relationship to the traceability matrix.
...
def export_traceability_matrix(
    project_id: str, format_type: str = "csv", output_path: Optional[str] = None
) -> str:
    ...
    Returns:
        Path to exported file
    """
    return get_traceability_manager().export_matrix(
        project_id, format_type, output_path
    )
