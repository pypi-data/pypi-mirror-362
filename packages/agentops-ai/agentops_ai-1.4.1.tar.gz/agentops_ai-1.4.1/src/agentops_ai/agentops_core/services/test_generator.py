...
class TestGenerator:
    ...
    def _ensure_project_analyzed(self, project_root: str = "."):
        ...
        if self.project_analyzer is None or self.project_analyzer.project_root != str(
            actual_project_root
        ):
    ...
    def _validate_test_imports(self, test_code: str, source_file: str) -> bool:
        ...
        try:
            ...
            if unused_names:
                notify_import_validation_issue(
                    source_file,
                    [f"Test code references names not in source: {unused_names}"],
                )
                return False
            ...
            notify_import_validation_issue(
                source_file, [f"Could not validate test imports: {e}"]
            )
        ...
        try:
            ...
            if result["success"]:
                ...
                if not self._validate_test_imports(result["tests"], target):
                    print(
                        f"Warning: Generated test code may have import issues for {target}"
                    )
    ...
    def _fix_test_imports(
        self, test_code: str, source_module: str, test_module: str
    ) -> str:
        ...
        if source_file_path:
            source_module_name = self.project_analyzer._get_module_name(
                source_file_path
            )
        else:
        ...
        try:
            ...
            if imports_to_add:
                print(
                    f"DEBUG: Importing from {source_module_name}: {', '.join(imports_to_add)}"
                )
            else:
        ...
        if imports_to_add:
            import_statement = (
                f"from {source_module_name} import {', '.join(imports_to_add)}"
            )
        ...
        if structure:
            ...
            if classes:
                ...
                for c in classes:
                    ...
                    summary.append(
                        f"- {c.get('name', 'unknown')} (methods: {methods_str})"
                    )
    ...
    def _process_response(
        ...
        try:
            ...
            return {
                "success": True,
                "tests": processed_code or test_code,
    ...
    def _post_process_test_code(
        self, test_code: str, requirement_id: int = None
    ) -> str:
    ...
    def generate_tests_from_requirement(
        ...
        try:
            ...
            processed_code = self._post_process_test_code(
                result["tests"], requirement_id=requirement_id
            )

            # Fix imports using project analyzer
            if self.project_analyzer:
                processed_code = self._fix_test_imports(
    ...
    def _create_requirement_driven_prompt(
        ...
        import_context = ""
        if self.project_analyzer:
            correct_imports = self.project_analyzer.get_test_imports(
