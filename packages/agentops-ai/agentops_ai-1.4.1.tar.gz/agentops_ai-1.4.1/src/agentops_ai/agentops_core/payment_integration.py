...
class PaymentConfig:
    ...
    # Provider settings
    providers: Set[PaymentProvider] = field(
        default_factory=lambda: {PaymentProvider.STRIPE}
    )
...
class PaymentManager:
    ...
    def _initialize_providers(self):
        ...
        if (
            PaymentProvider.STRIPE in self.config.providers
            and self.config.stripe_secret_key
        ):
            self.providers[PaymentProvider.STRIPE] = StripeProvider(self.config)

        if (
            PaymentProvider.CASHFREE in self.config.providers
            and self.config.cashfree_app_id
        ):
    ...
    def get_user_limits(self, user_id: str) -> Dict[str, int]:
        ...
        if not subscription:
            ...
            return {"max_users": 1, "max_projects": 5, "max_storage_gb": 1}
...
class BasePaymentProvider:
    ...
    def create_subscription(
        self, user_id: str, plan_id: str, customer_data: Dict[str, Any]
    ) -> Dict[str, Any]:
...
class CashfreeProvider(BasePaymentProvider):
    ...
    def __init__(self, config: PaymentConfig):
        ...
        self.base_url = (
            "https://sandbox.cashfree.com/pg"  # Use production URL in production
        )
