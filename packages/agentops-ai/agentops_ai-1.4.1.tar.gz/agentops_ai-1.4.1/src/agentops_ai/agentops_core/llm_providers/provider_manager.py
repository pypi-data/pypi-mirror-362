...
class ProviderManager:
    ...
    def __init__(
        self, strategy: LoadBalancingStrategy = LoadBalancingStrategy.PRIMARY_FALLBACK
    ):
    ...
    def add_provider(
        self, provider: BaseLLMProvider, weight: float = 1.0, is_primary: bool = False
    ) -> None:
    ...
    async def generate(
        ...
        **kwargs,
        ...
        raise ProviderConnectionError(f"All providers failed. Last error: {last_error}")
    ...
    def _get_providers_to_try(
        ...
        """
        available_providers = [
            name for name in self.providers.keys() if name not in self.failed_providers
        ]
        ...
        if self.strategy == LoadBalancingStrategy.PRIMARY_FALLBACK:
            if self.primary_provider and self.primary_provider in available_providers:
                ...
                remaining = [
                    p for p in available_providers if p != self.primary_provider
                ]
                providers.extend(remaining)
                return providers
            else:
            ...
            if available_providers:
                ...
                providers = (
                    available_providers[start_idx:] + available_providers[:start_idx]
                )
            ...
            return sorted(
                available_providers, key=lambda p: self.providers[p].request_count
            )
    ...
    def _mark_provider_failed(
        self, provider_name: str, temporary: bool = False
    ) -> None:
