...
from ..agentops_core.agent_selector import WorkflowMode, agent_selector
from ..agentops_core.context_engineering import context_engineering_manager
from ..agentops_core.export_manager import ExportConfig, export_manager
...
@click.option(
    "--config", type=click.Path(exists=True), help="Load configuration from file"
)
@click.pass_context
def cli(ctx, tier, config):
    ...
    if current_tier != PricingTier.DEVELOPER:
        console.print(
            f"[green]✓ AgentOps {current_tier.value.title()} Tier Active[/green]"
        )
        console.print(
            f"[dim]Available agents: {len(tier_features.available_agents)}[/dim]"
        )
...
def upgrade(ctx, tier_name):
    ...
    if new_tier.value <= current_tier.value:
        console.print(
            f"[yellow]Already on {current_tier.value.title()} tier or higher[/yellow]"
        )
        return
...
def run(ctx, file, mode, agents, parallel, workers, context, export, approve, output):
        ...
        for agent_name in agents.split(","):
            try:
                ...
                if pricing_manager.check_agent_access(agent_type):
                    ...
                    console.print(
                        f"[yellow]Warning: Agent {agent_name} not available in {current_tier.value} tier[/yellow]"
                    )
            except ValueError:
    ...
    if not pricing_manager.check_feature_access("parallel_processing"):
        parallel = False
        console.print(
            "[yellow]Warning: Parallel processing not available in current tier[/yellow]"
        )
    ...
    if workers:
        ...
        if workers > max_workers:
            console.print(
                f"[yellow]Warning: Reduced workers to {max_workers} (tier limit)[/yellow]"
            )
        ...
        for ctx_name in context:
            try:
                ...
                if pricing_manager.check_context_engineering(ctx_type):
                    ...
                    console.print(
                        f"[yellow]Warning: Context {ctx_name} not available in {current_tier.value} tier[/yellow]"
                    )
            except ValueError:
                console.print(
                    f"[yellow]Warning: Unknown context type {ctx_name}[/yellow]"
                )
        ...
        for fmt in export:
            try:
                ...
                if pricing_manager.check_export_format(fmt_enum):
                    ...
                    console.print(
                        f"[yellow]Warning: Export format {fmt} not available in {current_tier.value} tier[/yellow]"
                    )
            except ValueError:
    ...
    for i, step in enumerate(execution_plan, 1):
        if len(step) == 1:
            ...
            console.print(
                f"  {i}. Parallel: {', '.join(agent.value.replace('_', ' ').title() for agent in step)}"
            )
    ...
    try:
        ...
        with Progress(
            ...
            if config.context_engineering_enabled:
                ...
                progress.update(
                    task, description="[green]✓ Context engineering complete[/green]"
                )
            else:
                context_results = {}
            ...
            progress.update(
                task, description="[green]✓ Agent workflow complete[/green]"
            )
            ...
            if config.export_formats:
                ...
                requirements = workflow.requirement_store.get_requirements_for_file(
                    file
                )
                ...
                for fmt, export_result in export_results.items():
                    if export_result.success:
                        console.print(
                            f"  [green]✓ {fmt.value.title()}: {export_result.file_path}[/green]"
                        )
                        console.print(
                            f"    Records: {export_result.record_count}, Size: {export_result.size_bytes} bytes"
                        )
                    else:
                        console.print(
                            f"  [red]✗ {fmt.value.title()}: {', '.join(export_result.errors)}[/red]"
                        )
...
@click.option(
    "--format",
    "output_format",
    type=click.Choice(["junit", "json", "html"]),
    default="junit",
...
@click.option(
    "--include-context", is_flag=True, help="Include context engineering results"
)
@click.pass_context
