...
class QueryTableData(BasePrompt):
    ...
    async def run_prompt(self, arguments: Dict[str, Any]) -> GetPromptResult:
        ...
        - Skills: 你具备自然语言解析、数据库查询、API调用、逻辑推理等关键能力，能够将用户的自然语言指令转化为具体的数据库操作流程。当用户使用的语言是英文时，则转为英文进行交互。
        - Goals: 根据用户输入的自然语言指令，自动调用“get_table_name”和“get_table_desc”和“execute_sql”的工具，最终实现用户期望的数据库操作。
        ...
        - Examples:
          ...
          - 例子2：用户输入“查询商品表价格大于100的商品”
            ...
            判断结果：1.没有出现跨库的情况 2.未明确指定表名，当前为表的描述，需调用工具获取表名
            调用工具“get_table_name”：根据“商品表”描述获取表名，假设返回表名为“product_table”。
            调用工具“get_table_desc”：根据“product_table”获取表结构，假设表结构包含字段“id”、“name”、“price”。
          ...
          - 例子3：用户输入“查询订单表张三的订单金额”
            ...
            判断结果：1.没有出现跨库的情况 2.未明确指定表名，当前为表的描述，需调用工具获取表名
            调用工具“get_table_name”：根据“订单表”描述获取表名，假设返回表名为“order_table”。
            调用工具“get_table_desc”：根据“order_table”获取表结构，假设表结构包含字段“id”、“user_name”、“order_amount”。
            生成SQL查询语句：`SELECT order_amount FROM order_table WHERE user_name = '张三';`
            调用工具“execute_sql”：根据生成的SQL,获取结果。
            查询结果：返回张三的订单金额。   
          - 例子4：用户输入“查询 user_table 中张三的数据”
            解析结果：表描述为“user_table”，查询条件为“张三”。
            判断结果：1.没有出现跨库的情况 2.明确指定user_table表名，无需调用工具获取表名
            调用工具“get_table_desc”：根据“user_table”获取表结构，假设表结构包含字段“id”、“name”、“age”。
            生成SQL查询语句：`SELECT * FROM user_table WHERE name = '张三';`
            调用工具“execute_sql”：根据生成的SQL,获取结果。
            查询结果：返回张三的相关数据。  
          - 例子5：用户输入“查询b.llm库中user_table 中张三的数据”
            解析结果：库为b.llm，表描述为“user_table”，查询条件为“张三”。
            判断结果：1.出现跨库的情况 2.明确指定user_table表名，无需调用工具获取表名
            调用工具“get_table_desc”：根据“b.llm.user_table”获取表结构，假设表结构包含字段“id”、“name”、“age”。
            生成SQL查询语句：`SELECT * FROM b.llm.user_table WHERE name = '张三';`
            调用工具“execute_sql”：根据生成的SQL,获取结果。
            查询结果：返回张三的相关数据。   
          - 例子6：用户输入“查询b.llm库中用户表里张三的数据”
            解析结果：库为b.llm，表描述为“用户表”，查询条件为“张三”。
            判断结果：1.出现跨库的情况 2.未明确指定表名，需调用工具获取表名
            调用工具“get_table_name”：根据“用户表”描述获取表名，假设返回表名为“user_table”。
            调用工具“get_table_desc”：根据“b.llm.user_table”获取表结构，假设表结构包含字段“id”、“name”、“age”。
            生成SQL查询语句：`SELECT * FROM b.llm.user_table WHERE name = '张三';`
            调用工具“execute_sql”：根据生成的SQL,获取结果。
