...
class SimpleFormExtractor:
    def __init__(self, config=None):
        ...
        self.timeouts = {
            'navigation': self.config.get('navigation_timeout', 15000),  # 30000 -> 15000
            'element_wait': self.config.get('element_wait_timeout', 5000),  # 10000 -> 5000
            'interaction': self.config.get('interaction_timeout', 2500),  # 5000 -> 2500
            'loading': self.config.get('loading_timeout', 7500),  # 15000 -> 7500
            'short_wait': self.config.get('short_wait_timeout', 1500),  # 3000 -> 1500
            'network_idle': self.config.get('network_idle_timeout', 7500),  # 15000 -> 7500
            'dynamic_loading_wait': self.config.get('dynamic_loading_wait', 1250),  # 2500 -> 1250
            'scroll_detection_wait': self.config.get('scroll_detection_wait', 375)  # 750 -> 375
        }
    ...
    async def _wait_for_element_ready(self, page: Page, element: ElementHandle, timeout: int = None) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Element ready wait failed: {e}")
            return False
    ...
    async def _wait_for_network_calm(self, page: Page, timeout: int = None, idle_time: int = 500) -> None:
        ...
        try:
            ...
            # Fallback: monitor network requests manually
            try:
                request_count = 0
                ...
                while (asyncio.get_event_loop().time() - start_time) * 1000 < timeout:
                    if request_count == 0:
                        ...
                        elif (asyncio.get_event_loop().time() - stable_start) * 1000 >= idle_time:
                            break
                    else:
                        stable_start = None
                ...
                self.logger.debug(f"Network calm monitoring failed: {e}")
    ...
    async def _wait_for_loading_complete(self, page: Page, timeout: int = None) -> None:
        ...
        try:
            ...
            for selector in loading_selectors:
                try:
                    await page.wait_for_selector(selector, state='detached', timeout=1500)  # 3000 -> 1500
                    self.logger.debug(f"Loading indicator {selector} disappeared")
                except Exception:
                    pass  # Not found or didn't disappear - that's OK
            
            # Wait for content indicators to appear
            content_selectors = [
                'input', 'textarea', 'select', 'button',
                '[role="combobox"]', 'fieldset', '.form',
                '[data-testid*="form"]', '[data-qa*="form"]'
            ]
            
            for selector in content_selectors:
                try:
                    await page.wait_for_selector(selector, timeout=self.timeouts['element_wait'])
                    self.logger.debug(f"Content indicator {selector} appeared")
                    break  # Found at least one content element
                except Exception:
                    continue
    ...
    async def _smart_element_interaction_wait(self, page: Page, element: ElementHandle) -> None:
        """Smart wait before interacting with an element."""
        try:
            # Ensure element is ready
            if await self._wait_for_element_ready(page, element):
                ...
                await page.wait_for_function(
                    ...
                    element,
                    timeout=self.timeouts['interaction']
                )
        except Exception as e:
            self.logger.debug(f"Element interaction wait warning: {e}")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for attempt in range(max_retries):
                    try:
                        response = await page.goto(url, timeout=15000, wait_until='domcontentloaded')  # 30000 -> 15000
                        if response:
                        ...
                        if attempt < max_retries - 1:
                            await self._smart_wait(page, 'medium')  # Smart wait before retry
                            continue
                        else:
                            self.logger.warning("All navigation attempts failed, but proceeding")
                
                # Enhanced page waiting strategy
                await self._enhanced_page_wait(page)
                ...
                try:
                    loading_selectors = [
                        ...
                        '.preloader', '.progress-bar',
                        '[aria-busy="true"]', '.loading-overlay',
                        '.securiti-overlay-loading'  # Specific to Securiti cookie banners
                    ]
                    ...
                    for selector in content_selectors:
                        try:
                            await page.wait_for_selector(selector, timeout=2500)  # 5000 -> 2500
                            self.logger.debug(f"Content indicator {selector} appeared")
                            break  # Found at least one content element
                        except Exception:
                            continue
                    
                    # Enhanced final wait - try networkidle but don't fail if it times out
                    try:
                        await page.wait_for_load_state('networkidle', timeout=7500)  # 15000 -> 7500
                        self.logger.debug("Network became idle")
                    except Exception:
                        self.logger.debug("NetworkIdle timeout - common on heavy JS pages, proceeding")
                        # Use smart wait for heavy JavaScript pages
                        await self._smart_wait(page, 'extended')
    ...
    async def _find_form_page(self, page: Page) -> Page:
        ...
        try:
            ...
            # Check for "Apply" or "Apply Now" buttons that might need to be clicked first
            # Many career pages have an Apply button that scrolls to or reveals the form
            apply_buttons = await page.query_selector_all('a[href="#apply"], a[href="#applynow"], button:has-text("Apply"), a:has-text("Apply")')
            if apply_buttons:
                for button in apply_buttons:
                    try:
                        ...
                        if box and box['width'] > 0 and box['height'] > 0:
                            button_text = await button.text_content() or ""
                            self.logger.info(f"Found apply button: {button_text.strip()}")
                            
                            # Click the button to reveal the form
                            await button.click()
                            await self._smart_wait(page, 'medium')  # Wait for any animations/transitions
                            await page.wait_for_load_state('networkidle', timeout=2500)  # 5000 -> 2500
                            break
                    except Exception as e:
                        self.logger.debug(f"Error clicking apply button: {e}")
            ...
            if form_elements:
                for form in form_elements:
                    try:
                        # Check if this is likely an application form, not a search form or navigation form
                        inputs = await form.query_selector_all('input[type="text"], input[type="email"], textarea')
                        if len(inputs) >= 2:  # Application forms typically have multiple inputs
                            self.logger.info("Found application form element")
            ...
            # Check for dedicated "Apply now" sections
            ...
            for section in apply_sections:
                try:
                    # Check if this section has form elements
                    inputs = await section.query_selector_all('input, select, textarea, [role="combobox"]')
                    if len(inputs) >= 2:
                        self.logger.info("Found dedicated application section")
            ...
            # Look for common form containers if no specific form or apply section found
            form_containers = await page.query_selector_all('.form-container, [class*="form-container"], [class*="application"], [class*="apply-form"], [class*="job-form"]')
            for container in form_containers:
                try:
                    inputs = await container.query_selector_all('input, select, textarea')
                    if len(inputs) >= 2:
                        self.logger.info("Found form container")
                        self.iframe_context["form_container"] = container
                        await container.scroll_into_view_if_needed()
                        await self._smart_wait(page, 'short')
                        return page
                except Exception as e:
                    self.logger.debug(f"Error checking form container: {e}")
            ...
            for i, iframe in enumerate(iframes):
                try:
                    # Get iframe src to check if it's likely a form
                    src = await iframe.get_attribute('src')
                    if src:
                        self.logger.info(f"Iframe {i+1} src: {src}")
                        
                        # Check if it's a Greenhouse iframe or job-related
                        if any(keyword in src.lower() for keyword in ['greenhouse', 'job', 'career', 'apply', 'form', 'workday', 'lever', 'talent', 'recruit']):
                            # Enhanced iframe visibility handling
                            try:
                                iframe_box = await iframe.bounding_box()
                                if iframe_box and (iframe_box['width'] < 5 or iframe_box['height'] < 5):
                                    self.logger.debug(f"Iframe {i+1} is too small/hidden, scrolling to it")
                                    await iframe.scroll_into_view_if_needed()
                                    await self._smart_wait(page, 'medium')  # Increased wait time
                                    
                                    # Clear any overlays that might be hiding the iframe
                                    await self._dismiss_overlays(page)
                            except Exception:
                                pass
                            
                            # Get the frame content with enhanced error handling
                            frame = await iframe.content_frame()
                            if frame:
                                try:
                                    # Enhanced wait strategy - use domcontentloaded first, then networkidle
                                    await frame.wait_for_load_state('domcontentloaded', timeout=7500)  # 15000 -> 7500
                                    await self._smart_wait(page, 'medium')
                                    
                                    # Try networkidle but don't fail if it times out
                                    try:
                                        await frame.wait_for_load_state('networkidle', timeout=5000)  # 10000 -> 5000
                                    except Exception:
                                        self.logger.debug("NetworkIdle timeout, proceeding anyway")
                                    
                                    # Dismiss any overlays within the iframe
                                    await self._dismiss_overlays(frame)
                                    
                                    # Check for form elements with expanded selectors and better detection
                                    ...
                                    if len(frame_indicators) > 2:
                                        self.logger.info(f"Found job application form in iframe {i+1} with {len(frame_indicators)} form elements")
                                        
                                        # Store iframe context for JSON output
                                        self.iframe_context = {
                                            "is_iframe": True,
                                            "iframe_src": src,
                                            "iframe_selector": f'iframe[src*="{self._extract_domain_from_src(src)}"]',
                                            "iframe_index": i,
                                            "wait_strategy": "domcontentloaded",  # Updated strategy
                                            "load_timeout": 15000
                                        }
                    ...
                    # Even if src doesn't match, check frame content for job form indicators
                    frame = await iframe.content_frame()
                    if frame:
                        try:
                            # Enhanced wait for frame content
                            await frame.wait_for_load_state('domcontentloaded', timeout=5000)  # 10000 -> 5000
                            await self._smart_wait(page, 'medium')
                            
                            # Try networkidle but continue if it fails
                            try:
                                await frame.wait_for_load_state('networkidle', timeout=2500)  # 5000 -> 2500
                            except Exception:
                                pass
                            
                            # Dismiss overlays in iframe
                            await self._dismiss_overlays(frame)
                            
                            # Check for job application specific elements with expanded selectors
                            frame_indicators = await frame.query_selector_all('input, textarea, select, [role="combobox"], button[type="submit"], fieldset, [role="group"]')
                            if len(frame_indicators) > 2:
                                self.logger.info(f"Found job application form in iframe {i+1} (by content) with {len(frame_indicators)} form elements")
                            ...
                            self.logger.debug(f"Error checking iframe {i+1} content: {e}")
                                
                except Exception as e:
                    self.logger.debug(f"Error checking iframe {i+1}: {e}")
                    continue
            
            # Final check: Look for any form elements on the page
            ...
            if form_elements and len(form_elements) > 2:
                self.logger.info("Found form elements on main page")
                return page
            
            # If no iframe form found, return main page
            self.logger.info("No form found in iframes, using main page")
            return page
