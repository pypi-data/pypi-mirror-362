...
class RulesRequestBuilder(BaseRequestBuilder):
    ...
    def by_rule(self,rule: str) -> WithRuleItemRequestBuilder:
        """
        Manage the configuration of a single artifact rule.
        Args:
            rule: Unique identifier of the item
        Returns: WithRuleItemRequestBuilder
        """
        if not rule:
            raise TypeError("rule cannot be null.")
        from .item.with_rule_item_request_builder import WithRuleItemRequestBuilder

        url_tpl_params = get_path_parameters(self.path_parameters)
        url_tpl_params["rule"] = rule
    ...
    async def post(self,body: Optional[Rule] = None, request_configuration: Optional[RulesRequestBuilderPostRequestConfiguration] = None) -> None:
        """
    ...
    def to_post_request_information(self,body: Optional[Rule] = None, request_configuration: Optional[RulesRequestBuilderPostRequestConfiguration] = None) -> RequestInformation:
        """
        Adds a rule to the list of rules that get applied to the artifact when adding newversions.  All configured rules must pass to successfully add a new artifact version.This operation can fail for the following reasons:* No artifact with this `artifactId` exists (HTTP error `404`)* Rule (named in the request body) is unknown (HTTP error `400`)* A server error occurred (HTTP error `500`)
        Args:
            body: The request body
