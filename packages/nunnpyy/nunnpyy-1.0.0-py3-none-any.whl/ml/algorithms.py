def run():
    """
    Display all machine learning algorithm implementations
    """
    ...
    for x in range(items.shape[0]):
        for y in range(data.shape[0]):
            if data[y, col] == items[x]:
                count[x] += 1
    for x in range(items.shape[0]):
        dict[items[x]] = np.empty((int(count[x][0]), data.shape[1]), dtype="|S32")  # FIXED
        pos = 0
        for y in range(data.shape[0]):
            if data[y, col] == items[x]:
...
def entropy(S):
    ...
    for count in counts:
        sums += -1 * count[0] * math.log(count[0], 2)  # FIXED
    return sums
