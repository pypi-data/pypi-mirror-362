...
def start_xpra_session(session_id):
    ...
    try:
        ...
        # Step 6: Build Xpra command with proper option separation
        # Firefox options should be passed via --start-child, not as Xpra options
        firefox_cmd = f'{firefox_path} --no-first-run --no-default-browser-check --disable-extensions --disable-plugins'
        ...
        try:
            os.makedirs(xpra_runtime_dir, exist_ok=True)
            logger.info(f"Ensured XDG_RUNTIME_DIR exists: {xdg_runtime_dir}")
        except PermissionError:
            logger.warning(f"Could not create XDG_RUNTIME_DIR: {xdg_runtime_dir} - may cause warnings")
        ...
        if not env.get('DISPLAY'):
            ...
            logger.info(f"Using existing DISPLAY: {env.get('DISPLAY')}")
            env['DISPLAY'] = f':{display_num}'
...
def load_jupyter_server_extension(server_app):
    ...
    handlers = [
        ...
        (url_path_join(base_url, "firefox/session/([^/]+)/?"), FirefoxSessionHandler),
        # Match the frontend expectation: /proxy/firefox/{sessionId}/
