...
class FirefoxLauncherHandler(web.RequestHandler):
    ...
    def _is_firefox_direct_running(self):
        ...
        try:
            # Check if debug port is accessible (WebDriver BiDi server)
            import urllib.request
            response = urllib.request.urlopen('http://localhost:8888/', timeout=2)
            content = response.read().decode()
            if 'httpd.js' in content:  # Firefox WebDriver BiDi server response
                logger.info("Firefox direct debugging is already running")
                return True
            else:
                return False
    ...
    def _setup_firefox_direct(self):
        ...
        try:
            ...
            try:
                ...
                time.sleep(2)
                logger.info("Cleaned up existing Firefox processes")
            except Exception as e:
                logger.warning(f"Error cleaning up Firefox processes: {e}")
            ...
            firefox_cmd = [
                ...
                '--marionette',  # Enable marionette for WebDriver BiDi
                '--new-instance',
                '--profile', profile_dir,
            ...
            try:
                ...
                logger.error(f"❌ Failed to start Firefox process: {e}")
                return False
            
            logger.info(f"Firefox process started with PID: {firefox_process.pid}")
            
            # Wait for Firefox to start and check multiple times
            for attempt in range(10):  # Try for 10 seconds
                time.sleep(1)
                
                # Check if process is still running
                if firefox_process.poll() is not None:
                    logger.error(f"❌ Firefox process exited early (attempt {attempt + 1})")
                    logger.error(f"Firefox process exit code: {firefox_process.returncode}")
                    return False
                ...
                try:
                    ...
                    response = urllib.request.urlopen('http://localhost:8888/', timeout=2)
                    content = response.read().decode()
                    if 'httpd.js' in content:  # Firefox WebDriver BiDi server
                        logger.info(f"✅ Firefox started successfully (PID: {firefox_process.pid}) after {attempt + 1} seconds")
                    ...
                    logger.debug(f"Firefox debug port not ready yet (attempt {attempt + 1})")
                    continue
            
            # If we get here, Firefox didn't start properly
            logger.error("❌ Firefox debug port never became accessible")
            if firefox_process.poll() is None:
                # Process is still running but debug port not accessible
                logger.warning("Firefox process is running but debug port not accessible")
                return True  # Return True anyway, might work later
            else:
                # Process exited
                logger.error(f"Firefox process exit code: {firefox_process.returncode}")
                return False
            ...
            logger.error(f"Exception in Firefox direct setup: {str(e)}", exc_info=True)
            return False
