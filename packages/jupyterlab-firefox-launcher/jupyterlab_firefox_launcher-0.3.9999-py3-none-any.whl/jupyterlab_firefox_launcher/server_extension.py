...
class SimpleProxyHandler(JupyterHandler):
    ...
    async def _proxy_request(self, method, session_id, path):
        ...
        try:
            ...
            if not is_port_open('localhost', port, timeout=1):
            ...
            request = HTTPRequest(
                ...
                request_timeout=10,  # Shorter timeout for better responsiveness
                connect_timeout=5
            )
            ...
            try:
                ...
                logger.warning(f"Fetch error for {target_url}: {fetch_error}")
                # Return a more user-friendly error for connection issues
                if "Connection closed" in str(fetch_error) or "StreamClosedError" in str(fetch_error):
                    self.set_status(503)
