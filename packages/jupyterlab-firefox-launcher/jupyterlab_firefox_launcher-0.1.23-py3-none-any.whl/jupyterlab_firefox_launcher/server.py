        ...
        if not self._is_vnc_running():
            ...
            if IS_JUPYTERHUB:
                ...
                if service_prefix:
                    ...
                    host = self.request.headers.get('Host', 'localhost:8888')
                    protocol = 'https' if self.request.protocol == 'https' else 'http'
                    vnc_url = f"{protocol}://{host}{service_prefix}/proxy/{web_port}/vnc.html?autoconnect=true&resize=scale"
                else:
    ...
    def _setup_firefox_vnc(self):
        ...
        if IS_JUPYTERHUB:
            ...
            if service_prefix:
                ...
                # Use JupyterHub's proxy system with full URL
                if novnc_path:
                    vnc_url = f"{protocol}://{host}{service_prefix}/proxy/{web_port}/vnc.html?autoconnect=true&resize=scale"
                else:
                    vnc_url = f"{protocol}://{host}{service_prefix}/proxy/{web_port}/"
                    
                logger.debug(f"Built full JupyterHub URL: {vnc_url}")
            else:
