...
def start_xpra_session(session_id):
    ...
    try:
        ...
        # Step 7: Build Xpra command - HTML5 only, no WebSocket to avoid auth issues
        cmd = [
            xpra_path,
            'start',
            f':{display_num}',
            ...
            '--no-systemd-run',
            '--start-via-proxy=no',
            # Force HTML5 only - disable WebSocket to avoid 403 auth errors
            '--websockets=no',
        ]
    ...
    async def get(self):
        ...
        <body>
            <div class="container">
                ...
                <div class="info">Using HTML5-only connection via JupyterLab proxy (no WebSocket auth issues)</div>
            ...
            <script>
                async function startFirefoxSession() {
                    try {
                        ...
                        iframe.src = `/proxy/firefox/${data.session_id}/`;
                        iframe.title = "Firefox Browser via Xpra HTML5 (Proxied)";
                        document.getElementById('firefox-container').appendChild(iframe);
                        document.getElementById('status').textContent = 
                            `Firefox is ready! (Session: ${data.session_id}, HTML5 via proxy)`;
...
def load_jupyter_server_extension(server_app):
    ...
    # Add handlers - Using HTTP proxy for slurmspawner compatibility, NO WebSocket
    handlers = [
        ...
        # NOTE: WebSocket proxy REMOVED - HTML5-only approach eliminates 403 auth errors
        # All communication goes through HTTP proxy using proven working Tornado AsyncHTTPClient
    ]
