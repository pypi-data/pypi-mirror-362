...
def start_xpra_session(session_id):
    ...
    try:
        ...
        # Firefox will be started by Xpra, so we don't need to specify DISPLAY for Firefox explicitly
        firefox_cmd = f'{firefox_path} --no-first-run --no-default-browser-check --disable-extensions --disable-plugins --disable-web-security --disable-features=VizDisplayCompositor'
