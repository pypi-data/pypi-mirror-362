...
def start_xpra_session(session_id):
    ...
    try:
                ...
                try:
                    ...
                    if not extracted_port and process.stderr in ready_streams[0]:
                        ...
                        if output:
                            ...
                            # Check for HTTP service ready indicator
                            if 'serving html content' in output and extracted_port:
                                logger.info("Xpra reports HTML service ready, waiting 2 seconds for full initialization...")
                                time.sleep(2)  # Give HTTP service time to fully initialize
        ...
        if not http_ready:
            ...
            logger.info("Note: The Xpra HTML5 client may take additional time to become fully responsive")
...
class SimpleProxyHandler(JupyterHandler):
    ...
    async def _proxy_request(self, method, session_id, path):
        ...
        max_retries = 5  # Increased retries for slow Xpra startup
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Check if port is still available with patience for slow startup
                if not is_port_open('localhost', port, timeout=5):  # Longer timeout for each check
                    if attempt < max_retries - 1:
                        logger.warning(f"Port {port} not responding, retrying in {retry_delay}s... (attempt {attempt + 1}/{max_retries})")
                        await asyncio.sleep(retry_delay)
                        continue
                    else:
                        logger.warning(f"Port {port} is not responding for session {session_id} after {max_retries} attempts")
                        self.set_status(503)
                        self.write({'error': 'Xpra service not available - service may still be starting up'})
                        return
