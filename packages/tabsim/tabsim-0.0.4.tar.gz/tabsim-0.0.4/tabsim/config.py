...
def load_sky_model(file_path: str, freqs: Array, src_type: str) -> tuple:
    ...
    def calc_S_nu(I: Array, f0: Array, si: Array):
        return (
            I[:, None, None]
...
def gauss(A: float, mean: float, sigma: float, x: Array) -> Array:
    return A * da.exp(-0.5 * ((x - mean) / sigma) ** 2)


def generate_spectra(spec_df: pd.DataFrame, freqs: Array, id_key: str) -> tuple:
