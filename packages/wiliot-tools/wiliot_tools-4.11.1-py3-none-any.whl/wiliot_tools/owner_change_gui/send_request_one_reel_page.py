...
class OneReelRequestPage(ctk.CTkFrame):
    ...
    def __init__(self, parent, show_page_callback):
        ...
        self.cloud_destination = ctk.CTkComboBox(master=self, values=['AWS', 'GCP', 'WMT'], justify='center',width=entry_width)
        self.cloud_destination.place(x=self.parent.W / 2 + 10 - x_spacing, y=start_y + 6 * y_spacing)
        ...
        label.place(x=self.parent.W / 2 - label_width - 10 - x_spacing, y=start_y + 8 * y_spacing)

        self.wmt_api_key_entry = ctk.CTkEntry(self, width=entry_width, show="*")
        self.wmt_api_key_entry.place(x=self.parent.W / 2 + 10 - x_spacing, y=start_y + 8 * y_spacing)
        ...
        btn.place(x=self.parent.W / 2 - btn.winfo_reqwidth() / 2, y=start_y + 9 * y_spacing)
    ...
    def send_request(self):
        try:
            self.verify_request()
            self.parent.save_api_keys(self.api_key_entry.get(), self.gcp_api_key_entry.get(), self.wmt_api_key_entry.get())
