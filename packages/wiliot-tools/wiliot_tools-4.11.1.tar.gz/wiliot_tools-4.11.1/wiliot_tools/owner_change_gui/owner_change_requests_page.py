...
class OwnerChangeRequests(ctk.CTkFrame):
    ...
    def color_requests(self):
        for i, item in enumerate(self.tree.get_children()):
            ...
            if values[3].replace("GCP-", "").replace("WMT-", "") in ('not-started', 'processing', 'exported', 'processed', 'checking-fails'):
    ...
    def get_request_status(self, request_id, cloud_destination, status, account_id):
        ...
        elif cloud_destination == 'WMT' and status.startswith("WMT"):
    ...
    def update_requests(self):
        for item in self.tree.get_children():
            values = self.tree.item(item, "values")
            request_id = values[0]
            status = values[3].replace("GCP-", "").replace("WMT-", "")
            if status in ('not started', 'not-started', '-', 'processing', 'processed', 'exported', 'checking-fails'):
                ...
                if status_code == 200:
                    ...
                    new_status = new_values[3].replace("GCP-", "").replace("WMT-", "")
