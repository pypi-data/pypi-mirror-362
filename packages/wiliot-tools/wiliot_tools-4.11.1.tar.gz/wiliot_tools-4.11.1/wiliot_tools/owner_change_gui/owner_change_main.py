...
class OwnerChangeApp(ctk.CTk):
    ...
    def save_api_keys(self, api_key, gcp_api_key, wmt_api_key):
    ...
    def setup_config(self):
        ...
        if not os.path.exists('resources/config.ini'):
            config['API'] = {'key': '', 'gcp_key': '', 'wmt_key': '', 'tadbik': 'False'}
            with open('resources/config.ini', 'w') as configfile:
                config.write(configfile)
        config.read('resources/config.ini')
        self.api_key = config['API'].get('key', '')
        self.gcp_api_key = config['API'].get('gcp_key', '')
    ...
    def check_client(self):
        ...
        if self.prev_gcp_api_key != self.gcp_api_key and self.gcp_api_key != "":
    ...
    def send_request_file(self, from_owner, to_owner, df, cloud_destination=None):
        ...
        df.to_csv(file_path)
        if cloud_destination != 'GCP' and cloud_destination != 'WMT':
