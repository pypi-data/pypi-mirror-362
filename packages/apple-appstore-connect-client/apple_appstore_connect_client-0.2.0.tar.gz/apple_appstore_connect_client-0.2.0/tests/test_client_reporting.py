...
from appstore_connect.exceptions import AppStoreConnectError, NotFoundError
...
class TestReportDateFormatting:
    ...
    def test_weekly_report_date_formatting(self, api_client):
        ...
        expected_sunday = date(2023, 6, 11)  # Previous Sunday
        ...
        with patch.object(api_client, '_make_request') as mock_request:
            with patch.object(api_client, '_generate_token', return_value='token'):
                with patch('appstore_connect.client.datetime') as mock_datetime:
                    ...
                    has_daily = any(df.iloc[0]['frequency'] == 'DAILY' for df in all_sales if not df.empty)
                    has_weekly = any(df.iloc[0]['frequency'] == 'WEEKLY' for df in all_sales if not df.empty)
