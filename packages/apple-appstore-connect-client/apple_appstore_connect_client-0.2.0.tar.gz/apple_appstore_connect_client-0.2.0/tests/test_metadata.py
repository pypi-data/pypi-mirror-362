    ...
    return {
        '123456789': {
            ...
            'metadata': {
                ...
                'version_info': {'versionString': '1.0.0', 'appStoreState': 'READY_FOR_SALE'}
            },
        ...
        '987654321': {
            ...
            'metadata': {
                ...
                'version_info': {'versionString': '1.1.0', 'appStoreState': 'PREPARE_FOR_SUBMISSION'}
            },
            'editable_version': {
                'id': 'version_123',
                'attributes': {
...
class TestMetadataManager:
    ...
    def test_get_app_portfolio_cached(self, metadata_manager, sample_portfolio_data):
        """Test getting portfolio with cached data."""
        metadata_manager._app_cache = sample_portfolio_data
        
        result = metadata_manager.get_app_portfolio(refresh_cache=False)
    ...
    def test_get_app_portfolio_fresh(self, mock_datetime, metadata_manager, mock_api):
        ...
        assert '123456789' in result
        assert result['123456789']['basic_info']['name'] == 'Test App'
        assert result['123456789']['editable_version'] is None
    ...
    def test_update_app_listing_app_level_fields(self, metadata_manager, mock_api):
        ...
        assert result['privacy_url'] is True
        ...
        mock_api.update_privacy_url.assert_called_once_with('123456789', 'https://new.com/privacy', 'en-US')
    
    def test_update_app_listing_version_level_fields_with_editable_version(self, metadata_manager, mock_api):
        ...
        # Verify API calls
        mock_api.get_editable_version.assert_called_once_with('123456789')
        mock_api.update_app_description.assert_called_once_with('123456789', 'New description', 'en-US')
        mock_api.update_app_keywords.assert_called_once_with('123456789', 'new,keywords', 'en-US')
    
    def test_update_app_listing_version_level_fields_no_editable_version(self, metadata_manager, mock_api):
    ...
    def test_update_app_listing_validation_errors(self, metadata_manager, mock_api):
        ...
        assert result['description'] is False
        assert result['keywords'] is False
        assert result['promotional_text'] is False
    ...
    def test_batch_update_apps(self, metadata_manager):
        ...
        assert '123456789' in result
        assert '987654321' in result
        assert result['123456789']['name'] is True
        assert result['987654321']['subtitle'] is True
    ...
    def test_batch_update_apps_with_error(self, metadata_manager):
        ...
        assert result['123456789']['name'] is True
        assert 'error' in result['invalid_id']
    
    def test_standardize_app_names_dry_run(self, metadata_manager, sample_portfolio_data):
    ...
    def test_standardize_app_names_with_truncation(self, metadata_manager, sample_portfolio_data):
    ...
    def test_prepare_version_releases_dry_run(self, metadata_manager, mock_api):
        ...
        assert result['123456789']['status'] == 'ready'
        assert result['987654321']['status'] == 'exists'
    ...
    def test_prepare_version_releases_actual(self, metadata_manager, mock_api):
        ...
        app_versions = {'123456789': '1.2.0'}
        
        result = metadata_manager.prepare_version_releases(app_versions, dry_run=False)
        
        assert result['123456789']['status'] == 'created'
        assert result['123456789']['version_id'] == 'new_version_123'
        
        # Verify version creation was called
        mock_api.create_app_store_version.assert_called_once_with('123456789', '1.2.0')
    
    def test_get_localization_status(self, metadata_manager, sample_portfolio_data):
    ...
    def test_export_app_metadata(self, mock_to_csv, metadata_manager, sample_portfolio_data):
...
class TestMetadataManagerErrorHandling:
    ...
    def test_batch_update_stop_on_error(self, metadata_manager):
        ...
        assert '123456789' in result
        assert len(result) == 2  # Both apps processed, but second has error