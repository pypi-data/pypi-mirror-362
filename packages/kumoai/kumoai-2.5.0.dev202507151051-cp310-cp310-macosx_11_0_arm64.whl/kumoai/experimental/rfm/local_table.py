...
class LocalTable:
    ...
    def __init__(
        ...
        if not all(col.replace('_', '').isalnum() for col in df.columns):
            raise ValueError("Data frame must have alphanumeric column names")
        ...
        for column_name in df.columns:
            dtype = utils.to_dtype(df[column_name].dtype, df[column_name])
    ...
    def print_metadata(self) -> None:
        ...
        if in_notebook():
            ...
            display(Markdown(f"### 🏷️ Metadata of Table `{self.name}`"))
            df = self.metadata
            try:
