...
def normalize_text(line: str) -> list[str]:
...
class LocalGraphStore:
    ...
    def get_node_id(self, table_name: str, pkey: pd.Series) -> np.ndarray:
        ...
        try:
            pkey = pkey.astype(pkey_map.index.dtype)
        ...
        * removes rows with missing primary keys or time values
        ...
        for table in graph.tables.values():
            for col in table.columns:
                ...
                if col.stype == Stype.text:  # Normalize text in advance:
                    ser = df_dict[table.name][col.name]
                    ser = ser.fillna('').str.lower()
                    ser = ser.map(normalize_text)
                    df_dict[table.name][col.name] = ser
    ...
    def get_time_data(
        ...
        for table in graph.tables.values():
            ...
            time_dict[table.name] = time.astype(int).to_numpy()
    ...
    def get_csc(
        ...
        for src_table, fkey, dst_table in graph.edges:
            ...
            if src_table in self.time_dict:
                perm = np.lexsort([self.time_dict[src_table][src], dst])
            else:
                perm = np.argsort(dst)
