...
def load_basecall():
    ...
    try:
        ...
        for f in os.listdir(path):
            ...
            st = "f {0} complex {1} type {2}".format(f, _complex, _type)
            print(st)
            ...
            if f.endswith(".csv"):
                ...
                'position=@position,contig=@contig,basecall_reactivity=@reactivity,quality=@quality,mismatch=@mis,'
                                                              ...
                                                              'sequence=@sequence,complex='+str(_complex)+',type='+_type+'; \n')
...
def load_putative_structure():
    ...
    try:
        #LOAD DATA
        fname = '/Users/timshel/structure_landscapes/DashML/Deconvolution/putative_structures.csv'
        ...
        'SET ' +
        'position=@position,contig=@contig,sequence=@sequence; \n')
...
def load_dmso_raw(seqmap=None):
    con = create_connection()
    cursor = con.cursor()
    try:

        if seqmap == None:
            data_path ='/Users/timshel/Downloads/RAW_DMSO/temp/'
            seqmap = {
                "E_coli": data_path + "DMSO_e.txt",
                "hc16_ligase" : data_path + "DMSO_hc16.txt",
                "FMN": data_path + "DMSO_fmn.txt",
                "FMN_Run_2": data_path + "DMSO_fmn2.txt",
                "FMN_Lys_Run_3": data_path + "DMSO_fmn_lys_3.txt",
                "Lysine": data_path + "DMSO_lys.txt",
                "Lysine_Run_2": data_path + "DMSO_lys2.txt",
                # "cen_3'utr": data_path + "DMSO/cen_3'utr_decon_signal_preprocess.csv",
                # "cen_3'utr_complex" : data_path + "DMSO/cen_3'utr_complex_decon_signal_preprocess.csv",
                # "cen_FL": data_path + "DMSO/cen_FL_decon_signal_preprocess.csv",
                # "cen_FL_complex" : data_path + "DMSO/cen_FL_complex_decon_signal_preprocess.csv",
                ...
                # "ik2_FL": data_path + "DMSO/ik2_FL_decon_signal_preprocess.csv",
                # "ik2_FL_complex" : data_path + "DMSO/ik2_FL_complex_decon_signal_preprocess.csv",
                "RNAse_P": data_path + "DMSO_signal_rnasep.txt",
                "HSP70_HSPA1A_37": data_path + "DMSO_37_HSP.txt",
                "HSP70_HSPA1A_42": data_path + "DMSO_42.txt",
                "HCV": data_path + "DMSO_signal_hcv.txt",
                "T_thermophila": data_path + "DMSO_signal_t.txt"
            }
        ...
        for s,f in seqmap.items():
            ...
            queryacim = ("LOAD DATA LOCAL INFILE \"" + f + "\" INTO TABLE jwbear_DASH.RAW " +
                       "FIELDS TERMINATED BY '\t' " +
                       "LINES TERMINATED BY '\n' " +
                       'IGNORE 1 ROWS ' +
                       '(@contig,@position,@reference_kmer,@read_index,@strand,@event_index,@event_level_mean,'
                       '@event_stdv,@event_length,@model_kmer,@model_mean,@model_stdv,@standardized_level) ' +
                       'SET ' +
                       'position=@position,contig=@contig,read_index=@read_index,reference_kmer=@reference_kmer,'
                       'event_level_mean=@event_level_mean,event_length=@event_length,'
                       'event_stdv=@event_stdv,lid=1,temp='+str(_temp)+',run='+str(_run)+',timestamp="'+_timestamp+'"; \n')
...
def load_acim_raw(seqmap=None):
    ...
    try:
        ...
        if seqmap == None:
            data_path ='/Users/timshel/Downloads/RAW_DMSO/temp/'
            seqmap = {
                "Pool": data_path + "ACIM_pool.txt", #ecoli,ttherm,hcv, cen, ik2
                "hc16_rnasep" : data_path + "ACIM_hc16_rnasep.txt",
                "FMN": data_path + "ACIM_fmn.txt",
                "FMN_Lys_Run_2": data_path + "ACIM_fmn_lys2.txt",
                "Lysine": data_path + "ACIM_lys.txt",
                "Pool_Run_2": data_path + "ACIM_p2.txt", #hcv, ttherm, cen,ik2
                # "cen_3'utr": data_path + "DMSO/cen_3'utr_decon_signal_preprocess.csv",
                # "cen_3'utr_complex" : data_path + "DMSO/cen_3'utr_complex_decon_signal_preprocess.csv",
                # "cen_FL": data_path + "DMSO/cen_FL_decon_signal_preprocess.csv",
                # "cen_FL_complex" : data_path + "DMSO/cen_FL_complex_decon_signal_preprocess.csv",
                ...
                # "ik2_FL": data_path + "DMSO/ik2_FL_decon_signal_preprocess.csv",
                # "ik2_FL_complex" : data_path + "DMSO/ik2_FL_complex_decon_signal_preprocess.csv",
                "RNAse_P": data_path + "ACIM_rnasep.txt",
                "HSP70_HSPA1A_37": data_path + "ACIM_37_HSP.txt",
                "HSP70_HSPA1A_42": data_path + "ACIM_42.txt",
                "HSP70_HSPA1A_42_Run_2": data_path + "ACIM_422.txt"
            }
        ...
        for s,f in seqmap.items():
            ...
            queryacim = ("LOAD DATA LOCAL INFILE \"" + f + "\" INTO TABLE jwbear_DASH.RAW_MOD " +
                       "FIELDS TERMINATED BY '\t' " +
                       "LINES TERMINATED BY '\n' " +
                       'IGNORE 1 ROWS ' +
                       '(@contig,@position,@reference_kmer,@read_index,@strand,@event_index,@event_level_mean,'
...
def load_library():
    ...
    try:
        #LOAD DATA
        fname = '/Users/timshel/structure_landscapes/DashML/library.fasta'

        #read fasta into  dictionay
            ...
            with open(filepath, 'r') as f:
                for line in f:
                    if line.startswith('>'):
                        ...
                        sequence = line[:-1].upper().strip()
                        sequence_len = len(sequence)
                        sequences.append((sequence_name, sequence_len, sequence))
                        #print(sequence_len)
                        #print(line)
            return sequences

        sequences = parse_fasta(fname)
        vals=""
        for s in sequences:
            vals = vals + str(s) + ','
        vals = vals[:-1]

        query = ("INSERT INTO jwbear_DASH.Library (sequence_name, sequence_len, sequence) VALUES {0};".format(vals))
        ...
        # assign data query
        query2 = "select * from jwbear_DASH.Library"
        cursor.execute(query2)
