...
from typing import TYPE_CHECKING, Any, Optional, Union, TypedDict
...
class ChatGradientAI(BaseChatModel):
    api_key: str = Field(default=os.environ.get("DIGITALOCEAN_INFERENCE_KEY"))
    """GradientAI API key."""
    ...
    def _update_parameters_with_model_fields(self, parameters: dict):
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            print("Yielding:", type(chunk))
            yield chunk
            # Only yield usage metadata if requested via stream_options
            if getattr(self, "stream_options", None) and self.stream_options.get(
                "include_usage"
            ):
                usage = getattr(completion, "usage", {})
                usage_metadata = {
