...
class SubSection:
    def __init__(self, section_class, name: str = "subcommands", description: str = None):
    ...
    def mount_sub_section(self, arg_parser):
        sub_section_parser = arg_parser.add_parser(name=self.name, help=self.description)
        ...
        for name, func in inspect.getmembers(self.section_class, predicate=inspect.isfunction):
            if not getattr(func, "_is_command", False):
                continue

            parser = self.subparser.add_parser(func._command_name, description=parsers.extract_description(func._command_help), formatter_class=argparse.RawDescriptionHelpFormatter)
            ...
            def func_caller(func_to_run=func, **kwargs):
                ...
                object_instance = self.section_class(**{
                    k: v for k, v in kwargs.items() if k in self.init_signature.parameters.keys()
                })
                
                # Run the method on the interface class
                result = func_to_run(object_instance, **{
                    k: v for k, v in kwargs.items() if k in inspect.signature(func_to_run).parameters.keys()
                })
            ...
            for param in argspec.parameters.values():
                ...
                try:
                    ...
                    print(f"Failed to setup command {self.name}.{name} parameter type: {str(param.annotation)} is unknown, please add a handler")
        ...
        for name, func in inspect.getmembers(self.section_class, predicate=inspect.isfunction):
            ...
            for param in self.init_signature.parameters.values():
                ...
                if param.default is not inspect.Parameter.empty:
                    ...
                    print(f"Error, grcp class ({self.section_class.__name__}) has a non default __init__ paramater: {param_name}")
                    continue
