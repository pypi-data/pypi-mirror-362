...
def addon_mixin_fixture():
    ...
    os.makedirs(os.path.join(server.server_dir, "development_resource_packs"), exist_ok=True)
    os.makedirs(os.path.join(server.server_dir, "development_behavior_packs"), exist_ok=True)
...
def test_list_world_addons(addon_mixin_fixture):
    ...
    with open(os.path.join(world_dir, "resource_packs", "rp1_folder", "manifest.json"), "w") as f:
        json.dump({"header": {"name": "rp1", "uuid": "rp1_uuid", "version": [1, 0, 0]}, "modules": [{"type": "resources"}]}, f)
...
def test_process_addon_file(addon_mixin_fixture):
    ...
    with zipfile.ZipFile(addon_path, 'w') as zf:
        zf.writestr("manifest.json", '{"header": {"name": "test addon", "uuid": "rp1", "version": [1,0,0]}, "modules": [{"type": "resources"}]}')
...
def test_remove_addon(addon_mixin_fixture):
    ...
    addon_dir = os.path.join(server.server_dir, "worlds", "Bedrock level", "resource_packs", "test_addon_1.0.0")
    os.makedirs(addon_dir)
    with open(os.path.join(addon_dir, "manifest.json"), "w") as f:
        json.dump({"header": {"name": "test_addon", "uuid": "rp1", "version": [1,0,0]}, "modules": [{"type": "resources"}]}, f)
