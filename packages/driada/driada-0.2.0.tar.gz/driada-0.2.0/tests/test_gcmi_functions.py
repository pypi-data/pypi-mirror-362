"""Unit tests for GCMI functions: demean, ent_g, mi_model_gd, and gccmi_ccd.

This module provides comprehensive tests for the Gaussian Copula Mutual Information
functions that were not previously covered by existing tests.

CRITICAL ISSUES DISCOVERED:
==========================
1. GCMI ent_g function produces negative joint entropy for near-perfectly correlated variables
2. This violates the fundamental theorem H(X,Y) ≥ max(H(X), H(Y))
3. Root cause: GCMI cannot handle near-singular covariance matrices (condition number > 30k)
4. Same bug pattern as CDC case negative CMI issue
5. Problem exists both with and without bias correction

TODO HIGH PRIORITY FIXES NEEDED:
================================
- Fix GCMI numerical instability for ill-conditioned covariance matrices
- Add robustness checks for near-singular matrices
- Implement fallback entropy estimation methods
- Document GCMI limitations clearly
"""

import numpy as np
import pytest
from src.driada.information.gcmi import demean, ent_g, mi_model_gd, gccmi_ccd
from src.driada.information.info_base import TimeSeries, get_mi, conditional_mi
from scipy import stats as scipy_stats


def test_demean_single_row():
...
def test_demean_numerical_stability():
...
def test_ent_g_univariate_standard_normal():
...
def test_ent_g_univariate_scaled():
...
def test_ent_g_multivariate_independent():
...
def test_ent_g_multivariate_correlated():
...
def test_ent_g_bias_correction():
...
def test_mi_model_gd_perfect_dependence():
...
def test_mi_model_gd_independence():
...
def test_mi_model_gd_multivariate():
...
def test_mi_model_gd_different_class_sizes():
...
def test_mi_model_gd_many_classes():
...
def test_mi_model_gd_input_validation():
    """Test input validation."""
...
def test_gccmi_ccd_chain_structure():
...
def test_gccmi_ccd_common_cause():
...
def test_gccmi_ccd_multivariate():
...
def test_gccmi_ccd_independence_given_z():
...
def test_gccmi_ccd_perfect_dependence():
...
def test_gccmi_ccd_warning_repeated_values():
...
def test_gccmi_ccd_input_validation():
    """Test input validation."""
    x = np.random.randn(1, 100)
...
def test_conditional_mi_ccd_consistency():
...
def test_mi_model_gd_consistency():
