...
class DistributedWeightedRandomSampler(Sampler[int]):
    ...
    def __init__(
        ...
        if len(weights_tensor.shape) != 1:
            raise ValueError(
                f"'weights' should be a 1D sequence but given weights have shape {tuple(weights_tensor.shape)}"
            )
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        rand_tensor = torch.multinomial(
            self.weights[self.mask], self.num_samples, self.replacement, generator=self.generator
        )
