...
class VIVFatigueAnalysisComponents:
    ...
    def get_viv_modes_excited(self):
        ...
        cfg_temp_start_line = {
            "io": None,
            "line": {
                "key_words": [start_line_keyword_text],
                "transform": {"scale": 1, "shift": 3},
            },
        }
        cfg_temp_end_line = {
            "io": None,
            "line": {
                "key_words": [end_line_keyword_text],
        ...
        for file_index in range(0, len(self.cfg["Shear7_sets"])):
            ...
            io = (
                self.cfg["Shear7_sets"][file_index]["io"][
                    0 : len(self.cfg["Shear7_sets"][file_index]["io"]) - 3
                ]
                + "out"
            )
            ...
            try:
                start_line = (
                    read_data.from_ascii_file_get_line_number_containing_keywords(
                        cfg_temp_start_line
                    )
                )
                end_line = (
                ...
                file_df = (
                    read_data.from_ascii_file_get_structured_data_delimited_white_space(
                        cfg_temp
                    )
                )
                dominant_mode = file_df[file_df["Power Ratio"] == 1][
                    "Mode Number"
                ].iloc[0]
                summary_row = [
                    io,
                    dominant_mode,
                    file_df["Mode Number"].min(),
                    file_df["Mode Number"].max(),
                ]
                self.viv_modes_excited_summary.loc[
                    len(self.viv_modes_excited_summary)
                ] = summary_row
            except:
            ...
            save_data.DataFrameArray_To_xlsx_openpyxl(
                self.viv_modes_excited_df_array, cfg_temp
            )
            file_name = (
                self.cfg["Analysis"]["result_folder"]
                + self.cfg["Analysis"]["file_name"]
                + "_"
                + "viv_modes_excited_summary.csv"
            )
        ...
        for file_index in range(0, len(self.viv_plt_df_array)):
            if not self.viv_plt_df_array[file_index].empty:
                probability_of_occurrence = (
                    self.cfg["Shear7_sets"][file_index]["probability"] / 100
                )
                self.viv_plt_df_array[file_index]["current_profile_damage_per_year"] = (
                    self.viv_plt_df_array[file_index]["Damage (1/year)"]
                    * probability_of_occurrence
                )
                self.total_viv_fatigue["total_damage"] = (
                    self.total_viv_fatigue["total_damage"]
                    + self.viv_plt_df_array[file_index][
                        "current_profile_damage_per_year"
                    ]
                )

        self.total_viv_fatigue = self.add_riser_length_to_shear7_result(
            self.total_viv_fatigue
        )
            ...
            for table_index in range(0, len(self.cfg.viv_tables["length"])):
                ...
                for file_index in range(0, len(self.viv_plt_df_array)):
                    ...
                    if self.cfg["Shear7_sets"][file_index].__contains__("duration"):
                        ...
                        duration = 20
                    if not self.viv_plt_df_array[file_index].empty:
                        max_damage_rate = self.viv_plt_df_array[file_index][
                            "Damage (1/year)"
                        ][
                            (
                                self.viv_plt_df_array[file_index][
                            ...
                            & (
                                ...
                                < self.cfg.viv_tables["length"][table_index]["end"]
                            )
                        ].max()
                        max_damage_per_year = self.viv_plt_df_array[file_index][
                            "current_profile_damage_per_year"
                        ][
                            (
                                self.viv_plt_df_array[file_index][
                        ...
                        event_damage_with_fos = (
                            event_damage
                            * self.cfg.default["Analysis"]["viv_fatigue"][
                                "factor_of_safety"
                            ]
                        )
                        row_array = [
    ...
    def tabulate_overall_viv_results(self):
        ...
        if self.cfg.__contains__("viv_tables"):
            ...
            cfg_temp = {
                ...
                + self.cfg["Analysis"]["file_name"]
                + "_"
                + "total_viv_fatigue.xlsx",
                "SheetNames": ["total_viv_fatigue"],
            }
            save_data.DataFrameArray_To_xlsx_openpyxl(
                [self.total_viv_fatigue], cfg_temp
            )

    def plot_viv_tables(self):
        if self.cfg.__contains__("viv_tables") and self.cfg.viv_tables.__contains__(
            "plot"
        ):
            for table_index in range(0, len(self.cfg.viv_tables["length"])):
                viz = Visualization()
                ...
                plt_settings.update(
                    {
                        "file_name": self.cfg["Analysis"]["result_folder"]
                        + self.cfg["Analysis"]["file_name"]
                        + "_"
                        + plt_settings["file_suffix"]
                        + "_"
                        + self.cfg.viv_tables["length"][table_index]["label"]
                        + ".png"
                    }
                )
                ...
                viz.add_legend()
                print(
                    "Saving {0} plot for event damage  ..".format(
                        plt_settings["file_suffix"]
                    )
                )
                viz.save_and_close()
            ...
            for table_index in range(0, len(self.cfg.viv_tables["length"])):
                ...
                viz.add_title_and_axis_labels()
                print(
                    "Saving {0} plot for percentage damage  ..".format(
                        plt_settings["file_suffix"]
                    )
                )
                viz.save_and_close()
            ...
            for table_index in range(0, len(self.cfg.viv_tables["length"])):
                ...
                df = (
                    self.viv_tables[table_index]
                    .sort_values(by=["percent_damage"], ascending=False)
                    .copy()
                )
                if len(df) > 20:
    ...
    def add_riser_length_to_shear7_result(self, df):
        ...
        if hasattr(self, "riser_stack_up_properties"):
            logging.info(
                "Utilizing riser stack up properties for Shear7 X calculations"
            )
            xp = self.riser_stack_up_properties.shear7_bottom_x_by_L.tolist()
            fp = (
                self.riser_stack_up_properties.component_bottom_elevation_above_mudline.tolist()
            )
            result = np.interp(x, xp, fp)
        else:
            logging.info(
                "Utilizing model_state_information from configuration file for Shear7 X calculations"
            )
    ...
    def save_viv_fatigue_life_visualizations(self):
        ...
        for plot_index in range(0, len(self.cfg.viv_plots["individual"])):
            ...
            if self.cfg.Shear7_sets[0].__contains__("label"):
                plt_settings.update(
                    {"array_label": [[item["label"]] for item in self.cfg.Shear7_sets]}
                )
            else:
                plt_settings.update(
                    {
                        "array_label": [
                            [str(item)] for item in range(0, len(self.cfg.Shear7_sets))
                        ]
                    }
                )
    ...
    def add_riser_length_axis_to_orcaflex_result(self):
        ...
        xp = (
            self.riser_stack_up_properties.component_bottom_elevation_from_end_A.tolist()
        )
        fp = (
            self.riser_stack_up_properties.component_bottom_elevation_above_mudline.tolist()
        )
    ...
    def get_combined_fatigue(self):
        ...
        self.combined_fatigue["elevation_above_mudline"] = self.total_viv_fatigue[
            "elevation_above_mudline"
        ]
        self.combined_fatigue["total_viv_fatigue_damage"] = self.total_viv_fatigue[
            "total_damage"
        ]
        self.combined_fatigue[
            "total_wave_fatigue_damage"
        ] = self.get_wave_fatigue_converted_to_viv_divisions()
        self.combined_fatigue["total_combined_fatigue_damage"] = (
            self.total_viv_fatigue["total_damage"]
            + self.combined_fatigue["total_wave_fatigue_damage"]
        )
    ...
    def get_viv_cummulative_histograms(self):
        ...
        self.shear7_bins = list(
            range(
                self.cfg["histogram"]["shear7"]["range"][0],
                self.cfg["histogram"]["shear7"]["range"][1],
                self.cfg["histogram"]["shear7"]["bins"],
            )
        )
        self.viv_cummulative_histograms = pd.DataFrame(
            self.shear7_bins, columns=["bins"]
        )
        for label_index in range(0, len(self.viv_histogram_df_array[0].columns) - 1):
            label = self.histogram_labels[label_index]
            self.viv_cummulative_histograms[label] = pd.Series(
                [0] * len(self.shear7_bins)
            )
        ...
        for file_index in range(0, len(self.cfg["Shear7_sets"])):
            ...
            self.probability_array.append(
                self.cfg["Shear7_sets"][file_index]["probability"]
            )
            for label_index in range(
                0, len(self.viv_histogram_df_array[0].columns) - 1
            ):
                label = self.histogram_labels[label_index]
                self.viv_cummulative_histograms[label] = (
                    self.viv_cummulative_histograms[label]
                    + self.viv_histogram_df_array[file_index][label]
    ...
    def get_yearly_histogram_object_from_viv_run(self, file_index):
        ...
        for line_item_index in range(0, len(self.riser_stack_up_properties)):
            if self.riser_stack_up_properties.iloc[line_item_index]["stack_length"] > 0:
                component = self.riser_stack_up_properties.iloc[line_item_index][
                    "component"
                ]
                ...
                component_average_elevation_above_mudline = statistics.mean(
                    [
                        self.riser_stack_up_properties.iloc[line_item_index][
                ...
                label = (
                    ...
                    + ", "
                    + RadialPos
                    + "Dia, max_dam, @avg elev {:.1f}".format(
                ...
                histogram_object = (
                    self.get_histogram_object_at_shear7_x_by_L_from_viv_run(
                        file_index, shear7_x_by_L
                    )
                )
                transformed_histogram_object = list(
                    map(
                        lambda x: (x * (365.25 * 24 * 3600) / natural_time_period),
                        histogram_object,
                    )
                )
                histogram_file[label] = pd.Series(
                    transformed_histogram_object, index=histogram_file.index
                )

        return histogram_file

    def get_histogram_object_at_shear7_x_by_L_from_viv_run(
        self, file_index, shear7_x_by_L
    ):
        x = shear7_x_by_L
        if not self.viv_plt_df_array[file_index].empty:
            ...
            for i in range(0, len(self.shear7_bins)):
                if (stress_half_cycle > self.shear7_bins[i]) and (
                    stress_half_cycle < self.shear7_bins[i + 1]
                ):
    ...
    def get_viv_fundamental_natural_frequency(self, file_index):
        read_data = ReadData()
        cfg_temp = self.cfg["histogram"]["shear7"]["fundamental_natural_frequency"][
            "from_ascii"
        ].copy()
        io = (
            self.cfg["Shear7_sets"][file_index]["io"][
        ...
        keyword_line_number = (
            read_data.from_ascii_file_get_line_number_containing_keywords(cfg_temp)
        )
    ...
    def get_combined_histograms(self):
        self.combined_histograms = pd.DataFrame(
            columns=self.wave_cummulative_histograms.columns
        )
        ...
        for label_index in range(0, len(self.wave_cummulative_histograms.columns)):
            ...
            self.combined_histograms[label] = (
                self.wave_cummulative_histograms[label]
                + self.viv_cummulative_histograms[label]
            )
    ...
    def save_combined_histograms(self):
        ...
        save_data = SaveData()
        df_array = [
            self.combined_histograms,
            self.wave_cummulative_histograms,
            self.viv_cummulative_histograms,
        ]
        cfg_temp = {
            "FileName": self.cfg["Analysis"]["result_folder"]
            + self.cfg["Analysis"]["file_name"]
            + "_combined_histograms.xlsx",
    ...
    def save_fatigue_summary_result(self):
        ...
        columns_for_tabular_results = [
            ...
            "StressID",
            "max_damage",
            "elevation_above_mudline_at_max_damage",
            "total_viv_max_damage",
            "total_wave_max_damage",
        ]
        column_headers_for_tabular_results = [
            "Component",
            "Bottom Elev above Mudline",
            "Lenth",
            "Stress OD",
            "Stress ID",
            "Max Damage",
        ...
        tabular_results_df = self.riser_stack_up_properties[
            self.riser_stack_up_properties["stack_length"] > 0
        ][columns_for_tabular_results].copy()
        tabular_decimal_array = pd.Series(
            [0, 2, 1, 4, 4, 5, 2, 5, 5], index=tabular_results_df.columns.values
        )
        ...
        for row_index in range(0, len(tabular_results_df)):
            ...
            tabular_results_df.total_viv_max_damage.iloc[
                row_index
            ] = np.format_float_scientific(
                tabular_results_df.total_viv_max_damage.iloc[row_index],
                unique=False,
                precision=2,
            )
            tabular_results_df.total_wave_max_damage.iloc[
                row_index
            ] = np.format_float_scientific(
                tabular_results_df.total_wave_max_damage.iloc[row_index],
                unique=False,
                precision=2,
            )
        ...
        save_data.df_to_table_as_image(
            tabular_results_df,
            cfg={"file_name": basefile_name + "_damage_result_summary"},
        )
        save_data.df_to_table_as_docx(
            tabular_results_df,
            cfg={"file_name": basefile_name + "_damage_result_summary"},
        )
    ...
    def add_fatigue_damage_results_to_df(self):
        for line_item_index in range(0, len(self.riser_stack_up_properties)):
            if self.riser_stack_up_properties.iloc[line_item_index]["stack_length"] > 0:
                component_start = self.riser_stack_up_properties.iloc[line_item_index][
                    "component_bottom_elevation_above_mudline"
                ]
                component_end = self.riser_stack_up_properties.iloc[line_item_index][
                    "component_top_elevation_above_mudline"
                ]
                if line_item_index - 1 >= 0:
                    if (
                        self.riser_stack_up_properties.iloc[line_item_index - 1][
                            "component"
                        ]
                        == "Upper FJ"
                    ):
                ...
                damage_along_component = self.combined_fatigue[
                ...
                self.riser_stack_up_properties.loc[
                    line_item_index, "max_damage"
                ] = maximum_damage
                self.riser_stack_up_properties.loc[
                    line_item_index, "total_viv_max_damage"
                ] = self.combined_fatigue.iloc[maximum_damage_index][
                    "total_viv_fatigue_damage"
                ]
                self.riser_stack_up_properties.loc[
                    line_item_index, "total_wave_max_damage"
                ] = self.combined_fatigue.iloc[maximum_damage_index][
                    "total_wave_fatigue_damage"
                ]
                elevation_above_mudline_at_max_damage = self.combined_fatigue.iloc[
                    maximum_damage_index
                ]["elevation_above_mudline"]
                self.riser_stack_up_properties.loc[
                    line_item_index, "elevation_above_mudline_at_max_damage"
                ] = elevation_above_mudline_at_max_damage
                self.riser_stack_up_properties.loc[
                    line_item_index, "shear7_x_by_L_at_max_damage"
                ] = self.get_shear7_x_by_L_from_elevation_above_mudline(
                    elevation_above_mudline_at_max_damage
                )
                self.riser_stack_up_properties.loc[
                    line_item_index, "elevation_from_end_A_at_max_damage"
                ] = self.get_elevation_from_end_A_from_elevation_above_mudline(
                    elevation_above_mudline_at_max_damage
                )
            else:
    ...
    def orcaflex_wave_fatigue_analysis(self):
        ...
        if cfg["default"]["plot_fatigue_life"]:
            ...
            dfSummary = pd.DataFrame(
                columns=["FileName"] + cfg["FatigueLifeReadingSets"][0]["Label"]
            )
            for fileIndex in range(0, len(cfg["FatigueLifeReadingSets"])):
                customdata = {
                    "FileName": cfg["FatigueLifeReadingSets"][fileIndex]["io"],
                    "SheetName": cfg["FatigueLifeReadingSets"][fileIndex]["sheet_name"],
                    "KeyWords": cfg["FatigueLifeReadingSets"][fileIndex]["KeyWords"],
                    "RowsToSkip": cfg["FatigueLifeReadingSets"][fileIndex][
                        "RowsToSkip"
                    ],
                    "RowsToRead": cfg["FatigueLifeReadingSets"][fileIndex][
                        "RowsToRead"
                    ],
                    "Columns": cfg["FatigueLifeReadingSets"][fileIndex]["Columns"],
                    "FactorOfSafety": cfg["FatigueLifeReadingSets"][fileIndex][
                        "FactorOfSafety"
                    ],
                }
                ...
                #  For plot
                customdata = {
                    "PltSupTitle": cfg["FatigueLifePlotSettings"]["PltSupTitle"],
                    ...
                    "TextFields": [
                        {"x": 2000, "y": 40, "Text": "Minimum Fatigue Life"}
                    ],
                    "FileName": cfg["Analysis"]["result_folder"]
                    + os.path.basename(
                        cfg["FatigueLifeReadingSets"][fileIndex]["io"]
                    ).replace(".xlsx", ""),
                ...
                for SNCurveIndex in range(0, len(UniqueSNCurves)):
                    rowDF.append(
                        df[df["S-N Curve"] == UniqueSNCurves[SNCurveIndex]][
                            customdata["PltColumns"][1]
                        ].min()
                    )
        ...
        if cfg["default"]["plot_damage_distribution"]:
            ...
            for fileIndex in range(0, len(cfg["DamageContribution"]["ReadingSets"])):
                customdata = {
                    "FileName": cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "io"
                    ],
                    "SheetName": cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "sheet_name"
                    ],
                    "KeyWords": cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "KeyWords"
                    ],
                    "RowsToSkip": cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "RowsToSkip"
                    ],
                    "RowsToRead": cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "RowsToRead"
                    ],
                    "Columns": cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "Columns"
                    ],
                }
                ...
                for set_index in range(
                    0,
                    len(
                        cfg["DamageContribution"]["ReadingSets"][fileIndex][
                            "direction_sets"
                        ]["sets"]
                    ),
                ):
                    start_row = cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "direction_sets"
                    ]["sets"][set_index]["start_row"]
                    end_row = cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "direction_sets"
                    ]["sets"][set_index]["end_row"]
                    column = cfg["DamageContribution"]["ReadingSets"][fileIndex][
                        "direction_sets"
                    ]["columns"][set_index]
                    df_by_direction[column] = (
                        df.ix[start_row:end_row, max_column].values
                        / sum_of_damage
                        * 100
                    )

                damage_by_direction_array.append(df_by_direction)
                damage_by_direction_sum_array_column = cfg["DamageContribution"][
                    "ReadingSets"
                ][fileIndex]["label"]
                damage_by_direction_summed_df[
                    damage_by_direction_sum_array_column
                ] = df_by_direction.sum().values
                damage_by_direction_summed_df.index = (
                    damage_by_direction_array[0].sum().index
                )
                plt_settings = cfg["DamageContribution"]["ReadingSets"][fileIndex][
                    "viz"
                ]
                plt_settings.update(
                    {
                        "file_name": cfg["Analysis"]["result_folder"]
                        + cfg["Analysis"]["file_name"]
                        + "_damage_by_direction_"
                        + "{0}".format(fileIndex)
                        + ".png"
                    }
                )
