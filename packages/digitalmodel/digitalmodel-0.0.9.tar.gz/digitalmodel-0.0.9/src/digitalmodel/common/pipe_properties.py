...
class PipeProperties:
    ...
    def get_properties(self, cfg, pipe_system):
        pipe_properties = []
    ...
    def get_system_properties(self, pipe_properties):
        ...
        mass_in_air_without_internal_fluid = sum(
            [
                pipe["equivalent"]["mass"]["air"]["without_internal_fluid"]
                for pipe in pipe_properties
            ]
        )
        mass_in_air_with_internal_fluid = sum(
            [
                pipe["equivalent"]["mass"]["air"]["with_internal_fluid"]
                for pipe in pipe_properties
            ]
        )
        mass_in_water_without_internal_fluid = sum(
            [
                pipe["equivalent"]["mass"]["water"]["without_internal_fluid"]
                for pipe in pipe_properties
            ]
        )
        mass_in_water_with_internal_fluid = sum(
            [
                pipe["equivalent"]["mass"]["water"]["with_internal_fluid"]
                for pipe in pipe_properties
            ]
        )

        system_properties["mass"] = {
            "air": {
                "without_internal_fluid": mass_in_air_without_internal_fluid,
                "with_internal_fluid": mass_in_air_with_internal_fluid,
            },
            "water": {
                "without_internal_fluid": mass_in_water_without_internal_fluid,
                "with_internal_fluid": mass_in_water_with_internal_fluid,
            },
        }

        system_properties["A"] = sum([pipe["section"]["A"] for pipe in pipe_properties])
        system_properties["EI"] = sum(
            [pipe["equivalent"]["EI"] for pipe in pipe_properties]
        )
        system_properties["EA"] = sum(
            [pipe["equivalent"]["EA"] for pipe in pipe_properties]
        )
        system_properties["GJ"] = sum(
            [pipe["equivalent"]["GJ"] for pipe in pipe_properties]
        )
        ...
        system_properties["Poissonsratio"] = pipe_properties[0]["equivalent"][
            "Poissonsratio"
        ]
    ...
    def pipe_properties(self, pipe_cfg):
        if pipe_cfg["Nominal_ID"] == None:
            pipe_cfg["Nominal_ID"] = pipe_cfg["Nominal_OD"] - 2 * pipe_cfg["Design_WT"]
        ID_with_corrosion = (
            pipe_cfg["Nominal_ID"]
            + 2
            * pipe_cfg["Corrosion_Allowance"]
            * pipe_cfg["average_internal_metal_loss_percentage"]
            / 100
        )

        if pipe_cfg["Nominal_OD"] == None:
            pipe_cfg["Nominal_OD"] = pipe_cfg["Nominal_ID"] + 2 * pipe_cfg["Design_WT"]
        ...
        dimensions = {
            "nominal": {
                "OD": pipe_cfg["Nominal_OD"],
                "ID": pipe_cfg["Nominal_ID"],
                "WT": pipe_cfg["Design_WT"],
            },
            "with_corrosion": {
                "OD": pipe_cfg["Nominal_OD"],
                "ID": ID_with_corrosion,
                "WT": WT_with_corrosion,
            },
        }
        ...
        internal_fluid_properties = self.get_internal_fluid_properties(
            pipe_cfg, pipe_properties
        )
    ...
    def get_material_properties(self, pipe_cfg):
        ...
        material = pipe_cfg["Material"]["name"]
        if material == "steel":
            material_properties = pipe_cfg["Material"].copy()
            material_grade = pipe_cfg["Material"]["grade"]
            material_properties_master = steel_material_properties_ref["Grades"][
                material_grade
            ]
            material_properties = update_deep_dictionary(
                material_properties_master, material_properties
            )
            material_properties = update_deep_dictionary(
                steel_material_properties_ref["common"], material_properties
            )
    ...
    def get_equivalent_properties(self, pipe_properties):
        ...
        mass_air_without_internal_fluid = section_properties["mass"] + sum(
            [coating["mass"] for coating in coating_properties]
        )
        mass_air_with_internal_fluid = (
            mass_air_without_internal_fluid + internal_fluid_properties["mass"]
        )

        total_buoyancy = buoyancy_properties["mass"] + sum(
            [coating["buoyancy"] for coating in coating_properties]
        )
        mass_water_without_internal_fluid = (
            mass_air_without_internal_fluid - total_buoyancy
        )
        ...
        mass_dict = {
            "air": {
                "without_internal_fluid": mass_air_without_internal_fluid,
                "with_internal_fluid": mass_air_with_internal_fluid,
            },
            "water": {
                "without_internal_fluid": mass_water_without_internal_fluid,
                "with_internal_fluid": mass_water_with_internal_fluid,
            },
    ...
    def get_coating_properties(self, pipe_cfg):
        ...
        for coating in coatings:
            ...
            if coating["material"] == "concrete":
                concrete_water_absorption_rate_percent = coating.get(
                    "concrete_water_absorption_rate_percent", 0
                )
                coating["buoyancy"] = coating["mass"] * (
                    1 - concrete_water_absorption_rate_percent / 100
                )
            else:
                coating["buoyancy"] = 0
    ...
    def get_internal_fluid_properties(self, pipe_cfg, pipe_properties):
        ...
        depth = (
            internal_fluid["pressure"]["top_side_elevation"]
            + internal_fluid["pressure"]["reference_water_depth"]
        )
        depth = depth * 12
