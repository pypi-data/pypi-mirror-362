...
class DNV_RP_F103:
    ...
    def DNV_RP_F103_2010(self, cfg):
        ...
        current_demand = self.get_current_demand(
            cfg, breakdown_factor, surface_area_protected
        )
        bracelet_anode_mass = self.get_required_bracelet_anode_mass(cfg, current_demand)

        final_current_requirement = self.get_final_current_requirement(
            cfg, anode_mass, current_demand
        )
        max_pipe_length_check = self.max_pipe_length_check(
            cfg,
            breakdown_factor,
        ...
        final_check = self.CP_final_check(
            cfg,
        ...
        cfg["cathodic_protection"] = {
            "final_current_requirement": final_current_requirement,
            "max_pipe_length_check": max_pipe_length_check,
    ...
    def get_breakdown_factors(self, cfg):
        ...
        breakdown_factor = {
            "regular": breakdown_factor_regular,
            "field_joint": breakdown_factor_field_joint,
        }
    ...
    def calculate_anode_mass(self, cfg):
        ...
        outer_diameter = (
            structure["dimensions"]["Nominal_OD"]
            + 2 * coating["coatings"]["thickness"]
            + 2 * anode["physical_properties"]["thickness"]
        )
        inner_diameter = (
            structure["dimensions"]["Nominal_OD"] + 2 * coating["coatings"]["thickness"]
        )
    ...
    def get_surface_area(self, cfg):
        ...
        field_joint_length = (
            2
            * structure["dimensions"]["length"]["cutback"]
            * (
                structure["dimensions"]["length"]["total"]
                / structure["dimensions"]["length"]["joint"]
            )
        )
        ...
        surface_area = {
            "surface_area_pipeline": round(area_pipeline, 2),
            "surface_area_field_joint": round(area_field_joint, 3),
        }
    ...
    def get_current_demand(self, cfg, breakdown_factor, surface_area_protected):
        ...
        mean = (
            surface_area_protected["surface_area_pipeline"]
            * breakdown_factor["regular"]["mean"]
            * structure["electrical"]["anode_mean_current_density"]
            * design["factor"]
            + surface_area_protected["surface_area_field_joint"]
            * breakdown_factor["field_joint"]["mean"]
            * structure["electrical"]["anode_mean_current_density"]
            * design["factor"]
        )
        ...
        final = (
            surface_area_protected["surface_area_pipeline"]
            * breakdown_factor["regular"]["final"]
            * structure["electrical"]["anode_mean_current_density"]
            * design["factor"]
            + surface_area_protected["surface_area_field_joint"]
            * breakdown_factor["field_joint"]["final"]
            * structure["electrical"]["anode_mean_current_density"]
            * design["factor"]
        )
    ...
    def get_required_bracelet_anode_mass(self, cfg, current_demand):
        ...
        mass = (
            current_demand["mean"]
            * design_life_in_hours
            / (anode_cfg["utilisation_factor"] * anode_cfg["current_capacity"])
        )
        number = int(math.ceil(mass / anode_net_weight))

        anode_mass_bracelet = {
            "anode_mass": round(mass, 1),
            "anode_number": round(number, 1),
        }
    ...
    def get_final_current_requirement(self, cfg, anode_mass, current_demand):
        ...
        resistivity = round(
            0.315
            * environment_cfg["seawater_resistivity"]["input"]
            / (math.sqrt(surface_area)),
            3,
        )

        anode_current = (
            structure["electrical"]["material_protective_potential"]
            - (structure["electrical"]["anode_potential"])
        ) / resistivity
        ...
        final_current_requirement = {
            "anode": {
                ...
                "current_output": round(anode_current, 3),
                "number": round(anode_number, 1),
            }
        }

        return final_current_requirement

    def max_pipe_length_check(
        self,
        cfg,
        breakdown_factor,
        final_current_requirement,
        ...
        breakdown_factor = round(
            breakdown_factor["regular"]["final"]
            + 2
            * structure_cfg["dimensions"]["length"]["cutback"]
            / structure_cfg["dimensions"]["length"]["joint"]
            * breakdown_factor["field_joint"]["final"],
            4,
        )
        ...
        spacing_joints = math.floor(
            spacing_user / structure_cfg["dimensions"]["length"]["joint"]
        )
        final_spacing = spacing_joints * structure_cfg["dimensions"]["length"]["joint"]
        final_number = math.ceil(L / final_spacing)

        max_pipe_length_check = {
            "max_pipe_length": round(pipe_length, 2),
            "anode": {
                "final_spacing": round(final_spacing, 1),
                "final_number": round(final_number, 1),
            },
        }
    ...
    def CP_final_check(
        ...
        if (
            max_pipe_length_check["anode"]["final_number"]
            * anode_cfg["physical_properties"]["net_weight"]
            > bracelet_anode_mass["anode_mass"]
        ):
        ...
        if (
            max_pipe_length_check["anode"]["final_number"]
            * final_current_requirement["anode"]["current_output"]
            > current_demand["final"]
        ):
            current_requirement = " Acceptable"
        else:
            current_requirement = " Not Acceptable"
        if (
            max_pipe_length_check["max_pipe_length"]
            > max_pipe_length_check["anode"]["final_spacing"]
        ):
        ...
        final_check = {
            "anode": {
                ...
                "individual_mass": round(individual_mass, 2),
                "required_spacing": round(required_spacing, 1),
                "number_of_required_anodes": round(number_of_anodes, 1),
            },
            "mass_requirement_check": mass_requirement,
            "current_requirement_check": current_requirement,
            "pipe_length_check": pipe_check_length,
        }
