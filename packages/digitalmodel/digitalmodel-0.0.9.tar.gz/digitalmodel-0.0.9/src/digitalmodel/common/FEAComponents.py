...
class FEAComponents:
    ...
    def save_output(self):
        ...
        save_data.WriteOrcaFlexYMLFile(
            fea_file_data_list, self.cfg["Analysis"]["fe_filename"]
        )
    ...
    def load_variations(self):
        if (
            self.cfg["Environment"].__contains__("LoadVariation")
            and self.cfg["Environment"]["LoadVariation"]["flag"]
        ):
            ...
            self.loadvariations = pd.read_csv(
                self.cfg["Environment"]["LoadVariation"]["file_name"]
            )
            fe_filename_array = []
            ...
            for row_index in range(0, len(self.loadvariations)):
                ...
                load_variation = trans.convert_numpy_types_to_native_python_types(
                    cfg={"datatype": dict, "data": load_variation}
                )
                fea_data = environment.update_with_load_variation(load_variation)
                self.fea_model.update(fea_data)
                LoadVariation_cfg.update(
                    {
                        "index": str(row_index).rjust(
                            len(str(len(self.loadvariations))), "0"
                        )
                    }
                )
    ...
    def get_fe_filename(self, LoadVariation_cfg={"flag": False, "index": "000"}):
        if not LoadVariation_cfg["flag"]:
            if (
                self.cfg["Analysis"].__contains__("drop_basename")
                and self.cfg["Analysis"]["drop_basename"]
            ):
                fe_filename = self.cfg["Analysis"]["fe_folder"] + "basename.yml"
            else:
                fe_filename = (
                    self.cfg["Analysis"]["fe_folder"]
            ...
            if (
                ...
                fe_filename = (
                    self.cfg["Analysis"]["fe_folder"]
                    + self.cfg["Analysis"]["fe_file_prefix"]
                    + LoadVariation_cfg["index"]
                    + ".yml"
                )
            else:
                fe_filename = (
