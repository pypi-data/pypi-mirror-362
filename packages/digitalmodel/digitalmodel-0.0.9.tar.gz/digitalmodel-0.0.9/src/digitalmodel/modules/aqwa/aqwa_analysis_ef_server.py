...
class AqwaEFServer:
    ...
    def ef_server_router(self, cfg: dict) -> None:
        ...
        if number_of_client_runs is None:
            number_of_client_runs = len(
                self.cfg["file_management"]["input_files"]["DAT"]
            )
        ...
        for run_idx in range(0, number_of_client_runs):
            try:
                ...
                InputFileName = self.cfg["file_management"]["input_files"]["DAT"][
                    run_idx
                ]
                au.run_aqwa_analysis_as_subprocess(
                    cfg, InputFileName, process="detached"
                )
    ...
    def UF1(self, Analysis, Mode, Stage, Time, TimeStep, Pos, Vel):
        ...
        ActualFileName = Analysis.InputFileName.split("\\")[
            -1
        ]  # We strip the path part of the filename for easy comparison
        ...
        for s in range(Analysis.NOfStruct):
            for dof in range(6):
                ...
                for dof2 in range(6):
                    AddMass[s][dof2][dof] = 1e-3 * (
                        dof2 * 6 + dof
                    )  # AddMass goes [0.000, 0.001, 0.002, ..., 0.036]
    ...
    def UF2(self, Analysis, Mode, Stage, Time, TimeStep, Pos, Vel):
        ...
        if Analysis.R_Control[5] == 0.0:
            ...
            coef = Analysis.R_Control[
                5
            ]  # Should be set to 30000 in the Aqwa input file.
        ...
        CurPos = Analysis.GetNodeCurrentPosition(
            Struct=0, DefAxesX=DefPos[0], DefAxesY=DefPos[1], DefAxesZ=DefPos[2]
        )

        ExtraForce = Analysis.ApplyForceOnStructureAtPoint(
            Struct=0,
            FX=VertForce[0],
            FY=VertForce[1],
            FZ=VertForce[2],
    ...
    def UFFender(self, Analysis, Mode, Stage, Time, TimeStep, Pos, Vel):
        ...
        for def_pos_idx in range(0, len(def_pos_cfg)):
            ...
            CurVel = Analysis.GetNodeCurrentVelocity(
                Struct=Struct,
                DefAxesX=CurPos[0],
                DefAxesY=CurPos[1],
                DefAxesZ=CurPos[2],
            ...
            if delta_L > 0:
                ...
                if ramp_cfg["flag"] and Time < ramp_cfg["time"]:
                    force_dict = {
                        "stiffener_force": stiffener_force,
            ...
            self.result_df_array[def_pos_idx].loc[
                len(self.result_df_array[def_pos_idx])
            ] = result_array
    ...
    def wsp_dampener_by_def_pos(self, Analysis, Time, def_pos):
        ...
        CurPos = Analysis.GetNodeCurrentPosition(
            Struct=Struct, DefAxesX=DefPos[0], DefAxesY=DefPos[1], DefAxesZ=DefPos[2]
        )

        # TODO Should we give DefPos or CurPos to GetNodeCurrentVelocity?
        CurNodeVel = Analysis.GetNodeCurrentVelocity(
            Struct=Struct, DefAxesX=CurPos[0], DefAxesY=CurPos[1], DefAxesZ=CurPos[2]
        )

        dl = [
            CurPos[0] - DefPos[0] - dl_correction[0],
            CurPos[1] - DefPos[1] - dl_correction[1],
            CurPos[2] - DefPos[2] - dl_correction[2],
        ]
    ...
    def get_numerical_added_mass(self, Analysis, AddMass, Time):
        ...
        if "numerical_added_mass" in self.cfg["analysis_settings"]["ef_input"]:
            numerical_added_mass_flag = self.cfg["analysis_settings"]["ef_input"][
                "numerical_added_mass"
            ]["flag"]
            added_mass = self.cfg["analysis_settings"]["ef_input"][
                "numerical_added_mass"
            ]["added_mass"]
            added_mass_factor = self.cfg["analysis_settings"]["ef_input"][
                "numerical_added_mass"
            ]["factor"]
            time = self.cfg["analysis_settings"]["ef_input"]["numerical_added_mass"][
                "time"
            ]
    ...
    def get_wsp_dampener_force(self, Time, dof_pos_delta, dof_vel, dir="X"):
        stiffness = {
            "dynamic": {
                "X": {"dl": [0.05, 0.15, 0.70, 10], "k": [2e4, 1e5, 7.03e6, 2.8e7]},
                "Y": {"dl": [0.05, 0.15, 0.70, 10], "k": [2e4, 2e5, 14.06e6, 5.6e7]},
            },
            "static": {
                "X": {
                    "dl": [0.05, 0.15, 0.70, 10],
                    "k": [7.03e4, 7.03e5, 7.03e5, 2.8e7],
                },
                "Y": {
                    "dl": [0.05, 0.15, 0.70, 10],
                    "k": [14.06e4, 14.06e5, 14.06e5, 5.6e7],
                },
            },
        }
        stiffness_anal_type = {
        ...
        damping = {
            "dynamic": {
                "X": {"v": [0.005, 0.01, 100], "c": [3.75e06, 3.75e06, 3.75e06]},
                "Y": {"v": [0.005, 0.01, 100], "c": [7.51e06, 7.51e06, 7.51e06]},
            },
            "static": {
                "X": {"v": [0.005, 0.1, 100], "c": [3.75e03, 3.75e03, 3.75e06]},
                "Y": {"v": [0.005, 0.1, 100], "c": [7.51e03, 7.51e03, 7.51e06]},
            },
        }
        ...
        if dof_pos_delta_abs < stiffness_anal_type[dir]["dl"][0]:
            ...
            stiffness_force = (
                stiffness_anal_type[dir]["k"][0] * stiffness_anal_type[dir]["dl"][0]
            )
            stiffness_force = stiffness_force + stiffness_anal_type[dir]["k"][1] * (
                dof_pos_delta_abs - stiffness_anal_type[dir]["dl"][0]
            )
            ...
            stiffness_force = stiffness_force + stiffness_anal_type[dir]["k"][1] * (
                stiffness_anal_type[dir]["dl"][1] - stiffness_anal_type[dir]["dl"][0]
            )
            stiffness_force = stiffness_force + stiffness_anal_type[dir]["k"][2] * (
                dof_pos_delta_abs - stiffness_anal_type[dir]["dl"][1]
            )
        else:
            stiffness_force = (
                stiffness_anal_type[dir]["k"][0] * stiffness_anal_type[dir]["dl"][0]
            ...
            stiffness_force = stiffness_force + stiffness_anal_type[dir]["k"][2] * (
                stiffness_anal_type[dir]["dl"][2] - stiffness_anal_type[dir]["dl"][1]
            )
            stiffness_force = stiffness_force + stiffness_anal_type[dir]["k"][3] * (
                dof_pos_delta_abs - stiffness_anal_type[dir]["dl"][2]
            )
    ...
    def print_heartbeat(self, Time, complete_flag=False):
        if complete_flag:
            logging.info(
                f"Analysis Complete time: {Fore.GREEN}{round(Time, 3)} s {Style.RESET_ALL}"
            )
        elif Time % 50 == 0:
