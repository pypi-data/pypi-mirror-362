...
class VIVAnalysisComponents:
    ...
    def modal_analysis(self):
        ...
        for fileIndex in range(0, len(self.cfg["modal_sets"])):
            ...
            customdata = {
                "FileName": self.cfg["modal_sets"][fileIndex]["io"],
                "skiprows": 1,
            ...
            df = pd.read_table(
                customdata["FileName"],
                sep=customdata["sep"],
                skiprows=customdata["skiprows"],
                skipfooter=customdata["skipfooter"],
            ...
            df = pd.read_table(
                customdata["FileName"],
                sep=customdata["sep"],
                skiprows=customdata["skiprows"] + len(ModesSummaryDF),
                engine=customdata["engine"],
                header=customdata["header"],
            )
            df.rename(
            ...
            if self.cfg["modal_sets"][fileIndex].__contains__("x_by_L_range"):
                start_node = math.ceil(
                    self.cfg["modal_sets"][fileIndex]["x_by_L_range"][0]
                    * (NoOfNodes - 1)
                )
                end_node = math.floor(
                    self.cfg["modal_sets"][fileIndex]["x_by_L_range"][1]
                    * (NoOfNodes - 1)
                )
                df = df[
                    (df["Node Number"] > start_node) & (df["Node Number"] < end_node)
                ]
            ...
            for ModeNumber in range(1, NoOfModes + 1):
                ModeCurvature.append(
                    df[df["Mode Number"] == ModeNumber]["Mode Curvature"].max()
                )
                ModeShape.append(
                    df[df["Mode Number"] == ModeNumber]["Mode Shape"].max()
                )
                ModeSlope.append(
                    df[df["Mode Number"] == ModeNumber]["Mode Slope"].max()
                )
                ModeUnknown.append(
                    df[df["Mode Number"] == ModeNumber]["Mode Unknown"].max()
                )
    ...
    def save_modal_visualizations(self):
        ...
        for plt_index in range(0, len(self.cfg["modal_plots"])):
            ...
            plt_settings.update(
                {
                    "array_label": [
                        [modal_set["label"]] for modal_set in self.cfg.modal_sets
                    ]
                }
            )
    ...
    def save_mode_shapes(self):
        ...
        for plt_index in range(0, len(self.cfg["mode_shape_plots"]["each_modal_set"])):
            ...
            for file_index in range(0, len(self.cfg["modal_sets"])):
                ...
                for mode_number in plt_settings["label"]:
                    temp_df = self.ModalSolutiondfArray[file_index][
                        self.ModalSolutiondfArray[file_index]["Mode Number"]
                        == mode_number
                    ]
                    mode_plt_settings = extension_plt_settings.copy()
                    mode_plt_settings.update({"label": ["mode " + str(mode_number)]})
                    mode_plt_settings.update(
                        {
                            "title": extension_plt_settings["title"]
                            + " "
                            + self.cfg["modal_sets"][file_index]["label"]
                        }
                    )
        ...
        for plt_index in range(
            0, len(self.cfg["mode_shape_plots"]["between_modal_sets"])
        ):
    ...
    def plot_current_non_exceedance(self, reference_depth=0):
        ...
        for plot_index in range(
            0, len(self.cfg.current_data["plot"]["non_exceedance"])
        ):
            ...
            for current_data_index in range(0, len(self.cfg.current_data["from_xlsx"])):
                if (
                    not self.current_data[current_data_index]
                    .probability.isnull()
                    .values.any()
                ):
                    from common.visualizations import Visualization
                    ...
                    surface_current_speeds = self.current_data[current_data_index][
                        self.current_data[current_data_index].depth == 0
                    ].copy()
                    surface_current_speeds.sort_values(
                        by=["current_speed"], ascending=True, inplace=True
                    )
                    # QC currents at exceedabce depth
                    current_profile_id_array = (
                        self.current_data[current_data_index]
                        .current_profile_id.unique()
                        .tolist()
                    )
                    if len(current_profile_id_array) > len(surface_current_speeds):
                        print(
                            "All current profiles may not be captured for this exceedance depth"
                        )
                    surface_current_speeds["exceedance"] = 0
                    surface_current_speeds["exceedance"].iloc[
                        0
                    ] = surface_current_speeds["probability"].iloc[0]
                    ...
                    surface_current_speeds.to_csv(
                        self.cfg["Analysis"]["result_folder"]
                        + self.cfg["Analysis"]["file_name"]
                        + "_"
                        + "exceedance"
                        + "_"
                        + str(current_data_index)
                        + ".csv"
                    )
                    plt_settings["label"] = [
                        self.cfg.current_data["from_xlsx"][current_data_index]["label"]
                    ]
                    viz.from_df_columns(surface_current_speeds, plt_settings)
                else:
                    print(
                        "No exceedance plot due to missing probabilities for some currrent profiles"
                    )

            viz.add_title_and_axis_labels()
            viz.add_legend()
            print(
                "Saving {0} plot for current data {1}...".format(
                    plt_settings["file_suffix"], current_data_index
                )
            )
            viz.save_and_close()
        ...
        for plot_index in range(0, len(self.cfg.current_data["plot"]["exceedance"])):
            ...
            for current_data_index in range(0, len(self.cfg.current_data["from_xlsx"])):
                if (
                    ...
                    surface_current_speeds.sort_values(
                        by=["current_speed"], ascending=False, inplace=True
                    )
                    # QC currents at exceedabce depth
                    current_profile_id_array = (
                        self.current_data[current_data_index]
                    ...
                    surface_current_speeds["non_exceedance"].iloc[
                        0
                    ] = surface_current_speeds["probability"].iloc[0]
                    for row_index in range(1, len(surface_current_speeds)):
                    ...
                    print(
                        "No Non exceedance plot due to missing probabilities for some currrent profiles"
                    )
    ...
    def plot_current_profiles(self):
        ...
        for current_data_index in range(0, len(self.cfg.current_data["from_xlsx"])):
            ...
            for plot_index in range(
                0, len(self.cfg.current_data["plot"]["individual"])
            ):
                plt_settings = self.cfg.current_data["plot"]["individual"][
                    plot_index
                ].copy()
                plt_settings["title"] = (
                    plt_settings["title"]
                ...
                for current_index in range(0, len(current_profile_id_array)):
                    temp_df = self.current_data[current_data_index][
                        self.current_data[current_data_index].current_profile_id
                        == current_profile_id_array[current_index]
                    ]
    ...
    def plot_wave_data(self):
        ...
        for wave_data_index in range(0, len(self.wave_data)):
            for plot_index in range(0, len(self.cfg.wave_data["plot"]["individual"])):
                plt_settings = self.cfg.wave_data["plot"]["individual"][
                    plot_index
                ].copy()
                plt_settings["title"] = self.cfg.wave_data["from_xlsx"][
                    wave_data_index
                ]["label"]
                plt_settings.update(
                    {
                ...
                viz.add_title_and_axis_labels()
                print(
                    "Saving {0} plot for wave data {1}...".format(
                        plt_settings["file_suffix"], wave_data_index
                    )
                )
                viz.save_and_close()
    ...
    def plot_wave_exceedance(self):
        ...
        for plot_index in range(0, len(self.cfg.wave_data["plot"]["exceedance"])):
            ...
            for wave_data_index in range(0, len(self.cfg.wave_data["from_xlsx"])):
                if (
                    not self.wave_data[wave_data_index]
                    .probability.isnull()
                    .values.any()
                ):
                    ...
                    df["non_exceedance"] = 0
                    df["non_exceedance"].iloc[0] = df["probability"].iloc[0]
                    ...
                    plt_settings["label"] = [
                        self.cfg.wave_data["from_xlsx"][wave_data_index]["label"]
                    ]
