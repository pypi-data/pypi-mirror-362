...
class OrcModalAnalysis:
    def __init__(self):
        self.modes_df_columns = ["modeIndex", "name", "node", "dof", "shapeWrtGlobal"]
        ...
        self.all_file_summary_df_columns = ["Label"] + self.modes_summary_df_columns
    ...
    def run_modal_analysis(self, cfg=None):
        ...
        if cfg["default"]["Analysis"]["Analyze"]["modal"]:
            ...
            for file in files:
                ...
                self.all_file_summary.update({file["Label"]: all_modes_summary_df})
                self.all_file_selected_modes.update({file["Label"]: selected_modes})
    ...
    def summarize_all_files(self):
        ...
        for dof in dof_analysis:
            all_file_summary_df = pd.DataFrame(columns=self.all_file_summary_df_columns)
            for file in files:
                selected_modes_dof_df = self.all_file_selected_modes[file["Label"]][dof]
    ...
    def run_by_file(self, file=None):
        ...
        spec = OrcFxAPI.ModalAnalysisSpecification(
            calculateShapes=True, lastMode=lastMode
        )

        modes = OrcFxAPI.Modes(model, spec)

        all_modes_summary_df, selected_modes = self.save_mode_shapes(modes, file)
    ...
    def explore_mode_shapes(self, modes):
        ...
        for modeIndex in range(modes.modeCount):
            ...
            for dofIndex in range(modes.dofCount):
                print(
                    f"{modes.owner[dofIndex].name} node {modes.nodeNumber[dofIndex]:2} "
    ...
    def save_mode_shapes(self, modes, file):
        ...
        all_modes_summary_df = pd.DataFrame(columns=self.modes_summary_df_columns)
        for modeIndex in range(modes.modeCount):
    ...
    def get_mode_summary(self, mode_df) -> pd.DataFrame:
        ...
        ObjectNames = self.cfg["default"]["Analysis"]["Analyze"]["modal"]["ObjectName"]
        ...
        for ObjectName in ObjectNames:
            ...
            (
                ...
                abs_max_dof,
                modes_selected,
            ) = self.get_mode_dof_responses(ObjectName_mode_df)
    ...
    def get_mode_dof_responses(self, ObjectName_mode_df):
        ...
        for dof in dofs:
            ...
            if abs(mode_dof_max) > abs(mode_dof_min):
                shapeWrtGlobal = ObjectName_mode_df.shapeWrtGlobal.loc[dof_idxmax]
                node = ObjectName_mode_df.node.loc[dof_idxmin]

            else:
                shapeWrtGlobal = ObjectName_mode_df.shapeWrtGlobal.loc[dof_idxmin]
                node = ObjectName_mode_df.node.loc[dof_idxmin]
