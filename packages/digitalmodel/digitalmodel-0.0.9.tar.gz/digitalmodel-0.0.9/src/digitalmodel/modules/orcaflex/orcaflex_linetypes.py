...
class OrcaflexLineTypes:
    ...
    def get_umbilical_properties(self, line_type, cfg):
        ...
        umbilical_properties_dict = {
            "LineTypes": {
                "New": line_type["name"],
                line_type["name"]: umbilical_properties,
            }
        }
    ...
    def assign_key_umbilical_properties(self, line_type):
        ...
        umbilical_properties = umbilical_template["LineTypes"][
            umbilical_template["LineTypes"]["New"]
        ].copy()
        ID = line_type["ID"]
        buoyancy_force = line_type["mass"]["air"] - line_type["mass"]["water"]
        OD = round(
            ((buoyancy_force * 4) / (math.pi * line_type["seawater_density"]) + ID**2)
            ** 0.5,
            4,
        )
        ...
        umbilical_properties["EI"][0] = round(
            line_type["stiffness"]["bending"] / 1000, 3
        )
    ...
    def save_model(self, yaml_data, line_type, cfg):
        ...
        save_data.saveDataYaml(
            yaml_data, output_dir + "\\" + filename, default_flow_style=False
        )
