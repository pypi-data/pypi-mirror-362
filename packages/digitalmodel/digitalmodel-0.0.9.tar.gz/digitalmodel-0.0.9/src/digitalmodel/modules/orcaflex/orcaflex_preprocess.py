...
class OrcaflexPreProcess:
    ...
    def router(self, cfg):
        if "preprocess" in cfg["orcaflex"]:
            check_yml_flag = (
                cfg["orcaflex"]["preprocess"].get("check_yml", {}).get("flag", False)
            )
            if check_yml_flag:
                self.check_yml_file(cfg)
            load_vessel_flag = (
                cfg["orcaflex"]["preprocess"].get("load_vessel", {}).get("flag", False)
            )
            if load_vessel_flag:
    ...
    def check_yml_file(self, cfg):
        ...
        cfg["file_management"]["input_files"].update(
            {"yml_orcaflex_check": check_yml_list}
        )
    ...
    def check_1_yml_file(self, yml_file):
        try:
            ...
            raise RuntimeError(
                "OrcaFlex license not available. Run on different computer"
            )
