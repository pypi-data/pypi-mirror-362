...
class Mooring:
    ...
    def pretension_analysis(self, cfg, group):
        ...
        for fileIndex in range(0, len(sim_files)):
            ...
            force_balance_analysis_dict = self.mooring_pre_tension_analysis(
                cfg, group, file_meta_data, var_data_dict
            )
            fender_force_analysis_dict = self.fender_force_analysis(
                cfg, group, file_meta_data, var_data_dict
            )
    ...
    def fender_force_analysis(self, cfg, group, file_meta_data, var_data_dict):
        ...
        delta_compression = self.evaluate_compression_next_iteration(
            group, fender_force_df
        )

        vessel_df = var_data_dict["StaticResult"]["Vessel"][
            ["ObjectName", "X", "Y", "Z"]
        ]
        fender_contact_vessel = group["target_fender_force"]["contact_vessel"]
        vessel_df_filetered = vessel_df[
            vessel_df["ObjectName"] == fender_contact_vessel
        ].copy()
        vessel_df_filetered["Y_next_iteration"] = (
            vessel_df_filetered["Y"] - delta_compression
        )
        ...
        output_dict = {
            ...
            "vessel_df_filetered": vessel_df_filetered,
        }

        fender_criteria_pass_flag = self.get_fender_criteria(cfg, group, output_dict)
    ...
    def evaluate_compression_current(self, cfg, group, var_data_dict):
        ...
        columns = [
            ...
            "Displacement",
            "x",
            "In-frame connection GY force",
            "In-frame connection Ly force",
        ]
        ...
        for index, row in fender_force_df.iterrows():
            ...
            fender_force = current_var_df_filtered[
                "In-frame connection GY force"
            ].values[0]
    ...
    def evaluate_compression_next_iteration(self, group, fender_force_df):
        ...
        fender_force_df["new_fender_force"] = (
            total_target_force / total_force * fender_force_df["current_fender_force"]
        )
        for index, row in fender_force_df.iterrows():
            fender_properties = row["fender_properties"]
            fender_properties_df = pd.DataFrame(
                fender_properties, columns=["compression", "force"]
            )
            new_fender_force = row["new_fender_force"]
            new_compression = np.interp(
                -new_fender_force,
                fender_properties_df["force"],
                fender_properties_df["compression"],
            )
            fender_force_df.at[index, "new_compression"] = new_compression
            fender_force_df.at[index, "new_fender_force"] = new_fender_force
            fender_force_df.at[index, "delta_compression"] = (
                new_compression - row["compression"]
            )
        delta_compression = (
            fender_force_df["compression"].iloc[3]
            - fender_force_df["new_compression"].iloc[3]
        )
    ...
    def get_tension_criteria(self, cfg, group, target_pre_tension_df):
        ...
        for index, row in target_pre_tension_df.iterrows():
            ...
            target_tension = row["target_tension"]
            tension_difference = (
                abs(current_tension - target_tension) / target_tension * 100
            )
    ...
    def get_fender_criteria(self, cfg, group, output_dict):
        ...
        fender_force_difference = (
            abs(abs(total_fender_force_current) - total_fender_force_target)
            / total_fender_force_target
            * 100
        )
    ...
    def get_iteration_flag(
        self, cfg, group, force_balance_analysis_dict, current_iteration
    ):
    ...
    def prepare_includefile_for_all_lines(
        self, cfg, file_meta_data, target_pre_tension_df
    ):
        includefile_dict = {}
    ...
    def prepare_includefile_for_fender_compression(
        self, cfg, file_meta_data, output_dict
    ):
    ...
    def get_tension(self, cfg, group):
        ...
        is_file_valid, tension_filename = is_file_valid_func(
            tension_filename, analysis_root_folder
        )
    ...
    def get_length(self, cfg, group):
        ...
        analysis_root_folder = cfg["Analysis"]["analysis_root_folder"]
        is_file_valid, length_filename = is_file_valid_func(
            length_filename, analysis_root_folder
        )
    ...
    def get_target_pretension(self, cfg, group):
        ...
        pre_tension_df["line_length"] = [
            eval(item) for item in list(pre_tension_df["line_length"])
        ]
        pre_tension_df["line_EA"] = [
            eval(item) for item in list(pre_tension_df["line_EA"])
        ]
    ...
    def get_target_fender_force(self, cfg, group):
        ...
        analysis_root_folder = cfg["Analysis"]["analysis_root_folder"]
        is_file_valid, fender_force_filename = is_file_valid_func(
            fender_force_filename, analysis_root_folder
        )
