...
class MaterialProperties:
        ...
        for load_condition_index in range(0, len(self.cfg["Design"])):
            for code_index in range(
                ...
                update_deep_dictionary(
                    self.cfg["Design"][load_condition_index],
                    {
                        "Material": {
                            "temperature_derating": {
                                pipe_flag: {
                                    specification_code: result["temperature_derating"]
                                }
                            }
                        }
                    },
                ...
                logging.info(
                    "Material: Temperature derate check performed for {0}, per code {1}".format(
    ...
    def temperatureDerating(self, data):
        if data["Code"] == "Other":
            if data["temperature"] > 50 and data["temperature"] <= 100:
                data["S"] = (
                    data["S"]
                    - (25 - 0) / (100 - 50) * (data["temperature"] - 50) * 145.038
                )
                data["U"] = (
                    data["U"]
                    - (25 - 0) / (100 - 50) * (data["temperature"] - 50) * 145.038
                ...
                data["S"] = (
                    data["S"]
                    - (25 + (68 - 25) / (200 - 100) * (data["temperature"] - 100))
                    * 145.038
                )
                data["U"] = (
                    data["U"]
    ...
    def evaluate_mass_and_weight_properties(self, pipe_flag):
        ...
        for load_condition_index in range(0, len(self.cfg["Design"])):
            ...
            mass["Coupling"] = (
                mass["pipe"]
                * self.cfg[pipe_flag]["Manufacturing"]["Coupling Mass Ratio"]
            )
            mass["internal_fluid"] = (
                self.cfg[pipe_flag]["section_properties"]["pipe"]["Ai"]
                * self.cfg["Design"][load_condition_index]["InternalFluid"][pipe_flag]
            ) * 12
            mass["dry"] = mass["pipe"] + mass["Coupling"] + mass["internal_fluid"]
            mass["buoyancy"] = (
                self.cfg[pipe_flag]["section_properties"]["pipe"]["Ao"]
                * self.cfg["Design"][load_condition_index]["ExternalFluid"][pipe_flag]
            ) * 12
            mass["wet"] = mass["dry"] - mass["buoyancy"]

            update_deep_dictionary(
                self.cfg["Design"][load_condition_index], {"mass": mass}
            )
