...
def customUpdate(cfg):
    ...
    if cfg["geometry"]["NominalOD"] != None:
        cfg["plotSettings"]["plotFileName"] = cfg["plotSettings"][
            "plotFileName"
        ].format(
            "OD",
            str(cfg["geometry"]["NominalOD"]),
            str(cfg["geometry"]["CorrosionAllowance"]),
        )
        cfg["plotSettings"]["plotFileName"] = cfg["plotSettings"][
            "plotFileName"
        ].replace(".", "p")
        cfg["plotSettings"]["pltTitle"] = cfg["plotSettings"]["pltTitle"].format(
            str(cfg["geometry"]["NominalOD"]),
            str(cfg["geometry"]["CorrosionAllowance"]),
            "OD",
        )
