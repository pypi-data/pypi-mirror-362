...
class PipeSizing:
    ...
    def pipe_properties(self, pipe_flag):
        if self.cfg[pipe_flag]["Geometry"]["Nominal_ID"] == None:
            self.cfg[pipe_flag]["Geometry"]["Nominal_ID"] = (
    ...
    def pipe_section_properties(self, pipe_flag):
        ...
        self.cfg[pipe_flag]["section_properties"] = {
            "pipe": {
    ...
    def get_fea_properties(self, pipe_flag):
        ...
        try:
            ...
            self.cfg["Material"][material]["G"] = self.cfg["Material"][material][
                "E"
            ] / (2 * (1 + self.cfg["Material"][material]["Poissionsratio"]))
        MassPerUnitLength = (
            self.cfg[pipe_flag]["section_properties"]["pipe"]["A"]
            * self.cfg["Material"][material]["Rho"]
        )
        EI = (
            self.cfg[pipe_flag]["section_properties"]["pipe"]["I"]
            * self.cfg["Material"][material]["E"]
        )
        EA = (
            self.cfg[pipe_flag]["section_properties"]["pipe"]["A"]
            * self.cfg["Material"][material]["E"]
        ...
        GJ = (
            self.cfg[pipe_flag]["section_properties"]["pipe"]["J"]
            * self.cfg["Material"][material]["G"]
        )

        self.cfg[pipe_flag]["section_properties"]["pipe"].update(
            {
                ...
                "SMYS": self.cfg["Material"][material]["Grades"][material_grade][
                    "SMYS"
                ],
                "SMUS": self.cfg["Material"][material]["Grades"][material_grade][
                    "SMUS"
                ],
    ...
    def system_properties(self):
        if self.cfg["Inner_Pipe"] != None:
            MassPerUnitLength = (
                self.cfg["Outer_Pipe"]["section_properties"]["pipe"][
                    "MassPerUnitLength"
                ]
                + self.cfg["Inner_Pipe"]["section_properties"]["pipe"][
                    "MassPerUnitLength"
                ]
            )
            EI = (
                self.cfg["Outer_Pipe"]["section_properties"]["pipe"]["EI"]
                + self.cfg["Inner_Pipe"]["section_properties"]["pipe"]["EI"]
            )
            EA = (
                self.cfg["Outer_Pipe"]["section_properties"]["pipe"]["EA"]
                + self.cfg["Inner_Pipe"]["section_properties"]["pipe"]["EA"]
            )
            GJ = (
                self.cfg["Outer_Pipe"]["section_properties"]["pipe"]["GJ"]
            ...
            self.cfg["equivalent_pipe"]["section_properties"] = self.cfg["Outer_Pipe"][
                "section_properties"
            ]
    ...
    def insulationSectionProperties(self, cfg):
        OD = (
            ...
            + 2 * cfg["geometry"]["ExternalCoating"]["Thickness"]
        )
    ...
    def buoyancySectionProperties(self, cfg):
        OD = (
            ...
            + 2 * cfg["LazyWaveCatenaryDefinition"]["UniformBuoyancy"]["Thickness"]
        )
    ...
    def equivalentPipe(self, cfg, FluidDensity, Buoyancy=False):
        if cfg["geometry"]["Strakes"]["BaseThickness"] == None and Buoyancy == False:
            massPerUnitLength = (
                cfg["SteelSection"]["A"]
                * cfg["Material"]["Steel"]["Rho"]
                * (0.0254**2)
                + cfg["InsulationSection"]["A"]
                * cfg["Material"]["ExternalCoating"]["Rho"]
                * (0.0254**2)
            ...
            weightPerUnitLength = (
                ...
                * cfg["Material"]["SeaWater"]["Rho"]
                * (0.0254**2)
            ) * cfg["default"]["Constants"]["g"] + cfg["Material"]["Strakes"][
                "WeightPerUnitLength"
            ]
        elif Buoyancy == True:
            massPerUnitLength = (
