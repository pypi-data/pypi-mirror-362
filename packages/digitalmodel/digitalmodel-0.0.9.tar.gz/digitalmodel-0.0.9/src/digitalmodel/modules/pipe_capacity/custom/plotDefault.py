...
def plotDefault(cfg, dataDF):
    ...
    plt.text(
        1.2,
        cfg["design"]["pressure"],
        "Target Design Pressure of {0} psi".format(cfg["design"]["pressure"]),
        color="r",
    )
    plt.axvline(x=cfg["design"]["wallThickness"], color="m", dashes=[4, 2])
    plt.text(
        cfg["design"]["wallThickness"],
        10000,
        "Thickness Fabrication Limit : {0} inch".format(cfg["design"]["wallThickness"]),
        color="m",
        rotation=90,
    )
    plt.grid()
    plt.savefig("results\\ASMEB31\\" + cfg["plotSettings"]["plotFileName"], dpi=800)
