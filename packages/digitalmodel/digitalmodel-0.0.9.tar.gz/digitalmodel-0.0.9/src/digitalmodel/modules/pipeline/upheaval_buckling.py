...
class UpheavalBuckling:
    ...
    def get_friction_force(self, cfg):
        ...
        friction = {
            "axial": friction_force,
            "lateral": friction_force,
            "vertical": friction_force,
        }
    ...
    def get_upheaval_buckling(self, cfg):
        ...
        uplift_resistance_model = self.get_uplift_resistance_model(
            cfg, df, force_in_pipeline_section
        )
        ...
        level_1_result = self.get_level_1_analysis(
            cfg,
            df,
            uplift_resistance_model,
            trench_and_rock_burial_model,
    ...
    def get_buckle_result(
        ...
        array_length = 101
        Lb_array = [
            item / (array_length - 1) * L_end for item in list(range(0, array_length))
        ]
    ...
    def get_soil_pipe_friction_model(self, cfg):
        ...
        friction_angle_of_burial_material = soil_cfg["trench"][
            "friction_angle_of_burial_material"
        ]
        ...
        friction_coefficient = [
            ...
            * abs(item)
            / item
            * friction_coefficient_burial
            * (1 - math.e ** (-25 * abs(item) / u_m))
            if item > 0
            else 0
            for item in u
        ]
    ...
    def get_uplift_resistance_model(self, cfg, df, force_in_pipeline_section):
        ...
        uplift_resistance_model = {
            "vertical_displacement_at_max_uplift_resistance": vertical_displacement_at_max_uplift_resistance
        }
    ...
    def get_trench_and_rock_burial_model(self, cfg, df):
        soil_cfg = cfg["pipeline"]["soil"]
        pipe_properties = cfg["pipeline"]["pipe_properties"]
        mass_in_water_with_internal_fluid = cfg["pipeline"]["system_properties"][
            "mass"
        ]["water"]["with_internal_fluid"]
        friction_angle_of_burial_material = soil_cfg["trench"][
            "friction_angle_of_burial_material"
        ]
        trench_angle = soil_cfg["trench"]["trench_angle"]
        friction_coefficient_burial = math.tan(
            math.radians(friction_angle_of_burial_material)
        )
        H = soil_cfg["trench"]["burial_depth_to_pipe_center_line"]
        ...
        term_3 = (
            density_of_burial_material * H**2 * math.tan(friction_coefficient_burial)
        )
        term_4 = (
            density_of_burial_material
            * H**2
            * math.tan(friction_coefficient_burial)
            * math.cos(friction_coefficient_burial)
        )
        ...
        trench_and_rock_burial_model = {
            "burial_rock_weight": {
                "burial_pipe": burial_rock_weight,
                "pipe_seabed": weight_pipe_seabed,
            }
        }
    ...
    def get_force_in_pipe_section(self, cfg, df):
        ...
        force_in_pipeline_section = df_section[
            "longitudinal_force_fully_restrained"
        ].mean()
