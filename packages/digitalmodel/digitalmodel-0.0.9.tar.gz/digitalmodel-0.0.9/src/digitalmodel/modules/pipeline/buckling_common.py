...
class CommonBucklingCaculations:
    ...
    def get_internal_pressure(self, cfg, df):
        ...
        fluid_column = [
            (pressure_cfg["top_side_elevation"] + item) * 12
            for item in water_depth_array
        ]
        pressure_internal = [
            reference_pressure + density * item for item in fluid_column
        ]
    ...
    def get_circumferential_stress(self, cfg, df):
        ...
        circumferential_stress_array = [
            2 * (item[0] - item[1]) * Ai / A for item in zipped_items
        ]
    ...
    def get_longitudinal_stress_fully_restrained(self, cfg, df):
        ...
        longitudinal_stress_fully_restrained = [
            item[0] * Poissonsratio - E * ThermalExpansionCoefficient * item[1]
    ...
    def get_longitudinal_force_fully_restrained(self, cfg, df):
        ...
        longitudinal_stress_fully_restrained = df[
            "longitudinal_stress_fully_restrained"
        ]
        longitudinal_force_fully_restrained = [
            item * A for item in longitudinal_stress_fully_restrained
        ]
