class Buoy:
    ...
    def get_cylinder(self, cylinder_cfg):
        ...
        cylinder_cfg["CylinderOuterDiameter"] = (
            cylinder_cfg["CylinderOuterDiameter"] + 2 * MarineGrowthThickness
        ).__round__(3)
    ...
    def get_inertia_MomentsOfInertia(self, cfg):
        Mass = [
            cylinder_item["Mass"] for cylinder_item in cfg["StackedBuoy"]["Cylinders"]
        ]
        CylinderMassDiameter = [
            cylinder_item["CylinderMassDiameter"]
            for cylinder_item in cfg["StackedBuoy"]["Cylinders"]
        ]
        CylinderLength = [
            cylinder_item["CylinderLength"]
            for cylinder_item in cfg["StackedBuoy"]["Cylinders"]
        ]

        Inertia_Z = sum(
            [
                mass * (cylinderMassDiameter**2)
                for mass, cylinderMassDiameter in zip(Mass, CylinderMassDiameter)
            ]
        )
        ...
        Inertia_X = (
            Inertia_Z / 2
            + sum(Mass)
            * (abs(CentreOfMassEquivalent[2]) - sum(CylinderLength) / 2) ** 2
        )
        Inertia_Y = Inertia_X
        return [
            float(Inertia_X).__round__(3),
            float(Inertia_Y).__round__(3),
            float(Inertia_Z).__round__(3),
        ]
    ...
    def get_inertia_CentreOfMass(self, cfg):
        ...
        CentreOfMass = [
            cylinder_item["CentreOfMass"]
            for cylinder_item in cfg["StackedBuoy"]["Cylinders"]
        ]

        if len(CentreOfMass) > 1:
            CentreOfMassEquivalent_Array = [
                ([centreOfMass_item * mass for centreOfMass_item in centreOfMass])
            ...
            for item in range(1, len(CentreOfMassEquivalent_Array)):
                CentreOfMassEquivalent = np.add(
                    CentreOfMassEquivalent, CentreOfMassEquivalent_Array[item]
                )
            CentreOfMassEquivalent = [
                item / sum(Mass) for item in CentreOfMassEquivalent
            ]
        else:
            CentreOfMassEquivalent = CentreOfMass[0]

        CentreOfMassEquivalent = [
            float(item).__round__(3) for item in CentreOfMassEquivalent
        ]
