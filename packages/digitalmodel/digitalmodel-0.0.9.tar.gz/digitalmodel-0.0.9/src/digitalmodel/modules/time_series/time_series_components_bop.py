...
class TimeSeriesComponents:
        ...
        if self.cfg.default["data_source"] == "db":
            ...
            for env_index in range(0, len(self.environments)):
                ...
                try:
                    ...
                    print(
                        "No connection to environment {0}, error {1}".format(
                            self.env, e
                        )
                    )
                try:
                    ...
                    print(
                        "Error getting input data in environment {0}, error {1}".format(
                            self.env, e
                        )
                    )
        else:
    ...
    def perform_db_analysis(self):
        try:
            ...
            print(
                "Error Performing cfg analysis in environment {0}, error {1}".format(
                    self.env, e
                )
            )
    ...
    def write_db_analysis_results(self, db_analysis_result):
        ...
        if db_analysis_result.__contains__(
            "column_df_array_tables"
        ) and db_analysis_result.__contains__("table_column_df_array"):
            file_name = (
                self.cfg["Analysis"]["result_folder"]
                + self.cfg["Analysis"]["file_name"]
                + "_db_columns.xlsx"
            ...
            temp_cfg_data = {
                "SheetNames": column_df_array_tables,
                "FileName": file_name,
            }
            save_data.DataFrameArray_To_xlsx_openpyxl(
                table_column_df_array, temp_cfg_data
            )
        ...
        if db_analysis_result.__contains__("extreme_events_df_array"):
            ...
            for df_index in range(0, len(extreme_events_df_array)):
                ...
                file_name = (
                    self.cfg["Analysis"]["result_folder"]
                    + self.cfg["Analysis"]["file_name"]
                    + "_extreme_events_{}.csv".format(df_index)
                )
                # TODO save to db
                df.to_csv(file_name)
            ...
            if len(signal) > 1.5 * time_window:
                ...
                for window_index in range(0, number_of_windows):
                    ...
                    if window_index == 0:
                        ...
                        sum_window_fft["power"] = (
                            sum_window_fft["power"] + window_fft["power"]
                        )

                average_fft_df = sum_window_fft.copy()
                average_fft_df["power"] = average_fft_df["power"] / (
                    number_of_windows + 1
                )

                if cfg_fft["window"]["moving_average"]["flag"]:
                    rolling_window_size = cfg_fft["window"]["moving_average"][
                        "window_size"
                    ]
    ...
    def get_sample_time_series(self):
        ...
        sig = np.sin(2 * np.pi / peak_period * time_vector) + 0.5 * np.random.randn(
            time_vector.size
        )
    ...
    def run_example(self):
        ...
        # the signal with
        np.allclose(peak_freq, 1.0 / peak_period)
