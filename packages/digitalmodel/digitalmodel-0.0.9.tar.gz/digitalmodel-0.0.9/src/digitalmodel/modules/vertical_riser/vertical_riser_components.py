...
from digitalmodel.modules.orcaflex.orcaflex_model_components import (
...
class VerticalRiser:
    ...
    def read_riser_input_data(self):
        ...
        for riser_data_index in range(0, len(custom_cfg["files"]["from_xlsx"])):
            attribute_name = custom_cfg["files"]["from_xlsx"][riser_data_index]["label"]
            setattr(
                self, attribute_name, read_data.from_xlsx(custom_cfg, riser_data_index)
            )

    def assign_key_properties_to_self(self):
        self.host_id = self.cfg["default"]["analysis"]["host"]["id"]
        self.host_name = self.hosts[self.hosts["host_id"] == self.host_id][
            "host_name"
        ].values[0]
        self.host_construction_type = self.hosts[self.hosts["host_id"] == self.host_id][
            "construction_type"
        ].values[0]
        ...
        if (
            self.stack_up_table[self.stack_up_table["Component"] == "Support Spring"][
                "No. Of. Joints"
            ].values[0]
            == 1
        ):
        ...
        self.model_stroke_in_and_out_limits = self.cfg["stack_up"]["stroke_setting"][
            "model_stroke_in_and_out_limits"
        ]
    ...
    def get_fea_properties(self, row_index, weight_method=None):
        ...
        pipe_data = {
            "Outer_Pipe": {
                "Geometry": {
                    "Nominal_OD": self.riser_joints.iloc[row_index]["Pipe OD"],
                    "Nominal_ID": None,
                    "Design_WT": self.riser_joints.iloc[row_index]["Pipe WT"],
        ...
        property_dictionary["section_properties"][
            "pipe"
        ] = self.update_property_dictionary_to_si_units(
            property_dictionary["section_properties"]["pipe"]
        )

        cd_array = [
            self.riser_joints.iloc[row_index][
                "Lateral Drag Coefficient, " + self.fea_type + ", Cd"
            ],
            "~",
            self.riser_joints.iloc[row_index][
                "Axial Drag Coefficient, " + self.fea_type + ", Cd"
            ],
        ]
        ca_array = [
        ...
        if self.riser_joints.iloc[row_index]["Weight Method"] == "measured":
            ...
            buoyancy_diameter = math.sqrt(
                ...
                * 4
                / math.pi
                / self.cfg["Material"]["SeaWater"]["Rho"]
                + pipe_id**2
            )
        ...
        for line_item_index in range(0, len(self.stack_up_table)):
            if self.stack_up_table.iloc[line_item_index]["Aux Lines Present"] == 1:
                property_dictionary["section_properties"]["pipe"].update(
    ...
    def calculate_auxiliary_line_properties(self):
        ...
        auxiliary_lines = self.riser_joints[
            self.riser_joints["Joint Type"] == "auxiliary_line"
        ]
        ...
        for joint_item_index in range(0, len(auxiliary_lines)):
            for properties_index in range(0, len(self.pipe_properties)):
                if (
                    auxiliary_lines.loc[joint_item_index, "Component"]
                    == self.pipe_properties[properties_index]["section_properties"][
                        "pipe"
                    ]["Name"]
                ):
                    auxiliary_lines_internal_area = (
                        auxiliary_lines_internal_area
                        + self.pipe_properties[0]["section_properties"]["pipe"]["Ai"]
                    )
    ...
    def save_data(self):
        file_name = {
            ...
            + "_"
            + "{0}".format("stack_up_properties")
            + ".csv"
        }
        self.fea_model_prep.stack_up_properties_df.to_csv(file_name["file_name"])
            ...
            plt_settings.update(
                {
                    ...
                    + self.cfg["Analysis"]["file_name"]
                    + "_"
                    + "{0}".format(plt_settings["file_name"])
                    + ".png"
                }
            )
