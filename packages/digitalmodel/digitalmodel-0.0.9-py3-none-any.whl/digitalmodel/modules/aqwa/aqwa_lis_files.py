...
class AqwaLISFiles:
    ...
    def get_data(self, cfg, cfg_lis):
        ...
        for input_file in input_files:
            ...
            try:
                ...
                start_keyword_lines = (
                    rd.from_ascii_file_get_line_number_containing_keywords(
                        cfg_keyword_lines
                    )
                )
                start_keyword_line = start_keyword_lines[
                    cfg_lis["search_cfg"]["start"]["occurrence"] - 1
                ]
                ...
                end_keyword_line = end_keyword_lines[
                    cfg_lis["search_cfg"]["end"]["occurrence"] - 1
                ]
                ...
                df = self.get_refine_data_using_cfg(
                    data, cfg_refine, cfg_lis_search_cfg
                )
    ...
    def get_refine_data_using_cfg(self, data, cfg_refine, cfg_lis_search_cfg):
        refine_data_line_numbers = rd.get_array_rows_containing_keywords(
            data, cfg_refine["key_words"], cfg_refine
        )
        df_data = pd.DataFrame()
        for refine_data_line in refine_data_line_numbers:
            df_refine = self.get_data_for_refine_data_line_number(
                data, refine_data_line, cfg_refine, cfg_lis_search_cfg
            )
            df_data = pd.concat([df_data, df_refine], axis=0)

        return df_data

    def get_data_for_refine_data_line_number(
        self, data, refine_data_line, cfg_refine, cfg_lis_search_cfg
    ):
        columns = self.get_header_columns(data, refine_data_line, cfg_refine)

        cfg_data_trans = cfg_refine["data"]["transform"]
        data_start_line = (
            cfg_data_trans["scale"] * refine_data_line + cfg_data_trans["shift"] - 1
        )
        ...
        for data_line in data_array:
            ...
            if len(data_row) == len(columns):
                ...
                if (
                    ...
                    in cfg_lis_search_cfg["key_words"]
                ):
                    try:
                        float(data_row[0])
    ...
    def get_header_columns(self, data, refine_data_line, cfg_refine):
        if "columns" in cfg_refine["header"]:
            ...
            header = data[
                cfg_header_trans["scale"] * refine_data_line
                + cfg_header_trans["shift"]
                - 1
            ]
    ...
    def get_output_transformed_data(self, df, cfg):
        ...
        if ot_cfg is not None and ot_cfg["flag"]:
            ...
            df = self.transform_by_data_key(
                df, ot_cfg, frequency_column, transform_data_key
            )
    ...
    def transform_by_data_key(self, df, ot_cfg, frequency_column, transform_data_key):
        if ot_cfg["output"][transform_data_key]["flag"]:
            ...
            for column_idx in range(0, len(input_columns)):
                ...
                output_column = ot_cfg["output"][transform_data_key]["columns"][
                    column_idx
                ]
                if transform_data_key == "rao_velocity":
                    df[output_column] = df.apply(
                        lambda row: round(
                            (float(row[input_column]) * float(row[frequency_column])), 4
                        ),
                        axis=1,
                    )
                    ...
                    df[output_column] = df.apply(
                        lambda row: round(
                            (
                                float(row[input_column])
                                * float(row[frequency_column]) ** 2
                            ),
                            4,
                        ),
                        axis=1,
    ...
    def save_to_csv(self, df, result_item, cfg, input_file):
        ...
        if save_csv:
            ...
            csv_filename = os.path.join(
                cfg["Analysis"]["result_folder"], input_file + "_" + sheetname + ".csv"
            )
