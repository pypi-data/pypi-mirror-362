...
# The main class
class AqwaServerMgr:
    def __init__(self, port=0, dir_path=None):
        ...
        AQWA_SocketUserForceServerDetailscfg = os.path.join(
            dir_path, "AQWA_SocketUserForceServerDetails.cfg"
        )
        fd = open(AQWA_SocketUserForceServerDetailscfg, "w")
    ...
    def Receive(self, Descriptor):
        ...
        if Descriptor[0] == "s":
            ...
            try:
                ...
                PaddedLength = (
                    ExpectedLength
                    + (SizeOfInt - ExpectedLength % SizeOfInt) % SizeOfInt
                )
...
class Analysis:
    ...
    def GetNodeCurrentPosition(self, Struct, DefAxesX, DefAxesY, DefAxesZ):
        ...
        if (self.Pos == []) | (self.COGs == []):
            sys.stderr.write("")
            raise Exception(
                "Trying to use GetNodeCurrentPosition out of user defined function !"
            )

        RotMat = RotationMatrix(
            self.Pos[Struct][3:]
        )  # Pos[Struct][3:] are the rotation angles
    ...
    def GetNodeCurrentVelocity(self, Struct, DefAxesX, DefAxesY, DefAxesZ):
        ...
        # Calculating offset from COGs in definition axes system
        COGRotVelX = self.Vel[Struct][3]
        COGRotVelY = self.Vel[Struct][4]
        COGRotVelZ = self.Vel[Struct][5]

        # Need to apply rotation on that vector
        RotVelX, RotVelY, RotVelZ = RotMat.Apply(COGRotVelX, COGRotVelY, COGRotVelZ)

        # Now we just need to add the current COG position to the rotated vector
        FinalVelX = RotVelX + self.Vel[Struct][0]
        FinalVelY = RotVelY + self.Vel[Struct][1]
        FinalVelZ = RotVelZ + self.Vel[Struct][2]

        return (FinalVelX, FinalVelY, FinalVelZ)
...
class AqwaUserForceServer(AqwaServerMgr):
    def __init__(self, port=0, dir_path=None):
        AqwaServerMgr.__init__(self, port=port, dir_path=dir_path)

    def Start(self):
        self.CurrentInterface = 0
        self.LatestInterfaceOnClient = self.Receive("i")[0]
        Log.write(
            "Latest interface version on Client is v%d\n" % self.LatestInterfaceOnClient
        )
        ...
        Log.write('Input File Name is "' + self.Analysis.InputFileName + '.DAT"\n')
        ...
        for ns in range(self.Analysis.NOfStruct):
            ...
            Log.write(
                "Position of CoG of Aqwa structure #%d in the definition axis : (%f,%f,%f)\n"
                % (
                    ns + 1,
                    self.Analysis.COGs[-1][0],
                    self.Analysis.COGs[-1][1],
                    self.Analysis.COGs[-1][2],
                )
            )
    ...
    def Run(self, UserForceFunction, ClosingFuntion=None):
        ...
        while True:
            ...
            Force, AddMass, ErrorFlag = UserForceFunction(
                self.Analysis, Mode, Stage, Time, TimeStep, Pos, Vel
            )
...
class BlankForceAddMass(list):
    ...
    def MyBinOp(self, other, op):
        ...
        if len(self) != len(other):
            raise TypeError(
                "Trying to sum two " + Name + "s with different numbers of structures."
            )
...
class BlankForce(BlankForceAddMass):
    ...
    def Flat(self):
        return [
            float(ForceOnDof) for ForceOfStruct in self for ForceOnDof in ForceOfStruct
        ]
