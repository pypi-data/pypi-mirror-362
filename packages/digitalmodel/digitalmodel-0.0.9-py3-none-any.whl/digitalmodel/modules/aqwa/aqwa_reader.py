...
class AqwaReader:
    ...
    def postprocess(self, cfg):
        ...
        for result_item in cfg["result"]:
            result_item, cfg = self.update_iteration_item_with_master_settings(
                result_item, cfg
            )
            ...
            if len(input_files) > 0:
                for input_file in input_files:
                    input_file_result = []
                    result_item_dict = self.get_result_groups(
                        input_file, cfg, result_item
                    )
                    ...
                    if save_csv:
                        sheetname_suffix = result_item["inject_into"].get(
                            "sheetname", None
                        )
                        ...
                        csv_filename = self.save_to_csv(
                            df_item, result_item, cfg, sheetname
                        )
                ...
                if result_category == "equilibrium" and not master_df.empty:
                    ...
                    csv_filename = self.save_to_csv(
                        master_df, result_item, cfg, sheetname
                    )
    ...
    def get_result_groups(self, input_file, cfg, result_item):
        ...
        for aqwa_fundamental_cfg in result_item["groups"]:
            result_group = self.get_result_group(
                input_file, cfg, aqwa_fundamental_cfg, result_item
            )
    ...
    def get_result_group(self, input_file, cfg, aqwa_fundamental_cfg, result_item):
        ...
        for plt_1d_array in plt_2d_array:
            ...
            result_category = result_item["category"]
            if result_category == "equilibrium":
                result_plt_1d_item = self.process_equilibrium_result(
                    input_file, stdout_output
                )
            elif result_category == "frequency":
                result_plt_1d_item = self.process_frequency_result(
                    input_file, stdout_output
                )
            elif result_category == "timeresponse":
                result_plt_1d_item = self.process_timeresponse_result(
                    input_file, stdout_output
                )

            result_group = update_deep_dictionary(result_group, result_plt_1d_item)
            change_result_plt_1d_item = self.get_change_of_data_result_group(
                result_plt_1d_item, aqwa_fundamental_cfg
            )
            if change_result_plt_1d_item is not None:
                result_group = update_deep_dictionary(
                    result_group, change_result_plt_1d_item
                )

            save_aqwareader_csv = result_item.get("save_aqwareader_csv", False)
            if save_aqwareader_csv:
                args = self.get_args_for_data(
                    input_file, cfg, plt_1d_array, result_format="csv"
                )
    ...
    def get_change_of_data_result_group(self, result_plt_1d_item, aqwa_fundamental_cfg):
        ...
        if change_of_data_flag:
            ...
            if change_of_data["reference"] == "start":
                ...
                values = [
                    value * cfg_transform["scale"][idx] + cfg_transform["shift"][idx]
                    for value in values
                ]
            ...
            change_data_key = data_key.replace(
                change_of_data["label_substitution"]["before"],
                change_of_data["label_substitution"]["after"],
            )
            column_1_key = list(result_plt_1d_item.keys())[0]
            change_result_plt_1d_item = {
                column_1_key: result_plt_1d_item[column_1_key],
                change_data_key: values,
            }
    ...
    def get_property_args_array(self, aqwa_fundamental_cfg, result_item):
        ...
        plt_2d_array = [
            list(item) for item in list(itertools.product(*plt_2d_for_itertools))
        ]
    ...
    def process_frequency_result(self, input_file, stdout_output):
        ...
        structure_number = (
            df.iloc[0]["column2"]
            .replace("\t", "")
            .replace("PARAMETERS VERSUS FREQUENCY", "")
            .replace("STRUCTURE", "")
            .replace('"', "")
            .replace(" ", "")
        )
        third_level = (
            df.iloc[2]["column2"].replace("\t", "").replace('"', "").replace("-", "")
        )
        fourth_level = df.iloc[3]["column2"].replace("\t", "").replace('"', "")
    ...
    def inject_to_excel(self, df, result_item, cfg):
        if "inject_into" in result_item and result_item["inject_into"]["flag"]:
    ...
    def save_to_csv(self, df, result_item, cfg, sheetname):
        ...
        csv_filename = os.path.join(
            cfg["Analysis"]["result_folder"], sheetname + ".csv"
        )
        df.to_csv(csv_filename, index=False, header=True, float_format="%.6E")
    ...
    def save_statistics(self, cfg, df, sheetname):
        ...
        statistics_filename = os.path.join(
            cfg["Analysis"]["result_folder"], sheetname + "_statistics.csv"
        )
