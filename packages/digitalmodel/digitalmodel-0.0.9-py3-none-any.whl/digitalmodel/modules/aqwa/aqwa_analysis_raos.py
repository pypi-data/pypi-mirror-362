...
class AqwaRAOs:
    ...
    def rao_router(self, cfg: dict) -> None:
        ...
        if damping_cfg_no_damp["is_file_valid"]:
            cfg, additional_damping_values = self.prepare_damping(
                cfg, damping_cfg_no_damp, analysis_settings_key
            )
            ...
            cfg, additional_damping_values = self.prepare_damping(
                cfg, damping_cfg_damp_ad, analysis_settings_key
            )
        ...
        for input in input_cfg:
            ...
            for input_files_array in input_files_cfg:
                for file_idx in range(0, len(input_files_array)):
                    ...
                    is_file_valid, relative_input_file = is_file_valid_func(
                        input_file, analysis_root_folder
                    )
                    if is_file_valid:
                        ...
                        library_filename = wwy.get_library_filename(
                            library_filename_cfg
                        )
                        if os.path.isfile(library_filename):
                            ...
                            raise FileNotFoundError(
                                f"File {input_file} not found in {analysis_root_folder} or {module_path}"
                            )
    ...
    def create_decks_directory(self, cfg):
        file_management_input_directory = cfg["Analysis"][
            "file_management_input_directory"
        ]
        file_management_output_directory = os.path.join(
            file_management_input_directory, "decks"
        )
    ...
    def derive_damping(self, cfg, analysis_settings_key):
        filename_pattern = cfg.analysis_settings[analysis_settings_key]["output"][
            "filename"
        ]
        natural_period, is_file_valid = self.get_natural_period_from_lis_file(
            cfg, filename_pattern
        )
        percent_critical_damping, is_file_valid = self.get_percent_critical_damping(
            cfg, filename_pattern
        )
    ...
    def get_natural_period_from_lis_file(self, cfg, filename_pattern):
        ...
        template_yaml["file_management"]["files"]["files_in_current_directory"][
            "filename_pattern"
        ] = filename_pattern

        cfg_temp = dm_engine(inputfile=None, cfg=template_yaml, config_flag=False)
        cfg["analysis_settings"]["damping"]["output"] = {
            "natural_period": cfg_temp[cfg_temp["basename"]]
        }
        csv_file = cfg["analysis_settings"]["damping"]["output"]["natural_period"][
            "csv_filename"
        ][0]
        ...
        try:
            ...
            if "drafts" in cfg["analysis_settings"]:
                ...
                if len(natural_period) >= no_of_frequencies:
                    ...
                    logging.error(
                        "Natural period values not found for all frequencies."
                    )
        except Exception:
            is_file_valid = False
    ...
    def get_percent_critical_damping(self, cfg, filename_pattern):
        ...
        cfg_temp = dm_engine(inputfile=None, cfg=template_yaml, config_flag=False)
        cfg["analysis_settings"]["damping"]["output"] = {
            "critical_damping": cfg_temp[cfg_temp["basename"]]
        }

        csv_file = cfg["analysis_settings"]["damping"]["output"]["critical_damping"][
            "csv_filename"
        ][0]
    ...
    def get_absolute_damping(self, cfg, filename_pattern):
        ...
        csv_file = cfg["analysis_settings"]["damping"]["output"]["absolute_damping"][
            "csv_filename"
        ]
        try:
    ...
    def get_template_natural_period(self, cfg):
        logging.info("Extracting natural period values")
        template_file_name = cfg["analysis_settings"]["damping"]["template"][
            "natural_period"
        ]
        ...
        template_yaml["file_management"]["files"]["files_in_current_directory"][
            "directory"
        ] = cfg["Analysis"]["file_management_input_directory"]
    ...
    def prepare_damping(self, cfg, damping_cfg, analysis_settings_key):
        ...
        if (
            natural_period.empty
            or percent_critical_damping.empty
            or absolute_damping.empty
        ):
            logging.error(
                "One or more damping files are empty. Skipping damping preparation."
            )
        else:
            peak_periods = self.get_natural_periods(natural_period)
            additional_damping_values = self.get_damping_values(
                cfg,
                percent_critical_damping,
                absolute_damping,
                peak_periods,
    ...
    def prepare_damping_decks(self, cfg, additional_damping_values):
        ...
        deck_input["inputs"][1]["data"]["raw"]["added_damping"][
            "rxx"
        ] = additional_damping_values["roll"]
        deck_input["inputs"][1]["data"]["raw"]["added_damping"][
            "ryy"
        ] = additional_damping_values["pitch"]
    ...
    def get_natural_periods(self, natural_period):
        ...
        omega = {"roll": roll, "pitch": pitch, "heave": heave}
        ...
        peak_response = {"omega": omega, "frequency": frequency, "period": period}
        return peak_response

    def get_damping_values(
        self,
        cfg,
        percent_critical_damping,
        absolute_damping,
        ...
        absolute_damping_dict = {
            "x": "frequency",
            "y": "RADIATION DAMPING ROLL(X) _ROLL(X)",
        }
        ...
        roll_additional_damping = self.get_additional_damping_by_cfg(
            cfg,
            cfg_additional_damping,
            percent_critical_damping,
            absolute_damping,
        ...
        absolute_damping_dict = {
            "x": "frequency",
            "y": "RADIATION DAMPING PITCH(Y) _PITCH(Y)",
        }
        cfg_additional_damping["critical_damping"] = critical_damping_dict
        cfg_additional_damping["absolute_damping"] = absolute_damping_dict

        pitch_additional_damping = self.get_additional_damping_by_cfg(
            cfg,
            cfg_additional_damping,
            percent_critical_damping,
            absolute_damping,
    ...
    def get_additional_damping_by_cfg(
        self,
        cfg,
        cfg_additional_damping,
        percent_critical_damping,
        ...
        target_critical_damping = cfg["analysis_settings"]["damping"]["target_damping"][
            peak_omega_key
        ]

        if current_critical_damping > 0:
            additional_damping = self.get_additional_damping(
                current_critical_damping,
                current_absolute_damping,
                target_critical_damping,
            )
        elif current_critical_damping == 0:
            additional_damping = current_absolute_damping
        else:
            raise NotImplementedError(
                "Current critical damping is negative. No Logic implemented for negative damping."
            )
        ...
        cfg["analysis_settings"][analysis_settings_key][
            "output"
        ] = update_deep_dictionary(
            cfg["analysis_settings"][analysis_settings_key]["output"],
            {"damping": {peak_omega_key: damping_output}},
        )

        return additional_damping

    def get_additional_damping(
        self,
        current_critical_damping,
        current_absolute_damping,
        target_critical_damping,
    ...
    def get_raos_by_cfg(self, cfg, rao_plot_cfg):
        ...
        for direction in directions:
            ...
            template_yaml["result"][0]["inject_into"]["sheetname"] = (
                sheetname_base + "_" + str(direction)
            )
            cfg_temp = dm_engine(inputfile=None, cfg=template_yaml, config_flag=False)
            raos_cfg = {"raos": cfg_temp[cfg_temp["basename"]]}
            cfg["analysis_settings"]["damping"]["output"].update(raos_cfg)

            csv_file = cfg["analysis_settings"]["damping"]["output"]["raos"][
                "csv_filename"
            ]
            try:
        ...
        cfg["analysis_settings"][rao_key]["output"].update(
            {"raos": {"csv_filename": csv_file_dict}}
        )
    ...
    def plot_raos_by_cfg(self, cfg, rao_plot_cfg):
        ...
        no_damp_csv_file_dict = cfg["analysis_settings"][rao_key]["output"]["raos"][
            "csv_filename"
        ]
        ...
        plot_yml["master_settings"]["groups"]["columns"] = columns
        settings = {
            "file_name": f"{filename_prefix}_all_dir",
