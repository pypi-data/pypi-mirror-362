...
class DNVOSF101Pipe(RigidPipe):
    def __init__(self, nominalDiameter, nominalWallThickness, Parameters=None):
        ...
        if Parameters is None:
            ...
            self.Parameters.pressureTestSafetyFactors.system = ast.literal_eval(
                self.Parameters.pressureTestSafetyFactors.system
            )
            self.Parameters.pressureTestSafetyFactors.mill = ast.literal_eval(
                self.Parameters.pressureTestSafetyFactors.mill
            )
            self.Parameters.pressureContainment.safetyClass = ast.literal_eval(
                self.Parameters.pressureContainment.safetyClass
            )
            self.Parameters.materialFabrication.factors = ast.literal_eval(
                self.Parameters.materialFabrication.factors
            )
            self.Parameters.other.safetyClass = ast.literal_eval(
                self.Parameters.other.safetyClass
            )
            self.Parameters.loadEffectFactors.values = ast.literal_eval(
                self.Parameters.loadEffectFactors.values
            )
            self.Parameters.load.values = ast.literal_eval(self.Parameters.load.values)
            self.Parameters.conditionLoadEffectFactors.values = ast.literal_eval(
                self.Parameters.conditionLoadEffectFactors.values
            )
    ...
    def temperatureDeratingValues(self, fluidTemperature=None):
        if fluidTemperature is None:
            if (
                self.Parameters.loadPressure.fluidTemperature
                > self.Parameters.materialProperties.temperatureDerateLimit
            ):
                self.fy_temp = (
                    1.14 - self.Parameters.loadPressure.fluidTemperature / 850
                )  # Temperature Derating Factor for the Yield Stress.
    ...
    def updateStrengthValues(
        self, specifiedMinimumYieldStrength=None, specifiedMinimumTensileStrength=None
    ):
        if specifiedMinimumYieldStrength is not None:
            self.Parameters.materialProperties.specifiedMinimumYieldStrength = (
                specifiedMinimumYieldStrength
            )
        self.minimumYieldStrength = (
            self.Parameters.materialProperties.specifiedMinimumYieldStrength
            - self.fy_temp
        ) * self.Parameters.materialStrengthFactors.Normally

        if specifiedMinimumYieldStrength is not None:
            self.Parameters.materialProperties.specifiedMinimumTensileStrength = (
                specifiedMinimumTensileStrength
            )
        self.minimumTensileStrength = (
            self.Parameters.materialProperties.specifiedMinimumTensileStrength
            - self.fu_temp
        ) * self.Parameters.materialStrengthFactors.Normally

    def DNVThickness(self):
        self.thickness1["installation"] = (
            self.nominalWallThickness - self.thicknessFabricationTolerance
        )
        self.thickness1["operation"] = (
            self.nominalWallThickness
            - self.thicknessFabricationTolerance
            - self.corrosionAllowance
        )
        self.thickness2["installation"] = self.nominalWallThickness
        self.thickness2["operation"] = (
            self.nominalWallThickness - self.corrosionAllowance
        )
        self.thickness3["installation"] = self.nominalWallThickness
        self.thickness3["operation"] = (
            self.nominalWallThickness - 0.5 * self.corrosionAllowance
        )

    def pressureContainmentResistance(self, thickness=None):
        yield_ContainmentOrBurst = min(
            self.Parameters.materialProperties.specifiedMinimumYieldStrength,
            self.Parameters.materialProperties.specifiedMinimumTensileStrength / 1.15,
        )
        if thickness is None:
            self.pressureContainmentResistance = (
                2
                * self.nominalWallThickness
                / (self.nominalDiameter - self.nominalWallThickness)
                * yield_ContainmentOrBurst
                * 2
                / math.sqrt(3)
            )
        else:
            self.pressureContainmentResistance = (
                2
                * thickness
                / (self.nominalDiameter - thickness)
                * yield_ContainmentOrBurst
                * 2
                / math.sqrt(3)
            )
    ...
    def pressureContainment(
        self, operatingCondition, safetyClass="high", limitState="ULS"
    ):
        thickness = self.thickness1[operatingCondition]
        self.pressureContainmentResistance(thickness)
        quantity1 = (
            self.pressureContainmentResistance
            / self.Parameters.pressureTestSafetyFactors.system[safetyClass]
            / self.Parameters.materialResistanceFactors[limitState]
        )
        quantity2 = (
            self.Parameters.loadPressure.plt
            / self.Parameters.pressureTestSafetyFactors.system[safetyClass]
            - self.Parameters.loadPressure.external
        )
        quantity3 = (
            self.Parameters.loadPressure.ph
            * self.Parameters.materialStrengthFactors.Normally
            / self.Parameters.pressureTestSafetyFactors.mill[safetyClass]
        )
    ...
    def collapse(self, thickness, pipeFabricationMethod):
        ...
        self.elasticCollapseCapacity = (
            2
            * self.Parameters.materialProperties.youngsModulus
            * self.tOverDRatio**3
            / (1 - self.Parameters.materialProperties.poissionsratio**2)
        )
        self.plasticCollapseCapacity = (
            self.minimumYieldStrength
            * self.Parameters.materialFabrication.factors[pipeFabricationMethod]
            * 2
            * self.tOverDRatio
        )
        ...
        c = -(
            self.plasticCollapseCapacity**2
            + self.plasticCollapseCapacity
            * self.elasticCollapseCapacity
            * self.ovality
            * self.dOverTRatio
    ...
    def localBucklingUnderExternalPressure_polyMethod(
        self, thickness, pipeFabricationMethod
    ):
    ...
    def localBucklingUnderExternalPressure(
        self, thickness, pipeFabricationMethod, safetyClass="high", limitState="ULS"
    ):
        ...
        LHS = (
            self.Parameters.loadPressure.external
            - self.Parameters.loadPressure.minimumInternal
        )
        RHS = (
            self.collapsePressure
            / self.Parameters.other.safetyClass[safetyClass]
            / self.Parameters.materialResistanceFactors[limitState]
        )
    ...
    def propagationBuckling(
        self,
        thickness,
        pipeFabricationMethod,
        safetyClass="high",
        limitState="ULS",
        pipeCondition="Cold",
    ):
        ...
        if self.dOverTRatio < 15 or self.dOverTRatio < 45:
            self.propagationPressure = (
                35
                * yieldStrength
                * self.Parameters.materialFabrication.factors[pipeFabricationMethod]
                * (self.tOverDRatio**2.5)
            )
        else:
    ...
    def localBucklingCombinedLoadingLoadControlled(
        self,
        thickness,
        pipeFabricationMethod,
        operatingCondition,
        ...
        if pInternal >= pExternal:
            ...
            overPressureRatio = (
                pExternal - self.Parameters.loadPressure.minimumInternal
            ) / self.collapsePressure
        ...
        Msd1 = (
            self.Parameters.load.values["bendingMoment"]["functional"]
            * self.Parameters.loadEffectFactors.values[limitState]["functional"]
            * gammac
            + self.Parameters.load.values["bendingMoment"]["environmental"]
            * self.Parameters.loadEffectFactors.values[limitState]["environmental"]
            + self.Parameters.load.values["bendingMoment"]["interference"]
            * self.Parameters.loadEffectFactors.values[limitState]["interference"]
            * gammac
            + self.Parameters.load.values["bendingMoment"]["accidental"]
            * self.Parameters.loadEffectFactors.values[limitState]["accidental"]
            * gammac
        )
        Ssd1 = (
            self.Parameters.load.values["effectiveTension"]["functional"]
            * self.Parameters.loadEffectFactors.values[limitState]["functional"]
            * gammac
            + self.Parameters.load.values["effectiveTension"]["environmental"]
            * self.Parameters.loadEffectFactors.values[limitState]["environmental"]
            + self.Parameters.load.values["effectiveTension"]["interference"]
            * self.Parameters.loadEffectFactors.values[limitState]["interference"]
            * gammac
            + self.Parameters.load.values["effectiveTension"]["accidental"]
            * self.Parameters.loadEffectFactors.values[limitState]["accidental"]
            * gammac
        )
        ...
        plasticTensionCapacity = (
            yieldStrength * math.pi * (self.nominalDiameter - thickness) * thickness
        )
        plasticMomentCapacity = (
            yieldStrength
            * math.pi
            * ((self.nominalDiameter - thickness) ** 2)
            * thickness
        )

        beta = (60 - self.dOverTRatio) / 90
        alphaC = (1 - beta) + beta * (
            self.Parameters.materialProperties.tensileStrength
        ) / yieldStrength
        ...
        if (self.dOverTRatio <= 15) or (self.dOverTRatio <= 45):
            logging.critical(
                "CAUTION: D/t ratio outside of code limits. Adjust platic loads with FEA findings prior to use"
            )
        if self.Ssd / plasticTensionCapacity >= 0.4:
            logging.critical(
                "CAUTION: Tension loading ratio outside of code limits. Refer to code"
            )

        self.localBucklingCombinedLoadingLoadControlledCheckMargin = (
            RHS - interactionRatio
        ) / RHS
    ...
    def localBucklingCombinedLoadingDisplacementControlled(
        self,
        thickness,
        pipeFabricationMethod,
        operatingCondition,
        ...
        logging.critical(
            "Update alphah in .ini file to re-run Displacement controlled method"
        )
        ...
        gammac = self.Parameters.conditionLoadEffectFactors.values[conditionEffect]
        esd1 = (
            self.Parameters.load.values["strain"]["functional"]
            * self.Parameters.loadEffectFactors.values[limitState]["functional"]
            * gammac
            + self.Parameters.load.values["strain"]["environmental"]
            * self.Parameters.loadEffectFactors.values[limitState]["environmental"]
            + self.Parameters.load.values["strain"]["interference"]
            * self.Parameters.loadEffectFactors.values[limitState]["interference"]
            * gammac
            + self.Parameters.load.values["strain"]["accidental"]
            * self.Parameters.loadEffectFactors.values[limitState]["accidental"]
            * gammac
        )
        esd2 = (
        ...
        if pInternal >= pExternal:
            ec = (
                0.78
                * (self.tOverDRatio - 0.01)
                * (
                    1
                    + 5.75
                    * (self.Parameters.loadPressure.minimumInternal - pExternal)
                    / self.pressureContainmentResistance
                )
                * self.Parameters.defaults.alphah**-1.5
                * self.Parameters.defaults.alphagw
            )
            logging.critical("Internal Overpressure")

            self.localBucklingCombinedLoadingDisplacementControlledCheckMargin = (
                esd - ec
            ) / ec
            if esd <= ec:
            ...
            logging.critical("External Overpressure")
            LHS = (
                esd / ec * self.Parameters.resistanceFactors.gammae[safetyClass]
            ) ** 0.8 + (
                pExternal - self.Parameters.loadPressure.minimumInternal
            ) / self.collapsePressure * self.Parameters.other.safetyClass[
                safetyClass
            ] / self.Parameters.materialResistanceFactors[
                limitState
            ]
            RHS = 1
            self.localBucklingCombinedLoadingDisplacementControlledCheckMargin = (
