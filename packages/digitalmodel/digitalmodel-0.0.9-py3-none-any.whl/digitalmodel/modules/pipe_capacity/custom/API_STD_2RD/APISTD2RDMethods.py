...
def APISTD2RDBurst(data):
    ...
    Pb = (
        0.45
        * (data["S"] + data["U"])
        * math.log(data["OD"] / (data["ID"] + 2 * data["CorrosionAllowance"]))
    )
...
def variableWTBurst(cfg):
    ...
    if cfg["geometry"]["NominalOD"] != None:
        ...
        for t in range(0, len(cfg["geometry"]["NominalWT"])):
            newDFRow = [
                cfg["geometry"]["NominalID"] + 2 * cfg["geometry"]["NominalWT"][t],
        ...
        inputData.update(
            {
                ...
                "S": cfg["material"]["SMYS"],
                "U": cfg["material"]["SMUS"],
                "CorrosionAllowance": cfg["geometry"]["CorrosionAllowance"],
...
def APISTD2RDMethod1(data):
    ...
    My = (
        math.pi
        / 4
        * (data["S"] * (data["OD"] - data["t"]) ** 2 * data["t"])
        / 1000
        / 12
        * 1.355818
    )  # Unit kN.m
    ...
    FDPressureCorrected = math.sqrt(
        (data["designFactors"]["internalPressure"][data["LimitState"]] ** 2)
        - ((data["internalPressure"] - data["externalPressure"]) / data["Pb"]) ** 2
    )
    ...
    TensionLimitCheck = (
        ...
        + (Tlimiting / Ty) ** 2
    ) / (data["designFactors"]["internalPressure"][data["LimitState"]])
    ...
    for i in range(0, len(TArray)):
        ...
        # Method 2 Plotting
        MarrayPositive2.append(
            Mp
            * (FDPressureCorrected)
            * math.cos((math.pi / 2) * ((TArray[i] / Ty) / FDPressureCorrected))
        )
        MarrayNegative2.append(
            -Mp
            * (FDPressureCorrected)
            * math.cos((math.pi / 2) * ((TArray[i] / Ty) / FDPressureCorrected))
        )

    resultMatrix = np.stack(
        (TArray, MarrayPositive1, MarrayNegative1, MarrayPositive2, MarrayNegative2),
        axis=-1,
    )
...
def APISTD2RDMethod1Utilization(cfg):
    ...
    My = (
        ...
        * (
            cfg["Method1SLS"]["S"]
            * (cfg["Method1SLS"]["OD"] - cfg["Method1SLS"]["t"]) ** 2
            * cfg["Method1SLS"]["t"]
        )
        / 1000
        / 12
    ...
    for i in range(0, len(cfg["LoadingConditionResults"])):
        ...
        if (
            cfg["LoadingConditionResults"][i]["InternalPressure"]
            >= cfg["LoadingConditionResults"][i]["ExternalPressure"]
        ):
            FDPressureCorrected = math.sqrt(
                (cfg["designFactors"]["internalPressure"][loadCategory] ** 2)
                - (
                    (
                        cfg["LoadingConditionResults"][i]["InternalPressure"]
                        - cfg["LoadingConditionResults"][i]["ExternalPressure"]
            ...
            cfg["LoadingConditionResults"][i][
                "PressureCondition"
            ] = "Internal OverPressure"
        else:
            FDPressureCorrected = math.sqrt(
                (cfg["designFactors"]["externalPressure"][loadCategory] ** 2)
                - (
                    (
                        cfg["LoadingConditionResults"][i]["InternalPressure"]
                        - cfg["LoadingConditionResults"][i]["ExternalPressure"]
        ...
        cfg["LoadingConditionResults"][i]["DesignFactor"] = cfg["designFactors"][
            "internalPressure"
        ][loadCategory]
