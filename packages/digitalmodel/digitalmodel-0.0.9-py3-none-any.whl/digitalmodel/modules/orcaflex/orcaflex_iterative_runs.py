...
class OrcaflexIterativeRuns:
        ...
        for i in range(0, len(cfg_files)):
            ...
            if os.path.isfile(filename):
                if "iterate_yml" in cfg_files[0]:
                    logging.info(f"Iteration analysis for File: {filename} ... START")
                    ...
                    iterative_yml = self.get_iterative_yaml(iterate_cfg, filename_copy)
                    ...
                    logging.info(f"Iteration analysis for File: {filename} ... SKIP")
            else:
    ...
    def get_filename_copy(self, filename):
        ...
        filename_copy = os.path.join(filename_components["filepath"], filename_copy)
    ...
    def get_iterative_filename(self, filename):
        related_files, related_files_variations = self.find_related_files(filename)
        ...
        if len(related_files) > 1:
            logging.warning("Multiple files found ... FIles will be OVERWRITTEN")
    ...
    def run_iterative_simulations(self):
        for i in range(0, len(self.iterative_filenames)):
            ...
            filename_with_ext = iterative_filename["iterative_filename_with_ext"]
            filename_save_data = self.cfg["Analysis"]["input_files"]["with_ext"][i]
            file_ext = filename_save_data.split(".")[-1]
            filename_save_simulation = filename_save_data.replace(file_ext, "sim")
            ...
            logging.info(f"Finished running iterative file: {filename_with_ext}")
