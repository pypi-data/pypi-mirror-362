...
class OPPLinkedStatistics:
    ...
    def get_linked_statistics(
        self, cfg: Dict[str, Any], model, file_name
    ) -> Dict[str, Any]:
        ...
        for ls_group in ls_groups:
            ...
            for ls_cfg in ls_group["Columns"]:
                ...
                linked_statistics = self.get_linked_statistics_from_orcaflex_run(
                    model, ls_cfg
                )
    ...
    def get_linked_statistics_from_orcaflex_run(self, model, ls_cfg) -> Dict[str, Any]:
        ...
        (
            ...
            VariableName,
            Statistic_Type,
        ) = of_objects.get_orcaflex_objects(model, ls_cfg)
    ...
    def add_file_result_to_all_results(
        self, linked_statistics, linked_statistics_for_file
    ):
        if not linked_statistics:
            ...
            for key in linked_statistics_for_file.keys():
                linked_statistics[key] = pd.concat(
                    [linked_statistics[key], linked_statistics_for_file[key]],
                    ignore_index=True,
                )
        return linked_statistics
    ...
    def save_linked_statistics(self, linked_statistics, cfg):
        ...
        if "csv_decimal" in cfg.orcaflex["postprocess"]["linked_statistics"]:
            csv_decimal = cfg.orcaflex["postprocess"]["linked_statistics"][
                "csv_decimal"
            ]
        ...
        for key in linked_statistics.keys():
            ...
            file_name_with_path = os.path.join(
                cfg["Analysis"]["result_folder"], file_name
            )
        ...
        cfg[cfg["basename"]] = {
            "linked_statistics": {"groups": linked_statistics_array}
        }
