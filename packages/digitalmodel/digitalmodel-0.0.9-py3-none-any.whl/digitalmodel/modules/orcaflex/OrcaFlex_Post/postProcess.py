...
def postProcessRange(model, cfg, FileObjectName):
    ...
    for RangeGraphIndex in range(0, len(cfg["RangeGraph"])):
        ...
        for AdditionalDataIndex in range(
            ...
            AdditionalDataName = cfg["RangeGraph"][RangeGraphIndex]["AdditionalData"][
                AdditionalDataIndex
            ]
...
def postProcessSummary(
    model, cfg, SummaryDF, SummaryIndex, FileDescription, FileObjectName
):
    if SummaryDF.empty:
        columns = []
        ...
        for SummaryColumnIndex in range(
            ...
            columns.append(
                cfg["Summary"][SummaryIndex]["Columns"][SummaryColumnIndex]["Label"]
            )
    ...
    for SummaryColumnIndex in range(0, len(cfg["Summary"][SummaryIndex]["Columns"])):
        ...
        try:
            objectName = cfg["Summary"][SummaryIndex]["Columns"][SummaryColumnIndex][
                "ObjectName"
            ]
            OrcFXAPIObject = model[objectName]
        except:
        ...
        TimePeriod = exec(
            "OrcFxAPI.{0}".format(
                cfg["Summary"][SummaryIndex]["Columns"][SummaryColumnIndex][
                    "SimulationPeriod"
                ]
            )
        )
        VariableName = cfg["Summary"][SummaryIndex]["Columns"][SummaryColumnIndex][
            "Variable"
        ]
