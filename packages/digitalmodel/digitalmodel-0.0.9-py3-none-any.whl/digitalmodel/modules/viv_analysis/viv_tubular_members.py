...
class VIVTubularMembers:
    ...
    def analyze(self, cfg):
        ...
        safety_factors_df = self.get_safety_factors(
            cfg, natural_frequencies_df, vs_frequencies_df
        )

        self.save_results(
            cfg, natural_frequencies_df, vs_frequencies_df, safety_factors_df
        )

        return cfg

    def save_results(
        self, cfg, natural_frequencies_df, vs_frequencies_df, safety_factors_df
    ):
        ...
        filename = os.path.join(
            ...
            cfg["Analysis"]["file_name"] + "_natural_frequencies.csv",
        )
        natural_frequencies_df.to_csv(filename, index=False)

        filename = os.path.join(
            cfg["Analysis"]["result_folder"],
            cfg["Analysis"]["file_name"] + "_vs_frequencies.csv",
        )
        vs_frequencies_df.to_csv(filename, index=False)

        filename = os.path.join(
            cfg["Analysis"]["result_folder"],
            cfg["Analysis"]["file_name"] + "_safety_factors.csv",
        )
    ...
    def get_natural_frequencies(self, cfg):
        ...
        mass_without_internal_fluid = system_properties["mass"]["air"][
            "without_internal_fluid"
        ]
        mass_with_internal_fluid = system_properties["mass"]["air"][
            "with_internal_fluid"
        ]
        ...
        natural_frequencies_df = pd.DataFrame(
            columns=[
                "span_length",
                "boundary_condition",
                "internal_fluid",
        ...
        for span_length in span_length_array:
            if cfg["modes"]["analysis"]["simply_supported"] == True:
                self.get_simply_supported_modes(
                    E,
                    I,
                    mass_without_internal_fluid,
                    mass_with_internal_fluid,
                    natural_frequencies_df,
                    span_length,
                )

            if cfg["modes"]["analysis"]["clamped_clamped"] == True:
                self.get_clamped_modes(
                    cfg,
                    E,
                    I,
                    mass_without_internal_fluid,
    ...
    def get_free_free_modes(
        ...
        for mode_number in list(range(1, 4)):
            frequency = (
                (1 / (2 * math.pi))
                * (22.373 / span_length**2)
                * ((E * I) / (mass_without_internal_fluid / g_inch_per_second_squared))
                ** 0.5
            )
            free_free_empty.append(frequency)
        natural_frequencies_df.loc[len(natural_frequencies_df)] = [
            span_length / 12,
            "free_free",
            "empty",
            free_free_empty[0],
        ...
        for mode_number in list(range(1, 4)):
            frequency = (
                (1 / (2 * math.pi))
                * (22.373 / span_length**2)
                * ((E * I) / (mass_with_internal_fluid / g_inch_per_second_squared))
                ** 0.5
            )
    ...
    def get_clamped_modes(
        ...
        for mode_number in list(range(1, 4)):
            frequency = (
                (1 / (2 * math.pi))
                * (eigen_values_clamped[mode_number - 1] / span_length) ** 2
                * ((E * I) / (mass_without_internal_fluid / g_inch_per_second_squared))
                ** 0.5
            )
        ...
        natural_frequencies_df.loc[len(natural_frequencies_df)] = [
            ...
            "clamped_clamped",
            "empty",
            clamped_clamped_empty[0],
            clamped_clamped_empty[1],
            clamped_clamped_empty[2],
        ]
        ...
        natural_frequencies_df.loc[len(natural_frequencies_df)] = [
            ...
            "clamped_clamped",
            "flooded",
            clamped_clamped_flooded[0],
            clamped_clamped_flooded[1],
            clamped_clamped_flooded[2],
        ]

    def get_simply_supported_modes(
        self,
        E,
        I,
        mass_without_internal_fluid,
        ...
        for mode_number in list(range(1, 4)):
            frequency = (
                (1 / (2 * math.pi))
                * (mode_number * math.pi / span_length) ** 2
                * ((E * I) / (mass_without_internal_fluid / g_inch_per_second_squared))
                ** 0.5
            )
        ...
        natural_frequencies_df.loc[len(natural_frequencies_df)] = [
            ...
            "simply_supported",
            "empty",
            simply_supported_ends_empty[0],
            simply_supported_ends_empty[1],
            simply_supported_ends_empty[2],
        ]
        ...
        natural_frequencies_df.loc[len(natural_frequencies_df)] = [
            ...
            "simply_supported",
            "flooded",
            simply_supported_ends_flooded[0],
            simply_supported_ends_flooded[1],
            simply_supported_ends_flooded[2],
        ]
    ...
    def get_vs_frequencies(self, cfg):
        ...
        kinematic_viscosity = (
            external_fluid_cfg["kinematic_viscosity"] * (1 / 0.0254) ** 2
        )
        OD = cfg["pipeline"]["crossection"][0]["Nominal_OD"]
        current_cfg = cfg["environment"]["current"]

        viv_frequencies_df = pd.DataFrame(
            columns=[
                "current_label",
                "depth",
                "current_speed",
                "Re",
                "shredding_frequency_inline",
                "KC",
                "shredding_frequency_crossflow",
            ]
        )
        for current_item in current_cfg:
            ...
            for depth_item in current_item["data"]:
                ...
                viv_frequencies_df.loc[len(viv_frequencies_df)] = [
                    current_label,
                    current_depth,
                    current_speed,
                    Re,
    ...
    def get_safety_factors(self, cfg, natural_frequencies, vs_frequencies):
        safety_factor_df = pd.DataFrame(
            columns=[
                ...
                "internal_fluid",
                "mode 1",
                "safety_factor_inline",
                "safety_factor_crossflow",
            ]
        )
        ...
        for vs_index, vs_row in vs_frequencies.iterrows():
            ...
            for nf_index, nf_row in natural_frequencies.iterrows():
                ...
                safety_factor_df.loc[len(safety_factor_df)] = [
                    span_length,
                    current_label,
                    depth,
                    current_speed,
        ...
        for boundary_condition in boundary_condition_array:
            for internal_fluid in internal_fluid_array:
                for modes in modes_array:
                    group_item = {}
                    df_filtered = df[
                        (df["boundary_condition"] == boundary_condition)
                        & (df["internal_fluid"] == internal_fluid)
                    ]
                    ...
                    group_item["label"] = (
                        boundary_condition + ", " + internal_fluid + ", " + modes
                    )
    ...
    def get_vs_frequencies_plot(self, cfg):
        ...
        settings = {
            "file_name": cfg["Analysis"]["file_name_for_overwrite"] + "_vs_frequencies",
            "title": "VIV Frequency",
        ...
        for depth in depth_array:
            for current_label in current_label_array:
                ...
                for internal_fluid in internal_fluid_array:
                    for boundary_condition in boundary_condition_array:
                        df_filtered = df[
                            (df["current_label"] == current_label)
                            & (df["internal_fluid"] == internal_fluid)
                            & (df["boundary_condition"] == boundary_condition)
                            & (df["depth"] == depth)
                        ]
                ...
                settings = {
                    "file_name": cfg["Analysis"]["file_name_for_overwrite"]
                    + f"_safety_factors_curr_{current_label}_dp{str(depth)}",
