...
class RAOAnalysis:
    ...
    def write_filtered_orcaflex_seastate_rao_file(
        self, file_name, filter_SeaStateRAOs_data, filtered_file_vessel_data
    ):
        print("Reading file: {}".format(file_name))
        yml_data = ymlInput(file_name, updateYml=None)
        ...
        file_name_filtered = file_name.replace(
            file_name_stem + ".yml", file_name_stem + "_filtered"
        )
    ...
    def plot_single_vessel_data(self, file_vessel_data, vessel_cfg):
        ...
        vessel_data = self.get_vessel_data(file_vessel_data, vessel_cfg, vessel_names)
    ...
    def plot_displacement_RAOs_amplitudes_for_draught(self, RAOs, draught):
        ...
        fig = make_subplots(
            ...
            cols=ncols,
            shared_xaxes=True,
            subplot_titles=("Heave", "Surge", "Sway", "Yaw", "Roll", "Pitch"),
        )
        ...
        if len(xaxis_range) > 0:
            ...
            RAOs = RAOs[
                (RAOs[RAOPeriodOrFreq_column] >= xaxis_range[0])
                & (RAOs[RAOPeriodOrFreq_column] <= xaxis_range[1])
            ].copy()
        ...
        for row_index in range(0, nrows):
            fig.update_yaxes(
                title_text=yaxes_title_text[row_index], row=row_index + 1, col=col
            )

        fig.update_xaxes(title_text=xaxes_title_text[nrows - 1], row=nrows, col=ncols)
    ...
    def get_rao_data_for_vessel(self, vessel_data, draught_data):
        ...
        for i in range(0, len(draughts_index)):
            RAOs = vessel_data["Draughts"][draughts_index[i]]["DisplacementRAOs"][
                "RAOs"
            ]
    ...
    def get_DisplacementRAOs_for_draught(self, RAOs):
        ...
        columns_set_1_strip = [column.strip() for column in columns_set_1_columns]
        ...
        if self.cfg["rao_plot"]["seastate"]["flag"]:
            ...
            for file in cfg["Files"]:
                if os.path.isfile(file["Name"]):
                    ...
                    ) = self.filter_orcaflex_seastate_raos_visual_grid(file["Name"])
    ...
    def filter_orcaflex_seastate_raos(self, file_vessel_data):
        ...
        x_low = (
            filter_settings["cog"]["x"]
            - filter_settings["l"] / 2
            - filter_settings["beyond_boundary"]
        )
        x_high = (
            filter_settings["cog"]["x"]
            + filter_settings["l"] / 2
        ...
        df_columns = [
            "SeaStateRAODirection",
            "SeaStateRAOPeriodOrFrequency",
            "SeaStateRAOX",
            "SeaStateRAOY",
            "SeaStateRAOZ",
            "SeaStateRAOPotentialAmp",
            "SeaStateRAOPotentialPhase",
            "SeaStateRAOGradientXAmp",
            "SeaStateRAOGradientXPhase",
            "SeaStateRAOGradientYAmp",
            "SeaStateRAOGradientYPhase",
            "SeaStateRAOGradientZAmp",
            "SeaStateRAOGradientZPhase",
        ]
        ...
        filter_SeaStateRAOs_data = filter_SeaStateRAOs_df.to_dict("tight")["data"]
    ...
    def get_shape_data(self, file_name):
        ...
        rao_visual_grid_shape = self.cfg["rao_plot"]["seastate"]["filter"]["grid_label"]
        rao_visual_grid_shape_index = shape_names.index(rao_visual_grid_shape)
        shape_data = shapes[rao_visual_grid_shape_index]

        l_grid = (
            self.cfg["rao_plot"]["seastate"]["filter"]["l"]
            + 2 * self.cfg["rao_plot"]["seastate"]["filter"]["beyond_boundary"]
        )
        w_grid = (
            self.cfg["rao_plot"]["seastate"]["filter"]["w"]
            + 2 * self.cfg["rao_plot"]["seastate"]["filter"]["beyond_boundary"]
        )
