class Visualization:
    ...
    def generate_time_line(self, data, plt_settings=None):
        ...
        self.fig, ax = self.plt.subplots(
            figsize=(plt_settings["size"]["length"], plt_settings["size"]["height"])
        )
        ...
        for ii, (idate, iname) in enumerate(data.itertuples()):
            ...
            ax.plot((idate, idate), (0, level), c="r", alpha=0.7)
    ...
    def set_plot_size_orientation(self):
        if (
            self.plt_settings["size"] == "A4"
            and self.plt_settings["orientation"] == "landscape"
        ):
            self.fig.set_size_inches(11.69, 8.27)
        elif (
            self.plt_settings["size"] == "A4"
            and self.plt_settings["orientation"] == "portrait"
        ):
            self.fig.set_size_inches(8.27, 11.69)
        elif (
            self.plt_settings["size"] == "A3"
            and self.plt_settings["orientation"] == "landscape"
        ):
            ...
            self.plt_settings["size"] == "half_letter"
            and self.plt_settings["orientation"] == "landscape"
        ):
    ...
    def add_title_and_axis_labels(self, plt_settings=None):
        ...
        if self.plt_settings.__contains__("suptitle"):
            self.plot_object.suptitle(
                self.plt_settings["suptitle"],
                fontsize=12,
                fontweight="bold",
                color="black",
            )
        ...
        if self.plt_settings.__contains__("title"):
            if self.cfg_mult is None:
                self.plot_object.title(
                    self.plt_settings["title"],
                    fontsize=11,
                    fontweight="bold",
                    color="black",
                )
            else:
                self.plot_object.set_title(
                    self.plt_settings["title"], fontsize=10, color="black"
                )
        ...
        if self.cfg_mult is None:
            self.plot_object.xlabel(
                self.plt_settings["xlabel"],
                fontsize=8,
                fontweight="bold",
                color="black",
            )
            self.plot_object.ylabel(
                self.plt_settings["ylabel"],
                fontsize=8,
                fontweight="bold",
                color="black",
            )
        else:
            self.plot_object.set(
                xlabel=self.plt_settings["xlabel"], ylabel=self.plt_settings["ylabel"]
            )
    ...
    def add_text_fields(self):
        if self.plt_settings.__contains__("text_fields"):
            for text_fields_index in range(0, len(self.plt_settings["text_fields"])):
                ...
                if text_field.__contains__("color") and (
                    text_field["color"] is not None
                ):
    ...
    def multiple_plots(self, cfg_mult):
        ...
        self.fig, self.ax = self.plt.subplots(
            nrows=self.nrows, ncols=self.ncols, sharex=True, figsize=(8, self.nrows * 4)
        )
    ...
    def plotCustomFatigue(self, DF, data):
        ...
        for TextFieldsIndex in range(0, len(data["TextFields"])):
            plt.text(
                data["TextFields"][TextFieldsIndex]["x"],
        ...
        for fileIndex in range(0, len(DFArray)):
            ...
            for SNCurveIndex in range(0, len(UniqueSNCurves)):
                plt.plot(
                    ...
                    label=cfg["FatigueLifeReadingSets"][fileIndex]["Label"][
                        SNCurveIndex
                    ],
                    linestyle=data["LineStyles"][SNCurveIndex],
                    color=data["Colors"][fileIndex],
                )
