class ETL_components:
    ...
    def from_xlsx_to_yaml_file(self):
        ...
        for file_index in range(0, len(self.cfg["files"]["from_xlsx"])):
            ...
            for replace_index in range(
                0, len(self.cfg["files"]["from_xlsx"][file_index]["replace"])
            ):
                column = self.cfg["files"]["from_xlsx"][file_index]["replace"][
                    replace_index
                ]["column"]
                to_replace = self.cfg["files"]["from_xlsx"][file_index]["replace"][
                    replace_index
                ]["to_replace"]
                replace_with = self.cfg["files"]["from_xlsx"][file_index]["replace"][
                    replace_index
                ]["replace_with"]
                df_array[file_index][column] = df_array[file_index][column].replace(
                    to_replace=to_replace, value=replace_with, regex=True
                )
            ...
            for row_index in range(0, len(df_array[file_index])):
                ...
                row_items.update(
                    {
                        "Label": self.cfg["files"]["from_xlsx"][file_index]["label"]
                        + "_"
                        + df_array[file_index].loc[row_index, "// Wave Name"]
                        + "_"
            ...
            print(
                "File saved successfully: {0}".format(
                    self.cfg["Analysis"]["result_folder"]
                    + self.cfg["Analysis"]["file_name"]
                    + "_"
                    + self.cfg["files"]["from_xlsx"][file_index]["label"]
    ...
    def write_repeat_pattern_files(self):
        ...
        for file_index in range(0, no_of_output_files):
            ...
            if self.cfg["output_files"][self.file_index].__contains__(
                "repeat_patterns"
            ):
    ...
    def write_repeat_pattern_file(self):
        ...
        file_name_extension = self.cfg["output_files"][self.file_index]["settings"][
            "file_name"
        ]["extension"]

        for repeat_pattern_block_index in range(
            0, self.number_of_repeat_pattern_blocks
        ):
            repeat_pattern_block_file_contents = (
                self.get_repeat_pattern_block_file_contents(
                    all_line_field_lists, repeat_pattern_block_index
                )
            )
            ...
            output_file_multiple = (
                output_file_multiple + repeat_pattern_block_file_contents
            )
            ...
            if self.cfg["output_files"][self.file_index]["settings"]["multiple_file"]:
                ...
                save_data.save_ascii_file_from_array(
                    output_file_multiple, file_name, extension="." + file_name_extension
                )
        ...
        if self.cfg["output_files"][self.file_index]["settings"]["single_file"]:
            file_name_suffix = self.cfg["output_files"][self.file_index]["settings"][
                "file_name"
            ]["suffix"]
            file_name = (
                self.cfg["Analysis"]["result_folder"]
            ...
            save_data.save_ascii_file_from_array(
                output_file_single, file_name, extension="." + file_name_extension
            )
            print(
                "Successfully saved {0}{1}{2}".format(
                    file_name, ".", file_name_extension
                )
            )

    def get_repeat_pattern_block_file_contents(
        self, all_line_field_lists, repeat_pattern_block_index
    ):
        repeat_pattern_block_file_contents = []
        repeat_pattern_block = self.cfg["output_files"][self.file_index][
            "repeat_patterns"
        ]
        for repeat_pattern_index in range(0, len(repeat_pattern_block)):
            ...
            if line_fields == None:
                ...
                number_of_fields = len(
                    repeat_pattern_block[repeat_pattern_index]["fields"]
                )
                ...
                if number_of_fields == 1:
                    line_field_index = 0
                    line_field1_value = all_line_field_lists[repeat_pattern_index][
                        line_field_index
                    ][repeat_pattern_block_index]
                    repeat_pattern_block_file_contents.append(
                        line_text.format(line_field1_value)
                    )
                    ...
                    line_field2_value = all_line_field_lists[repeat_pattern_index][
                        line_field_index + 1
                    ][repeat_pattern_block_index]
                    repeat_pattern_block_file_contents.append(
                        line_text.format(line_field1_value, line_field2_value)
                    )
                    ...
                    line_field3_value = all_line_field_lists[repeat_pattern_index][
                        line_field_index + 2
                    ][repeat_pattern_block_index]
                    repeat_pattern_block_file_contents.append(
                        line_text.format(
                            line_field1_value, line_field2_value, line_field3_value
                        )
                    )
            ...
            if line_fields != None:
                ...
                for field_index in range(0, len(line_fields)):
                    ...
                    if line_fields[field_index].__contains__("from_xlsx"):
                        ...
                        if line_fields[field_index]["from_xlsx"][0].__contains__(
                            "scale"
                        ):
                    ...
                    if self.number_of_repeat_pattern_blocks == None:
                        ...
                        self.number_of_repeat_pattern_blocks = min(
                            self.number_of_repeat_pattern_blocks, len(field_list)
                        )
    ...
    def check_line_field_count(self, line_index, line_text):
        number_of_fields = len(
            self.cfg["output_files"][self.file_index]["repeat_patterns"][line_index][
                "fields"
            ]
        )
    ...
    def write_replace_files(self):
        ...
        if self.cfg["output_files"][self.file_index]["settings"]["multiple_file"]:
            cfg_field_lists = [
                self.cfg["output_files"][self.file_index]["settings"]["file_name"][
                    "suffix"
                ]
            ]
            file_name_suffix_fields = self.generic_get_all_line_field_data(
                cfg_field_lists
            )
            file_name_extension = self.cfg["output_files"][self.file_index]["settings"][
                "file_name"
            ...
            if cfg_field_lists[0]["fields"] != None:
                for field_index in range(0, len(file_name_suffix_fields[0][0])):
                    file_name_suffix = self.cfg["output_files"][self.file_index][
                        "settings"
                    ...
                    file_name_suffix = file_name_suffix.format(
                        file_name_suffix_fields[0][0][field_index]
                    )
                    if self.cfg["Analysis"]["custom_folder"] is not None:
                        file_name = os.path.join(
                            self.cfg["Analysis"]["custom_folder"], file_name_suffix
                        )
                    else:
                        file_name = (
                            self.cfg["Analysis"]["result_folder"]
                    ...
                    self.field_data_array = self.cfg["output_files"][self.file_index][
                        "replace"
                    ]
                    # Physically update the current profile id here.
                    # TODO perform this operation from .yml in long run
                    self.field_data_array[2]["fields"][0]["from_xlsx"][0]["filter"][2][
                        "value"
                    ] = file_name_suffix_fields[0][0][field_index]
                    self.field_data_array[2]["fields"][1]["from_xlsx"][0]["filter"][2][
                        "value"
                    ] = file_name_suffix_fields[0][0][field_index]

                    all_line_field_lists = self.generic_get_all_line_field_data(
                        self.field_data_array
                    )
                    all_line_field_lists = self.transform_shear7_data(
                        all_line_field_lists
                    )
                    result_file_array = self.replace_file_contents(all_line_field_lists)

                    self.write_replace_file(
                        result_file_array, file_name, file_name_extension
                    )
    ...
    def write_replace_file(self, result_file_array, file_name, file_name_extension):
        ...
        save_data = SaveData()
        save_data.save_ascii_file_from_array(
            result_file_array, file_name, extension="." + file_name_extension
        )
        print(
            "Successfully saved {0}{1}{2}".format(file_name, ".", file_name_extension)
        )
    ...
    def transform_shear7_data(self, all_line_field_lists):
        ...
        for line_index in range(0, len(self.field_data_array)):
            if self.field_data_array[line_index]["fields"] != None:
                for line_field_index in range(
                    0, len(self.field_data_array[line_index]["fields"])
                ):
                    if self.field_data_array[line_index]["fields"][
                        line_field_index
                    ].__contains__("from_xlsx"):
                        if self.field_data_array[line_index]["fields"][
                            line_field_index
                        ]["from_xlsx"][0].__contains__("transform"):
                            if self.field_data_array[line_index]["fields"][
                                line_field_index
                            ]["from_xlsx"][0]["transform"]:
                                water_depth = self.model_state_information[
                                    "water_depth"
                                ]
                                depth_below_msl = self.model_state_information[
                                    "depth_below_msl"
                                ]
                                shear7_x_by_L = self.model_state_information[
                                    "shear7_x_by_L"
                                ]
                                raw_depth = list(
                                    all_line_field_lists[line_index][line_field_index]
                                )
                                raw_current = list(
                                    all_line_field_lists[line_index][
                                        line_field_index + 1
                                    ]
                                )
                                modified_depth = []
                                ...
                                (
                                    modified_current,
                                    modified_depth,
                                ) = self.implement_current_shielding_effects(
                                    modified_current, modified_depth
                                )

                                transformed_data_depth = [
                                    np.interp(item, depth_below_msl, shear7_x_by_L)
                                    for item in modified_depth
                                ]
                                all_line_field_lists[line_index][
                                    line_field_index
                                ] = transformed_data_depth
                                ...
                                all_line_field_lists[line_index - 1].append(
                                    [len(modified_current)]
                                )

                    if self.field_data_array[line_index]["fields"][
                        line_field_index
                    ].__contains__("from_ascii"):
                        if self.field_data_array[line_index]["fields"][
                            line_field_index
                        ]["from_ascii"].__contains__("transform"):
                            all_line_field_lists[line_index][line_field_index][0] = (
                                all_line_field_lists[line_index][line_field_index][0]
                                - 1
                            )
            ...
            for shielding_index in range(0, len(shielding_info)):
                ...
                for depth_index in range(0, end_depth_index_for_logic):
                    ...
                    if depth >= shielding_info[0][0] and depth <= shielding_info[0][1]:
                        ...
                        if depth_index > 0 and (
                            depth_index < end_depth_index_for_logic - 1
                        ):
                            if modified_depth[depth_index - 1] > shielding_info[0][1]:
                                ...
                                modified_depth.insert(
                                    depth_index + 1, shielding_info[0][1] - 0.1
                                )
                                modified_current.insert(
                                    depth_index + 1,
                                    modified_current[depth_index]
                                    * self.model_state_information["shielding_factor"],
                                )
                            if modified_depth[depth_index + 1] < shielding_info[0][0]:
                                modified_depth.insert(
                                    depth_index + 1, shielding_info[0][0]
                                )
                                modified_current.insert(
                                    depth_index + 1,
                                    modified_current[depth_index + 1]
                                    * self.model_state_information["shielding_factor"],
                                )
                                modified_depth.insert(
                                    depth_index + 2, shielding_info[0][0] + 0.1
                                )
                                modified_current.insert(
                                    depth_index + 2, modified_current[depth_index + 2]
                                )
                                break
    ...
    def replace_file_contents(self, all_line_field_lists):
        ...
        result_file_array = self.replace_lines(
            self.base_file_content_array, replace_line_set_array, all_line_field_lists
        )
    ...
    def get_line_numbers_to_replace(self):
        ...
        for replace_line_set_index in range(0, len(replace_block)):
            start_line_contain_key_text = replace_block[replace_line_set_index][
                "lines"
            ]["from"]["containing_text"]
            start_line_number = self.get_array_index_containing_key_text(
                self.base_file_content_array, start_line_contain_key_text
            )
            end_line_number = None
            if (
                replace_block[replace_line_set_index]["lines"].__contains__("to")
                and replace_block[replace_line_set_index]["lines"]["to"] != None
            ):
                end_line_contain_key_text = replace_block[replace_line_set_index][
                    "lines"
                ]["to"]["containing_text"]
                end_line_number = self.get_array_index_containing_key_text(
                    self.base_file_content_array, end_line_contain_key_text
                )
            replace_line_set_array.append(
                {
    ...
    def replace_lines(self, line_array, replace_line_set_array, all_line_field_lists):
        ...
        for line_array_index in range(0, len(line_array)):
            if (
                replace_line_set_array[replace_line_set_array_index][
                    "start_line_number"
                ]
                == line_array_index
            ):
                ...
                if not line_field_lists:
                    result_array.append(
                        replace_block[replace_line_set_array_index]["template_line"]
                    )
                else:
                    for value_index in range(0, len(line_field_lists[0])):
                        if len(line_field_lists) == 1:
                            value = all_line_field_lists[replace_line_set_array_index][
                                0
                            ][value_index]
                            result_array.append(
                                replace_block[replace_line_set_array_index][
                                    "template_line"
                                ].format(value)
                            )
    ...
    def get_ascii_file_content_as_array(self):
        cfg_temp = {
            "io": self.cfg["output_files"][self.file_index]["settings"]["basefile"]
        }
    ...
    def get_model_state_information(self):
        ...
        cfg_temp = self.cfg["output_files"][self.file_index]["settings"][
            ...
            if line_fields != None:
                for field_index in range(0, len(line_fields)):
                    ...
                    if line_fields[field_index].__contains__("from_xlsx"):
                        ...
                        start_line = line_fields[field_index]["from_ascii"]["line"][
                            "from"
                        ]["number"]
                        if line_fields[field_index]["from_ascii"]["line"].__contains__(
                            "to"
                        ):
                            end_line = line_fields[field_index]["from_ascii"]["line"][
                                "to"
                            ]["number"]
                        else:
                            end_line = None
        ...
        for data_category_index in range(0, len(extract_data)):
            if extract_data[data_category_index].__contains__("from_xlsx"):
                for df_index in range(
                    0, len(extract_data[data_category_index]["from_xlsx"])
                ):
                    df = read_data.from_xlsx(
                        {"files": extract_data[data_category_index]}, df_index
                    )
    ...
    def transform_viv_current_reformat_1(self):
        ...
        for df_index in range(0, len(self.df_array)):
            depths = [
                self.df_array[df_index].columns[col_index]
                for col_index in range(3, len(self.df_array[df_index].columns))
            ]
            df_columns = self.df_array[df_index].columns[0:3].to_list() + [
                "depth_" + str(depth_index - 2)
                for depth_index in range(3, len(self.df_array[df_index].columns))
            ]
            ...
            for row_index in range(0, len(self.df_array[df_index])):
                for depth_index in range(0, len(depths)):
                    location_id = self.cfg.default["transform"][
                        "viv_current_reformat_1"
                    ]["location_id"]
                    loading_type = self.cfg.default["transform"][
                        "viv_current_reformat_1"
                    ]["loading_type"]
                    Type = self.cfg.default["transform"]["viv_current_reformat_1"][
                        "Type"
                    ]
                    current_profile_id = self.df_array[df_index].iloc[row_index]["No."]
                    depth = (
                        depths[depth_index]
                        * self.cfg.default["transform"]["viv_current_reformat_1"][
                            "scale"
                        ]["depth"]
                    )
                    current_speed = (
                        self.df_array[df_index].iloc[row_index][
                            "depth_" + str(depth_index + 1)
                        ]
                        * self.cfg.default["transform"]["viv_current_reformat_1"][
                            "scale"
                        ]["current_speed"]
                    )
                    direction = self.cfg.default["transform"]["viv_current_reformat_1"][
                        "direction"
                    ]
                    return_period = self.cfg.default["transform"][
                        "viv_current_reformat_1"
                    ]["return_period"]
                    probability = self.df_array[df_index].iloc[row_index]["%"]
                    Label = self.cfg.default["extract"][0]["from_xlsx"][df_index][
                        "label"
                    ]
    ...
    def save(self):
        ...
        cfg_temp = {
            ...
            "SheetNames": [
                df_data["label"]
                for df_data in self.cfg.default["extract"][0]["from_xlsx"]
            ],
        }
