...
class Shear7ModelComponents:
    ...
    def title(self):
        ...
        depth_below_msl = [
            -self.riser_model.stack_up_properties_df[
                self.riser_model.stack_up_properties_df["component"] == "MSL"
            ]["elevation_above_MSL"].values[0],
            -self.riser_model.stack_up_properties_df[
                self.riser_model.stack_up_properties_df["component"] == "Mudline"
            ]["elevation_above_MSL"].values[0],
        ]
        shear7_x_by_L = [
            self.riser_model.stack_up_properties_df[
                self.riser_model.stack_up_properties_df["component"] == "MSL"
            ]["shear7_bottom_x_by_L"].values[0],
            self.riser_model.stack_up_properties_df[
                self.riser_model.stack_up_properties_df["component"] == "Mudline"
            ]["shear7_bottom_x_by_L"].values[0],
        ]
        ...
        self.shear7_model.append(
            "Shear7 v4.6c {0}, {1} riser model".format(
                self.riser_model.cfg["Analysis"]["file_name"],
    ...
    def riser_joints_to_shear7_zones(self):
        ...
        for riser_joint_index in range(0, len(self.riser_model.stack_up_properties_df)):
            if (
                self.riser_model.stack_up_properties_df.iloc[riser_joint_index][
                    "stack_length"
                ]
                != 0
            ):
                ...
                zone_properties.append(
                    "{:.5f}       {:.5f}       zone 1   start and end point in x/L, {}".format(
                        self.riser_model.stack_up_properties_df.iloc[riser_joint_index][
                            "shear7_top_x_by_L"
                        ],
                        self.riser_model.stack_up_properties_df.iloc[riser_joint_index][
                            "shear7_bottom_x_by_L"
                        ],
                        self.riser_model.stack_up_properties_df.iloc[riser_joint_index][
                            "component"
                        ],
                ...
                zone_properties.append(
                    "{:.2E}	{:.2f}	{:.2f}			inertia(m^4), mass in air+fluids (kg/m), sub wt. (N/m)".format(
                        ...
                        (
                            self.riser_model.stack_up_properties_df.iloc[
                                riser_joint_index
                ...
                zone_properties.append(
                    "{:.2E}	1.0				Youngs Modulus, SN Curve Number".format(
                        self.riser_model.cfg["Material"]["Steel"]["E"]
                        * 0.4536
                        * 9.81
                        / (0.0254) ** 2
                    )
                ...
                zone_properties.append(
                    "1.00	0.20	0.18	0.20		Ca, DampCoeff1, DampCoef2, DampCoef3"
                )
    ...
    def add_block_2(self):
        ...
        self.shear7_model.append(
            "{:.2f}		model length".format(self.riser_model.total_riser_length)
        )
        ...
        self.shear7_model.append(
            "{0}		no. of zones to define sectional property".format(
    ...
    def add_block_3(self):
        ...
        self.shear7_model.append(
            "{0}  1.0   1      no. of profile data pts, probability, profile ID"
        )
    ...
    def add_block_4(self):
        ...
        self.shear7_model.append(
            "1                                   No of SN Curves Defined - DNV '84 B Seawater CP"
        )
        self.shear7_model.append(
            "1           1                       SN Curve ID No, No. of S-N curve segments"
        )
        self.shear7_model.append(
            "0                                   cut-off stress range (N/m**2), endurance limit"
        )
        self.shear7_model.append(
            "1.000E+06  6.3154E+11               stress range (N/m**2) cycles to failure"
        )
        self.shear7_model.append(
            "1.000E+09  6.3154E+02               stress range (N/m**2) cycles to failure"
        )
        self.shear7_model.append(
            "1.3                                 Global stress concentration factor"
        )
        self.shear7_model.append(
            "0                                  No. of local stress concentration positions"
        )
    ...
    def add_block_5(self):
        ...
        self.shear7_model.append(
            "0.05        0.3                     power cutoff, primary zone amplitude limit "
        )
        self.shear7_model.append(
        ...
        self.shear7_model.append(
            "0                                   flag for writing the *.anm file (0=no, 1=yes)"
        )
        self.shear7_model.append(
            "1                                   flag for generating *.scr file, (0=no; 1=yes)"
        )
        self.shear7_model.append(
            "1                                   flag for generating *.dmg file, (0=no; 1=yes)"
        )
        self.shear7_model.append(
            "0                                   flag for generating *.fat file, (0=no; 1=yes)"
        )
        self.shear7_model.append(
    ...
    def save_model(self):
        save_data = SaveData()
        file_name = (
            self.riser_model.cfg["Analysis"]["fe_folder"]
            + self.riser_model.cfg["Analysis"]["file_name"]
            + "_shear7"
        )
