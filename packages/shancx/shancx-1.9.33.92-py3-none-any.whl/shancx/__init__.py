...
def Mul_th(func, iterable, workers=2):
...
def Mul_thPlus(func, iterable, workers=2, timeout=None):
    with ThreadPoolExecutor(workers) as executor:
        futures = [executor.submit(func, item) for item in iterable]
        results = []
        for future in tqdm(as_completed(futures, timeout=timeout), total=len(futures), desc="Processing"):
            try:
                results.append(future.result())
            except Exception as e: 
                print(f"Error: {e}")
...
#task_args = [[chid,utcyear, utcmonth,latArr,lonArr]  
#                for chid in allcchannels]  
# results = Mul_thPlus(func, task_args)
...
with ThreadPoolExecutor(4) as executor:
    results = list(executor.map(self.process_goes_data, task_args))    
