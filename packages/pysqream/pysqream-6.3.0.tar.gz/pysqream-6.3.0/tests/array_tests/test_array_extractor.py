...
from tests.utils import ensure_empty_table, select
...
def cursor_with_arrays_allowed(sqream_cursor):
    ...
    sqream_cursor._Cursor__allow_array = True
    yield sqream_cursor
...
def tuple_to_pandas_timestamp(values):
    """Utility to convert tuple of values to pandas timestamp"""
    if values is None:
        return None

    year, month, day = values[0], values[1], values[2]
    hour, minute = values[3] if len(values) > 3 else 0, values[4] if len(values) > 4 else 0
    second = values[5] if len(values) > 5 else 0
    microsecond = values[6] if len(values) > 6 else 0

    return pd.Timestamp(f"{year:04d}-{month:02d}-{day:02d} {hour:02d}:{minute:02d}:{second:02d}.{microsecond:06d} +00:00")
...
DATATYPES_DATA = [
    ...
    ["DATETIME", ["'1955-11-05 01:24:00.000', '9999-12-31 23:59:59.999000', null"]],
    ["DATETIME2", ["'1955-11-05 01:24:00.123456789 +00:00', '9999-12-31 23:59:59.999000 +00:00', null"]],
]
...
DATA = [
    ...
    [([pd.Timestamp('1955-11-05 01:24:00.123456789+00:00'),
       pd.Timestamp('9999-12-31 23:59:59.999000 +00:00'),
       None],)],
]
...
DATETIME2_GTE8 = [
    [tuple_to_pandas_timestamp(vals) for vals in
     [(1955, 11, 5, 1, 24), None, (9999, 12, 31, 23, 59, 59, 999000), None,
      (5245, 8, 19, 7, 13, 16, 250000), (2020, 5, 22, 15, 21, 51, 772000),
      None, (1998, 1, 1)]],
    [tuple_to_pandas_timestamp(vals) for vals in
...
DATATYPES_DATA_GTE8 = [
    ...
    ["DATETIME",
     ...
     [','.join([f"'{d.strftime('%F %H:%M:%S')}.{d.microsecond*1000:03.0f}'"
      ...
      for r in DATETIME_GTE8]],
    ["DATETIME2",
     [','.join([f"'{str(d)}'".replace('+', ' +') if d is not None else "null" for d in r])
      for r in DATETIME2_GTE8]]
]
