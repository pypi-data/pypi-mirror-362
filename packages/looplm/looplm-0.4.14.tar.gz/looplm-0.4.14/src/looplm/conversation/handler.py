...
class ConversationHandler:
    ...
    def __init__(
        self, console: Optional[Console] = None, debug: bool = False, raw: bool = False
    ):
    ...
    def _stream_markdown(self, content: str, live: Live) -> None:
        """Update live display with markdown-formatted content or raw text based on raw flag"""
        try:
            if self.raw:
    ...
    def _handle_llm_interaction(
        ...
        # Handle tool calls if any - filter out incomplete ones first
        complete_tool_calls = _filter_complete_tool_calls(tool_calls)
        if complete_tool_calls and self.tool_manager:
            self._handle_tool_calls(
                model, messages, complete_tool_calls, tools, accumulated_text
            )
