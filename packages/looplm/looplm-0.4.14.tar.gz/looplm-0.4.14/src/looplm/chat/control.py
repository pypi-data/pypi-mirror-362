...
class CommandHandler:
    ...
    def __init__(
        ...
        tools_approval: bool = True,
        system_prompt: Optional[str] = None,
        system_prompt_file: Optional[str] = None,
        raw: bool = False,
    ):
        """Initialize command handler"""
        self.console = ChatConsole(raw=raw)
    ...
    def _handle_new(self) -> bool:
        ...
        # Set system prompt (custom or default)
        if self.system_prompt or self.system_prompt_file:
            try:
                from ..utils.prompts import PromptsManager
                ...
                session.set_system_prompt(resolved_system_prompt)
                self.console.display_info("Using custom system prompt")
            except Exception as e:
                self.console.display_error(f"Error with system prompt: {str(e)}")
    ...
    def start_session(self, initial_message: Optional[str] = None):
        ...
        if initial_message and self.session_manager.active_session:
            try:
                # Display the user's initial message in the same format as prompt_user
                from datetime import datetime

                timestamp = datetime.now()
                time_str = timestamp.strftime("%H:%M")
                # Use the same color as PromptManager: #0000ff bold (blue bold)
                self.console.console.print(
                    f"\n[#666666]{time_str}[/#666666] [#0000ff bold]User ►[/#0000ff bold] {initial_message}"
                )
                # Then send it to the LLM
                self.session_manager.active_session.send_message(
                    initial_message, stream=True, show_tokens=False, debug=self.debug
                )
            except Exception as e:
                self.console.display_error(f"Error sending initial message: {str(e)}")
