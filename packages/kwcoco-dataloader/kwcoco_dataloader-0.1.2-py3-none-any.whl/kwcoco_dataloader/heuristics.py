...
Column Info:
    ...
    * kwcoco_catname:
        ...
        defined here. This is used in kwcoco_dataloader reproject.
...
.. code:: python
    ...
    from kwcoco_dataloader.heuristics import *  # NOQA
    import pandas as pd
    import rich
...
def ensure_heuristic_category_tree_colors(classes, force=False):
    ...
    TODO:
        ...
        - [ ] Consolidate with ~/code/watch/kwcoco_dataloader/heuristics.py :: category_tree_ensure_color
        - [ ] Consolidate with ~/code/watch/kwcoco_dataloader/heuristics.py :: category_category_colors
        - [ ] Consolidate with ~/code/watch/kwcoco_dataloader/heuristics.py :: ensure_heuristic_category_tree_colors
        - [ ] Consolidate with ~/code/watch/kwcoco_dataloader/heuristics.py :: ensure_heuristic_coco_colors
...
# NOTE:
# QA Information Moved to ~/code/watch/kwcoco_dataloader/tasks/fusion/datamodules/qa_bands.py
...
def auto_expt_dvc():
    ...
    return kwcoco_dataloader.find_dvc_dpath(**DVC_FIND_EXPT_KWARGS)


def auto_data_dvc():
    import kwcoco_dataloader
    return kwcoco_dataloader.find_dvc_dpath(**DVC_FIND_DATA_KWARGS)
...
def normalize_sensors(coco_dset, sensor_warnings=True, format='te'):
    ...
    """
    from kwcoco_dataloader.heuristics import TE_SENSOR_NAMES
...
def register_known_fsspec_s3_buckets():
    ...
    Ignore:
        ...
        from kwcoco_dataloader.utils.util_fsspec import S3Path
        self = S3Path.coerce('/vsis3/usgs-landsat-ard/collection02')
        self.ls()
    """
    ...
    # This should not be part of this standalone module.
