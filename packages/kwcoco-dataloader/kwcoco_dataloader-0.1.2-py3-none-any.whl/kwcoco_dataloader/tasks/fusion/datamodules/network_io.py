...
class DictContainer(dict):
    def to(self, device):
        ...
        Example:
            >>> # xdoctest: +SKIP('requires gpu')
            >>> from kwcoco_dataloader.tasks.fusion.datamodules.network_io import *  # NOQA
            >>> self = HeterogeneousBatchItem.demo()
            >>> device = torch.device('cuda:0')
            >>> self.to(device)
        """
        import torch
        walker = IndexableWalkerWithParent(self)
        for parent, key, path, value in walker:
            if isinstance(value, torch.Tensor):
                parent[key] = value.to(device)
        return self
...
class HeterogeneousBatchItem(BatchItem):
    ...
    def is_homogeneous(self):
        """
        Check if the item is actually homogeneous
        """
        frame_modes = []
        for frame in self['frames']:
            frame_modeshapes = []
            for mode_key, mode_value in frame['modes'].items():
                frame_modeshapes.append((mode_key, tuple(mode_value.shape)))
            frame_modes.append(frame_modeshapes)
        flag = ub.allsame(frame_modes)
        return flag
    ...
    def iter_modes(self):
        """
        Iterate over all frames and modalities
        """
        from typing import NamedTuple
        class ModeKey(NamedTuple):
            frame_index: int
            sensor: str | None
            chans: str

        for frame_index, frame in enumerate(self['frames']):
            sensor = frame.get('sensor', None)
            for mode_key, mode_data in frame['modes'].items():
                key = ModeKey(frame_index, sensor, mode_key)
                yield key, mode_data
    ...
    def demo(cls):
        ...
        coco_dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader-msi', num_frames=10)
    ...
    def draw(self, item_output=None, combinable_extra=None, max_channels=5,
             ...
             requested_tasks=None, legend=True,
             default_combinable_channels='auto', **kwargs):
        """
        ...
        if default_combinable_channels == 'auto':
        ...
        requested_tasks = item.get('requested_tasks', None)
        predictable_classes = item.get('predictable_classes', None)
        ...
        if legend:
            ...
            if predictable_classes is not None:
                label_to_color = {
                    node: data['color']
                    for node, data in predictable_classes.graph.nodes.items()}
    ...
    def summarize(self, coco_dset=None, stats=False):
        ...
        for frame in item['frames']:
            ...
            if frame.get('date_captured', None):
...
class HomogeneousBatchItem(HeterogeneousBatchItem):
    ...
    def demo(cls):
        ...
        coco_dset = kwcoco_dataloader.coerce_kwcoco(
            'vidshapes1', num_frames=10, anchors=np.array([[0.1, 0.1]]),
            num_tracks=40,
        )
        dataset = kwcoco_dataset.KWCocoVideoDataset(
        ...
        if 1:
            # Try to find a sample that has a different number of annotations
            # in each frame.
            found = None
            for idx in dataset.sample_grid['positives_indexes']:
                item = dataset.sample_grid['targets'][idx]
                annots_per_img = list(map(len, item['annot_info']['gid_to_aids'].values()))
                if not ub.allsame(annots_per_img):
                    found = idx
            if found is None:
                # fallback onto something positive, shouldn't happen with the
                # given demo args.
                print('Warning: demo batch is too simple')
                found = dataset.sample_grid['positives_indexes'][0]
            index = found
        else:
            index = dataset.sample_grid['targets'][dataset.sample_grid['positives_indexes'][0]]
    ...
    def imdata_tchw(self):
        ...
        for frame in self['frames']:
            ...
            tostack.append(mode_val)
        ret = torch.stack(tostack, dim=0)
        return ret

    def _frame_collated(self):
        """
        For a homogeneous batch, sizes in each frame should match so we can
        stack the per-frame information along a time axis.

        TODO:
            - [ ] a homogeneous item should likely just store the data in the
                frame-collated form when possible, so this would be the
                conversion from heterogeneous to homogeneous structure.

        Example:
            >>> cls = HomogeneousBatchItem
            >>> self = cls.demo()
            >>> item = self._frame_collated()
        """
        import torch
        import numbers
        ...
        # Special case to denote dataloader keys that are not collatable.
        NON_COLLATABLE_KEYS = {
            'ann_aids',
            'box_ltrb',
            'box_cidxs',
        ...
        # Special cases for keys that represent slices
        SLICE_KEYS = {
            'output_space_slice',
        }
        ...
        for frame in self['frames']:
            ...
            noncollatable_frame = {}
            collatable_frame = {
                # mode val is chw, but we will stack it to include t-dim.
                'imdata_tchw': mode_val,
            }
            for k, v in frame.items():
                if k in SLICE_KEYS:
                    v = [(s.start, s.stop, s.step or 1) for s in v]
                    v = np.array(v)
                    v = torch.Tensor(v)
                if k in NON_COLLATABLE_KEYS:
                    noncollatable_frame[k] = v
                elif k in IGNORE_KEYS:
                    ...
                elif isinstance(v, COLLATABLE_TYPES):
                    collatable_frame[k] = v
                else:
                    noncollatable_frame[k] = v

            collatable.append(collatable_frame)
            non_collatable.append(noncollatable_frame)

        meta = ub.udict.difference(self, {'frames'}, cls=dict)

        assert mode_key is not None
        collated = torch_data.dataloader.default_collate(collatable)
        meta['mode'] = mode_key

        # Repack everything into a new structure for flexibility
        item = {
            'collated': collated,
            'non_collatable': non_collatable,
            'meta': meta,
        }
        return item
...
def _demo_dataset():
    ...
    from kwcoco_dataloader.tasks.fusion.datamodules import kwcoco_dataset
    import kwcoco_dataloader
    coco_dset = kwcoco_dataloader.coerce_kwcoco('vidshapes1', num_frames=1)
    dataset = kwcoco_dataset.KWCocoVideoDataset(
...
class UncollatedBatch(ListContainer):
    """
...
class CollatedBatch(DictContainer):
...
class NetworkOutputs(dict):
    ...
    def _debug_shape(self):
        # from kwcoco_dataloader.utils.util_netharn import _debug_inbatch_shapes
...
def decollate(collated):
    ...
    B = next(iter(collated.values()))
    if isinstance(B, torch.Tensor):
        B = B.shape[0]  # Extract batch size from tensor
    elif isinstance(B, Sequence) and not isinstance(B, (str, bytes)):
        B = len(B)  # Extract batch size if first value is a list-like structure
    else:
        raise ValueError("Collated data must contain tensors or sequences with a batch dimension.")

    # Initialize output list of empty dictionaries for each batch index
    uncollated = [{} for _ in range(B)]

    # Use a stack (deque) for iterative traversal instead of recursion
    stack = deque([(collated, uncollated)])

    while stack:
        current_collated, current_uncollated = stack.pop()

        for key, value in current_collated.items():
            if isinstance(value, torch.Tensor):
                # Directly split tensors across batch indices
                for bx in range(B):
                    current_uncollated[bx][key] = value[bx]
            elif isinstance(value, Mapping):
                # Ensure nested dictionaries exist in uncollated
                for bx in range(B):
                    current_uncollated[bx][key] = {}
                stack.append((value, [uncollated[bx][key] for bx in range(B)]))
            elif isinstance(value, Sequence) and not isinstance(value, (str, bytes)):
                # Handle lists of batched elements
                for bx in range(B):
                    current_uncollated[bx][key] = value[bx] if bx < len(value) else None
            else:
                # Directly copy scalars or unindexable elements
                for bx in range(B):
                    current_uncollated[bx][key] = value
    return uncollated


class IndexableWalkerWithParent(ub.IndexableWalker):
    """
    Extension of indexable walker to make it more efficient to do inplace
    modifications in containers.
    """
    ...
    def _walk(self, data=None, prefix=[]):
        ...
        stack = [(data, prefix)]
        while stack:
            _parent, _prefix = stack.pop()
            # Create an items iterable of depending on the indexable data type
            if isinstance(_parent, self.list_cls):
                items = enumerate(_parent)
            elif isinstance(_parent, self.dict_cls):
                items = _parent.items()
            else:
                raise TypeError(type(_parent))

            for key, value in items:
                # Yield the full path to this position and its value
                path = _prefix + [key]
                message = yield _parent, key, path, value
