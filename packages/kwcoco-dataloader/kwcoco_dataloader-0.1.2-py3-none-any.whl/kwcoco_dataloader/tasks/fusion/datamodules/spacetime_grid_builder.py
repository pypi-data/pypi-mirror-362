...
CommandLine:
    ...
    LINE_PROFILE=1 xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.spacetime_grid_builder __doc__:0
...
Example:
    ...
    >>> from kwcoco_dataloader.tasks.fusion.datamodules.spacetime_grid_builder import *  # NOQA
    >>> import ndsampler
    >>> import kwcoco
...
Example:
    ...
    >>> import kwcoco
    >>> dvc_dpath = kwcoco_dataloader.find_dvc_dpath(tags='phase3_data', hardware='ssd')
...
Example:
    ...
    >>> import kwcoco_dataloader
    >>> import kwcoco
    >>> dvc_dpath = kwcoco_dataloader.find_dvc_dpath(tags='phase3_data', hardware='hdd')
...
SPACETIME_CACHE_VERSION = 'spacetime_cache_v21'
...
class SpacetimeGridBuilder:
    ...
    Example:
        ...
        >>> from kwcoco_dataloader.tasks.fusion.datamodules.spacetime_grid_builder import *  # NOQA
        >>> import kwcoco_dataloader
        >>> dvc_dpath = kwcoco_dataloader.find_dvc_dpath(tags='phase2_data', hardware='ssd')
    ...
    def build(builder):
        ...
        Example:
            ...
            >>> assert len(sample_grid2['targets'][0]['gids']) == time_dims
    ...
    def visualize(builder, max_vids=2, max_frames=6, video_ids=None):
        ...
        Example:
            >>> from kwcoco_dataloader.tasks.fusion.datamodules.spacetime_grid_builder import *  # NOQA
            >>> from kwcoco_dataloader.demo.smart_kwcoco_demodata import demo_kwcoco_multisensor
        ...
        Example:
            ...
            >>> dvc_dpath = kwcoco_dataloader.find_dvc_dpath(hardware='ssd', tags='phase2_data')
...
def _build_grid(builder):
    ...
    if window_dims is not None and isinstance(window_dims, tuple) and len(window_dims) == 3:
        ub.schedule_deprecation(
            'kwcoco_dataloader', 'window_dims', 'argument in spacetime_grid_builder',
    ...
    if winspace_time_dims is None:
        if time_kernel is not None:
            from kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling.utils import coerce_multi_time_kernel
    ...
    cache_dpath = ub.Path.appdir('kwcoco_dataloader', 'grid_cache').ensuredir()
    ...
    if SHOW_INFO:
        ...
        from kwcoco_dataloader.utils.util_kwutil import distributed_subitems
...
def _sample_single_video_spacetime_targets(
    ...
    Note this introduces a new temporary space:
        ...
        space.
    """
    from kwcoco_dataloader.tasks.fusion.datamodules import temporal_sampling as tsm  # NOQA
