...
class OriginalBalancedSampleTree(ub.NiceRepr):
    """
    Manages a sampling from a tree of indexes. Helps with balancing
    samples over multiple criteria.

    Example:
        >>> from kwcoco_dataloader.tasks.fusion.datamodules.balanced_sampling import BalancedSampleTree
        ...
        >>> print(f'self={self}')
        >>> sampled = list(ub.take(sample_grid, self.sample_many(100)))
    ...
    def __init__(self, sample_grid, rng=None):
        ...
        OPTIMIZE = 1
        if OPTIMIZE:
            # New faster code path
            self._leaf_nodes = list(range(len(sample_grid)))
            graph.add_nodes_from((index, item) for index, item in zip(self._leaf_nodes, sample_grid))
            graph.add_edges_from((root_node, index) for index in self._leaf_nodes)
            self.graph = graph

            # Prepopulate the first parent/leaf relationship because it is
            # quick to do here.
            self._parent_to_leaf = {root_node: self._leaf_nodes}
        else:
            # Older slower code path
    ...
    def subdivide(self, key, weights=None, default_weight=0):
        ...
        # to read.
        _pred = self.graph.pred
        _nodes = self.graph.nodes
        if self._parent_to_leaf is None:
            # This might be precomputed. If not then we have to compute it.
            _leaf_groupids = [
                next(iter(_pred[n]))
                for n in self._leaf_nodes
            ]
            self._parent_to_leaf = ub.group_items(self._leaf_nodes, _leaf_groupids)

        for parent, children in self._parent_to_leaf.items():
            # Group children by the new attribute
            _child_groupids = [_nodes[n][key] for n in children]
            val_to_subgroup = ub.group_items(children, _child_groupids)
            # try:
        ...
        # The parent-to-leafe structure has changed, mark it as None
        # so it is recomputed if this function is called again
        self._parent_to_leaf = None
...
class BalancedSampleTree:
    """
    AI-Optimized version of BalancedSampleTree that produces the same outputs
    for public methods (constructor, subdivide, sample) without using networkx.

    Still need to evaluate the correctness of this implementation.

    References:
        https://chatgpt.com/c/67effb3f-1930-8002-b9a8-8e31d2409f9b
    """
    def __init__(self, sample_grid, rng=None):
        self.rng = kwarray.ensure_rng(rng)
        ...
        if not isinstance(sample_grid, list):
            raise TypeError(ub.paragraph(
                "BalancedSampleTree only accepts List[Dict], but outer type was {}.".format(type(sample_grid))
            ))
        if not sample_grid:
            raise ValueError('Input sample_grid is empty')
        if not isinstance(sample_grid[0], dict):
            raise TypeError(ub.paragraph(
                "BalancedSampleTree only accepts List[Dict], but inner type was {}.".format(type(sample_grid[0]))
            ))

        self._samples = sample_grid
        self._root = '__root__'
        # _tree maps node ids to a dict with: parent, children, weights, and data.
        # Leaves hold the original sample data; intermediate nodes have data=None.
        self._tree = {}
        # Create root node
        ...
        # Create leaf nodes (indices into sample_grid) as direct children of root.
        self._leaf_nodes = list(range(len(sample_grid)))
        for idx in self._leaf_nodes:
            self._tree[idx] = {
                'parent': self._root,
                'children': [],  # leaf has no children
                'weights': None,
                'data': sample_grid[idx],
            }
            self._tree[self._root]['children'].append(idx)
        # Precompute parent->leaf mapping (for subdivide)
        self._parent_to_leaf = {self._root: list(self._leaf_nodes)}
    ...
    def _reweight(self, node, idx_child):
        """Remove weight for child at idx_child and renormalize."""
        if self._tree[node]['weights'] is not None:
            w = self._tree[node]['weights']
            w = np.delete(w, idx_child)
            if w.sum() != 0:
                w = w / w.sum()
            else:
                w = np.zeros(1)
            self._tree[node]['weights'] = w

    def _remove_subtree(self, node):
        """Recursively remove node and its descendants from the tree."""
        for child in list(self._tree[node]['children']):
            self._remove_subtree(child)
        parent = self._tree[node]['parent']
        if parent is not None and node in self._tree[parent]['children']:
            self._tree[parent]['children'].remove(node)
        if node in self._tree:
            del self._tree[node]

    def _remove_subtree_multiple(self, nodes):
        """Remove multiple nodes (and their subtrees)."""
        for node in nodes:
            if node in self._tree:
                self._remove_subtree(node)

    def _prune_and_reweight(self, removals):
        """
        For each (parent, orphans) scheduled for removal, remove the parent's branch
        and update the parent's parent's weights (recursively, if needed).
        """
        for parent, orphans in removals:
            grandpa = self._get_parent(parent)
            if grandpa is None:
                # No grandparent: remove the entire branch
                self._remove_subtree(parent)
                self._remove_subtree_multiple(orphans)
                continue
            try:
                idx_parent = self._tree[grandpa]['children'].index(parent)
            except ValueError:
                continue
            self._remove_subtree(parent)
            self._remove_subtree_multiple(orphans)
            ...
            while queue:
                curr, idx = queue.pop()
                num_children = len(self._tree[curr]['children'])
                if num_children >= 1:
                    self._reweight(curr, idx)
                else:
                    _parent = curr
                    _grandpa = self._get_parent(curr)
                    if _grandpa is not None:
                        try:
                            _idx = self._tree[_grandpa]['children'].index(_parent)
                        except ValueError:
                            continue
                        queue.append((_grandpa, _idx))
                        self._remove_subtree(_parent)
        # Update _leaf_nodes: only those still in the tree
        self._leaf_nodes = [n for n in self._leaf_nodes if n in self._tree]
        if len(self._leaf_nodes) == 0:
            raise ValueError("Leaf nodes became empty.")

    def subdivide(self, key, weights=None, default_weight=0):
        """
        Adds a new layer to the tree balancing on the given attribute.
        For each current parent (group of leaves), subdivide them by sample[key].
        """
        removals = []  # List of (parent, orphans) to prune
        # Group leaves by their direct parent
        if self._parent_to_leaf is None:
            parent_ids = [self._get_parent(n) for n in self._leaf_nodes]
            self._parent_to_leaf = ub.group_items(self._leaf_nodes, parent_ids)
        for parent, children in self._parent_to_leaf.items():
            # Group children by the value of sample[key]
            child_vals = [self._tree[n]['data'][key] for n in children]
            grouped = ub.group_items(children, child_vals)
            # Set parent's weights if custom weights provided
            if weights is not None:
                weight_list = [weights.get(val, default_weight) for val in grouped.keys()]
                w_arr = np.asarray(weight_list, dtype=np.float64)
                if w_arr.sum() != 0:
                    w_arr = w_arr / w_arr.sum()
                    self._tree[parent]['weights'] = w_arr
                else:
                    removals.append((parent, children))
                    continue
            else:
                self._tree[parent]['weights'] = None
            # For each subgroup, create an intermediate node
            new_nodes = []
            for val, subgroup in grouped.items():
                new_parent = f"{parent}.{key}={val}"
                # Create new intermediate node with no sample data
                self._tree[new_parent] = {
                    'parent': parent,
                    'children': subgroup,
                    'weights': None,
                    'data': None,
                }
                new_nodes.append(new_parent)
                # Update each child's parent pointer to the new node
                for n in subgroup:
                    self._tree[n]['parent'] = new_parent
            # Replace parent's children with the new intermediate nodes
            self._tree[parent]['children'] = new_nodes
        # Invalidate cached grouping and prune if needed
        self._parent_to_leaf = None
        self._prune_and_reweight(removals)
    ...
    def sample(self):
        """
        Starting from the root, descend until a leaf is reached.
        At each intermediate node, use the stored weights (or uniform selection)
        to choose the child.
        Returns the leaf node id (an integer index into the sample grid).
        """
        current = self._root
        while len(self._tree[current]['children']) > 0:
            children = self._tree[current]['children']
            n = len(children)
            w = self._tree[current]['weights']
            if w is None:
                idx = self.rng.randint(0, n)
            else:
                idx = self.rng.choice(n, p=w)
            current = children[idx]
        return current
    ...
    def __repr__(self):
        return f"<BalancedSampleTree: {len(self._tree)} nodes, {len(self._leaf_nodes)} leafs>"
...
class BalancedSampleForest(ub.NiceRepr):
    ...
    CommandLine:
        LINE_PROFILE=1 xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.balanced_sampling BalancedSampleForest:1 --benchmark

    Example:
        >>> from kwcoco_dataloader.tasks.fusion.datamodules.balanced_sampling import BalancedSampleForest
        >>> sample_grid = [
