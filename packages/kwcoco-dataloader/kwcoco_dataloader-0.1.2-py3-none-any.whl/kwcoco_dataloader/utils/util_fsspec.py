...
class FSPath(str):
    ...
    def open(self, mode='rb', block_size=None, cache_options=None, compression=None):
        """
        Example:
            >>> from kwcoco_dataloader.utils.util_fsspec import *  # NOQA
            >>> from kwcoco_dataloader.utils import util_fsspec
            >>> dpath = util_fsspec.LocalPath.appdir('kwcoco_dataloader/fsspec/tests/open').ensuredir()
    ...
    def ls(self, detail=False, **kwargs):
        ...
        Example:
            >>> from kwcoco_dataloader.utils.util_fsspec import *  # NOQA
            >>> import ubelt as ub
            >>> dpath = ub.Path.appdir('kwcoco_dataloader', 'tests', 'fsspec', 'ls').ensuredir()
            >>> (dpath / 'file1').touch()
            >>> (dpath / 'file2').touch()
    ...
    def touch(self, truncate=False, **kwargs):
        ...
        Example:
            >>> import ubelt as ub
            >>> dpath = ub.Path.appdir('kwcoco_dataloader', 'tests', 'fsspec', 'touch').ensuredir()
    ...
    def move(self, path2, recursive='auto', maxdepth=None, idempotent=True, verbose=1, **kwargs):
        ...
        Ignore:
            import ubelt as ub
            root = ub.Path.appdir('kwcoco_dataloader', 'tests', 'fsspec', 'move').delete().ensuredir()
    ...
    def copy(self, dst, recursive='auto', maxdepth=None, on_error=None,
        ...
        Example:
            >>> from kwcoco_dataloader.utils import util_fsspec
            >>> dpath = util_fsspec.LocalPath.appdir('kwcoco_dataloader/fsspec/tests/copy').ensuredir()
...
class LocalPath(FSPath):
    ...
    CommandLine:
        xdoctest -m kwcoco_dataloader.utils.util_fsspec LocalPath
        xdoctest kwcoco_dataloader/utils/util_fsspec.py

    Example:
        >>> from kwcoco_dataloader.utils.util_fsspec import *  # NOQA
        >>> dpath = ub.Path.appdir('kwcoco_dataloader/tests/util_fsspec/demo')
...
class S3Path(RemotePath):
    ...
    SeeAlso:
        kwcoco_dataloader.heuristics.register_known_fsspec_s3_buckets
...
def _devtest_ssh_registry1():
    ...
    """
    from kwcoco_dataloader.utils.util_fsspec import SSHPath
...
def _devtest_ssh_registry2():
    # from kwcoco_dataloader.utils.util_fsspec import SSHPath, FSPath
