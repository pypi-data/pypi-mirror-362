...
class KWCocoVideoDataset(data.Dataset, GetItemMixin, BalanceMixin,
    ...
    Example:
        ...
        >>> import kwcoco
        >>> import kwcoco_dataloader
        >>> coco_dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader-multisensor-msi', geodata=True)
        >>> print({c.get('sensor_coarse') for c in coco_dset.images().coco_images})
        >>> print({c.channels.spec for c in coco_dset.images().coco_images})
        >>> sampler = ndsampler.CocoSampler(coco_dset)
    ...
    Example:
        ...
        >>> import kwcoco
        >>> import kwcoco_dataloader
        >>> coco_dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader', geodata=True)
        >>> print({c.get('sensor_coarse') for c in coco_dset.images().coco_images})
        >>> print({c.channels.spec for c in coco_dset.images().coco_images})
        >>> sampler = ndsampler.CocoSampler(coco_dset)
    ...
    def __init__(self, sampler, mode='fit', test_with_annot_info=False, autobuild=True, **kwargs):
        ...
        self.BACKWARDS_COMPAT_NEG_TO_POS = self.config['neg_to_pos_ratio'] is not None
    ...
    def _init_sensorchan(self):
        ...
        # The user can specify "dynamic channels", which are computed from
        # loaded channels on the fly.
        dynamic_channels_spec = copy.deepcopy(kwutil.Yaml.coerce(self.config['dynamic_channels']))
        dynamic_channel_names = []
        if dynamic_channels_spec is not None:
            self._dynamic_channels = DynamicChannels(dynamic_channels_spec)
            dynamic_channel_names = self._dynamic_channels._channel_names
        else:
            self._dynamic_channels = None
        ...
        if '*' in [s.sensor.spec for s in self.sensorchan.streams()]:
            ...
            for fused_sensorchan in self.sensorchan.streams():
                ...
                if sensor.spec == '*':
                    non_dynamic_chans = chans - dynamic_channel_names
                    for cand_sensor, cand_chans in sensorchan_hist.items():
                        for cand_chan_group in cand_chans:
                            ...
                            chan_isect = non_dynamic_chans & cand_chan_group
                            if chan_isect.spec == non_dynamic_chans.spec:
            ...
            if not expanded_input_sensorchan_streams:
                ...
                raise ValueError(f'The generic sensor * was given, but no data in the kwcoco file matched sensorchan: {self.sensorchan}')
        ...
        _input_sensorchans = []
        _sample_sensorchans = []
        # Holds cases that need to be dynamically computed, perhaps factor into
        # DynamicChannels later.
        self._special_inputs = {}
        for fused_sensorchan in self.sensorchan.streams():
            sensor = fused_sensorchan.sensor
            chans = fused_sensorchan.chans
            ...
            _input_sensorchan = sensor.spec + ':' + '|'.join(_stream)
            dynamic_chans = _stream & dynamic_channel_names
            if dynamic_chans:
                _sample_stream -= dynamic_chans
                # Sometimes the input channels will depened on other chanels
                # that can be sampled, but are not explicitly used in the input
                # so we need to extend the sample to load those.
                _other_req = sorted(set(ub.flatten(self._dynamic_channels._lut[n]['args'] for n in dynamic_chans)))

                _sample_stream = _sample_stream | _other_req
                _sample_sensorchan = sensor.spec + ':' + '|'.join(_sample_stream)

                self._special_inputs[_input_sensorchan] = {
                    'sample_sensorchan': _sample_sensorchan,
                    'input_sensorchan': _input_sensorchan,
                    'dynamic_chans': dynamic_chans,
                }
            else:
                _sample_sensorchan = _input_sensorchan
            _input_sensorchans.append(_input_sensorchan)
            _sample_sensorchans.append(_sample_sensorchan)
    ...
    def _init_robust_normalizers(self):
        """
        Handle coercion of normalizer_peritem and normalize_perframe.

        SeeAlso:
            _robust_normalize_frame_items
        """
        if self.config['normalize_peritem']:
            ub.schedule_deprecation(
                'kwcoco_dataloader', 'normalize_peritem', 'param',
                'use robust_normalize instead',
                deprecate='0.1.0', error='1.0.0', remove='1.1.0',
            )
        ...
        if self.config['normalize_perframe'] is None:
            ub.schedule_deprecation(
                'kwcoco_dataloader', 'normalize_perframe', 'param',
                'use robust_normalize instead',
                deprecate='0.1.0', error='1.0.0', remove='1.1.0',
        ...
        if self.config['robust_normalize'] not in [None, False]:
            self.robust_normalizer = RobustNormalizer.coerce(
                self.config['robust_normalize'],
                default_sensorchan=self.input_sensorchan,
            )
        else:
            self.robust_normalizer = None
    ...
    def _init(self):
        ...
        on the user input.
        """
        self._build_sample_grid()
        self._build_prenormalizers()
    ...
    def _build_prenormalizers(self):
        if self.config['prenormalize_inputs'] is not None:
            prenormalizers = None
    ...
    def __getitem__(self, index):
        ...
        Example:
            ...
            >>> import kwcoco_dataloader
            >>> import kwcoco
            >>> coco_dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader-msi-geodata-dates')
        ...
        Example:
            ...
            >>> import kwcoco_dataloader
            >>> import kwcoco
            >>> dvc_dpath = kwcoco_dataloader.find_dvc_dpath(tags='phase2_data', hardware='auto')
...
def more_demos():
    """
    CommandLine:
        USE_RTREE=1 DVC_DPATH=1 XDEV_PROFILE=1 xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.kwcoco_dataset more_demos:0
        USE_RTREE=0 DVC_DPATH=1 XDEV_PROFILE=1 xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.kwcoco_dataset more_demos:0
    ...
    Ignore:
        ...
        >>> item1 = self[index]
        >>> self.config['robust_normalize'] = FusedChannelSpec.coerce('red|green|blue|nir')
        >>> item2 = self[index]
    ...
    Example:
        >>> # Test Sampling with Dynamic Channels
        >>> from kwcoco_dataloader.tasks.fusion.datamodules.kwcoco_dataset import *  # NOQA
        >>> import ndsampler
        >>> import kwcoco
        >>> coco_dset = kwcoco.CocoDataset.demo('vidshapes1', num_frames=10)
        ...
        >>> index = self.sample_grid['targets'][self.sample_grid['positives_indexes'][0]]
        >>> item = self[index]
        >>> assert '*:g|neg_r' in item.sensorchan_histogram
        >>> # Summarize batch item in text
        >>> summary = self.summarize_item(item)
        >>> print('item summary: ' + ub.urepr(summary, nl=2))
        >>> # Draw batch item
