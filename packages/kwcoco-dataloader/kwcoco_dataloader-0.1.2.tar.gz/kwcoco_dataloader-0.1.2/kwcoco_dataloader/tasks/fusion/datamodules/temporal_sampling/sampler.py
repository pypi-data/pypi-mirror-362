...
CommandLine:
    SMART_DATA_DVC_DPATH=1 XDEV_PROFILE=1 xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling __doc__:3
    SMART_DATA_DVC_DPATH=1 xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling __doc__:3

    xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling __doc__:0 --show
    xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling __doc__:1 --show

Example:
    >>> # Basic overview demo of the algorithm
    >>> from kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling.sampler import *  # NOQA
    >>> import kwcoco_dataloader
    >>> dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader-msi', geodata=True, dates=True, num_frames=16, image_size=(8, 8))
...
Example:
    ...
    >>> from kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling.sampler import *  # NOQA
    >>> import kwcoco_dataloader
    >>> dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader-msi', geodata=True, dates=True, num_frames=16, image_size=(8, 8), num_videos=1)
    >>> vidid = dset.dataset['videos'][0]['id']
    >>> self = TimeWindowSampler.from_coco_video(
    >>>     dset, vidid,
    >>>     time_window=7,
...
Example:
    ...
    >>> from kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling.sampler import *  # NOQA
    >>> import kwcoco_dataloader
    >>> dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader-msi', geodata=True, dates=True, num_frames=1, num_videos=1, image_size=(8, 8))
    >>> vidid = dset.dataset['videos'][0]['id']
    >>> self = TimeWindowSampler.from_coco_video(
    >>>     dset, vidid,
    >>>     time_window=2,
...
Example:
    ...
    >>> dset = kwcoco_dataloader.coerce_kwcoco(coco_fpath)
    >>> vidid = dset.dataset['videos'][0]['id']
    >>> self = TimeWindowSampler.from_coco_video(
    >>>     dset, vidid,
...
Example:
    >>> # xdoctest: +REQUIRES(env:SMART_DATA_DVC_DPATH)
    >>> from kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling import *  # NOQA
    >>> import kwcoco_dataloader
    >>> data_dvc_dpath = kwcoco_dataloader.find_dvc_dpath(tags='phase2_data', hardware='auto')
    >>> coco_fpath = data_dvc_dpath / 'Drop6/imganns-KR_R001.kwcoco.zip'
    >>> dset = kwcoco_dataloader.coerce_kwcoco(coco_fpath)
...
class TimeWindowSampler(CommonSamplerMixin):
    ...
    def main_indexes(self):
        ub.schedule_deprecation(
            'kwcoco_dataloader', 'main_indexes', 'use indexes instead', deprecate='now')
        return self.indexes
    ...
    def sample(self, main_frame_idx=None, include=None, exclude=None,
        ...
        Example:
            ...
            >>> from kwcoco_dataloader.tasks.fusion.datamodules.temporal_sampling.sampler import *  # NOQA
            >>> import kwcoco_dataloader
            >>> dset = kwcoco_dataloader.coerce_kwcoco('kwcoco_dataloader-msi', geodata=True, dates=True, num_frames=32, image_size=(32, 32), num_videos=1)
            >>> vidid = dset.dataset['videos'][0]['id']
            >>> self = TimeWindowSampler.from_coco_video(
            >>>     dset, vidid,
            >>>     time_span='1y',
