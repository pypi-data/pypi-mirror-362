...
class BatchVisualizationBuilder:
    ...
    CommandLine:
        xdoctest -m kwcoco_dataloader.tasks.fusion.datamodules.batch_visualization BatchVisualizationBuilder

    Example:
        >>> from kwcoco_dataloader.tasks.fusion.datamodules.batch_visualization import *  # NOQA
        >>> from kwcoco_dataloader.tasks.fusion.datamodules.kwcoco_dataset import KWCocoVideoDataset
        >>> import kwcoco_dataloader
        >>> coco_dset = kwcoco_dataloader.coerce_kwcoco('vidshapes2-kwcoco_dataloader', num_frames=5)
    ...
    Example:
        ...
        >>> coco_dset = kwcoco_dataloader.coerce_kwcoco('vidshapes2-kwcoco_dataloader', num_frames=5)
        >>> channels = 'r|g|b,B10|B8a|B1|B8|B11,X.2|Y.2'
        >>> #coco_dset = kwcoco_dataloader.coerce_kwcoco('vidshapes2', num_frames=5)
        >>> #channels = None
        >>> combinable_extra = [['B10', 'B8', 'B8a']]  # special behavior
        >>> # combinable_extra = None  # uncomment for raw behavior
        >>> self = KWCocoVideoDataset(
        ...
        >>> plt.gcf().tight_layout()
        >>> ######
        >>> from kwcoco_dataloader.tasks.fusion.datamodules.batch_visualization import _debug_sample_in_context
    ...
    def __init__(builder, item, item_output=None, combinable_extra=None,
        ...
        if requested_tasks is None:
            requested_tasks = ub.ddict(lambda: False)
        builder.requested_tasks = requested_tasks

        builder.classes = classes
        if not default_combinable_channels:
    ...
    def _prepare_frame_metadata(builder):
        ...
        for frame_idx, frame_item in enumerate(item['frames']):
            ...
            for truth_key in truth_keys:
                ...
                if truth_data is not None:
                    truth_data = kwarray.ArrayAPI.numpy(truth_data)
            ...
            for weight_key in weight_keys:
                ...
                if weight_data is not None:
                    weight_data = kwarray.ArrayAPI.numpy(weight_data)
            ...
            for mode_code, mode_data in frame_modes.items():
                mode_data = kwarray.ArrayAPI.numpy(mode_data)
            ...
            max_perstrem = max(map(len, perstream_available))
            if max_perstrem > builder.max_channels:
                # Prioritize choosing a balance of channels from each stream
                # if there are more things than we can display
                frame_available_chans = list(more_itertools.roundrobin(*perstream_available))
            else:
                # Otherwise keep the order the same.
                frame_available_chans = list(ub.flatten(perstream_available))
        ...
        if builder.norm_over_time:
            # TODO: only normalize here if the values are out of the 0-1 range.
                    ...
                    if is_label_img:
                        ...
                        with warnings.catch_warnings():
                            ...
                            if raw_signal.dtype.kind == 'u' and raw_signal.dtype.itemsize == 1:
                                ...
                                try:
                                    # heuristic: only normalize if the data is outside the 0-1 range
