...
def test_push_capsules():
    capsule_data = pd.DataFrame([{
        ...
        'Capsule ID': 'bogus capsule ID'  # This will be ignored
    }, {
    ...
    assert not pd.isna(pull_result.iloc[0]['Capsule ID'])
    ...
    assert pull_result.iloc[1]['Capsule ID'] != 'bogus capsule ID'
    assert not pd.isna(pull_result.iloc[1]['Capsule ID'])
...
def test_push_capsules_property_unit_in_metadata():
    ...
    capsule_data = pd.DataFrame([{
        ...
        'Capsule Start': pd.to_datetime('2020-01-10T09:00:00.000Z'),
        'Capsule End': pd.to_datetime('2020-01-10T17:00:00.000Z'),
        'Condition': f'{condition_name} 2',
        'Distance': 10
    }])
    ...
    push_result = spy.push(data=capsule_data,
                           metadata=pd.DataFrame([{
                               ...
                               'Name': f'{condition_name} 2',
                               'Type': 'Condition',
                               'Maximum Duration': '2d',
                               'Capsule Property Units': {'Distance': 'm'}
                           }]).set_index('Name', drop=False),
                           workbook=workbook, worksheet=None)

    assert len(push_result) == 3
    assert sorted(push_result['Name'].to_list()) == ['A No Data Condition', condition_name, f'{condition_name} 2']
    assert push_result.at['A No Data Condition', 'Push Count'] == 0
    assert push_result.at[condition_name, 'Push Count'] == 2
    assert push_result.at[f'{condition_name} 2', 'Push Count'] == 1

    pull_result = spy.pull(push_result, start='2019-01-01T09:00:00.000Z', end='2020-02-01T09:00:00.000Z')

    assert len(pull_result) == 3

    condition_1 = pull_result[pull_result['Condition'] == condition_name]
    assert condition_1.iloc[0]['Capsule Start'] == pd.to_datetime('2019-01-10T09:00:00.000Z')
    assert condition_1.iloc[0]['Capsule End'] == pd.to_datetime('2019-01-10T17:00:00.000Z')
    assert condition_1.iloc[0]['Distance'] == 5
    assert condition_1.iloc[1]['Capsule Start'] == pd.to_datetime('2019-01-11T09:00:00.000Z')
    assert condition_1.iloc[1]['Capsule End'] == pd.to_datetime('2019-01-11T17:00:00.000Z')
    assert condition_1.iloc[1]['Distance'] == 8

    condition_2 = pull_result[pull_result['Condition'] == f'{condition_name} 2']
    assert condition_2.iloc[0]['Capsule Start'] == pd.to_datetime('2020-01-10T09:00:00.000Z')
    assert condition_2.iloc[0]['Capsule End'] == pd.to_datetime('2020-01-10T17:00:00.000Z')
    assert condition_2.iloc[0]['Distance'] == 10

    search_result = spy.search(push_result, all_properties=True)
    search_result.set_index('Name', drop=False, inplace=True)

    assert pd.isna(search_result.at['A No Data Condition', 'Metadata Properties'])
    assert search_result.at[condition_name, 'Metadata Properties'] == 'Distance=in'
    assert search_result.at[f'{condition_name} 2', 'Metadata Properties'] == 'Distance=m'
