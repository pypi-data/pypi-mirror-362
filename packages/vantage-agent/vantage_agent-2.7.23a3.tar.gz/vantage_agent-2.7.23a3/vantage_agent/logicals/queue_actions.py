...
async def fetch_cluster_queue_actions() -> list[dict[str, Any]]:
    ...
    query = dedent(
        """
        query clusterQueueActions($clusterName: String!) {
            clusterQueueActions(
                first: 100
                filters: {
                    clusterName: {
    ...
    body = {"query": query, "variables": {"clusterName": SETTINGS.CLUSTER_NAME}}
...
async def _process_cluster_queue_actions_async():
    ...
    for action in actions:
        action_id = action["id"]
        queue_info = action.get("queue", {}).get("info", {})
        ...
        if success:
            ...
            logger.error(f"Failed to cancel job {job_id} for action {action_id}, keeping action")
