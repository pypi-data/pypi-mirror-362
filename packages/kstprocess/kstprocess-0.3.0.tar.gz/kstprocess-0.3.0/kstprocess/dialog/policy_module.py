            ...
            if not self.nlu_result[session_id]["gender"]:
                ...
                for key in self.male_gender_keyword:
                    ...
                    if key in keyword:
                        self.nlu_result[session_id]["gender"] = "男"
            ### 增加孩子本人 识别模块
        elif user_utterance and not keyword:
            pattern = "(^((?!.*(控制)).)*自[个己])|我的?(奶|姥|爷爷|外[婆公]|父|母|爸|妈|叔|婶|姨|兄|姐).*(专政|管太多|听不懂|没耐心)|我[跟和]?(奶|姥|爷爷|外[婆公]|父|母|爸|妈|叔|婶|姨|兄|姐)|我就?是(孩子|小[男女]?孩|学生)|我(现在|已经?有?|[当目]前|刚满|快要?)?([1-9][0-9]?|[一二三四五六七八九十][一二三四五六七八九]?)岁|(是|^)我(?!.*(想咨询|小孩|孩子|女儿|儿子|侄[子女]|儿童?|娃[娃儿]?))|(本人|我)才?(十?[一二两三四五六七八九]|十|1[0-9]|[1-9])周?岁|本人"
