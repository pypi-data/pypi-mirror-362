...
class TicktickFocusHandler:
    ...
    def get_active_focus_start_time(self) -> datetime | None:
        ...
        try:
            ...
            if 'current' not in response_data or response_data['current']['status'] != 0:
                return None
            start_time_val = response_data['current']['startTime']
            start_time = datetime.strptime(start_time_val, '%Y-%m-%dT%H:%M:%S.%f%z')
            return start_time
    ...
    def get_active_focus_time(self, timezone_name: str = 'Europe/Berlin') -> float:
        start_time = self.get_active_focus_start_time()
