...
class BaseCompletion(ABC):
    ...
    def _execute_with_retry(self, params: dict, stream: bool | NotGiven, operation_name: str):
        ...
        return self._retry_with_backoff(self.client.create, **params, operation_name=operation_name, metric=metric)  # type: ignore

    async def _execute_with_retry_async(self, params: dict, stream: bool | NotGiven, operation_name: str):
        metric = self._llm.enable_metrics and not stream
        return await self._retry_with_backoff_async(self.client.acreate, **params, operation_name=operation_name, metric=metric)  # type: ignore
    ...
    def _retry_with_backoff(self, func, *args, operation_name: str = "operation", metric: bool = False, **kwargs):
