...
tamil_sleep = time.sleep
tamil_date_time = time.asctime
...
from .runtime import  Environment, BuiltinFunction, \
 BlindBuiltins, DebugUtils, tamilCustomFunction
...
from .tamil_library import Load_URL_APIs
...
def tamil_credits():
    return u"tamil language, version %g, was created by Muthiah Annamalai with several contributors."%(tamil_version())

def tamil_license():
    return u"Licensed under GPL Version 3"

def tamil_tamil_length(arg):
	return Number( len( tamil.utf8.get_letters( arg ) ) )

def tamil_getitem(*args):
    if len(args) < 2:
    ...
    if with_default:
        ...
        idxs = [idx]
    #print("dispatching tamil getitem",type(x),type(idx),x,idx,x[idx])
...
def tamil_isnumber( x ):
    return Boolean(isinstance(x,Number) or type(x) in [int, float] )

def tamil_setitem(x,key,val):
    #print("dispatching tamil setitem",type(x),type(idx),x,idx,x[idx])
...
def tamil_substr(str_var,start,end):
    return str_var[start:end]

def tamil_load(filename):
    # load the file into the interpreter
    # we have a symbol table and use it
    tamil_execute([filename])

class NoClobberDict(dict):
    """ dictionary structure with a set like mathematical structure.
        that way when you insert functions for tamil library, we don't trample on each other
        by accidentally overwriting stuff"""
    def __init__(self):
        dict.__init__(self)
    def __setitem__(self,key,val):
        if ( self.get(key,None) ):
            #print(u"-------> %s"%str(self.get(key)))
            traceback.print_stack()
            raise KeyError(u"Dictionary is getting clobbered; key '"+key+"' already present '%s'"%str(self.get(key)))
        dict.__setitem__(self,key,val)

global_interpreter = None

def tamil_list_functions(*args):
...
def tamil_execute(*args):
    ...
    if len(args) < 1:
        raise RuntimeException(u"tamil_execute: missing filename argument")
...
## Iyakki : tamil inge irunthu iyakkapadum
class Interpreter(DebugUtils):
    ...
    def __init__(self,lexer, debug = False,safe_mode=False,update=True,stacksize=500,env=None):
        ...
        self.stacksize = stacksize #for tamil stacksize
        self.lexer=lexer
        self.ast=None
    ...
    def RAWINPUT(args):
        try:
            op = tamilCustomFunction.raw_input(args)
    ...
    def INPUT(args):
        try:
            op = eval(tamilCustomFunction.raw_input(args))
        except KeyboardInterrupt as e :
            print(u"\nTo exit the program type : exit or press CTRL + D on your keyboard")
            return Number(0)
    ...
    def install_builtins(self):
        ...
        # special tamil commands
        self.builtin_map['copyright']=BlindBuiltins(tamil_copyright,'copyright',self.debug)
        self.builtin_map['version']=BlindBuiltins(tamil_version,'version',self.debug) 
        self.builtin_map['credits']=BlindBuiltins(tamil_credits,'credits',self.debug)
        
        # global interpreter
        self.builtin_map['dir']=BlindBuiltins(tamil_list_functions,'dir',self.debug)
        ...
        self.builtin_map['eval']=BlindBuiltins(tamil_eval,'eval',self.debug)
        self.builtin_map[u'மதிப்பீடு']=self.builtin_map['eval']
        self.builtin_map['execute']=BlindBuiltins(tamil_execute,'execute',self.debug)
        self.builtin_map[u'இயக்கு']=self.builtin_map['execute']
        
        self.builtin_map['profile'] = BlindBuiltins(tamil_profile,'profile',self.debug)
        ...
        if not PYTHON3:
            self.builtin_map['file']=BlindBuiltins(open,'file',self.debug)
            self.builtin_map['intern'] = BlindBuiltins(sys.intern, 'intern', self.debug)
        ...
        try:
            ...
            self.builtin_map['license']=BlindBuiltins(tamil_license,'license',self.debug)
        ...
        self.builtin_map['substr']=BlindBuiltins(tamil_substr,'substr',self.debug)
        ...
        self.builtin_map['quit']=BlindBuiltins(tamil_quit,'quit',self.debug)
        ...
        if not PYTHON3:
            ...
            self.builtin_map['reload']=BlindBuiltins(importlib.reload,'reload',self.debug)
        ...
        self.builtin_map['__getitem__']=BlindBuiltins(tamil_getitem,"__getitem__")
        self.builtin_map['__setitem__']=BuiltinFunction(tamil_setitem,"__setitem__",3)
        ...
        # keywords
        self.add_builtin("keywords",lambda : tamil_keywords()[0],nargin=0,ta_alias=u"குறிசொற்கள்")
        
        # assert
        self.builtin_map["assert"]=BuiltinFunction(Interpreter.tamil_assert,"assert")
        self.builtin_map[u"உறுதிப்படுத்த"]=self.builtin_map["assert"]
        
        # str - used for simple serialization in tamil
        ...
        # sleep/pause
        self.add_builtin("sleep",tamil_sleep,nargin=1,ta_alias=u"உரங்கு")
        self.builtin_map["pause"]=BlindBuiltins(Interpreter.tamil_pause,"pause")

        # date/time
        self.add_builtin("date_time",tamil_date_time,nargin=0,ta_alias=u"தேதி_நேரம்")
        ...
        self.add_builtin("islist",tamil_islist,nargin=1,ta_alias=u"பட்டியலா")
        self.add_builtin("isnumber",tamil_isnumber,nargin=1,ta_alias=u"எண்ணா")
        # random functions
        aslist = True;
        ...
        try:
            from TamilTurtle import TamilTurtle

            turtle_attrib = TamilTurtle.functionAttributes();
            for nargs,fcnName in list(turtle_attrib.items()):
                for vv in fcnName:
                    turtlefcn = u"turtle_"+vv;
                    if ( self.debug ): print(nargs, vv)
                    if ( nargs == -1 ):
                        self.builtin_map[turtlefcn] = BlindBuiltins(getattr(TamilTurtle, vv),vv,self.debug)
                    else:
                        self.builtin_map[turtlefcn] = BuiltinFunction( getattr( TamilTurtle, vv ), turtlefcn, nargs )
        except ImportError as ie:
            if ( self.debug ): 
                print(u"Cannot Import TamilTurtle module; ignoring for now")
        ...
        self.builtin_map["reverse"]= BuiltinFunction(Interpreter.tamil_reverse,"reverse",1)
        ...
        self.add_builtin("tamil_length",tamil_tamil_length,nargin=1,ta_alias=u"தநீளம்")     
...
tamil : A Tamil Programming Language - version %g, (C) 2007-2017
Type "help", "copyright", "credits" or "license" for more information."""%tamil_version()
...
class Interpreter:
    ...
    def execute(self, source_code):
        # your logic to parse & run Tamil code
        print("Executing Tamil code:")
        print(source_code)

# ✅✅✅ Add this BELOW your class!
def main(filename):
    with open(filename, encoding="utf-8") as f:
        source_code = f.read()

    interpreter = Interpreter()  # ✅ create your interpreter
    interpreter.execute(source_code)  # ✅ run it!
