...
import sys
import xml
from .tamil_scanner import tamilToken, Token
from .transform import TransformVisitor
from .tamil_program_utils import get_ast;
...
class GraphVisualizer(TransformVisitor):
    def __init__(self,interpreter,debug=False,filename=None):
        ...
        tobj = Tag(self.file,"tamil",self.tab)
        self.incr()
    ...
    def visit_expr(self, expr):
        ...
        toktype = tamilToken.token_types[expr.binop.kind]
...
def visualizeSourceFile(filename):
    parse_tree,tamil_interpreter = get_ast(filename)
    svgfilename = filename.replace(".n",".svg")
    GraphVisualizer(interpreter=tamil_interpreter,filename=svgfilename)

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python tamil_vsiualizer.py <srcfile1> <srcfile2> ...")
        sys.exit(255)
