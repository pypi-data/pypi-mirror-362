...
class tamilCustomFunction(object):
    _instance = None
    def __init__(self):
        object.__init__(self)
        self.input_fcn = input
    @staticmethod
    def get_instance():
        if not tamilCustomFunction._instance:
            tamilCustomFunction._instance = tamilCustomFunction()
        return tamilCustomFunction._instance
    
    @staticmethod
    def input(args):
        return tamilCustomFunction.get_instance().input_fcn(args)
    
    @staticmethod
    def reset():
        if PYTHON3:
            tamilCustomFunction.get_instance().input_fcn = input
        else:
            tamilCustomFunction.get_instance().input_fcn = input
    
    @staticmethod
    def set(newfcn):
        tamilCustomFunction.get_instance().input_fcn = newfcn
...
class BuiltinFunction:
    ...
    def evaluate(self,env):
        try:
            ...
            env.unroll_stack()
            raise RuntimeException( str(assert_excep) + u' Assertion failed!')
        except Exception as excep:
            env.unroll_stack()            
            print(u"failed dispatching function ",str(self),u"with exception",str(excep))
            if ( self.debug ):
    ...
    def do_evaluate(self,env):
        ...
        # because Python libraries don't recognize tamil AST objects.
...
class CountDownTimer(object):
    def __init__(self):
        object.__init__(self)
        self._engaged = False
        self._safeModeTimeout = 13  # seconds of wall clock timeout for tamil codes in SAFE MODE
        self._startTime = 0
...
class Environment(CountDownTimer):
    ...
    def __repr__(self):
        retval = u"CallStack =>"+str(self.call_stack) + u"\n" \
            u"LocalVars =>"+ str(self.local_vars) + u"\n" \
            u"ArgStack =>"+ str(self.arg_stack) + u"\n" 
        return retval
    ...
    def get_id(self, idee):
        ...
        if not self.has_id(idee):
            ...
            if idee in keyword.kwlist:
                note = 'Did you possibly confuse the Python english keyword %s for tamil keyword ?'%idee
    ...
    def call_function(self, fn,callsite=u""):
        ...
        self.dbg_msg(u"calling function"+str(fn)+callsite)
