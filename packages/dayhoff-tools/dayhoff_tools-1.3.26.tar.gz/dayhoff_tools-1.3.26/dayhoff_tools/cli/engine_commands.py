...
def engine_status(
    ...
    if idle_detector.get("available"):
        ...
        if idle_detector.get("coffee_lock"):
            status_lines.append(f"  • [cyan]☕ Coffee lock active for {idle_detector['coffee_lock']}[/cyan]")
        ...
        if ide_conn:
            status_lines.append(f"  • [magenta]🖥  {ide_conn['ide']} connected ({ide_conn['connection_count']} connections)[/magenta]")
