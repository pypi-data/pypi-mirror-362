    ...
    *,
    ...
    for p in paths:
        ...
        zip_path, _ = WorkspaceZipper(
            p,
            output_dir,
        ).zip(create_zips=create_zips, excludes=exclude_files)
    ...
    with _or_tempdir(zip_output_dir, delete=delete_local_zips) as zip_output_dir:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            ...
            for zip_paths in zips:
                res.append([])
                _LOG.info(f"Uploading {len(zip_paths)} zip files: {zip_paths}")
                for zip_path in zip_paths:
                    fut = executor.submit(
                        uploader.upload,
                        zip_path,
            ...
            with tqdm(
                ...
                for i in range(len(res)):
                    ...
                    res[i] = uploaded_uris
                _LOG.info(
                    f"Uploaded {len(res)} zip path file sets to"
                    f" {uploader.upload_dir}: {res}"
                )
