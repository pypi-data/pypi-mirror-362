...
def get_kernel_density_geographic(
    GeoreferenceObject: IGeoReference,
    bandwidth=0.005,
    xbins=100,
    ybins=100,
    gap_x=0,
    ...
    **kwargs
    ...
    x = np.linspace(
        GeoreferenceObject.lon.min() - bandwidth * 2 - gap_x,
        GeoreferenceObject.lon.max() + bandwidth * 2 + gap_x,
        xbins,
    )
    y = np.linspace(
        GeoreferenceObject.lat.min() - bandwidth * 2 - gap_y,
        GeoreferenceObject.lat.max() + bandwidth * 2 + gap_y,
        ybins,
    )
