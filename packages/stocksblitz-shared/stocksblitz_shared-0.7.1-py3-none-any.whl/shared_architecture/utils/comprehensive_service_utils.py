...
def _add_middleware_stack(app: FastAPI, config: ServiceConfig, logger: logging.Logger):
    ...
    if config.enable_request_logging:
        # Using a custom middleware class to avoid the EndOfStream bug
        ...
        class RequestLoggingMiddleware(BaseHTTPMiddleware):
            async def dispatch(self, request: Request, call_next):
                ...
                try:
                    ...
                    # Return 500 error response instead of raising
                    return Response(content=str(e), status_code=500)
        
        app.add_middleware(RequestLoggingMiddleware)
        logger.info("✅ Request logging middleware added")
    
    # 6. Error handling middleware (removed to avoid EndOfStream bug)
    # The error handling is now done in the RequestLoggingMiddleware above
    logger.info("✅ Error handling integrated into request logging middleware")
