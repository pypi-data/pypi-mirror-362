...
def get_timescaledb_session():
    ...
    try:
        ...
        db_name = config_loader.get("TIMESCALEDB_DB", "stocksblitz_unified", scope="common")
        # Override with environment variable if set
        env_db = os.getenv("TIMESCALEDB_DB")
        if env_db:
            logger.info(f"Overriding database from environment: {env_db}")
            db_name = env_db
