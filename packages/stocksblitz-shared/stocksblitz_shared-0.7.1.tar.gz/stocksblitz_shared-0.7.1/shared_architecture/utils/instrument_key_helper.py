...
def get_instrument_key(
    ...
    if product_type == "equities":
        return f"{exchange}@{stock_code}@equity_spot"
...
def symbol_to_instrument_key(symbol: str, exchange: str = 'NSE') -> str:
    ...
    Returns:
        str: Internal instrument_key format (e.g., "NSE@RELIANCE@equity_spot", "NSE@NIFTY@index_futures@25-JUN-2025")
    
    Examples:
        symbol_to_instrument_key("RELIANCE") -> "NSE@RELIANCE@equity_spot"
...
def parse_stocksdeveloper_symbol(symbol: str, exchange: str = 'NSE') -> dict:
    ...
    # Correct format: NSE@RELIANCE@equity_spot (no extra @)
    return {
        'instrument_key': f"{exchange}@{symbol}@equity_spot",
        'exchange': exchange,
        'stock_code': symbol,
        'product_type': 'equity_spot',
        'expiry_date': None,
        'option_type': None,
        'strike_price': None
    }
...
def test_instrument_key_generator():
    ...
    test_cases = [
        # Equities
        ("RELIANCE", "NSE@RELIANCE@equity_spot"),
        ("HDFC", "NSE@HDFC@equity_spot"),
        ("TCS", "NSE@TCS@equity_spot"),
