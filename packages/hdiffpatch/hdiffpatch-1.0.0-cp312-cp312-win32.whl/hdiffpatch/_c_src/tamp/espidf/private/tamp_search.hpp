...
class Locator {
   private:
    ...
    unrolled_find_3(const uint8_t*& data, const uint32_t vl, const uint32_t vh) noexcept {
         ...
         * This turns out to be slightly faster than loading (*(uint32_t*)data) and
         * (*(uint32_t*)(data+1)), and reduces register pressure on Xtensas because L32I requires
         * the offset to be a multiple of 4.
         */
        static_assert(N < M);
        if constexpr (N % 2 == 0) {
     ...
     */
    // For some reason, when gcc inlines this, this DOUBLES the total processing time. Needs
    // investigation!
    static const uint8_t* __attribute__((noinline))
    find_pattern_short_scalar(const uint8_t* const pattern, const uint32_t patLen, const uint8_t* data,
    ...
    // investigation!
    static const uint8_t* __attribute__((noinline))
    find_pattern_long_scalar(const uint8_t* pattern, const uint32_t patLen, const uint8_t* data,
                             uint32_t dataLen) noexcept {
        using T = uint32_t;
   ...
   public:
     ...
     * On ESP32-S3 targets, this uses SIMD instructions; delegates to ::find_pattern_scalar() on
     * other targets.
     *
     * @param pattern start of pattern to search for
     * @param patLen length of pattern to search for
     ...
     * @return first start of pattern in data, or \c nullptr if not found
     */
    static const uint8_t* __attribute__((noinline)) find_pattern(const uint8_t* const pattern, const uint32_t patLen,
                                                                 const uint8_t* data, const uint32_t dataLen) noexcept {
        if constexpr (Arch::ESP32S3) {
            ...
            do {
                ...
                if (tmp) {
                    ...
                    /* Yes, it takes no more than 7 instructions, plus the 'lookup table' from
                     * POS_U8 (in q7), to extract 16 bits from the 16 8-bit boolean results of a
                     * vector comparison.
                     *
