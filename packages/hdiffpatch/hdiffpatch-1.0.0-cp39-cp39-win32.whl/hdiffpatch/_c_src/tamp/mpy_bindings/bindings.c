...
static mp_obj_t compressor_write(mp_obj_t self_in, mp_obj_t data_in) {
    ...
    for (uint8_t *input_buffer = (uint8_t *)input_buffer_info.buf; input_buffer < input_buffer_end;
         input_buffer += input_consumed_size) {
        TAMP_CHECK(tamp_compressor_compress(&self->c, output_buffer, CHUNK_SIZE, &output_buffer_written_size,
                                            input_buffer, input_buffer_end - input_buffer, &input_consumed_size));
...
static mp_obj_t compressor_flush(mp_obj_t self_in, mp_obj_t write_token_in) {
    ...
    TAMP_CHECK(tamp_compressor_flush(&self->c, output_buffer, CHUNK_SIZE, &output_written_size,
...
static mp_obj_t decompressor_readinto(mp_obj_t self_in_obj, mp_obj_t output_buffer_obj) {
    ...
    do {
        ...
        res = tamp_decompressor_decompress(
            &self->d, output_buffer_info.buf + total_written_size, output_buffer_info.len - total_written_size,
            &output_buffer_written_size, input_buffer_info.buf + self->input_buffer_consumed_size,
            self->input_buffer_size - self->input_buffer_consumed_size, &input_buffer_consumed_size);
...
mp_obj_t mpy_init(mp_obj_fun_bc_t *self, size_t n_args, size_t n_kw, mp_obj_t *args) {
    ...
    // Add methods
    compressor_locals_dict_table[0] =
    ...
    // Add methods
    decompressor_locals_dict_table[0] =
        (mp_map_elem_t){MP_OBJ_NEW_QSTR(MP_QSTR_readinto), MP_OBJ_FROM_PTR(&decompressor_readinto_obj)};
