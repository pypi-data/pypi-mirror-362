...
int benchmark_compressor() {
    ...
    if (!(uncompressed_data = malloc(uncompressed_file_len))) EXIT(1, "OOM");

    if (!(compressed_data = malloc(uncompressed_file_len))) EXIT(1, "OOM");
    ...
    size_t consumed;
    if (TAMP_OK != tamp_compressor_compress_and_flush(&compressor, compressed_data, uncompressed_file_len,
                                                      &compressed_len, uncompressed_data, uncompressed_file_len,
                                                      &consumed, false))
...
exit:
    ...
    if (compressor_buffer) free(compressor_buffer);
    if (uncompressed_file) fclose(uncompressed_file);
...
int benchmark_decompressor() {
    ...
    if (!(window_buffer = malloc(32 << 10))) EXIT(1, "OOM");

    if (!(input = malloc(input_size))) EXIT(1, "OOM");

    if (!(output = malloc(output_size))) EXIT(1, "OOM");

    if (!(input_file = fopen("../../build/enwik8.tamp", "rb")))
        EXIT(1, "Unable to open input file %s", "../../build/enwik8.tamp");

    if (!(output_file = fopen("build/enwik8_reconstructed", "wb"))) EXIT(1, "Unable to open output file");
...
int main(int argc, char *argv[]) {
    if (strcmp(argv[1], "compressor") == 0)
        ...
        for (uint8_t i = 0; i < 16; i++) benchmark_decompressor();
    else
