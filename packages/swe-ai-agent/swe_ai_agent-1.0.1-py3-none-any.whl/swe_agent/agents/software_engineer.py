...
class SoftwareEngineerAgent:
    ...
    def _analyze_task(self, messages: List[BaseMessage]) -> str:
        ...
        if not messages:
            logger.debug("📋 No messages, starting with code analysis")
        ...
        if last_message and hasattr(last_message, 'content'):
            ...
            if "ANALYSIS COMPLETE" in content:
                logger.info("✅ Code analysis complete, moving to editing")
            ...
            if "Plan approved" in content or "APPROVAL: approved" in content:
                logger.info("✅ Plan approved, moving to implementation")
                ...
                for msg in messages:
                    if hasattr(msg, 'content') and msg.__class__.__name__ == 'HumanMessage':
                        ...
                        logger.info(f"✅ Found original task: {original_task}")
                        break
                ...
                if not original_task:
                    for msg in messages:
                        if hasattr(msg, 'content') and msg.content and not any(x in msg.content for x in ['APPROVAL:', 'Plan approved', 'EDIT FILE', 'ANALYZE CODE']):
                            original_task = msg.content
                            logger.info(f"✅ Found fallback task: {original_task}")
                            break
            ...
            if "EDITING COMPLETED" in content:
                logger.info("✅ Editing complete, workflow finished")
        ...
        try:
            ...
            system_prompt = """You are a software engineering workflow coordinator. 
            Analyze the conversation context and determine the next action. 
            Response must be one of:
            - "ANALYZE CODE - [reason]" - to start code analysis
            - "EDIT FILE - [reason]" - to start file editing  
            - "PATCH COMPLETED - [reason]" - to end workflow
            ...
            logger.debug(f"🤖 Claude analysis: {claude_response}")
