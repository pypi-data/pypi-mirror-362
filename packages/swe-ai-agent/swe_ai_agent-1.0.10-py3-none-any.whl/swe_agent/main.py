...
def execute_single_task(task: str, settings: Settings, use_planner: bool = False):
    ...
    # Initialize workflow
    workflow = CleanSWEWorkflow(str(settings.repo_path), str(settings.output_dir), use_planner=use_planner)
...
def start_interactive_mode(settings: Settings):
    ...
    cli = SWEInterface(settings)
    cli.start_interactive_mode()


def start_pair_programming(settings: Settings):
    """Start pair programming interface"""
    interface = PairProgrammingInterface(str(settings.repo_path), str(settings.output_dir))
    interface.run()
