    ...
    def _create_tools(self):
        ...
        def edit_file(filename: str, new_content: str, line_number: int, num_lines: int = 1) -> str:
            ...
            if result["success"]:
                ...
                return f"❌ Failed to edit file: {result.get('error', 'Unknown error')}"
...
{chr(10).join([f"  - {lang}: {count}" for lang, count in summary['languages'].items()])}"""
                ...
                return f"❌ Directory analysis failed: {result.get('error', 'Unknown error')}"
...
{chr(10).join([f"  - {m['name']} (line {m['line_start']})" for m in class_info['methods'][:10]])}"""
                ...
                return f"❌ Class not found: {result.get('error', 'Unknown error')}"
...
Arguments: {func_info.get('args', 'None')}"""
                ...
                return f"❌ Function not found: {result.get('error', 'Unknown error')}"
