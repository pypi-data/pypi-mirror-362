...
Some new fields of the SETUP structure may be added in new software
releases. This document is based on software release 792 (PhCon.dll,
PhFile.dll, PhInt.dll version 13.5.792.0, PCC version 3.5.792.0).
"Cinfe File Format" docu of Vision Research Version 13.5.792.0 (June 2020)
"""
...
tagSETUP._fields_ = [
    ...
    ("CameraModel", c_char * 1024),     # Camera model string.
                                        # End of SETUP in software version 742
    ("WBType", uint32_t),               # For raw color cines, it describes how meta WB is stored. 
                                        # If bit 0 is set - wb is stored in WBGain field. 
                                        # If bit 1 is set - wb is stored in fWBTemp, fWBCc fields.
    ("fDecimation", c_float),           # Decimation coefficient employed when this cine was saved to file.
                                        # End of SETUP in software version 745
    ("MagSerial", uint32_t),            # The serial of the magazine where the cine was recorded or stored (if any, null otherwise)
    ("CSSerial", uint32_t),             # Cine Save serial: The serial of the device (camera, cine station) used to save the cine to file
    ("dFrameRate", c_double),           # High precision acquisition frame rate, replace uint32_t FrameRate
                                        # End of SETUP in software version 751
    ("SensorMode", uint32_t),           # Camera sensor Mode used to define what mode the Sensor is in 
                                        # This is currently valid for the Flex4K, VEO 4K and VIRGO V2640 it is undefined for all other camera (should be 0)
                                        # Flex4K / VEO 4K
                                        ...
                                        # This is only valid for Virgo cameras currently (NA = not used currently)
                                        # 0x0000 = Standard
                                        # 0x0001 = NA
