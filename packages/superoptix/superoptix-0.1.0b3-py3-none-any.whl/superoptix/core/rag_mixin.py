...
class RAGMixin:
    ...
    def setup_rag(self, spec_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            retriever_type = config.get('retriever_type', config.get('vector_database', 'chroma')).lower()
            
            # Setup vector database
            self.vector_db = self._setup_vector_database(config)
            if not self.vector_db:
                logger.warning(f"⚠️ Failed to setup vector database for {retriever_type}")
                return False
    ...
    def _setup_vector_database(self, config: Dict[str, Any]):
        ...
        try:
            if retriever_type == 'chroma':
                ...
                logger.error("Supported types: chroma, lancedb, faiss, weaviate, qdrant, milvus, pinecone")
                return None
    ...
    def _print_dependency_help(self, package_name: str, db_name: str):
        ...
        print("")
        print("📚 For more information, see: https://superoptix.readthedocs.io/rag-setup")
        print("")
    ...
    async def retrieve_context(self, query: str, top_k: int = None) -> List[str]:
        ...
        try:
            ...
            if top_k is None:
                top_k = self.vector_db.get('config', {}).get('config', {}).get('top_k', 5)
    ...
    async def _query_weaviate(self, query: str, top_k: int) -> List[str]:
        ...
        try:
            ...
            results = client.query.get(collection_name, ["content"]).with_near_text({
                "concepts": [query]
            }).with_limit(top_k).do()
    ...
    def get_rag_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "doc_processor_available": hasattr(self, 'doc_processor') and self.doc_processor is not None
            }
