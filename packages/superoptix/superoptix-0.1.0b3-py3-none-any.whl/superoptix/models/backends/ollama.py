...
class SuperOptiXOllamaBackend(SuperOptiXBaseBackend):
    ...
    def _run_ollama_command(self, command: List[str], capture_output: bool = True) -> subprocess.CompletedProcess:
            ...
            for line in lines:
                if line.strip():
                    ...
                    if len(parts) >= 4:
                        name = parts[0]
                        size_str = parts[2]
                        modified_str = parts[3] + " " + parts[4] if len(parts) > 4 else "Unknown"
                        ...
                        models.append(SuperOptiXModelInfo(
                            ...
                            last_used=modified,
                            tags=["superoptix", "installed", task.value if task else "unknown"],
                        ))
    ...
    def install_model(self, model_name: str, **kwargs) -> bool:
        ...
        if not shutil.which("ollama"):
            ...
            console.print(f"  [bold green]super model install {model_name}[/bold green]")
            return False
        ...
        try:
            ...
            for model in installed_models:
                if model.name == model_name:
                    ...
                    console.print(f"  [bold green]super model dspy ollama/{model_name}[/bold green]")
                    return True
        except Exception as e:
            console.print(f"⚠️  Warning: Could not check installed models: {e}")
        
        # Step 4: Pull the model
        console.print(f"🦙 Pulling model [bold cyan]{model_name}[/bold cyan] from Ollama...")
        console.print("⏳ This may take a few minutes depending on your internet connection and model size.")
        console.print()
        
        try:
            # Run ollama pull command
            result = self._run_ollama_command(["pull", model_name], capture_output=False)  # noqa: F841
            ...
            try:
                ...
                if model_info:
                    console.print(f"  • Size: {model_info.size.value if model_info.size else 'Unknown'}")
                    console.print(f"  • Task: {model_info.task.value if model_info.task else 'General'}")
                    console.print(f"  • Parameters: {model_info.parameters or 'Unknown'}")
    ...
    def test_model(self, model_name: str, prompt: str = "Hello, world!") -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "tokens": 0,  # Ollama CLI doesn't provide token counts
                "prompt_tokens": 0,
                "superoptix": True,
            }
        except Exception as e:
