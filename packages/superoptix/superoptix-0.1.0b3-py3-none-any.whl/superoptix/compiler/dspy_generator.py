...
class DSPyOraclesPipelineGenerator:
    ...
    def __init__(self, agent_name: str, template_path: Optional[Path] = None, user_tier: TierLevel = TierLevel.ORACLES):
        ...
        self.template_path = template_path or (
            ...
            "templates" / "pipeline" / "dspy_oracles_pipeline.py.jinja2"
        )
    ...
    def _get_system_name(self) -> str:
        ...
        for item in self.project_root.iterdir():
            if (item.is_dir() and
                ...
                not item.name == 'superoptix' and
                'semantic' in item.name):
                return 'semantic'  # Just return 'semantic' as system name

        raise FileNotFoundError("Could not find semantic system directory in project root")
    ...
    def _load_playbook(self) -> Dict[str, Any]:
        ...
        try:
            ...
            playbook_path = (self.project_root / f'{system_name}' / 'agents' /
                            self.agent_name.lower() / 'playbook' /
    ...
    def _prepare_template_context(self, playbook: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not context['evaluation']:
                context['evaluation'] = {
                    'builtin_metrics': [{'name': 'answer_correctness', 'threshold': 0.7}],
                    'threshold': 0.7
                }
            ...
            if missing_fields:
                raise ValueError(f"Missing critical fields in playbook: {', '.join(missing_fields)}")
            ...
            if not context['tasks']:
                ...
                context['tasks'] = [{
                    'name': 'default_task',
                    'inputs': [{'name': 'query', 'type': 'str', 'description': 'User input'}],
                    'outputs': [{'name': 'response', 'type': 'str', 'description': 'Generated response'}]
                }]
            ...
            if 'persona' in spec:
                ...
                if 'job_description' in persona:
                    persona['job_description'] = self._format_docstring(persona['job_description'])
    ...
    def generate(self) -> Path:
        ...
        try:
            ...
            tier_issues = tier_system.validate_playbook_features(playbook, self.user_tier)
            ...
            code = re.sub(r'\n\s*\n\s*\n+', '\n\n', code)  # Replace multiple newlines with double
            ...
            # Save pipeline file
            pipeline_dir = (self.project_root /
                          f'{system_name}' /
                          'agents' /
...
🛠️  **Alternative:** You can modify your playbook to use features available in your current tier.
        ...
        raise RuntimeError(f"Tier restrictions prevent generation. Upgrade required for: {', '.join(tier_issues['blocked_features'])}")
    ...
    def _select_template_for_tier(self, playbook: Dict[str, Any]) -> Path:
        ...
        if spec.get("components", {}).get("tools") and tier_system.check_feature_access("tool_integration", self.user_tier):
