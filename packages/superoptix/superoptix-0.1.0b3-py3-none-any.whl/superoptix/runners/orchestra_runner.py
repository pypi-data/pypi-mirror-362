...
class EnhancedOrchestraRunner:
    ...
    async def _run_basic_style(
        ...
        if execution_strategy == ExecutionStrategy.SEQUENTIAL:
            ...
            self.console.print("[yellow]⚠️ Only sequential execution is supported in current version[/]")
            return await self._execute_sequential(orchestra.tasks, input_data)
    ...
    async def _write_task_output_to_workspace(self, task: Any, result: Any) -> None:
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                output_fields = [
                    'implementation', 'code', 'test_plan', 'analysis',
                    'response', 'output', 'content', 'solution'
                ]
                ...
                for field in output_fields:
                    if field in result and isinstance(result[field], str):
                        ...
                        self.console.print(f"[cyan]📝 Wrote {field} to:[/] {field_file}")

        except Exception as e:
            self.console.print(f"[yellow]⚠️ Warning: Could not write task output to workspace: {e}[/]")
