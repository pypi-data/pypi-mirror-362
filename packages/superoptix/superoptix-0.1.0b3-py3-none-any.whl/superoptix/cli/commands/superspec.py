...
from ...superspec import (
    SuperSpecXGenerator, SuperSpecXValidator, 
    SuperSpecXParser, SuperSpecXSchema
)
...
def _get_super_output_path(agent_name: str, tier: str, format: str = "yaml") -> str:
    ...
    try:
        ...
        if super_file:
            ...
            agent_dir = project_root / project_name / "agents" / safe_agent_name / "playbook"
            agent_dir.mkdir(parents=True, exist_ok=True)
            
            filename = f"{safe_agent_name}_playbook.{format}"
            output_path = agent_dir / filename
            
            console.print(f"📁 Using SuperOptiX project structure: {output_path.relative_to(project_root)}")
            ...
            console.print(f"📁 No SuperOptiX project detected, using current directory: {filename}")
            return filename
        ...
        safe_name = agent_name.lower().replace(" ", "_").replace("-", "_")
        filename = f"{safe_name}_{tier}_playbook.{format}"
        console.print(f"⚠️  Error determining project structure ({e}), using: {filename}")
        return filename
...
@click.option("--tier", "-t", type=click.Choice(["oracles", "genies"]), required=True,
              help="🎯 Agent tier: oracle (basic) or genie (advanced)")
@click.option("--name", "-n", required=True, help="🤖 Agent name (e.g., 'data-analyst', 'customer-support')")
@click.option("--namespace", "-ns", default="software", 
...
@click.option("--output", "-o", help="📁 Output directory or file path (auto-creates filename if directory)")
@click.option("--format", "-f", type=click.Choice(["yaml", "json"]), default="yaml",
              help="📄 Output format (yaml or json)")
@click.option("--memory/--no-memory", default=True, help="💾 Enable memory system (Genie only)")
@click.option("--tools/--no-tools", default=True, help="🔧 Enable tool integration (Genie only)")
@click.option("--rag/--no-rag", default=False, help="🔍 Enable RAG/retrieval (Genie only)")
def generate(tier: str, name: str, namespace: str, role: str, description: str,
             output: str, format: str, memory: bool, tools: bool, rag: bool):
    ...
    try:
        ...
        if generator.save_template(playbook, output, format):
            console.print(f"✅ Generated {tier} agent playbook: [bold green]{output}[/bold green]")
...
@click.option("--pattern", "-p", default="*.yaml", help="🔍 File pattern to match (default: *.yaml)")
@click.option("--format", "-f", type=click.Choice(["table", "json"]), default="table",
              help="📊 Output format (table or json)")
...
def info(file: str):
    ...
    if spec.agentflow:
        ...
        for step in spec.agentflow:
            depends = f" (depends: {', '.join(step.depends_on)})" if step.depends_on else ""
    ...
    # Validation check
    validator = SuperSpecXValidator()
    result = validator.validate(spec.__dict__ if hasattr(spec.__dict__, 'get') else {
        ...
        "spec": {
            ...
            "agentflow": [step.__dict__ for step in spec.agentflow] if spec.agentflow else None,
            "memory": spec.memory,
...
@superspec_cli.command()
@click.option("--tier", "-t", type=click.Choice(["oracles", "genies"]), 
...
def schema(tier: Optional[str]):
    ...
    if tier:
        ...
        if "agentflow_types" in features:
            console.print(Panel(
                "\n".join(f"• {step_type}" for step_type in features["agentflow_types"]),
                title="🔄 Allowed AgentFlow Steps"
            ))
        
        if "forbidden_features" in features:
            console.print(Panel(
                "\n".join(f"• {feature}" for feature in features["forbidden_features"]),
        ...
        if "allowed_features" in features:
            console.print(Panel(
                "\n".join(f"• {feature}" for feature in features["allowed_features"]),
...
@click.option("--output-dir", "-o", default="./generated_agents",
              help="📁 Output directory for generated agents")
@click.option("--tiers", "-t", multiple=True, type=click.Choice(["oracles", "genies"]),
              default=["oracles", "genies"], help="🎯 Tiers to generate (oracle, genie)")
def bootstrap(namespace: str, output_dir: str, tiers: List[str]):
    ...
    if generated_files:
        ...
        console.print(f"  2. Validate: [cyan]super spec validate {output_dir}/*.yaml[/cyan]")
