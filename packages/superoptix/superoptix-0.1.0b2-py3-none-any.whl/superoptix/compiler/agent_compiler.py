...
class AgentCompiler:
    ...
    def _find_project_root(self) -> Path:
        ...
        raise FileNotFoundError("Could not find .super file. Please run 'super init <project_name>' first.")

    def _load_playbook_and_get_context(self, agent_name: str, tier_level: str = None) -> Dict[str, Any]:
        ...
        playbook_path = next((self.project_root / system_name / 'agents').rglob(f"**/{agent_name}_playbook.yaml"), None)

        if not playbook_path:
            # Fallback to searching the source agents directory
            package_root = Path(__file__).parent.parent.parent
            playbook_path = next(package_root.rglob(f"**/agents/**/{agent_name}_playbook.yaml"), None)
            if not playbook_path:
    ...
    def _get_pipeline_path(self, agent_name: str) -> Path:
    ...
    def _get_template_for_tier(self, tier_level: str, use_abstracted: bool = False) -> str:
    ...
    def compile(self, args, tier_level: str = None, use_abstracted: bool = False) -> None:
        ...
        try:
            ...
            pipeline_path = self._get_pipeline_path(agent_name)
            pipeline_path.parent.mkdir(parents=True, exist_ok=True)

            # Get the appropriate template for the tier and preferences
            ...
            if use_abstracted:
                console.print("[cyan]🏗️  Infrastructure Abstraction: Auto-tracing + Tool management + Spec execution[/]")
                console.print("[green]🔧 Developer Controls: DSPy signatures + modules + forward logic[/]")
                console.print("[yellow]⚠️  Warning: Full abstraction - may break existing commands[/]")
            else:
                # Default mixin behavior (DSPy)
                console.print("[cyan]🧩 Mixin Pipeline (DSPy Default): Reusable components for complex agents.[/]")
                console.print("[green]🔧 Developer Controls: Modular mixins keep your codebase clean and customizable[/]")
                console.print("[magenta]🚀 Framework: DSPy (additional frameworks & custom builders coming soon) [/]")

            if effective_tier == 'genies':
                console.print("[cyan]🔧 Genies-Tier Features: ReAct Agents + Tool Integration + RAG Support + Memory[/]")
            else:
                console.print("[cyan]🔧 Oracles-Tier Features: Basic Chain of Thought + Sequential Orchestra[/]")
            ...
            console.print(f"✅ Successfully generated {tier_display}-tier pipeline{approach_note} at: {pipeline_path}")

            # Show guidance based on approach
            if use_abstracted:
                console.print("\n[dim]💡 Abstracted pipeline features:[/]")
                console.print("[dim]   • Infrastructure auto-handled by SuperOptixPipeline[/]")
                console.print("[dim]   • Focus on DSPy signatures and forward logic[/]")
                ...
                console.print("[dim]   • Separates pipeline logic into reusable mixins[/]")
                console.print("[dim]   • Ideal for building complex agents with shared components[/]")
                console.print("[dim]   • Built on DSPy – support for additional frameworks is on our roadmap[/]")
    ...
    def _extract_tier_level(self, playbook_snake_case: Dict[str, Any], user_tier: str = None) -> str:
        ...
        if effective_tier not in valid_tiers:
            console.print(f"[red]❌ Invalid tier '{effective_tier}'. Only Oracles and Genies tiers are supported in current version.[/]")
        ...
        if effective_tier == 'genies':
            ...
            spec = playbook_snake_case.get("spec", {})
            if 'tools' in spec:
                console.print("[green]✅ Tools configuration detected for Genies tier[/]")
            if 'react_config' in spec:
                console.print("[green]✅ ReAct configuration detected for Genies tier[/]")
            elif 'tool_calling' in spec:
                console.print("[green]✅ Tool calling configuration detected for Genies tier[/]")
            if 'memory' in spec:
                console.print("[green]✅ Memory configuration detected for Genies tier[/]")
