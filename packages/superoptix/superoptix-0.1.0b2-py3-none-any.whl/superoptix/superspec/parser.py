...
class SuperSpecXParser:
    ...
    def parse_dict(self, data: Dict[str, Any], source: str = "dict") -> Optional[AgentSpec]:
        """
    ...
    def parse_directory(self, directory_path: Union[str, Path], pattern: str = "*.yaml") -> List[AgentSpec]:
        """
    ...
    def find_specs_by_tier(self, tier: str) -> List[AgentSpec]:
        ...
        return [spec for spec in self.parsed_specs.values() if spec.metadata.level == tier]
    
    def find_specs_by_namespace(self, namespace: str) -> List[AgentSpec]:
        """Find all specs matching a specific namespace."""
        return [spec for spec in self.parsed_specs.values() if spec.metadata.namespace == namespace]
    ...
    def get_parsing_summary(self) -> Dict[str, Any]:
        ...
        feature_usage = {}
        features_to_check = ["memory", "tool_calling", "retrieval", "agentflow", "optimization"]
    ...
    def export_to_json(self, spec: AgentSpec, file_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            optional_fields = [
                "retrieval", "memory", "tool_calling", "evaluation", 
                "optimization", "training", "runtime", "observability", 
                "s3cur1ty", "feature_specifications"
            ]
        ...
        if tier == "oracles":
            for feature in genies_features:
                if getattr(spec, feature) is not None:
                    issues.append(f"Feature '{feature}' requires Genies tier but agent is Oracles tier")
        ...
        if spec.agentflow:
            ...
            for step in spec.agentflow:
                if step.type not in allowed_types:
                    issues.append(f"Agentflow step type '{step.type}' not allowed for {tier} tier")
