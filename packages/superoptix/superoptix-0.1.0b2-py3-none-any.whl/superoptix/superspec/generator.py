...
class SuperSpecXGenerator:
    ...
    def generate_genies_template(
        ...
        template = {
            ...
            "metadata": {
                ...
                "stage": "alpha",
                "agent_type": "Autonomous",
                            "description": description or f"Genies tier {role.lower()} with tools and memory",
        ...
        if enable_memory:
            template["spec"]["memory"] = {
                ...
                "backend": {
                    "type": "sqlite",
                    "config": {
                        "db_path": f".superoptix/{agent_id}_memory.db"
                    }
                },
                "short_term": {
                    "enabled": True,
        ...
        if enable_rag:
            ...
            template["spec"]["agentflow"].insert(1, {
    ...
    def _generate_feature_specifications(self, namespace: str, role: str, tier: str, task_inputs: List[Dict], task_outputs: List[Dict]) -> Dict:
        ...
        input_fields = {inp['name']: inp.get('description', 'Input field') for inp in task_inputs}
        output_fields = {out['name']: out.get('description', 'Output field') for out in task_outputs}
        ...
        scenarios = self._generate_namespace_scenarios(namespace, role, primary_input, primary_output)
        
        # Add tier-specific scenarios
        if tier == 'genies':
            scenarios.extend(self._generate_genies_scenarios(namespace, role, primary_input, primary_output))
        
        # Ensure we have at least 3 scenarios for optimization
        while len(scenarios) < 3:
            scenarios.append(self._generate_generic_scenario(role, primary_input, primary_output, len(scenarios) + 1))
    ...
    def _generate_namespace_scenarios(self, namespace: str, role: str, input_field: str, output_field: str) -> List[Dict]:
    ...
    def _generate_genies_scenarios(self, namespace: str, role: str, input_field: str, output_field: str) -> List[Dict]:
    ...
    def _generate_generic_scenario(self, role: str, input_field: str, output_field: str, index: int) -> Dict:
    ...
    def _generate_evaluation_config(self, tier: str) -> Dict:
        ...
        base_metrics = [
            {
                            'name': 'answer_exact_match' if tier == 'oracles' else 'answer_correctness',
            'threshold': 1.0 if tier == 'oracles' else 0.8
            }
        ]
        
        return {
            'builtin_metrics': base_metrics
        }
    ...
    def _generate_optimization_config(self, tier: str) -> Dict:
        ...
        return {
            'strategy': 'few_shot_bootstrapping',
            'metric': 'answer_exact_match' if tier == 'oracles' else 'answer_correctness',
            'metric_threshold': 0.9 if tier == 'oracles' else 0.7,
            'few_shot_bootstrapping_config': {
                'max_bootstrapped_demos': 4,
                'max_rounds': 1
            }
    ...
    def _generate_goal(self, role: str, namespace: str) -> str:
        ...
        return role_goals.get(role.lower(), f'Provide expert {role.lower()} services in {namespace} domain')
    ...
    def _generate_traits(self, role: str, namespace: str) -> List[str]:
        ...
        return base_traits + role_specific_traits.get(role.lower(), ['focused', 'reliable'])
    ...
    def _generate_instruction(self, role: str, namespace: str) -> str:
        ...
        return instructions.get(role.lower(), f'You are a {role.lower()} specializing in {namespace}. Provide professional assistance.')
    ...
    def _generate_task_io(self, role: str, namespace: str) -> tuple:
        ...
        role_inputs = {
            'developer': [
                {'name': 'feature_requirement', 'type': 'str', 'description': 'Feature specification or coding task', 'required': True}
            ],
            'analyst': [
                {'name': 'data_query', 'type': 'str', 'description': 'Data analysis request or question', 'required': True}
            ],
            'assistant': [
                {'name': 'user_query', 'type': 'str', 'description': 'User question or request for assistance', 'required': True}
            ],
            'manager': [
                {'name': 'workflow_request', 'type': 'str', 'description': 'Workflow or management task', 'required': True}
            ],
            'consultant': [
                {'name': 'consultation_request', 'type': 'str', 'description': 'Business or strategic consultation request', 'required': True}
            ]
        }
        ...
        role_outputs = {
            'developer': [
                {'name': 'implementation', 'type': 'str', 'description': 'Code implementation or technical solution'}
            ],
            'analyst': [
                {'name': 'analysis_report', 'type': 'str', 'description': 'Data analysis results and insights'}
            ],
            'assistant': [
                {'name': 'assistance_response', 'type': 'str', 'description': 'Helpful response to user query'}
            ],
            'manager': [
                {'name': 'workflow_plan', 'type': 'str', 'description': 'Organized workflow or management plan'}
            ],
            'consultant': [
                {'name': 'consultation_advice', 'type': 'str', 'description': 'Expert advice and recommendations'}
            ]
        }
        
        # Default fallback
        default_inputs = [{'name': 'query', 'type': 'str', 'description': 'User request or question', 'required': True}]
        default_outputs = [{'name': 'response', 'type': 'str', 'description': 'Professional response'}]
    ...
    def generate_playbook(self, tier: str, role: str, namespace: str = "software") -> Dict:
