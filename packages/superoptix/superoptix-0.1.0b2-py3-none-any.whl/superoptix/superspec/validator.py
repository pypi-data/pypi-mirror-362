...
class SuperSpecXValidator:
    ...
    def _validate_root_structure(self, playbook_data: Dict[str, Any]):
        ...
        if "apiVersion" in playbook_data:
            if playbook_data["apiVersion"] != "agent/v1":
                self.errors.append(f"Invalid apiVersion: {playbook_data['apiVersion']}. Must be 'agent/v1'")
        
        # Validate kind
        if "kind" in playbook_data:
            if playbook_data["kind"] != "AgentSpec":
                self.errors.append(f"Invalid kind: {playbook_data['kind']}. Must be 'AgentSpec'")
    ...
    def _validate_metadata(self, playbook_data: Dict[str, Any]):
        ...
        if tier not in valid_tiers:
            self.errors.append(f"Invalid tier '{tier}'. Valid options: {', '.join(valid_tiers)}")
        ...
        if "namespace" in metadata:
            ...
            if metadata["namespace"] not in valid_namespaces:
                self.warnings.append(f"Namespace '{metadata['namespace']}' not in standard list")
        
        # Validate agent_type
        if "agent_type" in metadata:
            valid_types = ["Autonomous", "Supervised", "Interactive", "Reactive", "Deliberative", "Hybrid"]
            if metadata["agent_type"] not in valid_types:
                self.errors.append(f"Invalid agent_type '{metadata['agent_type']}'. Valid options: {', '.join(valid_types)}")
        
        # Validate stage
        if "stage" in metadata:
            valid_stages = ["alpha", "beta", "stable"]
            if metadata["stage"] not in valid_stages:
                self.errors.append(f"Invalid stage '{metadata['stage']}'. Valid options: {', '.join(valid_stages)}")
        ...
        if "version" in metadata:
            ...
            if not re.match(version_pattern, metadata["version"]):
                self.errors.append(f"Invalid version format '{metadata['version']}'. Must be semantic versioning (e.g., '1.0.0')")
    ...
    def _validate_spec(self, playbook_data: Dict[str, Any]):
        ...
        if "persona" not in spec:
            self.warnings.append("No persona defined - recommended for better agent behavior")
        else:
        ...
        if "agentflow" in spec:
            self._validate_agentflow(spec["agentflow"], playbook_data.get("metadata", {}).get("level", "oracles"))
    ...
    def _validate_language_model(self, lm_config: Dict[str, Any]):
        ...
        if "location" in lm_config:
            ...
            if lm_config["location"] not in valid_locations:
                self.errors.append(f"Invalid location '{lm_config['location']}'. Valid options: {', '.join(valid_locations)}")
        ...
        if "max_tokens" in lm_config:
            ...
            if not isinstance(max_tokens, int) or max_tokens < 1:
                self.errors.append(f"max_tokens must be a positive integer, got: {max_tokens}")
        ...
        if "modalities" in lm_config:
            ...
            for modality in lm_config["modalities"]:
                if modality not in valid_modalities:
                    self.errors.append(f"Invalid modality '{modality}'. Valid options: {', '.join(valid_modalities)}")
    
    def _validate_persona(self, persona: Dict[str, Any]):
        """Validate persona configuration."""
        if "role" not in persona:
            self.warnings.append("Persona missing 'role' field - recommended for clarity")
        ...
        if "communication_preferences" in persona:
            ...
            if "style" in comm_prefs:
                ...
                if comm_prefs["style"] not in valid_styles:
                    self.warnings.append(f"Invalid communication style '{comm_prefs['style']}'")
            
            if "tone" in comm_prefs:
                valid_tones = ["professional", "friendly", "authoritative", "supportive"]
                if comm_prefs["tone"] not in valid_tones:
                    self.warnings.append(f"Invalid communication tone '{comm_prefs['tone']}'")
        ...
        for i, input_field in enumerate(inputs):
            ...
            for field in required_fields:
                if field not in input_field:
                    self.errors.append(f"Task {task_num} input {i+1} missing required field: {field}")
            
            if "type" in input_field and input_field["type"] not in valid_types:
                self.errors.append(f"Task {task_num} input {i+1} invalid type: {input_field['type']}")
        ...
        for i, output_field in enumerate(outputs):
            ...
            for field in required_fields:
                if field not in output_field:
                    self.errors.append(f"Task {task_num} output {i+1} missing required field: {field}")
            
            if "type" in output_field and output_field["type"] not in valid_types:
                self.errors.append(f"Task {task_num} output {i+1} invalid type: {output_field['type']}")
    ...
    def _validate_agentflow(self, agentflow: List[Dict[str, Any]], tier: str):
        ...
        genies_types = ["Generate", "Think", "ActWithTools", "Search", "Compare", "Route"]
        ...
        for i, step in enumerate(agentflow):
            ...
            for field in required_fields:
                if field not in step:
                    self.errors.append(f"Agentflow step {i+1} missing required field: {field}")
            ...
            if "type" in step:
                if step["type"] not in allowed_types:
                    self.errors.append(f"Step type '{step['type']}' not allowed for {tier} tier")
    ...
    def _validate_tier_features(self, playbook_data: Dict[str, Any]):
        ...
        genies_only_features = [
            "retrieval", "memory", "tool_calling", "react_config", 
            "rag_config", "streaming"
        ]
        ...
        if "backend" in memory_config:
            ...
            if isinstance(backend, dict) and "type" in backend:
                ...
                if backend["type"] not in valid_backends:
                    self.errors.append(f"Invalid memory backend type: {backend['type']}")
        ...
        if "short_term" in memory_config:
            ...
            if "capacity" in short_term:
                ...
                if not isinstance(capacity, int) or capacity < 10 or capacity > 1000:
                    self.warnings.append(f"Short-term memory capacity should be between 10 and 1000, got: {capacity}")
    ...
    def _validate_tool_calling_config(self, tool_config: Dict[str, Any]):
        ...
        if "max_tool_calls" in tool_config:
            ...
            if not isinstance(max_calls, int) or max_calls < 1 or max_calls > 20:
                self.warnings.append(f"max_tool_calls should be between 1 and 20, got: {max_calls}")
    ...
    def _validate_retrieval_config(self, retrieval_config: Dict[str, Any]):
        ...
        if "retriever_type" in retrieval_config:
            valid_retrievers = ["ColBERTv2", "Weaviate", "ChromaDB", "Pinecone", "FAISS", "Custom"]
            if retrieval_config["retriever_type"] not in valid_retrievers:
                self.errors.append(f"Invalid retriever_type: {retrieval_config['retriever_type']}")
        ...
        if "optimization" in spec:
            ...
            if "strategy" in opt_config:
                ...
                if strategy not in ["few_shot_bootstrapping"]:
                    self.errors.append(f"Advanced optimization strategy '{strategy}' not available in current version")
        ...
        if "orchestration" in spec:
            ...
            if "strategy" in orch_config:
                ...
                if strategy not in ["sequential"]:
                    self.errors.append(f"Advanced orchestration strategy '{strategy}' not available in current version")
        
        # Check for enterprise features
        enterprise_features = [
            "white_label", "custom_branding", "enterprise_s3cur1ty",
            "professional_services", "custom_teleprompt"
        ]
        
        for feature in enterprise_features:
            if feature in spec:
                self.errors.append(f"Enterprise feature '{feature}' not available in current version")
