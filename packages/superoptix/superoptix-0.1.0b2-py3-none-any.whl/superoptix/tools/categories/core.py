...
class JSONProcessorTool:
    ...
    def extract_json_field(self, json_string: str, field_path: str) -> str:
        ...
        try:
            ...
            return f"📄 {field_path}: {json.dumps(current, indent=2, ensure_ascii=False)}"
...
"""
                ...
                return summary if numeric_cols else "No numeric columns found for summary"
    ...
    def _format_table(self, headers: List[str], rows: List[List[str]]) -> str:
        ...
        for row in rows:
            row_str = '|' + '|'.join([f" {str(row[i]):<{col_widths[i]}} " if i < len(row) else f" {'':>{col_widths[i]}} " for i in range(len(headers))]) + '|'
            table += row_str + '\n'
