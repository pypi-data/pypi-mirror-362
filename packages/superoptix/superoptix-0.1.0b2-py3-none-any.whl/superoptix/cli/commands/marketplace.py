...
def browse_marketplace(args):
    ...
    if item_type == 'agents':
        ...
        console.print("[red]❌ Please specify what to browse: agents, tools, industries, or categories[/]")
        console.print("[cyan]💡 Examples:[/]")
        console.print("   [bold]super marketplace browse agents[/bold]")
        console.print("   [bold]super marketplace browse tools --category research[/bold]")
...
def browse_agents(args):
    ...
    console.print("   [bold]super marketplace show <agent_name>[/bold]    - View detailed info")
    console.print("   [bold]super marketplace deps <agent_name>[/bold]    - Check tool dependencies")
    console.print("   [bold]super marketplace install agent <name>[/bold] - Quick install (same as super agent pull)")
...
def browse_tools(args):
    ...
    if category_filter:
        ...
        cat_items = [(cat, len(registry.get_tools_by_category(cat))) for cat in categories]

        # Display categories in columns
        rows = [cat_items[i:i+3] for i in range(0, len(cat_items), 3)]
        for row in rows:
            cat_table.add_row(*[f"• [blue]{cat.replace('_', ' ').title()}[/blue] ({count})" for cat, count in row])
    ...
    console.print("\n[cyan]🛍️ Marketplace Actions:[/cyan]")
    console.print("   [bold]super marketplace show <tool_name>[/bold]     - View detailed info")
    console.print("   [bold]super marketplace examples <tool_name>[/bold] - See usage examples")
    if not category_filter:
        console.print("   [bold]super marketplace browse tools --category <name>[/bold] - Filter by category")
...
def browse_industries(args):
    ...
    console.print("🏢 [bold bright_magenta]Marketplace: Industries[/bold bright_magenta]")
    console.print("═" * 50)
...
def browse_categories(args):
    ...
    console.print("📂 [bold bright_blue]Marketplace: Tool Categories[/bold bright_blue]")
    console.print("═" * 50)
...
def search_marketplace(args):
    ...
    query = args.query
    console.print(f"🔍 [bold bright_cyan]Marketplace Search:[/bold bright_cyan] '{query}'")
    console.print("═" * 60)
    ...
    if total_results == 0:
        ...
        console.print("   • Browse by category: [bold]super marketplace browse agents[/bold]")
        console.print("   • Browse by industry: [bold]super marketplace browse industries[/bold]")
        return
    ...
    if agent_results:
        ...
        agent_table = Table(show_header=True, header_style="bold cyan", border_style="cyan")
    ...
    if tool_results:
        ...
        tool_table = Table(show_header=True, header_style="bold yellow", border_style="yellow")
    ...
    console.print("   [bold]super marketplace show <name>[/bold]    - View detailed info")
    console.print("   [bold]super marketplace install <type> <name>[/bold] - Install component")
...
def show_featured(args):
    ...
    console.print("⭐ [bold bright_green]Marketplace: Featured Components[/bold bright_green]")
    console.print("═" * 55)

    # Curated featured items
    featured_agents = [
        {'name': 'developer', 'industry': 'software', 'reason': 'Most versatile coding assistant'},
        {'name': 'financial_advisor', 'industry': 'finance', 'reason': 'Popular for financial analysis'},
        {'name': 'content_creator', 'industry': 'marketing', 'reason': 'Great for content generation'},
    ]

    featured_tools = [
        {'name': 'WebSearchTool', 'category': 'research', 'reason': 'Essential for information gathering'},
        {'name': 'CalculatorTool', 'category': 'computation', 'reason': 'Most used mathematical tool'},
        {'name': 'FileReaderTool', 'category': 'data_access', 'reason': 'Critical for document processing'},
    ]
...
def install_component(args):
    ...
    if component_type == 'agent':
        ...
        console.print("[dim]Note: Tools are automatically available once installed. No additional setup needed.[/dim]")
...
def _get_agent_stats():
    ...
    stats = {'total': 0, 'industries': 0, 'oracle': 0, 'genie': 0, 'autonomous': 0, 'supervised': 0}
                ...
                with open(playbook_file) as f:
                    ...
                    if data and 'metadata' in data:
                        ...
                        if (query_lower in meta.get('name', '').lower() or
                            ...
                            query_lower in meta.get('namespace', '').lower() or
                            query_lower in playbook_file.stem.lower()):
...
[bold]Tags:[/] {', '.join(metadata.tags) if metadata.tags else 'None'}"""
    ...
    console.print(Panel(info_content, title="📋 Tool Information", border_style="yellow"))
...
[cyan]from superoptix.tools.categories.{metadata.category.lower()} import {metadata.name.replace('_', '').title()}Tool[/cyan]"""
    ...
    console.print(f"   [bold]super marketplace browse tools --category {metadata.category.lower()}[/bold] - Similar tools")
    console.print(f"   [bold]super marketplace search {metadata.industry}[/bold] - Industry tools")
...
def _show_agent_details(agent_info):
    ...
    console.print(f"🤖 [bold bright_cyan]{metadata.get('name', 'Unknown Agent')}[/bold bright_cyan]")
    console.print("═" * 60)
...
[bold]License:[/] {metadata.get('license', 'MIT')}"""
    ...
    console.print(Panel(info_content, title="📋 Agent Information", border_style="cyan"))
    ...
    if capabilities:
        ...
        for cap in capabilities[:5]:  # Show top 5
            cap_table.add_row(cap.get('name', 'Unknown'), cap.get('description', 'No description'))

        console.print(cap_table)
        if len(capabilities) > 5:
            console.print(f"[dim]... and {len(capabilities) - 5} more capabilities[/dim]")
    ...
    console.print(f"   [bold]super marketplace install agent {agent_info['ref']}[/bold] - Add to project")
    console.print(f"   [bold]super marketplace deps {agent_info['ref']}[/bold] - Check dependencies")
    console.print(f"   [bold]super marketplace search {metadata.get('namespace', '')}[/bold] - Similar agents")
