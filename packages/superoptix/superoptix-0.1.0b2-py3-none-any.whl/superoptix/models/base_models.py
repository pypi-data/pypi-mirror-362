...
class TierValidator:
    ...
    def validate_orchestration_access(
        ...
        execution_strategy = orchestra_config.get("execution", {}).get("strategy", "sequential")
        ...
        max_parallel = orchestra_config.get("execution", {}).get("max_parallel_tasks", 1)
        if max_parallel > 1:
            errors.append(
...
class ExecutionConfig(BaseModel):
    ...
    strategy: ExecutionStrategy = Field(
        default=ExecutionStrategy.SEQUENTIAL, description="Execution strategy - sequential only."
    )
    max_parallel_tasks: int = Field(
        default=1, description="Maximum number of tasks to run in parallel - always 1 for sequential."
    )
    ...
    enable_trace: bool = Field(
        default=True, description="Enable execution tracing."
    )
...
class OrchestraFile(BaseModel):
    ...
    @validator("orchestra")
    def validate_tier_compatibility(cls, v, values):
        """Validate that the orchestra configuration is compatible with the required tier."""
        if v.required_tier and v.required_tier not in [AgentTier.ORACLES, AgentTier.GENIES]:
            raise ValueError(f"Unsupported tier: {v.required_tier}. Only Oracles and Genies tiers are supported.")
        return v
