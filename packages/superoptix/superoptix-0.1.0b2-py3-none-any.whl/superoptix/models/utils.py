...
def parse_superoptix_model_name(model_name: str) -> tuple[Optional[SuperOptiXBackendType], str]:
...
def validate_superoptix_model_compatibility(model_info: SuperOptiXModelInfo) -> tuple[bool, List[str]]:
    ...
    if model_info.backend == SuperOptiXBackendType.MLX:
        if not system_info["has_metal"]:
            issues.append("MLX models require Apple Silicon (M1/M2/M3) with Metal support")
    ...
    if model_info.parameters:
        ...
        if "70b" in param_count or "72b" in param_count:
            issues.append("Large models (70B+) require significant RAM (64GB+ recommended)")
        elif "30b" in param_count or "33b" in param_count:
            issues.append("Medium-large models (30B+) require substantial RAM (32GB+ recommended)")
