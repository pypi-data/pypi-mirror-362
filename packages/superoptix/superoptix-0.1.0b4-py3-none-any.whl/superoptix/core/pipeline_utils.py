...
class ModelSetupMixin:
    ...
    def setup_language_model(self, spec: Dict[str, Any], tier: str = "mid") -> dspy.LM:
        ...
        print(f"🚀 Configuring {model_name} with {provider} for {tier}-tier capabilities")
        ...
        if ChatAdapter and provider.lower() == "ollama":
            ...
            if not isinstance(current_adapter, ChatAdapter):
                ...
                print("🔁 ChatAdapter already configured – skipping reconfiguration to avoid DSPy async conflicts")

        # Build LM configuration
        lm_config = {
            "model": f"{provider}_chat/{model_name}" if provider == "ollama" else model_name,
            "temperature": model_config.get("temperature", self._get_default_temperature(tier)),
            "max_tokens": model_config.get("max_tokens", self._get_default_max_tokens(tier)),
        }
            ...
            if hasattr(self, 'tracer'):
                self.tracer.add_event(
                    ...
                    data={
                        ...
                        "provider": provider,
                        "tier": tier,
                        "adapter": "ChatAdapter" if ChatAdapter and provider == "ollama" else "default"
                    },
                    status="success"
                )
...
class ToolsMixin:
        ...
        try:
            ...
            if hasattr(self, 'tracer'):
                self.tracer.add_event(
                    "tools_initialized",
                    "pipeline",
                    data={"tool_count": tool_count, "tools": [getattr(tool, 'name', str(tool)) for tool in self.tools]},
                    status="success"
                )
    ...
    def _setup_builtin_tools(self, builtin_tools):
        ...
        for tool_config in builtin_tools:
            ...
            if tool_name == 'calculator':
                ...
                tool_count += self._add_text_analyzer_tool(tool_config.get('config', {}))
                ...
                tool_count += self._add_json_processor_tool(tool_config.get('config', {}))
    ...
    def _setup_custom_tools(self, custom_tools):
        ...
        for tool_config in custom_tools:
            ...
            if not all(field in tool_config for field in required_fields):
                print(f"⚠️ Skipping custom tool: missing required fields {required_fields}")
                continue

            try:
                tool_count += self._add_custom_tool(tool_config)
            except Exception as e:
                print(f"⚠️ Failed to add custom tool '{tool_config.get('name', 'unknown')}': {str(e)}")
    ...
    def _add_custom_tool(self, tool_config):
        ...
        def custom_tool_wrapper(*args, **kwargs):
            ...
            if hasattr(self, 'tracer'):
                with self.tracer.trace_operation(f"custom_{tool_name}", "tool", args=args, kwargs=kwargs):
                    return self._execute_custom_tool(tool_name, function_name, implementation, *args, **kwargs)
            else:
                return self._execute_custom_tool(tool_name, function_name, implementation, *args, **kwargs)
    ...
    def _execute_custom_tool(self, tool_name, function_name, implementation, *args, **kwargs):
    ...
    def _add_calculator_tool(self, config):
        ...
        def calculate(expression: str) -> str:
            ...
            if hasattr(self, 'tracer'):
                with self.tracer.trace_operation("calculate", "tool", expression=expression):
    ...
    def _add_text_analyzer_tool(self, config):
        ...
        def analyze_text(text: str) -> str:
            ...
            if hasattr(self, 'tracer'):
                with self.tracer.trace_operation("analyze_text", "tool", text_length=len(text)):
    ...
    def _add_file_reader_tool(self, config):
        ...
        allowed_extensions = config.get('allowed_extensions', ['txt', 'md', 'json', 'yaml', 'csv'])
        ...
        def read_file(file_path: str) -> str:
            ...
            if hasattr(self, 'tracer'):
                with self.tracer.trace_operation("read_file", "tool", file_path=file_path):
                    return self._execute_file_reader(file_path, allowed_extensions, max_file_size_mb)
            else:
                return self._execute_file_reader(file_path, allowed_extensions, max_file_size_mb)
    ...
    def _add_json_processor_tool(self, config):
        ...
        def process_json(json_string: str, operation: str = "parse") -> str:
            ...
            if hasattr(self, 'tracer'):
                with self.tracer.trace_operation("process_json", "tool", operation=operation):
    ...
    def _execute_file_reader(self, file_path: str, allowed_extensions: List[str] = None, max_file_size_mb: int = 5) -> str:
        """Execute file reader tool with error handling."""
        try:
            file_tool = FileReaderTool(
                allowed_extensions=allowed_extensions or ['txt', 'md', 'json', 'yaml', 'csv'],
    ...
    def _execute_web_search(self, query: str, search_engine: str = "duckduckgo", max_results: int = 3) -> str:
    ...
    def _execute_json_processor(self, json_string: str, operation: str = "parse") -> str:
...
class BDDTestMixin:
    ...
    def load_bdd_scenarios(self, spec_data: Dict[str, Any]) -> List[dspy.Example]:
        ...
        for scenario in scenarios:
            try:
                ...
                print(f"⚠️  Warning: Skipping invalid specification '{scenario.get('name', 'unknown')}': {e}")
                continue
    ...
    def run_bdd_test_suite(self, auto_tune: bool = False, ignore_checks: bool = False) -> Dict[str, Any]:
        ...
        with Progress(SpinnerColumn(), BarColumn(), TextColumn("{task.completed}/{task.total}"), transient=True) as prog:
            ...
            for scenario in self.test_examples:
                ...
                console.print(f"{symbol} {getattr(scenario, 'scenario_name', 'Unnamed')}", highlight=False)
        ...
        if ignore_checks:
            ...
            console.print("[yellow]⚠️  Validation checks ignored – treating all scenarios as passed.[/]")
    ...
    def _execute_single_scenario(self, scenario: dspy.Example) -> Dict[str, Any]:
        ...
        try:
            ...
            expected_outputs = {key: getattr(scenario, key) for key in scenario.labels()}
            ...
            if not expected_outputs:
                ...
                return {
                    ...
                    "failure_reason": None if passed else "Empty or no response generated",
                    "actual_output": str(result),
                    "expected_output": "Non-empty response"
                }

            # Comprehensive evaluation
            evaluation_result = self._evaluate_scenario_output(result, expected_outputs, scenario)
            ...
            return {
                ...
                "semantic_similarity": evaluation_result.get("semantic_similarity", 0.0),
    ...
    def _evaluate_scenario_output(self, actual_output: Any, expected_outputs: Dict[str, Any], scenario: dspy.Example) -> Dict[str, Any]:
        ...
        try:
            ...
            semantic_score = self._calculate_semantic_similarity(actual_str, expected_str)
    ...
    def _calculate_keyword_presence(self, actual: str, expected: str) -> float:
        ...
        common_words = {'the', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'can', 'this', 'that', 'these', 'those'}

        expected_keywords = {word.lower() for word in expected.split() if len(word) > 3 and word.lower() not in common_words}
        actual_keywords = {word.lower() for word in actual.split() if len(word) > 3 and word.lower() not in common_words}
    ...
    def _calculate_structure_score(self, actual: str, expected: str) -> float:
        ...
        line_ratio = min(actual_lines, expected_lines) / max(actual_lines, expected_lines)
    ...
    def _analyze_model_performance(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if avg_score < 0.6:
            ...
            suggested_upgrade = "Strongly recommend gpt-4 or claude-3 for reliable results"
        ...
        return {
            ...
            "performance_category": "excellent" if avg_score >= 0.8 else "good" if avg_score >= 0.6 else "needs_improvement"
        }

    def _generate_recommendations(self, results: List[Dict[str, Any]], pass_rate: float) -> List[str]:
        """Generate actionable recommendations based on test results."""
        recommendations = []

        if not results:
            recommendations.append("Add BDD scenarios to your agent playbook for proper testing")
            return recommendations

        failed_results = [r for r in results if not r.get("passed")]

        if pass_rate == 100:
            recommendations.append("Excellent! All scenarios pass. Consider adding more comprehensive test cases.")
            recommendations.append("Your agent is ready for production use.")
        elif pass_rate >= 80:
            recommendations.append(f"Good performance! {len(failed_results)} scenario(s) need minor improvements.")
            recommendations.append("Review failing scenarios and refine agent context.")
        elif pass_rate >= 60:
            recommendations.append(f"Moderate performance. {len(failed_results)} scenarios failing.")
            recommendations.append("Consider running optimization: super agent optimize <agent_name>")
            recommendations.append("Review and improve scenario expectations or agent capabilities.")
        else:
            recommendations.append(f"Poor performance. {len(failed_results)} scenarios failing.")
            recommendations.append("Strong recommendation: Run optimization before production use.")
            recommendations.append("Consider using a more capable model (llama3.1:8b or gpt-4).")
            recommendations.append("Review scenario complexity vs model capabilities.")

        # Analysis-based recommendations
        semantic_issues = sum(1 for r in failed_results if r.get("failure_reason", "").startswith("semantic"))
        keyword_issues = sum(1 for r in failed_results if "keyword" in r.get("failure_reason", ""))
        structure_issues = sum(1 for r in failed_results if "structure" in r.get("failure_reason", ""))

        if semantic_issues > 0:
            recommendations.append(f"Fix semantic relevance in {semantic_issues} scenario(s) - improve response clarity.")
        if keyword_issues > 0:
            recommendations.append(f"Add missing technical terms in {keyword_issues} scenario(s).")
        if structure_issues > 0:
            recommendations.append(f"Improve output formatting in {structure_issues} scenario(s).")

        return recommendations

    def create_examples(self, data: List[Dict[str, Any]], input_keys: List[str] | None = None) -> List[dspy.Example]:
...
class UsageTrackingMixin:
    ...
    def safe_update_usage_stats(self, cumulative_stats: Dict[str, Any], new_usage: Dict[str, Any]) -> None:
...
class EvaluationMixin:
    ...
    def semantic_f1_metric(self, gold: dspy.Example, pred: dspy.Prediction, input_field: str = "query", output_field: str = "response") -> float:
        """Semantic F1 evaluation metric with fallback."""
        try:
            if not SemanticF1:
                return self._fallback_similarity_metric(gold, pred, input_field, output_field)
            ...
            print(f"Warning: Semantic evaluation failed ({e}), using fallback similarity")
            return self._fallback_similarity_metric(gold, pred, input_field, output_field)

    def _fallback_similarity_metric(self, gold: dspy.Example, pred: dspy.Prediction, input_field: str, output_field: str) -> float:
    ...
    def evaluate_pipeline(self, test_data: List[Dict[str, Any]], input_field: str = "query", output_field: str = "response") -> Dict[str, Any]:
        ...
        try:
            ...
            for example in examples:
                try:
                    ...
                    # Calculate score
                    score = self.semantic_f1_metric(example, prediction, input_field, output_field)
                    scores.append(score)
...
class PipelineUtilities:
    ...
    @staticmethod
    def setup_language_model_quick(model_name: str = "llama3.1:8b", provider: str = "ollama", temperature: float = 0.1) -> dspy.LM:
    ...
    def get_optimizer(tier: str, k: int = 5):
        """Get appropriate optimizer for tier."""
        if tier == "oracles" and LabeledFewShot:
            return LabeledFewShot(k=k)
        elif tier == "genies" and BootstrapFewShot:
            return BootstrapFewShot(max_bootstrapped_demos=k)
        elif LabeledFewShot:
            return LabeledFewShot(k=k)
        else:
            # Return a dummy optimizer if none available
...
class TrainingMixin(UsageTrackingMixin, EvaluationMixin, BDDTestMixin):
    ...
    def _auto_prepare_examples(self, training_data: List[Dict[str, Any]], input_field: str, output_field: str):
        """Render a progress bar while copying data – purely cosmetic."""
        from rich.progress import Progress
        converted: List[Dict[str, Any]] = []
        with Progress() as progress:
            task = progress.add_task("[cyan]Preparing examples...", total=len(training_data))
        ...
        *,
        ...
        if not training_data:
            ...
            if scenarios:
                ...
                input_field = "feature_specification" if "feature_specification" in attrs else "query"
                output_candidates = [a for a in attrs if a not in (input_field, "reasoning", "query")]
        ...
        if hasattr(executor, "__annotations__"):
            required_inputs |= {k for k, v in executor.__annotations__.items() if k not in (output_field,)
                                 and k not in training_data[0]}
        ...
        training_data = self._auto_prepare_examples(training_data, input_field, output_field)
        ...
        try:
            with self.usage_tracking_context():
                with dspy.context(lm=self.lm):
                    ...
                    target = getattr(self, "react", None) or getattr(self, "module", None) or self
                    # Visual feedback while DSPy optimiser runs (can be slow)
                    from rich.progress import Progress, SpinnerColumn, TextColumn
                    with Progress(SpinnerColumn(), TextColumn("[cyan]{task.description}")) as pbar:
...
class CLITestShimMixin:
    ...
    def run_bdd_test_suite(self, auto_tune: bool = False, ignore_checks: bool = False):
        return super().run_bdd_test_suite(auto_tune=auto_tune, ignore_checks=ignore_checks)
...
class ExecutionMixin:
    ...
    async def forward(self, question: str, context: str = "") -> Dict[str, Any]:
        ...
        _ = next((p for p in preferred_order if p in param_names), param_names[0] if param_names else "query")  # noqa: F841
    ...
    def run(self, **inputs):
        ...
        question = inputs.get("feature_specification") or inputs.get("query") or inputs.get("question") or ""
