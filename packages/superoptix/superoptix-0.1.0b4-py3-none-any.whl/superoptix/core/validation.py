...
def validate_language_model_config(lm_config: Dict[str, Any]) -> bool:
    ...
    # Validate provider
    supported_providers = ['openai', 'ollama', 'anthropic', 'google', 'azure', 'mistral']
    if lm_config['provider'] not in supported_providers:
        raise ValidationError(f"Unsupported provider: {lm_config['provider']}. Supported: {supported_providers}")
    ...
    if 'max_tokens' in lm_config:
        ...
        if not isinstance(max_tokens, int) or max_tokens <= 0:
            raise ValidationError(f"max_tokens must be a positive integer, got: {max_tokens}")
...
def validate_task_definition(task: Dict[str, Any]) -> bool:
    ...
    if task['type'] not in supported_types:
        raise ValidationError(f"Unsupported task type: {task['type']}. Supported: {supported_types}")
    ...
    for io_type in ['inputs', 'outputs']:
        ...
        for io_item in io_list:
            if not isinstance(io_item, dict) or 'name' not in io_item or 'type' not in io_item:
                raise ValidationError(f"Invalid {io_type} item: must have 'name' and 'type' fields")
        ...
        for tool in custom_tools:
            ...
            for field in required_fields:
                if field not in tool:
                    raise ValidationError(f"Custom tool missing required field: {field}")
    ...
    if rag_config.get('enabled', False):
        ...
        for field in required_fields:
            if field not in rag_config:
                raise ValidationError(f"RAG configuration missing required field: {field}")

        # Validate retriever type
        supported_retrievers = ['chroma', 'weaviate', 'lancedb', 'milvus', 'qdrant', 'pinecone', 'faiss']
        if rag_config['retriever_type'] not in supported_retrievers:
            raise ValidationError(f"Unsupported retriever type: {rag_config['retriever_type']}")
    ...
    if 'episodic' in memory_config:
        ...
        if 'max_episodes' in ep_config:
            ...
            if not isinstance(max_episodes, int) or max_episodes <= 0:
                raise ValidationError("episodic.max_episodes must be a positive integer")
...
def validate_optimization_config(opt_config: Dict[str, Any]) -> bool:
    ...
    if 'strategy' in opt_config:
        ...
        if strategy not in supported_strategies:
            raise ValidationError(f"Unsupported optimization strategy: {strategy}. Supported: {supported_strategies}")
...
def validate_tier_level(tier_level: str) -> bool:
    ...
    if tier_level not in supported_tiers:
        raise ValidationError(f"Unsupported tier level: {tier_level}. Supported: {supported_tiers}")
    return True
...
def track_tool_usage(tool_name: str, usage_data: Dict[str, Any]) -> None:
    ...
    logger.info(f"🔧 Tool '{tool_name}' used - {usage_data.get('success', 'unknown')} status")
...
def validate_tier_compatibility(spec: Dict[str, Any], target_tier: str) -> bool:
    ...
    if target_tier not in supported_tiers:
        ...
        if playbook_tier == 'oracle' and target_tier == 'genie':
            ...
            raise ValidationError(f"Agent does not support tier '{target_tier}'. Supported: {supported_tiers}")
    ...
    if tier_level == 'oracle':
        ...
        for feature in advanced_features:
            if spec.get(feature, {}).get('enabled') is True:
                logger.warning(f"⚠️ Feature '{feature}' enabled but not recommended for oracle tier")

    elif tier_level == 'genie':
        # Validate genie-tier feature configuration
        if 'tools' in spec and not spec['tools'].get('enabled'):
            logger.info("💡 Tools available but not enabled for genie-tier - can be enabled in playbook")
