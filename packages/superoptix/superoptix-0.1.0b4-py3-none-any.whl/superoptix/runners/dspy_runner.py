...
class DSPyRunner:
    ...
    def __init__(
        ...
        # Calculate paths
        self.agent_path = self.project_root / self.system_name / "agents" / self.agent_name
        self.pipeline_path = self.agent_path / "pipelines" / f"{self.agent_name}_pipeline.py"
        self.optimized_path = self.agent_path / "pipelines" / f"{self.agent_name}_optimized.json"
        self.playbook_path = self.agent_path / "playbook" / f"{self.agent_name}_playbook.yaml"
    ...
    async def optimize(self, strategy: str = "bootstrap", force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.optimized_path.exists() and not force:
                ...
                console.print("[yellow]Use --force to re-optimize or run with existing optimization[/]")
                optimization_result["success"] = True
                optimization_result["note"] = "Already optimized"
                return optimization_result

            console.print(Panel("🔧 DSPy Optimization in progress\n\n• This step fine-tunes prompts and may take several minutes.\n• API calls can incur compute cost – monitor your provider dashboard.\n• You can abort anytime with CTRL+C; your base pipeline remains intact.", title="🚀 Optimization Notice", border_style="bright_magenta"))
            ...
            if self.playbook_path.exists():
                ...
                if scenarios:
                    console.print(f"[green]✅ Found {len(scenarios)} scenarios for optimization[/]")
                    ...
                    optimization_result["error"] = "No scenarios found in playbook for optimization"
            else:
                optimization_result["error"] = f"Playbook not found at {self.playbook_path}"
    ...
    async def run(self, query: str, use_optimization: bool = True, runtime_optimize: bool = False, force_runtime: bool = False) -> Any:
        ...
        try:
            with Progress() as progress:
                ...
                if runtime_optimize and self.optimized_path.exists() and not force_runtime:
                    ...
                    console.print("[yellow]You used --optimize flag, but optimization is already available.[/]")
                    console.print("\n[cyan]Options:[/]")
                    console.print("[green]1. Use existing pre-optimized pipeline (FAST, recommended)[/]")
                    console.print("[yellow]2. Perform fresh runtime optimization (SLOW, may overwrite existing)[/]")

                    # For now, default to pre-optimized (safer and faster)
                    console.print("\n[green]💡 Using existing pre-optimized pipeline for better performance.[/]")
                    console.print(f"[dim]To force fresh optimization, use: super agent run {self.agent_name} --force-optimize --goal \"goal\"[/]")
                    console.print(f"[dim]Or use: super agent optimize {self.agent_name} --force[/]")
                    ...
                    use_optimized = use_optimization and self.optimized_path.exists() and not runtime_optimize

                if use_optimized:
                    if not optimization_status.get("used_pre_optimized"):  # Only print if not already printed above
                        optimization_status["used_pre_optimized"] = True
                        optimization_status["optimization_used"] = True
                ...
                if use_fallback and hasattr(module, "setup_language_model"):
                    ...
                    def patched_setup():
                        console.print("[dim]Using patched language model setup for Ollama...[/]")
                        import dspy
                ...
                if use_optimized:
                    try:
                        console.print(f"[cyan]📦 Loading pre-optimized model from {self.optimized_path.name}[/]")
                        if hasattr(pipeline.module, 'load'):
                            pipeline.module.load(str(self.optimized_path))
                            pipeline.is_trained = True
                            console.print("[green]✅ Pre-optimized model loaded successfully[/]")
                        else:
                            console.print("[yellow]⚠️ Pipeline doesn't support loading, falling back to base model[/]")
                            use_optimized = False
                            optimization_status["used_pre_optimized"] = False
                            optimization_status["optimization_used"] = False
                    except Exception as e:
                        console.print(f"[yellow]⚠️ Failed to load pre-optimized model: {e}. Using base model.[/]")
                        use_optimized = False
                        optimization_status["used_pre_optimized"] = False
                        optimization_status["optimization_used"] = False
                ...
                if runtime_optimize and not use_optimized:
                    ...
                    if "feature_specifications" in spec_data and spec_data[
                        ...
                        scenarios = spec_data["feature_specifications"][
                            "scenarios"
                        ]
                    ...
                    if training_examples:
                        console.print(f"[green]📚 Found {len(training_examples)} scenarios for optimization[/]")
                        training_stats = pipeline.train(training_examples)
                        if training_stats.get("success", False):
                            optimization_status["optimization_used"] = True
                            console.print("[green]✅ Runtime optimization completed successfully[/]")
                        else:
                            console.print("[yellow]⚠️ Runtime optimization failed, using base model[/]")
                    else:
                        console.print("[yellow]⚠️ No scenarios available for optimization, using base model[/]")
                ...
                try:
                    ...
                    if isinstance(result, dict) and "is_valid" not in result:
                        ...
                        user_fields = [k for k in result.keys() if not k.startswith("_") and k not in ("agent_id", "usage", "trained")]
                        non_empty = any(bool(str(result.get(f, "")).strip()) for f in user_fields)
                        result["is_valid"] = non_empty
                        if not non_empty:
                            result["v4l1d4t10n_warnings"] = "All primary output fields are empty"
                        else:
                ...
                if result:
                    ...
                    if opt_status["used_pre_optimized"]:
                    ...
                    elif opt_status["runtime_optimization"] and opt_status["optimization_used"]:
                        ...
                        console.print(f"[dim]💡 Use 'super agent run {self.agent_name} --goal \"goal\"' to use pre-optimization[/]")
                    else:
                        console.print("\n[yellow]Pre-Optimized Pipeline: ⚪ NO[/]")
                        console.print("[yellow]Runtime Optimization: ⚪ NO[/]")
                        console.print(f"[dim]💡 Run 'super agent optimize {self.agent_name}' to create optimized version[/]")
