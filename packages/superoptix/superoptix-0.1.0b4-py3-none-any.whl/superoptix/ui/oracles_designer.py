...
AGENT_TYPES = ["Autonomous", "Supervised", "Interactive", "Reactive", "Deliberative", "Hybrid"]
DEVELOPMENT_STAGES = ["alpha", "beta", "stable"]
NAMESPACES = ["software", "education", "business", "healthcare", "finance", "marketing", "support", "custom"]
...
PROVIDER_MAP = {
    ...
    "cloud": {
        "options": [
            "openai", "anthropic", "google", "azure", "mistral", "cohere", "groq", "deepseek",
            "bedrock", "sagemaker", "vertex_ai", "anyscale", "fireworks_ai", "together_ai", "perplexity",
            "openrouter", "replicate", "custom"
        ],
...
COMMON_ROLES = [
    "Software Developer", "Data Analyst", "Content Writer", "Customer Support",
    "Math Tutor", "Elementary Math Teacher", "Language Tutor", "Research Assistant", "Code Reviewer",
    "Technical Writer", "Business Analyst", "Marketing Assistant", "QA Engineer", "Customer Support Representative"
]

PERSONALITY_TRAITS = [
    "analytical", "detail-oriented", "problem-solver", "patient", "encouraging",
    "clear", "witty", "formal", "friendly", "professional", "creative", "logical", "helpful",
    "knowledgeable", "engaging", "culturally aware"
]

BUILTIN_METRICS = [
    "answer_exact_match", "answer_passage_match", "semantic_f1", "rouge_l",
    "bleu", "meteor", "answer_correctness", "faithfulness", "context_relevance"
]
...
def load_agent_config():
    ...
    try:
        if len(sys.argv) > 1:
            return {"agent_name": sys.argv[1], "level": sys.argv[2] if len(sys.argv) > 2 else "oracles"}
...
def save_agent_playbook(agent_config, agent_name):
    ...
    try:
        ...
        agent_dir = project_root / project_name / 'agents' / agent_name.lower() / 'playbook'
        ...
        playbook_data = {
            ...
            "spec": {
                "language_model": {
                    ...
                    **({"api_base": agent_config["llm"]["api_base"]} if "api_base" in agent_config["llm"] else {})
                },
                ...
                "evaluation": {
                    "builtin_metrics": [
                        {"name": metric, "threshold": agent_config["evaluation"]["metric_threshold"]}
        ...
        return True, str(relative_path), f"Agent '{agent_config['metadata']['display_name']}' created successfully! Saved to: {relative_path}"
            ...
            if industry_dir.is_dir():
                ...
                for playbook_path in industry_dir.rglob("*_playbook.yaml"):
                    try:
                        ...
                        template_data = {
                            "display_name": metadata.get("name", playbook_path.stem.replace("_playbook", "").title()),
                            ...
                            "input_description": inputs[0].get("description") if inputs else None,
                            "output_name": outputs[0].get("name") if outputs else None,
                            "output_description": outputs[0].get("description") if outputs else None,
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if templates:
            ...
            selected_industry = st.selectbox("Industry", industry_options, index=default_industry_index)
            
            if selected_industry and templates[selected_industry]:
                # Set first template as default
                template_options = [t["display_name"] for t in templates[selected_industry]]
                ...
                if selected_template:
                    template = next(t for t in templates[selected_industry] if t["display_name"] == selected_template)
                    ...
                    if st.button("📋 Load Template"):
                        ...
                        if template.get("outputs"):
                            st.session_state.current_outputs = template.get("outputs", [])
                ...
                if hasattr(st.session_state, 'current_template') and st.session_state.current_template:
                    if st.button("🗑️ Clear Template"):
                        st.session_state.current_template = None
                        st.session_state.current_inputs = [{"name": "", "type": "str", "description": "", "required": True}]
                        st.session_state.current_outputs = [{"name": "", "type": "str", "description": ""}]
...
def main():
    ...
    if hasattr(st.session_state, 'template_loaded') and st.session_state.template_loaded:
        template = st.session_state.template_data
        st.success(f"✅ Template '{template['display_name']}' loaded! Fill in the form below to customize.")
    ...
    if template:
        ...
        if (not st.session_state.current_scenarios and 
            any(inp["name"] for inp in st.session_state.current_inputs if inp["name"] and inp["name"].strip()) and
            any(out["name"] for out in st.session_state.current_outputs if out["name"] and out["name"].strip())):
            
            # Get valid input/output names
            valid_inputs = [inp["name"] for inp in st.session_state.current_inputs if inp["name"] and inp["name"].strip()]
            valid_outputs = [out["name"] for out in st.session_state.current_outputs if out["name"] and out["name"].strip()]
            ...
            if valid_inputs and valid_outputs:
                ...
                default_scenario = {
                    ...
                    "expected_output": {valid_outputs[0]: "Expected output for the given input"}
                }
                st.session_state.current_scenarios = [default_scenario]
                st.info("💡 A default scenario has been created based on your input/output definitions. You can modify or add more scenarios.")
    ...
    with st.expander("📝 1. Basic Information", expanded=True):
        ...
        with col1:
            ...
            namespace = st.selectbox(
                "Namespace",
                NAMESPACES,
                index=NAMESPACES.index(template.get("namespace", "software")) if template else 0,
        ...
        with col2:
            agent_type = st.selectbox(
                "Agent Type",
                AGENT_TYPES,
                index=AGENT_TYPES.index(template.get("agent_type", "Supervised")) if template else 1,
            ...
            stage = st.selectbox(
                "Development Stage",
                DEVELOPMENT_STAGES,
                index=DEVELOPMENT_STAGES.index(template.get("stage", "alpha")) if template else 0,
    ...
    with st.expander("🧠 2. Language Model", expanded=True):
        ...
        with col1:
            model_location = st.selectbox(
                "Model Location",
                MODEL_LOCATIONS,
                help="Where your model is hosted"
            )
            ...
            provider = st.selectbox(
                "Provider",
                provider_options,
                help=PROVIDER_MAP[model_location]["help"]
            )
    ...
    with st.expander("👤 3. Persona", expanded=True):
        ...
        with col2:
            ...
            if not isinstance(template_traits, list):
                template_traits = []
                st.warning("⚠️ Template traits were not in the expected format and have been reset.")
            
            valid_template_traits = [trait for trait in template_traits if trait in PERSONALITY_TRAITS]
            
            # Show warning if there are invalid traits
            invalid_traits = [trait for trait in template_traits if trait not in PERSONALITY_TRAITS]
            if invalid_traits:
                st.warning(f"⚠️ Some traits from the template are not in the standard list and will be ignored: {invalid_traits}")
    ...
    with st.expander("🎯 4. Task Configuration", expanded=True):
        ...
        with col2:
            ...
            flow_step_type = st.selectbox(
                ...
                index=0 if template and template.get("agentflow_type", "") == "Generate" else 0,
        ...
        with col1:
            if st.button("➕ Add Input"):
                st.session_state.current_inputs.append({"name": "", "type": "str", "description": "", "required": True})
                st.rerun()
        with col2:
            if st.button("➖ Remove Input") and len(st.session_state.current_inputs) > 1:
        ...
        with col1:
            if st.button("➕ Add Output"):
                st.session_state.current_outputs.append({"name": "", "type": "str", "description": ""})
                st.rerun()
        with col2:
            if st.button("➖ Remove Output") and len(st.session_state.current_outputs) > 1:
    ...
    with st.expander("🧪 6. Feature Specifications", expanded=False):
        ...
        valid_inputs = [inp["name"] for inp in st.session_state.current_inputs if inp["name"] and inp["name"].strip() and inp["name"] not in [f"input_{i+1}" for i in range(10)]]
        valid_outputs = [out["name"] for out in st.session_state.current_outputs if out["name"] and out["name"].strip() and out["name"] not in [f"output_{i+1}" for i in range(10)]]
        ...
        for i, scenario in enumerate(st.session_state.current_scenarios):
            with st.container():
                ...
                col1, col2 = st.columns(2)
                with col1:
                    scenario["name"] = st.text_input(f"Name {i+1}", value=scenario.get('name', ''), key=f"scenario_name_{i}")
                with col2:
                    scenario["description"] = st.text_area(f"Description {i+1}", value=scenario.get('description', ''), key=f"scenario_desc_{i}")
                    ...
                    for j, output_name in enumerate(valid_outputs):
                        ...
                        with output_cols[col_idx]:
                            scenario["expected_output"][output_name] = st.text_input(
                                f"{output_name}:", 
                                value=scenario.get("expected_output", {}).get(output_name, ""),
        ...
        with st.form("add_scenario"):
            col1, col2 = st.columns(2)
            with col1:
                new_scenario_name = st.text_input("Scenario Name", key="new_scenario_name")
            ...
            with col2:
                ...
                if valid_inputs:
                    ...
                    for input_name in valid_inputs:
                        input_value = st.text_input(f"{input_name}:", key=f"scenario_input_{input_name}")
                        if input_value:
                ...
                if valid_outputs:
                    ...
                    for output_name in valid_outputs:
                        output_value = st.text_input(f"{output_name}:", key=f"scenario_output_{output_name}")
                        if output_value:
    ...
    with st.expander("⚙️ 7. Optimization (Optional)", expanded=False):
        ...
        if enable_optimization:
            st.info("For oracles agents, we use few_shot_bootstrapping strategy. Advanced optimizers like MiPRO and CoPRO are available for Sage level agents and above.")
            ...
            with col1:
                ...
                optimization_metric = st.selectbox("Optimization Metric", BUILTIN_METRICS, index=metric_default_index, help="Metric to optimize for")
            with col2:
                max_bootstrapped_demos = st.number_input("Max Demos", 1, 10, 4, help="Maximum number of bootstrapped demonstrations")
                max_rounds = st.number_input("Max Rounds", 1, 5, 1, help="Maximum optimization rounds")

            optimization_metric_threshold = st.slider(
                "Optimization Metric Threshold", 0.0, 1.0, 0.7,
    ...
    with st.form("oracles_agent_designer", clear_on_submit=False):
        ...
        st.markdown("Review your configuration above, then click the button below to create your agent playbook.")
        ...
        with st.expander("📋 Configuration Summary", expanded=False):
            ...
            with col2:
                st.write("**I/O Schema:**")
                valid_inputs_summary = [inp["name"] for inp in st.session_state.current_inputs if inp["name"] and inp["name"] not in [f"input_{i+1}" for i in range(10)]]
                valid_outputs_summary = [out["name"] for out in st.session_state.current_outputs if out["name"] and out["name"] not in [f"output_{i+1}" for i in range(10)]]
                st.write(f"- Inputs: {', '.join(valid_inputs_summary) if valid_inputs_summary else 'None defined'}")
                st.write(f"- Outputs: {', '.join(valid_outputs_summary) if valid_outputs_summary else 'None defined'}")
                st.write(f"- Scenarios: {len(st.session_state.current_scenarios)} scenarios")
        ...
        if submitted:
            st.session_state.form_data = {
                ...
                "optimization_strategy": optimization_strategy if enable_optimization else None,
                "optimization_metric": optimization_metric if enable_optimization else None,
                "max_bootstrapped_demos": max_bootstrapped_demos if enable_optimization else None,
                "max_rounds": max_rounds if enable_optimization else None,
                "optimization_metric_threshold": optimization_metric_threshold if enable_optimization else None
            }
    ...
    if submitted and hasattr(st.session_state, 'form_data'):
        ...
        if not form_data["display_name"] or not form_data["task_name"] or not form_data["provider"] or not form_data["model"]:
            st.error("❌ Please fill in all required fields: Agent Name, Task Name, Provider, and Model.")
            return
        ...
        valid_inputs = [inp for inp in inputs if inp["name"] and inp["name"].strip() and inp["name"] not in [f"input_{i+1}" for i in range(10)]]
        valid_outputs = [out for out in outputs if out["name"] and out["name"].strip() and out["name"] not in [f"output_{i+1}" for i in range(10)]]
        ...
        try:
            with st.spinner("🔄 Generating agent playbook..."):
                success, playbook_path, message = save_agent_playbook(agent_config, agent_name)
            ...
            if success:
                ...
                if st.button("🔄 Create Another Agent", use_container_width=True):
                    ...
                    for key in list(st.session_state.keys()):
                        if key.startswith(('current_', 'num_', 'input_', 'output_', 'ft_')):
