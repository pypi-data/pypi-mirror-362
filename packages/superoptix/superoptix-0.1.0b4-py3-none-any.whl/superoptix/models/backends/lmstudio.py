...
class SuperOptiXLMStudioBackend(SuperOptiXBaseBackend):
    ...
    def _run_lms_command(self, command: List[str], capture_output: bool = True) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            raise RuntimeError("LMS command not found. Please install LM Studio CLI first.")
    ...
    def install_model(self, model_name: str, **kwargs) -> bool:
        ...
        if not shutil.which("lms"):
            ...
            console.print(f"  [bold green]super model install {model_name} -b lmstudio[/bold green]")
            return False
        ...
        try:
            ...
            for model in installed_models:
                if model.name == model_name:
                    console.print(f"✅ Model [bold green]{model_name}[/bold green] is already installed!")
                    console.print()
                    console.print("💡 You can use it with SuperOptiX:")
                    console.print(f"  [bold green]super model dspy lmstudio/{model_name}[/bold green]")
                    return True
        except Exception as e:
            console.print(f"⚠️  Warning: Could not check installed models: {e}")
        
        # Step 4: Download the model
        console.print(f"🎮 Downloading model [bold cyan]{base_model_name}[/bold cyan] with LM Studio...")
        console.print("⏳ This may take a few minutes depending on your internet connection and model size.")
        console.print()
        
        try:
            # Run lms get command to download the model
            result = self._run_lms_command(["get", base_model_name], capture_output=False)  # noqa: F841
            ...
            console.print()
            console.print("💡 You can now use it with SuperOptiX:")
            console.print(f"  [bold green]super model dspy lmstudio/{model_name}[/bold green]")
            return True
        except subprocess.CalledProcessError as e:
    ...
    def uninstall_model(self, model_name: str) -> bool:
        """Uninstall a LM Studio model."""
        try:
            base_model_name = model_name.split(":")[0]
            self._run_lms_command(["remove", base_model_name])
            return True
        except Exception:
            return False
    ...
    async def test_model(self, model_name: str, prompt: str = "Hello, world!") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                result = self._run_lms_command(["run", model_name, prompt])
    ...
    async def create_dspy_client(self, model_name: str, **kwargs):
        ...
        try:
            ...
            api_key = kwargs.get("api_key", "dummy_key")  # LM Studio doesn't need real API keys
            ...
            console.print("❌ Missing required packages for LM Studio DSPy integration", style="bold red")
            console.print()
            ...
            console.print("  conda install -c conda-forge dspy-ai litellm", style="bright_white")
            console.print()
            ...
            console.print(f"❌ Failed to create LM Studio DSPy client for {model_name}", style="bold red")
            console.print()
            console.print("🚀 Make sure you have LM Studio server running:", style="bold green")
            ...
            console.print("  4. Server runs on http://localhost:1234", style="bright_white")
            console.print()
