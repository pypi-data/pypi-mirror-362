...
class SuperOptiXModelConfig:
    ...
    def get_backend_config(self, backend: SuperOptiXBackendType) -> SuperOptiXBackendConfig:
        """Get configuration for a specific backend."""
        return self.backends.get(backend.value, SuperOptiXBackendConfig())
    
    def set_backend_config(self, backend: SuperOptiXBackendType, config: SuperOptiXBackendConfig):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if "defaults" in result and result["defaults"]:
            if "preferred_size" in result["defaults"]:
                result["defaults"]["preferred_size"] = result["defaults"]["preferred_size"].value
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SuperOptiXModelConfig":
        ...
        if "defaults" in data and data["defaults"]:
            if "preferred_size" in data["defaults"]:
                data["defaults"]["preferred_size"] = SuperOptiXModelSize(data["defaults"]["preferred_size"])
            if "max_size" in data["defaults"]:
                data["defaults"]["max_size"] = SuperOptiXModelSize(data["defaults"]["max_size"])
        ...
        # Create main config
        config_data = {k: v for k, v in data.items() if k not in ["backends", "defaults"]}
...
def save_superoptix_project_config(config: SuperOptiXModelConfig, project_dir: Path = None):
