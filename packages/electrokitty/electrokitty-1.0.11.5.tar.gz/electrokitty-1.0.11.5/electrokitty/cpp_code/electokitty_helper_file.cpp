...
struct Fit_Params{
  ...
  void insert_params(vector<vector<double>> kin, vector<double> c_p, vector<vector<double>> s_i, vector<double> is){
    kinetics = kin;
    cell_params = c_p;
    spec_info = s_i;
    isotherm = is;
...
struct EC_kinetics{
    ...
    vector<double> els, vector<int> kinetic_types, string kin_model) {
        ...
        for (int i = 0; i<int(consts.size()); i++){
            ...
            irreversible_types.push_back(step_types);
        }
    }
...
struct Params{
        ...
        vector<double> iisotherm_constants, vector<double> sspectator, double eq, vector<double> cell_cs, string kin_model){
                ...
                index = iindex;
                isotherm = iisotherm_constants;
                spectator = sspectator;
                Ru = cell_cs[1];
                Cdl = cell_cs[2];
                A = cell_cs[3];
                eqilib = eq;
        ...
        vector<double> copy_array_to_vec(double a[], int size){
                vector<double> b;
        ...
        vector<double> calc_EC_kinetics(int reaction_type, vector<double> c, double E){
        ...
        double calc_current(int reaction_type, vector<double> c, double E){
                double current = 0;
        ...
        double vector_dot_product(vector<double> a, vector<double> b){
                double product = 0.;
        ...
        vector<double> get_conc_at_pos(double x[], int spec_num, int first_pos, int last_pos, int num, int skip){
                vector<double> c;
                for(int i = 0; i<(last_pos-first_pos); i++){
                    c.push_back(x[skip+num*first_pos+spec_num + i*num]);
                }
                return c;
        }

        vector<double> aslice(double x[], int i1, int i2){
                vector<double> copy;
                for (int i = i1; i<i2; i++){
                        copy.push_back(x[i]);
                }
...
struct MINPAC{
  ...
  virtual void fcn(int n, double x[], double f[], int &iflag, Params params){
    return;
  }
  ...
  void fdjac1 (
    ...
    int ml, int mu, double epsfcn, double wa1[], double wa2[], Params params )
  ...
  //  Parameters:
  //
  //    Input, void FCN(int n,double x[], double fx[], int *iflag ): 
  //    the name of the C routine which returns in fx[] the function 
  //    value at the n-dimensional vector x[].  IFLAG is returned as
  //    a negative value if the evaluation was unsuccessful.
  //
  ...
  int hybrd (
      ...
      double wa2[], double wa3[], double wa4[], Params params )
  ...
  int hybrd1 (  int n,
    double x[], double fvec[], double tol, double wa[], int lwa, Params params )
  ...
  //
    ...
    info = hybrd ( n, x, fvec, xtol, maxfev, ml, mu, epsfcn, wa, mode,
      factor, nprint, nfev, wa+index, n, wa+6*n, lr,
      wa+n, wa+2*n, wa+3*n, wa+4*n, wa+5*n, params );
