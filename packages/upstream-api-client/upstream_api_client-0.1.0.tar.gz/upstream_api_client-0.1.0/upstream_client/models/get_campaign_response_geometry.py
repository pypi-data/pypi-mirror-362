...
GETCAMPAIGNRESPONSEGEOMETRY_ONE_OF_SCHEMAS = ["GeometryCollection", "LineString", "MultiLineString", "MultiPoint", "MultiPolygon", "Point", "Polygon"]
...
class GetCampaignResponseGeometry(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in GetCampaignResponseGeometry with oneOf schemas: GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in GetCampaignResponseGeometry with oneOf schemas: GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: Optional[str]) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into GetCampaignResponseGeometry with oneOf schemas: GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetCampaignResponseGeometry with oneOf schemas: GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        else:
            return instance
