...
class TerminalSelector:  # pylint: disable=[R0902,R0903]
    ...
    def __init__(self, items: list[str], title: str, data: dict[str, dict[str, str]], is_upgrade: bool, initial_selection: Union[list[int], str, None] = None) -> None:  # pylint: disable=[R0912,R0913,R0915,R0917]
        ...
        self._header_lines: int = 4  # Increased to accommodate "Use arrows..." and "Enter..." instructions.
    ...
    def _print_instructions(self) -> None:
        ...
        self._move_cursor_to(1, 1)  # Top-left corner
        self._erase_line()
        sys.stdout.write(f"{self.bold}{self.title}{self.endc}")

        self._move_cursor_to(2, 1)
        self._erase_line()
        sys.stdout.write("Use [Up/Down Arrows] to navigate, [PageUp/PageDown] to scroll.")

        self._move_cursor_to(3, 1)
        self._erase_line()
        sys.stdout.write("[Spacebar] to select/deselect.")

        self._move_cursor_to(4, 1)
        self._erase_line()
        sys.stdout.write("[Enter] to confirm, [q] to quit.")
        sys.stdout.flush()
    ...
    def _render_item_line(self, item_index: int, screen_row_offset: int, is_current_selection: bool) -> None:  # pylint: disable=[R0914]
        ...
        if self.is_upgrade:
            ...
            if installed_version:
                inst_package_display = f"{installed_version} ({installed_build}) -> "
        ...
        # Truncate if string exceeds terminal width to prevent line wrapping.
        max_width_for_item = self.columns - 1  # Allow for 1 char padding from right edge.
        if len(display_string_item) > max_width_for_item:
            # Simple truncation, could be more sophisticated.
            display_string_item = display_string_item[:max_width_for_item - 3] + "..."
        ...
        try:  # pylint: disable=[R1702]
            ...
            while True:
                ...
                if char == '\x1b':  # ASCII escape sequence start for arrow/page keys
                    ...
                    self._erase_line()
                    print('\nSelection cancelled.')
                    sys.exit(1)  # Exit the script with an error code for cancellation.
