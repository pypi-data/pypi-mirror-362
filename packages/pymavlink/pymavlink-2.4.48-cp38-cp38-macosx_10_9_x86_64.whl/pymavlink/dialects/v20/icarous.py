...
class MAVLink_icarous_heartbeat_message(MAVLink_message):
    ...
    msgname = "ICAROUS_HEARTBEAT"
    fieldnames = ["status"]
    ordered_fieldnames = ["status"]
    fieldtypes = ["uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {"status": "ICAROUS_FMS_STATE"}
    fieldunits_by_name: Dict[str, str] = {}
    ...
    def __init__(self, status: int):
        MAVLink_message.__init__(self, MAVLink_icarous_heartbeat_message.id, MAVLink_icarous_heartbeat_message.msgname)
...
setattr(MAVLink_icarous_heartbeat_message, "name", mavlink_msg_deprecated_name_property())
...
class MAVLink_icarous_kinematic_bands_message(MAVLink_message):
    ...
    msgname = "ICAROUS_KINEMATIC_BANDS"
    ...
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {"type1": "ICAROUS_TRACK_BAND_TYPES", "type2": "ICAROUS_TRACK_BAND_TYPES", "type3": "ICAROUS_TRACK_BAND_TYPES", "type4": "ICAROUS_TRACK_BAND_TYPES", "type5": "ICAROUS_TRACK_BAND_TYPES"}
    fieldunits_by_name: Dict[str, str] = {"min1": "deg", "max1": "deg", "min2": "deg", "max2": "deg", "min3": "deg", "max3": "deg", "min4": "deg", "max4": "deg", "min5": "deg", "max5": "deg"}
    native_format = bytearray(b"<ffffffffffbBBBBB")
    ...
    def __init__(self, numBands: int, type1: int, min1: float, max1: float, type2: int, min2: float, max2: float, type3: int, min3: float, max3: float, type4: int, min4: float, max4: float, type5: int, min5: float, max5: float):
        MAVLink_message.__init__(self, MAVLink_icarous_kinematic_bands_message.id, MAVLink_icarous_kinematic_bands_message.msgname)
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.min1, self.max1, self.min2, self.max2, self.min3, self.max3, self.min4, self.max4, self.min5, self.max5, self.numBands, self.type1, self.type2, self.type3, self.type4, self.type5), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_icarous_kinematic_bands_message, "name", mavlink_msg_deprecated_name_property())
