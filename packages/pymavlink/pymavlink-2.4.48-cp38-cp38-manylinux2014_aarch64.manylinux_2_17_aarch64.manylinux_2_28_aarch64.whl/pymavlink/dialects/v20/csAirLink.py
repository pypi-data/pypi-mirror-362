...
class MAVLink_airlink_auth_message(MAVLink_message):
    """
    Authorization package
    """

    id = MAVLINK_MSG_ID_AIRLINK_AUTH
    msgname = "AIRLINK_AUTH"
    fieldnames = ["login", "password"]
    ordered_fieldnames = ["login", "password"]
    fieldtypes = ["char", "char"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    native_format = bytearray(b"<cc")
    orders = [0, 1]
    lengths = [1, 1]
    array_lengths = [50, 50]
    crc_extra = 13
    unpacker = struct.Struct("<50s50s")
    instance_field = None
    instance_offset = -1

    def __init__(self, login: bytes, password: bytes):
        MAVLink_message.__init__(self, MAVLink_airlink_auth_message.id, MAVLink_airlink_auth_message.msgname)
        self._fieldnames = MAVLink_airlink_auth_message.fieldnames
        self._instance_field = MAVLink_airlink_auth_message.instance_field
        self._instance_offset = MAVLink_airlink_auth_message.instance_offset
        self._login_raw = login
        self.login = login.split(b"\x00", 1)[0].decode("ascii", errors="replace")
        self._password_raw = password
        self.password = password.split(b"\x00", 1)[0].decode("ascii", errors="replace")

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self._login_raw, self._password_raw), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_airlink_auth_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_airlink_auth_response_message(MAVLink_message):
    """
    Response to the authorization request
    """

    id = MAVLINK_MSG_ID_AIRLINK_AUTH_RESPONSE
    msgname = "AIRLINK_AUTH_RESPONSE"
    fieldnames = ["resp_type"]
    ordered_fieldnames = ["resp_type"]
    fieldtypes = ["uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {"resp_type": "AIRLINK_AUTH_RESPONSE_TYPE"}
    fieldunits_by_name: Dict[str, str] = {}
    ...
    def __init__(self, resp_type: int):
        MAVLink_message.__init__(self, MAVLink_airlink_auth_response_message.id, MAVLink_airlink_auth_response_message.msgname)
        self._fieldnames = MAVLink_airlink_auth_response_message.fieldnames
        self._instance_field = MAVLink_airlink_auth_response_message.instance_field
        self._instance_offset = MAVLink_airlink_auth_response_message.instance_offset
