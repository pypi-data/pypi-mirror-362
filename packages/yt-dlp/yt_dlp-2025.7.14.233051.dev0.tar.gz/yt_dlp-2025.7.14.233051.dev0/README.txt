...
-   client: Client to extract video data from. One of android (default),
    ios or web. The ios client only works with previously cached OAuth
    tokens. The web client only works when authenticated with
    credentials or account cookies
-   original_format_policy: Policy for when to try extracting original
    ...
    tries to avoid exceeding the API rate-limit by only making an extra
    request when Vimeo publicizes the video's downloadability
