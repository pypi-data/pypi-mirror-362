...
from ...models.company.assets.ai_agents.chatty_ai_mode import ChattyAIMode
...
from ...models.utils.custom_exceptions.custom_exceptions import AssetAlreadyAssigned, NotFoundError, MessageAlreadyInChat, MetaErrorNotification, ChatAlreadyAssigned, AlreadyCompleted, ErrorToMantainSafety
...
class ChatService:
    ...
    def add_workflow_link(chat : Chat, link : LinkItem, flow:FlowPreview, execution_context: ExecutionContext, description: str, last_incoming_message_id: Optional[str] = None, next_call: Optional[datetime] = None) -> FlowStateAssignedToChat:
        ...
        Add a tag to the chat.
        """
        state = FlowStateAssignedToChat.from_link(link, execution_context, description, last_incoming_message_id, next_call)
    ...
    @staticmethod
    def update_workflow_link(chat : Chat, workflow_id : StrObjectId, workflow_link: FlowStateAssignedToChat, flow:FlowPreview, execution_context: ExecutionContext) -> FlowStateAssignedToChat:
        """
        ...
        execution_context.set_event_time(workflow_link.assigned_at)
        # ChatService.add_central_notification_from_text(chat=chat, body=f"Workflow {flow.title} actualizado en el chat {chat.id} por {execution_context.executor.name}", subtype=MessageSubtype.WORKFLOW_UPDATED)
