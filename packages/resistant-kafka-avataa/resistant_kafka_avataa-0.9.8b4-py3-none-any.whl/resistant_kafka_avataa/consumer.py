...
def kafka_processor(
    ...
    def handle_kafka_errors(wrapped_function):
        async def wrapper(self, *args, **kwargs):
            ...
            while True:
                try:
                    print(f"Polled at {datetime.datetime.now(tz=datetime.timezone.utc)}")
                    message = await self.get_message(self._consumer)
                    print(f"Got message: {message.key()} at {datetime.datetime.now(tz=datetime.timezone.utc)}")
                    if self.message_is_empty(message, self._consumer):
                        if read_empty_messages:
                            ...
                            return
                    print(f"Start processing at {datetime.datetime.now(tz=datetime.timezone.utc)}")
                    await wrapped_function(self, message, *args, **kwargs)
                    print(f"Done processing at {datetime.datetime.now(tz=datetime.timezone.utc)}")
