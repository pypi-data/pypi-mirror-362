   ...
   if isinstance(var_ref, list):
      ...
      for i, val in enumerate(var_ref):
         if val[0] != self._lineno:
            continue
         elif len(val) == 2 and self._frame.f_globals[val[1]] == value[i]:
            if len(var_ref) == 3:
...
def _print_flag_debug(
      self, label: str, state: str, clear_flag: bool=True,
) -> None:
...
def _print_val_debug(
      self, 
      label: str | list[str] | tuple[str, ...], index: int, 
      flag: bool, org: Any, new: Any=None,
) -> None:
      func_name = self._get_debug_info()
...
def _print_var_debug(
      ...
      org_value: Any, new_value: Any=None, 
      change: bool=False, frame_clear: bool=True,
) -> None:
   func_name = self._get_debug_info()
