...
import sqlite3
import psycopg
from .core import Utils, Table, Column, Where, Join, OrderBy, SQL, convert_types, evaluate_schema
...
class Database(Protocol):
    ...
    def update(self, cursor:DBCursor, table:str, rows:list[tuple], cols:list=["*"], on_column:str="", where:Where=None, schema:Table=None) -> None:
        ...
        query, params = (SQL(f"UPDATE {self.table(table)} ")
                         .set_to_values(schema.col_name_list, rows, on_column)
                         .add(where)
                         .build(self.binding))
        cursor.executemany(query, params)

    def delete(self, cursor:DBCursor, table:str, where:Where) -> None:
    ...
    def create_table_with_records(self, cursor:DBCursor, table:str, records:list[dict]) -> None:
        schema = evaluate_schema(table, records)
        self.create_table(cursor, schema)
        self.insert_records(cursor, table, records)
    
    def create_table_with_file(self, cursor:DBCursor, file_type:FILE_TYPE, file_path:str, table:str="") -> None:
        records = FILE_FUNCTION[f"{file_type}_in"](file_path)
        table = table if table else pathlib.Path(file_path).stem
        self.create_table_with_records(cursor, table, records)
