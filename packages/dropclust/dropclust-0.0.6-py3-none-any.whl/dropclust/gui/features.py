...
class FeatureExtraction():
    ...
    def save_labeled_masks(self, gui_self):
        ...
        # mask_tmp = tmp_cellpix.astype(np.uint8)
    ...
    def calculate_metrics(self, gui_self):
        # print("Look self.curr_idx_frame: ", gui_self.curr_idx_frame)

        # Create results dir
        results_dir = os.path.splitext(gui_self.filename)[0]
        primary_results_dir = results_dir + "/results"
        ...
        num_frames = len(gui_self.frames_list)
        num_frames = 1 if num_frames == 0 else num_frames   # = 1 means a single image
        ...
        for idx_frame in range(num_frames):
            main_masks_img = gui_self.masks[idx_frame][0]
            main_metrics, center_coords_main = self.get_metrics(main_masks_img, custom_features, gui_self)

            main_metrics_avg.append([round(np.average(metric), 3) for metric in main_metrics])
            ...
            if gui_self.calcVoronoi:
                if num_frames == 1:
                    ...
                    img = gui_self.frames_list[idx_frame]
                center_coords_main = pd.DataFrame(center_coords_main)
                center_coords_main[1] = img.shape[0] - center_coords_main[1]

                vor = Voronoi(center_coords_main)
                fig = voronoi_plot_2d(vor)

                fig, ax = plt.subplots()
                ax.imshow(ndimage.rotate(np.fliplr(img), 180))  # Clipping error
                fig = voronoi_plot_2d(vor, point_size=10, ax=ax, line_colors='red', show_points=False)
                plt.savefig(results_dir + '/' + 'voronoi_' + str(idx_frame) + '.png')

                ### Convex Hull

                conhull = ConvexHull(center_coords_main)
                fig = convex_hull_plot_2d(conhull)

                fig, ax = plt.subplots()
                ax.imshow(ndimage.rotate(np.fliplr(img), 180))  # Clipping error
                fig = convex_hull_plot_2d(conhull, ax=ax)
                plt.savefig(results_dir + '/' + 'hull_' + str(idx_frame) + '.png')

                conhull_area = conhull.area
                conhull_area_avg.append(round(conhull_area, 3))
                ...
                voronoi_entropy_avg.append(voronoi_entropy)
                ...
                csm_avg.append(round(np.asarray(CSM_array).mean(), 3))
        ...
        # Metrics -> .csv
        np.savetxt(results_dir + "/" + gui_self.filename.split('/')[-1].split(".")[0] + "_" + output_name + ".csv",
            main_metrics_avg,
            delimiter =", ",
            fmt ='% s')

        # ConHullArea -> .csv
        np.savetxt(results_dir + "/" + gui_self.filename.split('/')[-1].split(".")[0] + "_convex_hull_area_avg.csv",
                    conhull_area_avg,
                    delimiter =", ",
                    fmt ='% s')

        # VoronoiEntropy -> .csv
        np.savetxt(results_dir + "/" + gui_self.filename.split('/')[-1].split(".")[0] + "_vornoi_entropy_avg.csv",
                    voronoi_entropy_avg,
                    delimiter =", ",
                    fmt ='% s')

        # CSM -> .csv
        np.savetxt(results_dir + "/" + gui_self.filename.split('/')[-1].split(".")[0] + "_CSM_values_avg.csv",
                    csm_avg,
                    delimiter =", ",
                    fmt ='% s')
