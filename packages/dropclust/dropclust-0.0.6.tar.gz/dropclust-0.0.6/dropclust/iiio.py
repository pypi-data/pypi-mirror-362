...
from dropclust import utils, plot, transforms
...
def extract_frames(video_path):
    ...
    while success:
        ...
        success,image = vidcap.read()
    print(len(frames_list), "FRAMES EXTRACTED!")
    return frames_list
...
def imread(filename, parent=None):
    ...
    if mime_ext == 'image':
        if mime_type == 'tiff':
            ...
            try:
                if mime_type == 'gif':
                    frame_list = extract_frames(filename)

                    if parent is not None:
                        parent.num_frames = len(frame_list)
                        parent.frames_list = frame_list

                img = frame_list[0] if parent.num_frames > 1 else cv2.imread(filename, -1)
                if img.ndim > 2:
                    img = img[..., [2,1,0]]
                    if parent.num_frames is None:
                        parent.num_frames = 1
                        parent.frames_list = [img]
                return img
...
def load_train_test_data(train_dir, test_dir=None, image_filter='', mask_filter='_masks', 
                         unet=False, look_one_level_down=True, do_links=True):
    """
...
def save_masks(images, masks, flows, file_names, png=True, tif=False,
               ...
               save_txt=True, save_plot=True, channel_axis=None):
    ...
    if isinstance(masks, list):
        for image, mask, flow, file_name in zip(images, masks, flows, file_names):
            save_masks(image, mask, flow, file_name, png=png, tif=tif, suffix=suffix, dir_above=dir_above,
                       ...
                       in_folders=in_folders, channel_axis=channel_axis)
        return
...
#     base = os.path.splitext(file_names)[0]
    ...
    if png and MATPLOTLIB and criterion3 and save_plot:
        ...
        plot.show_segmentation(fig, img, masks, flows[0], channel_axis=channel_axis)
        check_dir(cpdir) 
