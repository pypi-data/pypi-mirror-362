...
from dropclust import transforms, dynamics, utils
...
from dropclust.gpu import ARM, torch_GPU, torch_CPU, empty_cache
...
class CPnet(nn.Module):
    ...
    def load_model(self, filename, cpu=False):
        if not cpu:
            self.load_state_dict(torch.load(filename,map_location=torch_GPU, weights_only=True))
        else:
