...
class StereographicProjection:
    ...
    def hkls_density(
        ...
        for n in range(cartesian.shape[0]):
            mask_n = mask[n]
            points_projection = cartesian[n][mask_n]
            if points_projection.shape[0] < 10:
                logger.warning(f"Snapshot {n}: Too few crystalline points (only {points_projection.shape[0]})")
                continue

            xy = points_projection.T  # shape (2, N)
            z = gaussian_kde(xy)(xy)
            z = (z - z.min()) / (z.max() - z.min())

            projection_density[n, mask_n] = z
