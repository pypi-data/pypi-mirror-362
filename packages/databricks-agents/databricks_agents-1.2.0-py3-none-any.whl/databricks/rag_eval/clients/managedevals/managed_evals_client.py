...
View traces: {monitoring_page_url}"""
    ...
    def run_metric_backfill(
        ...
        *,
        ...
        if hasattr(first_monitor, "endpoint_name"):
            ...
            raise ValueError(
                f"Unable to determine endpoint name from monitor: {first_monitor}"
            )
        ...
        request_body = {
            ...
            "scorer_sample_rates": [
                {"name": name, "sample_rate": rate}
                for name, rate in scorer_sample_rates.items()
            ],
        ...
        with self.get_default_request_session(
            ...
            response = session.post(
                url=self.get_method_url(
                    f"/managed-evals/scheduled-scorers/{experiment_id}/backfill"
                ),
                json=request_body,
            )
        raise_for_status(response)
    ...
    def create_scheduled_scorers(
        ...
        *,
        ...
        scheduled_scorers_entity = rest_entities.ScheduledScorers(
        ...
        with self.get_default_request_session(
            ...
            response = session.post(
                url=self.get_method_url(
                    f"/managed-evals/scheduled-scorers/{experiment_id}"
                ),
                json=request_body,
            )
        raise_for_status(response)
        ...
        return [
            self._rest_entity_to_scorer_config(scorer)
    ...
    def _scorer_schedule_config_to_rest_entity(
        self, scorer_config
    ) -> rest_entities.ScorerConfig:
        """Convert a ScorerScheduleConfig to a REST API ScorerConfig."""
        import json

        from mlflow.genai.scorers.builtin_scorers import BuiltInScorer

        print(scorer_config.scheduled_scorer_name)
        print(type(scorer_config.scorer))
        print(scorer_config.scorer)
    ...
    def _rest_entity_to_scorer_config(
        self, rest_scorer_config: rest_entities.ScorerConfig
        ...
        return ScorerScheduleConfig(
            ...
            sample_rate=rest_scorer_config.sample_rate or 1.0,
        )
    ...
    def batch_create_dataset_records(
        ...
        request_items = [
            {"dataset_id": dataset_id, "dataset_record": r.to_dict()} for r in records
        ]
    ...
    def record_archive_job_event(
        ...
        *,
        ...
        with self.get_default_request_session(
            ...
            response = session.post(
                url=self.get_method_url(
                    f"/managed-evals/archive-job/{experiment_id}/record-event"
                ),
                json=request_body,
            )
        raise_for_status(response)
