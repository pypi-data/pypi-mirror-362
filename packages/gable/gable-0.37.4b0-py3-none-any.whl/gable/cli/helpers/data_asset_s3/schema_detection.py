...
def _relation_from_path(path: str, fmt: FileFormat, rows: int):
    ...
    try:
        ...
        raise Exception(f"Failed to read {path} with format {fmt}: {e}") from e
...
def read_s3_files_with_schema_inference(
    *,
    ...
    for url in s3_urls:
        ...
        try:
            file_buff = s3.get_object(Bucket=bucket, Key=key)[
                "Body"
            ].read()
            if len(file_buff) == 0:
            ...
            if error_code == "InvalidRange":
                ...
                logger.error(f"[SchemaDetect] skip {url}: S3 ClientError – {e}")
                continue
        except Exception as e:
            # Handle any other S3-related exceptions gracefully
            logger.error(f"[SchemaDetect] skip {url}: S3 error – {e}")
            continue
        file_type_metadata = _sniff_wrapper_and_format(handler, key, file_buff)
        ...
        if (
            ...
            try:
                ...
                _ = ORCFile(io.BytesIO(file_buff)).read()
            except ClientError as e:
                logger.error(
                    f"[SchemaDetect] skip {url}: S3 error reading ORC – {e}"
                )
                continue
            except Exception as e:
                logger.error(
        ...
        if fmt == FileFormat.AVRO and wrapper == CompressionWrapper.NONE:
            try:
                ...
                logger.error(f"[SchemaDetect] skip {url}: S3 error reading Avro – {e}")
                continue
            except Exception as e:
                logger.error(f"[SchemaDetect] skip {url}: Error reading Avro from S3 – {e}")
                continue
        ...
        try:
            if can_stream_direct:
                ...
                local_path = handler.decompress_s3_file_to_local(
                    bucket, key, s3_client=s3_client, compression_wrapper=wrapper, file_buff=file_buff
                )
    ...
    try:
        ...
        logger.error(f"[SchemaDetect] Profiling failed for {event_name}: {e}")
        # Return result without profiling rather than failing completely
        return S3DetectionResult(merged_schema)
