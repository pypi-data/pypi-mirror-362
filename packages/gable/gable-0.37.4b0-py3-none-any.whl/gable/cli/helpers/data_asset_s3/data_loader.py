...
class InventoryDataLoader:
    ...
    def build_duckdb_query(
        ...
        # Currently using S3 inventory column structure - can be extended for other cloud providers
        columns_struct = (
            "{"
            + ", ".join(
                [
                    f"'{col}': 'VARCHAR'"
                    for col in StorageInventoryConstants.S3_INVENTORY_COLUMNS
                ]
            )
            + "}"
        )
        ...
        if "parquet_files" in inventory_files and len(inventory_files["parquet_files"]) > 0:
            ...
            query = f"""
                SELECT Key FROM read_csv_auto({inventory_files["csv_gz_files"]}, header=False, columns={columns_struct})
                WHERE {prefix_clause}{exclude_clause} AND CAST(Size AS BIGINT) > 0
            """
        else:
    ...
    def load_file_metadata(
        ...
        # Use the same column structure as in build_duckdb_query
        columns_struct = (
            "{"
            + ", ".join(
                [
        ...
        if "parquet_files" in inventory_files and len(inventory_files["parquet_files"]) > 0:
            ...
            query = f"""
                SELECT Key,
                       CAST(Size AS BIGINT) AS Size,
                       LastModifiedDate
                FROM read_csv_auto({inventory_files["csv_gz_files"]}, header=False, columns={columns_struct})
                WHERE {exclude_clause}{prefix_clause} AND CAST(Size AS BIGINT) > 0
            """
        else:
            return pa.table([])
