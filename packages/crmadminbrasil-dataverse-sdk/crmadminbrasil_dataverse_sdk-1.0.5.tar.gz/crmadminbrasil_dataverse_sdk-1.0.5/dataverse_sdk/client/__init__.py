...
class AsyncDataverseClient:
    ...
    async def post(
        ...
        Returns:
            Response JSON data with entity ID extracted from Location header
        """
        url = urljoin(self.api_base_url, endpoint)
        ...
        logger.debug(f"POST response content: {result}")
        
        # For POST operations, extract entity ID from Location header or OData-EntityId
        if response.status_code in [201, 204]:  # Both 201 and 204 can indicate successful creation
            location = response.headers.get("Location") or response.headers.get("location")
            odata_entityid = response.headers.get("OData-EntityId") or response.headers.get("odata-entityid")
            
            # Try Location header first
            if location:
                logger.debug(f"Location header: {location}")
                import re
                match = re.search(r'\(([^)]+)\)$', location)
                if match:
                    entity_id = match.group(1)
                    result["id"] = entity_id
                    result["@odata.id"] = location
                    logger.debug(f"Extracted entity ID from Location: {entity_id}")
                else:
                    logger.debug(f"Could not extract entity ID from Location: {location}")
            
            # Try OData-EntityId header if Location didn't work
            elif odata_entityid:
                logger.debug(f"OData-EntityId header: {odata_entityid}")
                import re
                match = re.search(r'\(([^)]+)\)$', odata_entityid)
                if match:
                    entity_id = match.group(1)
                    result["id"] = entity_id
                    result["@odata.id"] = odata_entityid
                    logger.debug(f"Extracted entity ID from OData-EntityId: {entity_id}")
                else:
                    logger.debug(f"Could not extract entity ID from OData-EntityId: {odata_entityid}")
            else:
                logger.debug(f"No Location or OData-EntityId header found")
        else:
            logger.debug(f"Status code {response.status_code} does not indicate successful creation")
