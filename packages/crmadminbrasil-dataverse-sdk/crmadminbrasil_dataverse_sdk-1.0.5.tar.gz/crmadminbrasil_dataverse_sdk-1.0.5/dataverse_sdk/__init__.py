...
class DataverseSDK:
    ...
    def __init__(
        ...
        # Initialize batch processor (will be updated after self is fully initialized)
    ...
    async def upsert(
        ...
        Args:
            entity_type: Entity logical name
            data: Entity data
            alternate_key: Alternate key for upsert (if provided, will attempt update first)
        ...
        if alternate_key:
            # Try to update first using the alternate key
            try:
                # Extract the entity ID from alternate key if it's the primary key
                entity_id = None
                for key, value in alternate_key.items():
                    if key.endswith("id"):  # Primary key field
                        entity_id = value
                        break
                
                if entity_id:
                    # Use regular update method
                    response = await self.update(entity_type, entity_id, data, return_record=return_record)
                    
                    if return_record:
                        return response
                    else:
                        return UpsertResult(entity_id=entity_id, created=False)
                else:
                    # Build alternate key path for true alternate key upsert
                    key_parts = []
                    for key, value in alternate_key.items():
                    ...
                    # Use PATCH for alternate key upsert
                    response = await self.client.patch(endpoint, data, headers=headers)
                    ...
                    if return_record:
                        ...
                        if not entity_id:
                            odata_id = response.get("@odata.id", "")
                            if odata_id:
                                entity_id = extract_entity_id(odata_id)
                        
                        return UpsertResult(entity_id=entity_id or "unknown", created=False)
                        
            except Exception as e:
                # If update fails, fall back to create
                logger.debug(f"Update failed, falling back to create: {str(e)}")
                pass
        
        # No alternate key provided or update failed - create new entity
        response = await self.client.post(entity_type, data, headers=headers)
        ...
        if return_record:
            ...
            entity_id = response.get("id")
            if not entity_id:
                # Try to extract from Location header or @odata.id
                odata_id = response.get("@odata.id", "")
                if odata_id:
                    entity_id = extract_entity_id(odata_id)
                else:
                    # For POST operations, try to get from response headers
                    # This is a fallback - in practice, the ID should be in the response
                    entity_id = "created_successfully"
            
            return UpsertResult(entity_id=entity_id or "unknown", created=True)
    ...
    async def bulk_upsert(
        ...
        Returns:
            Bulk operation result
        """
        return await self.batch_processor.bulk_upsert(
            entity_type, entities, alternate_key_field, batch_size, parallel
        )
