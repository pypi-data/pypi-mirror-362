...
class BatchProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            sdk: Reference to the main SDK instance for fallback operations
        """
        self.client = client
        self.sdk = sdk  # Reference to main SDK for fallback operations
    ...
    async def bulk_create(
        ...
        """
        # Fallback to sequential operations if batch operations are not supported
        logger.info(f"Executing bulk create for {len(entities)} entities using sequential operations")
        
        result = BulkOperationResult(total_processed=len(entities))
        
        for i, entity_data in enumerate(entities):
            try:
                # Use the SDK's create method directly
                entity_id = await self.sdk.create(entity_type, entity_data)
                result.successful += 1
                logger.debug(f"Created entity {i+1}/{len(entities)}: {entity_id}")
            except Exception as e:
                result.failed += 1
                result.errors.append({
                    "index": i,
                ...
                logger.error(f"Failed to create entity {i+1}/{len(entities)}: {str(e)}")
        
        logger.info(
            f"Bulk create completed: {result.successful} successful, {result.failed} failed"
        )
    ...
    async def bulk_update(
        ...
        """
        # Fallback to sequential operations if batch operations are not supported
        logger.info(f"Executing bulk update for {len(updates)} entities using sequential operations")
        
        result = BulkOperationResult(total_processed=len(updates))
        
        for i, update_data in enumerate(updates):
            try:
                # Extract entity ID without removing it from the original data
                entity_id = update_data.get("id")
                if not entity_id:
                    raise ValidationError("Entity ID is required for bulk update")
                
                # Create a copy of update data without the ID
                update_fields = {k: v for k, v in update_data.items() if k != "id"}
                
                # Use the SDK's update method directly
                await self.sdk.update(entity_type, entity_id, update_fields)
                result.successful += 1
                logger.debug(f"Updated entity {i+1}/{len(updates)}: {entity_id}")
            except Exception as e:
                result.failed += 1
                result.errors.append({
                    "index": i,
                ...
                logger.error(f"Failed to update entity {i+1}/{len(updates)}: {str(e)}")
        
        logger.info(
            f"Bulk update completed: {result.successful} successful, {result.failed} failed"
        )
    ...
    async def bulk_delete(
        ...
        """
        # Fallback to sequential operations if batch operations are not supported
        logger.info(f"Executing bulk delete for {len(entity_ids)} entities using sequential operations")
        
        result = BulkOperationResult(total_processed=len(entity_ids))
        
        for i, entity_id in enumerate(entity_ids):
            try:
                # Use the SDK's delete method directly
                await self.sdk.delete(entity_type, entity_id)
                result.successful += 1
                logger.debug(f"Deleted entity {i+1}/{len(entity_ids)}: {entity_id}")
            except Exception as e:
                result.failed += 1
                result.errors.append({
                    "index": i,
                ...
                logger.error(f"Failed to delete entity {i+1}/{len(entity_ids)}: {str(e)}")
        
        logger.info(
            f"Bulk delete completed: {result.successful} successful, {result.failed} failed"
        )
    ...
    async def bulk_upsert(
        ...
        entity_type: str,
        entities: List[Dict[str, Any]],
        alternate_key_field: Optional[str] = None,
        batch_size: Optional[int] = None,
        parallel: bool = True,
    ) -> BulkOperationResult:
        """
        Bulk upsert entities (create or update).
        
        Args:
            entity_type: Entity logical name
            entities: List of entity data (may include alternate keys)
            alternate_key_field: Field name to use as alternate key (e.g., 'accountid', 'emailaddress1')
            batch_size: Size of each batch
            parallel: Whether to execute batches in parallel
        ...
        """
        # Fallback to sequential operations if batch operations are not supported
        logger.info(f"Executing bulk upsert for {len(entities)} entities using sequential operations")
        
        result = BulkOperationResult(total_processed=len(entities))
        
        for i, entity_data in enumerate(entities):
            try:
                # Determine alternate key for upsert
                alternate_key = None
                if alternate_key_field and alternate_key_field in entity_data:
                    alternate_key = {alternate_key_field: entity_data[alternate_key_field]}
                    # Remove the alternate key from data to avoid duplication
                    upsert_data = {k: v for k, v in entity_data.items() if k != alternate_key_field}
                else:
                    # No alternate key specified, this will be a create operation
                    upsert_data = entity_data.copy()
                
                # Use the SDK's upsert method directly
                upsert_result = await self.sdk.upsert(entity_type, upsert_data, alternate_key=alternate_key)
                result.successful += 1
                
                operation_type = "created" if upsert_result.created else "updated"
                logger.debug(f"Upserted entity {i+1}/{len(entities)} ({operation_type}): {upsert_result.entity_id}")
                ...
                logger.error(f"Failed to upsert entity {i+1}/{len(entities)}: {str(e)}")
        
        logger.info(
            f"Bulk upsert completed: {result.successful} successful, {result.failed} failed"
        )
