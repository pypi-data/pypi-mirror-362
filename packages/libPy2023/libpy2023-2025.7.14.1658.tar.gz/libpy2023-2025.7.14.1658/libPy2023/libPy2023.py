...
def print(fila:int=10, columna:int=10, texto:str=""):
...
def cColor(texto:str="", color:str= Color.LAzul):
...
_TIPOEJECUCION=Color.Bold+Color.Verde+("TEST" if _PRUEBA else "PRODUCTIVO")+Color.Fin

class logs:
    """ logs: Esta clase permite mantener un log de eventos qeu se muestran en pantalla y se graban en un archivo de logs
        logs(NombreArchivo) --> NobreArchivo es el archivo donde se guardara el log.
        log.texto(xTexto)   -->

    """
    def __init__(self,NombreArchivo, xlog:bool=True):
