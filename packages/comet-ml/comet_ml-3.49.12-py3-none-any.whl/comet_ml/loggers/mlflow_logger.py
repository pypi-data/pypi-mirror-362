...
def _create_experiment(experiment_name: Optional[str] = None) -> CometExperiment:
    ...
    exp._log_other(OTHER_KEY_CREATED_FROM, FRAMEWORK, include_context=False)

    return exp


def _get_or_create_experiment(experiment: Optional[CometExperiment]) -> CometExperiment:
    if experiment:
        return experiment
...
def mlflow_log_metric(
    experiment, original, return_value, key, value, step=None, **kwargs
):
...
def mlflow_log_metrics(
    experiment, original, return_value, metrics, step=None, **kwargs
):
    # MLFlow auto-logging is passing epochs in the step parameter
    _get_or_create_experiment(experiment).log_metrics(metrics, epoch=step)


def mlflow_log_param(experiment, original, return_value, key, value, **kwargs):
    _get_or_create_experiment(experiment)._log_parameter(
        key, value, source=ParameterMessage.source_autologger
    )


def mlflow_log_params(experiment, original, return_value, params, **kwargs):
...
def mlflow_set_tag(experiment, original, return_value, key, value, **kwargs):
    _get_or_create_experiment(experiment).log_other(key, value)


def mlflow_set_tags(experiment, original, return_value, tags, **kwargs):
    _get_or_create_experiment(experiment).log_others(tags)


def mlflow_log_artifact(
    experiment, original, return_value, local_path, artifact_path=None, *args, **kwargs
):
...
def mlflow_log_artifacts(
    experiment, original, return_value, local_dir, artifact_path=None, *args, **kwargs
):
    global LOG_MODEL_MODEL_NAME
...
def mlflow_model_log_before(
    ...
    **kwargs
    ...
    if registered_model_name is not None:
        ...
        LOGGER.debug(
            "MLFlow log model called with registered model name: %r",
            registered_model_name,
        )
    else:
        LOG_MODEL_MODEL_NAME = artifact_path
        LOGGER.debug(
            "MLFlow log model called without registered model name will use artifact path: %r",
            artifact_path,
        )
...
def mlflow_set_experiment(
    experiment, original, return_value, experiment_name, **kwargs
):
    global PROJECT_NAME
