...
class Spkio:
    ...
    def upload(self, content_file, idKey=None, isSource=None):
        if content_file is None or not os.path.exists(content_file):
            raise Exception('No file selected or not exist: {content_file}')
        res = self.req.post(f'{self.URL_SPKIO_SERVICE}/content', files=files)
        return self._catchCreatedResponse(res)
