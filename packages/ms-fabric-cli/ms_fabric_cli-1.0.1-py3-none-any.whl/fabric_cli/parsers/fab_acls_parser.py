...
commands = {
    "Commands": {
        utils_ui.get_os_specific_command(
            "ls"
        ): "List ACLs for a workspace, item, or OneLake.",
        utils_ui.get_os_specific_command("rm"): "Remove an ACL from a workspace.",
...
def register_parser(subparsers: _SubParsersAction) -> None:
    ...
    acls_subparsers = parser.add_subparsers(dest="acls_command")
    ...
    set_parser.add_argument(
        ...
        metavar="",
        required=True,
        help="Role (admin, member, contributor, viewer)",
