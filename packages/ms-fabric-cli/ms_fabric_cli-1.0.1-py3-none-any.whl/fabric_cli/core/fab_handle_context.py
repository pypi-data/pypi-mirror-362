...
from fabric_cli.core.fab_decorators import FabricCLIError
...
def get_command_context(
    ...
    try:
        ...
        if supports_local_path and e.status_code in [
            ...
            # TODO: Check for writing permissions only in the case that the command is writing to the local path
...
def _handle_path_in_item(
    ...
    if folder is None:
        raise FabricCLIError(
            ErrorMessages.Common.folder_not_found(
...
def _handle_path_in_onelake(
    ...
    if response is None:
        ...
        response = _get_onelake_details(_path)
    if response is None:
        # If the path is not found, it depends on the raise_error flag
        # and if it is the last element of the path (e.g. for the "exists" and "mkdir" commands)
    ...
    # If the path is within a shortcut, we also have the header "x-ms-onelake-from-shortcut-path": "true"
    # If it is a shortcut, it will have the "x-ms-onelake-shortcut-path": "true",
    # In order to check if it is a table, we check if parent folder is Tables
