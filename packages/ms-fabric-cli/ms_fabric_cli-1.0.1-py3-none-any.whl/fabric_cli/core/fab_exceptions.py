...
class FabricAPIError(FabricCLIError):
    ...
    def formatted_message(self, debug=False):
        base_message = super().formatted_message(debug)
        ...
        final_message = (
            base_message
            if detailed_message == "" or not debug
...
class OnelakeAPIError(FabricCLIError):
    def __init__(self, response_text):
        ...
        # Check for 'RequestId=' in message, extract it, and remove from message
        # Check for 'Time=' in message, extract it, and remove from message
        if message:
            message = re.sub(r"\n(?=RequestId:)", "", message)
    ...
    def formatted_message(self, debug=False):
        message = super().formatted_message(debug)
