...
class Command(Enum):
    ...
    def get_command_path(args):
        ...
        if command_group == "acl":
            return cli_command_string + " " + (args.acls_command or "")
...
# TODO: Add support for CLoader, breaks the mypy type checking
# try:
...
def get_command_support_dict():
    ...
    if mem_dictionary is None:
        ...
        # yaml_file = fab_state_config.get_config(fab_constant.FAB_LOCAL_COMMAND_YAML)
        # if not yaml_file:
...
def _get_property_from_commands_and_subcommands(
    ...
    _sub_command_dict = _command_group_dict.get(
        "subcommands", {}).get(_sub_command, {})
...
def get_supported_elements(command: Command) -> list[FabricElementType]:
    ...
    elements: list[str] = _get_property_from_commands_and_subcommands(
        command, _map_key)
...
def _get_items(command: Command, key: str) -> list[_BaseItemType]:
    items: list[str] = _get_property_from_commands_and_subcommands(
        command, key)
