...
def export_bulk_items(workspace: Workspace, args: Namespace) -> None:
    ...
    if selected_items:
        ...
        if args.force or utils_ui.prompt_confirm("Item definition is exported without its sensitivity label. Are you sure?"):
...
def export_single_item(
    ...
    if args.force or utils_ui.prompt_confirm("Item definition is exported without its sensitivity label. Are you sure?"):
        ...
        if do_export:
            ...
            utils_ui.print_done(f"'{item.full_name}' exported")
