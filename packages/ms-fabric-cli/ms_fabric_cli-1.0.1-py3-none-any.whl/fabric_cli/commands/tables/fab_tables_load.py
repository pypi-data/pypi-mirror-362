...
def exec_command(args: Namespace, context: OneLakeItem) -> None:
    ...
    if args.format:
        ...
        # If format is 'Parquet', ignore other options (header, delimiter)
    ...
    if response.status_code == 202:
        utils_ui.print_done(f"Load table operation started")
    elif response.status_code in [200, 201]:
        utils_ui.print_done(f"Table '{args.table_name}' loaded successfully")
