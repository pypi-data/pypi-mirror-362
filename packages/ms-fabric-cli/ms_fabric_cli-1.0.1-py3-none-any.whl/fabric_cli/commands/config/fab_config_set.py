...
# TODO: Remove normalize when bumping breaking change version
...
def _set_config(key: str, value: Any, debug: bool = True) -> None:
    # Check if path exists
    if key in (
        fab_constant.FAB_LOCAL_DEFINITION_LABELS,
        # fab_constant.FAB_LOCAL_COMMAND_YAML,
    ):
        if value is None:
            raise FabricCLIError(
    ...
    if debug:
        utils_ui.print_done(f"Configuration '{key}' set to '{value}'")

    current_mode = fab_state_config.get_config(fab_constant.FAB_MODE)

    # Check for Mode transition and exit
    if (
...
def _set_capacity(args: Namespace, value: str) -> None:
    # Remove .Capacity from name if provided
    ...
    if response.status_code in (200, 201):
        ...
        for item in data.get("value", []):
            if item.get("displayName", "").lower() == value.lower():
                _set_config(fab_constant.FAB_DEFAULT_CAPACITY, value)
                _set_config(fab_constant.FAB_DEFAULT_CAPACITY_ID, item.get("id"), False)
                return
        raise FabricCLIError(
