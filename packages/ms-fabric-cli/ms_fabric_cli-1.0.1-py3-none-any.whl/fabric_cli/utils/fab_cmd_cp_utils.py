...
class OneLakeFileContent:
    def __init__(self, properties: dict, content: str) -> None:
...
def upload_file_onelake(
    ...
    if response.status_code == 201:
        ...
        if isinstance(file_content, bytes):
            ...
            content_length = file_content.get_content_length()
...
def get_file_content_onelake(args: Namespace) -> OneLakeFileContent:
    ...
    return OneLakeFileContent(properties, response.text)
...
def get_onelake_file_destination(
    ...
    # If the destination is a file or not exists (undefined and no id), return the context as is
        ...
        to_context.nested_type == OneLakeItemType.UNDEFINED
        and to_context.id is None
    ):
        return to_context
