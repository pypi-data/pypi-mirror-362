...
def search(data: Any, expression: str, verbose: Optional[bool] = False) -> str | None:
    if not expression:
        max_depth = float("inf") if verbose else 4
        paths = _get_json_paths(data, max_depth=max_depth)
        return "\n".join(paths)
    else:
        try:
            if "." == expression:
            ...
            fab_logger.log_error(
                "Not a valid jmespath query (https://jmespath.org)"
            )
            return None
...
def _get_json_paths(json_obj, current_path="", depth=0, max_depth=4):
    ...
    if isinstance(json_obj, dict):
        ...
        # If it's neither a dictionary nor a list, it's a leaf node (a scalar value)
        paths.append(current_path)
