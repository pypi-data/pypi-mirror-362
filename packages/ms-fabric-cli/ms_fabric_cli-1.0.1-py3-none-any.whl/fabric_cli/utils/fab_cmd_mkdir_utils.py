    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            # Mirrored database
                ...
                # Mounted Tables Parameter in form of 'schema.table,schema.table'
...
def get_connection_config_from_params(payload, con_type, con_type_def, params):
    ...
    if not params.get("connectiondetails").get("creationmethod"):
        if provided_params:
            ...
            raise FabricCLIError(
                f"Missing Connection Creation Method and Parameters. Please indicate either one of the following creation methods: {supported_creation_methods}, or provide parameters for automatic selection",
                fab_constant.ERROR_INVALID_INPUT,
            )
    else:
