            ...
            if "payload" in part:
                ...
                if payload_base64:
                    ...
                    try:
                        ...
                        # Try decoding as UTF-8, otherwise keep raw bytes
                        try:
...
def export_json_parts(definition: dict, export_path: dict) -> None:
    """
    ...
    for part in parts:
        ...
        utils_storage.write_to_storage(export_path, payload, export=True)


def clean_notebook_cells(ntbk_json: dict, tags_to_clean: list) -> dict:
    if ("definition" in ntbk_json and
            "parts" in ntbk_json["definition"] and
            len(ntbk_json["definition"]["parts"]) > 0 and
            "payload" in ntbk_json["definition"]["parts"][0] and
            "cells" in ntbk_json["definition"]["parts"][0]["payload"]):
