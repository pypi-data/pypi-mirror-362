...
def build_virtual_item_id_cache_key(container: VirtualItemContainer, item: Item):
    return keys.hashkey(container.tenant.id, container.workspace.id, item.id)
...
def get_external_data_shares(
    ...
    for item in ws_items:
        if item.item_type in utils.item_types_supporting_external_data_shares():
        ...
        for eds in data["value"]:
            try:
                external_data_shares_for_item.append(
                    ExternalDataShareVirtualItem(
                        utils.get_external_data_share_name(item.name, eds["id"]),
                        eds["id"],
                        container,
...
def _get_external_data_share_id(
    ...
    item_name = utils.get_item_name_from_eds_name(external_data_share_name)
...
def delete_external_data_share_from_cache(
    ...
    # TODO: Improve logic
    if state_config.get_config(fab_constant.FAB_CACHE_ENABLED) == "true":
        item_name = utils.get_item_name_from_eds_name(external_data_share.name)
