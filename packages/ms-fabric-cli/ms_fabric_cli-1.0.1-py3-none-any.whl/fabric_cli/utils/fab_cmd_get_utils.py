...
def query_and_export(
    data: Any, args: Namespace, file_name: str, debug: Optional[bool] = True
) -> None:
    json_path_response = utils_jmespath.search(data, args.query, args.verbose)

    if args.output:
        utils_storage.do_output(data=json_path_response, file_name=file_name, args=args)
    elif json_path_response and debug:
        utils_ui.print_grey(json_path_response)
