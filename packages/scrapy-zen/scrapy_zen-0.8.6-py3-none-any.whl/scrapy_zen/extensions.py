...
from scrapy_zen.logging import ZenBufferedLogHandler
...
class ZenBufferedLogs:
    ...
    def __init__(self, filename: str, buffer_length: int, formatter: LogFormatter):
        self.filename = filename
        self.buffer_length = buffer_length
        self.handler = ZenBufferedLogHandler(buffer_length)
    ...
    def from_crawler(cls, crawler: Crawler) -> Self:
        ...
        buffer_length = crawler.settings.getint("ZEN_BUFFEREDLOGS_LENGTH", 30)
        filename = pathlib.Path(log_file).stem + "_buffered.log"
        ext = cls(filename, buffer_length, crawler.logformatter)

        crawler.signals.connect(ext.new_item, signal=scrapy_zen.signals.new_item)
        return ext

    def new_item(self, item: Dict, spider: Spider):
        spider.logger.info("New Item detected")
        self.handler.flush(self.filename)
