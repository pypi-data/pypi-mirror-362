...
from scrapy_zen.logging import ScrapyDictFormatter, ZenBufferedLogHandler
...
class ZenBufferedLogs:

    def __init__(self, handler: ZenBufferedLogHandler):
    ...
    @classmethod
    def from_crawler(cls, crawler: Crawler) -> Self:
        if not crawler.settings.getbool("ZEN_BUFFEREDLOGS_ENABLED"):
            raise NotConfigured
        # disable other loggging
        crawler.settings.set("LOG_ENABLED", False, "project")
        crawler.settings.set("LOG_FILE", None, "project")
        
        log_file = crawler.settings.get("ZEN_BUFFEREDLOGS_LOG_FILE")
        if not log_file:
            raise NotConfigured("ZEN_BUFFEREDLOGS_LOG_FILE not defined")

        buffer_length = crawler.settings.getint("ZEN_BUFFEREDLOGS_LENGTH", 50)
        log_format = crawler.settings.get('LOG_FORMAT', '%(asctime)s [%(name)s] %(levelname)s: %(message)s')
        log_dateformat = crawler.settings.get('LOG_DATEFORMAT', '%Y-%m-%d %H:%M:%S')
        formatter = ScrapyDictFormatter(fmt=log_format, datefmt=log_dateformat)
        handler = ZenBufferedLogHandler(log_file, buffer_length)
    ...
    def item_scraped(self, item: Dict, spider: Spider):
        self.handler.flush()
