...
from .logging import ZenLogFormatter
...
class ZenAddon:
    ...
    def update_settings(self, settings: Settings) -> None:
        settings.set("LOG_FORMAT", "%(asctime)s.%(msecs)03d [%(name)s] %(levelname)s: %(message)s", "addon")
        ...
        settings["EXTENSIONS"].update(
            {
                ...
                "scrapy_zen.extensions.ZenStats": 552,
                "scrapy_zen.extensions.ZenBufferedLogs": 553,
                "scrapy.extensions.logstats.LogStats": None, # disable default logstats (ZenStats will handle it)
            }
        )
