...
class TestItem(pytest.Item):
    ...
    def runtest(self):
        ...
        if "files" in params.keys():
            params.update({"files": FileHelper.replace_file_to_stream(params.get("files"), 'files')})
        if "file"in params.keys():
            params.update({"files": FileHelper.replace_file_to_stream(params.get("file"))})
            params.pop("file")
            ...
            while exec_count < local_test_loop_times and not exec_flag :  # 异步任务，轮询执行local_test_loop_times次保证完成后进行校验
                ...
                if exec_count > 0 or not exec_flag:
                    if exec_count <= local_test_loop_times - 2:
                        ...
                        logger.error(f'用例:{self.name} 执行5min后仍失败，请检查！')
                        log_attach(f'用例:{self.name} 执行5min后仍失败，请检查！')
                        pytest.fail(f'用例:{self.name} 执行5min后仍失败，请检查！')
                exec_count += 1
        except Exception:
