...
class UnifiedSigilPipeline:
    ...
    def __init__(
            ...
            llm_config: Optional[Any] = None) -> None:
        self.config = config
                ...
                try:
                    if UnifiedLLMProcessor is not None:
                        self.unified_llm_processor = UnifiedLLMProcessor(
                            self.llm_config)
                        self.logger.info(
                            f"✅ Unified LLM Processor initialized with provider: {
                                self.llm_config.provider}")
                    else:
                        self.logger.warning(
                            "⚠️  UnifiedLLMProcessor is None at runtime; skipping initialization.")
            ...
            # (fallback)
                ...
                try:
                    if AzureOpenAIEnricher is not None:
                        ...
                        self.logger.info(
                            "✅ Azure OpenAI Enricher initialized successfully")
                    else:
                        self.logger.warning(
                            "⚠️  AzureOpenAIEnricher is None at runtime; skipping initialization.")
                except Exception as e:
                    self.logger.warning(
    ...
    async def analyze_crate(
            self,
            crate_name: str,
            crate_version: Optional[str] = None) -> SacredChainTrace:
    ...
    async def _gather_documentation(
            self, crate_name: str) -> Dict[str, ScrapingResult]:
        if not self.scraper:
    ...
    async def _perform_sacred_chain_analysis(
        ...
        self.logger.info(
            f"🔗 Performing Sacred Chain analysis for {crate_name}")

        try:
            sanitized_docs = self.sanitizer.sanitize_data(
    ...
    async def _handle_toolchain_override(self, crate_source_path: Path) -> Optional[Path]:
        ...
        for toolchain_file in toolchain_files:
            if toolchain_file.exists():
                ...
                backup_path = toolchain_file.with_suffix(toolchain_file.suffix + ".backup")
                try:
                    toolchain_file.rename(backup_path)
                    self.logger.info(f"Temporarily disabled toolchain override: {toolchain_file} -> {backup_path}")
                    return backup_path
                except Exception as e:
                    self.logger.warning(f"Failed to backup toolchain file {toolchain_file}: {e}")
    ...
    async def _restore_toolchain_override(self, backup_path: Optional[Path]) -> None:
        ...
        if backup_path and backup_path.exists():
            try:
                ...
                self.logger.info(f"Restored toolchain override: {backup_path} -> {original_path}")
            except Exception as e:
                self.logger.warning(f"Failed to restore toolchain file {backup_path}: {e}")

    async def _add_crate_analysis_results(
            self,
            crate_name: str,
            crate_version: str,
            trace: SacredChainTrace) -> None:
        ...
        try:
            if trace.audit_info.get("should_analyze_source_code", True):
                ...
                if not crate_source_path:
                    trace.audit_info["crate_analysis"] = {
                        "status": "error", "note": "Failed to download or extract crate."}
                    return
            ...
            self.logger.warning(
                f"⚠️  Failed to add crate analysis results: {e}")
            trace.audit_info["crate_analysis"] = {
    ...
    async def _download_and_extract_crate(
        ...
        try:
            ...
            self.logger.error(
                f"Error downloading or extracting crate {crate_name}: {e}")
            return None

    async def _get_latest_crate_version(
            self, crate_name: str) -> Optional[str]:
    ...
    async def _run_cargo_command(
            self, command: List[str], cwd: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if process.returncode != 0:
                self.logger.warning(
                    f"Cargo command failed with exit code {
                        process.returncode}")
                ...
                for line in stdout.decode(errors='ignore').splitlines():
                    if line.strip():
                        try:
                            ...
                            self.logger.warning(
                                f"Could not parse JSON line: {line}")
            return results
        except Exception as e:
            self.logger.error(f"Error running cargo command: {e}")
            return []
        finally:
    ...
    async def _run_cargo_audit(self, cwd: Path) -> Optional[Dict[str, Any]]:
        ...
        command = ["cargo", "+stable", "audit", "--json"]
        self.logger.info(f"Running command: {' '.join(command)} in {cwd}")
        process = None
        try:
            process = await asyncio.create_subprocess_exec(
            ...
            if process.returncode != 0:
                ...
                self.logger.info(
                    f"Cargo audit finished with exit code {
                        process.returncode}")
            ...
            if stdout:
                try:
                    ...
                    self.logger.warning(
                        f"Could not parse cargo audit JSON output: {
                            stdout.decode(
                                errors='ignore')}")

            if stderr:
                self.logger.warning(
                    f"Stderr from cargo audit: {
                        stderr.decode(
                            errors='ignore')}")

            return None
        except Exception as e:
            self.logger.error(f"Error running cargo audit: {e}")
            return None
        finally:
            # Ensure process is properly cleaned up
            if process and process.returncode is None:
    ...
    async def _add_ai_enrichment(
        ...
        if self.unified_llm_processor:
            ...
            self.logger.info(
                "ℹ️  No AI enricher available, skipping AI enrichment")
    ...
    async def _add_unified_llm_enrichment(
        ...
        try:
            self.logger.info(
                f"🤖 Adding unified LLM enrichment for {crate_name}")
            ...
            crate_metadata = CrateMetadata(
                ...
                code_snippets=[],
                readme_sections={},
                librs_downloads=scraped_data.get(
                    'librs.org',
                    {}).get(
                    'data',
                    {}).get(
            ...
            enriched_crate = self.unified_llm_processor.enrich_crate(
                crate_metadata)
            ...
            self.logger.info(
                f"✅ Enriched data for {crate_name} using Unified LLM")

        except Exception as e:
            self.logger.warning(
                f"⚠️  Failed to add unified LLM enrichment: {e}")
    ...
    def _extract_readme_content(self, scraped_data: Dict[str, Any]) -> str:
        ...
        for source in sources_priority:
            ...
            if readme_text and isinstance(readme_text, str) and len(readme_text.strip()) > 100:
                self.logger.info(f"📖 Using README content from {source} ({len(readme_text)} chars)")
                return readme_text
        ...
        if combined_content.strip():
            self.logger.info(f"📖 Using combined README content ({len(combined_content)} chars)")
    ...
    async def _add_azure_openai_enrichment(
            self, crate_name: str, trace: SacredChainTrace) -> None:
        ...
        try:
            self.logger.info(
                f"🤖 Adding Azure OpenAI enrichment for {crate_name}")
            ...
            self.logger.info(
                f"✅ Enriched data for {crate_name} using Azure OpenAI")

        except Exception as e:
            self.logger.warning(
                f"⚠️  Failed to add Azure OpenAI enrichment: {e}")
    ...
    async def _generate_analysis_report(
        ...
        try:
            ...
            enrichment_path = report_data.get(
                'audit_info', {}).get('llm_enrichment')
            if enrichment_path is not None and not isinstance(
                ...
                report_data['audit_info']['llm_enrichment'] = str(
                    enrichment_path)
    ...
    async def analyze_multiple_crates(
            self, crate_names: List[str]) -> Dict[str, SacredChainTrace]:
        if not crate_names:
            return {}
        ...
        async def analyze_single_crate(
                crate_name: str) -> "tuple[str, SacredChainTrace]":
            async with semaphore:
                try:
                    ...
                    self.logger.error(
                        f"❌ Analysis failed for {crate_name}: {e}")
...
def create_pipeline_from_args(
        args: argparse.Namespace) -> UnifiedSigilPipeline:
...
async def quick_analyze_crate(
        ...
        llm_config: Optional[Any] = None) -> SacredChainTrace:
...
async def batch_analyze_crates(crate_names: List[str],
                               config: Optional[PipelineConfig] = None,
                               llm_config: Optional[Any] = None) -> Dict[str,
                                                                         SacredChainTrace]:
