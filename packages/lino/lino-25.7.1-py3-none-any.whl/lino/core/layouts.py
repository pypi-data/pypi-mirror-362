...
class LayoutHandle:
    ...
    def __init__(self, layout, ui):
        ...
        if self.main is None:
            raise Exception(
                "Failed to create main element %r for %s." % (layout.main, layout)
            )
    ...
    def desc2elem(self, elemname, desc, **kwargs):
        ...
        if "*" in desc:
            ...
            if mk and mk not in explicit_specs and mk not in self.hidden_elements:
                desc += " " + mk
...
class BaseLayout:
    ...
    def __str__(self):
        return "{}.{} on {!r}".format(
            self.__class__.__module__, self.__class__.__name__, self._datasource
        )
        ...
        if "\n" in self.main:
            if hasattr(self, "general"):
                raise NotImplementedError(
                    """\
%s has both a vertical `main` and a panel called `general`."""
                    % self
                )
