...
class DisplayElement(FieldElement):
    ...
    def value2html(self, ar, v, **cellattrs):
        if isinstance(v, SafeString):
            ...
            # fixes #5687 (bad argument type: __proxy__('Foster mother'))
            v = str(v)
        ...
        try:
            ...
            logger.warning("Exception in value2html of %s:", self)
            logger.exception(e)
...
class LightWeightContainer(HtmlBoxElement):
    ...
    This creates an automatic VirtualField, which is special because it happens
    "dynamically" after the startup analysis, during
...
class SlaveSummaryPanel(LightWeightContainer):
    ...
    The panel used to display a slave table when `display_mode` is
