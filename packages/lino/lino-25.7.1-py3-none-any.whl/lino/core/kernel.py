...
class Kernel(object):
    ...
    def kernel_startup(self, site):
        ...
        if False:
            logger.info(
                "Languages: %s. %d apps, %d models, %s actors.",
        ...
        for a in actors.actors_list:
            if (
                isinstance(a.model, type)
                and issubclass(a.model, models.Model)
        ...
        for a in actors.actors_list:
            ...
            a._actions_list = sorted(
                a._actions_list, key=lambda ba: ba.action.sort_index
            )
        ...
        for model in models_list:
            for m, fk in model._lino_ddh.fklist:
                ...
                if fk.remote_field.on_delete == models.CASCADE:
                    ...
                    if fk.name in m.allow_cascaded_delete:
                        msg = (
                            "{0}.{1} specified in allow_cascaded_delete "
                            "but on_delete is not CASCADE"
                        ).format(fmn(m), fk.name)
                        raise Exception(msg)
    ...
    def run_action(self, ar):
        ...
        if not a.readonly:
            ...
            if self.site.log_each_action_request:
                ...
                # The following caused #5849 (You cannot access body after
                # reading from request's data stream) on a production site:
                # A(format_request(ar.request))
                ar.logger.info("run_action {0}".format(" ".join(flds)))
                # logger.info("run_action {0}".format(ar))
        # if str(a) != 'grid_put':
        #     raise Exception(f"20240911 run_action() {a}")
        try:
            if isinstance((resp := a.run_from_ui(ar)), HttpResponse):
                return resp
            if (
                ar.request.method != "GET"
            ...
            if (
                ...
                and ar.response.get("success", False)
            ):
...
def register_actors():
    ...
    for rpt in slave_tables:
        ...
        if not isinstance(rpt.master, type):
            raise Exception(
                "20160712 invalid master {!r} in {}".format(rpt.master, rpt)
            )
