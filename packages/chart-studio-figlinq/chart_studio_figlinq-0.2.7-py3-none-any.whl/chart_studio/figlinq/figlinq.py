...
class Grid(_Grid):
    ...
    def to_dataframe(self):
        ...
        :return: pandas DataFrame representation of the Grid.
        ...
        data = {col.name: col.data for col in self}
        return pd.DataFrame(data)
    
    @classmethod
    def from_dataframe(cls, df):
        """
        Create a Grid object from a pandas DataFrame.
        :param df: pandas DataFrame to convert.
        :return: Grid instance representing the DataFrame.
        :raises ImportError: if chart_studio is not installed.
        """
        try:
            from chart_studio.grid_objs import Grid as _Grid, Column
        except ImportError:
            raise ImportError("chart_studio is required to use from_dataframe")

        try:
            columns = [Column(name=col_name, data=df[col_name].tolist()) for col_name in df.columns]
