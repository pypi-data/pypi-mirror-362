...
try:
    from tika import detector, parser  # type:ignore
...
class FileToHTML(Pipeline):
    ...
    def __init__(self, backend="available"):
        ...
        Args:
            backend: backend to use to extract content, supports "tika", "docling" or
...
class Docling:
    ...
    def normalize(self, html):
        ...
        html = (
            html.replace("<head/>", "<head/><body>").replace("</html>", "</body></html>")
            if "<body>" not in html
            else html
        )
