...
from .db_encoder import EncoderFactory
from .db_sql import SQL, SQLError, Token
...
class Database:
    ...
    def search(self, query, similarity=None, limit=None, parameters=None, indexids=False):
        ...
        Runs a search against the database. Supports the following methods:
            ...
            3. SQL with no similarity query. This mode runs a SQL query and retrieves the results without similarity.

        Example queries:
            "natural language processing" - standard similarity only query
            "select * from melowrag where similar('natural language processing')" - similarity query as SQL
            "select * from melowrag where similar('nlp') and entry > '2021-01-01'" - similarity query with additional SQL
            "select id, text, score from melowrag where similar('nlp')" - similarity query with additional SQL column
            "select * from melowrag where entry > '2021-01-01' - database only query
    ...
    def registerfunctions(self, config):
        ...
        if inputs:
            ...
            for fn in inputs:
                ...
                if not isinstance(fn, types.FunctionType) and callable(fn):
