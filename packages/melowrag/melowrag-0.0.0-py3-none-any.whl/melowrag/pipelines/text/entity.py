...
class Entity(HFPipeline):
    ...
    def __call__(self, text, labels=None, aggregate="simple", flatten=None, join=False, workers=0):
        ...
        for result in results:
            if flatten:
                output = [
                    r["word"] for r in result if self.accept(r["entity_group"], labels) and r["score"] >= threshold
                ]
                outputs.append(" ".join(output) if join else output)
            else:
                outputs.append(
                    [
                        (r["word"], r["entity_group"], float(r["score"]))
                        for r in result
                        if self.accept(r["entity_group"], labels)
                    ]
                )
