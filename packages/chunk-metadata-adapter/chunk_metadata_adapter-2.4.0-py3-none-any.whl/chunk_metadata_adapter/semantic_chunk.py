...
class SemanticChunk(BaseModel):
    ...
    def is_code_content(self) -> bool:
        ...
        Returns True if:
        1. Chunk type is CODE_BLOCK, or
        2. Language is a programming language (not natural language or UNKNOWN)
    ...
    def to_flat_dict(self, for_redis: bool = True, include_embedding: bool = False) -> dict:
        """
        Converts SemanticChunk to flat dictionary for database or Redis storage.
        ...
        Behavior:
            - If for_redis=True (default): All values serialized to strings, dict/list to JSON, 
              bool to 'true'/'false', None to '', Enum to str, datetime to ISO, created_at auto-filled.
        ...
        Examples:
            ...
            flat = chunk.to_flat_dict()
            
            # Redis storage with embedding included
            flat = chunk.to_flat_dict(include_embedding=True)
...
def _autofill_min_length_str_fields(data, model_fields):
    for name, field in model_fields.items():
        ...
        if base_type is str and name != 'tags':
            # Extract min_length from field metadata
            min_len = 0
