...
class ChunkQuery(BaseModel):
    ...
    def to_flat_dict(self, for_redis: bool = True, include_embedding: bool = False) -> dict:
        """
        Serializes filter to flat dictionary for Redis or database storage.
        
        Parameters:
            for_redis (bool, optional): If True, serialize for Redis storage (strings, JSON). Defaults to True.
            include_embedding (bool, optional): If True, include embedding field in serialization. Defaults to False.
        
        Returns:
            dict: Flattened dictionary ready for storage or API response
            
        Behavior:
            - If for_redis=True (default): All values serialized to strings, nested structures to flat keys.
              Embedding is excluded unless include_embedding=True.
            - If for_redis=False: Values remain in original types, embedding always included.
            - If include_embedding=True: Embedding is always included regardless of for_redis setting.
            - For filters: created_at is not auto-filled (unlike SemanticChunk).
        ...
        Examples:
            ...
            flat = query.to_flat_dict(include_embedding=True)
            
            # API response - embedding always included
            flat = query.to_flat_dict(for_redis=False)
            
        Note:
            For Redis storage, embedding is typically excluded to avoid validation/serialization errors
            in the adapter/core. Use include_embedding=True only when explicitly needed for Redis.
        """
        d = to_flat_dict(self.model_dump(), for_redis=for_redis, include_embedding=include_embedding)
        # For filters: created_at should not be auto-filled
