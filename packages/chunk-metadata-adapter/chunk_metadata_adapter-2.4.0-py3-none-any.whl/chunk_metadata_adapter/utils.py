...
def to_flat_dict(data: dict, parent_key: str = '', sep: str = '.', for_redis: bool = True, first_call: bool = True, include_embedding: bool = False) -> dict:
    """
    Serializes dict to flat dictionary with support for nested operators ($in, $gte, $lte, $gt, $lt).
    Dicts with these operators are not flattened at any nesting level.
    In for_redis mode, all scalar values are serialized to strings.
    
    Parameters:
        data (dict): Input dictionary to flatten
        ...
        first_call (bool, optional): Internal parameter for recursion control. Defaults to True.
    ...
    Behavior:
        - Empty dict serialized as '{}' (Redis) or {} (API)
        - All keys are strings
        - embedding is excluded if include_embedding=False and for_redis=True
        - Nested collections in lists are properly handled (error indicates index)
        - created_at is auto-filled for Redis mode on first call
        
    Examples:
        # Redis mode (default) - embedding excluded
        flat = to_flat_dict(chunk_data, for_redis=True)
        
        # Redis mode with embedding included
        flat = to_flat_dict(chunk_data, for_redis=True, include_embedding=True)
        
        # API mode - embedding always included
        flat = to_flat_dict(chunk_data, for_redis=False)
    """
    import enum
