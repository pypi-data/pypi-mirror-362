...
def test_embedding_serialization_flat_dict_chunkquery():
    """
    Проверяет сериализацию embedding в to_flat_dict для ChunkQuery:
    - по умолчанию (for_redis=True) embedding отсутствует
    - с include_embedding=True embedding включается
    - для for_redis=False embedding включается
    """
    from chunk_metadata_adapter.chunk_query import ChunkQuery
    query = ChunkQuery(
        uuid="12345678-1234-4123-8123-123456789012",
    ...
    # По умолчанию для Redis embedding отсутствует
    flat_redis = query.to_flat_dict(for_redis=True)
    assert "embedding" not in flat_redis
    # Если явно запросить - embedding включается (как строки)
    flat_redis_with_emb = query.to_flat_dict(for_redis=True, include_embedding=True)
    assert "embedding" in flat_redis_with_emb
    assert flat_redis_with_emb["embedding"] == ["0.1", "0.2", "0.3"]
    # Для for_redis=False embedding включается всегда (как float)
    flat_api = query.to_flat_dict(for_redis=False)
    assert "embedding" in flat_api
    assert flat_api["embedding"] == [0.1, 0.2, 0.3] 