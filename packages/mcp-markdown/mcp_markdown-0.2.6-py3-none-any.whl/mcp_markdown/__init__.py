...
def generate_markdown_table(data_list: List[dict], zhibiao_list: Optional[List[str]] = None) -> tuple[str, dict]:
    ...
    if zhibiao_list:
        ...
        display_headers = actual_headers
    ...
    numeric_fields = [
        k for k in headers
        if all(safe_float(item.get(k)) or isinstance(item.get(k), (int, float)) for item in data_list if k in item)
    ]
    ...
    for item in data_list:
        ...
        for header in actual_headers:
            ...
            if is_numeric(value):
                summary[header] += safe_float(value)
