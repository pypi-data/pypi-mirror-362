...
def generate_markdown_table(data_list: List[dict], zhibiao_list: Optional[List[str]] = None) -> tuple[str, dict]:
    ...
    # 过滤掉数据中不存在的字段
    valid_headers = [h for h in headers if any(h in item for item in data_list)]
    ...
    numeric_fields = [
        k for k in headers
        if all(isinstance(item.get(k), (int, float)) for item in data_list if k in item)
    ]
    ...
    summary_row = []
    for header in actual_headers:
        if header in summary and header in numeric_fields:
