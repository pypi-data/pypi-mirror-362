...
def generate_markdown_table(data_list: List[dict], zhibiao_list: Optional[List[str]] = None) -> tuple[str, dict]:
    ...
    # 如果有 zhibiao_list，则按顺序使用对应的英文字段名作为实际列
    display_headers = []
    actual_headers = []
    ...
    if zhibiao_list:
        ...
        display_headers = zhibiao_list  # 显示中文字段名
    else:
        # 不做映射，直接显示原始字段名（或根据需要调整）
        actual_headers = sorted(headers)
        display_headers = actual_headers  # 或者可以映射为中文，如原逻辑

    # 初始化汇总统计字典
    summary = defaultdict(float)
    numeric_fields = [k for k in headers if isinstance(data_list[0].get(k), (int, float))]
    ...
    try:
        ...
        if result.get("code") == 0:
            ...
            if data_list:
                ...
                # 执行幻觉检测
                verify_response(result, data_list)
...
def verify_response(model_answer: str, data_list: List[dict]):
    """
    验证模型回答中的每一项数据是否存在于原始数据中。
    如果发现不匹配的字段值，则打印警告信息。
    """
    for line in model_answer.splitlines():
        if "：" in line or ":" in line:
            key = line.split("：", 1)[0].strip() if "：" in line else line.split(":", 1)[0].strip()
            ...
            for item in data_list:
                for k, v in item.items():
                    # 尝试匹配中文字段名和英文字段名
                    zh_key = next((k_ for k_, v_ in ZHI_BIAO_FIELD_MAP.items() if v_ == k), None)
                    if (k == key or zh_key == key) and str(v) == value:
                        matched = True
                        break
                if matched:
                    break
            if not matched:
                print(f"⚠️ 警告：发现幻觉数据 -> {key}: {value}")
