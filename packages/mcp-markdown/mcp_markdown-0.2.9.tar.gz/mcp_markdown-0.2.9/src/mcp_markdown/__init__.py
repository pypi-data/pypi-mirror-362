...
def generate_markdown_table(data_list: List[dict], zhibiao_list: Optional[List[str]] = None) -> tuple[str, dict]:
    ...
    if zhibiao_list:
        actual_headers = zhibiao_list  # 直接用API字段名
        # 优先用ZHI_BIAO_FIELD_MAP反查中文名，否则用字段名本身
        display_headers = [k for k, v in ZHI_BIAO_FIELD_MAP.items() if v in zhibiao_list]
        if len(display_headers) != len(zhibiao_list):
