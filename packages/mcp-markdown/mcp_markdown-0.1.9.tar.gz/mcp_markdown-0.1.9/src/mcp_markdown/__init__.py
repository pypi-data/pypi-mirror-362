...
def generate_markdown_table(data_list: List[dict], zhibiao_list: Optional[List[str]] = None) -> tuple[str, dict]:
    ...
    # 如果有 zhibiao_list，则按顺序使用对应的中文字段名作为显示
    display_headers = []
    actual_headers = []

    if zhibiao_list:
        for zh_name in zhibiao_list:
            en_name = ZHI_BIAO_FIELD_MAP.get(zh_name)
            if en_name and en_name in headers:
                display_headers.append(zh_name)
                actual_headers.append(en_name)
    else:
        # 自动提取所有字段并尝试匹配中文名称
        for key in sorted(headers):
            zh_name = next((k for k, v in ZHI_BIAO_FIELD_MAP.items() if v == key), f"未知字段({key})")
            display_headers.append(zh_name)
            actual_headers.append(key)

    # 初始化汇总统计字典
    summary = defaultdict(float)
    numeric_fields = [
        'accumulatedPayMoney', 'accumulatedPayUser', 'accumulativeROI', 'activeScale',
        'adBuyUsers', 'clickPrimeCost', 'clickScale', 'cost', 'createRolePrimeCost',
        'createRoleScale', 'dividefirstDayRoi', 'firstDayRoi', 'groupKey',
        'income_val_24h_roi', 'income_val_24hs', 'newPayArpu', 'newPayMoney',
        'newPayPrimeCost', 'newPayScale', 'newPayUser', 'payCount', 'payMoney',
        'payPrimeCost', 'payUser', 'regRoleCount', 'regUserCount'
    ]
...
5. 转换为markdown表格为中间处理过程，需要在查询到数据后立即执行，而不是在输出回答的过程中执行，除非用户明确要求需要图表分析数据。
    ...
    # 构建请求体前，将 zhibiao_list 中文字段转为英文字段
    if zhibiao_list:
        payload_zhibiao_list = [ZHI_BIAO_FIELD_MAP.get(z, z) for z in zhibiao_list]
    else:
        # 默认全部指标，也保持中文字段不变（由后端处理）
