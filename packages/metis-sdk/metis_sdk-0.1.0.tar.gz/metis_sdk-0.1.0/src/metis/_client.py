# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, Union, Mapping
from typing_extensions import Self, override

import httpx

from . import _exceptions
from ._qs import Querystring
from ._types import (
    NOT_GIVEN,
    Omit,
    Headers,
    Timeout,
    NotGiven,
    Transport,
    ProxiesTypes,
    RequestOptions,
)
from ._utils import is_given, get_async_library
from ._version import __version__
from .resources import test, oauth, health, connect, sessions, reconnect
from ._streaming import Stream as Stream, AsyncStream as AsyncStream
from ._exceptions import APIStatusError
from ._base_client import (
    DEFAULT_MAX_RETRIES,
    SyncAPIClient,
    AsyncAPIClient,
)
from .resources.mcp import mcp

__all__ = ["Timeout", "Transport", "ProxiesTypes", "RequestOptions", "Metis", "AsyncMetis", "Client", "AsyncClient"]


class Metis(SyncAPIClient):
    connect: connect.ConnectResource
    reconnect: reconnect.ReconnectResource
    oauth: oauth.OAuthResource
    mcp: mcp.McpResource
    sessions: sessions.SessionsResource
    health: health.HealthResource
    test: test.TestResource
    with_raw_response: MetisWithRawResponse
    with_streaming_response: MetisWithStreamedResponse

    # client options
    api_key: str | None
    model_api_keys: dict[str, str]

    def __init__(
        self,
        *,
        api_key: str | None = None,
        base_url: str | httpx.URL | None = None,
        model_api_keys: dict[str, str] | None = None,
        timeout: Union[float, Timeout, None, NotGiven] = NOT_GIVEN,
        max_retries: int = DEFAULT_MAX_RETRIES,
        default_headers: Mapping[str, str] | None = None,
        default_query: Mapping[str, object] | None = None,
        # Configure a custom httpx client.
        # We provide a `DefaultHttpxClient` class that you can pass to retain the default values we use for `limits`, `timeout` & `follow_redirects`.
        # See the [httpx documentation](https://www.python-httpx.org/api/#client) for more details.
        http_client: httpx.Client | None = None,
        # Enable or disable schema validation for data returned by the API.
        # When enabled an error APIResponseValidationError is raised
        # if the API responds with invalid data for the expected schema.
        #
        # This parameter may be removed or changed in the future.
        # If you rely on this feature, please open a GitHub issue
        # outlining your use-case to help us decide if it should be
        # part of our public interface in the future.
        _strict_response_validation: bool = False,
    ) -> None:
        """Construct a new synchronous Metis client instance.

        This automatically infers the `api_key` argument from the `METIS_API_KEY` environment variable if it is not provided.
        
        Args:
            model_api_keys: Dictionary of provider API keys (e.g., {"openai": "sk-...", "anthropic": "sk-ant-..."})
        """
        if api_key is None:
            api_key = os.environ.get("METIS_API_KEY")
        self.api_key = api_key
        self.model_api_keys = model_api_keys or {}

        if base_url is None:
            base_url = os.environ.get("METIS_BASE_URL")
        if base_url is None:
            base_url = f"https://api.example.com"

        super().__init__(
            version=__version__,
            base_url=base_url,
            max_retries=max_retries,
            timeout=timeout,
            http_client=http_client,
            custom_headers=default_headers,
            custom_query=default_query,
            _strict_response_validation=_strict_response_validation,
        )

        self.connect = connect.ConnectResource(self)
        self.reconnect = reconnect.ReconnectResource(self)
        self.oauth = oauth.OAuthResource(self)
        self.mcp = mcp.McpResource(self)
        self.sessions = sessions.SessionsResource(self)
        self.health = health.HealthResource(self)
        self.test = test.TestResource(self)
        self.with_raw_response = MetisWithRawResponse(self)
        self.with_streaming_response = MetisWithStreamedResponse(self)

    @property
    @override
    def qs(self) -> Querystring:
        return Querystring(array_format="comma")

    @property
    @override
    def auth_headers(self) -> dict[str, str]:
        api_key = self.api_key
        if api_key is None:
            return {}
        return {"Authorization": f"Bearer {api_key}"}

    @property
    @override
    def default_headers(self) -> dict[str, str | Omit]:
        return {
            **super().default_headers,
            "X-Stainless-Async": "false",
            **self._custom_headers,
        }

    @override
    def _validate_headers(self, headers: Headers, custom_headers: Headers) -> None:
        if self.api_key and headers.get("Authorization"):
            return
        if isinstance(custom_headers.get("Authorization"), Omit):
            return

        raise TypeError(
            '"Could not resolve authentication method. Expected the api_key to be set. Or for the `Authorization` headers to be explicitly omitted"'
        )

    def copy(
        self,
        *,
        api_key: str | None = None,
        base_url: str | httpx.URL | None = None,
        model_api_keys: dict[str, str] | None = None,
        timeout: float | Timeout | None | NotGiven = NOT_GIVEN,
        http_client: httpx.Client | None = None,
        max_retries: int | NotGiven = NOT_GIVEN,
        default_headers: Mapping[str, str] | None = None,
        set_default_headers: Mapping[str, str] | None = None,
        default_query: Mapping[str, object] | None = None,
        set_default_query: Mapping[str, object] | None = None,
        _extra_kwargs: Mapping[str, Any] = {},
    ) -> Self:
        """
        Create a new client instance re-using the same options given to the current client with optional overriding.
        """
        if default_headers is not None and set_default_headers is not None:
            raise ValueError("The `default_headers` and `set_default_headers` arguments are mutually exclusive")

        if default_query is not None and set_default_query is not None:
            raise ValueError("The `default_query` and `set_default_query` arguments are mutually exclusive")

        headers = self._custom_headers
        if default_headers is not None:
            headers = {**headers, **default_headers}
        elif set_default_headers is not None:
            headers = set_default_headers

        params = self._custom_query
        if default_query is not None:
            params = {**params, **default_query}
        elif set_default_query is not None:
            params = set_default_query

        http_client = http_client or self._client
        return self.__class__(
            api_key=api_key or self.api_key,
            base_url=base_url or self.base_url,
            model_api_keys=model_api_keys or self.model_api_keys,
            timeout=self.timeout if isinstance(timeout, NotGiven) else timeout,
            http_client=http_client,
            max_retries=max_retries if is_given(max_retries) else self.max_retries,
            default_headers=headers,
            default_query=params,
            **_extra_kwargs,
        )

    def with_api_keys(self, **api_keys: str) -> Self:
        """Create a new client instance with additional API keys"""
        new_keys = {**self.model_api_keys, **api_keys}
        return self.copy(model_api_keys=new_keys)

    # Alias for `copy` for nicer inline usage, e.g.
    # client.with_options(timeout=10).foo.create(...)
    with_options = copy

    @override
    def _make_status_error(
        self,
        err_msg: str,
        *,
        body: object,
        response: httpx.Response,
    ) -> APIStatusError:
        if response.status_code == 400:
            return _exceptions.BadRequestError(err_msg, response=response, body=body)

        if response.status_code == 401:
            return _exceptions.AuthenticationError(err_msg, response=response, body=body)

        if response.status_code == 403:
            return _exceptions.PermissionDeniedError(err_msg, response=response, body=body)

        if response.status_code == 404:
            return _exceptions.NotFoundError(err_msg, response=response, body=body)

        if response.status_code == 409:
            return _exceptions.ConflictError(err_msg, response=response, body=body)

        if response.status_code == 422:
            return _exceptions.UnprocessableEntityError(err_msg, response=response, body=body)

        if response.status_code == 429:
            return _exceptions.RateLimitError(err_msg, response=response, body=body)

        if response.status_code >= 500:
            return _exceptions.InternalServerError(err_msg, response=response, body=body)
        return APIStatusError(err_msg, response=response, body=body)


class AsyncMetis(AsyncAPIClient):
    connect: connect.AsyncConnectResource
    reconnect: reconnect.AsyncReconnectResource
    oauth: oauth.AsyncOAuthResource
    mcp: mcp.AsyncMcpResource
    sessions: sessions.AsyncSessionsResource
    health: health.AsyncHealthResource
    test: test.AsyncTestResource
    with_raw_response: AsyncMetisWithRawResponse
    with_streaming_response: AsyncMetisWithStreamedResponse

    # client options
    api_key: str | None
    model_api_keys: dict[str, str]

    def __init__(
        self,
        *,
        api_key: str | None = None,
        base_url: str | httpx.URL | None = None,
        model_api_keys: dict[str, str] | None = None,
        timeout: Union[float, Timeout, None, NotGiven] = NOT_GIVEN,
        max_retries: int = DEFAULT_MAX_RETRIES,
        default_headers: Mapping[str, str] | None = None,
        default_query: Mapping[str, object] | None = None,
        # Configure a custom httpx client.
        # We provide a `DefaultAsyncHttpxClient` class that you can pass to retain the default values we use for `limits`, `timeout` & `follow_redirects`.
        # See the [httpx documentation](https://www.python-httpx.org/api/#asyncclient) for more details.
        http_client: httpx.AsyncClient | None = None,
        # Enable or disable schema validation for data returned by the API.
        # When enabled an error APIResponseValidationError is raised
        # if the API responds with invalid data for the expected schema.
        #
        # This parameter may be removed or changed in the future.
        # If you rely on this feature, please open a GitHub issue
        # outlining your use-case to help us decide if it should be
        # part of our public interface in the future.
        _strict_response_validation: bool = False,
    ) -> None:
        """Construct a new async AsyncMetis client instance.

        This automatically infers the `api_key` argument from the `METIS_API_KEY` environment variable if it is not provided.
        
        Args:
            model_api_keys: Dictionary of provider API keys (e.g., {"openai": "sk-...", "anthropic": "sk-ant-..."})
        """
        if api_key is None:
            api_key = os.environ.get("METIS_API_KEY")
        self.api_key = api_key
        self.model_api_keys = model_api_keys or {}

        if base_url is None:
            base_url = os.environ.get("METIS_BASE_URL")
        if base_url is None:
            base_url = f"https://api.example.com"

        super().__init__(
            version=__version__,
            base_url=base_url,
            max_retries=max_retries,
            timeout=timeout,
            http_client=http_client,
            custom_headers=default_headers,
            custom_query=default_query,
            _strict_response_validation=_strict_response_validation,
        )

        self.connect = connect.AsyncConnectResource(self)
        self.reconnect = reconnect.AsyncReconnectResource(self)
        self.oauth = oauth.AsyncOAuthResource(self)
        self.mcp = mcp.AsyncMcpResource(self)
        self.sessions = sessions.AsyncSessionsResource(self)
        self.health = health.AsyncHealthResource(self)
        self.test = test.AsyncTestResource(self)
        self.with_raw_response = AsyncMetisWithRawResponse(self)
        self.with_streaming_response = AsyncMetisWithStreamedResponse(self)

    @property
    @override
    def qs(self) -> Querystring:
        return Querystring(array_format="comma")

    @property
    @override
    def auth_headers(self) -> dict[str, str]:
        api_key = self.api_key
        if api_key is None:
            return {}
        return {"Authorization": f"Bearer {api_key}"}

    @property
    @override
    def default_headers(self) -> dict[str, str | Omit]:
        return {
            **super().default_headers,
            "X-Stainless-Async": f"async:{get_async_library()}",
            **self._custom_headers,
        }

    @override
    def _validate_headers(self, headers: Headers, custom_headers: Headers) -> None:
        if self.api_key and headers.get("Authorization"):
            return
        if isinstance(custom_headers.get("Authorization"), Omit):
            return

        raise TypeError(
            '"Could not resolve authentication method. Expected the api_key to be set. Or for the `Authorization` headers to be explicitly omitted"'
        )

    def copy(
        self,
        *,
        api_key: str | None = None,
        base_url: str | httpx.URL | None = None,
        model_api_keys: dict[str, str] | None = None,
        timeout: float | Timeout | None | NotGiven = NOT_GIVEN,
        http_client: httpx.AsyncClient | None = None,
        max_retries: int | NotGiven = NOT_GIVEN,
        default_headers: Mapping[str, str] | None = None,
        set_default_headers: Mapping[str, str] | None = None,
        default_query: Mapping[str, object] | None = None,
        set_default_query: Mapping[str, object] | None = None,
        _extra_kwargs: Mapping[str, Any] = {},
    ) -> Self:
        """
        Create a new client instance re-using the same options given to the current client with optional overriding.
        """
        if default_headers is not None and set_default_headers is not None:
            raise ValueError("The `default_headers` and `set_default_headers` arguments are mutually exclusive")

        if default_query is not None and set_default_query is not None:
            raise ValueError("The `default_query` and `set_default_query` arguments are mutually exclusive")

        headers = self._custom_headers
        if default_headers is not None:
            headers = {**headers, **default_headers}
        elif set_default_headers is not None:
            headers = set_default_headers

        params = self._custom_query
        if default_query is not None:
            params = {**params, **default_query}
        elif set_default_query is not None:
            params = set_default_query

        http_client = http_client or self._client
        return self.__class__(
            api_key=api_key or self.api_key,
            base_url=base_url or self.base_url,
            model_api_keys=model_api_keys or self.model_api_keys,
            timeout=self.timeout if isinstance(timeout, NotGiven) else timeout,
            http_client=http_client,
            max_retries=max_retries if is_given(max_retries) else self.max_retries,
            default_headers=headers,
            default_query=params,
            **_extra_kwargs,
        )

    def with_api_keys(self, **api_keys: str) -> Self:
        """Create a new client instance with additional API keys"""
        new_keys = {**self.model_api_keys, **api_keys}
        return self.copy(model_api_keys=new_keys)

    # Alias for `copy` for nicer inline usage, e.g.
    # client.with_options(timeout=10).foo.create(...)
    with_options = copy

    @override
    def _make_status_error(
        self,
        err_msg: str,
        *,
        body: object,
        response: httpx.Response,
    ) -> APIStatusError:
        if response.status_code == 400:
            return _exceptions.BadRequestError(err_msg, response=response, body=body)

        if response.status_code == 401:
            return _exceptions.AuthenticationError(err_msg, response=response, body=body)

        if response.status_code == 403:
            return _exceptions.PermissionDeniedError(err_msg, response=response, body=body)

        if response.status_code == 404:
            return _exceptions.NotFoundError(err_msg, response=response, body=body)

        if response.status_code == 409:
            return _exceptions.ConflictError(err_msg, response=response, body=body)

        if response.status_code == 422:
            return _exceptions.UnprocessableEntityError(err_msg, response=response, body=body)

        if response.status_code == 429:
            return _exceptions.RateLimitError(err_msg, response=response, body=body)

        if response.status_code >= 500:
            return _exceptions.InternalServerError(err_msg, response=response, body=body)
        return APIStatusError(err_msg, response=response, body=body)


class MetisWithRawResponse:
    def __init__(self, client: Metis) -> None:
        self.connect = connect.ConnectResourceWithRawResponse(client.connect)
        self.reconnect = reconnect.ReconnectResourceWithRawResponse(client.reconnect)
        self.oauth = oauth.OAuthResourceWithRawResponse(client.oauth)
        self.mcp = mcp.McpResourceWithRawResponse(client.mcp)
        self.sessions = sessions.SessionsResourceWithRawResponse(client.sessions)
        self.health = health.HealthResourceWithRawResponse(client.health)
        self.test = test.TestResourceWithRawResponse(client.test)


class AsyncMetisWithRawResponse:
    def __init__(self, client: AsyncMetis) -> None:
        self.connect = connect.AsyncConnectResourceWithRawResponse(client.connect)
        self.reconnect = reconnect.AsyncReconnectResourceWithRawResponse(client.reconnect)
        self.oauth = oauth.AsyncOAuthResourceWithRawResponse(client.oauth)
        self.mcp = mcp.AsyncMcpResourceWithRawResponse(client.mcp)
        self.sessions = sessions.AsyncSessionsResourceWithRawResponse(client.sessions)
        self.health = health.AsyncHealthResourceWithRawResponse(client.health)
        self.test = test.AsyncTestResourceWithRawResponse(client.test)


class MetisWithStreamedResponse:
    def __init__(self, client: Metis) -> None:
        self.connect = connect.ConnectResourceWithStreamingResponse(client.connect)
        self.reconnect = reconnect.ReconnectResourceWithStreamingResponse(client.reconnect)
        self.oauth = oauth.OAuthResourceWithStreamingResponse(client.oauth)
        self.mcp = mcp.McpResourceWithStreamingResponse(client.mcp)
        self.sessions = sessions.SessionsResourceWithStreamingResponse(client.sessions)
        self.health = health.HealthResourceWithStreamingResponse(client.health)
        self.test = test.TestResourceWithStreamingResponse(client.test)


class AsyncMetisWithStreamedResponse:
    def __init__(self, client: AsyncMetis) -> None:
        self.connect = connect.AsyncConnectResourceWithStreamingResponse(client.connect)
        self.reconnect = reconnect.AsyncReconnectResourceWithStreamingResponse(client.reconnect)
        self.oauth = oauth.AsyncOAuthResourceWithStreamingResponse(client.oauth)
        self.mcp = mcp.AsyncMcpResourceWithStreamingResponse(client.mcp)
        self.sessions = sessions.AsyncSessionsResourceWithStreamingResponse(client.sessions)
        self.health = health.AsyncHealthResourceWithStreamingResponse(client.health)
        self.test = test.AsyncTestResourceWithStreamingResponse(client.test)


Client = Metis

AsyncClient = AsyncMetis
