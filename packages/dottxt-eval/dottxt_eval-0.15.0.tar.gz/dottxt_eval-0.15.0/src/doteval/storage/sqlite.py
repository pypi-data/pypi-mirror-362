...
class SQLiteStorage(Storage):
    ...
    def add_results(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for result in results:
                cursor.execute(
                    """
                    INSERT INTO results (evaluation_id, item_id, dataset_row, error, timestamp, prompt, model_answer)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        evaluation_id,
    ...
    def get_results(self, experiment_name: str, evaluation_name: str) -> list[Record]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                """
                SELECT id, item_id, dataset_row, error, timestamp, prompt, model_answer
                FROM results
                WHERE evaluation_id = ?
                ORDER BY item_id
                """,
            ...
            for result_row in cursor.fetchall():
                ...
                result_obj = Result(*scores, prompt=prompt, model_answer=model_answer)
                result = Record(
                    result=result_obj,
                    item_id=item_id,
    ...
    def get_error_results(
        self, experiment_name: str, evaluation_name: Optional[str] = None
    ) -> list[dict]:
