...
def test_result_error_propagation(temp_storage):
    ...
    result = eval_with_result_error_wrapped(
...
def test_result_error_with_model_answer_capture(temp_storage):
    """Test capturing model answer along with error for debugging."""
    test_data = [("math", "2+2=4"), ("math", "2+2=elephant")]

    def eval_with_model_answer_error(question, model_answer):
        prompt = f"Solve: {question}"
        try:
            # Try to parse numeric answer
            if "=" in model_answer:
                answer_part = model_answer.split("=")[1].strip()
            ...
            error_msg = f"Failed to parse answer '{model_answer}': {str(e)}"
            return Result(score, prompt=prompt, error=error_msg)

    session_manager = SessionManager(
        storage=f"json://{temp_storage}", experiment_name="model_answer_error_test"
    )

    custom_foreach = ForEach(storage=session_manager.storage)

    @custom_foreach("question,model_answer", test_data)
    def eval_with_model_answer_error_wrapped(question, model_answer):
        return eval_with_model_answer_error(question, model_answer)

    result = eval_with_model_answer_error_wrapped(
        evaluation_name="eval_with_model_answer_error",
        experiment_name="model_answer_error_test",
        samples=None,
    )
    ...
    completed_ids = session_manager.storage.completed_items(
        "model_answer_error_test", "eval_with_model_answer_error"
    )
...
def test_result_error_propagates_to_record_without_exception(temp_storage):
    ...
    result = eval_with_mixed_results(
...
def test_result_error_resumption_behavior(temp_storage):
    ...
    result1 = eval_resumption_result_error_wrapped(
...
def test_result_with_model_answer_field(temp_storage):
    """Test that Result objects can store model_answer for debugging."""
    test_data = [("question1", "expected1"), ("question2", "expected2")]

    def eval_with_model_answer(question, expected):
        ...
        try:
            ...
            return Result(score, prompt=prompt, model_answer=model_output)
        except Exception as e:
    ...
    session_manager = SessionManager(
        storage=f"json://{temp_storage}", experiment_name="model_answer_test"
    )

    custom_foreach = ForEach(storage=session_manager.storage)

    @custom_foreach("question,expected", test_data)
    def eval_with_model_answer_wrapped(question, expected):
        return eval_with_model_answer(question, expected)

    result = eval_with_model_answer_wrapped(
        evaluation_name="eval_with_model_answer",
        experiment_name="model_answer_test",
        samples=None,
    )
    ...
    # Check that model_answer is captured for both items
    for i, res in enumerate(result.results):
        assert res.error is None  # No errors in this test
        assert res.result.model_answer is not None
        assert f"expected{i+1}" in res.result.model_answer
        assert "plus extra text" in res.result.model_answer
        assert res.result.prompt == f"Q: question{i+1}"

    # Verify storage and retrieval preserves model_answer
    stored_results = session_manager.storage.get_results(
        "model_answer_test", "eval_with_model_answer"
    )
    assert len(stored_results) == 2

    for i, stored_res in enumerate(stored_results):
        assert stored_res.result.model_answer is not None
        assert f"expected{i+1}" in stored_res.result.model_answer
        assert "plus extra text" in stored_res.result.model_answer
...
def test_result_with_optional_prompt(temp_storage):
    ...
    def eval_without_prompt(input_val, expected):
        ...
        if input_val == "input1":
            ...
            return Result(score, prompt=None, model_answer=f"processed_{input_val}")
        else:
            # Omit prompt entirely (should default to None)
            return Result(score, model_answer=f"processed_{input_val}")
    ...
    result = eval_without_prompt_wrapped(
    ...
    # Check that prompt is None and model_answer is captured
    for i, res in enumerate(result.results):
        assert res.error is None
        assert res.result.prompt is None  # Optional prompt should be None
        assert res.result.model_answer == f"processed_input{i+1}"

    # Verify storage and retrieval preserves None prompt and model_answer
    stored_results = session_manager.storage.get_results(
    ...
    for i, stored_res in enumerate(stored_results):
        ...
        assert stored_res.result.model_answer == f"processed_input{i+1}"


def test_result_comprehensive_fields_usage(temp_storage):
    """Test Result with all fields: scores, prompt, error, model_answer."""
    ...
    result = eval_comprehensive_fields_wrapped(
    ...
    assert result.results[0].result.model_answer == "Model response for good_input"
    ...
    assert result.results[1].result.model_answer == "Model response for bad_input"
