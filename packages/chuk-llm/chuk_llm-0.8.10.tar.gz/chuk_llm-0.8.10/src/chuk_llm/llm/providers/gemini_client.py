...
class GeminiLLMClient(ConfigAwareProviderMixin, ToolCompatibilityMixin, BaseLLMClient):
    ...
    async def _stream_completion_async(
        ...
        """
        Real streaming using Gemini client with ENHANCED tool call handling and universal tool name restoration.
        """
        ...
        # CRITICAL FIX: Enhanced tool call tracking for Gemini
        ...
        try:
            ...
            async for chunk in stream:
                ...
                with UltimateSuppression():
                    try:
                        # CRITICAL FIX: Enhanced chunk processing for tool calls
                        chunk_text = ""
                        chunk_tool_calls = []
                        ...
                        if hasattr(chunk, 'candidates') and chunk.candidates:
                            ...
                            if hasattr(candidate, 'content') and candidate.content:
                                ...
                                # Check if content has parts (this is where function calls and text live)
                                if hasattr(content, 'parts') and content.parts:
                                    text_parts = []
                                    ...
                                    for part in content.parts:
                                        ...
                                        # CRITICAL FIX: Handle function call parts
                                        elif hasattr(part, 'function_call') and part.function_call:
                                            fc = part.function_call
                                            ...
                                            try:
                                                ...
                                                chunk_tool_calls.append(tool_call)
                                                
                                                # Also add to accumulated list (Gemini typically sends complete tool calls)
                                                found_tool_calls.append(tool_call)
                                                
                                                log.debug(f"Extracted function call from Gemini chunk: {function_name}")
                        ...
                        # IMPROVED: Handle text content with proper deduplication
                        if chunk_text:
                        ...
                        # CRITICAL FIX: Handle tool calls with universal name restoration
                        if chunk_tool_calls:
            ...
            # FINAL FIX: Ensure any accumulated tool calls are yielded if not already sent
            # (This handles cases where tool calls might be missed)
            if found_tool_calls and chunk_count == 1:
                # This might be a single-chunk response with tool calls
                final_tool_response = {
                    "response": "",
                ...
                if name_mapping:
                    final_tool_response = self._restore_tool_names_in_response(final_tool_response, name_mapping)
                
                # Only yield if we haven't already yielded these tool calls
                yield final_tool_response
            
            log.debug(f"Gemini streaming completed with {chunk_count} chunks, "
                    f"{len(accumulated_response)} total characters, {len(found_tool_calls)} tool calls")
