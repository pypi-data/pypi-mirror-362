...
class AzureOpenAILLMClient(ConfigAwareProviderMixin, ToolCompatibilityMixin, OpenAIStyleMixin, BaseLLMClient):
    ...
    async def _stream_completion_async(
        ...
        **kwargs: Any,
        ...
        """
        Enhanced async streaming for Azure OpenAI with FIXED tool call accumulation.
        Inherits core streaming logic but adds Azure-specific error handling.
        """
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # CRITICAL FIX: Use the same accumulation logic as OpenAI but with Azure specifics
                chunk_count = 0
                total_content = ""
                
                # Tool call accumulation (same as OpenAI fix)
                ...
                async for chunk in response_stream:
                    ...
                    try:
                        if hasattr(chunk, 'choices') and chunk.choices and len(chunk.choices) > 0:
                            ...
                            if hasattr(choice, 'delta') and choice.delta:
                                ...
                                # CRITICAL FIX: Handle tool calls with proper accumulation (same as OpenAI)
                                if hasattr(delta, 'tool_calls') and delta.tool_calls:
                                    for tc in delta.tool_calls:
                                        try:
                                            # Get tool call index (Azure uses same pattern as OpenAI)
                    ...
                    for tc_index, tc_data in accumulated_tool_calls.items():
                        if tc_data["name"] and tc_data["arguments"]:
                            ...
                            try:
                                # AZURE FIX: Handle Azure-specific argument formatting
                                args_str = tc_data["arguments"]
                                
                                # Azure might return arguments with extra quotes or escaping
                                if args_str.startswith('""') and args_str.endswith('""'):
                                    args_str = args_str[2:-2]  # Remove outer double quotes
                                ...
                                complete_tool_calls.append({
                                    ...
                                    "function": {
                                        "name": tc_data["name"],
                                        "arguments": json.dumps(parsed_args)  # Re-serialize to ensure consistency
                                    }
                                })
                ...
                for tc_index, tc_data in accumulated_tool_calls.items():
                    if tc_data["name"] and tc_data["arguments"]:
                        ...
                        if not already_yielded:
                            # AZURE FIX: Handle argument formatting
                            args_str = tc_data["arguments"]
                            try:
                                if args_str.startswith('""') and args_str.endswith('""'):
