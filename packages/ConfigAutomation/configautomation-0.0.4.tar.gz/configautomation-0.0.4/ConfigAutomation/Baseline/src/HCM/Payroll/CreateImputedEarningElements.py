...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        # Secondary Classification as Group Term Life
        if datadictvalue["C_SCNDRY_CLSSFCTN"]=='Group Term Life':
            ...
            if datadictvalue["C_CTGRY"]=='Standard':
                ...
                if datadictvalue["C_CLCLTN_RULE"] == 'Hour * Rate':
        ...
        # Secondary Classification as Noncash Award
        if datadictvalue["C_SCNDRY_CLSSFCTN"]=='Noncash Award':
            page.get_by_role("combobox", name="Secondary Classification").click()
            page.wait_for_timeout(3000)
            page.get_by_text(datadictvalue["C_SCNDRY_CLSSFCTN"], exact=True).click()
            page.wait_for_timeout(2000)
        ...
        # Secondary Classification as Domestic Partner Nondependent
        if datadictvalue["C_SCNDRY_CLSSFCTN"] == 'Domestic Partner Nondependent':
            page.get_by_role("combobox", name="Secondary Classification").click()
            page.wait_for_timeout(3000)
            page.get_by_text(datadictvalue["C_SCNDRY_CLSSFCTN"], exact=True).click()
            page.wait_for_timeout(2000)
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + PAYROLL_ELEMENTS_CONFIG_WRKBK, IMPUTED_EARNINGS):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + PAYROLL_ELEMENTS_CONFIG_WRKBK, IMPUTED_EARNINGS, PRCS_DIR_PATH + PAYROLL_ELEMENTS_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + PAYROLL_ELEMENTS_CONFIG_WRKBK, IMPUTED_EARNINGS)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk, VIDEO_DIR_PATH + re.split(".xlsx", PAYROLL_ELEMENTS_CONFIG_WRKBK)[0] + "_" + IMPUTED_EARNINGS)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", PAYROLL_ELEMENTS_CONFIG_WRKBK)[0] + "_" + IMPUTED_EARNINGS + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
