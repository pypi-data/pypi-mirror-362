...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        # Subscriber
        page.get_by_text('Questionnaire Library').nth(0).click()
        page.wait_for_timeout(3000)
        if page.get_by_role("cell", name="Expand Questionnaire Library").get_by_role("link").is_visible():
            page.get_by_role("cell", name="Expand Questionnaire Library").get_by_role("link").click()
        page.wait_for_timeout(3000)
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + PERF_CONFIG_WRKBK, QUESTIONNARIE):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + PERF_CONFIG_WRKBK, QUESTIONNARIE, PRCS_DIR_PATH + PERF_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + PERF_CONFIG_WRKBK, QUESTIONNARIE)
    if rows > 0:
        ...
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", PERF_CONFIG_WRKBK)[
            0] + "_" + QUESTIONNARIE + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
