...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_QSTNNR_LBBRY"] != PrevLEQuestionarie:
            ...
            if page.get_by_role("button", name="OK").is_visible():
                page.get_by_role("button", name="OK").click()
            # page.get_by_role("cell", name=datadictvalue["C_QSTNNR_LBBRY"], exact=True).locator("span").click()
            page.get_by_text(datadictvalue["C_QSTNNR_LBBRY"], exact=True).click(force=True)
            page.wait_for_timeout(5000)

            PrevLEQuestionarie = datadictvalue["C_QSTNNR_LBBRY"]
        ...
        page.wait_for_timeout(3000)
        page.get_by_role("cell", name=datadictvalue["C_NAME"], exact=True).locator("span").click()
        # page.get_by_text(datadictvalue["C_NAME"]).first.click()
        page.get_by_role("button", name="OK").click()
        page.wait_for_timeout(3000)
        ...
        # page.get_by_label("Folder").click()
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + LEARNINGS_CONFIG_WRKBK, LE_QUESTIONNARIE):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + LEARNINGS_CONFIG_WRKBK, LE_QUESTIONNARIE, PRCS_DIR_PATH + LEARNINGS_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + LEARNINGS_CONFIG_WRKBK, LE_QUESTIONNARIE)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk, VIDEO_DIR_PATH + re.split(".xlsx", LEARNINGS_CONFIG_WRKBK)[0])
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", LEARNINGS_CONFIG_WRKBK)[0] + "_" + LE_QUESTIONNARIE + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
