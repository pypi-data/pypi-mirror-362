...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_VALUE_SET_CODE"] != PreValue:
            ...
            page.get_by_label("Value Data Type").select_option(datadictvalue["C_DATA_TYPE"])
            ...
            page.get_by_label("Minimum Value").fill(str(datadictvalue["C_MNM_VALUE"]))
            page.get_by_label("Maximum Value").click()
            page.get_by_label("Maximum Value").fill(str(datadictvalue["C_MXM_VALUE"]))
        ...
        if datadictvalue["C_VALUE"] != '':
            ...
            page.get_by_label("Description").nth(1).fill(datadictvalue["C_VALUE_DSCRPTN"])
            ...
            page.wait_for_timeout(3000)
            page.get_by_role("cell", name="Press down arrow to access Calendar Start Date Select Date", exact=True).get_by_placeholder("m/d/yy").nth(0).fill(datadictvalue["C_START_DATE"])
            page.get_by_role("cell", name="Press down arrow to access Calendar End Date Select Date", exact=True).get_by_placeholder("m/d/yy").nth(0).fill(datadictvalue["C_END_DATE"])
            page.get_by_label("Sort Order").nth(0).fill(str(datadictvalue["C_SORT_ORDER"]))
    ...
    try:
        ...
        print("GL DFF vaues Saved Successfully")
        # datadictvalue["RowStatus"] = "GL DFF vaues are added successfully"

    except Exception as e:
        print("GL DFF vaues not saved")
        # datadictvalue["RowStatus"] = "GL DFF vaues not added"
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + GL_WORKBOOK, GL_DFF_VALUES):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + GL_WORKBOOK, GL_DFF_VALUES, PRCS_DIR_PATH + GL_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + GL_WORKBOOK, GL_DFF_VALUES)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,
                               VIDEO_DIR_PATH + re.split(".xlsx", GL_WORKBOOK)[0] + "_" + GL_DFF_VALUES)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", GL_WORKBOOK)[
            0] + "_" + GL_DFF_VALUES + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
