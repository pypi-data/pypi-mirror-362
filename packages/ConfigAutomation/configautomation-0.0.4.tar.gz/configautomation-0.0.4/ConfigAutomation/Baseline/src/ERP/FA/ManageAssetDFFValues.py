...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_VALUE_SET_CODE"] != PreValue:
            ...
            page.get_by_label("Description").fill(datadictvalue["C_VALUE_DSCRPTN"])
        ...
        if datadictvalue["C_VALUE"] != '':
            ...
            # page.get_by_role("cell", name="Description", exact=True).nth(3).get_by_label("Description").fill(datadictvalue["C_VALUE_DSCRPTN"])
    ...
    try:
        ...
        print("Assets DFF vaues Saved Successfully")
        # datadictvalue["RowStatus"] = "Payables DFF vaues are added successfully"

    except Exception as e:
        print("Assets DFF vaues not saved")
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + FA_WORKBOOK, MANAGE_DFF_VALUES):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + FA_WORKBOOK, MANAGE_DFF_VALUES, PRCS_DIR_PATH + FA_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + FA_WORKBOOK, MANAGE_DFF_VALUES)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,
                               VIDEO_DIR_PATH + re.split(".xlsx", FA_WORKBOOK)[0] + "_" + MANAGE_DFF_VALUES)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", FA_WORKBOOK)[
            0] + "_" + MANAGE_DFF_VALUES + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
