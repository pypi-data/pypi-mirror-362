...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        page.locator("[id=\"__af_Z_window\"]").get_by_text("Meals", exact=True).click()
        page.wait_for_timeout(2000)
        ...
        page.wait_for_timeout(2000)
        if datadictvalue[
            "C_RMBRS_UPPR_LIMIT_WHEN_EXPNS_EXCDS_DFND_RATE"] == 'Generate policy violation when expense exceeds defined rate':
            page.wait_for_timeout(2000)
        ...
        if datadictvalue["C_RATE_DAILY_SUM_LIMIT"] != '' and datadictvalue["C_ROLE"] != 'Yes' and datadictvalue["C_LCTN"] != 'Yes':
            page.get_by_role("button", name="Create Rates").click()
            page.wait_for_timeout(2000)
            if page.get_by_role("textbox", name="Single instance limit").is_visible():
                page.get_by_role("textbox", name="Single instance limit").fill(str(datadictvalue["C_RATE_SNGL_INSTNC_LIMIT"]))
        ...
        try:
            ...
            print("Meals Expense Policy Saved Successfully")
            datadictvalue["RowStatus"] = "Meals Expense Policy saved successfully"

        except Exception as e:
            print("Meals Expense Policy not saved")
            datadictvalue["RowStatus"] = "Meals Expense Policy not added"
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + EXP_WORKBOOK, MEALS_EXP_POLICY):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + EXP_WORKBOOK, MEALS_EXP_POLICY, PRCS_DIR_PATH + EXP_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + EXP_WORKBOOK, MEALS_EXP_POLICY)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,
                               VIDEO_DIR_PATH + re.split(".xlsx", EXP_WORKBOOK)[0] + "_" + MEALS_EXP_POLICY)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", EXP_WORKBOOK)[
            0] + "_" + MEALS_EXP_POLICY + "_Results_" + datetime.now().strftime(
