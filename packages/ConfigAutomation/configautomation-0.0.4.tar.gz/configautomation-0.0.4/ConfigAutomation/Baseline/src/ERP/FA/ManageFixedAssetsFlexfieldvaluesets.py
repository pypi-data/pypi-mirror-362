...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    page.get_by_role("textbox").fill("Manage Fixed Assets Flexfield Value Sets")
    page.get_by_role("textbox").press("Enter")
    page.get_by_role("link", name="Manage Fixed Assets Flexfield Value Sets", exact=True).click()
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_VALUE_SET_CODE"] != PreValue:
            ...
            if datadictvalue["C_VLDTN_TYPE"] == 'Dependent':
                page.get_by_role("combobox", name="Value Set Code").fill(datadictvalue["C_INDPNDNT_VALUE_SET"])
        ...
        if datadictvalue["C_VALUE"] != '':
            ...
            if datadictvalue["C_VALUE_SET_CODE"] != "":
                ...
                if datadictvalue["C_INDPNDNT_VALUE"] != "":
                    page.locator("//span[text()='Independent Value']//following::input[3]").fill(datadictvalue["C_INDPNDNT_VALUE"])
                if datadictvalue["C_ENBLD"] == 'Yes':
                    if not page.locator( "//span[text()='Enabled']//following::label[contains(@id,'Label0')][1]").is_checked():
                        page.locator("//span[text()='Enabled']//following::label[contains(@id,'Label0')][1]").click()
                elif datadictvalue["C_ENBLD"] == 'No':
                    if page.locator("//span[text()='Enabled']//following::label[contains(@id,'Label0')][1]").is_checked():
                        page.locator("//span[text()='Enabled']//following::label[contains(@id,'Label0')][1]").click()
                page.get_by_role("cell", name="Press down arrow to access Calendar Start Date Select Date",
                                 exact=True).get_by_placeholder("m/d/yy").nth(0).fill(datadictvalue["C_START_DATE"])
                page.get_by_role("cell", name="Press down arrow to access Calendar End Date Select Date",
                                 exact=True).get_by_placeholder("m/d/yy").nth(0).fill(datadictvalue["C_END_DATE"])
                page.get_by_label("Sort Order").nth(0).fill(datadictvalue["C_SORT_ORDER (SQNC_NMBR)"])
    ...
    try:
        ...
        print("Fixed Assets Flexfield values Saved Successfully")


    except Exception as e:
        print("Fixed Assets Flexfield values not saved")
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + FA_WORKBOOK, MANAGE_FA_KEYFF_VALUESETS):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + FA_WORKBOOK, MANAGE_FA_KEYFF_VALUESETS, PRCS_DIR_PATH + FA_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + FA_WORKBOOK, MANAGE_FA_KEYFF_VALUESETS)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,
                               VIDEO_DIR_PATH + re.split(".xlsx", FA_WORKBOOK)[0] + "_" + MANAGE_FA_KEYFF_VALUESETS)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", FA_WORKBOOK)[
            0] + "_" + MANAGE_FA_KEYFF_VALUESETS + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
