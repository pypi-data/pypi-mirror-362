...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        page.get_by_text(datadictvalue["C_STATE"], exact=True).click()
        page.wait_for_timeout(1000)
        ...
        try:
            ...
            print("Added Legal Entity Calculation Regional Saved Successfully")
            datadictvalue["RowStatus"] = "Added Legal Entity Calculation Regional"
        except Exception as e:
            print("Unable to save Legal Entity Calculation Regional")
            datadictvalue["RowStatus"] = "Unable to Add Legal Entity Calculation Regional"
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + PAYROLL_CONFIG_WRKBK, LEGAL_ENTITY_CALCULATION_REGION):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + PAYROLL_CONFIG_WRKBK, LEGAL_ENTITY_CALCULATION_REGION, PRCS_DIR_PATH + PAYROLL_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + PAYROLL_CONFIG_WRKBK, LEGAL_ENTITY_CALCULATION_REGION)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk, VIDEO_DIR_PATH + re.split(".xlsx", PAYROLL_CONFIG_WRKBK)[0] + "_" + LEGAL_ENTITY_CALCULATION_REGION)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", PAYROLL_CONFIG_WRKBK)[0] + "_" + LEGAL_ENTITY_CALCULATION_REGION + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
