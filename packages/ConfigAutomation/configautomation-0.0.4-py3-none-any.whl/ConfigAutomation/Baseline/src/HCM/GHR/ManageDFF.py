...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    page.get_by_label("Name").fill("Legal Entity Context")
    page.get_by_role("button", name="Search", exact=True).click()

    page.get_by_role("cell", name="Legal Entity Context", exact=True).click()
    ...
    while i < rowcount:
        ...
        page.get_by_label("Description").fill(datadictvalue["C_CODE"])
    ...
    try:
        ...
        print("HR-Descriptive Flexfields Executed Successfully")

    except Exception as e:
        print("HR-Descriptive Flexfields Executed UnSuccessfully")
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + GHR_CONFIG_WRKBK, MANAGE_HR_DFF):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + GHR_CONFIG_WRKBK, MANAGE_HR_DFF, PRCS_DIR_PATH + GHR_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + GHR_CONFIG_WRKBK, MANAGE_HR_DFF)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,VIDEO_DIR_PATH + re.split(".xlsx", GHR_CONFIG_WRKBK)[0] + "_" + MANAGE_HR_DFF)
            write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", GHR_CONFIG_WRKBK)[0] + "_" + MANAGE_HR_DFF + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
