...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_QSTN_LBRRY"] != PrevOCQuesFolder:
            ...
            if page.get_by_text(datadictvalue["C_QSTN_LBRRY"], exact=True).nth(0).is_visible():
                ...
                page.get_by_role("textbox", name="Folder").type(datadictvalue["C_QSTN_LBRRY"])
            ...
            PrevOCQuesFolder = datadictvalue["C_QSTN_LBRRY"]
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + LEARNINGS_CONFIG_WRKBK, OLC_QUESTIONS):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + LEARNINGS_CONFIG_WRKBK, OLC_QUESTIONS,PRCS_DIR_PATH + LEARNINGS_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + LEARNINGS_CONFIG_WRKBK, OLC_QUESTIONS)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,VIDEO_DIR_PATH + re.split(".xlsx", LEARNINGS_CONFIG_WRKBK)[0])
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", LEARNINGS_CONFIG_WRKBK)[0] + "_" + OLC_QUESTIONS + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
