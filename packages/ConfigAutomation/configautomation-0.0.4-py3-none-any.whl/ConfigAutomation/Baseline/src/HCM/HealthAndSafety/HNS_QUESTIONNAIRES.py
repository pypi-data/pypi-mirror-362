...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_QSTNNR_TMPLT_NAME"] != PrevQues:

            # Subscriber
            page.wait_for_timeout(5000)
            page.get_by_role("combobox", name="Subscriber").click()
            page.get_by_text(datadictvalue["C_SBSCRBR"],exact=True).click()
            page.wait_for_timeout(3000)

            if page.get_by_role("cell", name="Workforce Health and").locator("div").is_visible():
                page.get_by_role("cell", name="Workforce Health and").locator("div").click()
            ...
            page.get_by_label("Name").type(datadictvalue["C_QSTNNR_TMPLT_NAME"])
            page.locator("[id=\"__af_Z_window\"]").get_by_role("button", name="Search", exact=True).click()
            page.wait_for_timeout(3000)
            # page.get_by_text(datadictvalue["C_QSTNNR_TMPLT_NAME"]).nth(2).click(force=True)
            page.get_by_role("button", name="OK").click()
            page.wait_for_timeout(3000)
            ...
            PrevQues = datadictvalue["C_QSTNNR_TMPLT_NAME"]
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + HEAL_SAF_CONFIG_WRKBK, QUESTIONNAIRES_HNS):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + HEAL_SAF_CONFIG_WRKBK, QUESTIONNAIRES_HNS, PRCS_DIR_PATH + HEAL_SAF_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + HEAL_SAF_CONFIG_WRKBK, QUESTIONNAIRES_HNS)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk, VIDEO_DIR_PATH + re.split(".xlsx", HEAL_SAF_CONFIG_WRKBK)[0])
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", HEAL_SAF_CONFIG_WRKBK)[0] + "_" + QUESTIONNAIRES_HNS + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
