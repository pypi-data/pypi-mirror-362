...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while j < rowcount:
        ...
        j = j + 1
        datadictvalue["RowStatus"] = "Successfully Loaded values for Activity Value set"
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + GL_WORKBOOK, ACTIVITY):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + GL_WORKBOOK, ACTIVITY, PRCS_DIR_PATH + GL_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + GL_WORKBOOK, ACTIVITY)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk, VIDEO_DIR_PATH + re.split(".xlsx", GL_WORKBOOK)[0] + "_" + ACTIVITY)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", GL_WORKBOOK)[0] + "_" + ACTIVITY + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
