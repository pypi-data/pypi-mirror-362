...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_VALUE"] != '':
            page.get_by_role("button", name="Create").click()
            page.wait_for_timeout(2000)
            page.get_by_role("cell", name="Value").get_by_label("Value").nth(0).fill(datadictvalue["C_VALUE"])
            page.get_by_role("cell", name="Description").get_by_label("Description").nth(0).fill(datadictvalue["C_DSCRPTN"])
    ...
    try:
        ...
        print("Expenses DFF values Saved Successfully")


    except Exception as e:
        print("Expenses DFF values not saved")
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + EXP_WORKBOOK, EXP_DFF_VALUES):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + EXP_WORKBOOK, EXP_DFF_VALUES, PRCS_DIR_PATH + EXP_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + EXP_WORKBOOK, EXP_DFF_VALUES)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,
                               VIDEO_DIR_PATH + re.split(".xlsx", EXP_WORKBOOK)[0] + "_" + EXP_DFF_VALUES)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", EXP_WORKBOOK)[
            0] + "_" + EXP_DFF_VALUES + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
