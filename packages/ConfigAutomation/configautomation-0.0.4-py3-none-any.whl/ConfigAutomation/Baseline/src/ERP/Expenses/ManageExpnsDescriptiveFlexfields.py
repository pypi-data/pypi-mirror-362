...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    page.get_by_role("textbox").fill("Manage Descriptive Flexfields for Expense Reports")
    page.get_by_role("button", name="Search").click()
    # page.wait_for_timeout(3000)
    page.get_by_role("link", name="Manage Descriptive Flexfields for Expense Reports", exact=True).click()  #

    #Search Configuration Item
    page.get_by_label("Name").click()
    page.get_by_label("Name").fill("Expenses Report Header")
    page.get_by_role("button", name="Search", exact=True).click()
    page.get_by_role("cell", name="Expenses Report Header", exact=True).click()
    ...
    while i < rowcount:
        ...
        # page.locator("//div[text()='Search and Select: Value Set']//following::span[text()= '"+datadictvalue["C_VALUE_SET"]+"'][1]").click()
    ...
    try:
        ...
        print("Exp-Descriptive Flexfields Executed Successfully")

    except Exception as e:
        print("Exp-Descriptive Flexfields Executed UnSuccessfully")
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + EXP_WORKBOOK, EXP_DFF):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + EXP_WORKBOOK, EXP_DFF, PRCS_DIR_PATH + EXP_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + EXP_WORKBOOK, EXP_DFF)
    if rows > 0:
        with sync_playwright() as pw:
            ...
            write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", EXP_WORKBOOK)[
                0] + "_" + EXP_DFF + "_Results_" + datetime.now().strftime(
