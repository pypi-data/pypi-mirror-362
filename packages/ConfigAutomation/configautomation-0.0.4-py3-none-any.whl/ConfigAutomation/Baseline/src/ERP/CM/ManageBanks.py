...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    page.get_by_role("textbox").fill("Manage Banks")
    page.get_by_role("textbox").press("Enter")
    page.get_by_role("link", name="Manage Banks", exact=True).click()
    ...
    while i < rowcount:
        ...
        page.get_by_label("Name", exact=True).fill(datadictvalue["C_CNTRY"])
        page.get_by_role("button", name="Search", exact=True).click()
        page.get_by_role("cell", name=datadictvalue["C_CNTRY"], exact=True).click()
        page.get_by_role("button", name="OK").click()
        page.wait_for_timeout(1000)
        page.get_by_label("Bank Name", exact=True).fill(datadictvalue["C_BANK_NAME"])
        page.get_by_label("Alternate Bank Name").fill(datadictvalue["C_ALTRNT_BANK_NAME"])
        page.get_by_label("Bank Code").fill(datadictvalue["C_BANK_CODE"])
        page.get_by_label("Description").fill(datadictvalue["C_DSCRPTN"])
        page.get_by_label("Taxpayer ID").fill(datadictvalue["C_TXPYR_ID"])
        page.get_by_label("Tax Registration Number").fill(datadictvalue["C_TAX_RGSTRTN_NMBR"])

        # Address
        if datadictvalue["C_ADDRSS_LINE_1"] != '':
            page.get_by_label("Expand Addresses").click()
            page.get_by_role("button", name="Create").click()
            page.get_by_role("cell", name="Create Address Country *").get_by_label("Country", exact=True).select_option(
                datadictvalue["C_ADDRSS_CNTRY"])
            ...
            if datadictvalue["C_ADDR_PRMRY"] == 'Yes':
                page.get_by_role("button", name="Set Primary").click()

        # Create Bank Contact
        if datadictvalue["C_CNTCT_PHONE"] and datadictvalue["C_CNTCT_EMAIL"] and datadictvalue["C_FIRST_NAME"] != '':
            ...
            # Add Phone Numbers
            if datadictvalue["C_CNTCT_PHONE"] != '':
                ...
                page.get_by_role("cell", name="Phone Country Code").get_by_label("Phone Country Code").fill(str(datadictvalue["C_PHN_CNTRY_CODE"]))
                page.get_by_label("Area Code").fill(str(datadictvalue["C_AREA_CODE"]))
                page.get_by_label("Phone", exact=True).fill(str(datadictvalue["C_CNTCT_PHONE"]))
            ...
            if datadictvalue["C_CNTCT_EMAIL"] != '':
                ...
                page.get_by_label("Email").fill(datadictvalue["C_CNTCT_EMAIL"])
        ...
        try:
            ...
            print("Bank names Saved Successfully")
            datadictvalue["RowStatus"] = "Bank names are added successfully"

        except Exception as e:
            print("Bank names not saved")
            datadictvalue["RowStatus"] = "Bank names are not added"
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + CM_WORKBOOK, BANKS):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + CM_WORKBOOK, BANKS, PRCS_DIR_PATH + CM_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + CM_WORKBOOK, BANKS)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk, VIDEO_DIR_PATH + re.split(".xlsx", CM_WORKBOOK)[0] + "_" + BANKS)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", CM_WORKBOOK)[0] + "_" + BANKS + "_Results_" + datetime.now().strftime(
