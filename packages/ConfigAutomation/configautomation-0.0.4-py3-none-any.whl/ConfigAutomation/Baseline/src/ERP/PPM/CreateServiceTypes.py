...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    page.get_by_role("textbox").fill("Manage Service Types")
    page.get_by_role("textbox").press("Enter")
    page.get_by_role("link", name="Manage Service Types", exact=True).click()
    ...
    while i < rowcount:
        ...
        if datadictvalue["C_END_DATE"] != '':
            ...
            page.locator("//input[contains(@id,'id1')]").nth(0).fill(datadictvalue["C_END_DATE"])
    ...
    try:
        ...
        print("Service Types Saved Successfully")
        datadictvalue["RowStatus"] = "Service Types are added successfully"

    except Exception as e:
        print("Service Types not saved")
        datadictvalue["RowStatus"] = "Service Types are not added"
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + PPM_PFC_CONFIG_WRKBK, SER_TYP):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + PPM_PFC_CONFIG_WRKBK, SER_TYP, PRCS_DIR_PATH + PPM_PFC_CONFIG_WRKBK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + PPM_PFC_CONFIG_WRKBK, SER_TYP)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,
                               VIDEO_DIR_PATH + re.split(".xlsx", PPM_PFC_CONFIG_WRKBK)[0] + "_" + SER_TYP)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", PPM_PFC_CONFIG_WRKBK)[
            0] + "_" + SER_TYP + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
