...
def configure(playwright: Playwright, rowcount, datadict, videodir) -> dict:
    ...
    page.get_by_role("textbox").fill("Manage Fixed Assets Descriptive Flexfields")
    page.get_by_role("button", name="Search").click()
    # page.wait_for_timeout(3000)
    page.get_by_role("link", name="Manage Fixed Assets Descriptive Flexfields", exact=True).click()  #

    #Search Configuration Item
    page.get_by_label("Name").click()
    page.get_by_label("Name").fill("Asset Category")
    page.get_by_role("button", name="Search", exact=True).click()
    page.get_by_role("cell", name="Asset Category", exact=True).click()
    ...
    try:
        ...
        print("FA-Descriptive Flexfields Executed Successfully")

    except Exception as e:
        print("FA-Descriptive Flexfields Executed UnSuccessfully")
...
if CheckWrkbkForProcessing(SOURCE_DIR_PATH + FA_WORKBOOK, MANAGE_DFF):
    CreateWrkbkForProcessing(SOURCE_DIR_PATH + FA_WORKBOOK, MANAGE_DFF, PRCS_DIR_PATH + FA_WORKBOOK)
    rows, cols, datadictwrkbk = ImportWrkbk(PRCS_DIR_PATH + FA_WORKBOOK, MANAGE_DFF)
    if rows > 0:
        with sync_playwright() as pw:
            output = configure(pw, rows, datadictwrkbk,
                               VIDEO_DIR_PATH + re.split(".xlsx", FA_WORKBOOK)[0] + "_" + MANAGE_DFF)
        write_status(output, RESULTS_DIR_PATH + re.split(".xlsx", FA_WORKBOOK)[
            0] + "_" + MANAGE_DFF + "_Results_" + datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + ".xlsx")
    else:
