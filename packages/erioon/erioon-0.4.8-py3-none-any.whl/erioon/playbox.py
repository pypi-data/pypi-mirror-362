...
class Playbox:
    """
    Erioon playbox manages running Python code inside an isolated playbox environment.

    Usage:
    - Call runCode() with Python code to execute inside the playbox pod.
    - Optionally provide a shell command string to install packages before running the code.

    How it works:
    1. runCode():
       - Checks if the playbox pod exists and is running:
         - If the playbox exists but not running, deletes it and waits for deletion.
    ...
    """
    def __init__(self, namespace, sa_name, kubeconfig, playbox_id, cluster):
        self.namespace = namespace
        self.sa_name = sa_name
        self.kubeconfig = kubeconfig
        self.playbox_id = playbox_id
    ...
    def _read_only_response(self):
        return "[Erioon Error - Playbox access denied] This user is not allowed to perform any operations in the selected playbox."
    ...
    def runCode(self, code, packages=None):
        ...
        try:
            pod = v1.read_namespaced_pod(name=self.playbox_id, namespace=self.namespace)
            pod_exists = True
        ...
        if not pod_exists or not pod_running:
            if pod_exists:
                v1.delete_namespaced_pod(name=self.playbox_id, namespace=self.namespace)
                while True:
                    try:
                        v1.read_namespaced_pod(name=self.playbox_id, namespace=self.namespace)
                        time.sleep(1)
                    except ApiException as e:
                        if e.status == 404:
                            break
            ...
            pod_manifest = client.V1Pod(
                metadata=client.V1ObjectMeta(name=self.playbox_id),
            ...
            self._print_header(f"Erioon playbox '{self.playbox_id}' is preparing...", style="bold yellow")

            while True:
                pod_status = v1.read_namespaced_pod_status(self.playbox_id, self.namespace)
            ...
            self._print_header(f"Erioon playbox {self.playbox_id} is running...", style="bold green")

        if packages and packages.strip() != "":
            self._print_header(f"Running command: {packages}", style="green")
            ...
            self.console.print(Panel(syntax, title="Erioon command logs", expand=True))
        ...
        self.console.print(f"[bold cyan]Erioon playbox output ({elapsed_str}):[/bold cyan]")
        self.console.print(f"[bold cyan]{resp}")
