...
def simulateVV_GUI(paras, n, data, biOnly = 1,  strategy = 'ave' , fitType = 'dif', es_para = [1,1,1,1,1,0,0], fixvalue = [0.5,0.4,0.8,4000,2,0,0]):
    ...
    if es_para[5]==1:
        dU = abs(paras[pa_index])
        pa_index = pa_index + 1
    else:
        dU = fixvalue[5]
    
    if es_para[6]==1:
        dD = abs(paras[pa_index])
        pa_index = pa_index + 1
    else:
        dD = fixvalue[6]
...
def simulateLC_GUI (paras, n, data, strategy = 'ave', fittype = 'fre', es_para = [1,1,1,1,1,0,0], fixvalue = [0.5,2,8,4000,0,0,0]):
    # Import Data into Python
    dtl = data[0,:]
    cols = len (dtl)
    if cols == 3: 
    ...
    modelmat = np.empty((2, int(np.max(trialType).item())+1), dtype=object)
    datamat = np.empty((2, int(np.max(trialType).item())+1), dtype=object)
    ...
    for i in range(2):
        for j in range(int(np.max(trialType).item())+1):
            ...
            #print(modelprop)
            if cols == 3: 
                k2 = responses[np.where(trialType==j)[0]]
                datamat[0, j] = k2
    ...
    if maxV-minV > 100:
        lebu = (maxV - minV) / 10
        rangei= np.arange(minV- np.abs(minV)*0.4, maxV+ np.abs(maxV)*0.4, lebu)
    else:
    
        rangei= np.arange(-56, 56, 5)

    mid_points = (rangei[:-1] + rangei[1:]) / 2
    freqmat_data=[]
    freqmat_model=[]
    r2values = []
    klmat = []
    for m in range(int(np.max(trialType).item())+1):
        ...
        if fittype == 'fre':
            ...
            freq_VD = np.maximum(0.0001,hist_VD/np.sum(hist_VD))
            hist_VM,kk= np.histogram(modelmat[0,m], bins=rangei)
            freq_VM = np.maximum(0.0001,hist_VM/np.sum(hist_VM))
            #llV = np.sum(freq_VD * np.log(freq_VM))
            #llV = np.nan_to_num(llV)
            
            f = interp1d(mid_points, freq_VM, kind='cubic')
            #llV = np.sum(  np.nan_to_num(np.maximum (- np.log (f(datamat[0,m]) ), 0.0001) , nan = 0.0))
            llV = np.sum(  np.maximum (- np.log (np.nan_to_num(np.maximum (f(datamat[0,m]),0.001),nan = 0.01)), 0.0001))
            #re = f(datamat[0,m])
            #print(re)
            #print(llV)
            ...
            if cols == 4: 
                ...
                f = interp1d(mid_points, freq_AM, kind='cubic')
                
                #llA = np.sum(  np.nan_to_num(np.maximum (- np.log (f(datamat[1,m]) ), 0.0001) , nan = 0.0))
                llA = np.sum(  np.maximum (- np.log (np.nan_to_num(np.maximum (f(datamat[1,m]),0.001),nan = 0.01)), 0.0001))
            
            if not np.isnan(llV) and cols == 3: 
                loglike =loglike + llV
               
            if not np.isnan(llV) and not np.isnan(llA) and cols == 4: 
                loglike =loglike + llV + llA
        ...
        if fittype =='mr2':
            ...
            if datamat[0,m] is not None and datamat[0,m][0] < 1000:
                freqmat_data.append(freq_VD)
                freqmat_model.append(freq_VM)
                
            if datamat[1,m] is not None and datamat[1,m][0] < 1000 and cols == 4:
                freqmat_data.append(freq_AD)
                freqmat_model.append(freq_AM)

        if fittype=='emd':
            if datamat[0,m] is not None and datamat[0,m][1]> -1000:
            ...
            if datamat[1,m] is not None and datamat[1,m][1]>-1000 and cols == 4:    
...
def simulateLCD_GUI (paras, n, data, strategy = 'ave', fittype = 'mll', es_para = [1,1,1,1,1,0,0], fixvalue = [0.5,0.4,0.8,4000,2,0,0]):
    # Import Data into Python
    dtl = data[0,:]
    cols = len (dtl)
    if cols == 3: 
        ...
        responses_2col = np.hstack((responses, responses))
        responses = responses_2col
    ...
    modelmat = np.zeros((culnum, int(np.max(trialType).item())+1, N_l))
    datamat = np.zeros((culnum, int(np.max(trialType).item())+1, N_l))

    binM = unique_elements_r - 0.01
    binM = np.append(binM, binM[-1]+1)
    for i in range(culnum):
        for j in range(int(np.max(trialType).item())+1):
