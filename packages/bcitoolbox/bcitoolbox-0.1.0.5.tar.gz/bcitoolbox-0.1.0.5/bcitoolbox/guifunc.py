...
def gui():
    ...
    instdisc_url = "https://gitee.com/haochengzhu/bcitoolbox/raw/master/resources/instdisc_0028.png"
    ...
    local_instdisc = os.path.join(folder_path, "instdisc_0028.png")
    local_demo = os.path.join(folder_path, "demo.csv")

    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

    if not os.path.exists(local_BCI) or not os.path.exists(local_instdisc):
    ...
    bounds_l = [(0, 1),(1, 15),(1, 15),(5, 100),(-30, 30), (-100,100), (-100,100)]
    fixvalue = [0.5,0.4,0.8,4000,2,0,0]
    fixvalue_l = [0.5,2,8,4000,0,0,0]
...
def RDM_plot(ifsave):
    ...
    if ifsave==2:
        ...
        root2 = tk.Tk()
        root2.title("RDM Notes")

        # Set the window size
        root2.geometry("400x200")

        # Add a Text widget with the update instructions
        text_widget = tk.Text(root2, height=7, width=50)
        text_widget.pack(pady=10)
        update_message = ("Please note that, RDM functionality is an advanced\nfeature for comparison with Neuroimaging results.\nCare needs to be taken when using it. \nPlease check the documentation for details.")
        text_widget.insert(tk.END, update_message)
    ...
    for ndata in range (len(data_matrices)):
        fig = plt.figure()
        if disorcon == True:
            [error,modelprop,dataprop,d] = simulateVV_GUI(parameters_list[ndata], 10000, data_matrices[ndata], 1, strategy = strategy_list[ndata], fitType = 'mll', es_para = checkbox_vars, fixvalue = fixvalue)

        else :
            [err, modelprop, datamat, conditions]= simulateLC_GUI(parameters_list[ndata], 10000, data_matrices[ndata], strategy = strategy_list[ndata], es_para = checkbox_vars, fixvalue = fixvalue_l)

        if disorcon == True:
            data = np.concatenate((modelprop[0,:,:], modelprop[1,:,:]), axis=0)
            rows = data.shape[0]
        ...
        # Plot RDM
        plt.imshow(distances, cmap='BuGn', interpolation='nearest')
        plt.colorbar()
    ...
    #print(distances.shape)
        ...
        if ifsave == 1 and ndata == num_plot-1:
            ...
            if target_folder:
                ...
                file_name = os.path.basename(file_path)
                file_name = os.path.splitext(file_name)[0]
                figure_name = file_name + f"_{strategy_list[ndata]}"+ "_RDM.png"
                figure_path = os.path.join(target_folder, figure_name)
                plt.savefig(figure_path)

                # Save file
                mat_name = file_name + f"_{strategy_list[ndata]}"+ "_RDM.csv"
                mat_path = os.path.join(target_folder, mat_name)
                df = pd.DataFrame(distances)
                df.to_csv(mat_path, index=False, header=False)
...
def import_csv(text_entry):
    ...
    try:
        if file_paths:
            ...
            for file_path in file_paths:
                ...
                with open(file_path, 'r') as file:
                    ...
                    skip_header = not first_line.replace(',', '').replace('.', '').isdigit()

                df = np.loadtxt(file_path, delimiter=',', skiprows=1 if skip_header else 0)
        ...
        for ndata in range (len(data_matrices)):
            ...
            if disorcon == True:
                [error,modelprop,dataprop,d] = simulateVV_GUI([0.5,1,1,3,3], 10000, data_matrices[ndata], 1, strategy = 'ave', fitType = 'mll', es_para = checkbox_vars, fixvalue = fixvalue)

            else :
                [err, modelprop, datamat, conditions]= simulateLC_GUI([0.5,1,3,3,5], 10000, data_matrices[ndata], strategy = 'ave', es_para = checkbox_vars, fixvalue = fixvalue_l)
        ...
        text_widget = tk.Text(root2, height=7, width=50)
        text_widget.pack(pady=10)
        update_message = ("This error occurred due to potential inconsistencies detected in your data format.\nIt may or may not affect the results.\nPlease check them and our documentation! ")
        text_widget.insert(tk.END, update_message)
    ...
    if file_paths:
        ...
        for file_path in file_paths:
            ...
            with open(file_path, 'r') as file:
                ...
                first_cell = file.readline().split(',')[0].strip()
                has_header_flag = not is_number(first_cell)

            if has_header_flag:
                df = pd.read_csv(file_path, na_values='NaN')  # if header
            else:
                df = pd.read_csv(file_path, header=None, na_values='NaN')  # if no header
...
def open_file(text_entry):
    ...
    try:
        ...
        text_widget = tk.Text(root2, height=7, width=50)
        text_widget.pack(pady=10)
        update_message = ("Some errors have occurred.\nYour data format doesn't seem right.\nPlease check them and our documentation! ")
        text_widget.insert(tk.END, update_message)
...
def open_file2(text_entry):
    ...
    if file_paths:
        ...
        for file_path in file_paths:
            ...
            if has_header_flag:
                df = pd.read_csv(file_path)  # if header
            else:
                df = pd.read_csv(file_path, header=None, na_values='NaN')  # if no header
...
def about_bci():
    ...
    names_info_i = [
        ("Haocheng (Evans) Zhu", "University of California, Los Angeles\nSoochow University", "evanszhu2001@gmail.com"),   
    ]
...
def open_parameters_window():
    ...
    entry6_l.insert(tk.END, boundary[5][0])
    entry7_l.insert(tk.END, boundary[6][0])
    ...
    entry6_u.insert(tk.END, boundary[5][1])
    entry7_u.insert(tk.END, boundary[6][1])
...
def plot_func(ifsave):
    ...
    for ndata in range (len(data_matrices)):
        ...
        if init_numt == True:
            ...
            [error,modelprop,dataprop,c] = simulateLCD_GUI(parameters_list[ndata], 10000, data_matrices[ndata], strategy = strategy_list[ndata], fittype = 'mll', es_para = checkbox_vars, fixvalue = fixvalue_l)
...
def plot_func_l(ifsave):
    ...
    for ndata in range (len(data_matrices)):
        ...
        a = datamat.shape
        flat_datamat1 = np.hstack([item for item in np.ravel(datamat) if item is not None])
        flat_datamat2 = np.hstack([item for item in np.ravel(modelmat) if item is not None])
        clean_array1 = flat_datamat1[~np.isnan(flat_datamat1)]
        clean_array2 = flat_datamat2[~np.isnan(flat_datamat2)]
        combined = np.concatenate((clean_array1, clean_array2))
        maxV = np.max(combined)
        minV = np.min(combined)  
        condi = int(np.ceil((a[1] + 1) ** 0.5))
        ...
        if modelmat is not None:
            for i in range(a[1]):
                ...
                if modelmat[0, i][0] < 1000:
                    D1 = modelmat[0, i]
                    kde1 = gaussian_kde(D1)
                    x1 = np.linspace(min(D1), max(D1), 10)
                ...
                if modelmat[1, i][0] < 1000:    
            
                    D2 = modelmat[1, i]
                    kde2 = gaussian_kde(D2)
            
                    x2 = np.linspace(min(D2), max(D2), 10)
    ...
    if ifdata == 2:
        ...
        if file_path_simudata:
            with open(file_path_simudata, 'w') as file:
                ...
                arr = np.array([sti1, sti2])
                datamat = np.tile(arr, (n, 1))
                outmat = np.hstack((datamat, responsesSim))
                for row in outmat:
    ...
    if ifdata == 2:
        ...
        if file_path_simudata:
            with open(file_path_simudata, 'w') as file:
                ...
                datamat = np.tile(arr, (n, 1))
                outmat = np.hstack((datamat, responsesSim))
                for row in np.maximum(np.round(outmat),0):
...
def save_p():
    ...
    result_list = list(zip(file_list, parameters_list, error_list, strategy_list, BIC_list))
    file_path = filedialog.asksaveasfilename(defaultextension=".txt",
                                             filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")])
    if file_path:
        header = "File Name\t" + "\t".join(np.array(header_list)[checkbox_vars ==1 ]) + "\tError\tStrategy\tBIC\n"
        with open(file_path, "w") as txtfile:
            txtfile.write(header)
            for rdata in result_list:
                arr_str = '\t'.join(f"{element:.6f}" for element in rdata[1])
                txtfile.write(f"{rdata[0]}\t{arr_str}\t{rdata[2]}\t{rdata[3]}\t{rdata[4]}\n")
...
def run_code():
    global result_label, disorcon
    global progressbar
    global start_time
    global parameters_list, error_list, strategy_list, vb_list, BIC_list
    ...
    for ndata in range(len(data_matrices)):
        ...
        if best_result is not None:
            ...
            bic = 2 * best_error + count * np.log(len(data))
            bic_str = str(bic)
        ...
        BIC_list.append (bic_str)
        if init_V ==1:
            vb_list.append(best_vbmc)
...
def show_VB():
    ...
    try:
        for ndata in range(len(vb_list)):
            vp = vb_list[ndata]
            ...
            if target_folder:
                ...
                file_name = os.path.basename(file_path)
                file_name = os.path.splitext(file_name)[0]
                figure_name = file_name + f"_{strategy_list[ndata]}"+ "_VBMC" + ".png"
                figure_path = os.path.join(target_folder, figure_name)
                plt.savefig(figure_path)
        ...
        print(f"Failed to save the plot: {str(e)}")
...
def fit_page():
    ...
    plot_button.place(relx=0.3, rely=0.55, anchor="center")

    # RDM
    rdm_button = tk.Button(fit_frame, text="RDM", command= lambda: RDM_plot(1))
    rdm_button.place(relx=0.75, rely=0.55, anchor="center")

    # Save
    save_fit = tk.Button(fit_frame, text="Save All Figs", command=lambda: plot_func(2))
    save_fit.place(relx=0.3, rely=0.6, anchor="center")

    # RDM
    rdms_button = tk.Button(fit_frame, text="Save All RDMs", command= lambda: RDM_plot(2))
    rdms_button.place(relx=0.75, rely=0.6, anchor="center")

    if init_V ==1:
        vb_botton = tk.Button(fit_frame, text="VBMC Posteriors", command=show_VB)
        vb_botton.place(relx=0.5, rely=0.65, anchor="center")
...
def fit_page_l():
    ...
    # Save
    save_fit = tk.Button(fit_frame, text="Save All Figs", command=lambda: plot_func_l(2))
    save_fit.place(relx=0.3, rely=0.6, anchor="center")
...
def simu_page(singleif):
    ...
    if singleif==1:
        ...
        help_button.place(relx=0.2, rely=0.96, anchor="e")    
...
def show_help2():
    ...
    original_image = Image.open("resources/instdisc_0028.png")  
...
def open_welcome():
    ...
    version_label = tk.Label(window, text="Version 0.1.0.5", fg="gray")
