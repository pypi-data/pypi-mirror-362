...
class HelpdeskTicket(models.Model):
    ...
    def write(self, vals):
        ...
        self.synch_when_odoo_responds_change_stage_to_id()
        return res
    ...
    def synch_when_odoo_responds_change_stage_to_id(self):
        """
        Update when_odoo_responds_change_stage_to_id field since
        this field is not directly related to the stage_id field
        neither it is present in the UI to be modified by the user
        """
        for ticket in self:
            if ticket.when_odoo_responds_change_stage_to_id.id != (
                ticket.stage_id.when_odoo_responds_change_stage_to_id.id
            ):
                ticket.when_odoo_responds_change_stage_to_id = (
                    ticket.stage_id.when_odoo_responds_change_stage_to_id.id
                )
