...
from .auth import call_mcpauth_login, get_available_servers, McpAuthClient, McpAuthError
...
__all__ = [
    ...
    "call_mcpauth_login", "get_available_servers", "McpAuthClient", "McpAuthError"
]
    ...
    **provider_kwargs
    ...
    async with MCPAgent(
        ...
        **provider_kwargs
        ...
        async for chunk in agent.chat_with_tools_stream(
            message=message,
            max_tools=max_tools,
            min_score=min_score
        ):
...
async def _list_tools_async(
    ...
    # Create and use agent with async context manager
    async with MCPAgent(
        provider="anthropic",  # Use minimal provider for tool listing
        server_name=server_name,
        server_url=config["url"],
        auth_token=config["auth_token"],
        enable_filtering=False  # No filtering needed for listing
    ) as agent:
...
async def _get_tool_info_async(
    ...
    # Create and use agent with async context manager
    async with MCPAgent(
        provider="anthropic",  # Use minimal provider for tool info
        server_name=server_name,
        server_url=config["url"],
        auth_token=config["auth_token"],
        enable_filtering=False  # No filtering needed for tool info
    ) as agent:
...
async def _execute_tool_async(
    ...
    # Create and use agent with async context manager
    async with MCPAgent(
        provider="anthropic",  # Use minimal provider for tool execution
        server_name=server_name,
        server_url=config["url"],
        auth_token=config["auth_token"],
        enable_filtering=False  # No filtering needed for tool execution
    ) as agent:
        # Execute the tool
        return await agent.execute_tool(tool_name, tool_input)
