...
class RelationshipDetector:
    ...
    def __init__(self):
        ...
        self._data_connector_file_cache = {}
    ...
    def detect_shared_field_relationships(self, entities_map: Dict[str, Dict],
        ...
        Args:
            ...
            exclude_connected_pairs: Set of frozen sets containing entity pairs already connected via FK
    ...
    def _get_singular_plural_forms(self, word: str) -> set[str | Literal[False]]:
        """
    ...
    def _build_data_connector_schema_cache(self, entities_map: Dict[str, Dict]):
        """
        ...
        for entity_qnk, entity_info in entities_map.items():
            ...
            for connector_name in self.known_data_connectors:
                ...
                try:
                    ...
                    if connector_file_path not in self._data_connector_file_cache:
                        documents = load_yaml_documents(connector_file_path)
                        self._data_connector_file_cache[connector_file_path] = documents
                        logger.debug(f"Cached DataConnectorLink file: {connector_file_path}")
                    else:
                        documents = self._data_connector_file_cache[connector_file_path]
        ...
        logger.info(f"  - Cached {len(self._data_connector_file_cache)} files")
    ...
    def _build_data_connector_cache(self, entities_map: Dict[str, Dict]):
        ...
        self._build_data_connector_schema_cache(entities_map)
            ...
            try:
                ...
                for doc in documents:
                    if isinstance(doc, dict) and doc.get('kind') == 'Relationship':
                        from . import RelationshipGenerator
