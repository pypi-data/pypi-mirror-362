...
def docker_up():
    ...
    if not os.path.exists(".env"):
        ...
        with open(".env", "w") as f:
            f.write("AIRFLOW_UID=50000\nAIRFLOW_GID=0\nAIRFLOW_CONN_AWS_DEFAULT=aws://<login>:<senha>@/?host=https://s3.lema.ufpb.br&region_name=us-east-1")
            log.info("✅ .env file create successfully!")
    ...
    try:
        subprocess.run(["docker", "compose", "--project-name", "lema-dev", "up", "-d"], env=env, check=True)
