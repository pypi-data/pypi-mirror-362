...
class BaseHandler:
    ...
    async def response_process(self, data, func, need_feature):
        ...
        """
        content_type = self.infer_request.header.get('Content-Type', "").strip()
        # 简单评估下是否为流式请求 1. 判断request.header.get('Content-Type')是否包含stream 2. 判断data 是否包含stream
        is_stream = content_type.startswith("text/event-stream") or (isinstance(data, dict) and data.get('stream'))
