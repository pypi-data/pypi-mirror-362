...
class LinkedEssence(BaseModel):
    ...
    Link to essence.
    """ # noqa: E501
    essence_key: Annotated[str, Field(min_length=1, strict=True, max_length=1024)] = Field(description="Essence key, unique identifier.")
    ...
    def essence_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['LOCATION', 'ORDER', 'ASSIGNED_ORDER', 'PERFORMER', 'TRANSPORT', 'HARDLINK', 'TRIP', 'FACT', 'CREW']):
            raise ValueError("must be one of enum values ('LOCATION', 'ORDER', 'ASSIGNED_ORDER', 'PERFORMER', 'TRANSPORT', 'HARDLINK', 'TRIP', 'FACT', 'CREW')")
        return value
