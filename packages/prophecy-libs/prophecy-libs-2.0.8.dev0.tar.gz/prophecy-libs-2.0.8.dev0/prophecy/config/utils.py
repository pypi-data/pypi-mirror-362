...
def parse_config(args, pipeline_dot_conf=None, config_package=None):
    ...
    if args.projectConfInstance is not None:
        resource_file_name = f"{args.confInstance}.json"
        project_instance_file_content = get_resource_file_content(resource_file_name, config_package)
        project_instance_conf_tree = ConfigFactory.parse_string(project_instance_file_content)
    ...
    # Merge pipeline file configs with project configs here. Give less priority to project config here.
    conf = conf
