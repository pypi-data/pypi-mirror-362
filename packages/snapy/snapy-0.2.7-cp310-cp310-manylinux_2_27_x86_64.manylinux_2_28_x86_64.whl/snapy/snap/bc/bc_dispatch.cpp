  ...
  AT_DISPATCH_INTEGRAL_TYPES(iter.dtype(), "flip_zero_cpu", [&] {
    ...
    iter.for_each([&](char** data, const int64_t* strides, int64_t n) {
      for (int i = 0; i < n; i++) {
        ...
        auto fromLen = reinterpret_cast<scalar_t*>(data[2] + i * strides[2]);
        auto fromBit = reinterpret_cast<scalar_t*>(data[3] + i * strides[3]);
        auto usedFlip = reinterpret_cast<scalar_t*>(data[4] + i * strides[4]);

        num_flips += compute_min_flips<scalar_t>(
            solid, len, /*minRun0=*/3, /*minRun1=*/2,
            /*allowBothFlips=*/0, dir, stride, dp, fromLen, fromBit, usedFlip);

        reconstruct_solution<scalar_t>(solid, len, /*minRun0=*/3, /*minRun1=*/2,
