    ...
    def transform(
        ...
        if inference_type == InferenceType.TEXT:
            ...
            try:
                ...
                # Convert PDF to images if PDF support is available
                if PDF_SUPPORT:
                    try:
            ...
            if isinstance(data_content, list):
                ...
                if PDF_SUPPORT:
                    try:
                        images = InputTransformer.process_image_base64_list(
                            data_content
                        )
                        # Store processed images as base64 strings for DataFrame compatibility
                        processed_images = [
                            InputTransformer.pil_image_to_base64_str(img)
                            for img in images
                        ]
                        df_data = {
                            "data": [
                                data_content
                            ],  # Wrap in list for DataFrame compatibility
                            "processed_images": [
                                processed_images
                ...
                # Single base64 image - convert to list for consistency
