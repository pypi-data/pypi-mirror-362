    ...
    def transform(
        ...
        if inference_type == InferenceType.TEXT:
            ...
            try:
                ...
                logger.info(
                    f"DEBUG: Starting BYTE_STREAM processing for data type: {type(data_content)}"
                )
                pdf_bytes = InputTransformer.decode_input_data(data_content)
                logger.info(f"DEBUG: Successfully decoded {len(pdf_bytes)} bytes")
                ...
                logger.info(f"DEBUG: Detected type for binary data: {detected_type}")
