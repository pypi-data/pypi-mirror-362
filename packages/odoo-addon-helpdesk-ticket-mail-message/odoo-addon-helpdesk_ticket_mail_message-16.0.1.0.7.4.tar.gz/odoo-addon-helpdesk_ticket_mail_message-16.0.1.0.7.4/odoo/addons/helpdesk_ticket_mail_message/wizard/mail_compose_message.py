...
class MailComposeMessage(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if (
            ...
            and self._context.get("composition_mode") != "comment"
            and self._context.get("default_composition_mode") != "comment"
        ):
            result["body"] = (
                Markup(f"<br/><p>{self.env.user.signature}</p>").unescape()
                + Markup(result.get("body", "")).unescape()
            )
