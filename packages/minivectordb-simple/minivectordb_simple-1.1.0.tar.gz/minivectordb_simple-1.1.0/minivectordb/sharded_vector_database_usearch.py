...
import numpy as np, pickle, os, threading, faiss
from usearch.index import Index, Matches
from operator import gt, ge, lt, le, ne
from collections import defaultdict
from thefuzz import fuzz

class ShardedVectorDatabaseUsearch:
    def __init__(self, storage_dir='db_shards_usearch', shard_size=5000):
    ...
    def autocut_distances(self, distance_list):
        """
        This function takes a list of distances and determines if there is a significant increase in the distances.
        If there is an increase greater than 20%, it returns the indices of the distances that would be removed.
        If there is no such increase, it returns an empty list.

        Inspired by weaviate's golden ragtriever autocut feature.
        This is a basic implementation and can be improved.
        """
        # Find the percentage of each distance increase
        distance_increases = []
        for i in range(1, len(distance_list)):
            distance_increases.append((distance_list[i] - distance_list[i-1]) / distance_list[i-1])
        
        # Find the highest distance increase
        max_distance_increase = max(distance_increases)

        if max_distance_increase > 0.2:
            # Return all the indexes that would be removed if cut at the index of the max_distance_increase
            return list(range(distance_increases.index(max_distance_increase) + 1, len(distance_list)))
    ...
    def find_most_similar(self, embedding, metadata_filter=None, exclude_filter=None, or_filters=None, k=5, autocut=False):
        ...
        indices = np.arange(filtered_indices_np.shape[0])
        filtered_index.add(indices, filtered_embeddings, copy=False)

        search_results: Matches = filtered_index.search(embedding, search_k)
        for result in search_results:
            ...
            found_results.append((uid, result.distance, self.metadata[filtered_indices_np[idx]]))
        ...
        if autocut and len(distances) > 1:
            ...
            remove_indexes = self.autocut_distances(distances)
            if remove_indexes:
