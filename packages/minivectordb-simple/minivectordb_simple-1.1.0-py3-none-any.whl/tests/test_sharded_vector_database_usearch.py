from minivectordb.sharded_vector_database_usearch import ShardedVectorDatabaseUsearch
from minivectordb.vector_database import VectorDatabase
from minivectordb.embedding_model import EmbeddingModel
from contextlib import contextmanager
import numpy as np, shutil, uuid, os

model = EmbeddingModel()

@contextmanager
def delete_folder_after():
    try:
        folder_path = f'./db_shards_{uuid.uuid4()}'
...
def test_initialization():
    with delete_folder_after() as shards_path:
        db = ShardedVectorDatabaseUsearch(storage_dir=shards_path)
...
def test_persist_and_load():
    with delete_folder_after() as shards_path:
        db = ShardedVectorDatabaseUsearch(storage_dir=shards_path, shard_size=2)
        ...
        db2 = ShardedVectorDatabaseUsearch(storage_dir=shards_path, shard_size=2)
...
def test_migrate_from_non_sharded_version():
    with delete_folder_after() as shards_path:
        # Create an instance of ShardedVectorDatabase
        sdb = ShardedVectorDatabaseUsearch(storage_dir=shards_path)
...
def test_index_then_delete_everything_and_reload():
    with delete_folder_after() as shards_path:
        # Create an instance of ShardedVectorDatabase
        db = ShardedVectorDatabaseUsearch(storage_dir=shards_path, shard_size=50)
