...
def display_version():
    print('Query Search version 0.0.1-alpha3')


def replace_tool(text: str, a: str, b: str):
    print(text.replace(a, b), end='')


def main():
    # Handle version flags before parsing other arguments
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'replace':
        if len(sys.argv) != 4:
            print("Usage: qq replace <search_text> <replace_text>", file=sys.stderr)
            sys.exit(1)

        # Read from stdin
        text = sys.stdin.read()
        replace_tool(text, sys.argv[2], sys.argv[3])
        return

    parser = argparse.ArgumentParser(description="Query text files using SQL")
    parser.add_argument('-c', '--headers', default=False, action=argparse.BooleanOptionalAction,
                        help='Whether to include Headers (Column Names) or not')
    ...
    parser.add_argument("query",
                        help="SQL query to execute (use 'line' as column name). To use a file, type 'path:/path/to/file.sql'")
