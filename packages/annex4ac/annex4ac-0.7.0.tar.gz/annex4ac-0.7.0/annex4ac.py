...
# (убран глобальный импорт opa_wasm)
...
_WASM_BUNDLE = Path("annex4ac/annex4_validation.wasm")
...
def load_opa_runtime(wasm_bytes):
    if sys.platform == "win32":
        from opa_wasm._ffi import OPARuntime
        return OPARuntime(wasm_bytes, engine="wasmtime")
    else:
        from opa_wasm import OPARuntime
        return OPARuntime(wasm_bytes)

def _opa_check(payload, sarif_path=None, yaml_path=None):
    """
    Offline validation via built-in Wasm runtime.
    """
    rt = load_opa_runtime(_WASM_BUNDLE.read_bytes())
...
def fetch_annex3_tags(cache_path="high_risk_tags.json", cache_days=14):
    """
    Парсит Annex III, кэширует результат в high_risk_tags.json (рядом с annex4ac.py),
    возвращает множество тегов.
    """
