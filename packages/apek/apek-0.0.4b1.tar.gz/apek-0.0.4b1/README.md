...
# Installation
Install via pip: `pip install apek==0.0.4b1`
...
# Upgraded Contents
- New or changed:
  - Changed the storage type of `base` to `mpmath.mp.mpf`.
  - Modified the name of the lowest-level class in the `typing` module.
  - Added support for the `__iter__` and `__mpmath__` methods.
- Fixes:
  - Fixed an inevitable error in the constructor.
  - Removed new version content that was accidentally added but not yet implemented.
...
# 安装
通过pip安装：`pip install apek==0.0.4b1`
...
# 更新内容
- 新增或更改：
  - 将base的存储类型更改为`mpmath.mp.mpf`。
  - 修改了`typing`模块里的最底层类的名称。
  - 支持了`__iter__`和`__mpmath__`方法。
- 修复：
  - 修复了一个构造函数的不可避免的错误。
  - 移除了一个不慎添加但未开始实现的新版本内容。
