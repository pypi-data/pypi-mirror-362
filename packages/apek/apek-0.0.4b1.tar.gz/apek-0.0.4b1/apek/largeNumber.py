...
class LargeNumber(typing.Number):
    ...
    def setExp(self, exp, *args):
        ...
        self.base = int(exp) if not isinstance(exp, int) else exp
    ...
    def _calibrate(self):
        ...
        if base == 0:
            self.base, self.exp = _mp.mpf(0), 0
            return
        k = _mp.floor(_mp.log10(base))
        exp += k
        base /= _mp.power(10, k)
        prec = self.getConfig("realPrec")
        self.base = -base * (neg * 2 - 1)
        self.exp = exp
        base = _mp.floor(base * _mp.power(10, prec)) / _mp.power(10, prec)
    ...
    def parseString(self, *args, prec="default", expReprMode="comma", template="{}e{}", alwaysUseTemplate=False):
        """
    ...
    def __repr__(self):
        return self.parseString(template="LargeNumber({0}, {1})", alwaysUseTemplate=True)
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, (int, float)):
            new = self._hotCreate(self.getBase() * other, self.getExp())
            new.config = self.getConfig()
            return new
        other = self._parseLargeNumberOrShowError(other)
        return LargeNumber(
    ...
    def __mpmath__(self, prec, _):
        with _mp.workdps(prec):
            return _mp.mpf(self.parseString(expReprMode="none", template="{}e{}", alwaysUseTemplate=True))
