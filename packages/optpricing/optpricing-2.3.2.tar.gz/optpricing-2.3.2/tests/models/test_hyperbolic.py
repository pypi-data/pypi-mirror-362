...
@pytest.fixture
def model():
    """Provides a HyperbolicModel instance."""
    return HyperbolicModel(params=PARAMS)
...
def test_parameter_validation():
    ...
    with pytest.raises(ValueError, match="parameters must be positive: alpha"):
        HyperbolicModel(params={"alpha": -15.0, "beta": -5.0, "delta": 0.5, "mu": 0.0})
    with pytest.raises(ValueError, match="must satisfy |beta| < alpha"):
        HyperbolicModel(params={"alpha": 5.0, "beta": -15.0, "delta": 0.5, "mu": 0.0})
