...
class FileFlowHandle:
    ...
    def __init__(self, not_allow_file_type_list=None, postgres_url=None, database_name=None):
        ...
        self.postgres_engine = create_engine(f"{self.postgres_url}/{self.databases}")
    ...
    def file_flow_exit_point(
        ...
        join_stmt = op_meta.outerjoin(op_attachment, op_meta.c.id == op_attachment.c.meta_id)

        stmt = select(op_meta, op_attachment).select_from(join_stmt)
...
if __name__ == '__main__':
    ...
    # 读取用例
    result = file_flow.file_flow_exit_point(status='success', tags='test_data', limit_size=1)
    for data in result:
    ...
    status, ids, message = file_flow.file_flow_entry_point(records, start_stage, target_stage, tag_name, priority=priority)
