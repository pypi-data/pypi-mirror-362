...
class SynMem:
    ...
    # ─── Retrieve ────────────────────────────────────────────────────────
    def recallMemory(self, query: str, paths: list, user: str = None, type: str = 'Embedding', topK=5, minScore: int = 60, showProgress: bool = False):
        type = type.lower()
        if type == 'embedding':
            return self.recallMemoryEmbedding(query, paths, user, topK, showProgress)
        elif type == 'rapid':
            return self.recallMemoryRapid(query, paths, user, topK, minScore)
        else:
            raise ValueError(f"Unknown recall type: {type}, expected 'embedding' or 'rapid'.")

    def recallMemoryEmbedding(self, query: str, paths: list, user: str = None, topK=5, showProgress: bool = False):
            ...
            for dbFile in dbFiles:
                try:
                    ...
                    if 'user' in cols and user:
                        ...
                        params = []
                    db_rows = self.synMem.retrieveMemoryFromDb(dbFile, query_str, params)
                    for row in db_rows:
    ...
    def recallMemoryRapid(self, query: str, paths: list, user: str = None, topK=5, minScore: int = 60):
        """
