...
fn main() {
    ...
    let file_content = fs::read_to_string("003_Cantilever_with_Uniform_Distributed_Load.json").expect("Failed to read the JSON file");
    let mut fers_data: FERS = serde_json::from_str(&file_content).expect("JSON was not well-formatted");
    ...
    if let Some(first_load_case_id) = fers_data.load_cases.first().map(|lc| lc.id) {
        ...
        match fers_data.solve_for_load_case(first_load_case_id) {
            ...
            Err(e) => {
                debug!("Error during analysis for Load Case {}: {}", first_load_case_id, e);
            }
        }
    } else {
