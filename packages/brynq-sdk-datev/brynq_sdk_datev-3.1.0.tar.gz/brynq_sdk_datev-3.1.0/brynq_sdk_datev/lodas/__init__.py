...
class DatevLodas(BrynQ):
    def __init__(self, berater_nr: int = None, mandanten_nr: int = None):  # , pay_components: dict = None, departments: dict = None, costcenters: dict = None, companies: dict = None
        super().__init__()
        self.credentials = self.interfaces.credentials.get(system='datev', system_type='target')
        if berater_nr is None:
            self.berater_nr = self.credentials.get('data',{}).get('consultant_number')
        else:
            self.berater_nr = berater_nr
        if mandanten_nr is None:
    ...
    def create_template(self,
                        ...
                        df_wage_components: pd.DataFrame = None,
                        df_declaration: pd.DataFrame = None,
                        df_postcalculation: pd.DataFrame = None) -> (list, list):
    ...
    def employee_employment_periods(df: pd.DataFrame,
        ...
        kuendigung_am#psd               Kündigung am                    Resignation date                  resignation_date             12
        """
        ...
        for _, row in df.iterrows():
            formatted_string = (
                ...
                f"{row['resignation_date'] if 'resignation_date' in row.keys() else ''};\n"
            )
