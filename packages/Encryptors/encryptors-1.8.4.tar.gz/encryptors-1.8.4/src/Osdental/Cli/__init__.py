...
from Osdental.Models.Token import AuthToken
from ..Interfaces.{app} import {use_case_interface_content}
...
class {app}Repository({repository_interface_name}):
    ...
    @staticmethod
    async def get_all_{name_method}(self, {data}) -> List[Dict[str,str]]: pass
    
    @staticmethod
    async def get_{name_method}_by_id(self, id: str) -> Dict[str,str]: pass

    @staticmethod    
    async def create_{name_method}(self, {data}) -> str: pass

    @staticmethod                
    async def update_{name_method}(self, id: str, {data}) -> str:pass
        
    @staticmethod    
    async def delete_{name_method}(self, id: str) -> str: pass
    '''
...
from ..Resolvers.{app}Resolver import {app}Resolver
...
class {app}Resolver:
    ...
    async def resolve_get_all_{name_method}(self, _, info, data):
        return await use_case.get_all_{name_method}(info=info, aes_data=data)

    @staticmethod
    @handle_audit_and_exception
    async def resolve_get_{name_method}_by_id(self, _, info, data):
        return await use_case.get_{name_method}_by_id(info=info, aes_data=data)

    @staticmethod
    @handle_audit_and_exception
    async def resolve_create_{name_method}(self, _, info, data):
        return await use_case.create_{name_method}(info=info, aes_data=data)

    @staticmethod
    @handle_audit_and_exception
    async def resolve_update_{name_method}(self, _, info, data):
        return await use_case.update_{name_method}(info=info, aes_data=data)
    
    @staticmethod
    @handle_audit_and_exception
    async def resolve_delete_{name_method}(self, _, info, data):
