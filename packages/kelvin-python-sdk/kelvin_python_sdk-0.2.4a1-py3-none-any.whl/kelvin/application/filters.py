...
def is_asset_data_quality_message() -> Callable[[Message], TypeGuard[AssetDataQualityMessage]]:
    """Check if the message is an Asset Data Quality Message."""

    def _check(msg: Message) -> TypeGuard[AssetDataQualityMessage]:
        return isinstance(msg.resource, KRNAssetDataQuality) and isinstance(msg.type, KMessageTypeData)

    return _check


def is_asset_data_stream_quality_message() -> Callable[[Message], TypeGuard[AssetDataStreamDataQualityMessage]]:
    """Check if the message is an Asset Data Stream Data Quality Message."""

    def _check(msg: Message) -> TypeGuard[AssetDataStreamDataQualityMessage]:
