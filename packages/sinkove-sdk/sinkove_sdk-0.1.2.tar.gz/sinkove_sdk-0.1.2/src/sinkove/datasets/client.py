...
class Dataset:
    """
    A class representing a dataset with associated metadata and operations.

    Attributes:
    - conn: A connector object for making requests.
    - id: The unique identifier of the dataset.
    - model_id: The unique identifier of the model associated with the dataset.
    - organization_id: The unique identifier of the organization owning the dataset.
    - owner_id: The identifier of the owner of the dataset.
    - num_samples: The number of samples in the dataset.
    - args: Additional arguments related to the dataset.
    - created_at: The creation timestamp of the dataset.
    - updated_at: The last updated timestamp of the dataset.
    - metadata: Optional metadata associated with the dataset.
    """
    ...
    def finished(self):
        """
        Check if the dataset processing is finished.

        Returns:
        - True if the dataset state is neither "PENDING" nor "STARTED", otherwise False.
        """
    ...
    def ready(self):
        ...
        Returns:
        - True if the dataset state is "READY", otherwise False.
        """
    ...
    def state(self) -> metadata.State:
        """
        Retrieve the current state of the dataset.

        Returns:
        - The state of the dataset as a metadata.State object.
        """
    ...
    def download(
        ...
        """
        Download the dataset to a specified file.

        Parameters:
        - output_file: The path where the downloaded dataset will be saved.
        - strategy: The strategy to use if the file already exists. Options are:
          - "fail": Raise an exception if the file exists.
          - "skip": Do not download if the file exists.
          - "replace": Overwrite the existing file.
        - wait: If True, wait for the dataset to be ready before downloading.
        - timeout: Optional; the maximum time in seconds to wait for the dataset to be ready.
                   If None, wait indefinitely.

        Raises:
        - Exception: If the file exists and the strategy is "fail".
        - Exception: If the download URL cannot be retrieved.
    ...
    def wait(self, timeout: typing.Optional[int] = None):
        """
        Wait for the dataset processing to complete.
        ...
        Raises:
        - TimeoutError: If the waiting time exceeds the specified timeout.
        - Exception: If the dataset processing fails and the dataset is not ready.
        """
...
class DatasetClient:
    ...
    def list(self):
        """
        Retrieve a list of datasets for the organization.

        Returns:
        - A list of Dataset objects representing the datasets.
        """
    ...
    def get(self, dataset_id: uuid.UUID) -> Dataset:
        ...
        Returns:
        - A Dataset object representing the retrieved dataset.
        """
        response = self.conn.make_request(
