...
from httpx_ws import aconnect_ws, _exceptions as httpx_ws_exceptions
...
async def proxy_ws(client_ws: WebSocket):
    # Check if the model is running before accepting the connection
    inference_server_process_controller = (
        client_ws.app.state.inference_server_process_controller
    )
    proxy_client: httpx.AsyncClient = client_ws.app.state.proxy_client
    logger = client_ws.app.state.logger

    # Check if inference server process is running
    if not inference_server_process_controller.is_inference_server_running():
        logger.warning("WebSocket connection rejected: inference server not running")
        await client_ws.close(code=1000, reason="Inference server not running")
        return

    # Try to establish a websocket connection to verify the model is ready
    try:
        async with aconnect_ws("/v1/websocket", proxy_client):
            # If we can establish the connection, the model is ready
            pass
    except (
        httpx_ws_exceptions.WebSocketUpgradeError,
    ):
        logger.warning("WebSocket connection rejected: model not ready")
        await client_ws.close(code=1000, reason="Model not ready")
        return

    # If we get here, the model is running and ready
