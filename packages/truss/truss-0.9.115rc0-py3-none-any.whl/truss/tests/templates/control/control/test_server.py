...
@pytest.fixture()
async def ws_client(app, ports, client):
    async with websockets.connect(
        f"ws://localhost:{ports['control_server_port']}/v1/websocket",
    ) as ws:
        yield ws
...
@pytest.mark.anyio
async def test_patch_model_code_websocket_long_load_time(app, ports, ws_client):
    mock_model_file_content = """
import fastapi
...
class Model:
    ...
    async def websocket(self, websocket: fastapi.WebSocket) -> Any:
...
"""
    ...
    # Use the ws_client fixture instead of creating new connection
    await ws_client.send_text("hello")
    response = await ws_client.receive_text()
