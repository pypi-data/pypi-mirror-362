...
from alipayplusmcp.sdk.response.pay.alipay_cancel_payment_response import AlipayCancelPaymentResponse
...
class TestDefaultAlipayClient(unittest.TestCase):
    ...
    def test_initialization_is_sandbox(self):
        alipayCancelRequest = AlipayCancelPaymentRequest()

        alipayCancelRequest.payment_request_id = 'pay_1089760038715669_102775745070000'

        resp = self.client.execute(alipayCancelRequest)

        payResp = AlipayCancelPaymentResponse(resp)
