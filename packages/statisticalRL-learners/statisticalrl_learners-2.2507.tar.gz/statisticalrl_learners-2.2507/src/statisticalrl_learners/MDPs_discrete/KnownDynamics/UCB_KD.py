    ...
    def compute_indexes_and_info(self):
        ...
        if self.empirical_optimal is not None:
            if np.all(policy == self.empirical_optimal):
                ...
                # TODO
                ucb_index = gain_pool + np.sqrt(np.log(1 + self.t) / precision_pool)
                return ucb_index, policy_pool, precision_pool, stationary_pool, delta
    ...
    def play(self, state):
        if not self.all_visited:
            ...
            if self.new_episode:
                ...
                ucb_index, policies, pulls, distributions, delta = info

                imed_idx = np.argmax(ucb_index)
