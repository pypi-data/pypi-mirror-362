    ...
    def compute_indexes_and_info(self):
        ...
        if self.empirical_optimal is not None:
            if np.all(policy == self.empirical_optimal):
                ...
                # imed = delta + np.log(precision_pool)
                delta = np.isclose(delta, 0)
                # TODO
                ts_index = np.random.normal(gain_pool, 0.5 / np.sqrt(precision_pool))
                return ts_index, policy_pool, precision_pool, stationary_pool, delta
    ...
    def play(self, state):
        if not self.all_visited:
            ...
            if self.new_episode:
                ...
                ts_index, policies, pulls, distributions, delta = info

                imed_idx = np.argmax(ts_index)
