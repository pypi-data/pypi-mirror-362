...
def resume_progress(run_name: str, restart: bool, num_workers: int, total_items: int, tmp_dir: str):
    ...
    if checkpoint is None:
        ...
        logger.warning(
            "Resuming from checkpoint. This is expected if you are resuming from a previous interruption, "
...
def iterate_wrapper(
    ...
    **kwargs: ParamTypes.kwargs,
    ...
    retry_func = cast(
        Callable[[IO, DataType], ReturnType],
        retry_dec(retry, wait, on_error, None, retry_on_type_error)(wrapped_func),
    )
