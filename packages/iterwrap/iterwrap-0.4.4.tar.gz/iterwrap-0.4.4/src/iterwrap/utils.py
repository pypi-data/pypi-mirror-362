...
from typing import IO, Callable, Generic, Literal, ParamSpec, TypeVar
...
class Retriable(Generic[ParamTypes, ReturnType]):
    "provide func or subclass with call method"

    def __init__(
        self,
        func: Callable[ParamTypes, ReturnType] | None = None,
        max_retry=3,
        wait=0,
        on_error: Literal["raise", "continue"] = "raise",
        validate_fn: Callable[[ReturnType, float], bool] | None = None,
        retry_on_type_error=False,
    ):
        if func is None and not hasattr(self, "call"):
            raise ValueError("Either func or call method must be provided")
        self.func: Callable[ParamTypes, ReturnType] = self.call if func is None else func  # type: ignore
        self.max_retry = max_retry
        self.wait = wait
        self.on_error = on_error
        self.validate_fn = validate_fn
        self.retry_on_type_error = retry_on_type_error

    def __call__(self, *args: ParamTypes.args, **kwargs: ParamTypes.kwargs) -> ReturnType | None:
        if self.max_retry <= 1:
        ...
        for j in range(self.max_retry):
            ...
            try:
                ...
                if self.validate_fn is not None and not self.validate_fn(res, j / (self.max_retry - 1)):
                ...
                if not self.retry_on_type_error and isinstance(e, TypeError):
                    logger.error(traceback.format_exc())
                    raise e
                if j == self.max_retry - 1:
        ...
        # def call(self, *args: ParamTypes.args, **kwargs: ParamTypes.kwargs) -> ReturnType:
...
def retry_dec(
    ...
    Args:
        ...
        wait: the wait time between retries
        on_error: the action to take when all retries fail
        validate_fn: a function to validate the result, whose input is the result of the function and the retry ratio (retry / (max_retry - 1)) and output is a boolean
        retry_on_type_error: whether to retry on type error
    """
        ...
        def wrapper(*args: ParamTypes.args, **kwargs: ParamTypes.kwargs) -> ReturnType | None:
            ...
            for j in range(max_retry):
                try:
                    ...
                    if validate_fn is not None and not validate_fn(res, j / (max_retry - 1)):
                        raise ValueError(f"Validation failed: {res}")
                    return res
                except KeyboardInterrupt as e:
                    logger.error(traceback.format_exc())
                    ...
                    if not retry_on_type_error and isinstance(e, TypeError):
                        logger.error(traceback.format_exc())
                        raise e
                    if j == max_retry - 1:
                        if on_error == "raise":
