...
from . import log
from lance.catalog.hive_catalog import HiveCatalog
...
from .catalog.utils import Utils
from .dataset import (
    DataStatistics,
    FieldStatistics,
    LanceDataset,
...
def read_table(
    ...
    """
    Implement the table read from las_catalog
    :param identifier:
    :param version:
    :param asof:
    :param block_size:
    ...
    #add/overwrite table identifier in storage_options
    storage_options = storage_options.copy() if storage_options else {}
    if isinstance(identifier, tuple):
        str_identifier = ".".join(identifier)
    else:
        str_identifier = str(identifier)
    storage_options["table_identifier"] = str_identifier

    # implement get_table and get location
    return dataset(s3_uri, version, asof, block_size, commit_lock, index_cache_size, storage_options, default_scan_options)