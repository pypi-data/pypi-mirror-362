...
class LanceTableSink(LanceDatasink):
    """LAS catalog tableSink, inherited from LanceDataSink

    Write a Ray dataset to lance table.
    ...
    ----------
    identifier : str
        The identifierof the table,usually represented by db_name.tbl_name.
    schema : pyarrow.Schema
        The schema of the dataset.
    mode : str, optional
        The write mode. Default is 'append'.
    ...
    def __init__(
        ...
        # sub class process identifier to uri
        uri = _process_catalog_register(identifier, storage_options, schema)
...
def _process_catalog_register(identifier, storage_options, schema) -> str:
    ...
    output_uri = Utils.rename_scheme(uri)
    return output_uri
