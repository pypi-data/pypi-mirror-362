...
def tag_bucket(uri, storage_options) -> None:
    ...
    try:
        tag_enabled = (
                ...
                and str(uri).startswith(('s3', 's3a', 'tos'))
        )
        if not tag_enabled:
        ...
        if storage_options is None:
            ...
            # here we use aws compatible credentials because we use s3 bucket to access tos
        ak = storage_options.get("access_key_id")
        sk = storage_options.get("secret_access_key")
        ...
        if not can_tag:
            logging.debug("The ak, sk or session_token is none, will not init tag manager.")
            return
        ...
        logging.warning(f"skip tagging bucket due to warning : {str(e)}")
...
class BucketTagAction(Service):
    ...
    def get_service_info(region: str) -> ServiceInfo:
        ...
        if service_info:
            logging.debug(f"The service name is : {service_info.credentials.service}")
            return service_info
...
class BucketTagMgr:
    ...
    def add_bucket_tag(self, bucket: str) -> None:
        ...
        logging.debug(f"Need to tag buckets : {need_tag_buckets}")
