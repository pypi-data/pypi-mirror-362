...
from .catalog.utils import Utils,TableIdentifier
from .catalog.catalog import CatalogFactory
...
class LanceDataset(pa.dataset.Dataset):
    ...
    def __init__(
        ...
        tag_bucket(self._uri, storage_options)
    ...
    def alter_columns(self, *alterations: Iterable[AlterColumn]):
        ...
        if self._storage_options is not None and "table_identifier" in self._storage_options:
            catalog = CatalogFactory.create(self._storage_options)
            catalog.alter_table_columns(self._storage_options["table_identifier"], list(alterations))
    ...
    def add_columns(
        ...
        """
        original_schema = self._ds.schema
        ...
        new_schema = self._ds.schema
        added_fields = [
            field for field in new_schema
            if field.name not in original_schema.names
        ]

        if self._storage_options is not None and "table_identifier" in self._storage_options:
            catalog = CatalogFactory.create(self._storage_options)
            catalog.add_table_columns(self._storage_options["table_identifier"], added_fields)
    ...
    def drop_columns(self, columns: List[str]):
        ...
        if self._storage_options is not None and "table_identifier" in self._storage_options:
            catalog = CatalogFactory.create(self._storage_options)
            catalog.drop_table_columns(self._storage_options["table_identifier"], columns)
    ...
    def commit(
        ...
        3  4  d
        """
        tag_bucket(base_uri, storage_options)
    ...
    @staticmethod
    def drop_table(
        identifier: TableIdentifier, storage_options: Optional[Dict[str, str]] = None
    ) -> None:
        catalog  = CatalogFactory.create(storage_options)
        uri = catalog.convert_table_to_dataset_uri(identifier)
        s3_uri = Utils.rename_scheme(uri)
        _Dataset.drop(s3_uri, storage_options)
...
def write_table(
    ...
    catalog = CatalogFactory.create(storage_options)
    if schema is None:
        schema = Utils.get_schema_from_source(data_obj)
    if not catalog.table_exists(identifier):
        catalog.register_lance_table(identifier, schema)
    else:
        if mode == "create":
            mode = "append"
            logging.warning(
                "Table already exists, will append to existing table. "
                "If you want to create a new table, use mode='create'."
            )
    ...
    return write_dataset(
        ...
        progress=progress,
        storage_options=storage_options,
        data_storage_version=data_storage_version,  # type: ignore
...
def write_dataset(
    ...
    *,
    ...
    """
    tag_bucket(uri, storage_options=storage_options)
