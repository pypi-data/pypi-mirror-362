...
def discover_actions(
    ...
    try:
        entry_points = list(pkg_resources.iter_entry_points(entry_point_group))
        if not entry_points:
            logger.info(f"No entry points found in group '{entry_point_group}'")
            return registered_actions

        for entry_point in entry_points:
            try:
                logger.info(f"Loading actions from {entry_point.name}")

                # Import and validate the target
                target = import_target(entry_point.module_name)
                ...
                if inspect.ismodule(target):
                    ...
                    if any(actions for actions in registered.values()):
                        ...
                        logger.info(f"Successfully loaded actions from {entry_point.name}: {registered}")
                    else:
                        logger.info(f"No actions found in module: {entry_point.module_name}")
                else:
                    registry.register(entry_point.name, target)
                    registered_actions[entry_point.name] = ["direct"]
                    logger.info(f"Successfully loaded direct action from {entry_point.name}")
