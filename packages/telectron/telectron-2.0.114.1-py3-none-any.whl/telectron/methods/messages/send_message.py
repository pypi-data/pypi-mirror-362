...
import telectron
from telectron import raw, utils, enums
...
class SendMessage:
    async def send_message(
        ...
        for _ in range(8):
            try:
                result = await self.send_message_inner(
                    chat_id=chat_id,
                    text=text,
            ...
            except telectron.errors.exceptions.bad_request_400.PeerFlood:
                # Хз что за ошибка, телеграм начал так отвечать в последнее время рандомно
                continue
            if not result.outgoing:
                continue
            if isinstance(chat_id, int) and result.chat.id != chat_id:
                continue
            break
        return result
    ...
    async def send_message_inner(
        ...
        Example:
            .. code-block:: python
                ...
                # Simple example
                await app.send_message("me", "Message sent with **telectron**!")

                # Disable web page previews
                await app.send_message("me", "https://docs.telectron.org",
