...
import telectron
from telectron.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~telectron.Client.start`, :meth:`~telectron.idle` and :meth:`~telectron.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that telectron's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`telectron.compose`.
