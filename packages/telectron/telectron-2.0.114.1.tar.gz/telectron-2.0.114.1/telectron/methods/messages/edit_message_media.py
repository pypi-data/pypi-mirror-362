...
class EditMessageMedia:
    async def edit_message_media(
        ...
        if isinstance(media, types.InputMediaPhoto):
            if not isinstance(media.media, str) or os.path.isfile(media.media):
            ...
            if not isinstance(media.media, str) or os.path.isfile(media.media):
                uploaded_media = await self.invoke(
                    raw.functions.messages.UploadMedia(
                        ...
                        media=raw.types.InputMediaUploadedDocument(
                            mime_type=(self.guess_mime_type(file_name) if file_name else self.guess_mime_type(media.media)) or "video/mp4",
                ...
                media = utils.get_input_media_from_file_id(media.media, FileType.VIDEO, spoiler=media.has_spoiler)
            ...
            if not isinstance(media.media, str) or os.path.isfile(media.media):
                media = await self.invoke(
                    raw.functions.messages.UploadMedia(
                        peer=await self.resolve_peer(chat_id),
                        media=raw.types.InputMediaUploadedDocument(
                            mime_type=(self.guess_mime_type(file_name) if file_name else self.guess_mime_type(media.media)) or "audio/mpeg",
                            thumb=await self.save_file(media.thumb),
                            file=await self.save_file(media.media),
                            attributes=[
            ...
            if not isinstance(media.media, str) or os.path.isfile(media.media):
                ...
                media = utils.get_input_media_from_file_id(media.media, FileType.ANIMATION, spoiler=media.has_spoiler)
            ...
            if not isinstance(media.media, str) or os.path.isfile(media.media):
                media = await self.invoke(
                    raw.functions.messages.UploadMedia(
                        peer=await self.resolve_peer(chat_id),
                        media=raw.types.InputMediaUploadedDocument(
                            mime_type=(self.guess_mime_type(file_name) if file_name else self.guess_mime_type(media.media)) or "application/zip",
                            thumb=await self.save_file(media.thumb),
                            file=await self.save_file(media.media),
                            attributes=[
                                raw.types.DocumentAttributeFilename(
