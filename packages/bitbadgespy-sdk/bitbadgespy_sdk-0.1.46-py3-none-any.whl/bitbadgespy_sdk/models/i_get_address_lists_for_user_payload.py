...
class IGetAddressListsForUserPayload(BaseModel):
    """
    IGetAddressListsForUserPayload
    """ # noqa: E501
    bookmark: Optional[StrictStr] = None
    oldest_first: Optional[StrictBool] = Field(default=None, alias="oldestFirst")
    view_type: Optional[StrictStr] = Field(default=None, description="The view type to search for. Default is 'all' - 'all' will return all address lists the user is a member of (both on whitelist or on blacklist) - 'created' will return all address lists the user has created - 'whitelists' will return all address lists the user is on the whitelist of - 'blacklists' will return all address lists the user is on the blacklist of", alias="viewType")
    __properties: ClassVar[List[str]] = ["bookmark", "oldestFirst", "viewType"]
    ...
    def view_type_validate_enum(cls, value):
        ...
        if value not in set(['all', 'created', 'whitelists', 'blacklists']):
            raise ValueError("must be one of enum values ('all', 'created', 'whitelists', 'blacklists')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetAddressListsForUserPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetAddressListsForUserPayload from a dict"""
        if obj is None:
            return None
