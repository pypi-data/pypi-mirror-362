...
class IStatusDoc(BaseModel):
    ...
    block: ILatestBlockStatus = Field(description="The latest synced block status (i.e. height, txIndex, timestamp)")
    next_collection_id: NumberType = Field(description="The next collection ID to be used", alias="nextCollectionId")
    gas_price: Union[StrictFloat, StrictInt] = Field(description="The current gas price based on the average of the lastXGasAmounts", alias="gasPrice")
    last_x_gas_amounts: List[NumberType] = Field(description="The last X gas prices (internal use only)", alias="lastXGasAmounts")
    last_x_gas_limits: List[NumberType] = Field(description="The last X gas limits (internal use only)", alias="lastXGasLimits")
    __properties: ClassVar[List[str]] = ["_docId", "_id", "block", "nextCollectionId", "gasPrice", "lastXGasAmounts", "lastXGasLimits"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IStatusDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of next_collection_id
        if self.next_collection_id:
            _dict['nextCollectionId'] = self.next_collection_id.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in last_x_gas_amounts (list)
        _items = []
        if self.last_x_gas_amounts:
            for _item_last_x_gas_amounts in self.last_x_gas_amounts:
                if _item_last_x_gas_amounts:
                    _items.append(_item_last_x_gas_amounts.to_dict())
            _dict['lastXGasAmounts'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in last_x_gas_limits (list)
        _items = []
        if self.last_x_gas_limits:
            for _item_last_x_gas_limits in self.last_x_gas_limits:
                if _item_last_x_gas_limits:
                    _items.append(_item_last_x_gas_limits.to_dict())
            _dict['lastXGasLimits'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IStatusDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "_id": obj.get("_id"),
            "block": ILatestBlockStatus.from_dict(obj["block"]) if obj.get("block") is not None else None,
            "nextCollectionId": NumberType.from_dict(obj["nextCollectionId"]) if obj.get("nextCollectionId") is not None else None,
            "gasPrice": obj.get("gasPrice"),
            "lastXGasAmounts": [NumberType.from_dict(_item) for _item in obj["lastXGasAmounts"]] if obj.get("lastXGasAmounts") is not None else None,
            "lastXGasLimits": [NumberType.from_dict(_item) for _item in obj["lastXGasLimits"]] if obj.get("lastXGasLimits") is not None else None
        })
        return _obj
