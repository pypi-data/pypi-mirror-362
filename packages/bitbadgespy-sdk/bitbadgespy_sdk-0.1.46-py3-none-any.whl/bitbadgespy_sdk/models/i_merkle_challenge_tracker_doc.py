...
class IMerkleChallengeTrackerDoc(BaseModel):
    """
    ...
    challenge_tracker_id: StrictStr = Field(description="The challenge ID", alias="challengeTrackerId")
    approval_id: StrictStr = Field(description="The approval ID", alias="approvalId")
    approval_level: StrictStr = Field(description="The challenge level (i.e. \"collection\", \"incoming\", \"outgoing\")", alias="approvalLevel")
    ...
    used_leaf_indices: List[IUsedLeafStatus] = Field(description="The used leaf indices for each challenge. A leaf index is the leaf location in the bottommost layer of the Merkle tree", alias="usedLeafIndices")
    __properties: ClassVar[List[str]] = ["_docId", "_id", "collectionId", "challengeTrackerId", "approvalId", "approvalLevel", "approverAddress", "usedLeafIndices"]

    @field_validator('approval_level')
    def approval_level_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['collection', 'incoming', 'outgoing', '']):
            raise ValueError("must be one of enum values ('collection', 'incoming', 'outgoing', '')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMerkleChallengeTrackerDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in used_leaf_indices (list)
        _items = []
        if self.used_leaf_indices:
            for _item_used_leaf_indices in self.used_leaf_indices:
                if _item_used_leaf_indices:
                    _items.append(_item_used_leaf_indices.to_dict())
            _dict['usedLeafIndices'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMerkleChallengeTrackerDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "usedLeafIndices": [IUsedLeafStatus.from_dict(_item) for _item in obj["usedLeafIndices"]] if obj.get("usedLeafIndices") is not None else None
        })
        return _obj
