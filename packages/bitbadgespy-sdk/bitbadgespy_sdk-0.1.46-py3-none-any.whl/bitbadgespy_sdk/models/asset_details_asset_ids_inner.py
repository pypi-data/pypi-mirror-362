...
ASSETDETAILSASSETIDSINNER_ANY_OF_SCHEMAS = ["IUintRange", "str"]

class AssetDetailsAssetIdsInner(BaseModel):
    """
    ...
    # data type: IUintRange
    anyof_schema_2_validator: Optional[IUintRange] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[IUintRange, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "IUintRange", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = AssetDetailsAssetIdsInner.model_construct()
        error_messages = []
        ...
        # validate data type: IUintRange
        if not isinstance(v, IUintRange):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IUintRange`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in AssetDetailsAssetIdsInner with anyOf schemas: IUintRange, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[IUintRange] = None
        try:
            instance.actual_instance = IUintRange.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AssetDetailsAssetIdsInner with anyOf schemas: IUintRange, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], IUintRange, str]]:
