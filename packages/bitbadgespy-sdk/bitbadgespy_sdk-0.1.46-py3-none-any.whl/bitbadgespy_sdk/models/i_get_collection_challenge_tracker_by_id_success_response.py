...
class IGetCollectionChallengeTrackerByIdSuccessResponse(BaseModel):
    """
    IGetCollectionChallengeTrackerByIdSuccessResponse
    """ # noqa: E501
    challenge_tracker: IApprovalTrackerDoc = Field(alias="challengeTracker")
    __properties: ClassVar[List[str]] = ["challengeTracker"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetCollectionChallengeTrackerByIdSuccessResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of challenge_tracker
        if self.challenge_tracker:
            _dict['challengeTracker'] = self.challenge_tracker.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetCollectionChallengeTrackerByIdSuccessResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "challengeTracker": IApprovalTrackerDoc.from_dict(obj["challengeTracker"]) if obj.get("challengeTracker") is not None else None
        })
        return _obj
