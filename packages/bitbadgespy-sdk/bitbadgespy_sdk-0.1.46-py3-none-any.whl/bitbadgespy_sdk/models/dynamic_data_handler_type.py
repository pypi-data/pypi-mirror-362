...
DYNAMICDATAHANDLERTYPE_ANY_OF_SCHEMAS = ["OauthAppName", "str"]

class DynamicDataHandlerType(BaseModel):
    """
    DynamicDataHandlerType
    """

    # data type: OauthAppName
    anyof_schema_1_validator: Optional[OauthAppName] = None
    # data type: str
    anyof_schema_2_validator: Optional[StrictStr] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[OauthAppName, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "OauthAppName", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = DynamicDataHandlerType.model_construct()
        error_messages = []
        # validate data type: OauthAppName
        if not isinstance(v, OauthAppName):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OauthAppName`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in DynamicDataHandlerType with anyOf schemas: OauthAppName, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[OauthAppName] = None
        try:
            instance.actual_instance = OauthAppName.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DynamicDataHandlerType with anyOf schemas: OauthAppName, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], OauthAppName, str]]:
