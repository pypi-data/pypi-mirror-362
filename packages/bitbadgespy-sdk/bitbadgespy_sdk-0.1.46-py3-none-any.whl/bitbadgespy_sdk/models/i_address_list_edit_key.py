...
class IAddressListEditKey(BaseModel):
    """
    IAddressListEditKey
    """ # noqa: E501
    key: StrictStr = Field(description="The key that can be used to edit the address list")
    expiration_date: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="expirationDate")
    must_sign_in: Optional[StrictBool] = Field(default=None, description="True if the user can only add their signed in address to the list", alias="mustSignIn")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["key", "expirationDate", "mustSignIn"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IAddressListEditKey from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IAddressListEditKey from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "key": obj.get("key"),
            "expirationDate": NumberType.from_dict(obj["expirationDate"]) if obj.get("expirationDate") is not None else None,
            "mustSignIn": obj.get("mustSignIn")
        })
