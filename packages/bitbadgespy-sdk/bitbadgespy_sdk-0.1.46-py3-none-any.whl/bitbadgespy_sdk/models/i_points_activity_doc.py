...
class IPointsActivityDoc(BaseModel):
    """
    ...
    old_points: NumberType = Field(description="The amount of points before the activity", alias="oldPoints")
    new_points: NumberType = Field(description="The amount of points after the activity", alias="newPoints")
    application_id: StrictStr = Field(description="The application ID of the points activity", alias="applicationId")
    page_id: StrictStr = Field(description="The page ID of the points activity", alias="pageId")
    __properties: ClassVar[List[str]] = ["_docId", "_id", "timestamp", "block", "_notificationsHandled", "private", "bitbadgesAddress", "oldPoints", "newPoints", "applicationId", "pageId"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IPointsActivityDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of old_points
        if self.old_points:
            _dict['oldPoints'] = self.old_points.to_dict()
        # override the default output from pydantic by calling `to_dict()` of new_points
        if self.new_points:
            _dict['newPoints'] = self.new_points.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IPointsActivityDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "oldPoints": NumberType.from_dict(obj["oldPoints"]) if obj.get("oldPoints") is not None else None,
            "newPoints": NumberType.from_dict(obj["newPoints"]) if obj.get("newPoints") is not None else None,
