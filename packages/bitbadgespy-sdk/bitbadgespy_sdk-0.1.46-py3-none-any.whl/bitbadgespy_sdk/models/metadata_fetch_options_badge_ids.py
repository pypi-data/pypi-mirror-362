...
METADATAFETCHOPTIONSBADGEIDS_ANY_OF_SCHEMAS = ["List[IUintRange]", "List[float]", "str"]

class MetadataFetchOptionsBadgeIds(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = MetadataFetchOptionsBadgeIds.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in MetadataFetchOptionsBadgeIds with anyOf schemas: List[IUintRange], List[float], str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MetadataFetchOptionsBadgeIds with anyOf schemas: List[IUintRange], List[float], str. Details: " + ", ".join(error_messages))
        else:
            return instance
