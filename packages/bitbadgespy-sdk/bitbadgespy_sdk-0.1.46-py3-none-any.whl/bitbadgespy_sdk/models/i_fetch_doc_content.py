...
IFETCHDOCCONTENT_ANY_OF_SCHEMAS = ["Dict[str, List[IBalance]]", "IApprovalInfoDetails", "IChallengeDetails", "IMetadata"]

class IFetchDocContent(BaseModel):
    """
    The content of the fetch document. Note that we store balances in BALANCES_DB and not here to avoid double storage.
    """

    # data type: IMetadata
    anyof_schema_1_validator: Optional[IMetadata] = None
    # data type: IApprovalInfoDetails
    anyof_schema_2_validator: Optional[IApprovalInfoDetails] = None
    # data type: Dict[str, List[IBalance]]
    anyof_schema_3_validator: Optional[Dict[str, List[IBalance]]] = None
    # data type: IChallengeDetails
    anyof_schema_4_validator: Optional[IChallengeDetails] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Dict[str, List[IBalance]], IApprovalInfoDetails, IChallengeDetails, IMetadata]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Dict[str, List[IBalance]]", "IApprovalInfoDetails", "IChallengeDetails", "IMetadata" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = IFetchDocContent.model_construct()
        error_messages = []
        ...
        # validate data type: IApprovalInfoDetails
        if not isinstance(v, IApprovalInfoDetails):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IApprovalInfoDetails`")
        else:
            return v

        # validate data type: Dict[str, List[IBalance]]
        try:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in IFetchDocContent with anyOf schemas: Dict[str, List[IBalance]], IApprovalInfoDetails, IChallengeDetails, IMetadata. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[IApprovalInfoDetails] = None
        try:
            instance.actual_instance = IApprovalInfoDetails.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # deserialize data into Dict[str, List[IBalance]]
        try:
            # validation
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into IFetchDocContent with anyOf schemas: Dict[str, List[IBalance]], IApprovalInfoDetails, IChallengeDetails, IMetadata. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Dict[str, List[IBalance]], IApprovalInfoDetails, IChallengeDetails, IMetadata]]:
