...
PARSEDQSVALUE_ANY_OF_SCHEMAS = ["Dict[str, ParsedQsValue]", "List[Dict[str, ParsedQsValue]]", "List[str]", "str"]

class ParsedQsValue(BaseModel):
    """
    ParsedQsValue
    """
    ...
    # data type: Dict[str, ParsedQsValue]
    anyof_schema_3_validator: Optional[Dict[str, ParsedQsValue]] = None
    # data type: List[Dict[str, ParsedQsValue]]
    anyof_schema_4_validator: Optional[List[Dict[str, ParsedQsValue]]] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[Dict[str, ParsedQsValue], List[Dict[str, ParsedQsValue]], List[str], str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "Dict[str, ParsedQsValue]", "List[Dict[str, ParsedQsValue]]", "List[str]", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ParsedQsValue.model_construct()
        error_messages = []
        ...
        try:
            ...
            error_messages.append(str(e))
        # validate data type: Dict[str, ParsedQsValue]
        try:
            instance.anyof_schema_3_validator = v
            return v
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # validate data type: List[Dict[str, ParsedQsValue]]
        try:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ParsedQsValue with anyOf schemas: Dict[str, ParsedQsValue], List[Dict[str, ParsedQsValue]], List[str], str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into Dict[str, ParsedQsValue]
        try:
            # validation
            ...
            error_messages.append(str(e))
        # deserialize data into List[Dict[str, ParsedQsValue]]
        try:
            # validation
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ParsedQsValue with anyOf schemas: Dict[str, ParsedQsValue], List[Dict[str, ParsedQsValue]], List[str], str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], Dict[str, ParsedQsValue], List[Dict[str, ParsedQsValue]], List[str], str]]:
...
ParsedQsValue.model_rebuild(raise_errors=False)
