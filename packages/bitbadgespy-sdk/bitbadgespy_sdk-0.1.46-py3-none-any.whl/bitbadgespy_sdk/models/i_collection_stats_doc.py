...
class ICollectionStatsDoc(BaseModel):
    """
    ...
    floor_prices: Optional[List[ICosmosCoin]] = Field(default=None, description="Floor price of the collection", alias="floorPrices")
    unique_owners: List[IBalance] = Field(description="Number of unique owners by time", alias="uniqueOwners")
    floor_price_history: Optional[List[IFloorPriceHistory]] = Field(default=None, description="Floor price history", alias="floorPriceHistory")
    payout_rewards: Optional[List[ICosmosCoin]] = Field(default=None, description="The payout reward", alias="payoutRewards")
    __properties: ClassVar[List[str]] = ["_docId", "_id", "overallVolume", "dailyVolume", "weeklyVolume", "monthlyVolume", "yearlyVolume", "lastUpdatedAt", "collectionId", "floorPrices", "uniqueOwners", "floorPriceHistory", "payoutRewards"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ICollectionStatsDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in floor_prices (list)
        _items = []
        if self.floor_prices:
            for _item_floor_prices in self.floor_prices:
                if _item_floor_prices:
                    _items.append(_item_floor_prices.to_dict())
            _dict['floorPrices'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in unique_owners (list)
        _items = []
        if self.unique_owners:
            for _item_unique_owners in self.unique_owners:
                if _item_unique_owners:
                    _items.append(_item_unique_owners.to_dict())
            _dict['uniqueOwners'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in floor_price_history (list)
        _items = []
        if self.floor_price_history:
            for _item_floor_price_history in self.floor_price_history:
                if _item_floor_price_history:
                    _items.append(_item_floor_price_history.to_dict())
            _dict['floorPriceHistory'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in payout_rewards (list)
        _items = []
        if self.payout_rewards:
            for _item_payout_rewards in self.payout_rewards:
                if _item_payout_rewards:
                    _items.append(_item_payout_rewards.to_dict())
            _dict['payoutRewards'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ICollectionStatsDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "floorPrices": [ICosmosCoin.from_dict(_item) for _item in obj["floorPrices"]] if obj.get("floorPrices") is not None else None,
            "uniqueOwners": [IBalance.from_dict(_item) for _item in obj["uniqueOwners"]] if obj.get("uniqueOwners") is not None else None,
            "floorPriceHistory": [IFloorPriceHistory.from_dict(_item) for _item in obj["floorPriceHistory"]] if obj.get("floorPriceHistory") is not None else None,
            "payoutRewards": [ICosmosCoin.from_dict(_item) for _item in obj["payoutRewards"]] if obj.get("payoutRewards") is not None else None
        })
        return _obj
