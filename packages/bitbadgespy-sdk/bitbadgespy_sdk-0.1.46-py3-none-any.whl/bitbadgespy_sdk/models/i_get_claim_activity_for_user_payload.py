...
class IGetClaimActivityForUserPayload(BaseModel):
    """
    ...
    view_type: Optional[StrictStr] = Field(default=None, description="The view type to search for. Default is 'public'  - 'all' will return all claim activity even private (must have permission to view private activity) - 'public' will only return public claim activity", alias="viewType")
    ...
    def view_type_validate_enum(cls, value):
        ...
        if value not in set(['all', 'public']):
            raise ValueError("must be one of enum values ('all', 'public')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetClaimActivityForUserPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetClaimActivityForUserPayload from a dict"""
        if obj is None:
            return None
