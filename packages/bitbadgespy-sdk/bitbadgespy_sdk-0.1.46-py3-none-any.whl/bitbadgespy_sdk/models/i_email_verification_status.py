...
class IEmailVerificationStatus(BaseModel):
    """
    The verification status of the user's email.
    """ # noqa: E501
    verified: Optional[StrictBool] = Field(default=None, description="Whether or not the email has been verified.")
    verified_at: Optional[NumberType] = Field(default=None, description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="verifiedAt")
    token: Optional[StrictStr] = Field(default=None, description="The email verification token. This is used for verification and unsubscription.")
    expiry: Optional[NumberType] = Field(default=None, description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.")
    anti_phishing_code: Optional[StrictStr] = Field(default=None, description="A unique code that we will send with all emails to verify that BitBadges is the one sending the email.", alias="antiPhishingCode")
    __properties: ClassVar[List[str]] = ["verified", "verifiedAt", "token", "expiry", "antiPhishingCode"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IEmailVerificationStatus from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of verified_at
        if self.verified_at:
            _dict['verifiedAt'] = self.verified_at.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IEmailVerificationStatus from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "verifiedAt": NumberType.from_dict(obj["verifiedAt"]) if obj.get("verifiedAt") is not None else None,
            "token": obj.get("token"),
            "expiry": NumberType.from_dict(obj["expiry"]) if obj.get("expiry") is not None else None,
            "antiPhishingCode": obj.get("antiPhishingCode")
        })
        return _obj
