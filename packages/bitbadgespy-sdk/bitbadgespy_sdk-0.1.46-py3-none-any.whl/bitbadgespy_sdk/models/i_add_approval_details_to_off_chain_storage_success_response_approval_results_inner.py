...
class IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInner(BaseModel):
    """
    IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInner
    """ # noqa: E501
    metadata_result: IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInnerMetadataResult = Field(alias="metadataResult")
    challenge_results: Optional[List[IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInnerChallengeResultsInner]] = Field(default=None, description="The result for the approval challenge details (if applicable).", alias="challengeResults")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["metadataResult", "challengeResults"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInner from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of metadata_result
        if self.metadata_result:
            _dict['metadataResult'] = self.metadata_result.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in challenge_results (list)
        _items = []
        if self.challenge_results:
            for _item_challenge_results in self.challenge_results:
                if _item_challenge_results:
                    _items.append(_item_challenge_results.to_dict())
            _dict['challengeResults'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInner from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "metadataResult": IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInnerMetadataResult.from_dict(obj["metadataResult"]) if obj.get("metadataResult") is not None else None,
            "challengeResults": [IAddApprovalDetailsToOffChainStorageSuccessResponseApprovalResultsInnerChallengeResultsInner.from_dict(_item) for _item in obj["challengeResults"]] if obj.get("challengeResults") is not None else None
        })
