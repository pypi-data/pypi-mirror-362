...
class IUtilityListingDocApprovalStatus(BaseModel):
    """
    Approval status - can be used for moderation
    """ # noqa: E501
    is_approved: StrictBool = Field(description="Whether the listing is approved", alias="isApproved")
    is_featured: Optional[StrictBool] = Field(default=None, description="Is  Featured", alias="isFeatured")
    featured_priority: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Featured Priority", alias="featuredPriority")
    rejected: Optional[StrictBool] = Field(default=None, description="Rejected or just pending")
    reason: Optional[StrictStr] = Field(default=None, description="Optional reason if not approved")
    updated_by: Optional[StrictStr] = Field(default=None, description="All supported addresses map to a Bech32 BitBadges address which is used by the BitBadges blockchain behind the scenes. For conversion, see the BitBadges documentation. If this type is used, we must always convert to a BitBadges address before using it.", alias="updatedBy")
    __properties: ClassVar[List[str]] = ["isApproved", "isFeatured", "featuredPriority", "rejected", "reason", "updatedBy"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUtilityListingDocApprovalStatus from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUtilityListingDocApprovalStatus from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "featuredPriority": obj.get("featuredPriority"),
