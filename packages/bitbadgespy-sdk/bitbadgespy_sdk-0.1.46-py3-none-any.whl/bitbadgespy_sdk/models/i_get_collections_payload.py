...
class IGetCollectionsPayload(BaseModel):
    """
    IGetCollectionsPayload
    """ # noqa: E501
    collections_to_fetch: List[GetCollectionRequestBody] = Field(alias="collectionsToFetch")
    __properties: ClassVar[List[str]] = ["collectionsToFetch"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetCollectionsPayload from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in collections_to_fetch (list)
        _items = []
        if self.collections_to_fetch:
            for _item_collections_to_fetch in self.collections_to_fetch:
                if _item_collections_to_fetch:
                    _items.append(_item_collections_to_fetch.to_dict())
            _dict['collectionsToFetch'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetCollectionsPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "collectionsToFetch": [GetCollectionRequestBody.from_dict(_item) for _item in obj["collectionsToFetch"]] if obj.get("collectionsToFetch") is not None else None
        })
        return _obj
