...
class IGetAccountsPayload(BaseModel):
    """
    IGetAccountsPayload
    """ # noqa: E501
    accounts_to_fetch: List[AccountFetchDetails] = Field(alias="accountsToFetch")
    __properties: ClassVar[List[str]] = ["accountsToFetch"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetAccountsPayload from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in accounts_to_fetch (list)
        _items = []
        if self.accounts_to_fetch:
            for _item_accounts_to_fetch in self.accounts_to_fetch:
                if _item_accounts_to_fetch:
                    _items.append(_item_accounts_to_fetch.to_dict())
            _dict['accountsToFetch'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetAccountsPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "accountsToFetch": [AccountFetchDetails.from_dict(_item) for _item in obj["accountsToFetch"]] if obj.get("accountsToFetch") is not None else None
        })
        return _obj
