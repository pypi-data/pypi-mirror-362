...
class IGetDynamicDataStoreValuesPaginatedPayload(BaseModel):
    """
    IGetDynamicDataStoreValuesPaginatedPayload
    """ # noqa: E501
    data_secret: Optional[StrictStr] = Field(default=None, description="The data secret to fetch. Only needed if you are not signed in as creator.", alias="dataSecret")
    bookmark: Optional[StrictStr] = Field(default=None, description="The pagination bookmark to start from")
    lookup_type: Optional[StrictStr] = Field(default=None, description="The lookup type to fetch (if you need to specify).", alias="lookupType")
    __properties: ClassVar[List[str]] = ["dataSecret", "bookmark", "lookupType"]

    @field_validator('lookup_type')
    def lookup_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['id', 'username']):
            raise ValueError("must be one of enum values ('id', 'username')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetDynamicDataStoreValuesPaginatedPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetDynamicDataStoreValuesPaginatedPayload from a dict"""
        if obj is None:
            return None
