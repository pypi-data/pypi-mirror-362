...
class GetAdditionalCollectionDetailsPayloadViewsToFetchInner(BaseModel):
    """
    GetAdditionalCollectionDetailsPayloadViewsToFetchInner
    """ # noqa: E501
    view_type: CollectionViewKey = Field(description="The base view type to fetch.", alias="viewType")
    view_id: StrictStr = Field(description="A unique view ID. This is used for pagination. All fetches w/ same ID should be made with same criteria.", alias="viewId")
    bookmark: StrictStr = Field(description="A bookmark to pass in for pagination. \"\" for first request.")
    oldest_first: Optional[StrictBool] = Field(default=None, description="If defined, we will return the oldest items first.", alias="oldestFirst")
    address: Optional[StrictStr] = Field(default=None, description="If specified, we will only fetch this users' activity.")
    badge_id: Optional[NumberType] = Field(default=None, description="IF specified, we will filter to this abdge ID (only applicable to utiity listings view currently)", alias="badgeId")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["viewType", "viewId", "bookmark", "oldestFirst", "address", "badgeId"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetAdditionalCollectionDetailsPayloadViewsToFetchInner from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetAdditionalCollectionDetailsPayloadViewsToFetchInner from a dict"""
        if obj is None:
            return None
