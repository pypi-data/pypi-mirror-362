...
class IQueueDoc(BaseModel):
    ...
    uri: StrictStr = Field(description="The URI of the metadata to be fetched. If {id} is present, it will be replaced with each individual ID in badgeIds")
    collection_id: StrictStr = Field(alias="collectionId")
    load_balance_id: NumberType = Field(description="The load balance ID of the metadata to be fetched. Only the node with the same load balance ID will fetch this metadata", alias="loadBalanceId")
    refresh_request_time: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="refreshRequestTime")
    num_retries: NumberType = Field(description="The number of times this metadata has been tried to be fetched but failed", alias="numRetries")
    last_fetched_at: Optional[NumberType] = Field(default=None, description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="lastFetchedAt")
    error: Optional[StrictStr] = Field(default=None, description="The error message if this metadata failed to be fetched")
    ...
    next_fetch_time: Optional[NumberType] = Field(default=None, description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="nextFetchTime")
    pending: Optional[StrictBool] = Field(default=None, description="Whether this document is pending to be fetched or not")
    email_message: Optional[StrictStr] = Field(default=None, alias="emailMessage")
    recipient_address: Optional[StrictStr] = Field(default=None, alias="recipientAddress")
    activity_doc_id: Optional[StrictStr] = Field(default=None, alias="activityDocId")
    notification_type: Optional[StrictStr] = Field(default=None, description="Type of the doc / purpose", alias="notificationType")
    initiated_by: Optional[StrictStr] = Field(default=None, description="All supported addresses map to a Bech32 BitBadges address which is used by the BitBadges blockchain behind the scenes. For conversion, see the BitBadges documentation. If this type is used, we must always convert to a BitBadges address before using it.", alias="initiatedBy")
    action_config: Optional[Any] = Field(default=None, description="For use for post-claim actions", alias="actionConfig")
    claim_info: Optional[IQueueDocClaimInfo] = Field(default=None, alias="claimInfo")
    faucet_info: Optional[IQueueDocFaucetInfo] = Field(default=None, alias="faucetInfo")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["_docId", "_id", "uri", "collectionId", "loadBalanceId", "refreshRequestTime", "numRetries", "lastFetchedAt", "error", "deletedAt", "nextFetchTime", "pending", "emailMessage", "recipientAddress", "activityDocId", "notificationType", "initiatedBy", "actionConfig", "claimInfo", "faucetInfo"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IQueueDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of load_balance_id
        if self.load_balance_id:
            _dict['loadBalanceId'] = self.load_balance_id.to_dict()
        # override the default output from pydantic by calling `to_dict()` of refresh_request_time
        if self.refresh_request_time:
            _dict['refreshRequestTime'] = self.refresh_request_time.to_dict()
        # override the default output from pydantic by calling `to_dict()` of num_retries
        if self.num_retries:
            _dict['numRetries'] = self.num_retries.to_dict()
        # override the default output from pydantic by calling `to_dict()` of last_fetched_at
        if self.last_fetched_at:
            _dict['lastFetchedAt'] = self.last_fetched_at.to_dict()
        # override the default output from pydantic by calling `to_dict()` of deleted_at
        if self.deleted_at:
            _dict['deletedAt'] = self.deleted_at.to_dict()
        # override the default output from pydantic by calling `to_dict()` of next_fetch_time
        if self.next_fetch_time:
            _dict['nextFetchTime'] = self.next_fetch_time.to_dict()
        # override the default output from pydantic by calling `to_dict()` of claim_info
        if self.claim_info:
            _dict['claimInfo'] = self.claim_info.to_dict()
        # override the default output from pydantic by calling `to_dict()` of faucet_info
        if self.faucet_info:
            _dict['faucetInfo'] = self.faucet_info.to_dict()
        ...
        if self.action_config is None and "action_config" in self.model_fields_set:
            _dict['actionConfig'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IQueueDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "loadBalanceId": NumberType.from_dict(obj["loadBalanceId"]) if obj.get("loadBalanceId") is not None else None,
            "refreshRequestTime": NumberType.from_dict(obj["refreshRequestTime"]) if obj.get("refreshRequestTime") is not None else None,
            "numRetries": NumberType.from_dict(obj["numRetries"]) if obj.get("numRetries") is not None else None,
            "lastFetchedAt": NumberType.from_dict(obj["lastFetchedAt"]) if obj.get("lastFetchedAt") is not None else None,
            "error": obj.get("error"),
            "deletedAt": NumberType.from_dict(obj["deletedAt"]) if obj.get("deletedAt") is not None else None,
            "nextFetchTime": NumberType.from_dict(obj["nextFetchTime"]) if obj.get("nextFetchTime") is not None else None,
            "pending": obj.get("pending"),
            "emailMessage": obj.get("emailMessage"),
            "recipientAddress": obj.get("recipientAddress"),
            "activityDocId": obj.get("activityDocId"),
            "notificationType": obj.get("notificationType"),
            "initiatedBy": obj.get("initiatedBy"),
            "actionConfig": obj.get("actionConfig"),
            "claimInfo": IQueueDocClaimInfo.from_dict(obj["claimInfo"]) if obj.get("claimInfo") is not None else None,
            "faucetInfo": IQueueDocFaucetInfo.from_dict(obj["faucetInfo"]) if obj.get("faucetInfo") is not None else None
        })
