...
class AccountFetchDetailsViewsToFetchInner(BaseModel):
    """
    AccountFetchDetailsViewsToFetchInner
    """ # noqa: E501
    view_id: StrictStr = Field(description="Unique view ID. Used for pagination. All fetches w/ same ID should be made with same criteria.", alias="viewId")
    view_type: AccountViewKey = Field(description="The base view type to fetch.", alias="viewType")
    specific_collections: Optional[List[IBatchBadgeDetails]] = Field(default=None, description="If defined, we will filter the view to only include the specified collections.", alias="specificCollections")
    specific_lists: Optional[List[StrictStr]] = Field(default=None, description="If defined, we will filter the view to only include the specified lists.", alias="specificLists")
    oldest_first: Optional[StrictBool] = Field(default=None, description="Oldest first. By default, we fetch newest", alias="oldestFirst")
    bookmark: StrictStr = Field(description="A bookmark to pass in for pagination. \"\" for first request.")
    __properties: ClassVar[List[str]] = ["viewId", "viewType", "specificCollections", "specificLists", "oldestFirst", "bookmark"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AccountFetchDetailsViewsToFetchInner from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in specific_collections (list)
        _items = []
        if self.specific_collections:
            for _item_specific_collections in self.specific_collections:
                if _item_specific_collections:
                    _items.append(_item_specific_collections.to_dict())
            _dict['specificCollections'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AccountFetchDetailsViewsToFetchInner from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "specificCollections": [IBatchBadgeDetails.from_dict(_item) for _item in obj["specificCollections"]] if obj.get("specificCollections") is not None else None,
            "specificLists": obj.get("specificLists"),
            "oldestFirst": obj.get("oldestFirst"),
