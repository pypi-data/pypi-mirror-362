...
class ICreateClaimPayload(BaseModel):
    """
    ICreateClaimPayload
    """ # noqa: E501
    claims: List[CreateClaimRequest] = Field(description="The claims to create.  By default, it will create standalone (non-test claims) or list / collection linked claims if the corresponding fields are specified in the claim (listId, collectionId, ...).  Note that collection / list linked claims require the proper permissions and have special setup required.  For test claims, you must specify the `testClaims` field to be true.")
    test_claims: Optional[StrictBool] = Field(default=None, description="Create test claims (e.g. the claim tester). Used for frontend testing. Test claims are auto-deleted after the browser session is terminated and do not show up in search results.", alias="testClaims")
    __properties: ClassVar[List[str]] = ["claims", "testClaims"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ICreateClaimPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ICreateClaimPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "claims": [CreateClaimRequest.from_dict(_item) for _item in obj["claims"]] if obj.get("claims") is not None else None,
            "testClaims": obj.get("testClaims")
        })
        return _obj
