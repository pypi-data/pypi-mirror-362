...
class IUpdateUtilityListingPayload(BaseModel):
    """
    IUpdateUtilityListingPayload
    """ # noqa: E501
    listing_id: StrictStr = Field(description="The listing ID to update", alias="listingId")
    metadata: IMetadataWithoutInternals = Field(description="The overall metadata for the listing")
    content: List[IUtilityListingContent] = Field(description="The content for the listing. This is only used for a dedicated listing page (not compatible with direct link or inherited metadata).")
    links: List[IUtilityListingLink] = Field(description="The links for the listing. This is only used for a dedicated listing page (not compatible with direct link or inherited metadata).")
    ...
    direct_link: Optional[StrictStr] = Field(default=None, description="The direct link for the listing. If specified, we will skip the entire content / listing page. Thus, content and links should be empty [].  This is incompatible with inherited metadata.", alias="directLink")
    ...
    inherit_metadata_from: Optional[IInheritMetadataFrom] = Field(default=None, description="Where to inherit metadata from? Only one can be specified.  If specified, we automatically override the metadata from what is specified and automatically set a direct link to the page.  Ex: Inherit claim metadata and direct link to the claim page.", alias="inheritMetadataFrom")
    locale: Optional[StrictStr] = Field(default=None, description="Locale (ex: es, fr, etc.). If not specified, we assume \"en\" (English).")
    ...
    __properties: ClassVar[List[str]] = ["listingId", "metadata", "content", "links", "visibility", "displayTimes", "directLink", "categories", "linkedTo", "inheritMetadataFrom", "locale", "estimatedCost", "estimatedTime"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUpdateUtilityListingPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUpdateUtilityListingPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "content": [IUtilityListingContent.from_dict(_item) for _item in obj["content"]] if obj.get("content") is not None else None,
            "links": [IUtilityListingLink.from_dict(_item) for _item in obj["links"]] if obj.get("links") is not None else None,
