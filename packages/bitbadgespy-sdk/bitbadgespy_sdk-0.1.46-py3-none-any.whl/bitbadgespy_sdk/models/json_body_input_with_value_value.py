...
JSONBODYINPUTWITHVALUEVALUE_ANY_OF_SCHEMAS = ["bool", "float", "str"]

class JsonBodyInputWithValueValue(BaseModel):
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = JsonBodyInputWithValueValue.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in JsonBodyInputWithValueValue with anyOf schemas: bool, float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into JsonBodyInputWithValueValue with anyOf schemas: bool, float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
