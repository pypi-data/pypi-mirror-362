...
class IUtilityListingDocHomePageView(BaseModel):
    """
    Home page view
    """ # noqa: E501
    type: StrictStr
    category: StrictStr
    __properties: ClassVar[List[str]] = ["type", "category"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['badges', 'lists', 'claims', 'applications']):
            raise ValueError("must be one of enum values ('badges', 'lists', 'claims', 'applications')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUtilityListingDocHomePageView from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUtilityListingDocHomePageView from a dict"""
        if obj is None:
            return None
