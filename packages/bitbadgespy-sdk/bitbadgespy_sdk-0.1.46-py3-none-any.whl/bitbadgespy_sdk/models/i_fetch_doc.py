...
class IFetchDoc(BaseModel):
    ...
    content: Optional[IFetchDocContent] = None
    fetched_at: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="fetchedAt")
    fetched_at_block: NumberType = Field(description="The block the document was fetched", alias="fetchedAtBlock")
    db: StrictStr = Field(description="The type of content fetched. This is used for querying purposes")
    is_permanent: StrictBool = Field(description="True if the document is permanent (i.e. fetched from a permanent URI like IPFS)", alias="isPermanent")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["_docId", "_id", "content", "fetchedAt", "fetchedAtBlock", "db", "isPermanent"]

    @field_validator('db')
    def db_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ApprovalInfo', 'Metadata', 'Balances', 'ChallengeInfo']):
            raise ValueError("must be one of enum values ('ApprovalInfo', 'Metadata', 'Balances', 'ChallengeInfo')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IFetchDoc from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IFetchDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "_id": obj.get("_id"),
            "content": IFetchDocContent.from_dict(obj["content"]) if obj.get("content") is not None else None,
