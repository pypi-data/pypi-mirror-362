...
class OwnershipRequirementsOptions(BaseModel):
    """
    OwnershipRequirementsOptions
    """ # noqa: E501
    num_matches_for_verification: Optional[NumberType] = Field(default=None, alias="numMatchesForVerification")
    __properties: ClassVar[List[str]] = ["numMatchesForVerification"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OwnershipRequirementsOptions from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of num_matches_for_verification
        if self.num_matches_for_verification:
            _dict['numMatchesForVerification'] = self.num_matches_for_verification.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OwnershipRequirementsOptions from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "numMatchesForVerification": NumberType.from_dict(obj["numMatchesForVerification"]) if obj.get("numMatchesForVerification") is not None else None
        })
        return _obj
