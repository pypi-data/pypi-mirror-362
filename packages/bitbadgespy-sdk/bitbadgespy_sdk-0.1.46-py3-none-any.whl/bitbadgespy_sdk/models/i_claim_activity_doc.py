...
class IClaimActivityDoc(BaseModel):
    """
    ...
    success: StrictBool = Field(description="Whether the claim attempt was successful or not")
    claim_id: StrictStr = Field(description="The claim ID of the claim attempt", alias="claimId")
    claim_attempt_id: StrictStr = Field(description="The claim attempt ID of the claim attempt", alias="claimAttemptId")
    ...
    claim_type: Optional[StrictStr] = Field(default=None, description="The claim type of the claim attempt", alias="claimType")
    __properties: ClassVar[List[str]] = ["_docId", "_id", "timestamp", "block", "_notificationsHandled", "private", "success", "claimId", "claimAttemptId", "bitbadgesAddress", "claimType"]

    @field_validator('claim_type')
    def claim_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['standalone', 'collection', 'list']):
            raise ValueError("must be one of enum values ('standalone', 'collection', 'list')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IClaimActivityDoc from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IClaimActivityDoc from a dict"""
        if obj is None:
            return None
