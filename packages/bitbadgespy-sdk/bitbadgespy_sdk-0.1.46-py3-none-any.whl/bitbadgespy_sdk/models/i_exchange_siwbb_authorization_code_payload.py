...
class IExchangeSIWBBAuthorizationCodePayload(BaseModel):
    """
    IExchangeSIWBBAuthorizationCodePayload
    """ # noqa: E501
    code: Optional[StrictStr] = Field(default=None, description="The SIWBB request.")
    options: Optional[VerifySIWBBOptions] = Field(default=None, description="We attempt to verify the current status with each request. You can provide additional options for verification here.")
    client_secret: Optional[StrictStr] = Field(default=None, description="Client secret for the SIWBB request.")
    client_id: Optional[StrictStr] = Field(default=None, description="Client ID for the SIWBB request.")
    redirect_uri: Optional[StrictStr] = Field(default=None, description="The redirect URI for the SIWBB request. Only required if the code was created with a redirect URI.")
    grant_type: Optional[StrictStr] = Field(default=None, description="The grant type for the SIWBB request.")
    refresh_token: Optional[StrictStr] = Field(default=None, description="The refresh token to use for the SIWBB request.")
    code_verifier: Optional[StrictStr] = Field(default=None, description="The code verifier for the SIWBB request (if used with PKCE).")
    __properties: ClassVar[List[str]] = ["code", "options", "client_secret", "client_id", "redirect_uri", "grant_type", "refresh_token", "code_verifier"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IExchangeSIWBBAuthorizationCodePayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IExchangeSIWBBAuthorizationCodePayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "code": obj.get("code"),
            "options": VerifySIWBBOptions.from_dict(obj["options"]) if obj.get("options") is not None else None,
