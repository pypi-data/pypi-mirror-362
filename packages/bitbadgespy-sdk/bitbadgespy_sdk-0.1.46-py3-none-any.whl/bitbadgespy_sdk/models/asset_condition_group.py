...
ASSETCONDITIONGROUP_ANY_OF_SCHEMAS = ["AndGroup", "OrGroup", "OwnershipRequirements"]

class AssetConditionGroup(BaseModel):
    """
    AssetConditionGroup
    """

    # data type: AndGroup
    anyof_schema_1_validator: Optional[AndGroup] = None
    # data type: OrGroup
    anyof_schema_2_validator: Optional[OrGroup] = None
    # data type: OwnershipRequirements
    anyof_schema_3_validator: Optional[OwnershipRequirements] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[AndGroup, OrGroup, OwnershipRequirements]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "AndGroup", "OrGroup", "OwnershipRequirements" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = AssetConditionGroup.model_construct()
        error_messages = []
        # validate data type: AndGroup
        if not isinstance(v, AndGroup):
            error_messages.append(f"Error! Input type `{type(v)}` is not `AndGroup`")
        else:
            return v

        # validate data type: OrGroup
        if not isinstance(v, OrGroup):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OrGroup`")
        else:
            return v

        # validate data type: OwnershipRequirements
        if not isinstance(v, OwnershipRequirements):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OwnershipRequirements`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in AssetConditionGroup with anyOf schemas: AndGroup, OrGroup, OwnershipRequirements. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[AndGroup] = None
        try:
            instance.actual_instance = AndGroup.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[OrGroup] = None
        try:
            instance.actual_instance = OrGroup.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_3_validator: Optional[OwnershipRequirements] = None
        try:
            instance.actual_instance = OwnershipRequirements.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AssetConditionGroup with anyOf schemas: AndGroup, OrGroup, OwnershipRequirements. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], AndGroup, OrGroup, OwnershipRequirements]]:
...
AssetConditionGroup.model_rebuild(raise_errors=False)
