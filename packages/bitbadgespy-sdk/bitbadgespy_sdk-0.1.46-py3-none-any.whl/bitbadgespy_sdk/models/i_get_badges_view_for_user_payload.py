...
class IGetBadgesViewForUserPayload(BaseModel):
    """
    ...
    view_type: Optional[StrictStr] = Field(default=None, description="The view type to search for. Default is 'collected'  - 'collected' will return the badges the user has a balance of - 'managing' will return the badges the user is managing - 'created' will return the badges the user has created", alias="viewType")
    __properties: ClassVar[List[str]] = ["bookmark", "oldestFirst", "collectionId", "viewType"]
    ...
    def view_type_validate_enum(cls, value):
        ...
        if value not in set(['collected', 'managing', 'created']):
            raise ValueError("must be one of enum values ('collected', 'managing', 'created')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetBadgesViewForUserPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetBadgesViewForUserPayload from a dict"""
        if obj is None:
            return None
