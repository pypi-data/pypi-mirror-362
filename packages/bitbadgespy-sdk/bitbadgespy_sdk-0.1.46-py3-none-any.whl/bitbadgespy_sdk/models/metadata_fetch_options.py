...
class MetadataFetchOptions(BaseModel):
    """
    Defines the options for fetching metadata.
    """ # noqa: E501
    do_not_fetch_collection_metadata: Optional[StrictBool] = Field(default=None, description="If true, collection metadata will not be fetched.", alias="doNotFetchCollectionMetadata")
    uris: Optional[List[StrictStr]] = Field(default=None, description="If present, the metadata corresponding to the specified URIs will be fetched.")
    badge_ids: Optional[MetadataFetchOptionsBadgeIds] = Field(default=None, alias="badgeIds")
    __properties: ClassVar[List[str]] = ["doNotFetchCollectionMetadata", "uris", "badgeIds"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MetadataFetchOptions from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of badge_ids
        if self.badge_ids:
            _dict['badgeIds'] = self.badge_ids.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MetadataFetchOptions from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "doNotFetchCollectionMetadata": obj.get("doNotFetchCollectionMetadata"),
            "uris": obj.get("uris"),
            "badgeIds": MetadataFetchOptionsBadgeIds.from_dict(obj["badgeIds"]) if obj.get("badgeIds") is not None else None
        })
        return _obj
