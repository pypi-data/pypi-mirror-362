...
class IGetClaimsPayload(BaseModel):
    """
    IGetClaimsPayload
    """ # noqa: E501
    claim_ids: List[StrictStr] = Field(description="The claim IDs to fetch.", alias="claimIds")
    fetch_private_params: Optional[StrictBool] = Field(default=None, description="Fetch private parameters for the claim. Only applicable if you are the creator / manager of the claim.", alias="fetchPrivateParams")
    private_states_to_fetch: Optional[List[IGetClaimsPayloadPrivateStatesToFetchInner]] = Field(default=None, description="Which private state instance IDs to fetch. claimId and instanceId are required and must match a claimId in claimIds and the claim must have the corresponding instanceId.", alias="privateStatesToFetch")
    fetch_all_claimed_users: Optional[StrictBool] = Field(default=None, description="Fetch all claimed users for the claim. If true, you will be able to find all { [bitbadgesAddress]: [...zeroIndexedClaimNumbers] } on the numUses plugin's publicState.", alias="fetchAllClaimedUsers")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["claimIds", "fetchPrivateParams", "privateStatesToFetch", "fetchAllClaimedUsers"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetClaimsPayload from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in private_states_to_fetch (list)
        _items = []
        if self.private_states_to_fetch:
            for _item_private_states_to_fetch in self.private_states_to_fetch:
                if _item_private_states_to_fetch:
                    _items.append(_item_private_states_to_fetch.to_dict())
            _dict['privateStatesToFetch'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetClaimsPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "claimIds": obj.get("claimIds"),
            "fetchPrivateParams": obj.get("fetchPrivateParams"),
            "privateStatesToFetch": [IGetClaimsPayloadPrivateStatesToFetchInner.from_dict(_item) for _item in obj["privateStatesToFetch"]] if obj.get("privateStatesToFetch") is not None else None,
            "fetchAllClaimedUsers": obj.get("fetchAllClaimedUsers")
        })
