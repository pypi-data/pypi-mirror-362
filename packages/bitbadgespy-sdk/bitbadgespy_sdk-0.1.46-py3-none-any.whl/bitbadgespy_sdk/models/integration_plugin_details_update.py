...
class IntegrationPluginDetailsUpdate(BaseModel):
    """
    IntegrationPluginDetailsUpdate
    """ # noqa: E501
    instance_id: StrictStr = Field(description="The ID of the plugin instance. This is a unique identifier for referencing this instance of the plugin within this claim (e.g. differentiate between duplicates of the same plugin type).  This is different from the pluginId, which is a unique identifier for the plugin itself. All instances of the same plugin will have the same pluginId.", alias="instanceId")
    plugin_id: NumberType = Field(description="The ID of the plugin (e.g. \"numUses\"). This is the reusable plugin ID. Do not use this as a unique identifier for the plugin instance as there could be duplicate pluginIds. Use instanceId instead.", alias="pluginId")
    version: StrictStr = Field(description="The version of the plugin")
    public_params: Optional[Any] = Field(description="The parameters of the plugin that are visible to the public. These are custom per plugin type.", alias="publicParams")
    private_params: Optional[Any] = Field(description="The parameters of the plugin that are not visible to the public. These are custom per plugin type.", alias="privateParams")
    metadata: Optional[IntegrationPluginParamsMetadata] = None
    reset_state: Optional[StrictBool] = Field(default=None, description="If resetState = true, we will reset the state of the plugin back to default. If false, we will keep the current state. Incompatible with newState.", alias="resetState")
    new_state: Optional[Any] = Field(default=None, description="If newState is present, we will set the state to the new state. Incompatible with resetState. Can be used alongside onlyUpdateProvidedNewState. By default, we will overwrite the whole state. If onlyUpdateProvidedNewState is true, we will only update the specific provided fields.  Warning: This is an advanced feature and should be used with caution. Misconfiguring this can lead to unexpected behavior of this plugin.  Note: Each plugin may have different state schemas. Please refer to the documentation of the plugin you are updating for more information.", alias="newState")
    only_update_provided_new_state: Optional[StrictBool] = Field(default=None, description="If true, we will only update the specific fields provided in newState. If falsy, we will overwrite the whole state with newState.  Only applicable if newState is present.  Note that we do this on a recursive level. If you have nested objects, we will only update the specific fields provided for those nested objects and leave all else as-is.", alias="onlyUpdateProvidedNewState")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["instanceId", "pluginId", "version", "publicParams", "privateParams", "metadata", "resetState", "newState", "onlyUpdateProvidedNewState"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IntegrationPluginDetailsUpdate from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of plugin_id
        if self.plugin_id:
            _dict['pluginId'] = self.plugin_id.to_dict()
        ...
        # set to None if public_params (nullable) is None
        # and model_fields_set contains the field
        if self.public_params is None and "public_params" in self.model_fields_set:
            _dict['publicParams'] = None

        # set to None if private_params (nullable) is None
        # and model_fields_set contains the field
        if self.private_params is None and "private_params" in self.model_fields_set:
            _dict['privateParams'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IntegrationPluginDetailsUpdate from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "pluginId": NumberType.from_dict(obj["pluginId"]) if obj.get("pluginId") is not None else None,
            "version": obj.get("version"),
            "publicParams": obj.get("publicParams"),
            "privateParams": obj.get("privateParams"),
            "metadata": IntegrationPluginParamsMetadata.from_dict(obj["metadata"]) if obj.get("metadata") is not None else None,
            "resetState": obj.get("resetState"),
            "newState": obj.get("newState"),
            "onlyUpdateProvidedNewState": obj.get("onlyUpdateProvidedNewState")
        })
