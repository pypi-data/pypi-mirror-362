...
class IUpdateAccountInfoPayloadCustomPages(BaseModel):
    """
    An array of custom pages on the user's portolio. Used to customize, sort, and group badges / lists into pages.
    """ # noqa: E501
    badges: List[ICustomPage]
    lists: List[ICustomListPage]
    attestations: List[ICustomListPage]
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["badges", "lists", "attestations"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUpdateAccountInfoPayloadCustomPages from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUpdateAccountInfoPayloadCustomPages from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "badges": [ICustomPage.from_dict(_item) for _item in obj["badges"]] if obj.get("badges") is not None else None,
            "lists": [ICustomListPage.from_dict(_item) for _item in obj["lists"]] if obj.get("lists") is not None else None,
            "attestations": [ICustomListPage.from_dict(_item) for _item in obj["attestations"]] if obj.get("attestations") is not None else None
        })
