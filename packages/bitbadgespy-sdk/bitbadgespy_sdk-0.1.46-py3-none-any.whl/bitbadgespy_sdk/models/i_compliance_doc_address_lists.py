...
class IComplianceDocAddressLists(BaseModel):
    """
    IComplianceDocAddressLists
    """ # noqa: E501
    nsfw: List[IComplianceDocAddressListsNsfwInner]
    reported: List[IComplianceDocAddressListsNsfwInner]
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["nsfw", "reported"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IComplianceDocAddressLists from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in nsfw (list)
        _items = []
        if self.nsfw:
            for _item_nsfw in self.nsfw:
                if _item_nsfw:
                    _items.append(_item_nsfw.to_dict())
            _dict['nsfw'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in reported (list)
        _items = []
        if self.reported:
            for _item_reported in self.reported:
                if _item_reported:
                    _items.append(_item_reported.to_dict())
            _dict['reported'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IComplianceDocAddressLists from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "nsfw": [IComplianceDocAddressListsNsfwInner.from_dict(_item) for _item in obj["nsfw"]] if obj.get("nsfw") is not None else None,
            "reported": [IComplianceDocAddressListsNsfwInner.from_dict(_item) for _item in obj["reported"]] if obj.get("reported") is not None else None
        })
