...
class IGetStatusSuccessResponse(BaseModel):
    """
    IGetStatusSuccessResponse
    """ # noqa: E501
    status: IStatusDoc = Field(description="Status details about the indexer / blockchain.")
    out_of_sync: Optional[StrictBool] = Field(default=None, description="If true, we are out of sync with the blockchain. If undefined, we did not check for out of sync.", alias="outOfSync")
    __properties: ClassVar[List[str]] = ["status", "outOfSync"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetStatusSuccessResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetStatusSuccessResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "status": IStatusDoc.from_dict(obj["status"]) if obj.get("status") is not None else None,
            "outOfSync": obj.get("outOfSync")
        })
        return _obj
