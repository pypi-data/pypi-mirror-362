...
class IGetBadgeBalanceByAddressPayload(BaseModel):
    """
    IGetBadgeBalanceByAddressPayload
    """ # noqa: E501
    fetch_private_params: Optional[StrictBool] = Field(default=None, description="If true, we will fetch private parameters for any claims / approvals. Must be creator.  This is only applicable to incoming / outgoing approvals with claims.", alias="fetchPrivateParams")
    forceful: Optional[StrictBool] = Field(default=None, description="If true, we will forcefully fetch the balance even if it is already cached. Only applicable to non-indexed / on-demand collections.")
    __properties: ClassVar[List[str]] = ["fetchPrivateParams", "forceful"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetBadgeBalanceByAddressPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetBadgeBalanceByAddressPayload from a dict"""
        if obj is None:
            return None
