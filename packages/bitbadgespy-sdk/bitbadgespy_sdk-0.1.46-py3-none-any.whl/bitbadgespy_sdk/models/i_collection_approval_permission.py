...
class ICollectionApprovalPermission(BaseModel):
    """
    ICollectionApprovalPermission
    """ # noqa: E501
    from_list_id: StrictStr = Field(description="The list ID of the from addresses of the approved transfers.", alias="fromListId")
    from_list: IAddressList = Field(alias="fromList")
    to_list_id: StrictStr = Field(description="The list ID of the to addresses of the approved transfers.", alias="toListId")
    to_list: IAddressList = Field(alias="toList")
    initiated_by_list_id: StrictStr = Field(description="The list ID of the initiatedBy addresses of the approved transfers.", alias="initiatedByListId")
    initiated_by_list: IAddressList = Field(alias="initiatedByList")
    transfer_times: List[IUintRange] = Field(description="The transfer times of the approved transfers.", alias="transferTimes")
    badge_ids: List[IUintRange] = Field(description="The badge IDs of the approved transfers.", alias="badgeIds")
    ownership_times: List[IUintRange] = Field(description="The owned times of the approved transfers.", alias="ownershipTimes")
    approval_id: StrictStr = Field(description="The approval ID of the approved transfers. Can use \"All\" to represent all IDs, \"!approvalId\" to represent all IDs except approvalId, or \"approvalId\" to represent only approvalId.", alias="approvalId")
    permanently_permitted_times: List[IUintRange] = Field(description="The permitted times of this permission.", alias="permanentlyPermittedTimes")
    permanently_forbidden_times: List[IUintRange] = Field(description="The forbidden times of this permission.", alias="permanentlyForbiddenTimes")
    __properties: ClassVar[List[str]] = ["fromListId", "fromList", "toListId", "toList", "initiatedByListId", "initiatedByList", "transferTimes", "badgeIds", "ownershipTimes", "approvalId", "permanentlyPermittedTimes", "permanentlyForbiddenTimes"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ICollectionApprovalPermission from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in permanently_permitted_times (list)
        _items = []
        if self.permanently_permitted_times:
            for _item_permanently_permitted_times in self.permanently_permitted_times:
                if _item_permanently_permitted_times:
                    _items.append(_item_permanently_permitted_times.to_dict())
            _dict['permanentlyPermittedTimes'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in permanently_forbidden_times (list)
        _items = []
        if self.permanently_forbidden_times:
            for _item_permanently_forbidden_times in self.permanently_forbidden_times:
                if _item_permanently_forbidden_times:
                    _items.append(_item_permanently_forbidden_times.to_dict())
            _dict['permanentlyForbiddenTimes'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ICollectionApprovalPermission from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "permanentlyPermittedTimes": [IUintRange.from_dict(_item) for _item in obj["permanentlyPermittedTimes"]] if obj.get("permanentlyPermittedTimes") is not None else None,
            "permanentlyForbiddenTimes": [IUintRange.from_dict(_item) for _item in obj["permanentlyForbiddenTimes"]] if obj.get("permanentlyForbiddenTimes") is not None else None
        })
        return _obj
