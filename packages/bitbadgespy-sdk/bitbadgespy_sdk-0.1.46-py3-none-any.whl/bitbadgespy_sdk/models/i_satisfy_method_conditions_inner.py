...
ISATISFYMETHODCONDITIONSINNER_ANY_OF_SCHEMAS = ["ISatisfyMethod", "str"]

class ISatisfyMethodConditionsInner(BaseModel):
    """
    ...
    # data type: ISatisfyMethod
    anyof_schema_2_validator: Optional[ISatisfyMethod] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ISatisfyMethod, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ISatisfyMethod", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ISatisfyMethodConditionsInner.model_construct()
        error_messages = []
        ...
        # validate data type: ISatisfyMethod
        if not isinstance(v, ISatisfyMethod):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ISatisfyMethod`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ISatisfyMethodConditionsInner with anyOf schemas: ISatisfyMethod, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_2_validator: Optional[ISatisfyMethod] = None
        try:
            instance.actual_instance = ISatisfyMethod.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ISatisfyMethodConditionsInner with anyOf schemas: ISatisfyMethod, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ISatisfyMethod, str]]:
...
ISatisfyMethodConditionsInner.model_rebuild(raise_errors=False)
