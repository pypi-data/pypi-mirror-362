...
class VerifyAttestationSignaturesParameters(BaseModel):
    """
    VerifyAttestationSignaturesParameters
    """ # noqa: E501
    scheme: StrictStr = Field(description="Must be 'bbs' or 'standard'")
    messages: List[StrictStr] = Field(description="The messages to attest to. Must be length 1 for standard")
    message_format: StrictStr = Field(alias="messageFormat")
    data_integrity_proof: Optional[VerifyAttestationSignaturesParametersDataIntegrityProof] = Field(default=None, alias="dataIntegrityProof")
    proof_of_issuance: Optional[VerifyAttestationSignaturesParametersProofOfIssuance] = Field(default=None, alias="proofOfIssuance")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["scheme", "messages", "messageFormat", "dataIntegrityProof", "proofOfIssuance"]

    @field_validator('message_format')
    def message_format_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['json', 'plaintext']):
            raise ValueError("must be one of enum values ('json', 'plaintext')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of VerifyAttestationSignaturesParameters from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of VerifyAttestationSignaturesParameters from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "dataIntegrityProof": VerifyAttestationSignaturesParametersDataIntegrityProof.from_dict(obj["dataIntegrityProof"]) if obj.get("dataIntegrityProof") is not None else None,
            "proofOfIssuance": VerifyAttestationSignaturesParametersProofOfIssuance.from_dict(obj["proofOfIssuance"]) if obj.get("proofOfIssuance") is not None else None
        })
