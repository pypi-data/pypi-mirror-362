...
GETMETADATAFORCOLLECTIONPAYLOADBADGEFLOORPRICESTOFETCH_ANY_OF_SCHEMAS = ["List[IUintRange]", "List[float]", "str"]

class GetMetadataForCollectionPayloadBadgeFloorPricesToFetch(BaseModel):
    """
    ...
    # data type: List[IUintRange]
    anyof_schema_3_validator: Optional[List[IUintRange]] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[List[IUintRange], List[float], str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "List[IUintRange]", "List[float]", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetMetadataForCollectionPayloadBadgeFloorPricesToFetch.model_construct()
        error_messages = []
        ...
        try:
            ...
            error_messages.append(str(e))
        # validate data type: List[IUintRange]
        try:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetMetadataForCollectionPayloadBadgeFloorPricesToFetch with anyOf schemas: List[IUintRange], List[float], str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into List[IUintRange]
        try:
            # validation
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetMetadataForCollectionPayloadBadgeFloorPricesToFetch with anyOf schemas: List[IUintRange], List[float], str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], List[IUintRange], List[float], str]]:
