...
JSONBODYINPUTSCHEMADEFAULTVALUE_ANY_OF_SCHEMAS = ["bool", "float", "str"]

class JsonBodyInputSchemaDefaultValue(BaseModel):
    """
    JsonBodyInputSchemaDefaultValue
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = JsonBodyInputSchemaDefaultValue.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in JsonBodyInputSchemaDefaultValue with anyOf schemas: bool, float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into JsonBodyInputSchemaDefaultValue with anyOf schemas: bool, float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
