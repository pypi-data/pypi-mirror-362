...
class CodeGenQueryParams(BaseModel):
    """
    CodeGenQueryParams
    """ # noqa: E501
    claim_id: Optional[StrictStr] = Field(default=None, description="We will display this claim on the authorize screen. Just for display purpses. This is still to be checked by you post-authentication.", alias="claimId")
    hide_if_already_claimed: Optional[StrictBool] = Field(default=None, description="For the claimId, we will hide the claim if the user has already completed it (successCount >= 1).", alias="hideIfAlreadyClaimed")
    expect_verify_success: Optional[StrictBool] = Field(default=None, description="We will expect the claim verification to succeed. If false, we will not let user attempt to sign in.  Note: This is not a replacement for checking the claim on your side because users can manipulate the client-side URL parameters.", alias="expectVerifySuccess")
    expect_attestations: Optional[StrictBool] = Field(default=None, description="We will expect the user to provide attestations. Consider adding an additional instructions to the user for which ones to present. You still need to check the attestations on your side. @deprecated It is preferred to receive attestations via claims instead.", alias="expectAttestations")
    redirect_uri: Optional[StrictStr] = Field(default=None, description="The redirect URI to redirect to after the user signs in. Must match the one in developer portal.")
    client_id: StrictStr = Field(description="The client ID to use for the SIWBB request. Must match the one in developer portal.")
    state: Optional[StrictStr] = Field(default=None, description="The state to use for the SIWBB request.")
    scope: Optional[StrictStr] = Field(default=None, description="The scopes to request (e.g. `completeClaims,approveSignInWithBitBadges`).")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["claimId", "hideIfAlreadyClaimed", "expectVerifySuccess", "expectAttestations", "redirect_uri", "client_id", "state", "scope"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CodeGenQueryParams from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CodeGenQueryParams from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "hideIfAlreadyClaimed": obj.get("hideIfAlreadyClaimed"),
            "expectVerifySuccess": obj.get("expectVerifySuccess"),
            "expectAttestations": obj.get("expectAttestations"),
