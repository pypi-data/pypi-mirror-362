...
class ICreatePaymentIntentPayload(BaseModel):
    """
    ICreatePaymentIntentPayload
    """ # noqa: E501
    amount: Union[StrictFloat, StrictInt] = Field(description="The amount in USD to pay")
    purpose: StrictStr = Field(description="Purpose of the payment")
    affiliate_code: Optional[StrictStr] = Field(default=None, description="The affiliate code to use for the payment", alias="affiliateCode")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["amount", "purpose", "affiliateCode"]

    @field_validator('purpose')
    def purpose_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['credits', 'deposit']):
            raise ValueError("must be one of enum values ('credits', 'deposit')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ICreatePaymentIntentPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ICreatePaymentIntentPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "affiliateCode": obj.get("affiliateCode")
        })
