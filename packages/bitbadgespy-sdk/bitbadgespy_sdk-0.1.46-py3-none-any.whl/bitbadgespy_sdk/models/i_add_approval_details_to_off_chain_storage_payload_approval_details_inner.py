...
class IAddApprovalDetailsToOffChainStoragePayloadApprovalDetailsInner(BaseModel):
    """
    IAddApprovalDetailsToOffChainStoragePayloadApprovalDetailsInner
    """ # noqa: E501
    name: StrictStr = Field(description="The name of the approval.")
    description: StrictStr = Field(description="The description of the approval.")
    image: StrictStr = Field(description="The image of the approval.")
    challenge_info_details: Optional[List[IChallengeInfoDetailsUpdate]] = Field(default=None, description="For any merkle challenge claims that we are implementing", alias="challengeInfoDetails")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["name", "description", "image", "challengeInfoDetails"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IAddApprovalDetailsToOffChainStoragePayloadApprovalDetailsInner from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in challenge_info_details (list)
        _items = []
        if self.challenge_info_details:
            for _item_challenge_info_details in self.challenge_info_details:
                if _item_challenge_info_details:
                    _items.append(_item_challenge_info_details.to_dict())
            _dict['challengeInfoDetails'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IAddApprovalDetailsToOffChainStoragePayloadApprovalDetailsInner from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "image": obj.get("image"),
            "challengeInfoDetails": [IChallengeInfoDetailsUpdate.from_dict(_item) for _item in obj["challengeInfoDetails"]] if obj.get("challengeInfoDetails") is not None else None
        })
