...
class ISearchPluginsPayload(BaseModel):
    """
    ISearchPluginsPayload
    """ # noqa: E501
    plugins_for_signed_in_user: Optional[StrictBool] = Field(default=None, description="If true, we will fetch all plugins for the authenticated user (with plugin secrets).  This will include plugins created by the signed in user and also those where they are explicitly approved / invited.", alias="pluginsForSignedInUser")
    bookmark: Optional[StrictStr] = Field(default=None, description="Bookmark for pagination of the plugins (obtained from a previous call to this endpoint).")
    search_value: Optional[StrictStr] = Field(default=None, description="Search value", alias="searchValue")
    locale: Optional[StrictStr] = Field(default=None, description="Locale to restrict results to. By default, we assume 'en'. This is not applicable if you specify createdPluginsOnly, speciifc pluginIds, or an invite code.")
    __properties: ClassVar[List[str]] = ["pluginsForSignedInUser", "bookmark", "searchValue", "locale"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ISearchPluginsPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ISearchPluginsPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "pluginsForSignedInUser": obj.get("pluginsForSignedInUser"),
