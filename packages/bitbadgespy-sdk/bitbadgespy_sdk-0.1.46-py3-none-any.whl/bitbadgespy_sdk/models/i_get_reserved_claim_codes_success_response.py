...
class IGetReservedClaimCodesSuccessResponse(BaseModel):
    """
    IGetReservedClaimCodesSuccessResponse
    """ # noqa: E501
    reserved_codes: Optional[List[StrictStr]] = Field(default=None, description="The previously reserved claim codes for the user. These are what are used in the eventual on-chain merkle proof to complete the transaction.", alias="reservedCodes")
    leaf_signatures: Optional[List[StrictStr]] = Field(default=None, description="The leaf signatures for the reserved claim codes to prove address <-> leaf mapping.", alias="leafSignatures")
    __properties: ClassVar[List[str]] = ["reservedCodes", "leafSignatures"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetReservedClaimCodesSuccessResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetReservedClaimCodesSuccessResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "reservedCodes": obj.get("reservedCodes"),
            "leafSignatures": obj.get("leafSignatures")
        })
        return _obj
