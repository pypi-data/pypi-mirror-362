...
class IGetCollectionAmountTrackersSuccessResponse(BaseModel):
    """
    IGetCollectionAmountTrackersSuccessResponse
    """ # noqa: E501
    amount_trackers: List[IApprovalTrackerDoc] = Field(alias="amountTrackers")
    pagination: PaginationInfo
    __properties: ClassVar[List[str]] = ["amountTrackers", "pagination"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetCollectionAmountTrackersSuccessResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in amount_trackers (list)
        _items = []
        if self.amount_trackers:
            for _item_amount_trackers in self.amount_trackers:
                if _item_amount_trackers:
                    _items.append(_item_amount_trackers.to_dict())
            _dict['amountTrackers'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetCollectionAmountTrackersSuccessResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "amountTrackers": [IApprovalTrackerDoc.from_dict(_item) for _item in obj["amountTrackers"]] if obj.get("amountTrackers") is not None else None,
