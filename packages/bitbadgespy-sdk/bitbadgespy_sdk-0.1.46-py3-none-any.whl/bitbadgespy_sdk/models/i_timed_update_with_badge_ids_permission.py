...
class ITimedUpdateWithBadgeIdsPermission(BaseModel):
    """
    ITimedUpdateWithBadgeIdsPermission
    """ # noqa: E501
    timeline_times: List[IUintRange] = Field(description="The timeline times that the permission applies to.", alias="timelineTimes")
    badge_ids: List[IUintRange] = Field(description="The badge IDs that the permission applies to.", alias="badgeIds")
    permanently_permitted_times: List[IUintRange] = Field(description="The permitted times of the permission.", alias="permanentlyPermittedTimes")
    permanently_forbidden_times: List[IUintRange] = Field(description="The forbidden times of the permission.", alias="permanentlyForbiddenTimes")
    __properties: ClassVar[List[str]] = ["timelineTimes", "badgeIds", "permanentlyPermittedTimes", "permanentlyForbiddenTimes"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ITimedUpdateWithBadgeIdsPermission from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in timeline_times (list)
        _items = []
        if self.timeline_times:
            for _item_timeline_times in self.timeline_times:
                if _item_timeline_times:
                    _items.append(_item_timeline_times.to_dict())
            _dict['timelineTimes'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ITimedUpdateWithBadgeIdsPermission from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "timelineTimes": [IUintRange.from_dict(_item) for _item in obj["timelineTimes"]] if obj.get("timelineTimes") is not None else None,
