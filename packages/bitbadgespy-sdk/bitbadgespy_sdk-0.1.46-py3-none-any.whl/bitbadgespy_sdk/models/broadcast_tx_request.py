...
BROADCASTTXREQUEST_ONE_OF_SCHEMAS = ["BroadcastPostBody", "str"]

class BroadcastTxRequest(BaseModel):
    """
    BroadcastTxRequest
    """
    # data type: BroadcastPostBody
    oneof_schema_1_validator: Optional[BroadcastPostBody] = None
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = None
    actual_instance: Optional[Union[BroadcastPostBody, str]] = None
    one_of_schemas: Set[str] = { "BroadcastPostBody", "str" }
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = BroadcastTxRequest.model_construct()
        error_messages = []
        match = 0
        # validate data type: BroadcastPostBody
        if not isinstance(v, BroadcastPostBody):
            error_messages.append(f"Error! Input type `{type(v)}` is not `BroadcastPostBody`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in BroadcastTxRequest with oneOf schemas: BroadcastPostBody, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in BroadcastTxRequest with oneOf schemas: BroadcastPostBody, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into BroadcastPostBody
        try:
            instance.actual_instance = BroadcastPostBody.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BroadcastTxRequest with oneOf schemas: BroadcastPostBody, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BroadcastTxRequest with oneOf schemas: BroadcastPostBody, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], BroadcastPostBody, str]]:
