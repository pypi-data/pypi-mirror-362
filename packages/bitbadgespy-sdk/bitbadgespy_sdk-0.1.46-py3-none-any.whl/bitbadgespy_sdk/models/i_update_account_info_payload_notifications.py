...
class IUpdateAccountInfoPayloadNotifications(BaseModel):
    """
    The notification preferences for the user. Will only be returned if user is authenticated with full access.
    """ # noqa: E501
    email: Optional[StrictStr] = None
    discord: Optional[IUpdateAccountInfoPayloadNotificationsDiscord] = None
    anti_phishing_code: Optional[StrictStr] = Field(default=None, alias="antiPhishingCode")
    preferences: Optional[IUpdateAccountInfoPayloadNotificationsPreferences] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["email", "discord", "antiPhishingCode", "preferences"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUpdateAccountInfoPayloadNotifications from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUpdateAccountInfoPayloadNotifications from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "email": obj.get("email"),
            "discord": IUpdateAccountInfoPayloadNotificationsDiscord.from_dict(obj["discord"]) if obj.get("discord") is not None else None,
            "antiPhishingCode": obj.get("antiPhishingCode"),
            "preferences": IUpdateAccountInfoPayloadNotificationsPreferences.from_dict(obj["preferences"]) if obj.get("preferences") is not None else None
        })
