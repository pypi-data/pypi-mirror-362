...
class VerifyChallengeOptions(BaseModel):
    """
    VerifyChallengeOptions
    """ # noqa: E501
    expected_challenge_params: Optional[VerifyChallengeOptionsExpectedChallengeParams] = Field(default=None, alias="expectedChallengeParams")
    balances_snapshot: Optional[Dict[str, Any]] = Field(default=None, description="For verification of assets, instead of dynamically fetching the assets, you can specify a snapshot of the assets.  This is useful if you have a snapshot, balances will not change, or you are verifying in an offline manner.", alias="balancesSnapshot")
    skip_timestamp_verification: Optional[StrictBool] = Field(default=None, description="If true, we do not check timestamps (expirationDate / notBefore). This is useful if you are verifying a challenge that is expected to be verified at a future time.", alias="skipTimestampVerification")
    skip_asset_verification: Optional[StrictBool] = Field(default=None, description="If true, we do not check asset ownership. This is useful if you are verifying a challenge that is expected to be verified at a future time.", alias="skipAssetVerification")
    earliest_issued_at: Optional[StrictStr] = Field(default=None, description="The earliest issued At ISO date string that is valid. For example, if you want to verify a challenge that was issued within the last minute, you can specify this to be 1 minute ago.", alias="earliestIssuedAt")
    issued_at_time_window_ms: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="If set, we will verify the issuedAt is within this amount of ms ago (i.e. issuedAt >= Date.now() - issuedAtTimeWindowMs)", alias="issuedAtTimeWindowMs")
    skip_signature_verification: Optional[StrictBool] = Field(default=None, description="If true, we do not check the signature. You can pass in an undefined ChainDriver", alias="skipSignatureVerification")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["expectedChallengeParams", "balancesSnapshot", "skipTimestampVerification", "skipAssetVerification", "earliestIssuedAt", "issuedAtTimeWindowMs", "skipSignatureVerification"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of VerifyChallengeOptions from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of expected_challenge_params
        if self.expected_challenge_params:
            _dict['expectedChallengeParams'] = self.expected_challenge_params.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of VerifyChallengeOptions from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "expectedChallengeParams": VerifyChallengeOptionsExpectedChallengeParams.from_dict(obj["expectedChallengeParams"]) if obj.get("expectedChallengeParams") is not None else None,
            "balancesSnapshot": obj.get("balancesSnapshot"),
            "skipTimestampVerification": obj.get("skipTimestampVerification"),
            "skipAssetVerification": obj.get("skipAssetVerification"),
            "earliestIssuedAt": obj.get("earliestIssuedAt"),
            "issuedAtTimeWindowMs": obj.get("issuedAtTimeWindowMs"),
            "skipSignatureVerification": obj.get("skipSignatureVerification")
        })
