...
class IAddBalancesToOffChainStoragePayload(BaseModel):
    """
    IAddBalancesToOffChainStoragePayload
    """ # noqa: E501
    balances: Optional[Dict[str, List[IBalance]]] = None
    claims: Optional[List[IAddBalancesToOffChainStoragePayloadClaimsInner]] = Field(default=None, description="The new set of claims for the collection. This should be ALL claims. We currently do not support fine-grained claim updates.  If undefined, we leave the existing claims as is. If defined, we set the new claims to what is provided.  If a claim has existing state, you can reset the individual plugin's state with plugin.resetState = true. Or, claims with new, unique IDs have blank state for all plugins.  We soft delete any claims that are no longer in the claims array. By soft delete, we mean that we will flag it as deleted, but if you want to reinstate it, you can do so by adding it back with the same claim ID.")
    method: StrictStr = Field(description="The method for storing balances (ipfs or centralized).")
    collection_id: StrictStr = Field(alias="collectionId")
    is_non_indexed: Optional[StrictBool] = Field(default=None, description="Whether this is for a non-indexed collection. Bypasses some validation.", alias="isNonIndexed")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["balances", "claims", "method", "collectionId", "isNonIndexed"]

    @field_validator('method')
    def method_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ipfs', 'centralized']):
            raise ValueError("must be one of enum values ('ipfs', 'centralized')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IAddBalancesToOffChainStoragePayload from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in balances (dict of array)
        _field_dict_of_array = {}
        if self.balances:
            for _key_balances in self.balances:
                if self.balances[_key_balances] is not None:
                    _field_dict_of_array[_key_balances] = [
                        _item.to_dict() for _item in self.balances[_key_balances]
                    ]
            _dict['balances'] = _field_dict_of_array
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IAddBalancesToOffChainStoragePayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "balances": dict(
                (_k,
                        [IBalance.from_dict(_item) for _item in _v]
                        if _v is not None
                        else None
                )
                for _k, _v in obj.get("balances", {}).items()
            ),
            "claims": [IAddBalancesToOffChainStoragePayloadClaimsInner.from_dict(_item) for _item in obj["claims"]] if obj.get("claims") is not None else None,
            "method": obj.get("method"),
            "collectionId": obj.get("collectionId"),
            "isNonIndexed": obj.get("isNonIndexed")
        })
