...
class IGetOwnersForBadgePayload(BaseModel):
    """
    IGetOwnersForBadgePayload
    """ # noqa: E501
    bookmark: Optional[StrictStr] = Field(default=None, description="The pagination bookmark for where to start the request. Bookmarks are obtained via the previous response. \"\" for first request.")
    sort_by: Optional[StrictStr] = Field(default=None, description="Sort by amount descending.", alias="sortBy")
    __properties: ClassVar[List[str]] = ["bookmark", "sortBy"]
    ...
    def sort_by_validate_enum(cls, value):
        ...
        if value not in set(['amount']):
            raise ValueError("must be one of enum values ('amount')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetOwnersForBadgePayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetOwnersForBadgePayload from a dict"""
        if obj is None:
            return None
