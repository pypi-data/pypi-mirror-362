...
class ITierWithOptionalWeight(BaseModel):
    """
    ITierWithOptionalWeight
    """ # noqa: E501
    claim_id: StrictStr = Field(description="The claim ID to satisfy the tier", alias="claimId")
    weight: Optional[NumberType] = Field(default=None, description="The weight of the tier")
    uncheckable: Optional[StrictBool] = Field(default=None, description="Uncheckable? If so, we will not display success or failure for this tier.  We will just display the claim criteria and metadata.")
    points_calculation_method: Optional[StrictStr] = Field(default=None, description="The calculation method to use for this tier. This is used for calculating the tier weight.  By default, we check if the user has met the criteria for non-indexed and for indexed, we check claimed successfully at least one time.", alias="pointsCalculationMethod")
    __properties: ClassVar[List[str]] = ["claimId", "weight", "uncheckable", "pointsCalculationMethod"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ITierWithOptionalWeight from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ITierWithOptionalWeight from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "weight": NumberType.from_dict(obj["weight"]) if obj.get("weight") is not None else None,
            "uncheckable": obj.get("uncheckable"),
            "pointsCalculationMethod": obj.get("pointsCalculationMethod")
        })
        return _obj
