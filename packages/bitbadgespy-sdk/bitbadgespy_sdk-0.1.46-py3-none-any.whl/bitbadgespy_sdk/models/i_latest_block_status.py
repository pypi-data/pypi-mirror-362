...
class ILatestBlockStatus(BaseModel):
    """
    ILatestBlockStatus
    """ # noqa: E501
    height: NumberType = Field(description="The height of the latest block")
    tx_index: NumberType = Field(description="The transaction index of the latest block", alias="txIndex")
    timestamp: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.")
    __properties: ClassVar[List[str]] = ["height", "txIndex", "timestamp"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ILatestBlockStatus from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of tx_index
        if self.tx_index:
            _dict['txIndex'] = self.tx_index.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ILatestBlockStatus from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "height": NumberType.from_dict(obj["height"]) if obj.get("height") is not None else None,
            "txIndex": NumberType.from_dict(obj["txIndex"]) if obj.get("txIndex") is not None else None,
