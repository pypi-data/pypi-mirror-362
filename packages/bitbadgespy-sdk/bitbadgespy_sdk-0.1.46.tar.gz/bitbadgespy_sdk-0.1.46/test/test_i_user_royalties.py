...
class TestIUserRoyalties(unittest.TestCase):
    """IUserRoyalties unit test stubs"""
    ...
    def make_instance(self, include_optional) -> IUserRoyalties:
        """Test IUserRoyalties
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `IUserRoyalties`
        """
        model = IUserRoyalties()
        if include_optional:
            return IUserRoyalties(
                percentage = None,
                payout_address = ''
            )
        else:
