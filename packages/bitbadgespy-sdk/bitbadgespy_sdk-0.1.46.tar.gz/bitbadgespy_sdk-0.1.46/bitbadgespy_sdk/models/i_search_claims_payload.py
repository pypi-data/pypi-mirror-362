...
class ISearchClaimsPayload(BaseModel):
    """
    ISearchClaimsPayload
    """ # noqa: E501
    bookmark: Optional[StrictStr] = Field(default=None, description="Bookmark to start from. Obtained from previous request. Leave blank to start from the beginning. Only applicable when no additional criteria is specified.")
    fetch_private_params: Optional[StrictBool] = Field(default=None, description="Fetch private parameters for the claim. Only applicable if you are the creator / manager of the claim. Otherwise, it will be the public read-only view.", alias="fetchPrivateParams")
    search_value: Optional[StrictStr] = Field(default=None, description="If provided, we will only return claims with names that regex match the search value.", alias="searchValue")
    __properties: ClassVar[List[str]] = ["bookmark", "fetchPrivateParams", "searchValue"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ISearchClaimsPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ISearchClaimsPayload from a dict"""
        if obj is None:
            return None
