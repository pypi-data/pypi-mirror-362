...
class IPredeterminedBalances(BaseModel):
    """
    IPredeterminedBalances
    """ # noqa: E501
    manual_balances: List[IManualBalances] = Field(description="Manually define the balances for each transfer. Cannot be used with incrementedBalances. Order number corresponds to the index of the balance in the array.", alias="manualBalances")
    incremented_balances: IIncrementedBalances = Field(description="Define a starting balance and increment the badge IDs and owned times by a certain amount after each transfer. Cannot be used with manualBalances. Order number corresponds to number of times we increment.", alias="incrementedBalances")
    order_calculation_method: IPredeterminedOrderCalculationMethod = Field(description="The order calculation method.", alias="orderCalculationMethod")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["manualBalances", "incrementedBalances", "orderCalculationMethod"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IPredeterminedBalances from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in manual_balances (list)
        _items = []
        if self.manual_balances:
            for _item_manual_balances in self.manual_balances:
                if _item_manual_balances:
                    _items.append(_item_manual_balances.to_dict())
            _dict['manualBalances'] = _items
        # override the default output from pydantic by calling `to_dict()` of incremented_balances
        if self.incremented_balances:
            _dict['incrementedBalances'] = self.incremented_balances.to_dict()
        # override the default output from pydantic by calling `to_dict()` of order_calculation_method
        if self.order_calculation_method:
            _dict['orderCalculationMethod'] = self.order_calculation_method.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IPredeterminedBalances from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "manualBalances": [IManualBalances.from_dict(_item) for _item in obj["manualBalances"]] if obj.get("manualBalances") is not None else None,
            "incrementedBalances": IIncrementedBalances.from_dict(obj["incrementedBalances"]) if obj.get("incrementedBalances") is not None else None,
            "orderCalculationMethod": IPredeterminedOrderCalculationMethod.from_dict(obj["orderCalculationMethod"]) if obj.get("orderCalculationMethod") is not None else None
        })
