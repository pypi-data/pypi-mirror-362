...
class IGetPostActionStatusesSuccessResponsePostActionStatusesInner(BaseModel):
    """
    IGetPostActionStatusesSuccessResponsePostActionStatusesInner
    """ # noqa: E501
    last_fetched_at: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="lastFetchedAt")
    claim_id: StrictStr = Field(alias="claimId")
    bitbadges_address: StrictStr = Field(alias="bitbadgesAddress")
    plugin_id: StrictStr = Field(alias="pluginId")
    claim_attempt_id: StrictStr = Field(alias="claimAttemptId")
    num_retries: NumberType = Field(alias="numRetries")
    next_fetch_time: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="nextFetchTime")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["lastFetchedAt", "claimId", "bitbadgesAddress", "pluginId", "claimAttemptId", "numRetries", "nextFetchTime"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetPostActionStatusesSuccessResponsePostActionStatusesInner from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetPostActionStatusesSuccessResponsePostActionStatusesInner from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "claimAttemptId": obj.get("claimAttemptId"),
