...
class IChallengeDetails(BaseModel):
    """
    IChallengeDetails
    """ # noqa: E501
    leaves: List[StrictStr] = Field(description="The leaves of the Merkle tree. Leaves should be considered public. Use preimages for the private codes + isHashed. For whitelist trees, these can be the plaintext BitBadges addresses.")
    is_hashed: StrictBool = Field(description="True if the leaves are hashed. Hash(preimage[i]) = leaves[i]", alias="isHashed")
    preimages: Optional[List[StrictStr]] = Field(default=None, description="The preimages of the leaves (only used if isHashed = true). Oftentimes, this is used for private codes so should not be present when user-facing.")
    seed_code: Optional[StrictStr] = Field(default=None, description="Seed code for generating the leaves", alias="seedCode")
    tree: Optional[Any] = Field(default=None, description="The Merkle tree")
    tree_options: Optional[Any] = Field(default=None, description="The Merkle tree options for how to build it", alias="treeOptions")
    num_leaves: Optional[NumberType] = Field(default=None, description="The number of leaves in the Merkle tree. This takes priority over leaves.length if defined (used for buffer time between leaf generation and leaf length select)", alias="numLeaves")
    curr_code: Optional[NumberType] = Field(default=None, description="The current code being used for the challenge. Used behind the scenes", alias="currCode")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["leaves", "isHashed", "preimages", "seedCode", "tree", "treeOptions", "numLeaves", "currCode"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IChallengeDetails from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        if self.num_leaves:
            _dict['numLeaves'] = self.num_leaves.to_dict()
        # override the default output from pydantic by calling `to_dict()` of curr_code
        if self.curr_code:
            _dict['currCode'] = self.curr_code.to_dict()
        ...
        # set to None if tree_options (nullable) is None
        # and model_fields_set contains the field
        if self.tree_options is None and "tree_options" in self.model_fields_set:
            _dict['treeOptions'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IChallengeDetails from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "preimages": obj.get("preimages"),
            "seedCode": obj.get("seedCode"),
            "tree": obj.get("tree"),
            "treeOptions": obj.get("treeOptions"),
            "numLeaves": NumberType.from_dict(obj["numLeaves"]) if obj.get("numLeaves") is not None else None,
            "currCode": NumberType.from_dict(obj["currCode"]) if obj.get("currCode") is not None else None
        })
