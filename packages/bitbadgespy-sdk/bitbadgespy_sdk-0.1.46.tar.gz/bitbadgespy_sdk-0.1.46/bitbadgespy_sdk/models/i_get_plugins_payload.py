...
class IGetPluginsPayload(BaseModel):
    """
    IGetPluginsPayload
    """ # noqa: E501
    plugin_ids: List[StrictStr] = Field(description="If true, we will fetch only the specific plugin with the plugin ID (no secrets).", alias="pluginIds")
    invite_code: Optional[StrictStr] = Field(default=None, description="Invite code to fetch the plugin with.", alias="inviteCode")
    __properties: ClassVar[List[str]] = ["pluginIds", "inviteCode"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetPluginsPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetPluginsPayload from a dict"""
        if obj is None:
            return None
