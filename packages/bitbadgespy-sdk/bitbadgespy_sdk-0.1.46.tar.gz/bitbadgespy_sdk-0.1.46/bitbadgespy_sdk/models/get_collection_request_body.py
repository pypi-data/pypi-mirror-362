...
class GetCollectionRequestBody(BaseModel):
    """
     Defines the options for fetching additional collection details.  A view is a way of fetching additional details about a collection, and these will be queryable in the response via the `views` property. Each view has a bookmark that is used for pagination and must be supplied to get the next page. If the bookmark is not supplied, the first page will be returned.
    """ # noqa: E501
    views_to_fetch: Optional[List[GetCollectionRequestBodyViewsToFetchInner]] = Field(default=None, description="If present, the specified views will be fetched.", alias="viewsToFetch")
    fetch_total_balances: Optional[StrictBool] = Field(default=None, description="If true, the total and mint balances will be fetched and will be put in owners[].  collection.owners.find(x => x.bitbadgesAddresss === 'Mint')", alias="fetchTotalBalances")
    challenge_trackers_to_fetch: Optional[List[IChallengeTrackerIdDetails]] = Field(default=None, description="If present, the merkle challenge trackers corresponding to the specified merkle challenge IDs will be fetched.", alias="challengeTrackersToFetch")
    disable_defaults: Optional[StrictBool] = Field(default=None, description="Disable appending default approvals.", alias="disableDefaults")
    approval_trackers_to_fetch: Optional[List[IAmountTrackerIdDetails]] = Field(default=None, description="If present, the approvals trackers corresponding to the specified approvals tracker IDs will be fetched.", alias="approvalTrackersToFetch")
    fetch_private_params: Optional[StrictBool] = Field(default=None, description="Fetches private parameters for any claims in addition to public parameters.", alias="fetchPrivateParams")
    metadata_to_fetch: Optional[MetadataFetchOptions] = Field(default=None, description="If present, we will fetch the metadata corresponding to the specified options.  Consider using pruneMetadataToFetch for filtering out previously fetched metadata.", alias="metadataToFetch")
    badge_floor_prices_to_fetch: Optional[GetMetadataForCollectionPayloadBadgeFloorPricesToFetch] = Field(default=None, alias="badgeFloorPricesToFetch")
    collection_id: StrictStr = Field(alias="collectionId")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["viewsToFetch", "fetchTotalBalances", "challengeTrackersToFetch", "disableDefaults", "approvalTrackersToFetch", "fetchPrivateParams", "metadataToFetch", "badgeFloorPricesToFetch", "collectionId"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetCollectionRequestBody from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in views_to_fetch (list)
        _items = []
        if self.views_to_fetch:
            for _item_views_to_fetch in self.views_to_fetch:
                if _item_views_to_fetch:
                    _items.append(_item_views_to_fetch.to_dict())
            _dict['viewsToFetch'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in challenge_trackers_to_fetch (list)
        _items = []
        if self.challenge_trackers_to_fetch:
            for _item_challenge_trackers_to_fetch in self.challenge_trackers_to_fetch:
                if _item_challenge_trackers_to_fetch:
                    _items.append(_item_challenge_trackers_to_fetch.to_dict())
            _dict['challengeTrackersToFetch'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in approval_trackers_to_fetch (list)
        _items = []
        if self.approval_trackers_to_fetch:
            for _item_approval_trackers_to_fetch in self.approval_trackers_to_fetch:
                if _item_approval_trackers_to_fetch:
                    _items.append(_item_approval_trackers_to_fetch.to_dict())
            _dict['approvalTrackersToFetch'] = _items
        # override the default output from pydantic by calling `to_dict()` of metadata_to_fetch
        if self.metadata_to_fetch:
            _dict['metadataToFetch'] = self.metadata_to_fetch.to_dict()
        # override the default output from pydantic by calling `to_dict()` of badge_floor_prices_to_fetch
        if self.badge_floor_prices_to_fetch:
            _dict['badgeFloorPricesToFetch'] = self.badge_floor_prices_to_fetch.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetCollectionRequestBody from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "viewsToFetch": [GetCollectionRequestBodyViewsToFetchInner.from_dict(_item) for _item in obj["viewsToFetch"]] if obj.get("viewsToFetch") is not None else None,
            "fetchTotalBalances": obj.get("fetchTotalBalances"),
            "challengeTrackersToFetch": [IChallengeTrackerIdDetails.from_dict(_item) for _item in obj["challengeTrackersToFetch"]] if obj.get("challengeTrackersToFetch") is not None else None,
            "disableDefaults": obj.get("disableDefaults"),
            "approvalTrackersToFetch": [IAmountTrackerIdDetails.from_dict(_item) for _item in obj["approvalTrackersToFetch"]] if obj.get("approvalTrackersToFetch") is not None else None,
            "fetchPrivateParams": obj.get("fetchPrivateParams"),
            "metadataToFetch": MetadataFetchOptions.from_dict(obj["metadataToFetch"]) if obj.get("metadataToFetch") is not None else None,
            "badgeFloorPricesToFetch": GetMetadataForCollectionPayloadBadgeFloorPricesToFetch.from_dict(obj["badgeFloorPricesToFetch"]) if obj.get("badgeFloorPricesToFetch") is not None else None,
