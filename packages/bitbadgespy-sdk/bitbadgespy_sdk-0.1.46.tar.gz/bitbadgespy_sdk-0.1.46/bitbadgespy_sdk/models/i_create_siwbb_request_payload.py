...
class ICreateSIWBBRequestPayload(BaseModel):
    """
    ICreateSIWBBRequestPayload
    """ # noqa: E501
    response_type: StrictStr = Field(description="The response type for the SIWBB request.")
    scopes: List[OAuthScopeDetails] = Field(description="The scopes to request.")
    name: Optional[StrictStr] = Field(default=None, description="The name of the SIWBB request for display purposes.")
    description: Optional[StrictStr] = Field(default=None, description="The description of the SIWBB request for display purposes.")
    image: Optional[StrictStr] = Field(default=None, description="The image of the SIWBB request for display purposes.")
    attestations: Optional[List[IAttestationsProof]] = Field(default=None, description="If required, you can additionally add proof of attestations to the authentication flow. This proves sensitive information (e.g. GPAs, SAT scores, etc.) without revealing the information itself.")
    client_id: StrictStr = Field(description="Client ID for the SIWBB request.")
    redirect_uri: Optional[StrictStr] = Field(default=None, description="Redirect URI if redirected after successful sign-in.")
    state: Optional[StrictStr] = Field(default=None, description="State to be passed back to the redirect URI.")
    code_challenge: Optional[StrictStr] = Field(default=None, description="The code challenge for the SIWBB request.")
    code_challenge_method: Optional[StrictStr] = Field(default=None, description="The code challenge method for the SIWBB request.")
    __properties: ClassVar[List[str]] = ["response_type", "scopes", "name", "description", "image", "attestations", "client_id", "redirect_uri", "state", "code_challenge", "code_challenge_method"]
    ...
    def code_challenge_method_validate_enum(cls, value):
        ...
        if value not in set(['S256', 'plain']):
            raise ValueError("must be one of enum values ('S256', 'plain')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ICreateSIWBBRequestPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ICreateSIWBBRequestPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "scopes": [OAuthScopeDetails.from_dict(_item) for _item in obj["scopes"]] if obj.get("scopes") is not None else None,
            "name": obj.get("name"),
