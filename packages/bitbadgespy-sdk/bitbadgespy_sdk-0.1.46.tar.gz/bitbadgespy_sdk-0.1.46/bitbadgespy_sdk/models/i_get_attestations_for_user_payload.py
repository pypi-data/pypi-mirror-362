...
class IGetAttestationsForUserPayload(BaseModel):
    """
    ...
    view_type: Optional[StrictStr] = Field(default=None, description="The view type to search for. Default is 'all'  - 'all' will return all attestations the user has - 'created' will return all attestations the user has created - 'received' will return all attestations the user has received", alias="viewType")
    ...
    def view_type_validate_enum(cls, value):
        ...
        if value not in set(['all', 'created', 'received']):
            raise ValueError("must be one of enum values ('all', 'created', 'received')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetAttestationsForUserPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetAttestationsForUserPayload from a dict"""
        if obj is None:
            return None
