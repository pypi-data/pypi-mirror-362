...
class AccountFetchDetails(BaseModel):
    """
    This defines the options for fetching additional account details.  A view is a way of fetching additional details about an account, and these will be queryable in the response via the `views` property.  Each view has a bookmark that is used for pagination and must be supplied to get the next page.
    """ # noqa: E501
    address: Optional[StrictStr] = Field(default=None, description="A native address is an address that is native to the user's chain. For example, an Ethereum address is native to Ethereum (0x...). If this type is used, we support any native address type. We do not require conversion to a BitBadges address like the BitBadgesAddress type.")
    username: Optional[StrictStr] = Field(default=None, description="The username of the user. Only one of address or username should be specified.")
    partial_profile: Optional[StrictBool] = Field(default=None, description="If true, we will only fetch a partial set of the document for the user.  Currently includes: solAddress, username, profile pic, and latest signed in chain  Pretty much, anything you need to display the address but not the full profile", alias="partialProfile")
    views_to_fetch: Optional[List[AccountFetchDetailsViewsToFetchInner]] = Field(default=None, description="An array of views to fetch", alias="viewsToFetch")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["address", "username", "partialProfile", "viewsToFetch"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AccountFetchDetails from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AccountFetchDetails from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "partialProfile": obj.get("partialProfile"),
            "viewsToFetch": [AccountFetchDetailsViewsToFetchInner.from_dict(_item) for _item in obj["viewsToFetch"]] if obj.get("viewsToFetch") is not None else None
        })
