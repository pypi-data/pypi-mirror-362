...
class ICompleteClaimPayload(BaseModel):
    """
    ICompleteClaimPayload
    """ # noqa: E501
    expected_version: Union[StrictFloat, StrictInt] = Field(description="Needs to be provided so we check that no plugins or claims have been updated since the claim was fetched. To override, set to -1.", alias="_expectedVersion")
    specific_instance_ids: Optional[List[StrictStr]] = Field(default=None, description="If provided, we will only complete the claim for the specific plugins w/ the provided instance IDs. Must be compatible with the satisfaction logic.", alias="_specificInstanceIds")
    __properties: ClassVar[List[str]] = ["_expectedVersion", "_specificInstanceIds"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ICompleteClaimPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ICompleteClaimPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "_expectedVersion": obj.get("_expectedVersion"),
            "_specificInstanceIds": obj.get("_specificInstanceIds")
        })
        return _obj
