...
class ErrorResponse(BaseModel):
    """
    If an error occurs, the response will be an ErrorResponse.  400 - Bad Request (e.g. invalid request body) 401 - Unauthorized 500 - Internal Server Error
    """ # noqa: E501
    error: Optional[StrictStr] = Field(default=None, description="Serialized error object for debugging purposes. Technical users can use this to debug issues.")
    error_message: StrictStr = Field(description="UX-friendly error message that can be displayed to the user. Always present if error.", alias="errorMessage")
    unauthorized: Optional[StrictBool] = Field(default=None, description="Authentication error. Present if the user is not authenticated.")
    __properties: ClassVar[List[str]] = ["error", "errorMessage", "unauthorized"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "unauthorized": obj.get("unauthorized")
        })
        return _obj
