...
class IGetClaimAttemptsSuccessResponseDocsInner(BaseModel):
    """
    IGetClaimAttemptsSuccessResponseDocsInner
    """ # noqa: E501
    success: StrictBool
    attempted_at: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="attemptedAt")
    claim_id: StrictStr = Field(alias="claimId")
    bitbadges_address: StrictStr = Field(description="A native address is an address that is native to the user's chain. For example, an Ethereum address is native to Ethereum (0x...). If this type is used, we support any native address type. We do not require conversion to a BitBadges address like the BitBadgesAddress type.", alias="bitbadgesAddress")
    claim_attempt_id: StrictStr = Field(alias="claimAttemptId")
    claim_number: Union[StrictFloat, StrictInt] = Field(description="Zero-based index claim number", alias="claimNumber")
    error: Optional[StrictStr] = None
    attempt_data: Optional[Dict[str, Any]] = Field(default=None, description="This is in the format of { [instanceId: string]: Record<string, any> } where the object is what was configured via the plugin.", alias="attemptData")
    __properties: ClassVar[List[str]] = ["success", "attemptedAt", "claimId", "bitbadgesAddress", "claimAttemptId", "claimNumber", "error", "attemptData"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetClaimAttemptsSuccessResponseDocsInner from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of attempted_at
        if self.attempted_at:
            _dict['attemptedAt'] = self.attempted_at.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetClaimAttemptsSuccessResponseDocsInner from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "attemptedAt": NumberType.from_dict(obj["attemptedAt"]) if obj.get("attemptedAt") is not None else None,
            "claimId": obj.get("claimId"),
            "bitbadgesAddress": obj.get("bitbadgesAddress"),
            "claimAttemptId": obj.get("claimAttemptId"),
            "claimNumber": obj.get("claimNumber"),
            "error": obj.get("error"),
            "attemptData": obj.get("attemptData")
        })
        return _obj
