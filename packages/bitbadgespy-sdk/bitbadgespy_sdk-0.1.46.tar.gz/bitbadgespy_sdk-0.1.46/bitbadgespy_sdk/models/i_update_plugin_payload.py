...
class IUpdatePluginPayload(BaseModel):
    """
    IUpdatePluginPayload
    """ # noqa: E501
    plugin_id: StrictStr = Field(description="The unique plugin ID", alias="pluginId")
    invite_code: Optional[StrictStr] = Field(default=None, description="Invite code for the plugin", alias="inviteCode")
    remove_self_from_approved_users: Optional[StrictBool] = Field(default=None, description="Remove self from approved users?", alias="removeSelfFromApprovedUsers")
    metadata: Optional[IUpdatePluginPayloadMetadata] = None
    to_publish: Optional[StrictBool] = Field(default=None, description="To publish in the directory. This will trigger the start of the review process.", alias="toPublish")
    approved_users: Optional[List[StrictStr]] = Field(default=None, description="The addresses that are allowed to use this plugin.", alias="approvedUsers")
    rotate_plugin_secret: Optional[StrictBool] = Field(default=None, description="Rotate the plugin secret?", alias="rotatePluginSecret")
    version_updates: Optional[List[IUpdatePluginPayloadVersionUpdatesInner]] = Field(default=None, description="Update an existing version", alias="versionUpdates")
    version_create: Optional[PluginVersionConfigPayload] = Field(default=None, description="Create a new version", alias="versionCreate")
    locale: Optional[StrictStr] = Field(default=None, description="Locale that is supported by the plugin. By default, we assume 'en' is supported if not specified.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["pluginId", "inviteCode", "removeSelfFromApprovedUsers", "metadata", "toPublish", "approvedUsers", "rotatePluginSecret", "versionUpdates", "versionCreate", "locale"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUpdatePluginPayload from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in version_updates (list)
        _items = []
        if self.version_updates:
            for _item_version_updates in self.version_updates:
                if _item_version_updates:
                    _items.append(_item_version_updates.to_dict())
            _dict['versionUpdates'] = _items
        # override the default output from pydantic by calling `to_dict()` of version_create
        if self.version_create:
            _dict['versionCreate'] = self.version_create.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUpdatePluginPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "removeSelfFromApprovedUsers": obj.get("removeSelfFromApprovedUsers"),
            "metadata": IUpdatePluginPayloadMetadata.from_dict(obj["metadata"]) if obj.get("metadata") is not None else None,
            "toPublish": obj.get("toPublish"),
            "approvedUsers": obj.get("approvedUsers"),
            "rotatePluginSecret": obj.get("rotatePluginSecret"),
            "versionUpdates": [IUpdatePluginPayloadVersionUpdatesInner.from_dict(_item) for _item in obj["versionUpdates"]] if obj.get("versionUpdates") is not None else None,
            "versionCreate": PluginVersionConfigPayload.from_dict(obj["versionCreate"]) if obj.get("versionCreate") is not None else None,
