...
class IClaimReward(BaseModel):
    """
    IClaimReward
    """ # noqa: E501
    reward_id: StrictStr = Field(description="The ID of the reward (either a pre-configured one or \"custom\"). Currently, this is not used for anything.", alias="rewardId")
    instance_id: StrictStr = Field(description="The instance ID of the reward. A unique identifier for the reward.", alias="instanceId")
    metadata: Optional[IClaimRewardMetadata] = None
    automatic: Optional[StrictBool] = Field(default=None, description="If true, the reward is automatically given to the user upon completion. No in-site logic is required.")
    gated_content: IClaimGatedContent = Field(description="The gated content to display upon completion.", alias="gatedContent")
    calculation_method: Optional[IClaimRewardCalculationMethod] = Field(default=None, alias="calculationMethod")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["rewardId", "instanceId", "metadata", "automatic", "gatedContent", "calculationMethod"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IClaimReward from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of gated_content
        if self.gated_content:
            _dict['gatedContent'] = self.gated_content.to_dict()
        # override the default output from pydantic by calling `to_dict()` of calculation_method
        if self.calculation_method:
            _dict['calculationMethod'] = self.calculation_method.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IClaimReward from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "metadata": IClaimRewardMetadata.from_dict(obj["metadata"]) if obj.get("metadata") is not None else None,
            "automatic": obj.get("automatic"),
            "gatedContent": IClaimGatedContent.from_dict(obj["gatedContent"]) if obj.get("gatedContent") is not None else None,
            "calculationMethod": IClaimRewardCalculationMethod.from_dict(obj["calculationMethod"]) if obj.get("calculationMethod") is not None else None
        })
