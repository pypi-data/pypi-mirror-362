...
class AssetDetails(BaseModel):
    ...
    """ # noqa: E501
    chain: StrictStr
    collection_id: NumberType = Field(alias="collectionId")
    asset_ids: List[AssetDetailsAssetIdsInner] = Field(alias="assetIds")
    ownership_times: List[IUintRange] = Field(alias="ownershipTimes")
    must_own_amounts: IUintRange = Field(alias="mustOwnAmounts")
    additional_criteria: Optional[StrictStr] = Field(default=None, alias="additionalCriteria")
    __properties: ClassVar[List[str]] = ["chain", "collectionId", "assetIds", "ownershipTimes", "mustOwnAmounts", "additionalCriteria"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.collection_id:
            _dict['collectionId'] = self.collection_id.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in asset_ids (list)
        _items = []
        if self.asset_ids:
            for _item_asset_ids in self.asset_ids:
                if _item_asset_ids:
                    _items.append(_item_asset_ids.to_dict())
            _dict['assetIds'] = _items
        ...
        # override the default output from pydantic by calling `to_dict()` of must_own_amounts
        if self.must_own_amounts:
            _dict['mustOwnAmounts'] = self.must_own_amounts.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "chain": obj.get("chain"),
            "collectionId": NumberType.from_dict(obj["collectionId"]) if obj.get("collectionId") is not None else None,
            "assetIds": [AssetDetailsAssetIdsInner.from_dict(_item) for _item in obj["assetIds"]] if obj.get("assetIds") is not None else None,
            ...
            "mustOwnAmounts": IUintRange.from_dict(obj["mustOwnAmounts"]) if obj.get("mustOwnAmounts") is not None else None,
            "additionalCriteria": obj.get("additionalCriteria")
        })
        return _obj
