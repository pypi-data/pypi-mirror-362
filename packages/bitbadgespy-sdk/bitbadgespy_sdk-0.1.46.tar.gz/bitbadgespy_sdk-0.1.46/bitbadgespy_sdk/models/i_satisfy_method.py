...
class ISatisfyMethod(BaseModel):
    """
    ISatisfyMethod
    """ # noqa: E501
    type: StrictStr
    conditions: List[ISatisfyMethodConditionsInner] = Field(description="Conditions can either be the instance ID string of the plugin to check success for or another satisfyMethod object.")
    options: Optional[ISatisfyMethodOptions] = None
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["type", "conditions", "options"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ISatisfyMethod from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ISatisfyMethod from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "conditions": [ISatisfyMethodConditionsInner.from_dict(_item) for _item in obj["conditions"]] if obj.get("conditions") is not None else None,
            "options": ISatisfyMethodOptions.from_dict(obj["options"]) if obj.get("options") is not None else None
        })
...
ISatisfyMethod.model_rebuild(raise_errors=False)
