...
class IPointsDoc(BaseModel):
    ...
    address: StrictStr = Field(description="All supported addresses map to a Bech32 BitBadges address which is used by the BitBadges blockchain behind the scenes. For conversion, see the BitBadges documentation. If this type is used, we must always convert to a BitBadges address before using it.")
    points: NumberType = Field(description="The points for the address")
    last_calculated_at: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="lastCalculatedAt")
    application_id: StrictStr = Field(description="The application ID", alias="applicationId")
    page_id: StrictStr = Field(description="The page ID", alias="pageId")
    claim_success_counts: Optional[Dict[str, Union[StrictFloat, StrictInt]]] = Field(default=None, description="Claim success counts. These were the claim success counts calculated for this points calculation.", alias="claimSuccessCounts")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["_docId", "_id", "address", "points", "lastCalculatedAt", "applicationId", "pageId", "claimSuccessCounts"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IPointsDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of last_calculated_at
        if self.last_calculated_at:
            _dict['lastCalculatedAt'] = self.last_calculated_at.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IPointsDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "points": NumberType.from_dict(obj["points"]) if obj.get("points") is not None else None,
            "lastCalculatedAt": NumberType.from_dict(obj["lastCalculatedAt"]) if obj.get("lastCalculatedAt") is not None else None,
            "applicationId": obj.get("applicationId"),
            "pageId": obj.get("pageId"),
            "claimSuccessCounts": obj.get("claimSuccessCounts")
        })
