...
class IMerkleProof(BaseModel):
    """
    IMerkleProof
    """ # noqa: E501
    aunts: List[IMerklePathItem] = Field(description="The aunts of the merkle proof.")
    leaf: StrictStr = Field(description="The leaf of the merkle proof. If useCreatorAddressAsLeaf is true, this will be populated with the creator BitBadges address.")
    leaf_signature: StrictStr = Field(description="The signature for the leaf. With an ETH message signature, sign(leaf + \"-\" + intendedBitBadgesAddress).", alias="leafSignature")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["aunts", "leaf", "leafSignature"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMerkleProof from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in aunts (list)
        _items = []
        if self.aunts:
            for _item_aunts in self.aunts:
                if _item_aunts:
                    _items.append(_item_aunts.to_dict())
            _dict['aunts'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMerkleProof from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "aunts": [IMerklePathItem.from_dict(_item) for _item in obj["aunts"]] if obj.get("aunts") is not None else None,
            "leaf": obj.get("leaf"),
            "leafSignature": obj.get("leafSignature")
        })
