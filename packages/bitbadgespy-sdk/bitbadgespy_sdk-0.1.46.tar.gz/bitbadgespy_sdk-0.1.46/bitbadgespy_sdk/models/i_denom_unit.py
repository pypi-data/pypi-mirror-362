...
class IDenomUnit(BaseModel):
    """
    IDenomUnit
    """ # noqa: E501
    decimals: NumberType = Field(description="The number of decimal places for this denomination unit.")
    symbol: StrictStr = Field(description="The symbol for this denomination unit.")
    is_default_display: StrictBool = Field(description="Whether this denomination unit is the default display unit.", alias="isDefaultDisplay")
    __properties: ClassVar[List[str]] = ["decimals", "symbol", "isDefaultDisplay"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IDenomUnit from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of decimals
        if self.decimals:
            _dict['decimals'] = self.decimals.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IDenomUnit from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "decimals": NumberType.from_dict(obj["decimals"]) if obj.get("decimals") is not None else None,
            "symbol": obj.get("symbol"),
            "isDefaultDisplay": obj.get("isDefaultDisplay")
        })
        return _obj
