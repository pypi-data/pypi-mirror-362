...
NUMBERTYPE_ANY_OF_SCHEMAS = ["float", "str"]
...
class NumberType(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = NumberType.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in NumberType with anyOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into NumberType with anyOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return instance
