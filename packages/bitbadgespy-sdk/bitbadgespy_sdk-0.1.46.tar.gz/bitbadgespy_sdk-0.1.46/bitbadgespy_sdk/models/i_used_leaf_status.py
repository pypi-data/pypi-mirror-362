...
class IUsedLeafStatus(BaseModel):
    """
    IUsedLeafStatus
    """ # noqa: E501
    leaf_index: NumberType = Field(description="The leaf index", alias="leafIndex")
    used_by: StrictStr = Field(description="All supported addresses map to a Bech32 BitBadges address which is used by the BitBadges blockchain behind the scenes. For conversion, see the BitBadges documentation. If this type is used, we must always convert to a BitBadges address before using it.", alias="usedBy")
    __properties: ClassVar[List[str]] = ["leafIndex", "usedBy"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUsedLeafStatus from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of leaf_index
        if self.leaf_index:
            _dict['leafIndex'] = self.leaf_index.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUsedLeafStatus from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "leafIndex": NumberType.from_dict(obj["leafIndex"]) if obj.get("leafIndex") is not None else None,
