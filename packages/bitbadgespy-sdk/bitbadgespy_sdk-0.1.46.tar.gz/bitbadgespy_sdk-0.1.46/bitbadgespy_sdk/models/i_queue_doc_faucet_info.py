...
class IQueueDocFaucetInfo(BaseModel):
    """
    For use for airdrops
    """ # noqa: E501
    tx_hash: StrictStr = Field(alias="txHash")
    amount: NumberType
    recipient: StrictStr = Field(description="All supported addresses map to a Bech32 BitBadges address which is used by the BitBadges blockchain behind the scenes. For conversion, see the BitBadges documentation. If this type is used, we must always convert to a BitBadges address before using it.")
    denom: StrictStr
    __properties: ClassVar[List[str]] = ["txHash", "amount", "recipient", "denom"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IQueueDocFaucetInfo from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IQueueDocFaucetInfo from a dict"""
        if obj is None:
            return None
