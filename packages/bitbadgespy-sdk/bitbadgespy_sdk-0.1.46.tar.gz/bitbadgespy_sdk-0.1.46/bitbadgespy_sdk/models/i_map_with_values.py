...
class IMapWithValues(BaseModel):
    """
    ...
    map_id: StrictStr = Field(alias="mapId")
    inherit_manager_timeline_from: NumberType = Field(alias="inheritManagerTimelineFrom")
    manager_timeline: List[IManagerTimeline] = Field(alias="managerTimeline")
    update_criteria: IMapUpdateCriteria = Field(alias="updateCriteria")
    value_options: IValueOptions = Field(alias="valueOptions")
    default_value: StrictStr = Field(alias="defaultValue")
    permissions: IMapPermissions
    metadata_timeline: List[IMapMetadataTimeline] = Field(alias="metadataTimeline")
    values: Dict[str, IValueStore] = Field(description="The (key, value) pairs for the maps that are set.")
    metadata: Optional[IMetadata] = Field(default=None, description="The fetched metadata for the map (if any).")
    update_history: List[IUpdateHistory] = Field(description="The update history for the map. Maps are maintained through blockchain transactions.", alias="updateHistory")
    __properties: ClassVar[List[str]] = ["creator", "mapId", "inheritManagerTimelineFrom", "managerTimeline", "updateCriteria", "valueOptions", "defaultValue", "permissions", "metadataTimeline", "values", "metadata", "updateHistory"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMapWithValues from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of inherit_manager_timeline_from
        if self.inherit_manager_timeline_from:
            _dict['inheritManagerTimelineFrom'] = self.inherit_manager_timeline_from.to_dict()
        ...
        # override the default output from pydantic by calling `to_dict()` of update_criteria
        if self.update_criteria:
            _dict['updateCriteria'] = self.update_criteria.to_dict()
        # override the default output from pydantic by calling `to_dict()` of value_options
        if self.value_options:
            _dict['valueOptions'] = self.value_options.to_dict()
        # override the default output from pydantic by calling `to_dict()` of permissions
        if self.permissions:
            _dict['permissions'] = self.permissions.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in metadata_timeline (list)
        _items = []
        if self.metadata_timeline:
            for _item_metadata_timeline in self.metadata_timeline:
                if _item_metadata_timeline:
                    _items.append(_item_metadata_timeline.to_dict())
            _dict['metadataTimeline'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMapWithValues from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "mapId": obj.get("mapId"),
            "inheritManagerTimelineFrom": NumberType.from_dict(obj["inheritManagerTimelineFrom"]) if obj.get("inheritManagerTimelineFrom") is not None else None,
            ...
            "updateCriteria": IMapUpdateCriteria.from_dict(obj["updateCriteria"]) if obj.get("updateCriteria") is not None else None,
            "valueOptions": IValueOptions.from_dict(obj["valueOptions"]) if obj.get("valueOptions") is not None else None,
            "defaultValue": obj.get("defaultValue"),
            "permissions": IMapPermissions.from_dict(obj["permissions"]) if obj.get("permissions") is not None else None,
            "metadataTimeline": [IMapMetadataTimeline.from_dict(_item) for _item in obj["metadataTimeline"]] if obj.get("metadataTimeline") is not None else None,
            "values": dict(
                (_k, IValueStore.from_dict(_v))
