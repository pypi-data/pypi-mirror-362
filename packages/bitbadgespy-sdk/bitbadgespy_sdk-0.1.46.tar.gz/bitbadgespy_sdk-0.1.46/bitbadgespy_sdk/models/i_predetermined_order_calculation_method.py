...
class IPredeterminedOrderCalculationMethod(BaseModel):
    """
    IPredeterminedOrderCalculationMethod
    """ # noqa: E501
    use_overall_num_transfers: StrictBool = Field(description="Use the overall number of transfers this approval has been used with as the order number. Ex: If this approval has been used 2 times by ANY address, then the order number for the next transfer will be 3.", alias="useOverallNumTransfers")
    use_per_to_address_num_transfers: StrictBool = Field(description="Use the number of times this approval has been used by each to address as the order number. Ex: If this approval has been used 2 times by to address A, then the order number for the next transfer by to address A will be 3.", alias="usePerToAddressNumTransfers")
    use_per_from_address_num_transfers: StrictBool = Field(description="Use the number of times this approval has been used by each from address as the order number. Ex: If this approval has been used 2 times by from address A, then the order number for the next transfer by from address A will be 3.", alias="usePerFromAddressNumTransfers")
    use_per_initiated_by_address_num_transfers: StrictBool = Field(description="Use the number of times this approval has been used by each initiated by address as the order number. Ex: If this approval has been used 2 times by initiated by address A, then the order number for the next transfer by initiated by address A will be 3.", alias="usePerInitiatedByAddressNumTransfers")
    use_merkle_challenge_leaf_index: StrictBool = Field(description="Use the merkle challenge leaf index as the order number. Must specify ONE merkle challenge with the useLeafIndexForTransferOrder flag set to true. If so, we will use the leaf index of each merkle proof to calculate the order number. This is used to reserve specific balances for specific leaves (such as codes or whitelist address leafs)", alias="useMerkleChallengeLeafIndex")
    challenge_tracker_id: StrictStr = Field(description="Use the merkle challenge leaf index as the order number. Must specify ONE merkle challenge with the useLeafIndexForTransferOrder flag set to true. If so, we will use the leaf index of each merkle proof to calculate the order number. This is used to reserve specific balances for specific leaves (such as codes or whitelist address leafs)", alias="challengeTrackerId")
    __properties: ClassVar[List[str]] = ["useOverallNumTransfers", "usePerToAddressNumTransfers", "usePerFromAddressNumTransfers", "usePerInitiatedByAddressNumTransfers", "useMerkleChallengeLeafIndex", "challengeTrackerId"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IPredeterminedOrderCalculationMethod from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IPredeterminedOrderCalculationMethod from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "useOverallNumTransfers": obj.get("useOverallNumTransfers"),
            "usePerToAddressNumTransfers": obj.get("usePerToAddressNumTransfers"),
            "usePerFromAddressNumTransfers": obj.get("usePerFromAddressNumTransfers"),
            "usePerInitiatedByAddressNumTransfers": obj.get("usePerInitiatedByAddressNumTransfers"),
            "useMerkleChallengeLeafIndex": obj.get("useMerkleChallengeLeafIndex"),
            "challengeTrackerId": obj.get("challengeTrackerId")
        })
        return _obj
