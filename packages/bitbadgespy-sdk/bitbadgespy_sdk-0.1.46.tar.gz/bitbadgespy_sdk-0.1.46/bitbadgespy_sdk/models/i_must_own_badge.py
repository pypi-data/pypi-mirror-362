...
class IMustOwnBadge(BaseModel):
    """
    IMustOwnBadge
    """ # noqa: E501
    collection_id: StrictStr = Field(alias="collectionId")
    amount_range: IUintRange = Field(description="The range of badge amounts the user must own (min to max).", alias="amountRange")
    ownership_times: List[IUintRange] = Field(description="The time ranges during which the user must own the badges.", alias="ownershipTimes")
    badge_ids: List[IUintRange] = Field(description="The badge IDs the user must own.", alias="badgeIds")
    override_with_current_time: StrictBool = Field(description="If true, override ownershipTimes with the current time.", alias="overrideWithCurrentTime")
    must_satisfy_for_all_assets: StrictBool = Field(description="If true, the user must meet ownership requirements for all specified badges; else, must meet requirements for any single badge.", alias="mustSatisfyForAllAssets")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMustOwnBadge from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMustOwnBadge from a dict"""
        if obj is None:
            return None
