...
class IPrecalculationOptions(BaseModel):
    """
    IPrecalculationOptions
    """ # noqa: E501
    override_timestamp: Optional[NumberType] = Field(default=None, description="The timestamp to use for the transfer.", alias="overrideTimestamp")
    badge_ids_override: Optional[List[IUintRange]] = Field(default=None, description="The badge IDs to use for the transfer.", alias="badgeIdsOverride")
    __properties: ClassVar[List[str]] = ["overrideTimestamp", "badgeIdsOverride"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IPrecalculationOptions from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of override_timestamp
        if self.override_timestamp:
            _dict['overrideTimestamp'] = self.override_timestamp.to_dict()
        # override the default output from pydantic by calling `to_dict()` of each item in badge_ids_override (list)
        _items = []
        if self.badge_ids_override:
            for _item_badge_ids_override in self.badge_ids_override:
                if _item_badge_ids_override:
                    _items.append(_item_badge_ids_override.to_dict())
            _dict['badgeIdsOverride'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IPrecalculationOptions from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "overrideTimestamp": NumberType.from_dict(obj["overrideTimestamp"]) if obj.get("overrideTimestamp") is not None else None,
            "badgeIdsOverride": [IUintRange.from_dict(_item) for _item in obj["badgeIdsOverride"]] if obj.get("badgeIdsOverride") is not None else None
        })
        return _obj
