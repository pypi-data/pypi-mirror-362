...
class IClaimCachePolicy(BaseModel):
    """
    IClaimCachePolicy
    """ # noqa: E501
    ttl: Optional[NumberType] = Field(default=None, description="The number of seconds to cache the result. Default is 5 minutes (300 seconds) if none is specified.  Note: This may be overridden by other options")
    always_permanent: Optional[StrictBool] = Field(default=None, description="Permanent once the claim is calculated once. We will cache results indefinitely.", alias="alwaysPermanent")
    permanent_after: Optional[NumberType] = Field(default=None, description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="permanentAfter")
    __properties: ClassVar[List[str]] = ["ttl", "alwaysPermanent", "permanentAfter"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IClaimCachePolicy from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of permanent_after
        if self.permanent_after:
            _dict['permanentAfter'] = self.permanent_after.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IClaimCachePolicy from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "ttl": NumberType.from_dict(obj["ttl"]) if obj.get("ttl") is not None else None,
            "alwaysPermanent": obj.get("alwaysPermanent"),
            "permanentAfter": NumberType.from_dict(obj["permanentAfter"]) if obj.get("permanentAfter") is not None else None
        })
        return _obj
