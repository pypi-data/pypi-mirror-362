...
class IRefreshStatusSuccessResponse(BaseModel):
    """
    IRefreshStatusSuccessResponse
    """ # noqa: E501
    in_queue: StrictBool = Field(description="Boolean indicating if the collection is currently in the queue.", alias="inQueue")
    error_docs: List[IQueueDoc] = Field(description="Array of error documents corresponding to the collection.", alias="errorDocs")
    refresh_doc: IRefreshDoc = Field(description="The status information corresponding to the collection.", alias="refreshDoc")
    __properties: ClassVar[List[str]] = ["inQueue", "errorDocs", "refreshDoc"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IRefreshStatusSuccessResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in error_docs (list)
        _items = []
        if self.error_docs:
            for _item_error_docs in self.error_docs:
                if _item_error_docs:
                    _items.append(_item_error_docs.to_dict())
            _dict['errorDocs'] = _items
        # override the default output from pydantic by calling `to_dict()` of refresh_doc
        if self.refresh_doc:
            _dict['refreshDoc'] = self.refresh_doc.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IRefreshStatusSuccessResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "errorDocs": [IQueueDoc.from_dict(_item) for _item in obj["errorDocs"]] if obj.get("errorDocs") is not None else None,
            "refreshDoc": IRefreshDoc.from_dict(obj["refreshDoc"]) if obj.get("refreshDoc") is not None else None
        })
        return _obj
