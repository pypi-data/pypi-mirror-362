...
class IGetActiveAuthorizationsSuccessResponse(BaseModel):
    """
    IGetActiveAuthorizationsSuccessResponse
    """ # noqa: E501
    authorizations: List[IAccessTokenDoc]
    developer_apps: List[IDeveloperAppDoc] = Field(description="Developer app docs for each authorization.  Undefined if deleted.", alias="developerApps")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["authorizations", "developerApps"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGetActiveAuthorizationsSuccessResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in developer_apps (list)
        _items = []
        if self.developer_apps:
            for _item_developer_apps in self.developer_apps:
                if _item_developer_apps:
                    _items.append(_item_developer_apps.to_dict())
            _dict['developerApps'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGetActiveAuthorizationsSuccessResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "authorizations": [IAccessTokenDoc.from_dict(_item) for _item in obj["authorizations"]] if obj.get("authorizations") is not None else None,
            "developerApps": [IDeveloperAppDoc.from_dict(_item) for _item in obj["developerApps"]] if obj.get("developerApps") is not None else None
        })
