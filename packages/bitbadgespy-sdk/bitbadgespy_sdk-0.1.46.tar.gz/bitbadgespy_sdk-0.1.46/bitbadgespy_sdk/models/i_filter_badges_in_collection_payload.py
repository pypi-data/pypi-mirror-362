...
class IFilterBadgesInCollectionPayload(BaseModel):
    """
    IFilterBadgesInCollectionPayload
    """ # noqa: E501
    badge_ids: Optional[List[IUintRange]] = Field(default=None, description="Limit to specific badge IDs. Leave undefined to not filter by badge ID.", alias="badgeIds")
    categories: Optional[List[StrictStr]] = Field(default=None, description="Limit to specific lists. Leave undefined to not filter by list.")
    tags: Optional[List[StrictStr]] = Field(default=None, description="Limit to specific lists. Leave undefined to not filter by list.")
    most_viewed: Optional[StrictStr] = Field(default=None, description="mostViewed is a special view that sorts by most viewed badges. May be incompatible with other filters.", alias="mostViewed")
    bookmark: Optional[StrictStr] = Field(default=None, description="Pagination bookmark. Leave undefined or \"\" for first request.")
    attributes: Optional[List[IFilterBadgesInCollectionPayloadAttributesInner]] = Field(default=None, description="Attribute queries")
    price_range: Optional[IUintRange] = Field(default=None, description="The listing prices", alias="priceRange")
    denom: Optional[StrictStr] = Field(default=None, description="Denom for the price range. Defaults to ubadge.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["badgeIds", "categories", "tags", "mostViewed", "bookmark", "attributes", "priceRange", "denom"]

    @field_validator('most_viewed')
    def most_viewed_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['daily', 'allTime', 'weekly', 'monthly', 'yearly']):
            raise ValueError("must be one of enum values ('daily', 'allTime', 'weekly', 'monthly', 'yearly')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IFilterBadgesInCollectionPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IFilterBadgesInCollectionPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "tags": obj.get("tags"),
            "mostViewed": obj.get("mostViewed"),
            "bookmark": obj.get("bookmark"),
            "attributes": [IFilterBadgesInCollectionPayloadAttributesInner.from_dict(_item) for _item in obj["attributes"]] if obj.get("attributes") is not None else None,
            "priceRange": IUintRange.from_dict(obj["priceRange"]) if obj.get("priceRange") is not None else None,
            "denom": obj.get("denom")
        })
