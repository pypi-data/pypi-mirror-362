...
class IUpdateHistory(BaseModel):
    """
    IUpdateHistory
    """ # noqa: E501
    tx_hash: StrictStr = Field(description="The transaction hash of the on-chain transaction that updated this.", alias="txHash")
    block: NumberType = Field(description="The block number of the on-chain transaction that updated this.")
    block_timestamp: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.", alias="blockTimestamp")
    timestamp: NumberType = Field(description="Numeric timestamp - value is equal to the milliseconds since the UNIX epoch.")
    __properties: ClassVar[List[str]] = ["txHash", "block", "blockTimestamp", "timestamp"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IUpdateHistory from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of block_timestamp
        if self.block_timestamp:
            _dict['blockTimestamp'] = self.block_timestamp.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IUpdateHistory from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "blockTimestamp": NumberType.from_dict(obj["blockTimestamp"]) if obj.get("blockTimestamp") is not None else None,
