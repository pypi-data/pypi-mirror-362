...
class IBadgeMetadataTimeline(BaseModel):
    """
    ...
    badge_metadata: List[IBadgeMetadataDetails] = Field(description="The badge metadata, with off-chain details populated.", alias="badgeMetadata")
    __properties: ClassVar[List[str]] = ["timelineTimes", "badgeMetadata"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IBadgeMetadataTimeline from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in badge_metadata (list)
        _items = []
        if self.badge_metadata:
            for _item_badge_metadata in self.badge_metadata:
                if _item_badge_metadata:
                    _items.append(_item_badge_metadata.to_dict())
            _dict['badgeMetadata'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IBadgeMetadataTimeline from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "badgeMetadata": [IBadgeMetadataDetails.from_dict(_item) for _item in obj["badgeMetadata"]] if obj.get("badgeMetadata") is not None else None
        })
        return _obj
