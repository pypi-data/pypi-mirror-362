...
class IMapPermissions(BaseModel):
    """
    IMapPermissions
    """ # noqa: E501
    can_update_metadata: List[ITimedUpdatePermission] = Field(alias="canUpdateMetadata")
    can_update_manager: List[ITimedUpdatePermission] = Field(alias="canUpdateManager")
    can_delete_map: List[IActionPermission] = Field(alias="canDeleteMap")
    __properties: ClassVar[List[str]] = ["canUpdateMetadata", "canUpdateManager", "canDeleteMap"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMapPermissions from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in can_update_metadata (list)
        _items = []
        if self.can_update_metadata:
            for _item_can_update_metadata in self.can_update_metadata:
                if _item_can_update_metadata:
                    _items.append(_item_can_update_metadata.to_dict())
            _dict['canUpdateMetadata'] = _items
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in can_delete_map (list)
        _items = []
        if self.can_delete_map:
            for _item_can_delete_map in self.can_delete_map:
                if _item_can_delete_map:
                    _items.append(_item_can_delete_map.to_dict())
            _dict['canDeleteMap'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMapPermissions from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "canUpdateMetadata": [ITimedUpdatePermission.from_dict(_item) for _item in obj["canUpdateMetadata"]] if obj.get("canUpdateMetadata") is not None else None,
            ...
            "canDeleteMap": [IActionPermission.from_dict(_item) for _item in obj["canDeleteMap"]] if obj.get("canDeleteMap") is not None else None
        })
        return _obj
