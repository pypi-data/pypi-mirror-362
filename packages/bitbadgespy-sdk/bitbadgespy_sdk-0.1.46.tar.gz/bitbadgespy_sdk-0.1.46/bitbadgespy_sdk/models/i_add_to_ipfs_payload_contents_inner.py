...
IADDTOIPFSPAYLOADCONTENTSINNER_ANY_OF_SCHEMAS = ["IBadgeMetadataDetails", "IChallengeDetails", "ICollectionMetadataDetails", "IMetadata"]

class IAddToIpfsPayloadContentsInner(BaseModel):
    """
    IAddToIpfsPayloadContentsInner
    """

    # data type: IBadgeMetadataDetails
    anyof_schema_1_validator: Optional[IBadgeMetadataDetails] = None
    # data type: IMetadata
    anyof_schema_2_validator: Optional[IMetadata] = None
    # data type: ICollectionMetadataDetails
    anyof_schema_3_validator: Optional[ICollectionMetadataDetails] = None
    # data type: IChallengeDetails
    anyof_schema_4_validator: Optional[IChallengeDetails] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[IBadgeMetadataDetails, IChallengeDetails, ICollectionMetadataDetails, IMetadata]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "IBadgeMetadataDetails", "IChallengeDetails", "ICollectionMetadataDetails", "IMetadata" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = IAddToIpfsPayloadContentsInner.model_construct()
        error_messages = []
        # validate data type: IBadgeMetadataDetails
        if not isinstance(v, IBadgeMetadataDetails):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IBadgeMetadataDetails`")
        else:
            return v

        # validate data type: IMetadata
        if not isinstance(v, IMetadata):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IMetadata`")
        else:
            return v

        # validate data type: ICollectionMetadataDetails
        if not isinstance(v, ICollectionMetadataDetails):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ICollectionMetadataDetails`")
        else:
            return v

        # validate data type: IChallengeDetails
        if not isinstance(v, IChallengeDetails):
            error_messages.append(f"Error! Input type `{type(v)}` is not `IChallengeDetails`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in IAddToIpfsPayloadContentsInner with anyOf schemas: IBadgeMetadataDetails, IChallengeDetails, ICollectionMetadataDetails, IMetadata. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[IBadgeMetadataDetails] = None
        try:
            instance.actual_instance = IBadgeMetadataDetails.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[IMetadata] = None
        try:
            instance.actual_instance = IMetadata.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_3_validator: Optional[ICollectionMetadataDetails] = None
        try:
            instance.actual_instance = ICollectionMetadataDetails.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_4_validator: Optional[IChallengeDetails] = None
        try:
            instance.actual_instance = IChallengeDetails.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into IAddToIpfsPayloadContentsInner with anyOf schemas: IBadgeMetadataDetails, IChallengeDetails, ICollectionMetadataDetails, IMetadata. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], IBadgeMetadataDetails, IChallengeDetails, ICollectionMetadataDetails, IMetadata]]:
