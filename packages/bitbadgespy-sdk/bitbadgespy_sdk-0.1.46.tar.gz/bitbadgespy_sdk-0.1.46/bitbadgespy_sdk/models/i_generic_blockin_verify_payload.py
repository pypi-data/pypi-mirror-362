...
class IGenericBlockinVerifyPayload(BaseModel):
    """
    ...
    signature: StrictStr = Field(description="The signature of the message")
    alt_signer: Optional[StrictStr] = Field(default=None, description="A native address is an address that is native to the user's chain. For example, an Ethereum address is native to Ethereum (0x...). If this type is used, we support any native address type. We do not require conversion to a BitBadges address like the BitBadgesAddress type.", alias="altSigner")
    social_sign_in: Optional[StrictStr] = Field(default=None, description="Selected social to attempt to sign in with.", alias="socialSignIn")
    password: Optional[StrictStr] = Field(default=None, description="The password to sign in with.")
    public_key: Optional[StrictStr] = Field(default=None, description="Required for some chains (Cosmos) to verify signature. The public key of the signer.", alias="publicKey")
    options: Optional[VerifyChallengeOptions] = Field(default=None, description="Additional options for verifying the challenge.")
    attestations: Optional[List[IAttestationsProof]] = Field(default=None, description="Additional attestations to verify in the challenge.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["message", "signature", "altSigner", "socialSignIn", "password", "publicKey", "options", "attestations"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IGenericBlockinVerifyPayload from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IGenericBlockinVerifyPayload from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "altSigner": obj.get("altSigner"),
            "socialSignIn": obj.get("socialSignIn"),
            "password": obj.get("password"),
            "publicKey": obj.get("publicKey"),
            "options": VerifyChallengeOptions.from_dict(obj["options"]) if obj.get("options") is not None else None,
