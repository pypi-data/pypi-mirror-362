...
class IRecurringOwnershipTimes(BaseModel):
    """
    IRecurringOwnershipTimes
    """ # noqa: E501
    start_time: NumberType = Field(description="The start time of the recurring ownership times.", alias="startTime")
    interval_length: NumberType = Field(description="The interval length of the recurring ownership times.", alias="intervalLength")
    charge_period_length: NumberType = Field(description="Grace period length of when this is valid.", alias="chargePeriodLength")
    __properties: ClassVar[List[str]] = ["startTime", "intervalLength", "chargePeriodLength"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IRecurringOwnershipTimes from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of interval_length
        if self.interval_length:
            _dict['intervalLength'] = self.interval_length.to_dict()
        # override the default output from pydantic by calling `to_dict()` of charge_period_length
        if self.charge_period_length:
            _dict['chargePeriodLength'] = self.charge_period_length.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IRecurringOwnershipTimes from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "startTime": NumberType.from_dict(obj["startTime"]) if obj.get("startTime") is not None else None,
            "intervalLength": NumberType.from_dict(obj["intervalLength"]) if obj.get("intervalLength") is not None else None,
            "chargePeriodLength": NumberType.from_dict(obj["chargePeriodLength"]) if obj.get("chargePeriodLength") is not None else None
        })
        return _obj
