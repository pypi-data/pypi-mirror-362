...
class IGetStatusPayload(BaseModel):
    ...
    chain: Optional[StrictStr] = Field(default=None, description="Chain to check? Defaults to BitBadges poller.")
    __properties: ClassVar[List[str]] = ["withOutOfSyncCheck", "chain"]
    ...
    def chain_validate_enum(cls, value):
        ...
        if value not in set(['Thorchain', 'BitBadges']):
            raise ValueError("must be one of enum values ('Thorchain', 'BitBadges')")
        return value
