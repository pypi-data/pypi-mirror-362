...
class IMerkleChallenge(BaseModel):
    """
    IMerkleChallenge
    """ # noqa: E501
    root: StrictStr = Field(description="The root of the merkle tree.")
    expected_proof_length: NumberType = Field(description="The expected proof length of the merkle proof.", alias="expectedProofLength")
    use_creator_address_as_leaf: StrictBool = Field(description="Whether or not to override any leaf value and use the creator address as the leaf. Used for whitelist trees.", alias="useCreatorAddressAsLeaf")
    max_uses_per_leaf: NumberType = Field(description="Whether or not to enforce max uses per leaf. Used to prevent replay attacks.", alias="maxUsesPerLeaf")
    uri: StrictStr = Field(description="The URI where to fetch the merkle challenge metadata from.")
    custom_data: StrictStr = Field(description="Arbitrary custom data that can be stored on-chain.", alias="customData")
    challenge_tracker_id: StrictStr = Field(description="Tracker ID details for the merkle challenge.", alias="challengeTrackerId")
    leaf_signer: StrictStr = Field(description="The signer of the leaf. Currently only supports ETH addresses.", alias="leafSigner")
    __properties: ClassVar[List[str]] = ["root", "expectedProofLength", "useCreatorAddressAsLeaf", "maxUsesPerLeaf", "uri", "customData", "challengeTrackerId", "leafSigner"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMerkleChallenge from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of expected_proof_length
        if self.expected_proof_length:
            _dict['expectedProofLength'] = self.expected_proof_length.to_dict()
        # override the default output from pydantic by calling `to_dict()` of max_uses_per_leaf
        if self.max_uses_per_leaf:
            _dict['maxUsesPerLeaf'] = self.max_uses_per_leaf.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMerkleChallenge from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "root": obj.get("root"),
            "expectedProofLength": NumberType.from_dict(obj["expectedProofLength"]) if obj.get("expectedProofLength") is not None else None,
            "useCreatorAddressAsLeaf": obj.get("useCreatorAddressAsLeaf"),
            "maxUsesPerLeaf": NumberType.from_dict(obj["maxUsesPerLeaf"]) if obj.get("maxUsesPerLeaf") is not None else None,
            "uri": obj.get("uri"),
            "customData": obj.get("customData"),
            "challengeTrackerId": obj.get("challengeTrackerId"),
            "leafSigner": obj.get("leafSigner")
        })
        return _obj
