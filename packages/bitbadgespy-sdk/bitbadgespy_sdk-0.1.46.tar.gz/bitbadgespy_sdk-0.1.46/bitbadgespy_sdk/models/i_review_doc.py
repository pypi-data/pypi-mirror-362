...
class IReviewDoc(BaseModel):
    ...
    review: StrictStr = Field(description="The review text (max 2048 characters).")
    stars: NumberType = Field(description="The number of stars given (1-5).")
    ...
    reviewed_address: Optional[StrictStr] = Field(default=None, description="All supported addresses map to a Bech32 BitBadges address which is used by the BitBadges blockchain behind the scenes. For conversion, see the BitBadges documentation. If this type is used, we must always convert to a BitBadges address before using it.", alias="reviewedAddress")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["_docId", "_id", "timestamp", "block", "_notificationsHandled", "private", "review", "stars", "from", "collectionId", "reviewedAddress"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IReviewDoc from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of stars
        if self.stars:
            _dict['stars'] = self.stars.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IReviewDoc from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "stars": NumberType.from_dict(obj["stars"]) if obj.get("stars") is not None else None,
            "from": obj.get("from"),
            "collectionId": obj.get("collectionId"),
            "reviewedAddress": obj.get("reviewedAddress")
        })
