...
class Utils:
    ...
    def timestamp_now_plus_less_than(plus_seconds: int, timestamp: Union[int, str]):
        return Utils.datetime_now_plus(seconds=plus_seconds).timestamp() < int(timestamp)

    @staticmethod
    def timestamp_now_minus_less_than(minus_seconds: int, timestamp: Union[int, str]):
        return Utils.datetime_now_minus(seconds=minus_seconds).timestamp() < int(timestamp)
...
class ClientUtils:
    ...
    @staticmethod
    def compute_signature(secret: str, method: str, path: str, timestamp: str, body: str) -> str:
    ...
    async def verify_signature(request: Request, client_secret: str):
        ...
        expected_signature = ClientUtils.compute_signature(client_secret_decrypted, method, path, timestamp, body.decode())
