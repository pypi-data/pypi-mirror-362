...
def get_files(
...
) -> tuple[pdata.Document, SimSettings, pd.DataFrame]:
    test_dir = Path("assets") if test_dir is None else test_dir
    ...
    return load_document(file=path / f"{prefix}-sbml-l3v2.xml"), sim_settings, expected
