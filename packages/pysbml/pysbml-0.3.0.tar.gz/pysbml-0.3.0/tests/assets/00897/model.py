...
reaction1_k2: float = 0.700000000000000
reaction3_k2: float = 1.00000000000000
...
S3_conc = S3 / C
S4_conc = S4 / C
reaction1 = S1_conc * reaction1_k2 * time
reaction2 = S2_conc * k2 * time
reaction3 = S3_conc * reaction3_k2 * time
y0 = [S1, S2, S3, S4]
...
def model(time: float, variables: tuple[float, ...]) -> tuple[float, ...]:
    ...
    S2_conc: float = S2 / C
    S3_conc: float = S3 / C
    reaction1: float = S1_conc * reaction1_k2 * time
    reaction2: float = S2_conc * k2 * time
    reaction3: float = S3_conc * reaction3_k2 * time
