...
time: float = 0.0
k1: float = 160.000000000000
k2: float = 0.700000000000000
compartment: float = math.nan
S1: float = 0.0100000000000000
S2: float = 0.00500000000000000
S3: float = 0.0100000000000000

# Initial assignments
reaction1 = S1**2 * S2 * k1
reaction2 = S3 * k2
y0 = [S1, S2, S3]
...
def model(time: float, variables: tuple[float, ...]) -> tuple[float, ...]:
    S1, S2, S3 = variables
    reaction1: float = S1**2 * S2 * k1
    reaction2: float = S3 * k2
    dS1dt: float = -2.0 * reaction1 + 2.0 * reaction2
