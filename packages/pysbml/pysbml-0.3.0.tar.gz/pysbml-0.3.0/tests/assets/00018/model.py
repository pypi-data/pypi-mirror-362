...
reaction3 = S2_conc * k3
reaction4 = S3_conc * S4_conc * k4
y0 = [S1, S2, S3, S4]
...
def model(time: float, variables: tuple[float, ...]) -> tuple[float, ...]:
    ...
    reaction3: float = S2_conc * k3
    reaction4: float = S3_conc * S4_conc * k4
    dS1dt: float = -compartment * reaction1 + compartment * reaction2
    dS2dt: float = (
        compartment * reaction1
        - compartment * reaction2
        - compartment * reaction3
        + compartment * reaction4
    )
    dS3dt: float = compartment * reaction3 - compartment * reaction4
    dS4dt: float = compartment * reaction3 - compartment * reaction4
