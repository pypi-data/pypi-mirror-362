...
def model(time: float, variables: tuple[float, ...]) -> tuple[float, ...]:
    ...
    reaction3: float = S3_conc * k3
    dS1dt: float = (
        -compartment * reaction1 + compartment * reaction2 + compartment * reaction3
    )
    dS2dt: float = -compartment * reaction1 + compartment * reaction2
    dS3dt: float = (
        compartment * reaction1 - compartment * reaction2 - compartment * reaction3
    )
    dS4dt: float = compartment * reaction3
