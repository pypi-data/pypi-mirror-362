...
def main() -> None:
    ...
    stdout, stderr, returncode = run_command(
        ["clang", "-Xclang", "-ast-dump", "-fsyntax-only", "-nostdinc", args.input_file]
    )
    ...
    if returncode != 0:
        print(
            "Warning: Clang reported an error (possibly due to missing stdio.h), but continuing to process output.",
            file=sys.stderr,
        )
    ...
    prototypes_str: list[str] = [str(proto) for proto in prototypes if proto]
