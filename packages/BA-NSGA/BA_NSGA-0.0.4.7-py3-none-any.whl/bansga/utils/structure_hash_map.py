...
Supported methods (tag passed via __init__(method=...))
-------------------------------------------------------
"robust"   –  Lattice+coordinates + KD‑tree + E/V (the earlier SHA‑256 robust
               descriptor; invariances: T,R,P).
"rdf"       –  Element‑resolved pair‑RDF histogram + E/V (fast, T,R,P).
"soap"      –  Placeholder: raises NotImplementedError unless DScribe is
               installed (invariant power‑spectrum – heavy but most accurate).

The public API is identical to CanonicalHashMap / PairRDFHashMap so you can
swap it into existing GA code without further edits:

    cmap = StructureHashMap(method="rdf", r_max=10.0, bin_width=0.02)
    new  = cmap.add_structure(container)  # True if unique, False duplicate

Author: ChatGPT – 25 Jun 2025
"""
...
def _rdf_hash_factory(
    *,
    ...
    def _rdf_hash(container) -> str:
        ...
        # 3) recorremos sólo esa lista, usando apm_primitive.distance
        for i, j in pairs:
...
============================
Comprehensive, PEP-8-conformant unit-test suite for the ``StructureHashMap``
framework.
...
class StructureHashMapTests(unittest.TestCase):
    """Invariance, quantisation and API‑robustness tests for ``StructureHashMap``."""
    ...
    R_MAX     = 5.0
    BIN_WIDTH = 0.1
    HASH_MAP  = StructureHashMap(method="rdf", r_max=R_MAX, bin_width=BIN_WIDTH)
    ...
    def test_bin_width_parameter_sensitivity(self) -> None:
        ...
        map_fine = StructureHashMap(method="rdf", r_max=4.0, bin_width=0.05)
        map_coarse = StructureHashMap(method="rdf", r_max=4.0, bin_width=0.20)
    ...
    def test_method_variation_produces_distinct_hashes(self) -> None:
        ...
        h_rdf = StructureHashMap(method="rdf")._hash_fn(sr)
        h_rob = StructureHashMap(method="robust")._hash_fn(sr)
        self.assertNotEqual(h_rdf, h_rob)

    def test_invalid_method_raises(self) -> None:
        """Requesting an unsupported hashing tag must raise ValueError."""
        with self.assertRaises(ValueError):
            StructureHashMap(method="nonexistent")
