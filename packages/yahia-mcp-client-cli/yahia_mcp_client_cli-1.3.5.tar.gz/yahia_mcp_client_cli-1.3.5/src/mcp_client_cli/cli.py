...
def create_llm_model(app_config: AppConfig) -> BaseChatModel:
    """Create the appropriate LLM model based on configuration."""
    try:
        if app_config.llm.custom_provider:
            # Use custom LLM provider
            if not app_config.llm.endpoint_url:
            ...
            print(f"Creating custom LLM with endpoint: {app_config.llm.endpoint_url}")
            print(f"Auth type: {app_config.llm.auth_type}")
            print(f"Headers: {app_config.llm.headers}")
            
            return create_custom_llm(
                endpoint_url=app_config.llm.endpoint_url,
                api_key=app_config.llm.api_key,
                auth_type=app_config.llm.auth_type,
                headers=app_config.llm.headers,
                temperature=app_config.llm.temperature,
                model_name=app_config.llm.model,
                verify_ssl=app_config.llm.verify_ssl,
                ssl_cert_path=app_config.llm.ssl_cert_path
            )
        else:
            # Use standard LangChain providers
            extra_body = {}
...
async def handle_conversation(args: argparse.Namespace, query: HumanMessage, 
    ...
    # Create the LLM model (custom or standard)
    model = create_llm_model(app_config)
