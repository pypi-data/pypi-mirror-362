...
def test_trigger_detector():
    ...
    try:
        ...
        if len(detector.df_triggers) > 0:
            ...
            response = input("Test filter_bad_windows()? Requires ML model files (y/n): ")
            if response.lower() == 'y':
                clf_path = input("Enter path to classifier (.pkl file): ").strip()
                classes_path = input("Enter path to classes (.npy file): ").strip()
                
                if clf_path and classes_path:
                    try:
                        detector.filter_bad_windows(clf_path=clf_path, classes_path=classes_path)
                        print("✅ ML-based filtering completed successfully")
                    except Exception as e:
                        print(f"⚠️  filter_bad_windows() failed: {e}")
                else:
                    print("⏭️  Skipping ML filtering - paths not provided")
            else:
