...
This package is included in `requirements.txt` of the backend and can also be installed directly:

```bash
pip install -e ./pointr_cloud_common
```
...
The service authenticates and exposes helpers via sub services (``site_service``, ``building_service``, ``level_service``, ``sdk_service`` and ``client_service``).
...
Entry point for interacting with the V8 API.
