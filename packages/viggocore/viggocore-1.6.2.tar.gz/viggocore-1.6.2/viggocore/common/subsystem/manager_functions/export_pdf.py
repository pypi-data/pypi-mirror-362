...
from viggocore.common import exception, utils as core_utils
from viggocore.common.subsystem.gerar_pdf import (
...
class ExportPdf(operation.Operation):
    ...
    def _get_domain_display_name(self, entities):
        ...
        if not core_utils.is_empty_or_blank(entities[0].domain.display_name):
            domain_display_name = entities[0].domain.display_name

        if self.com_endereco:
            address = ''
            address2 = ''
            if len(entities[0].domain.addresses) > 0:
                addr = entities[0].domain.addresses[0]
                address = (
                ...
                if entities[0].domain.doc is not None:
                    address2 = f'{address2} - CNPJ: ' \
                        if len(entities[0].domain.doc) > 11 \
                        else f'{address2} - CPF: '
                    address2 = address2 + \
                        ru.apply_mask_cpf_cnpj(entities[0].domain.doc)
    ...
    def pre(self, **kwargs):
        ...
        if style_pagination is None:
            self.style_pagination = ru.STYLE_PAGINATION_TYPE.RETRATO
        else:
            self.style_pagination = ru.STYLE_PAGINATION_TYPE[style_pagination]
