...
class ScheduleService:
    ...
    async def process_delay_messages(self) -> None:
        """延时消息处理协程 - 使用优化的调度逻辑"""
        self.context.logger.info("启动延时任务调度器")
        await self.start_delay_monitoring()

    async def start_delay_monitoring(self) -> None:
        """启动延时任务监听和主调度循环"""
        if self.is_running:
            self.context.logger.warning("延时任务调度器已在运行")
            return
    ...
    async def stop_delay_processing(self) -> None:
        ...
        self.is_running = False
        self.context.logger.info("开始停止延时任务调度器...")
        ...
        self.context.logger.info("延时任务调度器已停止")
    ...
    async def delay_scheduler_loop(self) -> None:
        ...
        """
        self.context.logger.info("延时调度主循环已启动")
        
        # 首次启动时，短暂延时后立即触发一次调度检查
        await asyncio.sleep(0.2)
        self.context.logger.info("进程启动，初始化延时任务调度")
        
        while self.is_running:
            try:
                # 1. 从Redis获取下一个任务信息和等待时间
                lua_script: AsyncScript = self.context.lua_scripts["get_next_delay_task"]
                result = await lua_script(
                    keys=[self.context.get_global_key(GlobalKeys.DELAY_TASKS)], args=[]
                )
                status = result[0]
                
                # 🔍 详细日志：调试 Lua 脚本返回值
                self.context.logger.info(f"get_next_delay_task 返回: {result}")

                wait_milliseconds: float | None = None
                
                # 2. 根据状态决定下一步操作
                if status == "NO_TASK":
                    self.context.logger.info("没有延时任务，等待新任务通知...")
                    wait_milliseconds = None # 无限期等待
                elif status == "EXPIRED":
                    self.context.logger.info(f"发现过期任务 {result[2]}，立即处理")
    ...
    async def pubsub_listener(self) -> None:
        ...
        while self.is_running:
            try:
                ...
                if pubsub is None:
                    ...
                    self.context.logger.info("开始监听延时任务通知", channel=channel)
                ...
                async for message in pubsub.listen():
                    ...
                    if message["type"] == "message":
                        try:
                            ...
                            self.context.logger.info(f"收到延时任务通知: {notified_time}")
        ...
        if pubsub:
            try:
                await pubsub.close()
                self.context.logger.info("Pubsub连接已关闭")
            except Exception as e:
    ...
    async def try_process_expired_tasks(self) -> None:
        """
        尝试处理过期任务 - 逻辑已简化。
        这个函数现在只负责处理任务，不再关心后续的调度。
        """
        try:
            self.context.logger.info("开始处理过期延时任务")
            ...
            self.context.logger.info(f"Lua脚本执行结果, results: {results}, results_len: {len(results) if results else 0}")

            if results:
                self.context.logger.info("处理延时消息", count=len(results))
                for task_id, queue_name in results:
                    self.context.log_message_event("延时消息已就绪", task_id, queue_name)
            else:
                self.context.logger.info("尝试处理延时任务，但没有获取到任务")

        except Exception as e:
            self.context.log_error("处理过期任务失败", e)
        
        # 注意：这里的 finally 块中不再有任何调度逻辑，因为主循环会自动处理！
