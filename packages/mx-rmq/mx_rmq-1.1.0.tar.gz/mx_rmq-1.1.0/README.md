...
MX-RMQ 采用门面模式设计的统一日志系统，类似于 Java 的 SLF4J，支持多种日志后端的无缝切换。
...
```python
from mx_rmq import auto_configure_logging, get_logger

# 自动配置最佳可用的日志后端
backend = auto_configure_logging(level="INFO")
print(f"使用日志后端: {backend}")

# 获取日志器实例
logger = get_logger("MyApp")
logger.info("应用启动", version="1.0")
logger.error("发生错误", error_code=500)
```

### 支持的日志后端

#### 1. Standard Logging (默认)
Python 标准库日志系统，总是可用：

```python
from mx_rmq import setup_standard_logging, LoggerFactory

# 配置标准日志
setup_standard_logging(level="INFO")
LoggerFactory.set_adapter_type("standard")

logger = get_logger("MyApp")
logger.info("使用标准日志", backend="standard")
```

#### 2. Loguru (推荐)
如果安装了 loguru，会自动优先使用：
...
```python
from mx_rmq import setup_loguru_integration, LoggerFactory

# 配置 loguru
setup_loguru_integration(
    level="INFO",
    intercept_standard_logging=True  # 拦截标准 logging
)
LoggerFactory.set_adapter_type("loguru")

logger = get_logger("MyApp")
logger.info("使用 Loguru", backend="loguru", user_id=123)
```
...
```python
from mx_rmq import setup_structlog_integration, LoggerFactory
...
logger = get_logger("MyApp")
logger.info("使用结构化日志", user_id=123, action="login")
```
...
```python
from mx_rmq import auto_configure_logging, get_available_backends

# 检查可用后端
backends = get_available_backends()
print(f"可用后端: {backends}")
# 输出: {'standard': True, 'loguru': True, 'structlog': False}

# 自动选择最佳后端（优先级：loguru > structlog > standard）
backend = auto_configure_logging(level="INFO")

# 指定首选后端
backend = auto_configure_logging(prefer_backend="loguru", level="DEBUG")
```

#### 手动配置

```python
from mx_rmq import LoggerFactory, setup_standard_logging
...
```python
from mx_rmq import MQConfig, RedisMessageQueue, auto_configure_logging, get_logger

# 初始化日志系统
auto_configure_logging(level="INFO")
...
async def handle_order(payload: dict) -> None:
    ...
    try:
        ...
        logger.info("订单处理成功", order_id=order_id)
        
    except Exception as e:
        logger.error("订单处理失败", order_id=order_id, error=e)
        raise
...
# 使用门面模式日志接口
logger_service.logger.info("支付服务启动", version="2.0")
...
```python
from mx_rmq import setup_loguru_integration
...
```python
from mx_rmq import setup_standard_logging
...
```python
import os
from mx_rmq import auto_configure_logging

# 从环境变量读取配置
log_level = os.getenv("LOG_LEVEL", "INFO")
log_backend = os.getenv("LOG_BACKEND", "auto")

backend = auto_configure_logging(
    prefer_backend=log_backend if log_backend != "auto" else None,
    level=log_level
)
```
...
```python
# DEBUG: 详细的调试信息
logger.debug("计算折扣", original_price=100, discount_rate=0.1)

# INFO: 重要的业务事件
logger.info("订单支付成功", order_id="ORD_123", payment_id="PAY_456")

# WARNING: 潜在问题但不影响功能
logger.warning("库存不足", product_id="PROD_789", requested=10, available=5)

# ERROR: 错误需要关注
logger.error("支付网关错误", error=e, order_id="ORD_123")

# CRITICAL: 严重错误需要立即处理
logger.critical("数据库连接失败", database="order_db")
```
