...
class DispatchService:
    ...
    async def dispatch_messages(self, topic: str) -> None:
        ...
        self.context.logger.info(f"启动消息分发协程,topic:{topic},pending_key:{pending_key},processing_key:{processing_key}")

        while self.context.is_running():
            try:
                self.context.logger.debug(f"等待【Redis】消息分发，topic:{topic},pending_key:{pending_key}")
                ...
                # 卫语句：没有消息则继续下次循环
                if not message_id:
                    self.context.logger.debug("BLMOVE超时，无消息", topic=topic)
                    continue
                
                self.context.logger.info("成功获取消息", topic=topic, message_id=message_id)
