...
from cdk_project_constructs.utils import apply_tags
...
def reduce_items_number(values: dict[list, dict], standard_character_number: int = 4096) -> dict[list, dict] | dict[str, str]:
    """Counts the number of characters in the values.
...
def notifications_topic(self, pipeline_vars: PipelineVars) -> sns.Topic:
    ...
    notifications_sns_topic.add_subscription(topic_subscription=sns_subscriptions.EmailSubscription(email_address=pipeline_vars.ci_cd_notification_email))
...
def _configure_pipeline_slack_notifications(scope, notifications_sns_topic: sns.Topic | sns.ITopic, pipeline_vars: PipelineVars) -> None:
...
def _configure_pipeline_ms_teams_notifications(scope, notifications_sns_topic: sns.Topic | sns.ITopic, pipeline_vars: PipelineVars) -> None:
