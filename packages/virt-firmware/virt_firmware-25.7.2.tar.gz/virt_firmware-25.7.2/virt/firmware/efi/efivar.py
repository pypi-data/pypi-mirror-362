...
initial_ts  = datetime.datetime(2010, 1, 1, tzinfo = datetime.timezone.utc)
...
class EfiVar:
    ...
    def __init__(self, name,
        ...
        if isinstance(name, str):
            self.name = ucs16.from_string(name)
        else:
            self.name = name
    ...
    def sigdb_add_external_mgmt(self, guid):
        if self.sigdb is None:
            raise RuntimeError
        self.sigdb.add_external_mgmt(guid)
        self.data = bytes(self.sigdb)
        self.update_time(initial_ts)
    ...
    def sigdb_merge(self, mdb):
        if self.sigdb is None:
            raise RuntimeError
        if mdb.sigdb is None:
            raise RuntimeError
        self.sigdb.merge(mdb.sigdb)
        self.data = bytes(self.sigdb)
        self.update_time(initial_ts)
...
class EfiVarList(collections.UserDict):
    ...
    def add_external_mgmt(self, owner):
        var = self.get('PK')
        if not var:
            var = self.create('PK')
        logging.info('add external management guid to PK')
        var.sigdb_add_external_mgmt(owner)
    ...
    def enroll_platform_with_cert(self, cert,
        ...
        self.add_microsoft_kek_keys(ms_kek)
        self.add_dummy_dbx(guids.OvmfEnrollDefaultKeys)
    ...
    def enroll_managed_platform(self, ms_kek = 'all'):
        self.add_external_mgmt(guids.OvmfEnrollDefaultKeys)
        self.add_microsoft_kek_keys(ms_kek)
        self.add_dummy_dbx(guids.OvmfEnrollDefaultKeys)
    ...
    def add_microsoft_kek_keys(self, ms_kek = 'all'):
    ...
    @staticmethod
    def print_siglist(slist, print_hashes = True):
        ...
        if slist.x509:
            ...
            try:
                ...
                print('    no issuer CN')
        elif str(slist.guid) == guids.EfiCertSha256 and print_hashes:
    ...
    def print_normal(self, hexdump = False, print_hashes = True):
        for (key, var) in sorted(self.items()):
            ...
            if var.sigdb:
                for slist in var.sigdb:
                    self.print_siglist(slist, print_hashes)
            if hexdump:
