...
def main():
    ...
    pgroup.add_argument('--set-pk', '--set-pk-cert', dest = 'pk',  nargs = 2,
                        help = 'set PK to x509 cert, loaded in pem or der format ' +
                        'from FILE and with owner GUID',
                        metavar = ('GUID', 'FILE'))
    pgroup.add_argument('--add-kek', '--add-kek-cert', dest = 'kek',  action = 'append', nargs = 2,
                        help = 'add x509 cert to KEK, loaded in pem or der format ' +
                        'from FILE and with owner GUID, can be specified multiple times',
                        metavar = ('GUID', 'FILE'))
    pgroup.add_argument('--add-db', '--add-db-cert', dest = 'db',  action = 'append', nargs = 2,
                        help = 'add x509 cert to db, loaded in pem or der format ' +
                        'from FILE and with owner GUID, can be specified multiple times',
                        metavar = ('GUID', 'FILE'))
    ...
    pgroup.add_argument('--add-dbx', dest = 'add_dbx', action = 'append',
                        help = 'append dbx update from FILE', metavar = 'FILE')
    pgroup.add_argument('--add-dbx-cert', dest = 'dbx_cert',  action = 'append', nargs = 2,
                        help = 'add x509 cert to dbx, loaded in pem or der format ' +
                        'from FILE and with owner GUID, can be specified multiple times',
                        metavar = ('GUID', 'FILE'))
    pgroup.add_argument('--add-dbx-hash', dest = 'dbx_hash',  action = 'append', nargs = 2,
                        help = 'add sha256 HASH to dbx, with owner GUID, ' +
                        'can be specified multiple times',
                        metavar = ('GUID', 'HASH'))
    pgroup.add_argument('--add-mok', '--add-mok-cert', dest = 'mok', action = 'append', nargs = 2,
                        help = 'add x509 cert to MokList, loaded in pem or der format ' +
                        'from FILE and with owner GUID, can be specified multiple times',
                        metavar = ('GUID', 'FILE'))
    ...
    pgroup.add_argument('--enroll-mgmt', dest = 'enroll_mgmt',
                        action = 'store_true', default = False,
                        help = 'enroll using external management guid')
    ...
    pgroup.add_argument('--hashes', dest = 'hashes',
                        action = 'store_true', default = False,
                        help = 'print signature db sha256 hashes')
    ...
    pgroup.add_argument('--output-auth', dest = 'output_auth', type = str,
                        help = 'write *.auth files to DIR', metavar = 'DIR')
    ...
    if options.enroll_mgmt:
        varlist.enroll_managed_platform(ms_kek = options.ms_kek)
        if options.microsoft:
            varlist.add_microsoft_keys()
    ...
    if options.add_dbx:
        if not varlist.get('dbx'):
            varlist.add_dummy_dbx(guids.OvmfEnrollDefaultKeys)
        for item in options.add_dbx:
            logging.info('adding dbx update from %s', item)
            with open(item, 'rb') as f:
                dbxupdate = f.read()
            update = efivar.EfiVar(ucs16.from_string('dbx'),
                                   authdata = dbxupdate)
            varlist['dbx'].sigdb_merge(update)

    if options.dbx_cert:
        for item in options.dbx_cert:
            varlist.add_cert('dbx', item[0], item[1])

    if options.dbx_hash:
        for item in options.dbx_hash:
            varlist.add_hash('dbx', item[0], item[1])
    ...
    if options.print:
        if options.verbose:
            varlist.print_normal(options.hexdump, options.hashes)
        else:
    ...
    if options.output_auth:
        authfiles.AuthFilesVarStore.write_varstore(options.output_auth, varlist)
