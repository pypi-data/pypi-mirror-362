...
from huibiao_framework.execption.ocr import (
    ConvertToPdfResponseCodeError,
    ConvertToPdfResponseFormatError
)
...
class ConvertToPdfClient:
    ...
    def __init__(self, section_id: str = ""):
        self.CONVERT_TO_PDF_URL = ModelConfig.CONVERT_TO_PDF_URL
    ...
    async def init(self) -> "ConvertToPdfClient":
        """初始化客户端（可选，会自动调用）"""
    ...
    async def convert_to_pdf(self, file_path: str, pdf_path: str) -> Optional[str]:
        ...
        try:
            ...
            data = {"FileName": file_name, "Timeout": 1200}
            # 发送异步POST请求
            async with self.__session.post(self.CONVERT_TO_PDF_URL, data=data, files=file_dict) as resp:
                ...
                response_data =  resp.json() # 异步解析JSON
        ...
        if code != 200:
            logger.error(
                f"ConvertToPdf[{self.session_id}]响应失败，code={code}"
            )
