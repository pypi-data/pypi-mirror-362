...
class DocumentParseClient:
    ...
    async def convert_to_pdf(self, pdf_path: str, page_count: int, page_start: int, reqid: str) -> Optional[str]:
        """发送查询请求到转pdf模型（实例方法）"""
        ...
        try:
            ...
            # 发送异步POST请求
            async with self.__session.post(self.DOCUMENT_PARSER_TYY_URL,params=params, files=file_dict, headers=headers) as resp:
        ...
        if code != 200:
            logger.error(
                f"DocumentParse[{self.session_id}]响应失败，code={code}"
            )
            raise DocumentParseResponseCodeError(code)
        if "result" not in response_data:
            logger.error(f"DocumentParse[{self.session_id}]响应格式异常, 缺少result字段")
