...
from huibiao_framework.resource import (
    BatchFileResource,
    Resource,
    SingleFileResource,
    TaskResource,
...
SF = TypeVar("SF", bound=SingleFileResource)
...
class TaskResourceSyncMinio:
    ...
    def __init__(
        ...
        *,
        ...
        if task_resource is None:
            raise ValueError("Task resource cant not be None")
        self._task_resource = task_resource
        self._object_name_base = (
            f"task_resource/{task_resource.task_type}/{task_resource.task_id}/"
        )
    ...
    async def upload_file(self, resource_name: str, resource: Resource):
        if self.enabled:
            pass  # todo
