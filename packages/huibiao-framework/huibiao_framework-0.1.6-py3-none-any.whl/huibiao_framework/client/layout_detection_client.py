...
class LayoutDetectionClient:
    ...
    async def layout_detection(self, image_path: str, reqid: str) -> Optional[str]:
        ...
        try:
            ...
            # 发送异步POST请求
            async with self.__session.post(self.LAYOUT_DETECTION_TYY_URL, json=payload,
        ...
        if code != 200:
            logger.error(
                f"LayoutDetection[{self.session_id}]响应失败，code={code}"
            )
            raise LayoutDetectionResponseCodeError(code)
        if "result" not in response_data:
            logger.error(f"LayoutDetection[{self.session_id}]响应格式异常, 缺少result字段")
            raise LayoutDetectionResponseFormatError("响应体缺少result字段")
        if "layouts" not in response_data['result']:
            logger.error(f"LayoutDetection[{self.session_id}]响应格式异常, 缺少result.layouts字段")
