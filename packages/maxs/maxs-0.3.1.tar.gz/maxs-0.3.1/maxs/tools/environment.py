...
from maxs.tools import environment
...
def format_env_vars_table(
    env_vars: Dict[str, str], masked: bool, prefix: Optional[str] = None
) -> Table:
    """
...
def environment(tool: ToolUse, **kwargs: Any) -> ToolResult:
    ...
    default_content: List[ToolResultContent] = [
        {"text": "Unknown error in environment tool"}
    ]
    default_result = {
    ...
    env_vars_masked_default = (
        os.getenv("ENV_VARS_MASKED_DEFAULT", "true").lower() == "true"
    )
    ...
    if strands_dev:
        console.print(
            "[bold green]Running in BYPASS_TOOL_CONSENT mode - confirmation bypassed[/bold green]"
        )
    ...
    try:
        ...
        if action == "list":
            ...
            # Format rich table
            table = format_env_vars_table(
                dict(os.environ), masked=masked, prefix=prefix
            )
            ...
            console.print("")
            console.print(
                Panel(table, title=title, border_style="blue", box=box.ROUNDED)
            )
            ...
            display_value = (
                mask_sensitive_value(name, safe_value) if masked else safe_value
            )

            # Show operation preview
            console.print(
                format_operation_preview(action="get", name=name, value=display_value)
            )
            ...
            get_content: List[ToolResultContent] = [
                {"text": f"{name} = {final_display_value}"}
            ]
            return {
                "toolUseId": tool_use_id,
                "status": "success",
            ...
            if (
                needs_confirmation or True
            ):  # Always show preview regardless of confirmation mode
                console.print(
                    format_operation_preview(action="set", name=name, value=value)
                )
            ...
            if confirm.strip().lower() != "y":
                ...
                return {
                    ...
                    "status": "error",
                    "content": [
                        {"text": f"Operation cancelled by user, reason: {confirm}"}
                    ],
                }
            ...
            # Format content for return
            validate_content: List[ToolResultContent] = [
                {"text": f"Environment variable {name} is valid"}
            ]
            return {
                "toolUseId": tool_use_id,
                "status": "success",
            ...
            if needs_confirmation:
                ...
                console.print(
                    format_operation_preview(
                        action="delete", name=name, value=os.environ[name]
                    )
                )
            ...
            # Format content for return
            delete_content: List[ToolResultContent] = [
                {"text": f"Deleted environment variable: {name}"}
            ]
            return {
                "toolUseId": tool_use_id,
                "status": "success",
        ...
        exception_content: List[ToolResultContent] = [
            {"text": f"Environment tool error: {str(e)}"}
        ]
        return {
            "toolUseId": tool_use_id,
            "status": "error",
