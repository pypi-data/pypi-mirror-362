...
class Agent(AgentModel):
    ...
    @staticmethod
    async def create_in_db(access_token: str, agent: AgentModel) -> dict:
        try:
            agent_dict = agent.to_dict()
            if not agent.user_id:
                raise ValueError('User ID is required to create an agent')
            agent.validate()
            print('Creating agent with dict:', agent_dict)
            return await Agent.create_agent_in_db(access_token, agent_dict, agent.user_id)
        except Exception as e:
            print('Error creating agent', str(e))
            raise e

    @staticmethod
    async def create_agent_in_db(access_token: str, agent: dict, user_id: str) -> dict:
        try:
