    ...
    def run_optimization(
        ...
        for i in range(self.config.AE_NUM_TRIALS):
            ...
            try:
                ...
                # Legacy best trial tracking (for backward compatibility)
                is_best_trial = current_recall > best_recall
                if is_best_trial:
    ...
    def _update_best_trial_flags(self, trial_results: List[Dict], best_trial_idx: int) -> None:
        ...
        try:
            ...
            if 0 <= best_trial_idx < len(trial_results):
                ...
                best_recall = safe_float_extract(trial_results[best_trial_idx]['results']['soft_recall'])
                self.logger.info(f"Marked trial {best_trial_idx + 1} as best with recall: {best_recall:.4f}")
            else:
