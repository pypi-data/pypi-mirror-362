...
class IsotpSocket:
    ...
    def stop_stream(self) -> None:
        """
        Closes the gRPC stream if it is not already closed. A new IsotpSocket object will have to be created to start
        another stream.
        """
    ...
    def _receive_and_handle_response(self, request_payload: str, timeout: float | None) -> str:
        ...
        try:
            ...
            raise NoResponseException(request=request_payload, response=None, request_id=self._channel.request_id, response_id=self._channel.response_id)
