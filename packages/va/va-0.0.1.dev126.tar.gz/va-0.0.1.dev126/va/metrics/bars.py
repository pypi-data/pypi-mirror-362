...
def bar(new_entry_dict, score_type, work_dir, score_dir, plot_name):
    ...
    if score_type and new_entry_dict[score_type]:
        ...
        whole_res_low = df['resolution'].min()
        whole_res_hight = df['resolution'].max()
    else:
        to_whole = None
        to_whole_real = None
    ...
    if new_entry_dict['resolution']:
        ...
        relative_res_low = df1000['resolution'].min()
        relative_res_high = df1000['resolution'].max()
    else:
        to_two = None
        to_two_real = None
        to_two_counts = None
    ...
    return ((to_whole_real, to_whole_counts, whole_res_low, whole_res_hight), (to_two_real, to_two_counts, relative_res_low, relative_res_high))
