...
class DynamicNetworkCipher:
    ...
    def _hash_cpu(self, data: bytes) -> bytes:
    ...
    def _derive_kdf(self, salt_base: bytes, out_len: int) -> bytes:
        ...
        while len(output) < out_len:
            ...
            chunk = self._hash_cpu(self.key + current_salt)
            output += chunk
            counter += 1
    ...
    def _generate_gpu_mask(self, data_batch, mask_key):
        pde_a = (mask_key[0] | 1)
        pde_b = mask_key[1]
        knot_r = mask_key[2] % 8
        game_key = self.backend.array(numpy.frombuffer(mask_key[3:3+self.half_size], dtype=numpy.uint8))
        temp_data = pde_transform(data_batch, pde_a, pde_b, self.backend)
        temp_data = knot_transform(temp_data, knot_r, self.backend)
        return self.backend.bitwise_xor(temp_data, game_key)

    def _F(self, half_block_batch, round_idx: int):
        params = self.round_params[round_idx]
        
        if self.backend.__name__ == 'cupy':
            mask = self._generate_gpu_mask(half_block_batch, params['mask_key'])
        else:
            data_for_hash = half_block_batch.tobytes()
    ...
    def decrypt_all_blocks(self, all_blocks_arr):
        R_batch, L_batch = all_blocks_arr[:, :self.half_size], all_blocks_arr[:, self.half_size:]
