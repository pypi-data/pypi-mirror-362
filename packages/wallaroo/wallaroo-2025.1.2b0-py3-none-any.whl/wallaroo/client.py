...
class Client(object):
    ...
    def get_user_by_email(self, email: str) -> Optional[User]:
        ...
        headers = _get_base_headers(auth_token=self.auth._bearer_token_str())
        headers.update(
            {
                "Content-Type": "application/json",
            }
        )
        resp = _make_get_request(
            api_endpoint=self.api_endpoint, path=url, headers=headers
        )
        jresp = resp.json()
    ...
    def deactivate_user(self, email: str) -> None:
        ...
        get_user_response = _make_get_request(
            api_endpoint=self.api_endpoint, path=url_path, headers=headers
        )

        cur_user_rep = get_user_response.json()
        cur_user_rep["enabled"] = False

        resp = _make_put_request(
    ...
    def activate_user(self, email: str) -> None:
        ...
        resp = _make_put_request(
            api_endpoint=self.api_endpoint, path=url, headers=headers, json=cur_user_rep
        )
    ...
    def list_users(self) -> List[User]:
        ...
        resp = User.list_users(
            auth_token=self.auth._bearer_token_str(),
            api_endpoint=self.api_endpoint,
        )
    ...
    def _wait_for_model(self, model_version: ModelVersion) -> ModelVersion:
        ...
        while datetime.now() < expire_time:
            ...
            # Print dots with proper line handling
            sys.stdout.write(".")
            sys.stdout.flush()
            last_status = status
    ...
    def get_topic_name(self, pipeline_pk_id: int) -> str:
        return _make_post_request(
            api_endpoint=self.api_endpoint,
            path="v1/api/plateau/get_topic_name",
            headers=_get_base_headers(auth_token=self.auth._bearer_token_str()),
            json={
