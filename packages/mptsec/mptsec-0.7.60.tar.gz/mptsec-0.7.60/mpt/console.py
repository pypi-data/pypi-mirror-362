...
def show_focused_app():

    # adb shell dumpsys activity activities | grep -i "focusedApp\|mCurrentFocus" | grep --color=always -E '[a-zA-Z0-9\.]+/[a-zA-Z0-9\.]+'
    output = functions.run_command(
        command="adb shell dumpsys activity activities | grep -i \"focusedApp\|mCurrentFocus\"", print_output=False, return_output=True)

    if isinstance(output, list):
        output = ''.join(output)
    match = re.search(r'(\b[\w\.]+/[\w\.]+)', output)
    if match:
        result = match.group(1)
        package = result.split('/')[0]
        activity = result.split('/')[1]
        log.info("Identifying current application on top ...")
        print(result)
        print(f"Running application:   {Style.BRIGHT}{Fore.CYAN}{package}{Style.RESET_ALL}")
        print(f"Current activity:      {Style.BRIGHT}{Fore.CYAN}{activity}{Style.RESET_ALL}")
...
def cli():
    ...
    app.add_argument('-if', '--info-focused-app', help='Show current activity and focused application on the device', action='store_true')
