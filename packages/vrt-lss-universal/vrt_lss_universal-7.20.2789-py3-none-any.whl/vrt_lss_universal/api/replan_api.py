...
class ReplanApi:
    ...
    def run_replan_refine(
        ...
        Cleaning up data for re-planning before calculation:   * If one entity is referenced by several facts, only the latest one by the time field is taken into account; if the time is the same, a random one is taken into account.   * All facts that occurred later than `actualize_settings.current_time` are deleted.   * If `plan_new_orders: false` is specified - all orders that are not in the specified trips are removed from the original task.   * If `create_new_trips: false` is specified - all performers and transport that are not in the specified trips are removed from the original task, and unplanned shifts of performers and transport that are in the specified trips are also removed.   * Afterwards, [cleaning up data for planning](#operation/run_plan_refine) is performed.  As a result, a task for planning is returned. 
