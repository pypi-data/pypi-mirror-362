...
"""
    ...
    The version of the OpenAPI document: 7.20.2789
...
class TripStatistics(BaseModel):
    ...
    roundtrip_statistics: Annotated[List[RoundtripStatistics], Field(min_length=0, max_length=15001)] = Field(description="Statistics per roundtrip during the trip.")
    ...
    __properties: ClassVar[List[str]] = ["trip_key", "general_statistics", "load_statistics", "stop_statistics", "roundtrip_statistics", "attributes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        if self.stop_statistics:
            for _item_stop_statistics in self.stop_statistics:
                if _item_stop_statistics:
                    _items.append(_item_stop_statistics.to_dict())
            _dict['stop_statistics'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in roundtrip_statistics (list)
        _items = []
        if self.roundtrip_statistics:
            for _item_roundtrip_statistics in self.roundtrip_statistics:
                if _item_roundtrip_statistics:
                    _items.append(_item_roundtrip_statistics.to_dict())
            _dict['roundtrip_statistics'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "roundtrip_statistics": [RoundtripStatistics.from_dict(_item) for _item in obj["roundtrip_statistics"]] if obj.get("roundtrip_statistics") is not None else None,
