...
class BrassRunner:
    ...
    def __init__(self, config: BrassConfig):
        ...
        self.dcp = DCPAdapter(storage=self.storage, dcp_path=self.project_root)
    ...
    async def run_scout_analysis(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 🔧 UNIVERSAL ISSUE RESOLUTION DETECTION: Detect and mark resolved issues
            try:
                # Collect all current TODO findings from the analysis results
                current_todo_findings = []
                for file_path in files_to_analyze:
                    try:
                        # Re-run just the TODO detection to get TODOFinding objects
                        from .agents.scout.todo_detector import TODODetector
                        detector = TODODetector()
                        file_todos = detector.scan_file(str(file_path))
                        current_todo_findings.extend(file_todos)
                    except Exception as e:
                        logger.debug(f"Failed to collect TODOs from {file_path} for resolution detection: {e}")
                        continue
                
                # TODO Resolution Detection (backward compatibility)
                if current_todo_findings:
                    resolved_todos = self.storage.detect_resolved_issues(current_todo_findings, 'todo')
                    if resolved_todos > 0:
                        logger.info(f"🔄 TODO Resolution: Marked {resolved_todos} TODOs as resolved")
                else:
                    logger.debug("No current TODO findings for resolution detection")
                
                # Universal Resolution Detection for other issue types
                # Use the observations from Scout analysis to detect resolved security/code issues
                if observations:
                    # Create mock finding objects from Scout observations for resolution detection
                    class MockFinding:
                        def __init__(self, file_path, line_number):
                            self.file_path = file_path
                            self.line_number = line_number
                    
                    # Collect current findings by type for resolution detection
                    current_findings_by_type = {}
                    for obs in observations:
                        obs_type = obs.get('type')
                        if obs_type and obs_type != 'todo':  # TODOs already handled above
                            data = obs.get('data', {})
                            file_path = data.get('file_path')
                            line_number = data.get('line_number')
                            
                            if file_path and line_number:
                                if obs_type not in current_findings_by_type:
                                    current_findings_by_type[obs_type] = []
                                current_findings_by_type[obs_type].append(
                                    MockFinding(file_path, line_number)
                                )
                    
                    # Run resolution detection for each type with current findings
                    total_universal_resolved = 0
                    for obs_type, findings in current_findings_by_type.items():
                        try:
                            resolved_count = self.storage.detect_resolved_issues(findings, obs_type)
                            if resolved_count > 0:
                                logger.info(f"🔄 {obs_type.replace('_', ' ').title()} Resolution: Marked {resolved_count} issues as resolved")
                                total_universal_resolved += resolved_count
                        except Exception as e:
                            logger.warning(f"Resolution detection failed for {obs_type}: {e}")
                    
                    if total_universal_resolved > 0:
                        logger.info(f"🎯 Universal Resolution Complete: {total_universal_resolved} total issues resolved")
                    
            except Exception as e:
                logger.error(f"Issue resolution detection failed (continuing): {e}")
                # Continue execution - don't fail the entire analysis
            ...
            if observations:
                ...
                for obs in observations:
                    try:
                        ...
                        logger.error(f"Failed to store observation in DCP: {e}")
                ...
                try:
                    ...
                    if obs.get('classification') in ['critical', 'important']:
                        logger.info(
                            ...
                            f"{obs.get('file_path', obs.get('file', 'unknown'))}: "
    ...
    def run_scout(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                observations = loop.run_until_complete(self.run_scout_analysis())
