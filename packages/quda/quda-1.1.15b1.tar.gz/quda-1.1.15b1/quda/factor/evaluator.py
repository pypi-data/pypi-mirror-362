...
class Evaluator:
    ...
    def read_data(fac: 'Factor', date_cond: str, time_cond: str, eager: bool = True):
        ...
        data = database.sql(query, lazy=not eager).drop(FIELD_VERSION).with_columns(pl.col(FIELD_DATE).cast(pl.Utf8))
        return data
    ...
    def get_history(fac: 'Factor', ctx: 'FactorContext') -> pl.DataFrame:
        ...
        cols = data.columns
        if ctx.avoid_future and val_time < fac.insert_time:
        ...
        data = data.with_columns(pl.lit(ctx.time).alias(FIELD_TIME))
        return data[cols].sort(INDEX)
