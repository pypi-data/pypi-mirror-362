...
class Connection(api.Connection):
    _CA_QUERY_FIELDS = {'common_name', 'key_algorithm', 'signature_algorithm', 'issuer_id'}
    _CERT_QUERY_FIELDS = {'common_name', 'key_algorithm', 'signature_algorithm', 'issuer_id'}

    def _validate_filters(self, filters, enabled_items):
        if filters is None:
            filters = dict()
    ...
    def get_certificate_authorities(self, filters=None, limit=None, marker=None, sort_key=None, sort_dir=None):
        ...
        query = _base_query(models.PrivateCertificateAuthorityModel)
        query = self._add_certificate_authorities_filters(query, filters)
        return _paginate_query(models.PrivateCertificateAuthorityModel, limit, marker_obj, sort_key, sort_dir, query)
    ...
    def get_certificate_authority(self, ca_id):
        try:
            with _session_for_read() as session:
                query = session.query(models.PrivateCertificateAuthorityModel).filter_by(id=ca_id)
                return query.one()
    ...
    def create_certificate_authority(self, values):
        ...
        ca = models.PrivateCertificateAuthorityModel()
        ca.update(values)
    ...
    def get_certificate(self, cert_id):
        try:
            with _session_for_read() as session:
                query = session.query(models.PrivateCertificateModel).filter_by(id=cert_id)
                return query.one()
        except NoResultFound:
    ...
    def get_certificates(self, filters=None, limit=None, marker=None, sort_key=None, sort_dir=None):
        ...
        query = _base_query(models.PrivateCertificateModel)
        query = self._add_certificates_filters(query, filters)
        return _paginate_query(models.PrivateCertificateModel, limit, marker_obj, sort_key, sort_dir, query)
    ...
    def create_certificate(self, values):
        ...
        cert = models.PrivateCertificateModel()
        cert.update(values)
        with _session_for_write() as session:
            session.add(cert)
            session.flush()
