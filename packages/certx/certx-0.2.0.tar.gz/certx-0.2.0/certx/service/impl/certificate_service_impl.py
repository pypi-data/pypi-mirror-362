...
from certx.provider import crypto, key, x509
...
class CertificateServiceImpl(CertificateService):
    ...
    def create_certificate_authority(self, ca_option) -> models.PrivateCertificateAuthority:
        ...
        ca_cert = x509.get_provider(key_algorithm, signature_algorithm).generate_ca_certificate(
    ...
    def _valid_sub_ca_option(self, ca_option):
        ...
        if not algorithm_util.validate_key_algorithm(key_algorithm, ca_model.key_algorithm) \
                or not algorithm_util.validate_signature_algorithm(signature_algorithm, ca_model.signature_algorithm):
        ...
        return self._load_ca_cert_and_key(ca_model)

    @staticmethod
    def _load_ca_cert_and_key(ca_model):
        _key_provider = key.get_provider(ca_model.key_algorithm)
        ...
        _ca_key_provider = key.get_provider(ca_model.key_algorithm)
    ...
    def create_certificate(self, cert_option) -> models.PrivateCertificate:
        ...
        key_usage = self._build_x509_key_usage(cert_option.get('key_usage'))
        ...
        ca_cert, ca_key = self._load_ca_cert_and_key(ca_model)
    ...
    def export_certificate(self, cert_id, export_option) -> models.CertificateContent:
        ...
        return models.CertificateContent(
            ...
            certificate_chain=self._get_certificate_chain(ca_model))

    def _get_certificate_chain(self, ca_model):
        chain = []
        ...
        if ca_model.issuer_id:
            chain.extend(self._get_certificate_chain(self.get_certificate_authority(ca_model.issuer_id)))

        chain.reverse()
        return chain

    @staticmethod
    def _build_x509_key_usage(key_usage) -> KeyUsage:
        if not key_usage:
            return None
        ...
        return KeyUsage(key_usage.get('digital_signature'),
