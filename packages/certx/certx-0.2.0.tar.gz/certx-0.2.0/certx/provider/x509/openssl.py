...
from certx.common.model.models import DistinguishedName, SignatureAlgorithm, Validity, KeyAlgorithm
...
from certx.utils import executil
from certx.utils import generator

logger = logging.getLogger(__name__)

openssl_file_directory = os.path.normpath(CONF.openssl.path)

_SIG_ALG_MAP = {
    SignatureAlgorithm.SM3: 'SM3'
}
...
def detect_openssl():
    ...
    try:
        result = executil.execute('openssl version')
...
def make_openssl_directory():
    ...
    if os.path.exists(openssl_file_directory):
        return

    os.makedirs(openssl_file_directory)
...
def generate_openssl_file_path(file_name=None):
    ...
    return os.path.join(openssl_file_directory, file_name)
...
class OpensslGMCertificateProvider(CertificateProvider):
    ...
    def _generate_ca_certificate(self, dn, private_key, validity, extension_param: OpensslExtensionParamBuilder):
        ...
        ca_cert_file = os.path.join(openssl_file_directory, '{}.crt'.format(generator.gen_uuid()))
        ...
        ca_cmd_result = executil.execute(ca_cmd)
    ...
    def _generate_csr(self, private_key, dn: DistinguishedName):
        ...
        csr_cmd_result = executil.execute(csr_cmd)
    ...
    def _generate_certificate(self, ca_cert, ca_key, cert_dn: DistinguishedName, cert_private_key, validity: Validity,
        ...
        cert_file = os.path.join(openssl_file_directory, '{}.crt'.format(generator.gen_uuid()))
        ...
        cert_cmd_result = executil.execute(cert_cmd)
    ...
    def _delete_tmp_file(cert: OpensslX509Certificate):
        ...
        enable_delete_temp_file = CONF.openssl.enable_delete_temp_file
