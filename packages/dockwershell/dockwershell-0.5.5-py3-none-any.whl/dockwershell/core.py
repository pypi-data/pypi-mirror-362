...
class Docker:
    ...
    async def run(self, cmd: str | list[str], headless: bool = True, **kwargs) -> CMDResult | None:
        ...
        return await self.debian.run(cmds, headless=headless)
...
async def docker(**kwargs) -> Docker:
    """
    ...
    try:
        ...
        _docker_singleton_instance = await Docker(**kwargs)
        if DEBUG:
