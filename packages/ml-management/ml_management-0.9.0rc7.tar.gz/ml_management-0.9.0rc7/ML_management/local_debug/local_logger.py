...
class LocalLogger:
    ...
    def prepare_job_dir(self):
        job_name = variables.secret_uuid
        job_dir = os.path.join(self.registry_path, "jobs", job_name)
        if os.path.exists(job_dir):
            shutil.rmtree(job_dir)

    def log_metrics(self, metrics: list[dict]):
        job_name = variables.secret_uuid
        metrics_dir = os.path.join(self.registry_path, "jobs", job_name, "metrics")
    ...
    def log_params(self, params: Dict[str, str]):
        ...
        job_name = variables.secret_uuid
        params_dir = os.path.join(self.registry_path, "jobs", job_name, "params")
    ...
    def log_artifact(self, local_path: str, artifact_path: Optional[str] = None):
        job_name = variables.secret_uuid
        artifact_dir = os.path.join(self.registry_path, "jobs", job_name, "artifacts")
