...
from ML_management.graphql.schema import AvailableResources, ExecutionJob, Param
...
def job_by_name(name: str) -> ExecutionJob:
    ...
    job = send_graphql_request(op, json_response=False).job_from_name
...
def job_metric_by_name(name: str) -> pd.DataFrame:
    ...
    """
    if get_debug():
        metrics = LocalRegistry().get_metrics(name)
...
def available_metrics(job_name: str) -> List[str]:
    ...
    """
    if get_debug():
        return list(LocalRegistry().get_metrics(job_name).keys())
...
def metric_history(
    ...
    **kwargs,
    ...
    """
    if get_debug():
        metrics = LocalRegistry().get_metrics(job_name).get(metric_name, {})
    else:
        op = Operation(schema.Query)
        base = op.job_from_name(name=job_name)
...
def add_ml_job(
    ...
    resources: ResourcesForm | None = None,
    ...
    visibility: Union[Literal[VisibilityOptions.PRIVATE, VisibilityOptions.PUBLIC], VisibilityOptions]
...
def set_job_visibility(
    name: str, visibility: Union[Literal[VisibilityOptions.PRIVATE, VisibilityOptions.PUBLIC], VisibilityOptions]
) -> ExecutionJob:
    """
    Set job visibility.
    ...
    """
    op = Operation(schema.Mutation)
    set_visibility = op.update_job(name=name, visibility=VisibilityOptions(visibility).name)
    set_visibility.name()
