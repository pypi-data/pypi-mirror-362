...
def download_artifacts_by_name_version(
    ...
    model_type: Union[
        Literal[ModelType.MODEL, ModelType.EXECUTOR, ModelType.DATASET_LOADER], ModelType
    ] = ModelType.MODEL,
    verbose: bool = True,
) -> str:
    ...
    model_type: Union[Literal[ModelType.MODEL, ModelType.EXECUTOR, ModelType.DATASET_LOADER] , ModelType]
    ...
    if variables.NO_CACHE:
        local_path = CachedLoader.download_artifacts_by_name_version(
            name=name, version=version, model_type=model_type, path=path, dst_path=dst_path, verbose=verbose
        )
    else:
...
def download_job_artifacts(name: str, path: str = "", dst_path: Optional[str] = None, verbose: bool = True) -> str:
    ...
    """
    if variables.NO_CACHE:
        local_path = CachedLoader.download_job_artifacts(name, path, dst_path, verbose=verbose)
    else:
        local_path = CachedLoader().cached_download_job_artifacts(name, path, dst_path, verbose=verbose)
    return local_path


def download_job_metrics(name: str, dst_path: Optional[str] = None, verbose: bool = True) -> str:
    ...
    return _request_download_artifacts(url, params, dst_path, f"{name}_metrics", verbose=verbose)
...
def _load_model_type(
    ...
    if variables.NO_CACHE:
        local_path = CachedLoader.download_artifacts_by_name_version(
            name=name, version=version, model_type=model_type, path="", dst_path=dst_path, verbose=verbose
        )
    else:
        local_path = CachedLoader().cached_download_artifacts_by_name_version(
            name=name, version=version, model_type=model_type, path="", dst_path=dst_path, verbose=verbose
...
def load_dataset(
    ...
    return _load_model_type(
        name, version, ModelType.DATASET_LOADER, install_requirements, dst_path, kwargs_for_init, verbose
    )
...
def load_model(
    ...
    """
    return _load_model_type(name, version, ModelType.MODEL, install_requirements, dst_path, kwargs_for_init, verbose)
...
def load_executor(
    ...
    """
    return _load_model_type(name, version, ModelType.EXECUTOR, install_requirements, dst_path, verbose=verbose)
...
class CachedLoader:
    ...
    def cached_download_job_artifacts(
        self, name: str, path: str = "", dst_path: Optional[str] = None, verbose: bool = True
    ) -> str:
        ...
        local_path = self.download_job_artifacts(name, path, dst_path, verbose=verbose)
