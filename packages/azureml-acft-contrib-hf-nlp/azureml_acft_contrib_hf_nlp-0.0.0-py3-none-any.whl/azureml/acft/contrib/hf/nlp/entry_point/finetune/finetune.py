...
def get_parser():
    ...
    parser.add_argument(
        "--save_on_singularity_preemption",
        type=str2bool,
        default="false",
        help="Checkpoint is saved on Singularity Preemption. Overrides save_strategy.",
    )
    parser.add_argument(
        "--save_total_limit",
        type=int,
...
def finetune(args: Namespace):
    ...
    if args.save_on_singularity_preemption:
        logger.info("Setting save strategy to save on singularity preemption")
        # Overriding save strategy to evaluation strategy. Reason: besides checkpoint save on Singularity Preemption,
        # we still need to support checkpoint save after every evaluation since load_best_model_at_end=True
        args.save_strategy = args.evaluation_strategy
        args.save_steps = args.eval_steps
    elif args.save_strategy == SaveStrategy.EVALUATION_STRATEGY:
