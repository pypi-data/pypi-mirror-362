...
class BaseModelRadar:
    ...
    def __init__(
        ...
        self.models = (
            self._get_model_names(cv_df) if model_names is None else model_names
        )
...
class ModelRadarAcrossId:
    ...
    def get_hard_uids(self, err_df: pd.DataFrame, return_df: bool = True):
        ...
        self.hardness_threshold = err_df[self.reference].quantile(
            self.hardness_quantile
        )
    ...
    def expected_shortfall(
        self, err_df: pd.DataFrame, return_plot: bool = False, **kwargs
    ):
        ...
        **kwargs
        ...
        shortfall = err_df.apply(
            lambda x: x[x > x.quantile(self.cvar_quantile)].agg(self.agg_func)
        )
        ...
        if return_plot:
            ...
            plot = ModelRadarPlotter.error_barplot(
                data=shortfall_df, x="Model", y="Exp. Shortfall", **kwargs
            )
...
class ModelRadar(BaseModelRadar):
    ...
    def evaluate(
        ...
        **kwargs
        ...
        if keep_uids:
            scores_df = scores_df.groupby(self.id_col).agg(
                self.agg_func, numeric_only=True
            )
        else:
            scores_df = scores_df.drop(
                columns=[self.id_col, self.COLUMNS.get("metric")]
            ).agg(self.agg_func, numeric_only=True)
            ...
            if return_plot:
                ...
                plot = ModelRadarPlotter.error_barplot(
                    data=scores_df, x="Model", y="Error", **kwargs
                )
    ...
    def evaluate_by_horizon(
        ...
        if group_by_freq:
            ...
            scores_df[self.COLUMNS.get("horizon")] = np.arange(
                1, scores_df.shape[0] + 1
            )
    ...
    def evaluate_by_horizon_bounds(
        self,
        cv: Optional[pd.DataFrame] = None,
        return_plot: bool = False,
        ...
        **kwargs
        ...
        if self.COLUMNS.get("cutoff") in cv_:
            sorted_cv = cv_.sort_values(
                [self.id_col, self.COLUMNS.get("cutoff"), self.COLUMNS.get("horizon")]
            )
        else:
            sorted_cv = cv_.sort_values([self.id_col, self.COLUMNS.get("horizon")])
        ...
        errors_combined = errors_first_df.merge(
            errors_last_df, on=self.DF_RESULT_COLUMNS[0]
        )
        ...
        if return_plot:
            ...
            plot = ModelRadarPlotter.error_by_horizon_fl(
                errors_combined_lg, model_cats=plot_model_cats, **kwargs
            )
    ...
    def evaluate_by_anomaly(
        ...
        for uid, df_uid in cv_.groupby(self.id_col):
            ...
            if has_anomalies:
                uid_ = (
                    df_uid.loc[df_uid[anomaly_col] > 0, :]
    ...
    def evaluate_by_group(
        ...
        **kwargs
        ...
        if return_plot:
            ...
            plot = ModelRadarPlotter.error_by_group(
                data=results, model_cats=plot_model_cats, **kwargs
            )
