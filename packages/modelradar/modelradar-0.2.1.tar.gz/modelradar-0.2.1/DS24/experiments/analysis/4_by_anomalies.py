...
plot11 = Plots.average_error_barplot(summary) + p9.labs(y=f"SMAPE in {expr}")
plot12 = Plots.average_error_barplot(shortfall) + p9.labs(
    y=f"Expected shortfall in {expr}"
)
# plot2 = Plots.error_dist_by_model(df_m)
plot13 = Plots.error_dist_by_model(df_ranks_m) + p9.labs(
    y=f"Rank distribution on {expr}", x=""
)
