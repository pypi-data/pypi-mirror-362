...
plot7_0 = Plots.error_distribution_baseline(
    df=df_all, baseline="SNaive", thr=eval_wf.hard_thr
)
plot7 = Plots.average_error_barplot(df_avg) + p9.labs(y="SMAPE on difficult series")

plot8 = Plots.average_error_barplot(shortfall) + p9.labs(
    y="Expected shortfall on difficult series"
)
# plot2 = Plots.error_dist_by_model(df_m)
plot9 = Plots.error_dist_by_model(df_ranks_m) + p9.labs(
