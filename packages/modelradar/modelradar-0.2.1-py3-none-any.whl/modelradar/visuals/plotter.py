...
class ModelRadarPlotter:
    ...
    def error_barplot(
        ...
        if isinstance(fill_, dict):
            plot = p9.ggplot(data=df, mapping=p9.aes(x=x, y=y, fill=x)) + p9.geom_bar(
                position="dodge", stat="identity", width=0.9
            )
        else:
            plot = p9.ggplot(data=df, mapping=p9.aes(x=x, y=y)) + p9.geom_bar(
                position="dodge", stat="identity", width=0.9, fill=fill_
            )
        ...
        if isinstance(fill_, dict):
            plot = plot + p9.scale_fill_manual(values=fill_) + p9.guides(fill="none")
    ...
    def error_by_horizon_fl(
        ...
        if isinstance(fill_, dict):
            ...
            plot = (
                p9.ggplot(
                    data=data, mapping=p9.aes(x="Model", y="Error", group="Horizon")
                )
                + p9.facet_grid("~Horizon")
                + p9.geom_bar(position="dodge", stat="identity", width=0.9, fill=fill_)
            )
    ...
    def error_by_horizon(data: pd.DataFrame, break_interval: int = 3):
        ...
        breaks = np.unique(
            [1, *np.arange(break_interval, max_h, break_interval), max_h]
        )
    ...
    @staticmethod
    def winning_ratios(
        data: pd.DataFrame, reference: str, extra_theme_settings: Optional = None
    ):
        ...
        plot = (
            p9.ggplot(data, p9.aes(fill="Result", y="Probability", x="Model"))
        ...
        if isinstance(fill_, dict):
            plot = (
                p9.ggplot(
                    data=data_m,
                    mapping=p9.aes(
                        x="Model", y="value", group="variable", fill="Model"
                    ),
                )
            ...
            plot = (
                p9.ggplot(
                    data=data_m, mapping=p9.aes(x="Model", y="value", group="variable")
                )
                + p9.facet_grid("~variable")
                + p9.geom_bar(position="dodge", stat="identity", width=0.9, fill=fill_)
            )
    ...
    def error_distribution(
        cls, data: pd.DataFrame, model_cats: List[str], log_transform: bool = False
    ):
        ...
        data_melted["Model"] = pd.Categorical(
            data_melted["Model"].values.tolist(), categories=model_cats
        )
    ...
    def error_histogram(
        ...
        plot = (
            p9.ggplot(df)
            + p9.aes(x=x_col)
            + p9.geom_histogram(alpha=0.95, bins=30, color="black", fill=fill_color)
            + THEME
            + p9.labs(x=f"Error distribution of {x_col}", y="Count")
        )

        if x_threshold is not None:
            plot = plot + p9.geom_vline(xintercept=x_threshold, colour="red", size=1)
    ...
    def multidim_parallel_coords(cls, df: pd.DataFrame, values: str = "raw"):
        ...
        if values == "normalize":
            ...
            plot_df["value"] = plot_df.groupby("variable")["value"].transform(
                cls._normalize
            )
        elif values == "rank":
            y_name = "Rank scores"
        ...
        plot = (
            p9.ggplot(
                data=plot_df,
                mapping=p9.aes(x="variable", y="value", group="Model", color="Model"),
            )
...
class SpiderPlot:
    ...
    @classmethod
    def create_plot(
        cls, df: pd.DataFrame, models_col: str = "Model", values: str = "raw", **kwargs
    ):
        ...
        **kwargs
        ...
        radar_df, circle_df, axis_df, variables, angles = cls._get_plot_data(
            df=plot_df, models_col=models_col
        )
    ...
    def _make_plot(
        ...
        plot = (
            ...
            + p9.geom_point(
                radar_df[radar_df["group"] != len(variables)],
                p9.aes(x="x", y="y", color="Model"),
                size=3,
            )
            + p9.annotate(
                "text",
                x=[0.95 * np.cos(a) for a in angles],
    ...
    def _get_plot_data(cls, df: pd.DataFrame, models_col: str):
        ...
        for r in np.linspace(0.2, 1, 5):
            ...
            temp_df = pd.DataFrame({"x": circle_x, "y": circle_y, "r": r})
        ...
        for angle in angles:
            temp_df = pd.DataFrame(
                {"x": [0, np.cos(angle)], "y": [0, np.sin(angle)], "angle": angle}
            )
