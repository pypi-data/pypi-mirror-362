...
# Getting Login Token
# -------------------------------------------------------------------------------------------------------------------------------
LOGIN_CACHE = {}  # Global cache

def login(env_path, format="PLAIN"):
    global LOGIN_CACHE
    ...
    def decode_base64(encoded_value):
        try:
            ...
            raise ValueError("Invalid Base64-encoded credential")
    
    if format == "ENCODED":
        username_enc = creds.get("FERMA_USERNAME_ENC")
        password_enc = creds.get("FERMA_PASSWORD_ENC")

        if not username_enc or not password_enc:
            raise ValueError("Missing encoded credentials in .env file")

        username = decode_base64(username_enc)
        password = decode_base64(password_enc)
    ...
    response = requests.post("https://admin-portal.ferma.ai/users/login", json={
        "username": username,
        "password": password,
        "persist": 1
    })
...
# --------------------------------------------------------------------------------------------------------------------------------
def get_all_sessions(CONGRESS_ID: str, HEADERS: dict = None) -> pd.DataFrame:
    if HEADERS is None:
        if not LOGIN_CACHE:
            raise RuntimeError("No login session found. Please call login(env_path) first.")
        HEADERS = list(LOGIN_CACHE.values())[0]
...
# --------------------------------------------------------------------------------------------------------------------------------
def get_skg(CONGRESS_ID: str, HEADERS: dict = None) -> pd.DataFrame:
    if HEADERS is None:
        if not LOGIN_CACHE:
            raise RuntimeError("No login session found. Please call login(env_path) first.")
        HEADERS = list(LOGIN_CACHE.values())[0]  # Use cached headers
...
# --------------------------------------------------------------------------------------------------------------------------------
def get_tweets(CONGRESS_ID: str, HEADERS: dict = None) -> pd.DataFrame:
    if HEADERS is None:
        if not LOGIN_CACHE:
            raise RuntimeError("No login session found. Please call login(env_path) first.")
        HEADERS = list(LOGIN_CACHE.values())[0]  # Use cached headers
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            sessions_metadata_df = get_all_sessions(CONGRESS_ID)
            ...
            priority_df = get_priority(CONGRESS_ID)
...
# --------------------------------------------------------------------------------------------------------------------------------
def get_priority(CONGRESS_ID: str, include=None, exclude=None, HEADERS: dict = None) -> pd.DataFrame:
    if HEADERS is None:
        if not LOGIN_CACHE:
            raise RuntimeError("No login session found. Please call login(env_path) first.")
        HEADERS = list(LOGIN_CACHE.values())[0]  # Use cached headers
    ...
    for _, row in tqdm(planner_df.iterrows(), total=len(planner_df), desc="Planners"):
        ...
        try:
            planner_file_response = requests.get(row['link'], headers=HEADERS)
    ...
    sessions_metadata_df['start_date'] = pd.to_datetime(sessions_metadata_df['start_date']).dt.strftime('%Y-%m-%d %H:%M:%S')
    sessions_metadata_df['end_date'] = pd.to_datetime(sessions_metadata_df['end_date']).dt.strftime('%Y-%m-%d %H:%M:%S')
