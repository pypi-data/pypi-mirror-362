...
"""
from akida import Dense1D, Dense2D, FullyConnected, ActivationType
...
def _set_dense_v2_variables(ak_layer, dense):
    ...
    This function converts the variables of a dense block of Keras layers and sets them into
    the corresponding variables of the equivalent Akida Dense1D/Dense2D layer.
...
def _parse_dense_v1(layer):
    ...
    Args:
        layer (:obj:`tf.keras.Layer`): the quantizeml.QuantizedDense layer to parse.
...
def _parse_dense_v2(layer):
    """Parses a quantizeml.QuantizedDense parameters that target the akida v2 Dense1D or Dense2D
    layer.
...
def _get_dense_layer(block):
    ...
    Args:
        block (list(:obj:`tf.keras.Layer`)): the layers block to convert.

    Return:
        :obj:`tf.keras.Layer`: the QuantizedDense layer of the block
    """
    for layer in block:
...
def convert_block_to_akida_dense_layer(model_ak, block, akida_type):
...
class DenseBlockConverterV1(BlockConverter):
    ...
    def convert(self, model_ak):
        return convert_block_to_akida_dense_layer(model_ak, self._block, FullyConnected)
    ...
    def _dense_layer_checks(self):
        ...
        if not (np.prod(dense_shape) == dense_shape[-1]):
            raise RuntimeError("In v1 the dense layer only handles flattened shapes"
...
class DenseBlockConverterV2(BlockConverter):
    ...
    conversion and provides a method to convert it in an equivalent Akida Dens1D or Dense2D layer.
    ...
    def __init__(self, block):
        ...
        dense_shape = dense.input_shape[1:]
        # For a shape (X, Y, Z), if X*Y*Z==Z <=> X=Y=1 <=> the shape is flattened.
        self._flag_collapse_dims = (np.prod(dense_shape) == dense_shape[-1])

    def convert(self, model_ak):
        return convert_block_to_akida_dense_layer(model_ak, self._block, Dense1D) if \
            self._flag_collapse_dims else convert_block_to_akida_dense_layer(model_ak, self._block,
                                                                             Dense2D)
