...
def convert_inputconv_block_v1(model_ak, block):
...
def convert_inputconv_block_v2(model_ak, block):
...
class InputConvBlockConverterV1(BlockConverter):
    ...
    def convert(self, model_ak):
        assert len(model_ak.layers) == 0, "InputConvolutional should be the first model layer."
        return convert_inputconv_block_v1(model_ak, self._block)
...
class InputConvBlockConverterV2(BlockConverter):
    ...
    def convert(self, model_ak):
        assert len(model_ak.layers) == 0, "InputConv2D should be the first model layer."
        return convert_inputconv_block_v2(model_ak, self._block)
