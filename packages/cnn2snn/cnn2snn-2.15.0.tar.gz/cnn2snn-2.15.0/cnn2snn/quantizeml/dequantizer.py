...
def _get_dequantizer_scales(dequantizer):
    ...
    Args:
        dequantizer (:obj:`tf.keras.Layer`): the keras Dequantizer layer.
...
def _convert_dequantizer_v1(model_ak, block):
    ...
    broadcast_and_set_variable(layer_variables, "act_step", layer_variables["act_step"] / scales)
    return True


def _convert_dequantizer_v2(model_ak, block):
    ...
    inbound_layers_ak = get_inbound_layers(model_ak, dequantizer)
...
class DequantizerBlockConverterV1(BlockConverter):
    ...
    def convert(self, model_ak):
        return _convert_dequantizer_v1(model_ak, self._block)
...
class DequantizerBlockConverterV2(BlockConverter):
    ...
    def convert(self, model_ak):
        return _convert_dequantizer_v2(model_ak, self._block)
