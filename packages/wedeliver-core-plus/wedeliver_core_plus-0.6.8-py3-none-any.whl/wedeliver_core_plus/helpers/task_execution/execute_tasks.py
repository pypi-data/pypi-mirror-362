...
def _execute_single_task(task_info: Dict[str, str]) -> Any:
    ...
    try:
        ...
        if not hasattr(task_module, 'execute'):
            raise AttributeError(f"Task module {module_path} does not have an 'execute' function")
        
        execute_function = getattr(task_module, 'execute')
