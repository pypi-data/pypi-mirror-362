...
CAPTURE_INTERVAL = 0.10  # seconds between cached frames (immutable)
DOUBLE_CLICK_WINDOW = 0.35  # seconds to treat consecutive clicks as a double-click
...
class ClickTracker:
    def __init__(self, *, event_callback: Callable[[float], None] | None = None, window: float = DOUBLE_CLICK_WINDOW):
        self._click_times: Deque[float] = deque(maxlen=3)  # stores timestamps of recent clicks
        ...
        self._kl._listener.join() # Wait for listener to start
    ...
    def _on_click(self, *_):
        ...
        while self._click_times and ts - self._click_times[0] > self._window:
            self._click_times.popleft()

        self._click_times.append(ts)
        click_count = len(self._click_times)

        # Determine label and token based on count
        if click_count == 1:
            self._click_label = "CLICK"
            # Notify recorder to capture pre-click frame
            ...
            self._click_label = "DOUBLE CLICK"
            token = "<DBLCLICK>"
        elif click_count >= 3:
            self._click_label = "TRIPLE CLICK"
            token = "<TRPCLICK>"
            # keep only the last 3 timestamps to bound deque
            while len(self._click_times) > 3:
                self._click_times.popleft()
        else:
            # Fallback
            self._click_label = "CLICK"
        ...
        self._last_click = ts
        self._kl.add_token(token)

    def recently_clicked(self, window: float = 1.0) -> bool:
        return (time.time() - self._last_click) < window

    def click_label(self) -> str | None:
        """Return label for overlay if within recent-click window."""
...
class ImageBatchRecorder:
    ...
    def __init__(self, cfg: BatchRecorderConfig | None = None, *, work_queue: queue.Queue | None = None):
        ...
        self._frame_cache: Deque[Tuple[float, Tuple[str, bytes]]] = deque(maxlen=int(3 / CAPTURE_INTERVAL))
        ...
        self._running = True
        self._last_sent_ts: float = time.time()  # controls periodic sending
        self._last_event_ts: float = 0.0  # timestamp of last click/Enter
    ...
    def run(self):
        ...
        try:
            while self._running:
                # ---- capture frame every CAPTURE_INTERVAL ----
                frame_capture_start = time.time()
                fname, data = self._capture_image()

                # Add to rolling cache
                self._frame_cache.append((frame_capture_start, (fname, data)))

                # Drain queued events and attach corresponding frames to batch
                event_appended = False
                while not self._event_queue.empty():
                    evt_ts = self._event_queue.get_nowait()
                    frame_tuple = None
                    for ts, frame_dat in reversed(self._frame_cache):
                        if ts <= evt_ts:
                            frame_tuple = frame_dat
                            break
                    if frame_tuple is None and self._frame_cache:
                        frame_tuple = self._frame_cache[0][1]
                    if frame_tuple is not None:
                        self._batch.append(frame_tuple)
                        event_appended = True

                # Periodic frame logic: ensure max two frames per click and regular cadence
                now_ts = time.time()
                if event_appended:
                    self._last_sent_ts = now_ts  # reset timer after event-driven send
                elif (
                    not self._ct.recently_clicked() and
                    (now_ts - self._last_sent_ts) >= self.cfg.capture_interval
                ):
                    # Send current frame only if outside click window and interval elapsed
                    self._batch.append((fname, data))
                    self._last_sent_ts = now_ts

                # If batch reached threshold, enqueue
                if len(self._batch) >= self.cfg.batch_size:
                    self._enqueue_batch()
                    self._batch.clear()

                # Sleep remaining interval
                elapsed = time.time() - frame_capture_start
                to_sleep = max(0.0, CAPTURE_INTERVAL - elapsed)
                time.sleep(to_sleep)
    ...
    def _capture_image(self) -> Tuple[str, bytes]:
        ...
        if 0 <= cx < self._mon["width"] and 0 <= cy < self._mon["height"]:
            ...
            click_text = self._ct.click_label()
            if click_text:
                cv2.putText(frame, click_text, (cx+12, cy+5), self.cfg.font, 0.8, (0,255,0), 2)
