...
from diffusers_nodes_library.pipelines.audioldm2.audioldm2_pipeline_parameters import (
    Audioldm2PipelineParameters,  # type: ignore[reportMissingImports]
)
...
class Audioldm2Pipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.audioldm2.AudioLDM2Pipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = Audioldm2PipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        self.preprocess()
        self.log_params.append_to_logs("Preparing AudioLDM2 models...\n")
        ...
        with self.log_params.append_profile_to_logs("Loading model metadata"):
            ...
            # Load the language model separately using GPT2LMHeadModel for generation capabilities
            language_model = model_cache.from_pretrained(
            ...
            # Load the main pipeline with the corrected language model
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_audio_ldm_2_pipeline_memory_footprint(pipe)
