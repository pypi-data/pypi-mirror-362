...
AUDIO_LDM_2_PIPELINE_COMPONENT_NAMES = [
    "vae",
    "text_encoder",
    "unet",
    "vocoder",
...
def print_audio_ldm_2_pipeline_memory_footprint(
    pipe: diffusers.AudioLDMPipeline,  # type: ignore[reportMissingImports]
) -> None:
    """Print memory footprint for the main sub-modules of Stable Diffusion 3 pipelines."""
    print_pipeline_memory_footprint(pipe, AUDIO_LDM_2_PIPELINE_COMPONENT_NAMES)


@cache
def optimize_audio_ldm_2_pipeline_memory_footprint(
    pipe: diffusers.AudioLDMPipeline,  # type: ignore[reportMissingImports]
) -> None:
    ...
    model_memory = get_total_memory_footprint(pipe, AUDIO_LDM_2_PIPELINE_COMPONENT_NAMES)
    ...
    if device.type == "cuda":
        ...
        logger.info("Require memory for Allegro Pipeline: %s", to_human_readable_size(model_memory))
    ...
    print_audio_ldm_2_pipeline_memory_footprint(pipe)
