...
class StableDiffusionAttendAndExcitePipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.stable_diffusion_attend_and_excite.StableDiffusionAttendAndExcitePipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = StableDiffusionAttendAndExcitePipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_stable_diffusion_attend_and_excite_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback(step: int, _timestep: Any, latents: Any) -> None:
