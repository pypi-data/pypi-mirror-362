...
def print_wan_pipeline_memory_footprint(pipe: diffusers.WanPipeline) -> None:
    """Print memory footprint for the main sub-modules of WAN pipelines."""
    print_pipeline_memory_footprint(pipe, WAN_PIPELINE_COMPONENT_NAMES)


@cache
def optimize_wan_pipeline_memory_footprint(
    pipe: diffusers.WanPipeline,  # type: ignore[reportMissingImports]
) -> None:
    ...
    model_memory = get_total_memory_footprint(pipe, WAN_PIPELINE_COMPONENT_NAMES)
    ...
    if device.type == "cuda":
        ...
        logger.info("Require memory for WAN Pipeline: %s", to_human_readable_size(model_memory))
