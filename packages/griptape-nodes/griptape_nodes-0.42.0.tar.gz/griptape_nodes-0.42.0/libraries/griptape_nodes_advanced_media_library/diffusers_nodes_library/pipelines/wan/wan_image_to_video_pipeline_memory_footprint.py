...
WAN_IMAGE_TO_VIDEO_PIPELINE_COMPONENT_NAMES = [
    "vae",
    "image_encoder",
    "transformer",
]
...
def print_wan_pipeline_memory_footprint(pipe: diffusers.WanImageToVideoPipeline) -> None:
    """Print memory footprint for the main sub-modules of WAN Image-to-Video pipelines."""
    print_pipeline_memory_footprint(pipe, WAN_IMAGE_TO_VIDEO_PIPELINE_COMPONENT_NAMES)


@cache
def optimize_wan_image_to_video_pipeline_memory_footprint(
    pipe: diffusers.WanImageToVideoPipeline,  # type: ignore[reportMissingImports]
) -> None:
    ...
    model_memory = get_total_memory_footprint(pipe, WAN_IMAGE_TO_VIDEO_PIPELINE_COMPONENT_NAMES)
    ...
    if device.type == "cuda":
        ...
        logger.info("Require memory for WAN Image-to-Video Pipeline: %s", to_human_readable_size(model_memory))
    ...
    print_wan_pipeline_memory_footprint(pipe)
