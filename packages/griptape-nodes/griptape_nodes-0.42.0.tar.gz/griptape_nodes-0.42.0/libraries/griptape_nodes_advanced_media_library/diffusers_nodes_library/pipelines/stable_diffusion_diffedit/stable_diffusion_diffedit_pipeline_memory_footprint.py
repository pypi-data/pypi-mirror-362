...
STABLE_DIFFUSION_DIFFEDIT_PIPELINE_COMPONENT_NAMES = [
    "text_encoder",
    "unet",
    "vae",
]
...
def print_stable_diffusion_diffedit_pipeline_memory_footprint(pipe: diffusers.StableDiffusionDiffEditPipeline) -> None:
    """Print memory footprint for the main sub-modules of Stable Diffusion DiffEdit pipelines."""
    print_pipeline_memory_footprint(pipe, STABLE_DIFFUSION_DIFFEDIT_PIPELINE_COMPONENT_NAMES)
...
def optimize_stable_diffusion_diffedit_pipeline_memory_footprint(
    ...
    model_memory = get_total_memory_footprint(pipe, STABLE_DIFFUSION_DIFFEDIT_PIPELINE_COMPONENT_NAMES)
    ...
    if device.type == "cuda":
        ...
        logger.info("Require memory for DiffEdit Pipeline: %s", to_human_readable_size(model_memory))
    ...
    print_stable_diffusion_diffedit_pipeline_memory_footprint(pipe)
