...
class LastFrameToVideoWanVaceAux(ControlNode):
    def __init__(self, **kwargs) -> None:
        ...
        self.add_parameter(
            Parameter(
                ...
                tooltip="Generated video with input image as last frame",
        ...
        self.add_parameter(
            Parameter(
                name="output_mask",
                output_type="VideoUrlArtifact",
                tooltip="Generated mask video for last frame conditioning",
                allowed_modes={ParameterMode.OUTPUT},
            )
        )
    ...
    def _process(self) -> AsyncResult | None:
        ...
        pil_image = image_artifact_to_pil(last_frame_image)
        ...
        # Create video frames - all black except last frame
        frames = []
        black_frame = Image.new("RGB", (width, height), (0, 0, 0))

        frames.extend([black_frame] * (num_frames - 1))
        frames.append(pil_image)  # Last frame is the input image

        # Create mask frames - white for all frames except last (black)
        mask_frames = []
        white_frame = Image.new("RGB", (width, height), (255, 255, 255))  # Generate frames
        black_frame = Image.new("RGB", (width, height), (0, 0, 0))  # Preserve last frame

        mask_frames.extend([white_frame] * (num_frames - 1))  # Frames to be generated
