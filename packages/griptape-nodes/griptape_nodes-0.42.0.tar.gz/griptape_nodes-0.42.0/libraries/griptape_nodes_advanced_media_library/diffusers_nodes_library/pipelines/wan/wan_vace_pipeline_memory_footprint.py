...
WAN_VACE_PIPELINE_COMPONENT_NAMES = [
    "vae",
    "text_encoder",
    "transformer",
]
...
def print_wan_vace_pipeline_memory_footprint(pipe: diffusers.WanVACEPipeline) -> None:
    """Print memory footprint for the main sub-modules of WAN VACE pipelines."""
    print_pipeline_memory_footprint(pipe, WAN_VACE_PIPELINE_COMPONENT_NAMES)


@cache
def optimize_wan_vace_pipeline_memory_footprint(
    ...
    model_memory = get_total_memory_footprint(pipe, WAN_VACE_PIPELINE_COMPONENT_NAMES)
    ...
    if device.type == "cuda":
        ...
        logger.info("Require memory for WAN VACE Pipeline: %s", to_human_readable_size(model_memory))
    ...
    print_wan_vace_pipeline_memory_footprint(pipe)
