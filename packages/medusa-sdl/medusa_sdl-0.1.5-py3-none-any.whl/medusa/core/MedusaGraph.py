...
from medusa.core.driver_loader import get_driver, DEVICE_REGISTRY, get_category_map
...
class NodeFactory:
    ...
    def generate_node_instance(cls, node_settings: Dict):  # -> Union[HeatStirPlate, SyringePump, Relay, SwitchingValve, Vessel, TecanXCPump]:
        ...
        device_class = get_driver(device_type)
...
class MedusaGraph(nx.DiGraph):
    DEVICE_CATEGORIES: dict = {
        cls: getattr(cls, "category", cls.__name__) for cls in DEVICE_REGISTRY.values()
        }
    CATEGORY_TO_CLASSES = {}
    for cls, cat in DEVICE_CATEGORIES.items():
        CATEGORY_TO_CLASSES.setdefault(cat, []).append(cls)
    ...
    def _add_directed_edge(self, source: str, target: str, edge_settings: Dict):
        """
        Add as single uni directed edge
        """
        if self.has_edge(source, target):
            self.logger.warning(f"Overwriting edge from {source} to {target}")
        super().add_edge(source, target, **edge_settings)

    def _add_edge_pair(self, source: str, target: str, edge_settings: Dict):
        """
        Add a pair of edges for draw_dispense, unless "uni_direction" is True
        """
        edge_settings = edge_settings.copy()
        uni_direction = edge_settings.pop("uni_direction", False)

        if edge_settings.get("type") == "volumetric":
            edge_settings.setdefault("content", "empty")

        self._add_directed_edge(source=source, target=target, edge_settings=edge_settings)
        if not uni_direction:
            self._add_directed_edge(source=target, target=source, edge_settings=edge_settings)
    ...
    def from_json(
        ...
        for edge in graph_layout["links"]:
            ...
            graph._add_edge_pair(source, target, edge_settings)
    ...
    def find_path(
        ...
        if edge_type is None:
            ...
            # subgraph.add_edges_from(self.nodes(data=True))
            # for u, v, data in self.edges(data=True):
            #     if data.get("type") in allowed_edges:
            #         subgraph._add_edge_pair(u, v, data)
        ...
        if source != target:
            ...
            required = set(traversed_nodes or []) | {source}
            min_length = np.inf
            best_cycle = None
            for cycle in nx.simple_cycles(subgraph):
                ...
                if len(cycle) < min_length:
                    best_cycle, min_length = cycle, len(cycle)
            if best_cycle is None:
                raise PathError(f"No cycle found with {source} and {target}")
            node_list = best_cycle + [source]
    ...
    def add_edge_from_data(self, edge_data: Dict) -> None:
        ...
        edge_settings = edge_data.copy()
        edge_settings.pop("source")
        edge_settings.pop("target")

        if edge_type == "volumetric" and "content" not in edge_settings:
            edge_settings["content"] = "empty"

        # self.add_edge(source, target, **edge_settings)
        self._add_edge_pair(source, target, edge_settings)
