...
class TSKParser:
    ...
    def parse(content: str) -> Dict[str, Any]:
        ...
        data, _ = TSKParser.parse_with_comments(content)
        return data
    ...
    def parse_with_comments(content: str) -> Tuple[Dict[str, Any], Dict[int, str]]:
        ...
        for i, line in enumerate(lines):
            ...
            # Section header
            section_match = re.match(r'^\[(.+)\]$', trimmed_line)
            if section_match:
            ...
            # Key-value pair
            if current_section and '=' in trimmed_line:
                separator_index = trimmed_line.index('=')
                key = trimmed_line[:separator_index].strip()
                value_str = trimmed_line[separator_index + 1:].strip()
                ...
                value = TSKParser._parse_value(value_str)
    ...
    def _parse_value(value_str: str) -> Any:
        """Parse a TSK value string into appropriate Python type"""
        # Null
        if value_str == 'null':
            return None
...
class TSK:
    ...
    def from_string(cls, content: str) -> 'TSK':
        ...
        data, comments = TSKParser.parse_with_comments(content)
        tsk = cls(data)
    ...
    def execute_env(self, expression: str, context: Dict[str, Any]) -> Optional[str]:
        ...
        match = re.match(r'env\("([^"]+)"(?:\s*,\s*"([^"]+)")?\)', expression)
        if not match:
            return None
...
def parse_with_comments(content: str) -> Tuple[Dict[str, Any], Dict[int, str]]:
    ...
    return TSKParser.parse_with_comments(content)
