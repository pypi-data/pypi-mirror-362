...
async def exist_channel(
    ...
    async with httpx.AsyncClient(base_url=base_url, headers=headers, timeout=100) as client:
        path = "/api/channel/" if "api.chatfire.cn" in base_url else "/api/channel/search"
        response = await client.get(path, params={"channel_id": request.id, "keyword": request.id})
        response.raise_for_status()
...
if __name__ == '__main__':
    ...
    request = ChannelInfo(id=44, key=key, used_quota=0.001)
