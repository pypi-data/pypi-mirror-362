...
from meutils.config_utils.lark_utils import get_series, get_next_token_for_polling
from meutils.llm.check_utils import check_token_for_ppinfra as check
...
async def get_valid_token(tokens: Optional[list] = None):
    ...
    for token in tokens:
        if await check(token, threshold=1800):
