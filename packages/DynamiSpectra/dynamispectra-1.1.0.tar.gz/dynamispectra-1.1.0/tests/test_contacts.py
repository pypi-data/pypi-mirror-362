...
def test_contact_analysis_runs(tmp_path):
    # Path to the directories with test data
    base_path = os.path.join(os.path.dirname(__file__), "data", "Contacts")
    ...
    output_dir = tmp_path / "contacts_output"
    ...
    # Run the contact analysis function
    contact_analysis(
    str(output_dir),
    *simulation_file_groups,
    ...
    assert (output_dir / "contacts_plot.png").exists(), "Contacts plot was not generated"
    assert (output_dir / "contacts_density.png").exists(), "Density plot was not generated"
