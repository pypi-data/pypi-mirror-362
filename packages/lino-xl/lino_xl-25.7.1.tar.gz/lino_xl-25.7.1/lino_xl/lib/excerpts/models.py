...
class Excerpt(TypedPrintable, UserAuthored, Controllable,
    ...
    def get_target_file(self):
        ...
        if et is None:
            return
        bm = et.build_method or self.get_default_build_method()
        return bm.get_target_file(self.do_print, self)
    ...
    def get_printable_target_stem(self):
        ...
        if o is None:
            return super().get_printable_target_stem()
        if et.primary:
            return o.get_printable_target_stem()
        return f"{o._meta.app_label}.{o.__class__.__name__}.{et.pk}-{o.pk}"
    ...
    def must_build_printable(self, bm):
        if not super().must_build_printable(bm):
            return False
        if self.owner is not None:
            return self.owner.must_build_printable(bm)
...
class ExcerptsByOwner(Excerpts):
    ...
    def get_table_summary(self, ar):
        ...
        def add(title, flt):
            links = []
            sar = self.create_request(master_instance=obj, filter=flt)
