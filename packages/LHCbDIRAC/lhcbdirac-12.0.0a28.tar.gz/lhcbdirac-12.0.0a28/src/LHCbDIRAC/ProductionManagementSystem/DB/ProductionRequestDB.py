...
class ProductionRequestDB(DB):
    ...
    @staticmethod
    def __addMonitoring(req, order, columns):
        ...
        if columns and "RealNumberOfEvents" in columns:
    ...
    def getProductionRequest(
        ...
        if filterIn is None:
            filterIn = {}
        if columns is None:
            columns = self.requestFields[:]
            # Reduce the size of the returned response while maintaining backward compatibility
            columns.pop(columns.index("RawRequest"))
        try:  # test parameters
        ...
        fields = ",".join(["t." + x for x in columns if x in self.requestFields[:-7]])
        ...
        result = self._query(self.__addMonitoring(req, order, columns))
        if not result["OK"]:
            return result

        rows = [dict(zip(columns, row)) for row in result["Value"]]
        total = len(rows)
        if limit:
    ...
    def __checkUpdate(self, update, old, creds, connection):
        ...
        if requestState in ["New", "BK OK", "Rejected"]:
            ...
            if creds["Group"] not in {"lhcb_ppg", "lhcb_tech", "lhcb_prmgr"}:
                self.lock.release()
        ...
        if requestState == "New":
            if "RequestState" not in update:
                return S_OK(inform)
            if update["RequestState"] == "Submitted":
                # !!! full information check must be here, but currently in the JS...
                # so we only check EventType consistency
            ...
            self.lock.release()
            return S_ERROR("Rejected requests cannot be resurrected")
        elif requestState in {"BK Check", "BK OK"}:
            self.lock.release()
            raise NotImplementedError("This shouldn't happen any more")
        elif requestState == "Submitted":
            if not update["RequestState"] in {"Tech OK", "Rejected", "Accepted", "PPG OK", "Active"}:
                self.lock.release()
                _msgTuple = (requestState, update["RequestState"])
                return S_ERROR("The request is '%s' now, moving to '%s' is not possible" % _msgTuple)
    ...
    def __moveChildDeep(self, requestID, masterID, setParent, connection):
        ...
        if setParent:
            updates = f"ParentID={str(masterID)},MasterID={str(masterID)}"
        else:
