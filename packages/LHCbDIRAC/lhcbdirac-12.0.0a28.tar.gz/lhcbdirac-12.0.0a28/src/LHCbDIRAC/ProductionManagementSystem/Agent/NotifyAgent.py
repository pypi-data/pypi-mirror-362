...
class NotifyAgent(AgentModule):
    ...
    def initialize(self):
        ...
        self.fromAddress = gConfig.getValue(f"{self.csS}/fromAddress", "")
        ...
        self.statusUpdateMail = self.am_getOption("StatusUpdateMail", self.statusUpdateMail)
    ...
    def _executeForProductionManagementSystem(self, conn):
        ...
        for thegroup, reqName, reqWG, reqInform, reqType in result:
            link = (
                "https://lhcb-portal-dirac.cern.ch/DIRAC/s:LHCb-Production/g:"
                + thegroup
      ...
      """
            ...
            if reqInform:
                for emailaddress in reqInform.split(","):
                    res = self.diracAdmin.sendMail(
                        emailaddress,
                        f"Notifications for production requests - Group {thegroup}; {reqWG}; {reqName}",
                        aggregated_body,
                        self.fromAddress,
                        html=True,
                    )
            ...
            for people in _getMemberMails(thegroup):
                ...
                if res["OK"]:
                    ...
                    self.log.error(f"_inform_people: can't send email: {res['Message']}")

    def _executeForProductionStatusAgent(self, conn):
        """This is for the ProductionStatusAgent"""

        if self.statusUpdateMail:
            aggregated_body = """\
            <!DOCTYPE html>
            <html>
            <head>
        ...
        """
            ...
            if not res["OK"]:
                self.log.error(f"Can't send email: {res['Message']}")
                return S_OK()
