...
class ExecutionLogger:
    ...
    def __init__(self, script_name: str, 
             ...
             dv_client_id: str = None,
             dv_client_secret: str = None, 
             dv_tenant_id: str = None,
             dv_scope: str = None,
             dv_api_url: str = None,
        ...
        # Configure logging properly - CREATE A DEDICATED LOGGER
        level = logging.DEBUG if debug else logging.INFO
        
        # Create a unique logger for this instance
        self.logger = logging.getLogger(f"{script_name}_{id(self)}")
        ...
        file_handler = logging.FileHandler(self.temp_log_file_path, mode='w')
    ...
    def finalize(self):
        ...
        # Ensure all logs are flushed to file before proceeding
        try:
            if hasattr(self, 'file_handler'):
                self.file_handler.flush()
        except Exception as e:
            print(f"[WARNING] Error flushing log file: {e}")
        
        # The temp log file should exist now since we set up proper file logging
        if not os.path.exists(self.temp_log_file_path):
            error_msg = f"Temp log file still not found after proper setup: {self.temp_log_file_path}"
            print(f"[ERROR] {error_msg}")
            # This shouldn't happen with the fixed logging setup, but create emergency log
            try:
                with open(self.temp_log_file_path, 'w') as f:
                    f.write(f"EMERGENCY LOG - Original file missing\n")
                    f.write(f"Log file created during finalize for {self.script_name}\n")
                    f.write(summary + "\n")
                    f.write(f"Error: {error_msg}\n")
            except Exception as e:
                print(f"[ERROR] Could not create emergency log file: {e}")
                self.cleanup()
                return
    ...
    def cleanup(self):
        ...
        try:
            # Close and remove handlers before shutdown
            if hasattr(self, 'file_handler'):
            ...
            # Now shutdown logging
            logging.shutdown()
            
            # Remove temp file after ensuring handlers are closed
