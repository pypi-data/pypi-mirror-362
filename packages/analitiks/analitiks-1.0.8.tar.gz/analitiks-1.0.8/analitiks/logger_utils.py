...
class UsernameFilter(logging.Filter):
    ...
    def filter(self, record):
        record.username = getattr(record, 'username', self.username)  # Безопасно добавляем username
        return True
...
def setup_logging(log_file_name="library.log"):
    ...
    file_handler.addFilter(UsernameFilter(username))  # Добавляем фильтр
    logger.addHandler(file_handler)

    # Консоль (для удобства отладки)
    console_handler = logging.StreamHandler()
    console_handler.setFormatter(formatter)
    console_handler.addFilter(UsernameFilter(username))  # Добавляем фильтр
