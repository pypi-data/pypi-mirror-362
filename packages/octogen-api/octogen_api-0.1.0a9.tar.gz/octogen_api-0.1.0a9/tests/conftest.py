...
from octogen.api import OctogenAPI, AsyncOctogenAPI, DefaultAioHttpClient
from octogen.api._utils import is_dict
...
logging.getLogger("octogen.api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[OctogenAPI]:
...
@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncOctogenAPI]:
    ...
    async with AsyncOctogenAPI(
        base_url=base_url, octogen_api_key=octogen_api_key, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
