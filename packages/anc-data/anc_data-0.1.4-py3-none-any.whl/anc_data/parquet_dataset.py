    ...
    def __init__(self, filepath, columns=None):
        ...
        for pf in self.parquet_file:
            assert self.num_row_groups == pf.num_row_groups, \
                f'parquet file {pf.reader.file_path} has {pf.num_row_groups} row groups but {self.num_row_groups} is expected'
            assert self.row_group_rows == [pf.metadata.row_group(rid).num_rows for rid in self.row_groups], \
                f'parquet file {pf.reader.file_path} has different rows in row group than expected {self.row_group_rows}'
...
class ParquetConcateDataset:
    ...
    def __getitem__(self, idx):
        if idx < 0:
            idx += len(self)
        assert 0 <= idx < len(self), f"idx shoud be in [0, {len(self)}) but get {idx}"
