...
def assert_2_layer_list(x):
    assert isinstance(x, list) and all(isinstance(i, list) for i in x), \
        "For parquet, AncDataset's filepaths need to be a 2-layer list: list of datasets (ParquetConcateDataset for each) -> list of ParquetDataset. " \
        "ParquetDataset accepts both a single str filepath and a list of filepaths for cases including correlated files such as wav"
