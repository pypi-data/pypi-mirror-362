...
def assert_3_layer_list(x):
    assert isinstance(x, list) and all(isinstance(i, list) for i in x) and all(isinstance(j, list) for i in x for j in i), \
        "For parquet, AncDataset's filepaths need to be a 3-layer list: Datasets -> ParquetConcateDataset -> ParquetDataset (including correlated files such as wav)"
...
class AncDataset(IterableDataset):
    ...
    def __init__(
        ...
        if dataset_type == "parquet":
            assert_3_layer_list(filepaths)
            self.ds = [ParquetConcateDataset(filepath, columns=ds_args.get("columns")) for filepath in filepaths]
        elif dataset_type == "jsonl":
            assert isinstance(filepaths[0], str), f"jsonl dataset does not support multisources"
            ...
            self.ds = [JsonlDataset(filepaths[0], decode_fn=decode_fn)]
    ...
    def get_generator(self, sampler_iter):
        ...
        while True:
            ...
            while True:
                try:
                    # TODO: is_last isn't set if item_generator returns nothing
                    ...
                    # TODO: check if this is correct when next() throws
    ...
    def _getitem(self, idxs):
        ...
        if self.processor.transform:
            ...
            for data_idx, data in enumerate(raw_data):
                ...
                if isinstance(data, dict) and '_ds_idx' not in data:
        ...
        # TODO: what if processed_data is not empty but no batch_transform?
