...
from anc_data.anc_sampler import AncSampler, AncMultiSourceSampler
from anc_data.anc_processor import Processor, AncProcessor
...
class AncDataLoader:
    ...
    def __next__(self):
        ...
        if need_logging:
            logging.info(f"__next__ called time {now} pid {os.getpid()} rank {self.rank} return step {self.step} consumed time {consumed_time} data with buffer {safe_qsize(self.loader_iter._data_queue)}")
        return data
