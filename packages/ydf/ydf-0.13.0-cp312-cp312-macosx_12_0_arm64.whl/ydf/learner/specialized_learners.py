...
class CartLearner(generic_learner.GenericCCLearner):
  ...
  Attributes:
    ...
    categorical_algorithm: How to learn splits on categorical attributes.
      ...
      - `ONE_HOT`: One-hot encoding. Find the optimal categorical split of the
        ...
        than converting each possible categorical value into a boolean feature.
    ...
    categorical_set_split_greedy_maximum_mask_size: For categorical set splits
      e.g. texts. Maximum number of attribute values on the positive side of
      the split mask. Smaller values might improve training speed but lead to
      worse models. Setting this parameter to 1 is equal to one-hot encoding
      the attribute values. Set to -1 for no maximum (default). Default: -1.
    ...
    numerical_vector_sequence_enable_closer_than_conditions: For datasets with
      NUMERICAL_VECTOR_SEQUENCE features (i.e., sequence of fixed-size
      numerical vectors). If true, enable conditions of type |x-A|^2 <= T for
      learned anchor A and threshold T, and any vector x in the sequence.
      Default: True.
    numerical_vector_sequence_enable_projected_more_than_conditions: For
      ...
      fixed-size numerical vectors). If true, enable conditions of type x*A >=
    ...
    numerical_vector_sequence_num_examples: For datasets with
      NUMERICAL_VECTOR_SEQUENCE features (i.e., sequence of fixed-size
      numerical vectors). Maximum number of examples to use to find splits. A
      larger value can improve the model quality but takes longer to train.
      Default: 1000.
    numerical_vector_sequence_num_random_anchors: For datasets with
      NUMERICAL_VECTOR_SEQUENCE features (i.e., sequence of fixed-size
      numerical vectors). The number of randomly generated anchor values. A
      larger value can improve the model quality but takes longer to train.
      Default: 100.
    ...
    sparse_oblique_max_num_features: For sparse oblique splits i.e.
      ...
      Controls the maximum number of features in a split.Set to -1 for no
      maximum.
      ...
      of features per projection. Default: None.
    sparse_oblique_max_num_projections: For sparse oblique splits i.e.
      ...
      model development, if every bit of accuracy is important, increase this
      value.
    ...
    sparse_oblique_num_projections_exponent: For sparse oblique splits i.e.
      ...
      drastically increases the training time, and does not impact the
      inference time.
      Oblique splits try out max(p^num_projections_exponent,
      max_num_projections) random projections for choosing a split, where p is
    ...
    sparse_oblique_weights: For sparse oblique splits i.e.
      ...
      - `POWER_OF_TWO`: The oblique weights are powers of two. The exponents
        are sampled
                                          uniformly in
                        [sparse_oblique_weights_power_of_two_min_exponent,
                        sparse_oblique_weights_power_of_two_max_exponent], the
                        sign is uniformly sampled.
      - `INTEGER`: The weights are integers sampled uniformly from the range
    ...
    sparse_oblique_weights_integer_maximum: For sparse oblique splits i.e.
      `split_axis=SPARSE_OBLIQUE` with 
      integer weights i.e. `sparse_oblique_weights=INTEGER`. Maximum value of
      the weights Default: None.
    sparse_oblique_weights_integer_minimum: For sparse oblique splits i.e.
      `split_axis=SPARSE_OBLIQUE` with 
      integer weights i.e. `sparse_oblique_weights=INTEGER`. Minimum value of
      the weights. Default: None.
    sparse_oblique_weights_power_of_two_max_exponent: For sparse oblique splits
      i.e. `split_axis=SPARSE_OBLIQUE` with 
      power-of-two weights i.e. `sparse_oblique_weights=POWER_OF_TWO`. Maximum
      exponent of the weights Default: None.
    sparse_oblique_weights_power_of_two_min_exponent: For sparse oblique splits
      i.e. `split_axis=SPARSE_OBLIQUE` with 
      power-of-two weights i.e. `sparse_oblique_weights=POWER_OF_TWO`. Minimum
      exponent of the weights Default: None.
    split_axis: What structure of split to consider for numerical features.
      ...
      - `MHLD_OBLIQUE`: Multi-class Hellinger Linear Discriminant splits from
        ...
        Canete-Sifuentes et al., 2019 Default: "AXIS_ALIGNED".
  ...
  def __init__(self,
    ...
    hyper_parameters = {
                      ...
                      "categorical_set_split_greedy_maximum_mask_size" : categorical_set_split_greedy_maximum_mask_size,
                      ...
                      "numerical_vector_sequence_enable_closer_than_conditions" : numerical_vector_sequence_enable_closer_than_conditions,
                      "numerical_vector_sequence_enable_projected_more_than_conditions" : numerical_vector_sequence_enable_projected_more_than_conditions,
                      "numerical_vector_sequence_num_examples" : numerical_vector_sequence_num_examples,
                      "numerical_vector_sequence_num_random_anchors" : numerical_vector_sequence_num_random_anchors,
                      ...
                      "sparse_oblique_weights_integer_maximum" : sparse_oblique_weights_integer_maximum,
                      "sparse_oblique_weights_integer_minimum" : sparse_oblique_weights_integer_minimum,
                      "sparse_oblique_weights_power_of_two_max_exponent" : sparse_oblique_weights_power_of_two_max_exponent,
                      "sparse_oblique_weights_power_of_two_min_exponent" : sparse_oblique_weights_power_of_two_min_exponent,
...
class DistributedGradientBoostedTreesLearner(generic_learner.GenericCCLearner):
  ...
  Attributes:
    ...
    apply_link_function: If true, applies the link function (a.k.a. activation
      ...
      output is a logit while the post-link function is a probability. Default:
      True.
    ...
    loss: The loss optimized by the model. If not specified (DEFAULT) the loss
      ...
      - `LAMBDA_MART_NDCG5`: DEPRECATED, use LAMBDA_MART_NDCG. LambdaMART with
        NDCG@5. 
        Default: "DEFAULT".
    max_depth: Maximum depth of the tree. `max_depth=1` means that all trees
      will be roots. `max_depth=-1` means that tree depth is not restricted by
    ...
    num_trees: Maximum number of decision trees. The effective number of
      trained tree can be smaller if early stopping is enabled. For multi-class
...
class GradientBoostedTreesLearner(generic_learner.GenericCCLearner):
  ...
  Attributes:
    ...
    early_stopping: Early stopping detects the overfitting of the model and
      halts its training using the validation dataset. If not provided
      directly, the validation dataset is extracted from the training dataset
      (see "validation_ratio" parameter):
    ...
    l2_categorical_regularization: L2 regularization applied to the training
      loss for categorical features. Impact the tree structures and leaf
      values. Default: 1.0.
    ...
    total_max_num_nodes: Limit the total number of nodes in the model over all
      ...
      models, prefer adapting max_num_nodes / max_depth and num_trees. Set to
      -1 (default) for no limit. Default: -1.
    uplift_min_examples_in_treatment: For uplift models only. Minimum number of
      examples per treatment in a node. Default: 5.
    uplift_split_score: For uplift models only. Splitter score i.e. score
      optimized by the splitters. The scores are introduced in "Decision trees
    ...
    validation_interval_in_trees: Evaluate the model on the validation set
      every "validation_interval_in_trees" trees. Increasing this value reduces
