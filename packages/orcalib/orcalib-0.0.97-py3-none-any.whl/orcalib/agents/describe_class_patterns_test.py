...
async def test_describe_class_patterns_agent():
    ...
    context = DescribeClassPatternsContext(
        memoryset_description="sentiment analysis of product reviews"
    )
    ...
    # Verify the result
    assert isinstance(result.data, ClassPatternsDescription)
...
def test_class_patterns_input_validation():
    ...
    class_rep = ClassRepresentatives(
        label=0,
        label_name="test_class",
        representative_values=["example 1", "example 2"]
    )
...
def test_prompt_generation():
    ...
    class_reps = [
        ClassRepresentatives(
            label=0,
            label_name="negative",
            representative_values=["bad example", "poor quality"]
        ),
        ClassRepresentatives(
            label=1,
            label_name=None,  # Test with no label name
            representative_values=["good example"]
        )
    ]
