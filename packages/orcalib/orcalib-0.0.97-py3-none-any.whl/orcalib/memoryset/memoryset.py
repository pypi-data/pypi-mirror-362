...
class Memoryset[T: MemoryType]:
    ...
    def document_prompt(self, prompt: str | None):
        self.config = self.repository.update_config(
            MemorysetConfig(**(self.config.model_dump() | {"embedding_model_document_prompt_override": prompt}))
        )
    ...
    def query_prompt(self, prompt: str | None):
        self.config = self.repository.update_config(
            MemorysetConfig(**(self.config.model_dump() | {"embedding_model_query_prompt_override": prompt}))
        )
    ...
    def lookup(
        ...
        *,
        ...
        embedded_queries = self._embed(
            ...
            value_kind="query",
            prompt_override=prompt or getattr(self.config, "embedding_model_query_prompt_override", None),
            use_cache=use_cache,
        )
