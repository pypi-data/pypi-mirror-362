...
class ClassPatternsDescription(BaseModel):
    """Result of the class patterns description."""
    description: str
...
{examples}
    ...
    return f"""
        Analyze the following representative examples from each class and describe the key patterns 
        that distinguish these classes from each other:

        {chr(10).join(class_sections)}

        Provide a clear description of what characterizes each class and what makes them distinct 
        from one another. Focus on the underlying patterns rather than just listing examples.
    """
