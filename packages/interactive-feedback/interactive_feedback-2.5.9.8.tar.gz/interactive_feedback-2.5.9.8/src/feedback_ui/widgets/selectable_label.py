...
class SelectableLabel(QLabel):
    ...
    def _get_text_formatter(self):
        ...
        strategies = [
            # 策略1：相对导入（开发环境）
            lambda: __import__('feedback_ui.utils.text_formatter', fromlist=['text_formatter']).text_formatter,
            # 策略2：绝对导入（uv安装环境）
            lambda: importlib.import_module('feedback_ui.utils.text_formatter').text_formatter,
        ]
    ...
    def _fallback_markdown_detection(self, text: str) -> bool:
        ...
        for line in lines:
            ...
            # 标题、代码块、列表、引用（必须在行首）
            if (line.startswith('#') and len(line) > 1 and line[1] in (' ', '#')) or \
               line.startswith('```') or \
               line.startswith(('- ', '* ', '> ')) or \
               re.match(r'^\d+\.\s+', line):
                return True
        ...
        # 检测成对的格式标记
        if (text.count('**') >= 2 and text.count('**') % 2 == 0) or \
           (text.count('`') >= 2 and text.count('`') % 2 == 0) or \
           re.search(r'\[.+\]\(.+\)', text):
            return True
