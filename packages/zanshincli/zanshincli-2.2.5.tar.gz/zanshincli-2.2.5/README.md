...
### `zanshin summary alerts`

**Usage**:

```console
$ zanshin summary alerts [OPTIONS] ORGANIZATION_ID
```
...
* `--scan-target-ids UUID`: Only summarize alerts from the specified scan target ids
* `--search TEXT`: Text to search for in the alerts  [default: ]
* `--lang [pt-BR|en-US]`: Show alerts titles in the specified language  [default: en-US]
* `--help`: Show this message and exit.

### `zanshin summary following_scan`

**Usage**:

```console
$ zanshin summary following_scan [OPTIONS] ORGANIZATION_ID
```
...
* `--following-ids UUID`: Only summarize scans from the following ids
