...
def map_phone_to_tokendict(item, pad_bos_eos=True):
    ...
    merged_phone[ch_phone_idx] = (
        (merged_phone[ch_phone_idx] - 3) * 6 + 200 + tone_tmp[ch_phone_idx]
    )
...
def split_ph_timestamp(ph_timestamp):
    ...
    ph_seq, tone_seq, dur_seq = (
        torch.LongTensor(ph_list),
        torch.LongTensor(tone_list),
        torch.LongTensor(dur_list),
    )
