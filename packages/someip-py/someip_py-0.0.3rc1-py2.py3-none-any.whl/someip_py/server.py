...
class SOMEIPServer:
    ...
    def _handle_method(self, frame: SOMEIPFrame, sock: socket.socket, addr=None):
        if not self._services.get(frame.service_id):
            ...
            if frame.message_type == SOMEIPMessageType.REQUEST:
                ...
                frame.message_type = SOMEIPMessageType.RESPONSE
                frame.payload = self._services[frame.service_id].methods[
                    frame.method_id
                ]
                self._send(frame.encode(), sock, addr=addr)

    def _handler_event(self, frame: SOMEIPFrame, sock: socket.socket, addr=None):
    ...
    def _handle_response(self, response: bytes, sock: socket.socket, addr=None):
        try:
            incoming_frame = SOMEIPFrame.decode(response)
            if incoming_frame.service_id == SOMEIPSDFrame.SD_SERVICE_ID:
        ...
        for interface in service["interface"]:
            if interface["type"] == "method":
                self._services[service_id].register_method(
                    hex2int(interface["method_id"]), interface["payload"]["response"]
                )
    ...
    def close(self):
        ...
        for tcp_sock in self._tcps.values():
            tcp_sock["running"] = False
            if tcp_sock["thread"].is_alive():
                tcp_sock["thread"].join(A_PROCESSING_TIME + 1.0)

            for conn in tcp_sock["connection"].values():
                conn.close()

            tcp_sock["socket"].close()
