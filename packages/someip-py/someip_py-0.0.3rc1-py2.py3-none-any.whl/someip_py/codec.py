...
from .utils import JsonEncoderWrapper
...
class Uint8:
    """
    someip datatype representing an unsigned 8 bit integer on the wire.
    """
...
class Int8:
    """
    someip datatype representing a signed 8 bit integer on the wire.
    """
...
class Uint16:
    """
    someip datatype representing an unsigned 16 bit integer on the wire.
    """
...
class Int16:
    """
    someip datatype representing a signed 16 bit integer on the wire.
    """
...
class Uint32:
    """
    someip datatype representing an unsigned 32 bit integer on the wire.
    """
...
class Int32:
    """
    someip datatype representing a signed 32 bit integer on the wire.
    """
...
class Uint64:
    """
    someip datatype representing an unsigned 64 bit integer on the wire.
    """
...
class Int64:
    """
    someip datatype representing a signed 64 bit integer on the wire.
    """
...
class Bool:
    """
    someip datatype representing a boolean type transported as a single byte on the wire.
    """
    ...
    def deserialize(self, payload):
        ...
        This method deserializes the payload into the value of the object.
        It expects the payload to be a single byte representing a boolean value.
        If the payload is 0, the value of the object is set to False.
        If the payload is 1, the value of the object is set to True.
        The deserialized object is then returned.
        """
...
class Float32:
    """
    someip datatype representing a 32 bit floating type.
    """
    ...
    def serialize(self) -> bytes:
        ...
        This method serializes the value of the object into bytes using the big-endian byte order.
        It expects the value to be a float. The serialized value is returned as a bytes object.
        """
    ...
    def deserialize(self, payload):
        ...
        This method deserializes the payload into the value of the object.
        It expects the payload to be a 4-byte float in big-endian byte order.
        The deserialized value is assigned to the `value` attribute of the object.
        The deserialized object is then returned.
        """
    ...
    def __eq__(self, other) -> Bool:
        ...
        This method compares the serialized representation of the current object and
        the other object to determine if they are equal.
        It returns a Bool object indicating whether the objects are equal or not.
...
class Float64:
    """
    someip datatype representing a 64 bit floating type.
    """
    ...
    def deserialize(self, payload):
        ...
        This method deserializes the payload into the value of the object.
        It expects the payload to be an 8-byte float in big-endian byte order.
        The deserialized value is assigned to the `value` attribute of the object.
        The deserialized object is then returned.
        """
...
def serialize(obj) -> bytes:
    """
    Serializes an object into bytes by iterating over its attributes,
    excluding those starting with double underscores or underscores.
    For each attribute, it calls the `serialize` method of the attribute and appends
    the returned bytes to the output.
    ...
    for _, value in ordered_items:
        ...
        if hasattr(value, "_include_struct_len"):
            output = struct.pack(">I", len(output)) + output
    return output


class SomeIpPayload:
    """
    A base class for defining a custom SOME/IP payload ("structs").
    It can be recursively nested, i.e. a SomeIpPayload object may contain other SomeIpPayload objects.
    """
    ...
    def __str__(self):
        return json.dumps(self.__dict__, cls=JsonEncoderWrapper)
    ...
    def deserialize(self, payload: bytes):
        ...
        It iterates over the attributes of the object, excluding those starting with "__".
        For each attribute, it calculates the length of the corresponding value and deserializes it
        using the `deserialize` method of the value object.
        The deserialized values are assigned back to the corresponding attributes of the object.
        ...
        for key, value in ordered_items:
            ...
            if hasattr(value, "_include_struct_len") and value._include_struct_len:
                pos += 4

            if hasattr(value, "_include_array_len") and value._include_array_len:
                pos += 4

            if hasattr(value, "_has_dynamic_size") and value._has_dynamic_size:
        ...
        if hasattr(self, "_replace_fields"):
            getattr(self, "_replace_fields")()

        return self

    def setValues(self, values: dict):
        ordered_items = [
            name for name in self.__dict__.keys() if not name.startswith("__")
        ]
        for key in ordered_items:
            if values.get(key):
                if isinstance(values[key], dict):
                    self.__dict__[key].setValues(values[key])
                elif isinstance(values[key], list):
                    self.__dict__[key].data = values[key]
                else:
                    self.__dict__[key].value = values[key]
            else:
                if hasattr(self.__dict__[key], "setValues"):
                    self.__dict__[key].setValues(values)
        return self
...
class SomeIpFixedSizeArray(Generic[T]):
    """
    A datatype for a SOME/IP fixed size array.
    This type shall be used with someip datatypes that support serialization and deserialization.
    """

    _json_array = True

    def __init__(self, class_reference: Type[T], size: int, include_array_len=False):
        """
        ...
        self._include_array_len = include_array_len

    def __eq__(self, other):
        """
        Compare two SomeIpFixedSizeArray objects for equality.

        This method compares the length (number of elements) of the current array and
        the other array to determine if they are equal.
        It also compares the length of the bytes representation of the arrays. Finally,
        it compares the content of all elements in the arrays to check if they are equal.
    ...
    def __len__(self) -> int:
        ...
        This method calculates the length of the object based on the number of elements in
        the `data` list and the length of each element. If the `data` list is empty, it returns 0.
        Otherwise, it returns the product of the length of the `data` list and
        the length of the first element in the `data` list.
        ...
        if len(self.data) == 0:
            ...
            if self._include_array_len:
                return 4 + len(self.data) * len(self.data[0])
            return len(self.data) * len(self.data[0])

    def serialize(self) -> bytes:
        """
        Serialize the object into bytes by iterating over its attributes,
        excluding those starting with double underscores or underscores.
        For each attribute, it calls the `serialize` method of the attribute and
        appends the returned bytes to the output.
    ...
    def deserialize(self, payload: bytes):
        ...
        This method deserializes the payload into the object.
        It iterates over the `data` list and for each element,
        it calls the `deserialize` method of that element,
        passing a slice of the payload corresponding to the element's length.
        The deserialized values are assigned back to the corresponding elements of the `data` list.
        If the `data` list is empty, the method returns immediately.
        Finally, the deserialized object is returned.
        """
        if len(self.data) == 0:
            return
...
class SomeIpDynamicSizeArray(Generic[T]):
    """
    A datatype for a SOME/IP dynamically sized array.
    This type shall be used in someip datatypes that support serialization and deserialization.
    """
    ...
    def __eq__(self, other) -> bool:
        ...
        This method compares the length (number of elements) of the current array and
        the other array to determine if they are equal.
        It also compares the length of the bytes representation of the arrays.
        Finally, it compares the content of all elements in the arrays to check if they are equal.
    ...
    def __len__(self) -> int:
        ...
        This method calculates the length of the object based on the number of elements in
        the `data` list and the length of each element. If the `data` list is empty,
        it returns 0. Otherwise, it returns the product of the length of the `data` list
        and the length of the first element in the `data` list.
...
class SomeIpDynamicSizeString(Generic[T]):
    ...
    def deserialize(self, payload: bytes):
        ...
        This method deserializes the payload into the string.
        It automatically detects the encoding from the BOM
        at the beginning of the payload.
        """
