...
class Generator:
    def __init__(self, latent_dim=100):
        ...
        self.params = {
            ...
            "W2": rng.normal(0, 0.1, (3, 3, 64, 64)),  # upsample 후 conv
            "b2": anp.zeros(64),
            "W2b": rng.normal(0, 0.1, (3, 3, 64, 32)),  # conv 쌓기
            "b2b": anp.zeros(32),

            "W3": rng.normal(0, 0.1, (3, 3, 32, 32)),  # 두 번째 업샘플 후
            "b3": anp.zeros(32),
            "W3b": rng.normal(0, 0.1, (3, 3, 32, 16)),
            "b3b": anp.zeros(16),

            "W4": rng.normal(0, 0.1, (3, 3, 16, 1)),  # 마지막 conv
            "b4": anp.zeros(1)
        }
    ...
    def forward(self, z, params=None):
        ...
        h = upsample(h)  # 7 → 14
        h = gelu(conv2d(h, params["W2"], params["b2"], stride=1, padding=1))
        h = gelu(conv2d(h, params["W2b"], params["b2b"], stride=1, padding=1))

        h = upsample(h)  # 14 → 28
        h = gelu(conv2d(h, params["W3"], params["b3"], stride=1, padding=1))
        h = gelu(conv2d(h, params["W3b"], params["b3b"], stride=1, padding=1))

        x_fake = sigmoid(conv2d(h, params["W4"], params["b4"], stride=1, padding=1))
        return x_fake
