...
class Generator:
    def __init__(self, latent_dim=100):
        ...
        self.params = {
            "W1": rng.normal(0, 0.02, (latent_dim, 7 * 7 * 64)),
            "b1": anp.zeros(7 * 7 * 64),
            "W_out": rng.normal(0, 0.02, (3, 3, 16, 1)),
            "b_out": anp.zeros(1)
        }

        # 2단계 업샘플 conv 쌓기: 64 → 32
        for i in range(5):
            self.params[f"W2_{i}"] = rng.normal(0, 0.02, (3, 3, 64 if i == 0 else 32, 32))
            self.params[f"b2_{i}"] = anp.zeros(32)

        # 3단계 업샘플 conv 쌓기: 32 → 16
        for i in range(5):
            self.params[f"W3_{i}"] = rng.normal(0, 0.02, (3, 3, 32 if i == 0 else 16, 16))
            self.params[f"b3_{i}"] = anp.zeros(16)
    ...
    def forward(self, z, params=None):
        ...
        h = gelu(h).reshape((-1, 7, 7, 64))

        # ⬆️ 업샘플 1: 7x7 → 14x14
        h = upsample(h)
        for i in range(5):
            h = gelu(conv2d(h, params[f"W2_{i}"], params[f"b2_{i}"], stride=1, padding=1))

        # ⬆️ 업샘플 2: 14x14 → 28x28
        h = upsample(h)
        for i in range(5):
            h = gelu(conv2d(h, params[f"W3_{i}"], params[f"b3_{i}"], stride=1, padding=1))

        # 마지막 conv → sigmoid
        x_fake = sigmoid(conv2d(h, params["W_out"], params["b_out"], stride=1, padding=1))
        return x_fake
...
def gan_loss(D_real, D_fake):
    real_loss = anp.mean((D_real - 1) ** 2)
    fake_loss = anp.mean((D_fake - 0) ** 2)
