...
from .commons_pb2 import OuterContextItem, SomeRequest, SomeResponse
from .ChatManager_pb2 import (
    ChatManagerRequest,
    ChatManagerResponse,
    ChatManagerRequest2,
    ChatManagerResponse2,
    DomainsRequest,
    DomainsResponse,
    DomainInfo,
    TracksRequest,
...
class ChatManagerClient(AbstractClient):
    ...
    def get_response(chat_json: str, request_id: str = "") -> Annotated[str, 'serialized list[replicas]']:
        request = SomeRequest(Payload=chat_json)
        response = self._stub.GetChatResponse2(request, metadata=form_metadata(request_id))
