...
def test_hash_walk_one_file(path: Path) -> None:
    ...
    assert given_value["sha"] == str(hashlib.sha256(value).digest())
...
def test_hash_walk_many_files(path: Path) -> None:
    ...
    assert given_value["sha"] == str(hashlib.sha256(value2).digest())

    given_key, given_value = result.popitem()
    assert file_name1 in given_key
    assert given_value["sha"] == str(hashlib.sha256(value1).digest())
