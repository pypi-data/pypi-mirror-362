...
"""
class View(klovve.pieces.dropdown.View, klovve.drivers.viwid.View):
    ...
    def create_native(self):
        ...
        def _handle_selection():
           # if model.selected_item is not None:  # TODO pass
                btn.selected_index = self.model.items.index(self.model.selected_item) if self.model.selected_item is not None else None
          #  else:
            #    btn.selected_index = None

        @klovve.reaction(owner=btn)
        def _handle_items():
            btn.items = [(self.model.item_label_func or str)(item) for item in self.model.items]

        def mkclick(ite):
            def f(_):
                self.model.selected_item = ite
        ...
        def sei():
            self.model.selected_item = self.model.items[btn.selected_index] if btn.selected_index is not None else None
