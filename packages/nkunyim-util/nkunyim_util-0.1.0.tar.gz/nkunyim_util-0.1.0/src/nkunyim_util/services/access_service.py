...
class AccessService:
    ...
    def _get_from_api(self) -> Optional[tuple[Optional[dict], Optional[List[dict]]]]:
        try:
            ...
            access = dict(response.json()) if response.ok else {}
            
            role = access.get("role", None)
            menus = access.get("menus", None) 
            return role, menus
        except:
            return None
