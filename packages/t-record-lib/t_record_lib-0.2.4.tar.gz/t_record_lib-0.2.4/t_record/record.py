...
def dump_records_html(file_path: str = "records_report.html") -> None:
    """Dump all records to an HTML file."""
    if _records_manager is not None:
        try:
            return _records_manager.dump_records_html(file_path)
        except Exception as e:
            logger.warning(f"Error getting records HTML: {e}")
            report_error(exception=e)
    else:
        logger.warning("T-Record wasn't configured. Cannot get records HTML.")
