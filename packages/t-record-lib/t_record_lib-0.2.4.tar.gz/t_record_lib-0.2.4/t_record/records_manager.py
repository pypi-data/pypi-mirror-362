...
from .utils.utils import compare_dicts, format_traceback_short
...
class RecordsManager:
    ...
    def dump(self) -> None:
        ...
        self.storage.save_html(generate_enhanced_html(merged_logs.data))
        self.temp_dir.cleanup()

    def pack_sharable_zip(self, file_path: str) -> None:
        """Pack all records logs and HTML report into a zip file."""
        self.storage.get_zip(file_path)

    def dump_records_html(self, file_path: str) -> None:
        """Dump all records logs to an HTML file."""
        merged_logs = self.storage.get_records_logs(include_image_src_uri=True)
    ...
    def log_trace(
        ...
        if image:
            unique_image_name = f"{record.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}{Path(image).suffix}".replace(
                " ", "_"
            )
            dest = Path(self.temp_dir.name) / unique_image_name
