...
from .utils.utils import compare_dicts, format_traceback_short, sanitize_path_filename
...
class RecordsManager:
    ...
    def log_trace(
        ...
        if image:
            unique_image_name = f"{record.id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}{Path(image).suffix}"
