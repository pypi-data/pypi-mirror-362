...
class BaseMcp(abc.ABC):
    ...
    def get_langchain_tools(
        ...
        # Todo: consider using get_mcp_tools as source to copy annotations from them
        # and to enable filtering by scope.
        tools = []
        for tool in self.get_tools():
            ...
            if with_prefix:
                if not self.get_tag():
                    raise ValueError(
                        "To add prefix to tool names, the get_tag method must return a non-empty string."
                    )

                tool.name = f"{self.get_tag()}_{tool.name}"
