...
class OuterProductMean(nn.Module):
    ...
    def __init__(self, c_m, c_z, c_hidden, eps=1e-3):
        ...
        self.output_w = nn.Parameter(torch.zeros(c_hidden, c_hidden, c_z))
        self.output_b = nn.Parameter(torch.zeros(c_z))

    def _opm(self, a, b, chunk=False):
        if chunk:
            a = a.transpose(-3, -2) 
            b = b.transpose(-3, -2) 
        # a: [*, N_seq, N_res, C]
        ...
        outer = torch.einsum("...acb,...ade->...dceb", a, b)
        
        dtype = outer.dtype
        outer = torch.einsum("...dceb,...cef->...dbf", outer, self.output_w) + self.output_b 
    ...
    def _chunk(self, 
        ...
        for a_prime, b_prime in zip(a_reshape, b_reshape):
            outer = chunk_layer(
                partial(self._opm, b=b_prime, chunk=True),
                {"a": a_prime},
