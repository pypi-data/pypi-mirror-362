...
from scapy.layers.dhcp6 import (
    ...
    DHCP6OptIfaceId,
    DUID_EN,
    DHCP6, All_DHCP_Relay_Agents_and_Servers
)
...
class Dhcp6Pkt(BasePkt):
    ...
    def make_pkts(self, message_type) -> Union[DHCP6, DHCP6_RelayForward]:
        """
        解析并制作报文
    ...
    def dhcp6_solicit(self) -> DHCP6:
        ...
        if self.args.broadcast is False:
            solicit_pkt = self.dhcp6_relay_ward(solicit_pkt[DHCP6])

            return solicit_pkt

        return solicit_pkt

    def dhcp6_advertise(self) -> DHCP6:
        pass

    def dhcp6_request(self) -> DHCP6:
        """
        制作request包
        :return:
        """
        ...
        if self.args.broadcast is False:
            request_pkt = self.dhcp6_relay_ward(request_pkt[DHCP6])
            return request_pkt
        return request_pkt
    ...
    def dhcp6_renew(self) -> DHCP6:
        ...
        if self.args.broadcast is False:
            renew_pkt = self.dhcp6_relay_ward(renew_pkt[DHCP6])
            return renew_pkt
        return renew_pkt

    def dhcp6_release(self) -> DHCP6:
        """
        制作release包
        :return:
        """
        ...
        if self.args.broadcast is False:
            release_pkt = self.dhcp6_relay_ward(release_pkt[DHCP6])
            return release_pkt
        return release_pkt

    def dhcp6_decline(self) -> DHCP6:
        """
        制作decline包
        :return:
        """
        ...
        if self.args.broadcast is False:
            decline_pkt = self.dhcp6_relay_ward(decline_pkt[DHCP6])
            return decline_pkt
        return decline_pkt
    ...
    def dhcp6_relay_ward(self, pkt=None) -> DHCP6_RelayForward:
        ...
        if pkt.getlayer(DHCP6OptIfaceId):
            copy_pkt = copy.deepcopy(pkt)
            face_id_layer = copy.deepcopy(copy_pkt.getlayer(DHCP6OptIfaceId))
            face_id_sub_layer = copy.deepcopy(face_id_layer[1:])
            face_id_layer.remove_payload()
            del copy_pkt[DHCP6OptIfaceId]
            relay_forward_pkt = self.ether_ipv6_udp / self.relay_forward / face_id_layer
            relay_forward_pkt.add_payload(DHCP6OptRelayMsg(message=copy_pkt / face_id_sub_layer))
        else:
