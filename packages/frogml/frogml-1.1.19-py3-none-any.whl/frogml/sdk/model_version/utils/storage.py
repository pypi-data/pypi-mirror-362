...
def _log_model(config: ModelLogConfig) -> None:
    ...
    is_customer_exists_in_jml: bool = (
        jml_customer_client.check_if_customer_exists_in_jml()
    )
    ...
    is_setup_template_files: bool = SetupTemplateFilesManager(
        config=config
    ...
    zipped_code_path: Optional[str] = build_zip_code_path(
        ...
        is_setup_template_files=is_setup_template_files,
    )
...
def build_zip_code_path(
    code_dir_path: Optional[str], parent_dir_path: str, is_setup_template_files: bool
) -> Optional[str]:
    """
    ...
    :param is_setup_template_files: Flag indicating whether template files are set up
    :return: The path to the zipped code directory
    """
    if not code_dir_path and not is_setup_template_files:
        return None
