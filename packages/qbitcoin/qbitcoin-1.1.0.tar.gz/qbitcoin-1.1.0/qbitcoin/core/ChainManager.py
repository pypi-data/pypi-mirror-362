...
from qbitcoin.core import config, BlockHeader
from qbitcoin.core.config import DevConfig
from qbitcoin.core.formulas import block_reward
from qbitcoin.core.StateContainer import StateContainer
from qbitcoin.core.StateMigration import StateMigration
from qbitcoin.core.AddressState import AddressState
from qbitcoin.core.VoteStats import VoteStats
from qbitcoin.core.LastTransactions import LastTransactions
from qbitcoin.core.TransactionMetadata import TransactionMetadata
...
from qbitcoin.core.Indexer import Indexer
...
from qbitcoin.core.PoWValidator import PoWValidator
...
from qbitcoin.core.TransactionPool import TransactionPool
from qbitcoin.core.misc import logger
from qbitcoin.crypto.Qryptonight import Qryptonight
from qbitcoin.generated import qbit_pb2, qbitstateinfo_pb2
...
class ChainManager:
    ...
    def get_address_state(self,
        ...
        Note: OTS tracking removed for Falcon signature support.
        """
        optimized_address_state = self.get_optimized_address_state(address)
        ots_bitfield = []  # Empty bitfield for Falcon signatures
        transaction_hashes = list()
        tokens = OrderedDict()
        slave_pks_access_type = OrderedDict()

        # OTS tracking not needed for Falcon signatures - skip bitfield processing
        ...
        addr_state = AddressState.create(address=optimized_address_state.address,
                                         ...
                                         falcon_pk_list=[],  # Empty list since using Falcon signatures
                                         tokens=tokens,
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> qbit_pb2.SlaveMetadata:
        with self.lock:
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> qbit_pb2.TokenBalance:
        with self.lock:
    ...
    def get_headerhashes(self, start_blocknumber):
        with self.lock:
            ...
            node_header_hash = qbit_pb2.NodeHeaderHash()
    ...
    def load(self, genesis_block):
        ...
        if height == -1:
            ...
            block_number_mapping = qbit_pb2.BlockNumberMapping(headerhash=genesis_block.headerhash,
    ...
    def _try_branch_add_block(self, block, dev_config: DevConfig, check_stale=True) -> bool:
        ...
        if new_block_difficulty > last_block_difficulty:
            if self._last_block.headerhash != block.prev_headerhash:
                fork_state = qbitstateinfo_pb2.ForkState(initiator_headerhash=block.headerhash)
    ...
    def _rollback(self, forked_header_hash: bytes, fork_state: qbitstateinfo_pb2.ForkState = None):
        """
    ...
    def add_chain(self, hash_path: list, fork_state: qbitstateinfo_pb2.ForkState) -> bool:
        """
    ...
    def _fork_recovery(self, block: Block, fork_state: qbitstateinfo_pb2.ForkState) -> bool:
    ...
    def _update_block_number_mapping(self, block, batch):
        block_number_mapping = qbit_pb2.BlockNumberMapping(headerhash=block.headerhash,
    ...
    def update_state_container(self,
        ...
        if isinstance(tx, TransferTokenTransaction):
            ...
            if key not in state_container.lattice_pk.data:
                lattice_pk.load(key, qbit_pb2.LatticePKMetadata())
    ...
    def get_block_datapoint(self, headerhash):
        ...
        data_point = qbit_pb2.BlockDataPoint()
    ...
    def get_unused_ots_index2(self, address, start_ots_index=0):
        # OTS tracking not needed for Falcon signatures
        return None
