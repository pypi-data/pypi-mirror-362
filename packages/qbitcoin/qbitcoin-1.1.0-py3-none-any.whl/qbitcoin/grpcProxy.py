...
def get_addr_state(addr: bytes) -> AddressState:
    ...
    response = stub.GetAddressState(request=qbit_pb2.GetAddressStateReq(address=addr))
...
def valid_payment_permission(public_stub, master_address_state, payment_xmss, json_slave_txn):
    ...
    if access_type == -1:
        ...
        public_stub.PushTransaction(request=qbit_pb2.PushTransactionReq(transaction_signed=tx.pbdata))
        return None
...
def getblocktemplate(reserve_size, wallet_address):
    stub = get_mining_stub()
    request = qbitmining_pb2.GetBlockToMineReq(wallet_address=wallet_address.encode())
...
def getbalance():
    stub = get_public_stub()
    grpc_response = stub.GetOptimizedAddressState(request=qbit_pb2.GetAddressStateReq(address=payment_slaves[0]))
...
def transfer(destinations, fee, mixin, unlock_time):
    ...
    response = stub.PushTransaction(request=qbit_pb2.PushTransactionReq(transaction_signed=tx.pbdata))
