...
class DatabaseRegistry:
    ...
    def get_connection(self, name: str):
        ...
        # For testing purposes, we'll return a mock connection
        ...
        class MockConnection:
            ...
            def execute(self, query, params=None):
                """Mock execute method."""
                logger.info(f"Executing query on {self.config.name}: {query}")
                return MockCursor()

            def commit(self):
                """Mock commit method."""
                logger.info(f"Committing transaction on {self.config.name}")

            def rollback(self):
                """Mock rollback method."""
                logger.info(f"Rolling back transaction on {self.config.name}")
        ...
        return MockConnection(db_config)
    ...
    def auto_configure_from_url(self, url: str, name: str = None) -> DatabaseConfig:
        ...
        if not name:
            ...
            parsed = urlparse(url)
            name = f"{parsed.scheme}_{parsed.hostname}_{parsed.path.strip('/')}"
