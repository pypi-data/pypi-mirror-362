...
class DataFlowGateway:
    ...
    def _create_bulk_create_workflow(self) -> Dict[str, Any]:
        ...
        bulk_create_config = {
            ...
            "conflict_resolution": "{{ conflict_resolution | default('error') }}",
            "auto_timestamps": "{{ auto_timestamps | default(true) }}",
            "multi_tenant": "{{ multi_tenant | default(false) }}",
        }
        ...
        if self.enable_monitoring:
            workflow.add_node(
                "DataFlowTransactionMetricsNode",
                "metrics",
                {"operation_type": "bulk_create", **self.monitoring_config},
            )
    ...
    def _create_bulk_update_workflow(self) -> Dict[str, Any]:
        ...
        bulk_update_config = {
            ...
            "auto_timestamps": "{{ auto_timestamps | default(true) }}",
            "multi_tenant": "{{ multi_tenant | default(false) }}",
            "version_control": "{{ version_control | default(false) }}",
        }
    ...
    def _create_bulk_upsert_workflow(self) -> Dict[str, Any]:
        ...
        bulk_upsert_config = {
            "table_name": "{{ table_name }}",
            "batch_size": "{{ batch_size | default(1000) }}",
            "merge_strategy": "{{ merge_strategy | default('update') }}",
            "conflict_columns": "{{ conflict_columns | default(['email']) }}",
            "auto_timestamps": "{{ auto_timestamps | default(true) }}",
            "version_control": "{{ version_control | default(false) }}",
        }
    ...
    def _create_bulk_delete_workflow(self) -> Dict[str, Any]:
        ...
        bulk_delete_config = {
            ...
            "safe_mode": "{{ safe_mode | default(true) }}",
            "archive_before_delete": "{{ archive_before_delete | default(false) }}",
        }
    ...
    def _create_secure_bulk_workflow(self) -> Dict[str, Any]:
        ...
        workflow.add_node(
            "DataFlowMFANode",
            "mfa_check",
            {"require_mfa": True, **self.security_config},
        )
        ...
        # Secure bulk operation (configurable)
        secure_bulk_config = {
            "table_name": "{{ table_name }}",
    ...
    def _create_distributed_transaction_workflow(self) -> Dict[str, Any]:
        ...
        workflow.add_node(
            ...
            "transaction_manager",
            {
                "transaction_type": "{{ transaction_type | default('saga') }}",
    ...
    def _create_saga_workflow(self) -> Dict[str, Any]:
        ...
        # Use the main transaction manager with saga configuration
        workflow.add_node(
            "DataFlowTransactionManagerNode",
            "saga_coordinator",
            {
                ...
                "auto_rollback": "{{ auto_rollback | default(true) }}",
            },
        )
    ...
    def _create_audit_workflow(self) -> Dict[str, Any]:
        ...
        workflow.add_node(
            ...
            {"audit_mode": True, "log_all_access": True, **self.security_config},
        )
    ...
    async def create_nexus_gateway(self, **nexus_kwargs) -> Any:
        ...
        self.nexus = create_nexus(**nexus_config)
