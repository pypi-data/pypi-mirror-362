...
def configure(
    ...
    if Confirm.ask("\nWould you like to run the analyzer now?"):
        ...
        save_local_mode = os.environ.get('SPARK_ANALYZER_SAVE_LOCAL_MODE') == '1'
        save_local_flag = "--save-local " if save_local_mode else ""
        ...
        if connection_mode == "1":
            if history_server_url != "http://localhost:18080/api/v1":
                ...
                if cost_estimator_id:
                    os.system(f"spark-analyzer {save_local_flag}{original_args_str}--server-url '{history_server_url}' --cost-estimator-id '{cost_estimator_id}'")
                else:
                    os.system(f"spark-analyzer {save_local_flag}{original_args_str}--server-url '{history_server_url}'")
            else:
                # Default URL
                if cost_estimator_id:
                    os.system(f"spark-analyzer {save_local_flag}{original_args_str}--local --cost-estimator-id '{cost_estimator_id}'")
                else:
                    os.system(f"spark-analyzer {save_local_flag}{original_args_str}--local")
        elif connection_mode == "2":
            if cost_estimator_id:
                os.system(f"spark-analyzer {save_local_flag}{original_args_str}--browser --cost-estimator-id '{cost_estimator_id}'")
            else:
                os.system(f"spark-analyzer {save_local_flag}{original_args_str}--browser")
