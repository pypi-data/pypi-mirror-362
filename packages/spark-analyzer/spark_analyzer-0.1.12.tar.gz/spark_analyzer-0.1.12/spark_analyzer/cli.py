...
def process_app_id(app_id, applications, client, spark_stage_analyzer, s3_uploader, args, s3_upload_enabled, processed_app_ids, successful_upload_app_ids, failed_upload_app_ids, summary_app_ids, app_index=0, app_count=1):
    ...
    if s3_upload_enabled and len(stages) > 0 and not (args and getattr(args, 'save_local', False)):
        batch_size = 1000
    ...
    if args and getattr(args, 'save_local', False):
...
def run_analysis():
    ...
    try:
        ...
        if getattr(args, 'save_local', False):
            import subprocess
            
            s3_upload_enabled = False
            print("💾 Running in local save mode - analysis data will be saved locally")
            
            called_from_config = os.environ.get('SPARK_ANALYZER_SAVE_LOCAL_MODE') == '1'
            
            if not called_from_config:
                ...
                original_args = []
                if args.opt_out:
                    original_args.extend(['--opt-out', args.opt_out])
                if args.verbose:
                    original_args.append('--verbose')
                if args.debug:
                    original_args.append('--debug')
                if args.env != 'production':
                    original_args.extend(['--env', args.env])
                if args.cost_estimator_id:
                    original_args.extend(['--cost-estimator-id', args.cost_estimator_id])
                if args.server_url:
                    original_args.extend(['--server-url', args.server_url])
                if args.app_id:
                    original_args.extend(['--app-id', args.app_id])
                if args.stage_id:
                    original_args.extend(['--stage-id', args.stage_id])
                if args.upload_url:
                    original_args.extend(['--upload-url', args.upload_url])
                if args.local:
                    original_args.append('--local')
                if args.browser:
                    original_args.append('--browser')
                
                if original_args:
                    env['SPARK_ANALYZER_ORIGINAL_ARGS'] = ' '.join(original_args)
                
                result = subprocess.run(['spark-analyzer-configure'], check=False, env=env)
                if result.returncode != 0:
                    print("❌ Configuration was cancelled or failed. Exiting.")
                    sys.exit(1)
                else:
                    sys.exit(0)
            ...
            if is_databricks:
                ...
                try:
                    ...
                    if applications and len(applications) > 0:
                        ...
                        print(f"✅ Found application: {applications[0]['id']}")
                        logging.info(f"Using application ID: {applications[0]['id']}")
                    else:
                        print_error_box(
        ...
        if app_ids and len(app_ids) > 0:
            ...
            if s3_upload_enabled and successful_upload_app_ids and not getattr(args, 'save_local', False):
