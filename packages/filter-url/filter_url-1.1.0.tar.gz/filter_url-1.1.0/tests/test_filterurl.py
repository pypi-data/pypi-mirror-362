...
def test_logging_filter_fallback_disabled(caplog):
    ...
    logger.setLevel(logging.INFO)
    logger.addFilter(URLFilter(fmt='| (URL data: {filtered_url})', fallback=False))
...
def test_logging_filter_init_with_instance(caplog):
    ...
    logger.setLevel(logging.INFO)
    logger.addFilter(URLFilter(fmt='| (URL data: {filtered_url})', url_filter_instance=custom_rules))
