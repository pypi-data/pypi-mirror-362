...
class _FeatureStoreModelWrapper:
    def __init__(self, path: str):
        ...
        # TODO: deprecate latency debugger and prefer trace
        ...
        if self.eligible_for_pgsql_connection:
            self.pg_sql_fetcher = PostgresFetcher(
                self.feature_spec, self.ft_to_lookup_client, self.ft_metadata
            )
        if self.has_feature_functions:
    ...
    def _map_feature_to_data_type(
        ...
        for ft_name, ft_meta in ft_metadatas.items():
            ...
            for feature_detail in oft.features:
                table_feature_data_type_mapping[oft.online_feature_table_name][
                    feature_detail.name
                ] = feature_detail.data_type
    ...
    def _create_lookup_clients(
        ...
        ft_to_lookup_client = {}
        for ft, meta in ft_metadata.items():
            # TODO[ML-55577]: for postgres, this will create multiple Engine with the
            #   same url. we should make the clients share the same engine, i.e. connection
            #   pool.
    ...
    def _predict(
        ...
        if self.eligible_for_pgsql_connection:
            if os.getenv(ENV_SERVABLE_TYPE) == "FEATURES":
                ...
                self._latency_debug("pg_lookup_features")
            else:
                ...
                self._latency_debug("pg_get_model_input_df")
                result = self._raw_model_predict(model_input_df, params)
                self._latency_debug("model_predict")
        else:
    ...
    def _eligible_for_pgsql_connection(self):
        if os.getenv(FEATURE_SERVING_HP) != "true":
            return False
        is_basic_lookup = (
            not self.feature_spec.has_dag()
            and not self.has_feature_functions
            and not self.feature_spec.has_feature_renaming()
            and not self.feature_spec.has_default_value()
        )
        if not is_basic_lookup:
            return False
        online_stores = [
            ft_metadata.online_ft.online_store
            for ft_metadata in self.ft_metadata.values()
        ]
        if not online_stores or not all(
