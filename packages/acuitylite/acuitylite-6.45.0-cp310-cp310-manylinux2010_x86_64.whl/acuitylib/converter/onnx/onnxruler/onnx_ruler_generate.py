...
@rule_pyfunc_def
def r_softmax_get_sf_axis(self, node, tensor, name_of_node_with_axis):
    axis = self.attr_pick(node[name_of_node_with_axis], 'axis', None)
    if axis is None:
...
@rule_pyfunc_def
def r_softmax_get_reshaped_shape(self, node, tensor, name_of_node_with_axis):
    import numpy as np
    axis = self.attr_pick(node[name_of_node_with_axis], 'axis', None)
    if axis is None:
        shape = self.shape_pick(tensor['I:out0'])
        if len(shape) > 2 and self.minor_version < 13:
            axis = 1
        else:
            axis = -1
    in_shape = shape = self.shape_pick(tensor['I:out0'])
    reshapedped_shape = [np.prod(in_shape[:axis]), np.prod(in_shape[axis:])]
    return reshapedped_shape
...
@rule_pyfunc_def
def r_softmax_opset_1_12_pre_cond(self, node, tensor, name_of_node_with_axis):
    axis = self.attr_pick(node[name_of_node_with_axis], 'axis', None)
    input_rank = len(self.shape_pick(tensor['I:out0']))
    if axis is None and input_rank > 2:
        return True
    else:
        return False

@rule_pyfunc_def
def r_softmax_pre_cond(self, node, tensor, name_of_node_with_axis):
    axis = self.attr_pick(node[name_of_node_with_axis], 'axis', None)
    input_rank = len(self.shape_pick(tensor['I:out0']))
...
@rule_pyfunc_def
def r_conv_add_pre_condition(self, node, tensor):
    ret1 = self.is_single_refs(tensor['Conv:out0']) is True
    ret2 = False
    weight_data = self.tensor_to_numpy(tensor['Constant_0:out0'])
    weights = weight_data.shape[0]
    add_const = self.tensor_to_numpy(tensor['Constant_1:out0'])
    const_shape = add_const.shape
    if len(const_shape) == 0 or (len(const_shape) == 1 and (const_shape[0] == 1 or const_shape[0] == weights)):
        ret2 = True
...
@rule_pyfunc_def
def r_conv_mul_pre_condition(self, node, tensor):
    # y=(xw+b)m --> x(wm) + (bm)
    # m need to be broadcastable with w and b
    m_shape = list(self.tensor_to_numpy(tensor['Constant:out0']).shape)  # mul_const input
    b_shape = list(self.tensor_to_numpy(tensor['Constant_2:out0']).shape)  # bias
    m_rank = len(m_shape)
    b_rank = len(b_shape)
    # only could be one data array or a scalar
    if (m_rank == 0 or (m_rank == 1 and (m_shape[0] == 1 or b_shape[0] == 1 or m_shape[0] == b_shape[0]))):
        return True
    return False
...
@rule_pyfunc_def
def r_fc_check_rank(self, node, tensor, in_tensor, weight, bias = None):
    input_shape = self.shape_pick(tensor[in_tensor])
    weight_shape = self.shape_pick(tensor[weight])
    if len(input_shape) != 3 or len(weight_shape) != 3 or bias is not None:
        return False
    return True
...
"blob_map": {
    "variable": {
        'data': ["CODE", "self.tensor_to_numpy(tensor['Constant:out0']) "
                         "* self.attr_pick(node['Gemm'], 'alpha', np.float32(1))"]
    },
    "variable_1": {
        'data': ["CODE", "self.tensor_to_numpy(tensor['Constant_1:out0']) "
                         "* self.attr_pick(node['Gemm'], 'beta', np.float32(1))"]
    }
},
"priority_tip": 0,
...
"blob_map": {
    ...
    "variable": {
        'data': ["CODE", "self.tensor_to_numpy(tensor['Constant:out0']) "
                         "* self.attr_pick(node['Gemm'], 'beta', np.float32(0))"]
    }
},
"priority_tip": 0,
         ...
         {"weight": ["CODE",
          ...
          "bias":
              ["CODE",
               ...
               "* self.attr_pick(node['Gemm'], 'beta', np.float32(1.0))"]
          }
     },
"acu_inter_flow": [],
"priority_tip": 0,
...
"blob_map":
    {"fullconnect":
         {"weight":
          ...
          "bias":
              ["CODE",
               "self.tensor_to_numpy(tensor['Constant:out0']) * self.attr_pick(node['Gemm'], 'beta', np.float32(1.0))"]
          }
     },
"priority_tip": 0,
"pre_condition": "self.attr_pick(node['Gemm'], 'transA', 0) == 0 and"\
...
r_matmul_to_fc = {
"ruler_name": "r_matmul_to_fc",
"src_ops_alias": ["MatMul", "Constant"],
"src_inter_flow": [["Constant:out0", "MatMul:in1"]],
"src_in_anchor": [["I_0:out0", "MatMul:in0"]],
"src_out_tensor": ["MatMul:out0"],
"acu_lys_alias": ["fullconnect"],
"src_acu_in_tensor_map": [["I_0:out0", "fullconnect:in0"]],
"src_acu_out_tensor_map": [["MatMul:out0", "fullconnect:out0"]],
"acu_inter_flow": [],
"param_map": {"fullconnect":
                  {"weights": ["INT", "CODE", "self.shape_pick(tensor['Constant:out0'])[-1]"],
                   "axis": ["INT", "VALUE", 2],
                   "bias": ["BOOL", "VALUE", False]}
              },
"blob_map": {"fullconnect":
                 {"weight":["CODE", "self.matmul_weight(tensor['Constant:out0'])"],}
             },
"priority_tip": 0,
"pre_condition": r_fc_check_rank(in_tensor='I_0:out0', weight='Constant:out0'),
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_matmul_to_fc)
...
"src_ops_alias": ["Div", "Mul", "Constant", "Clip", "Add", "Constant_1"],
"src_inter_flow": [["Mul:out0", "Div:in0"], ["Constant:out0", "Div:in1"],
                   ["Clip:out0", "Mul:in1"], ["Add:out0", "Clip:in0"], ["Constant_1:out0", "Add:in1"]],
...
"priority_tip": 0,
"pre_condition": "(self.tensor_to_numpy(tensor['Constant:out0']) == 6.0).all() and "\
                "(self.tensor_to_numpy(tensor['Constant_1:out0']) == 3.0).all()",
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
...
"src_ops_alias": ["Mul", "Mul_1", "Clip", "Constant", "Add", "Constant_1", "Constant_2", "Constant_3"],
"src_inter_flow": [["Mul_1:out0", "Mul:in1"], ["Clip:out0", "Mul_1:in0"], ["Constant:out0", "Mul_1:in1"],
...
"blob_map": {"hard_swish": {}},
"priority_tip": 0,
"pre_condition": "(self.tensor_to_numpy(tensor['Constant_2:out0']) == 6.0).all() and "\
                "(self.tensor_to_numpy(tensor['Constant_1:out0']) == 0.0).all() and "\
...
"dilation": ["INTS", "CODE", "self.conv_dilation(node['Conv'], self.shape_pick(tensor['Constant_0:out0']))"],
"padding":
   ["STRING",
    "CODE",
...
r_conv_dequantizelinear_bias = {
"ruler_name": "r_conv_dequantizelinear_bias",
"src_ops_alias": ["Conv", "Constant_0", "Constant_1", "DequantizeLinear", "Constant_2"],
"src_inter_flow": [["Constant_0:out0", "Conv:in1"], ["Constant_1:out0", "DequantizeLinear:in0"],
                   ["Constant_2:out0", "DequantizeLinear:in1"], ["Constant_1:out0", "DequantizeLinear:in2"],
                   ["DequantizeLinear:out0", "Conv:in2"]],
"src_in_anchor": [["I:out0", "Conv:in0"]],
"src_out_tensor": ["Conv:out0"],
"acu_lys_alias": ["convolution"],
"src_acu_in_tensor_map": [["I:out0", "convolution:in0"]],
...
"blob_map": {"convolution":
                 {"weight": ["CODE", "self.tensor_to_numpy(tensor['Constant_0:out0'])"],
                  "bias": ["CODE", "self.qtensor_to_numpy(tensor['Constant_1:out0'], "
                                   "self.tensor_to_numpy(tensor['Constant_2:out0']),"
                                   " self.tensor_to_numpy(tensor['Constant_1:out0']),)"]}},
"acu_inter_flow": [],
"priority_tip": 0,
"extension": [
["CODE", "self.qnt_coef_tensor('bias', acu_ly['convolution'], tensor['Constant_2:out0'], tensor['Constant_1:out0'])"]
],
"pre_condition": "len(self.shape_pick(tensor['Constant_0:out0'])) == 4",
"src_ops_main_version": None,
"src_ops_minior_version": None}
ruler_list.append(r_conv_dequantizelinear_bias)
# Constant_1: bias and its zp, Constant_2: bias's scale
...
"acu_inter_flow": [],
"priority_tip": 0,
"pre_condition": r_conv_add_pre_condition(),
"src_ops_main_version": None,
"src_ops_minior_version": None}
...
r_mm_nb_mean_variance_instancenormalize = {
"ruler_name": 'r_mm_nb_mean_variance_instancenormalize',
"src_ops_alias": ["BatchNormalization", "Constant", "Constant_1", "Squeeze", "Div", "ReduceMean", "Constant_2",
                  "ReduceSumSquare", "Constant_4", "Sub"],
"src_inter_flow": [["Constant:out0", "BatchNormalization:in1"], ["Constant_1:out0", "BatchNormalization:in2"],
                   ["Squeeze:out0", "BatchNormalization:in3"], ["Div:out0", "BatchNormalization:in4"],
                   ["ReduceMean:out0", "Squeeze:in0"], ["Constant_2:out0", "Squeeze:in1"],
                   ["ReduceSumSquare:out0", "Div:in0"], ["Constant_4:out0", "Div:in1"],
                   ["Sub:out0", "ReduceSumSquare:in0"],
                    ["ReduceMean:out0", "Sub:in1"],
                   ],
"src_in_anchor": [["I_0:out0", "ReduceMean:in0"], ["I_0:out0", "Sub:in0"],
                  ["I_0:out0", "BatchNormalization:in0"]],
"src_out_tensor": ["BatchNormalization:out0"],
"acu_lys_alias": ["instancenormalize"],
"src_acu_in_tensor_map": [["I_0:out0", "instancenormalize:in0"]],
"src_acu_out_tensor_map": [["BatchNormalization:out0", "instancenormalize:out0"]],
"acu_inter_flow": [],
"param_map":{
    "instancenormalize": {"eps": ["FLOAT", "CODE", "self.attr_pick(node['BatchNormalization'], 'epsilon', 10e-9)"],
                       "axis_list": ["INTS", "CODE", "self.reducex_axis_list("
                                                        "node['ReduceMean'],"
                                                        "self.shape_pick(tensor['I_0:out0']))"],
                       "keep_dims": ["BOOL", "CODE", "self.attr_pick(node['ReduceSumSquare'],"
                                                     "'keepdims', True)"],
                       }
},
"blob_map": {"instancenormalize": {"scale": ['CODE', "self.tensor_to_numpy(tensor['Constant:out0'])"],
             "bias": ['CODE', "self.tensor_to_numpy(tensor['Constant_1:out0'])"]}},
"priority_tip": 0,
"pre_condition": None,
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_mm_nb_mean_variance_instancenormalize)
...
r_maxpool_beta = {
    "ruler_name": "r_maxpool_beta",
    "src_ops_alias": ["MaxPool"],
    "src_inter_flow": [],
    "src_in_anchor": [["I_0:out0", "MaxPool:in0"]],
    "src_out_tensor": ["MaxPool:out0", "MaxPool:out1"],
    "acu_lys_alias": ["pooling"],
    "src_acu_in_tensor_map": [["I_0:out0", "pooling:in0"]],
    "src_acu_out_tensor_map": [["MaxPool:out0", "pooling:out0"], ["MaxPool:out1", "pooling:out1"]],
    "acu_inter_flow": [],
    "param_map": {"pooling": {
        "type": ["STRING", "VALUE", "MAX"],
        "pad_method": ["STRING", "CODE",
                       "'padding_const' if self.attr_pick(node['MaxPool'], 'auto_pad', None) == None or "
                       "self.attr_pick(node['MaxPool'], 'auto_pad') == 'NOTSET' else 'auto'"],
        "pad_h": ["INT", "CODE", "self.attr_pick(node['MaxPool'], 'pads', [0, 0, 0, 0])[0]"],
        "ksize_w": ["INT", "CODE", "self.attr_pick(node['MaxPool'], 'kernel_shape')[1]"],
        "stride_w": ["INT", "CODE", "self.attr_pick(node['MaxPool'], 'strides', [1, 1])[1]"],
        "ksize_h": ["INT", "CODE", "self.attr_pick(node['MaxPool'], 'kernel_shape')[0]"],
    ...
    "src_ops_main_version": None,
    "src_ops_minior_version": [1, -1]}
ruler_list.append(r_maxpool_beta)
...
r_softmax_opset_1_12 = {
"ruler_name": "r_softmax_opset_1_12",
"src_ops_alias": ["Softmax"],
"src_inter_flow": [],
"src_in_anchor": [["I:out0", "Softmax:in0"]],
"src_out_tensor": ["Softmax:out0"],
"acu_lys_alias": ["reshape_0","softmax","reshape_1"],
"src_acu_in_tensor_map": [["I:out0", "reshape_0:in0"]],
"src_acu_out_tensor_map": [["Softmax:out0", "reshape_1:out0"]],
"acu_inter_flow": [["reshape_0:out0", "softmax:in0"],["softmax:out0", "reshape_1:in0"]],
"param_map": {
"reshape_0": {"shape": ['INTS', 'PYFUNC', r_softmax_get_reshaped_shape(name_of_node_with_axis="Softmax")]},
"softmax": {"sf_axis": ['INT', 'VALUE', 1]},
"reshape_1": {"shape": ['INTS', 'CODE', "self.shape_pick(tensor['I:out0'])"]}
},
"blob_map": {},
"priority_tip": 0,
"pre_condition": r_softmax_opset_1_12_pre_cond(name_of_node_with_axis="Softmax"),
"src_ops_main_version": None,
"src_ops_minior_version": [1, 12]}
ruler_list.append(r_softmax_opset_1_12)
...
"param_map": {
"softmax": {"sf_axis": ['INT', 'PYFUNC', r_softmax_get_sf_axis(name_of_node_with_axis="Softmax")]}
},
"blob_map": {},
"acu_inter_flow": [],
"priority_tip": 0,
"pre_condition": r_softmax_pre_cond(name_of_node_with_axis="Softmax"),
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_softmax)

r_log_softmax_opset_1_12 = {
"ruler_name": "r_log_softmax_opset_1_12",
"src_ops_alias": ["LogSoftmax"],
"src_inter_flow": [],
"src_in_anchor": [["I:out0", "LogSoftmax:in0"]],
"src_out_tensor": ["LogSoftmax:out0"],
"acu_lys_alias": ["reshape_0","log_softmax","reshape_1"],
"src_acu_in_tensor_map": [["I:out0", "reshape_0:in0"]],
"src_acu_out_tensor_map": [["LogSoftmax:out0", "reshape_1:out0"]],
"acu_inter_flow": [["reshape_0:out0", "log_softmax:in0"],["log_softmax:out0", "reshape_1:in0"]],
"param_map": {
"reshape_0": {"shape": ['INTS', 'PYFUNC', r_softmax_get_reshaped_shape(name_of_node_with_axis="LogSoftmax")]},
"log_softmax": {"sf_axis": ['INT', 'VALUE', 1]},
"reshape_1": {"shape": ['INTS', 'CODE', "self.shape_pick(tensor['I:out0'])"]}
},
"blob_map": {"log_softmax": {}},
"priority_tip": 0,
"pre_condition": r_softmax_opset_1_12_pre_cond(name_of_node_with_axis="LogSoftmax"),
"src_ops_main_version": None,
"src_ops_minior_version": [1, 12]}
ruler_list.append(r_log_softmax_opset_1_12)
...
"acu_inter_flow": [],
"param_map": {
"log_softmax": {"sf_axis": ['INT', 'PYFUNC', r_softmax_get_sf_axis(name_of_node_with_axis="LogSoftmax")]}
},
"blob_map": {"log_softmax": {}},
"priority_tip": 0,
"pre_condition": r_softmax_pre_cond(name_of_node_with_axis="LogSoftmax"),
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
...
r_l2normalize_2 = {
"ruler_name": "r_l2normalize_2",
"src_ops_alias": ["Div", "Expand", "Clip", "Constant_1", "ReduceL2", "Constant"],
"src_inter_flow": [["Expand:out0", "Div:in1"], ["Clip:out0", "Expand:in0"], ["Constant_1:out0", "Expand:in1"],
                   ["ReduceL2:out0", "Clip:in0"], ["Constant:out0", "Clip:in1"]],
"src_in_anchor": [["I_0:out0", "ReduceL2:in0"], ["I_0:out0", "Div:in0"]],
"src_out_tensor": ["Div:out0"],
"acu_lys_alias": ["l2normalize"],
"src_acu_in_tensor_map": [["I_0:out0", "l2normalize:in0"]],
"src_acu_out_tensor_map": [["Div:out0", "l2normalize:out0"]],
"acu_inter_flow": [],
"param_map": {"l2normalize": {
    'l2n_dim': ['INT', 'CODE', "self.attr_pick(node['ReduceL2'], 'axes')[0]"],
    'eps': ['FLOAT', 'CODE', "self.tensor_to_numpy(tensor['Constant:out0'])"]}
},
"blob_map": {"l2normalize": {}},
"priority_tip": 0,
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_l2normalize_2)

r_l2normalaize_3 = {
"ruler_name": "r_l2normalaize_3",
"src_ops_alias": ["Div", "Unsqueeze", "Add", "Constant", "Pow", "Constant_1",
                  "ReduceSum", "Constant_2", "Pow_1", "Abs", "Constant_3"],
"src_inter_flow": [["Unsqueeze:out0", "Div:in1"], ["Add:out0", "Unsqueeze:in0"], ["Constant:out0", "Unsqueeze:in1"],
                   ["Pow:out0", "Add:in0"], ["Constant_1:out0", "Add:in1"], ["ReduceSum:out0", "Pow:in0"],
                   ["Constant_2:out0", "Pow:in1"], ["Pow_1:out0", "ReduceSum:in0"], ["Constant:out0", "ReduceSum:in1"],
                   ["Abs:out0", "Pow_1:in0"], ["Constant_3:out0", "Pow_1:in1"]],
"src_in_anchor": [["I_0:out0", "Abs:in0"], ["I_0:out0", "Div:in0"]],
"src_out_tensor": ["Div:out0"],
"acu_lys_alias": ["l2normalize"],
"src_acu_in_tensor_map": [["I_0:out0", "l2normalize:in0"]],
"src_acu_out_tensor_map": [["Div:out0", "l2normalize:out0"]],
...
"param_map": {
    "l2normalize": {
        'l2n_dim': ['INT', 'CODE', "self.tensor_to_numpy(tensor['Constant:out0'])"],
        #'l2n_dim': ['INT', 'CODE', "self.attr_pick(node['ReduceSum'], 'axes')"],
        'eps': ['FLOAT', 'CODE', "self.tensor_to_numpy(tensor['Constant_1:out0'])"]
    }
},
"blob_map": {"l2normalize": {}},
"priority_tip": 0,
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_l2normalaize_3)

r_l2normalize_4 = {
"ruler_name": "r_l2normalize_4",
"src_ops_alias": ["Div", "Expand", "Clip", "Constant_3", "Pow", "Constant", "ReduceSum",
                  "Constant_1", "Pow_1", "Abs", "Constant_2"],
"src_inter_flow": [["Expand:out0", "Div:in1"], ["Clip:out0", "Expand:in0"], ["Constant_3:out0", "Expand:in1"],
                   ["Pow:out0", "Clip:in0"], ["Constant:out0", "Clip:in1"], ["ReduceSum:out0", "Pow:in0"],
                   ["Constant_1:out0", "Pow:in1"], ["Pow_1:out0", "ReduceSum:in0"], ["Abs:out0", "Pow_1:in0"],
                   ["Constant_2:out0", "Pow_1:in1"]],
"src_in_anchor": [["I_0:out0", "Abs:in0"], ["I_0:out0", "Div:in0"]],
"src_out_tensor": ["Div:out0"],
"acu_lys_alias": ["l2normalize"],
"src_acu_in_tensor_map": [["I_0:out0", "l2normalize:in0"]],
...
"acu_inter_flow": [],
"param_map": {"l2normalize": {
    'l2n_dim': ['INT', 'CODE', "self.attr_pick(node['ReduceSum'], 'axes')[0]"],
    'eps': ['FLOAT', 'CODE', "self.tensor_to_numpy(tensor['Constant:out0'])"]}
},
"blob_map": {"l2normalize": {}},
"priority_tip": 0,
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_l2normalize_4)
...
r_resize_bicubic = {
"ruler_name": "r_resize_bicubic",
"src_ops_alias": ["Resize", "Constant"],
"src_inter_flow": [["Constant:out0", "Resize:in2"]],
"src_in_anchor": [["I_0:out0", "Resize:in0"]],
"src_out_tensor": ["Resize:out0"],
"acu_lys_alias": ["image_resize"],
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_resize_bicubic)
#Resize:bic/Resize_output_0;Constant:bic/Constant_output_0
...
r_conv_mul = {
"ruler_name": "r_conv_mul",
"src_ops_alias": ["Mul", "Constant", "Conv", "Constant_1", "Constant_2"],
"src_inter_flow": [["Constant:out0", "Mul:in0"], ["Conv:out0", "Mul:in1"], ["Constant_1:out0", "Conv:in1"],
                   ["Constant_2:out0", "Conv:in2"]],
"src_in_anchor": [["I_0:out0", "Conv:in0"]],
"src_out_tensor": ["Mul:out0"],
"acu_lys_alias": ["convolution"],
"src_acu_in_tensor_map": [["I_0:out0", "convolution:in0"]],
"src_acu_out_tensor_map": [["Mul:out0", "convolution:out0"]],
"acu_inter_flow": [],
"param_map": {"convolution": {
    "weights": ["INT", "CODE", "self.shape_pick(tensor['Constant_1:out0'])[0]"],
    "pad_method":
        ["STRING", "CODE", "'auto' if self.attr_pick(node['Conv'], 'pads', None) == None else 'padding_const'"],
    "bias": ["BOOL", "VALUE", True],
    "ksize_w": ["INT", "CODE", "self.shape_pick(tensor['Constant_1:out0'])[3]"],
    "ksize_h": ["INT", "CODE", "self.shape_pick(tensor['Constant_1:out0'])[2]"],
    "stride_w": ["INT", "CODE", "self.attr_pick(node['Conv'], 'strides', [1, 1])[1]"],
    "stride_h": ["INT", "CODE", "self.attr_pick(node['Conv'], 'strides', [1, 1])[0]"],
    "group_number": ["INT", "CODE", "self.attr_pick(node['Conv'], 'group', 1)"],
    "dilation": ['INT', 'CODE', "self.attr_pick(node['Conv'], 'dilations') "\
...
"blob_map": {"convolution": {
    "weight": ["CODE",
               "self.tensor_to_numpy(tensor['Constant_1:out0'])*self.tensor_to_numpy(tensor['Constant:out0'])"],
    "bias": ["CODE",
             "self.tensor_to_numpy(tensor['Constant_2:out0']) * self.tensor_to_numpy(tensor['Constant:out0'])"]}
},
"priority_tip": 0,
"pre_condition": r_conv_mul_pre_condition(),
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_conv_mul)
...
"src_ops_alias": ["Mul", "Mul_1", "Constant", "Div", "Constant_1", "Sqrt",
                  "Add", "ReduceMean", "Constant_2", "Pow", "Constant_3"],
"src_inter_flow": [["Mul_1:out0", "Mul:in1"], ["Constant:out0", "Mul:in0"], ["Div:out0", "Mul_1:in1"],
                   ["Constant_1:out0", "Div:in0"], ["Sqrt:out0", "Div:in1"], ["Add:out0", "Sqrt:in0"],
                   ["ReduceMean:out0", "Add:in0"], ["Constant_2:out0", "Add:in1"], ["Pow:out0", "ReduceMean:in0"],
                   ["Constant_3:out0", "Pow:in1"]],
"src_in_anchor": [["I_0:out0", "Pow:in0"], ["I_0:out0", "Mul_1:in0"]],
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_rmsnormalize_1)

r_rmsnormalize_2 = {
"ruler_name": "r_rmsnormalize_2",
"src_ops_alias": ["Mul", "Div", "Constant", "Clip", "Mul_1", "Constant_1", "Sqrt", "Constant_2",
                  "ReduceSum", "Mul_2"],
"src_inter_flow": [["Div:out0", "Mul:in0"], ["Constant:out0", "Mul:in1"], ["Clip:out0", "Div:in1"],
                   ["Mul_1:out0", "Clip:in0"], ["Constant_1:out0", "Clip:in1"], ["Sqrt:out0", "Mul_1:in0"],
                   ["Constant_2:out0", "Mul_1:in1"], ["ReduceSum:out0", "Sqrt:in0"], ["Mul_2:out0", "ReduceSum:in0"]],
"src_in_anchor": [["I_0:out0", "Mul_2:in0"], ["I_0:out0", "Mul_2:in1"], ["I_0:out0", "Div:in0"]],
"src_out_tensor": ["Mul:out0"],
"acu_lys_alias": ["rmsnormalize"],
"src_acu_in_tensor_map": [["I_0:out0", "rmsnormalize:in0"]],
"src_acu_out_tensor_map": [["Mul:out0", "rmsnormalize:out0"]],
...
"param_map": {
    "rmsnormalize": {
        "axis_list": ["INTS", "CODE", "self.attr_pick(node['ReduceSum'], 'axes', [-1])"],
        "eps": ['FLOAT', 'CODE', "self.tensor_to_numpy(tensor['Constant_1:out0'])"],
    }
},
"blob_map": {
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_rmsnormalize_2)

r_rmsnormalize_3 = {
"ruler_name": "r_rmsnormalize_3",
"src_ops_alias": ["Mul", "Div", "Constant", "Clip", "Mul_1", "Constant_1", "Sqrt", "Constant_2",
                  "ReduceSum", "Pow"],
"src_inter_flow": [["Div:out0", "Mul:in0"], ["Constant:out0", "Mul:in1"], ["Clip:out0", "Div:in1"],
                   ["Mul_1:out0", "Clip:in0"], ["Constant_1:out0", "Clip:in1"], ["Sqrt:out0", "Mul_1:in0"],
                   ["Constant_2:out0", "Mul_1:in1"], ["ReduceSum:out0", "Sqrt:in0"], ["Pow:out0", "ReduceSum:in0"]],
"src_in_anchor": [["I_0:out0", "Pow:in0"], ["I_0:out0", "Div:in0"]],
"src_out_tensor": ["Mul:out0"],
"acu_lys_alias": ["rmsnormalize"],
"src_acu_in_tensor_map": [["I_0:out0", "rmsnormalize:in0"]],
"src_acu_out_tensor_map": [["Mul:out0", "rmsnormalize:out0"]],
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_rmsnormalize_3)

r_rmsnormalize_4 = {
"ruler_name": "r_rmsnormalize",
"src_ops_alias": ["Mul", "Mul_1", "Constant", "Div", "Constant_1", "Sqrt",
                  "Add", "ReduceMean", "Constant_2", "Pow", "Constant_3", "Cast"],
"src_inter_flow": [["Cast:out0", "Mul:in1"], ["Constant:out0", "Mul:in0"],["Mul_1:out0", "Cast:in0"],
                   ["Div:out0", "Mul_1:in1"],["Constant_1:out0", "Div:in0"], ["Sqrt:out0", "Div:in1"],
                   ["Add:out0", "Sqrt:in0"],["ReduceMean:out0", "Add:in0"],
                   ["Constant_2:out0", "Add:in1"], ["Pow:out0", "ReduceMean:in0"],
                   ["Constant_3:out0", "Pow:in1"]],
"src_in_anchor": [["I_0:out0", "Pow:in0"], ["I_0:out0", "Mul_1:in0"]],
"src_out_tensor": ["Mul:out0"],
"acu_lys_alias": ["rmsnormalize"],
...
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_rmsnormalize_4)

r_l1_layernormalize = {
    "ruler_name": "r_l1_layernormalize",
    "src_ops_alias": ["Add", "Mul", "Constant", "Div", "Constant_1", "Sub", "Mul_1", "ReduceMean",
                      "Constant_2", "ReduceSum", "Abs", "Constant_3"],
    "src_inter_flow": [["Mul:out0", "Add:in0"], ["Constant:out0", "Add:in1"], ["Div:out0", "Mul:in0"],
                       ["Constant_1:out0", "Mul:in1"], ["Sub:out0", "Div:in0"], ["Mul_1:out0", "Div:in1"],
                       ["ReduceMean:out0", "Sub:in1"], ["Constant_2:out0", "Mul_1:in0"],
                       ["ReduceSum:out0", "Mul_1:in1"], ["Abs:out0", "ReduceSum:in0"],
                       ["Constant_3:out0", "ReduceSum:in1"], ["Sub:out0", "Abs:in0"]],
    "src_in_anchor": [["I_0:out0", "ReduceMean:in0"], ["I_0:out0", "Sub:in0"]],
    "src_out_tensor": ["Add:out0"],
    "acu_lys_alias": ["l1_layernormalize"],
    "src_acu_in_tensor_map": [["I_0:out0", "l1_layernormalize:in0"]],
    "src_acu_out_tensor_map": [["Add:out0", "l1_layernormalize:out0"]],
    "acu_inter_flow": [],
    "param_map": {"l1_layernormalize": {
        "axis_list": ['INTS', 'CODE', "self.tensor_to_numpy(tensor['Constant_3:out0']).tolist()"]}},
    "blob_map": {"l1_layernormalize": {
        "scale": ['CODE', "self.tensor_to_numpy(tensor['Constant_1:out0'])"],
        "bias": ['CODE', "self.tensor_to_numpy(tensor['Constant:out0'])"],
        "gamma": ['CODE', "self.tensor_to_numpy(tensor['Constant_2:out0'])"]}},
    "priority_tip": 0,
    "pre_condition": "self.tensor_to_numpy(tensor['Constant_3:out0']) == "
                     "self.attr_pick(node['ReduceMean'], 'axes', [-1])",
    "src_ops_main_version": None,
    "src_ops_minior_version": [1, -1]}
ruler_list.append(r_l1_layernormalize)

r_l1_layernormalize_1 = {
    "ruler_name": "r_l1_layernormalize_1",
    "src_ops_alias": ["Add", "Mul", "Constant", "Div", "Constant_1", "Sub", "Add_1", "ReduceMean", "Mul_1",
                      "Constant_2", "Constant_3", "ReduceSum", "Abs", "Constant_4"],
    "src_inter_flow": [["Mul:out0", "Add:in0"], ["Constant:out0", "Add:in1"], ["Div:out0", "Mul:in0"],
                       ["Constant_1:out0", "Mul:in1"], ["Sub:out0", "Div:in0"], ["Add_1:out0", "Div:in1"],
                       ["ReduceMean:out0", "Sub:in1"], ["Mul_1:out0", "Add_1:in0"], ["Constant_2:out0", "Add_1:in1"],
                       ["Constant_3:out0", "Mul_1:in0"], ["ReduceSum:out0", "Mul_1:in1"], ["Abs:out0", "ReduceSum:in0"],
                       ["Constant_4:out0", "ReduceSum:in1"], ["Sub:out0", "Abs:in0"]],
    "src_in_anchor": [["I_0:out0", "ReduceMean:in0"], ["I_0:out0", "Sub:in0"]],
    "src_out_tensor": ["Add:out0"],
    "acu_lys_alias": ["l1_layernormalize"],
    "src_acu_in_tensor_map": [["I_0:out0", "l1_layernormalize:in0"]],
    ...
    "acu_inter_flow": [],
    "param_map": {"l1_layernormalize": {
        "axis_list": ['INTS', 'CODE', "self.tensor_to_numpy(tensor['Constant_4:out0']).tolist()"],
        "eps": ['FLOAT', 'CODE', "self.tensor_to_numpy(tensor['Constant_2:out0'])"]}},
    "blob_map": {"l1_layernormalize": {
        "scale": ['CODE', "self.tensor_to_numpy(tensor['Constant_1:out0'])"],
        "bias": ['CODE', "self.tensor_to_numpy(tensor['Constant:out0'])"],
        "gamma": ['CODE', "self.tensor_to_numpy(tensor['Constant_3:out0'])"]}},
    "priority_tip": 0,
    "pre_condition": "self.tensor_to_numpy(tensor['Constant_4:out0']) == "
                     "self.attr_pick(node['ReduceMean'], 'axes', [-1])",
    "src_ops_main_version": None,
    "src_ops_minior_version": [1, -1]}
ruler_list.append(r_l1_layernormalize_1)
...
r_layernormalize_7 = {
"ruler_name": "r_layernormalize_7",
"src_ops_alias": ["Div", "Sub", "Sqrt", "ReduceMean", "Add", "ReduceMean_1", "Constant", "Pow", "Constant_1"],
"src_inter_flow": [["Sub:out0", "Div:in0"], ["Sqrt:out0", "Div:in1"], ["ReduceMean:out0", "Sub:in1"],
                   ["Add:out0", "Sqrt:in0"], ["ReduceMean_1:out0", "Add:in0"], ["Constant:out0", "Add:in1"],
                   ["Pow:out0", "ReduceMean_1:in0"], ["Sub:out0", "Pow:in0"], ["Constant_1:out0", "Pow:in1"]],
"src_in_anchor": [["I_0:out0", "ReduceMean:in0"], ["I_0:out0", "Sub:in0"]],
"src_out_tensor": ["Div:out0"],
"acu_lys_alias": ["layernormalize"],
"src_acu_in_tensor_map": [["I_0:out0", "layernormalize:in0"]],
"src_acu_out_tensor_map": [["Div:out0", "layernormalize:out0"]],
"acu_inter_flow": [],
"param_map": {
    "layernormalize": {
        "axis_list": ['INTS', 'CODE', "self.attr_pick(node['ReduceMean'], 'axes', [-1])"],
...
"blob_map": {"layernormalize": {}},
"priority_tip": 0,
"pre_condition": None,
"src_ops_main_version": None,
"src_ops_minior_version": [1, -1]}
ruler_list.append(r_layernormalize_7)
...
r_gelu_5 = {
    ...
    "src_ops_alias": ["Gelu"],
    "src_inter_flow": [],
    "src_in_anchor": [["I_0:out0", "Gelu:in0"]],
    "src_out_tensor": ["Gelu:out0"],
    "acu_lys_alias": ["gelu"],
    "src_acu_in_tensor_map": [["I_0:out0", "gelu:in0"]],
    "src_acu_out_tensor_map": [["Gelu:out0", "gelu:out0"]],
    "acu_inter_flow": [],
    "param_map": {"gelu": {
        'approximate': ['BOOL', 'CODE',
                        "False if self.attr_pick(node['Gelu'], 'approximate', 'none') == 'none' else True"]}},
    "blob_map": {"gelu": {}},
    "priority_tip": 0,
    "pre_condition": None,
    "src_ops_main_version": None,
    "src_ops_minior_version": [1, -1]}
ruler_list.append(r_gelu_5)
...
r_rope = {
    "ruler_name": "r_rope",
    "src_ops_alias": ["Concat", "Sub", "Add", "Mul", "Mul_1", "Mul_2", "Mul_3", "Slice", "Slice_1",
                      "Constant", "Constant_1", "Constant_2", "Constant_3", "Constant_4"],
    "src_inter_flow": [["Sub:out0", "Concat:in0"], ["Add:out0", "Concat:in1"], ["Mul:out0", "Sub:in0"],
                   ["Mul_1:out0", "Sub:in1"], ["Mul_2:out0", "Add:in0"], ["Mul_3:out0", "Add:in1"],
                   ["Slice:out0", "Mul:in0"], ["Slice_1:out0", "Mul_1:in0"], ["Slice:out0", "Mul_2:in0"],
                   ["Slice_1:out0", "Mul_3:in0"], ["Constant:out0", "Slice:in1"], ["Constant_1:out0", "Slice:in2"],
                   ["Constant_2:out0", "Slice:in3"], ["Constant_3:out0", "Slice:in4"],
                   ["Constant_1:out0", "Slice_1:in1"], ["Constant_4:out0", "Slice_1:in2"],
                   ["Constant_2:out0", "Slice_1:in3"], ["Constant_3:out0", "Slice_1:in4"]],
    "src_in_anchor": [["I_0:out0", "Slice:in0"], ["I_0:out0", "Slice_1:in0"], ["I_1:out0", "Mul:in1"],
                      ["I_1:out0", "Mul_3:in1"], ["I_2:out0", "Mul_1:in1"], ["I_2:out0", "Mul_2:in1"]],
    "src_out_tensor": ["Concat:out0"],
    "acu_lys_alias": ["rope"],
    "src_acu_in_tensor_map": [["I_0:out0", "rope:in0"], ["I_1:out0", "rope:in1"], ["I_2:out0", "rope:in2"]],
    "src_acu_out_tensor_map": [["Concat:out0", "rope:out0"]],
    "acu_inter_flow": [],
    "param_map": {"rope": {'axis': ['INT', 'CODE', "self.attr_pick(node['Concat'], 'axis')"]}},
    "blob_map": {},
    "priority_tip": 0,
    "pre_condition": None,
    "src_ops_main_version": None,
    "src_ops_minior_version": [1, -1]}
ruler_list.append(r_rope)

r_rope_1 = {
    "ruler_name": "r_rope_1",
    "src_ops_alias": ["Concat", "Unsqueeze", "Unsqueeze_1", "Sub", "Constant", "Add", "Mul", "Mul_1", "Mul_2", "Mul_3",
                      "Squeeze", "Constant_1", "Squeeze_1", "Constant_2", "Split", "Constant_3", "Constant_4"],
    "src_inter_flow": [["Unsqueeze:out0", "Concat:in0"], ["Unsqueeze_1:out0", "Concat:in1"],
                       ["Sub:out0", "Unsqueeze:in0"], ["Constant:out0", "Unsqueeze:in1"],
                       ["Add:out0", "Unsqueeze_1:in0"], ["Constant:out0", "Unsqueeze_1:in1"],
                       ["Mul:out0", "Sub:in0"], ["Mul_1:out0", "Sub:in1"], ["Mul_2:out0", "Add:in0"],
                       ["Mul_3:out0", "Add:in1"], ["Squeeze:out0", "Mul:in0"], ["Constant_1:out0", "Mul:in1"],
                       ["Squeeze_1:out0", "Mul_1:in0"], ["Constant_2:out0", "Mul_1:in1"],
                       ["Squeeze_1:out0", "Mul_2:in0"], ["Constant_1:out0", "Mul_2:in1"], ["Squeeze:out0", "Mul_3:in0"],
                       ["Constant_2:out0", "Mul_3:in1"], ["Split:out0", "Squeeze:in0"],
                       ["Constant_3:out0", "Squeeze:in1"], ["Split:out1", "Squeeze_1:in0"],
                       ["Constant_3:out0", "Squeeze_1:in1"], ["Constant_4:out0", "Split:in1"]],
    "src_in_anchor": [["I_0:out0", "Split:in0"]],
    "src_out_tensor": ["Concat:out0"],
    "acu_lys_alias": ["rope", "variable", "variable_1"],
    "src_acu_in_tensor_map": [["I_0:out0", "rope:in0"]],
    "src_acu_out_tensor_map": [["Concat:out0", "rope:out0"]],
    "acu_inter_flow": [["variable:out0", "rope:in1"], ["variable_1:out0", "rope:in2"]],
    "param_map": {"rope": {
            'axis': ['INT', 'CODE', "-1"],
            'interleaved': ['BOOL', 'CODE', "True if len(self.shape_pick(tensor['I_0:out0'])) == 5 else False"]
        },
        "variable": {
            'shape': ['ORIGIN', 'CODE', "self.shape_pick(tensor['Constant_1:out0'])"],
        },
    ...
    "blob_map": {
        ...
        "variable_1": {
            'data': ["CODE", "self.tensor_to_numpy(tensor['Constant_2:out0'])"]
        }},
    "priority_tip": 0,
    "pre_condition": "len(self.shape_pick(tensor['I_0:out0'])) == 5",
    "src_ops_main_version": None,
    "src_ops_minior_version": [1, -1]}
ruler_list.append(r_rope_1)
