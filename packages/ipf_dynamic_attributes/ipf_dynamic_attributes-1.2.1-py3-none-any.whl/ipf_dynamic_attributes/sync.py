...
from ipf_dynamic_attributes.configs import Config, TableRule, ConfigRule, MultiConfig, InventoryRule, load_config
from ipf_dynamic_attributes.models import Attribute, MatchedRule, MatchedConfig, CONFIG_MAP
...
class AttributeSync(BaseModel):
    ...
    def _process_table_rule(self, rule: TableRule) -> None:
        ...
        for data in self._client.fetch_all(
            ...
            columns=list(self._endpoints[self._endpoints_map[url]]),
    ...
    @staticmethod
    def _get_config(dev: Attribute, rule: ConfigRule) -> Union[None, str]:
        ...
        if not config and rule.overwrite_unsupported:
            if rule.overwrite_unsupported and match.current_value:
