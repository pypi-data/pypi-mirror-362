...
# Importer la fonction d’annotation
from audreylab_summarypostqc.annotator import annotate_gwas

def main():
    parser = argparse.ArgumentParser(description="Plot QQ and/or Manhattan plots from GWAS results and optionally annotate significant SNPs.")
    ...
    parser.add_argument("--annotate", action="store_true", help="If set, annotate significant SNPs using BioMart and MyVariant")
    parser.add_argument("--pval-threshold", type=float, default=5e-2, help="P-value threshold for annotation (default: 5e-2)")

    args = parser.parse_args()

    # === Gestion des noms de fichiers de sortie ===
    if not args.qqplot and not args.manhattan and not args.out and not args.annotate:
        parser.error("You must specify --qqplot, --manhattan, --annotate, or a common --out prefix.")
    ...
    # === Annotation ===
    if args.annotate:
        output_annot = f"{args.out}_annotated.csv" if args.out else "annotated_results.csv"
        annotate_gwas(args.input, output_annot, args.pval_threshold)
