...
from .plugin_base import PluginBase, cookie_property, lazy_property
...
class RemoteAppInfo:
    ...
    need_info: bool = True
    launch_information: List[str] = field(default_factory=list)
...
class CredentialStorePlugin(PluginBase):
    ...
    *New in version: 1.7.0*
    ...
    .. py:attribute:: remote_app_account

    The :py:attr:`self.remote_app_account` attribute contains the account indentifier that should be checked out out for
    RemoteApp.

    .. py:attribute:: remote_app_asset

    The :py:attr:`self.remote_app_asset` attribute contains the asset indentifier that should be checked out out for
    RemoteApp.
    """
    ...
    def do_get_remote_app_credentials(self):
       ...
       *New in version: 1.7.0*
        ...
        The :meth:`do_get_remote_app_credentials` method should implement fetching passwords for the given
        :meth:`remote app account <CredentialStorePlugin.remote_app_account>` name and possible :meth:`remote app assets <CredentialStorePlugin.remote_app_assets>`

        :return: a dict containing a list of passwords: {'passwords': ['secret', 'topsecret']} or empty list [] if \
        no password could be fetched.
        """
        raise NotImplementedError("do_get_remote_app_credentials")
    ...
    @cookie_property
    def remote_app_asset(self):
        return None

    @cookie_property
    def remote_app_account(self):
        return None
    ...
    def get_remote_app_credentials(
        self,
        cookie,
        session_cookie,
        # base credential store arguments
        ...
        try:
            self.remote_app_account = session_cookie.get("account") or server_username
            self.remote_app_asset = session_cookie.get("asset") or asset

            if self._need_to_ask():
                return self.finalize_hook(self._add_cookies(RemoteAppInfo().as_dict()))

            result = self.do_get_remote_app_credentials()
            if result:
                remoteapp_info = RemoteAppInfo(
                    account=self.remote_app_account,
                    asset=self.remote_app_asset,
                    passwords=result.get("passwords", []),
                    launch_information=self._parse_launch_info(result),
            ...
            return self.finalize_hook(self._add_cookies(remoteapp_info))
        except NotImplementedError:
            self.logger.info("This plugin does not support fetching Remote App credentials")

        self.remote_app_account = None
        self.remote_app_asset = None

        return self._add_cookies(RemoteAppInfo().as_dict())

    @staticmethod
    def _parse_launch_info(result):
        raw_launch_info = result.get("launch_information")
        host_app = result.get("host_app")

        if raw_launch_info is None:
            return []

        result = [host_app]
        for part in [i.strip() for i in raw_launch_info.split("--") if i]:
            if part.startswith("cmd"):
                result.append("--cmd")
                result.append(part.replace("cmd ", ""))
            elif part.startswith("args"):
                result.append("--args")
                result.append(part.replace("args ", ""))
            else:
                result.append(f"--{part}")
        return result

    def _need_to_ask(self):
        return not all((self.remote_app_account, self.remote_app_asset))
    ...
    def __do_check_in_if_needed(self):
        if self.account or self.remote_app_account:
            account = self.account or self.remote_app_account
            asset = self.asset or self.remote_app_asset
            self.logger.info(
    ...
    def session_ended(self, cookie, session_cookie, session_id):
        self._setup_self(cookie, session_cookie, ConnectionInfo(session_id=session_id))
        if not self._get_private("check_in_called", False) and (self.account or self.remote_app_account):
            account = self.account or self.remote_app_account
            asset = self.asset or self.remote_app_asset
            reason = (
                "postponed"
