...
def register_experiment_commands(subparsers: argparse._SubParsersAction) -> None:
    ...
    # Stop experiment run subcommand
    stop_run_parser = subparsers.add_parser("stop", help="Stop a running experiment")

    stop_run_parser.add_argument(
        "--experiment-run-name",
        type=str,
        help="Name of the experiment run to stop",
        required=True,
    )
...
async def handle_experiment_command(args: argparse.Namespace) -> DictResponse:
    ...
    if subcommand == "run":
        ...
        return await handle_stop_experiment(args)
    else:
...
async def handle_stop_experiment(args: argparse.Namespace) -> DictResponse:
    """Handle the stop experiment command."""
    client = ExperimentClient()

    try:
        return await client.stop_experiment(
            experiment_run_name=args.experiment_run_name,
        )

    except Exception as e:
        return DictResponse(
            error=f"Failed to stop experiment: {str(e)}",
        )
