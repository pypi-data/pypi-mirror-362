...
class ExperimentClient(Initializer):
    ...
    async def stop_experiment(
        ...
        ----------
        run_name : str | None
            The name of the experiment run to stop. If not provided,
            uses the currently selected experiment run.

        Returns
        -------
        DictResponse
            A dictionary containing the response or error information.
            Returns status 204 (no content) on successful stop.
        """
        project_id = await self.project_id()
        ...
        stop_run_result = await self.client.exec(
            url=f"/projects/{project_id}/experiment_runs/{self.experiment_run_id}/stop",
            method="DELETE",
            params={
                "identifier": "id",
            },
        ...
        if is_error(stop_run_result):
            return DictResponse(
                error=stop_run_result["error"],
            )

        return DictResponse(response={"status": "Experiment stopped successfully"})
