...
def time_align(df: pd.DataFrame, col_time: str, col_anomaly: str = "time_anomaly", sampling: int = 300,
    ...
    try:
        ...
        df_resampled[col_anomaly] = 0
        df_resampled.loc[~valid_mask, col_anomaly] = 3
        df_resampled.loc[(df_resampled["time_diff"] > anomaly_interval) & valid_mask, col_anomaly] = 1

        if method == "nearest":
            normal_mask = df_resampled[col_anomaly] == 0
            ...
            for idx, count in nearest_counts.items():
                if count > 1:
                    ...
                    df_resampled.loc[drop_idx, col_anomaly] = 2
