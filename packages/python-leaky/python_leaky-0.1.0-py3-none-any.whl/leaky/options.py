...
class Options:
    ...
    included_types: set[Type[Any]] = field(default_factory=set)
    """
    The types of objects to include in the leak report. By default all types are checked,
    but this can be limited to a subset of types.
    """

    excluded_types: set[Type[Any]] = field(default_factory=set)
    """
    The types of objects to exclude from the leak report. By default no types are excluded.

    It is not possible to specify both `included_types` and `excluded_types`.
    """

    max_types_in_leak_summary: int = 200
    """
    ...
    max_object_summaries: int = 30
    """
    The maximum number of objects for which to print summaries. Defaults to `30`.

    We try to check the same number of objects for each type, within this limit.
    For example, if there are 10 types of objects, and `max_referrers` is 30, then
    ...
    referrers_module_prefixes: Set[str] | None = None
    """
    ...
    """
    A function that writes leak reports. If this is not provided leak reports are
    printed to the console.

    This option can be used to, for example, write leak reports to a log file.
