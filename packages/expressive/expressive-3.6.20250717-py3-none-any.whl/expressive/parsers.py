...
def tidy_list_str(collection, maxlen=5, stringify=None):
    ...
    if len(collection) > maxlen:  # FIXME bad output if maxlen < 2 (don't do that)
        head = ",".join(map(stringify, collection[:maxlen - 2]))
        tail = ",".join(map(stringify, collection[-2:]))
        return f"[{head} .. {tail}]"
...
def extra_functionality_parse(extend):
    """ split and transform extend into dicts
            extra_functions  {sympy.Function: callable or closure string}
            extra_symbols    {sympy.NumberSymbol: Number}

        general workflow
            allowed types for keys
            allowed types for values
            narrow over number or Function
            use the right mapper results..

        misc research

        NumberSymbol is not derived from Symbol, but both inherit from `AtomicExpr`:
            class NumberSymbol(sympy.core.expr.AtomicExpr)
            class Symbol(sympy.core.expr.AtomicExpr, sympy.logic.boolalg.Boolean)
            class Number(sympy.core.expr.AtomicExpr)
            class AtomicExpr(sympy.core.basic.Atom, Expr)
        this is especially helpful when relying on `.atoms()`
            >>> parse_expr("10 * pi * x").atoms(Symbol)
            {x}
            >>> parse_expr("10 * pi * x").atoms(NumberSymbol)
            {pi}

        Number rejects complex values (Traceback stacks removed)
            >>> Number("7+5.3j")
            ValueError: String "7+5.3j" does not denote a Number
            >>> Number(7+5.3*I)
            TypeError: expected str|int|long|float|Decimal|Number object but got 'Add'
        but sympify and the complex (builtin and more ideal?) can consume it
            >>> sympify("7+5.3j")
            7 + 5.3*I
            >>> complex("7+5.3j")
            (7+5.3j)
        Number can coerce many strings and interestingly create Rational instances, while
        Rational is defined `Rational(Number)` and has different limitations
        really both represent a relatively powerful and intentionally-incomplete parser
        for different numeric values
            >>> type(Number("6/3"))                 # even division
            <class 'sympy.core.numbers.Integer'>
            >>> type(Rational("6/3"))               # same with Rational
            <class 'sympy.core.numbers.Integer'>
            >>> type(Number("7/3"))                 # uneven division creates Rational
            <class 'sympy.core.numbers.Rational'>
            >>> type(Number("7//3"))                # parses integer division
            <class 'sympy.core.numbers.Integer'>
            >>> type(Rational("7//3"))
            TypeError: invalid input: 7//3
            >>> type(Number("2.1"))
            <class 'sympy.core.numbers.Float'>
            >>> Number("2.1")                       # coerced to float with default precision (53)
            2.10000000000000
            >>> Rational("2.1")                     # coerced to exact fraction
            21/10
        but won't coerce others
            >>> type(Number("pi"))                  # not a full parser (traceback omitted)
            ValueError: String "pi" does not denote a Number
            >>> type(sympify("pi"))                 # use sympify for a full parser (ha symPIfy)
            <class 'sympy.core.numbers.Pi'>
        while trickery and experimentation can work!
            >>> Number("S.Pi")
            ValueError: String "S.Pi" does not denote a Number
            >>> Number("S.Pi.n()")
            3.14159265358979
        example with zero too
            >>> Number("Zero")                      # (traceback omitted)
            ValueError: String "Zero" does not denote a Number
            >>> Number("S.Zero")
            0
            >>> type(Number("S.Zero"))
            <class 'sympy.core.numbers.Zero'>
            >>> type(Rational("S.Zero"))            # Rational does not behave the same!
            TypeError: invalid input: S.Zero

        BEWARE this is not intended to create an inner system hell, just accelerate some math!
               see also https://en.wikipedia.org/wiki/Greenspun%27s_tenth_rule

        consider also supporting a global collection of user functions and constants
    """
    ...
    extend_names    = {}  # {NumberSymbol: number-like}  # FUTURE {Function: Numba callable}
    extend_closures = {}  # {Function: string function} ideally this helps fix callable typing woes
    ...
    for key, value in extend.items():
        ...
        if callable(value):  # try to coerce function to its full text
            ...
            # NOTE that value will have its leading indentation as-written in the code
...
def string_expr_cleanup(expr_string):
    ...
    # allow the name mod, but transform mod->Mod when it's used as a Function
    # `Mod()` finally becomes `numpy.mod()` in the builder, which is probably what users
    # expect to happen .. this shouldn't break any existing uses of the name literally "mod"
    # as it couldn't be used as a Function yet
...
def parse_inputs(expr, symbols, name_result, extend):
