...
def test_create_dm():
    ...
    assert shared_data.dm.blockchain.blockchain_id in existing_blockchain_ids,        "Created DidManagerWithSupers."
...
def test_create_super():
    ...
    assert shared_data.super == dm.get_super(
        shared_data.super.did),        "  -> get_super()"

    assert shared_data.super.did in dm.get_active_supers() and shared_data.super.did not in dm.get_archived_supers(
    ),        "  -> get_active_supers() & get_archived_supers()"

    active_ids, archived_ids = dm._read_super_registry()
    assert shared_data.super.did in active_ids and shared_data.super.did not in archived_ids,    "  -> _read_super_registry()"
...
def test_archive_super():
    ...
    assert shared_data.super.did not in dm.get_active_supers() and shared_data.super.did in dm.get_archived_supers(
    ),        "  -> get_active_supers() & get_archived_supers()"
    active_ids, archived_ids = dm._read_super_registry()
    assert shared_data.super.did not in active_ids and shared_data.super.did in archived_ids,        "  -> _read_super_registry()"
...
def test_delete_dm():
    ...
    assert shared_data.dm.blockchain.blockchain_id not in existing_blockchain_ids,        "Deleted DidManagerWithSupers."
