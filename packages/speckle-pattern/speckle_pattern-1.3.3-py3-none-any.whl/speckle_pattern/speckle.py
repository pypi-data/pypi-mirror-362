...
def add_scale_bars(im, l, t, orientation='vertical'):
    """
    Add scale bars to `im` image, `l` pixels apart and `t` pixels thick.
    """
    ...
    if orientation == 'vertical':
        if l + 2*t > h:
            raise ValueError(f'Scale bars can not be larger that image size ({l+2*t} > {h}).')
        
        row_0 = int(h//2 - l//2) - t - 1
        row_1 = row_0 + t
        row_2 = row_1 + l + 1
        row_3 = row_2 + t

        col_0 = 0
        col_1 = bar_length_ratio*t
        col_2 = w - 1 - bar_length_ratio*t
        col_3 = w - 1

    elif orientation == 'horizontal':
        if l + 2*t > w:
            raise ValueError(f'Scale bars can not be larger that image size ({l+2*t} > {w}).')
        
        col_0 = int(w//2 - l//2) - t - 1
        col_1 = col_0 + t
        col_2 = col_1 + l + 1
        col_3 = col_2 + t

        row_0 = 0
        row_1 = bar_length_ratio*t
        row_2 = h - 1 - bar_length_ratio*t
        row_3 = h - 1
    else:
        raise TypeError('Incorrect `orientation`. Select either "vertical" or "horizontal".')

    im[row_0:row_1, col_0:col_1] = 0
    im[row_0:row_1, col_2:col_3] = 0
    im[row_2:row_3, col_0:col_1] = 0
    im[row_2:row_3, col_2:col_3] = 0
...
def generate_and_save(height, width, dpi, speckle_diameter, path, size_randomness=0.5, 
    ...
    grid_step: float
        ...
        of `speckle_diameter`.
    scale_bars: float
        spacing of scale bars, added to the generated image, in mm.
        If None, scale bars are not added. Defaults to None.
    bar_width: float
        width of scale bars, in mm. Defaults to `D`.
    ...
    if scale_bars is not None:
        l = int(np.round(scale_bars*ppmm))
        if not bar_width:
            t = D
        else:
            t = int(round(bar_width*ppmm))
        ...
        im = add_scale_bars(im, l, t, orientation)
