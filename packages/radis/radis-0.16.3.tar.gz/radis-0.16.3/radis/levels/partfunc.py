    ...
    def __init__(self, electronic_state, mode="full summation", verbose=False):
        ...
        if mode == "tabulation":
            try:
                ...
                raise ImportError(
                    r"On-the-fly tabulation of partition functions require Vaex. Install it. /!\ as of June 2024 Vaex is not available on Python>=3.11 "
                )
...
class PartFuncBarklem(RovibParFuncTabulator):
    ...
    def __init__(self, species):
        ...
        try:
            ...
            warn(
                "Note: the partition functions from Barklem & Collet (2016) don't include this species."
            )
            self.pf_values = self.pfT_values = None
        else:
            self.pfT_values = self.pfTdat.values.flatten().astype(
                float
