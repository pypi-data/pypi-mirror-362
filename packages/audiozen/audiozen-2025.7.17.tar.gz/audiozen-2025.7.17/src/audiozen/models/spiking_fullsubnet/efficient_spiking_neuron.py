...
def efficient_spiking_neuron(
    ...
    """Instantiate efficient spiking networks where each spiking neuron uses the gating mechanism to control the decay
...
class GSUCell(nn.Module):
    ...
    def forward(self, input, state):
        ...
        gates = (
            torch.mm(input, weight_ih.t()) + self.bias_ih + torch.mm(hx, weight_hh.t())
            # + self.bias_hh
        )
