...
def nn_feed_forward_ensemble(
    ...
    for k in range(n_networks):
        for n in range(ncond):
            _, output = nn_feed_forward(data[n, :], neural_net_params, weights_hidden[k], weights_out[k])
...
def nn_feed_forward(
    ...
    for j in range(1, neural_net_params["hidden_layer"] + 1):
        ...
        hidden[j] = (
            neural_net_params["maximum_activation"] / (1 + np.exp(-neural_net_params["activation_function"] * sumhid))
    ...
    for k in range(1, neural_net_params["output_layer"] + 1):
        ...
        output[k] = (
            neural_net_params["maximum_activation"] / (1 + np.exp(-neural_net_params["activation_function"] * sumout))
        ) - neural_net_params["offset_activation"]
