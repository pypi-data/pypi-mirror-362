...
from audiozen.trainer_utils import TrainerState, collect_env, seed_worker
...
class Trainer:
    def __init__(
        ...
        # Create model
        # We are able to `prepare` the model directly in the `__init__` method.
        # However, there is a bug in the `prepare` method. We must prepare model and optimizer simultaneously. (2024-03-31)
        self.model = model
    ...
    def _save_checkpoint(self, epoch, is_best_epoch, safe_serialization=False):
        """Save checkpoint.
        ...
        # Save checkpoint
        if is_best_epoch:
            self.accelerator.save_state(self.checkpoints_dir / "best", safe_serialization=safe_serialization)
        else:
            # Regular checkpoint
            ckpt_path = self.checkpoints_dir / f"epoch_{str(epoch).zfill(4)}"
            self.accelerator.save_state(ckpt_path.as_posix(), safe_serialization=safe_serialization)
