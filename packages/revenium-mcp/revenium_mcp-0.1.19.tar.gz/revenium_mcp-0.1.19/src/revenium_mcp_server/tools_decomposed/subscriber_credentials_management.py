...
class SubscriberCredentialsManagement(ToolBase):
    ...
    async def handle_action(
        ...
        try:
            ...
            logger.error(f"Error in subscriber credentials management: {e}")
            logger.error(f"Exception type: {type(e)}")
            logger.error(f"Exception details: {str(e)}")

            # Enhanced error handling for better debugging
            error_message = str(e)
            error_context = {"action": action, "tool": "subscriber credentials management"}

            # Extract more details from different error types
            if hasattr(e, "__class__") and "ReveniumAPIError" in str(type(e)):
                error_context["error_type"] = "API_ERROR"
                # Try to extract more specific error details
                if hasattr(e, "status_code"):
                    error_context["status_code"] = getattr(e, "status_code", "unknown")
                if hasattr(e, "response_data"):
                    error_context["response_data"] = getattr(e, "response_data", "no response data")
            elif hasattr(e, "error_code"):
                error_context["error_type"] = "TOOL_ERROR"
                error_context["error_code"] = getattr(e, "error_code", "unknown")
            else:
                error_context["error_type"] = "UNEXPECTED_ERROR"
                error_context["exception_type"] = type(e).__name__

            # Provide action-specific suggestions
            suggestions = [
                "Check that subscriberId and organizationId are valid hashed IDs",
                "Use list operations to find valid IDs",
                "Verify all required fields are provided with correct values",
            ]

            if action == "update":
                suggestions.extend(
                    [
                        "For UPDATE operations, only provide the fields you want to change",
                        "Verify the credential_id exists using get(credential_id='...')",
                        "Check that you have permission to update this credential",
                    ]
                )
            elif action == "create":
                suggestions.append("Try using dry_run=true to validate before creating")

            error_response = self.formatter.format_error_response(
                message=error_message, context=error_context, suggestions=suggestions
            )
    ...
    async def _create_credential(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result["business_context"] = self.business_context.get_billing_impact_explanation(
            "create", credential_data
        )
