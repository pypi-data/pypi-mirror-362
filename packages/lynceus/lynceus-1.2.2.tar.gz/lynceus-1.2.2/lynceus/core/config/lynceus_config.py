...
class LynceusConfig(MutableMapping):
    ...
    def recursive_merge(dict1: dict | MutableMapping, dict2: dict | MutableMapping, *,
        ...
        """
        # TODO: maybe special management here for 'key' key of various logger fine-tuned configuration, to merge everything ...
    ...
    def update_from_configuration_file(self, file_path: Path, *,
        ...
        """
        Merges configuration coming from configuration file whose path is specified, and returns the loaded configuration corresponding to specified filePath
         allowing caller to do any additional operations, like performing/updating fine-tuned loggers configuration.
    ...
    @staticmethod
    def format_config(config: MutableMapping | dict, *, obfuscation_value: str | None = None, sensitive_keys: list[str] | None = None) -> dict:
        ...
        def obfuscator(key, value):
            ...
            return obfuscation_value if any(sensitive_key in key for sensitive_key in sensitive_keys) else value

        def process_value(key, value):
            if isinstance(value, dict):
                return LynceusConfig.format_config(value, obfuscation_value=obfuscation_value, sensitive_keys=sensitive_keys)
