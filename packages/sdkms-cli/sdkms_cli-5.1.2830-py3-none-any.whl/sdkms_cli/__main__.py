...
def export_pfx(pfx_file_name, **kwargs):
    # imports for pfx creation
    from cryptography.hazmat.primitives.serialization import pkcs12, load_pem_private_key, BestAvailableEncryption
    from cryptography.x509 import load_pem_x509_certificate

    certificate_pem = export_object(kid=kwargs["cert_uuid"], der=False, quorum=False, no_display=True)
    private_key_pem = export_object(kid=kwargs["key_uuid"], der=False, quorum=False, no_display=True)
    ...
    if certificate.public_key().public_numbers() != private_key.public_key().public_numbers():
        raise ClientException('Given Certificate and Key do not match.')
    # if password is not given, get the input from cli
    password = getpass.getpass('Enter passphrase: ') if kwargs["pfx_password"] is None else kwargs["pfx_password"]
    ...
    # write pfx to a file
    extension = pfx_file_name[-4:]
    if extension != ".p12" and extension != ".pfx":
        pfx_file_name += ".pfx"
    with open(pfx_file_name, "wb") as pfx_file:
        pfx_file.write(pfx_data)
    print(f"PFX file has been exported: {pfx_file_name}")
...
def der2pem(response, **kwargs):
    if response.obj_type.value in ('AES', 'DES', 'DES3'):
        return to_bytes(hex_encode(response.value))
         ...
         return to_bytes(key)
    elif response.obj_type.value == 'CERTIFICATE' and "cert_tag" in kwargs and kwargs["cert_tag"]:
         begin_certificate = '-----BEGIN CERTIFICATE-----\n'
         end_certificate = '\n-----END CERTIFICATE-----'
         cert = begin_certificate + '\n'.join(textwrap.wrap(b64_encode(response.value), 64)) + end_certificate
         return to_bytes(cert)
    else:
...
def export_object(**kwargs):
    ...
    if quorum:
        ...
        if kwargs['der']:
            ...
            # This function has been used in other places where it displays the value in cli
            # In the export_pfx function, it just needs the value, no need to display.
            # Hence, no_display flag will just return the value of an object.
            if 'no_display' in kwargs and kwargs['no_display']:
                return der2pem(response, cert_tag=True)
            else:
...
""")
    ...
    # Parser to support pfx export
    parser_export_pfx = subparsers.add_parser("export-pfx", help="Export key to pfx. This will export both a Certificate and a Key into the pfx file.",
                                              formatter_class=argparse.RawDescriptionHelpFormatter,
                                              epilog=textwrap.dedent('''\
                                                  Output:
                                                    1) A PFX file creation in the given path.
                                                  Exit Status:
                                                    Zero on success and 1 on error. Error message is printed on stderr.
                                                        ''')
                                              )

    _add_version(parser_export_pfx)
    parser_export_pfx.add_argument("--out", help="pfx file path", required=True, dest="pfx_file_name")
    parser_export_pfx.add_argument("--password", help="pfx file password", required=False, dest="pfx_password")
    parser_export_pfx.add_argument("--cert-uuid", help="UUID of a Certificate", required=True, dest="cert_uuid")
    parser_export_pfx.add_argument("--key-uuid", help="UUID of a Key", required=True, dest="key_uuid")
    parser_export_pfx.set_defaults(func=export_pfx)
