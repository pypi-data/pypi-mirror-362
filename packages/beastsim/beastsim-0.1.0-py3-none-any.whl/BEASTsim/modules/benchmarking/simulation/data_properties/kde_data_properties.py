...
class KDE:
    ...
    def kde_test_with_R(self, x: ndarray, y: ndarray, print: bool = False) -> list:
        ...
        if len(x.shape) < 2:
            # If the data is univariate, convert to R's FloatVector
            real = robjects.FloatVector(x)
            sim = robjects.FloatVector(y)
        else:
            # If the data is multivariate, transpose and convert to R matrix
            x = x.T
            y = y.T
            real = numpy2ri.py2rpy(x)
            sim = numpy2ri.py2rpy(y)

        # Perform the KDE test using R's ks package
        results = ksr.kde_test(real, sim)

        if print:
            # Extract detailed statistics from the R results
            ...
            # Return the essential test statistics as a list
