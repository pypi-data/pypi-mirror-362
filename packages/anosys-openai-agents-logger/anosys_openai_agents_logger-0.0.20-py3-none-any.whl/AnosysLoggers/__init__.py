...
def span2json(span):
    ...
    extended = {
        "agent": lambda: {
            ...
            "cvs12": ", ".join(span_data.get("handoffs") or []),
            "cvs13": ", ".join(span_data.get("tools") or []),
    ...
    if type_ in extended:
        ...
        print(f"Unknown span_data type: {type_}")
...
class AnosysOpenAIAgentsLogger:
    ...
    def __init__(self, get_user_context=None):
        ...
        try:
            response = requests.get(f"https://api.anosys.ai/api/resolveapikeys?apikey={os.getenv('ANOSYS_API_KEY', 'AnoSys_mock_api_key')}")
    ...
    def on_trace_start(self, trace):
        ...
        self._log_summary(serialized_data)
