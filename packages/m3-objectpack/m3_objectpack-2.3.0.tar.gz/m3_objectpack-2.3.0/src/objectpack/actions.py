...
class BaseAction(Action):
    ...
    def get_perm_code(self, subpermission=None):
        ...
        if self.perm_code is None:
            ...
            perm_code = self.perm_code + (('/' + subpermission) if subpermission else '')
    ...
    def need_check_permission(self):
        ...
        if self.perm_code is not None:
            result = self.parent.need_check_permission and (self.perm_code in self.parent.sub_permissions)
        else:
            result = False
        return result
...
class BaseWindowAction(BaseAction):
    ...
    def set_windows_params(self):
        ...
        """
        warnings.warn('Please, replace "set_windowS_params"->"set_window_params"!', category=FutureWarning)
    ...
    def _apply_windows_params(self):
        ...
        :TODO: Выпилить, ато опечатка портит всю семантику!
        """
        warnings.warn('Please, replace "_apply_windowS_params"->"_apply_window_params"!', category=FutureWarning)
    ...
    def run(self, request, context):
        ...
        new_self.win_params = (getattr(self.__class__, 'win_params', None) or {}).copy()
        ...
        return ExtUIScriptResult(new_self.win, context=new_self.context)
...
class ObjectListWindowAction(BaseWindowAction):
    ...
    def set_window_params(self):
        ...
        params['read_only'] = getattr(self.parent, 'read_only', None) or (not self.has_perm(self.request))

        self.win_params = self.parent.get_list_window_params(params, self.request, self.context)

    def create_window(self):
        self.win = self.parent.create_list_window(
            is_select_mode=self.win_params['is_select_mode'], request=self.request, context=self.context
        )
...
class ObjectSelectWindowAction(ObjectListWindowAction):
    ...
    def set_window_params(self):
        ...
        self.win_params['column_name_on_select'] = self.parent.column_name_on_select
        self.win_params['additional_data_names'] = self.parent.additional_data_names
...
class ObjectEditWindowAction(BaseWindowAction):
    ...
    def set_window_params(self):
        ...
        read_only = getattr(self.parent, 'read_only', None) or (not self.has_perm(self.request))

        params['read_only'] = read_only
        params['title'] = self.parent.format_window_title(
            'Просмотр' if read_only else 'Добавление' if create_new else 'Редактирование'
        )

        self.win_params = self.handle(
            'set_window_params', self.parent.get_edit_window_params(params, self.request, self.context)
        )

    def create_window(self):
        assert 'create_new' in self.win_params, 'You must call "set_window_params" method of superclass!'
        self.win = self.handle(
            'create_window', self.parent.create_edit_window(self.win_params['create_new'], self.request, self.context)
        )
...
class ObjectSaveAction(BaseAction):
    ...
    def save_obj(self):
        ...
        try:
            ...
            raise ApplicationLogicException(str(err))
        except dj_exceptions.ValidationError as err:
            raise ApplicationLogicException('<br/>'.join(err.messages))
...
class ObjectRowsAction(BaseAction):
    ...
    def set_query(self):
        ...
        """
        self.query = self.handle('query', self.parent.get_rows_query(self.request, self.context))
    ...
    def apply_search(self):
        ...
        """
        self.query = self.handle('apply_search', self.parent.apply_search(self.query, self.request, self.context))
    ...
    def apply_filter(self):
        ...
        """
        self.query = self.handle('apply_filter', self.parent.apply_filter(self.query, self.request, self.context))
    ...
    def apply_sort_order(self):
        ...
        self.query = self.handle(
            'apply_sort_order', self.parent.apply_sort_order(self.query, self.request, self.context)
        )
        ...
        def parse_data_indexes(obj, col, result):
            ...
            if subcol:
                ...
                if obj:
                    ...
                    except (AttributeError, IndexError, FieldDoesNotExist):
                        fld = None
                else:
                    fld = None
                # получаем значение
    ...
    def handle_row_editing(self, request, context, data):
        ...
        """
        return self.handle('row_editing', self.parent.handle_row_editing(request, context, data))
    ...
    def run(self, request, context):
        ...
        if request_params.get('xaction') not in ['read', None]:
            ...
            if not isinstance(data, list):
                data = [data]
            success, message = self.handle_row_editing(request=request, context=context, data=data)
            ...
            result = PreJsonResult({'rows': rows, 'total': total_count})
        return result
...
class ObjectPack(BasePack, IMultiSelectablePack):
    ...
    .. note::
        ...
        Любой из элементов можно отключить вернув вместо него None. Например::
            ...
            desk.Item('Name', pack=self) if some_condition else None
    ...
    def title(self):
        ...
        """
        return str(
            self.model._meta.verbose_name_plural or self.model._meta.verbose_name or repr(self.model)
        ).capitalize()
    ...
    MSG_DOESNOTEXISTS = 'Запись не найдена в базе данных.<br/>Возможно, она была удалена. Пожалуйста, обновите таблицу!'
    ...
    def __init__(self):
        ...
        self.actions.extend(
            [self.list_window_action, self.select_window_action, self.multi_select_window_action, self.rows_action]
        )
        ...
        if self.can_delete is None:
            self.can_delete = (self.add_window or self.edit_window) and not self.read_only
        if self.can_delete:
            ...
            for c in cols:
                ...
                if sub_cols is not None:
                    ...
                    if not related_fields:
                        lookup = '__'.join(tools.get_related_fields(self.model, data_index.split('.')))
        ...
        self._filter_engine = self.filter_engine_clz(
            [(c['data_index'], c['filter']) for c in self._columns_flat if 'filter' in c]
        )
    ...
    def declare_context(self, action):
        ...
        if action in (self.edit_window_action, self.new_window_action, self.save_action):
            ...
            if self.allow_paging:
                result.update(
                    start={'type': 'int', 'default': self._DEFAULT_PAGING_START},
                    limit={'type': 'int', 'default': self._DEFAULT_PAGING_LIMIT},
                )
        return result
        ...
        if row is not None:
            try:
                ...
                try:
                    ...
                    raise Exception(
                        'Не получается получить поле {0} для DictSelectField.pack = {1}'.format(attr_name, self)
                    )
    ...
    def configure_grid(self, grid, *args, **kwargs):
        ...
        cc = self.column_constructor_fabric(
            config=columns, ignore_attrs=['searchable', 'search_fields', 'sort_fields', 'filter']
        )
    ...
    def apply_search(self, query, request, context):
        ...
        """
        return apply_search_filter(query, get_request_params(request).get('filter'), self.get_search_fields())
    ...
    def apply_sort_order(self, query, request, context):
        ...
        if sorting_key:
            ...
            sort_order = self.get_sort_order(data_index=sorting_key, reverse=reverse)
    ...
    def delete_row(self, obj_id, request, context):
        ...
        if not result:
            raise RelatedError('Не удалось удалить элемент {0}. Возможно на него есть ссылки.'.format(obj_id))
        return obj
    ...
    def get_filter_plugin(self):
        ...
        if list_columns_filter:
            for k, v in list_columns_filter.items():
                params = {'type': v.get('type', 'string'), 'data_index': k}
                f_options = v.get('options', [])
                if callable(f_options):
                    f_options = f_options()
                params['options'] = '[%s]' % ','.join(
                    (("'%s'" % item) if isinstance(item, str) else ((item is None and '[]') or ("['%s','%s']" % item)))
...
class SelectorWindowAction(BaseAction):
    ...
    def run(self, request, context):
        ...
        assert new_self.data_pack, 'Не задан ActionPack-источник данных!'

        new_context = new_self.configure_context(request, context)

        # вызов экшна показа окна выбора
        win_result = new_self.data_pack.select_window_action.run(request, context)
        win = getattr(win_result, 'data', None)
        if not win:
            return win_result

        if not isinstance(win, ui.BaseSelectWindow):
            raise ApplicationLogicException('Класс окна выбора должен быть потомком BaseSelectWindow!')
...
def multiline_text_window_result(data, success=True, title='', width=600, height=500):
    """
    ...
    return OperationResult(
        ...
        code=(
            ...
            """
            % (title, width, height, data.replace('\n', r'\n').replace(r"'", r'"'))
        ),
    )
