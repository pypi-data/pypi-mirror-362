...
def main():
    ...
    # TREE command (existing)
    tree = sub.add_parser("tree", help="print directory tree")
    tree.add_argument("path", nargs="?", default=".", help="root directory (default: .)")

    # GENERATE command
    gen = sub.add_parser("generate", help="generate documentation files")
    gen.add_argument("doc", choices=["readme", "license", "srs", "all"],
    ...
    if args.command == "tree":
        ...
        targets = [args.doc] if args.doc != "all" else ["readme","license","srs"]
        for doc in targets:
            if doc == "readme":
                content = make_readme()
