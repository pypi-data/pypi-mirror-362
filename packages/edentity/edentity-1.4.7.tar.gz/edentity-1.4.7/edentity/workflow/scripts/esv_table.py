...
def process_file(file):
    ...
    if temp_df.height > 0:
        # print(temp_df.shape)
        temp_df = temp_df.rename({temp_df.columns[0]: "ESV_ID", temp_df.columns[1]: sample_name})
    else:
...
try:
    esv_tables = [[f, os.path.basename(f).split("_ESV_table.tsv")[0]] for f in sorted(snakemake.input.sample_esv_tables) if os.path.isfile(os.path.join(f))]
    ...
    for path_f, sample_name in esv_tables:
        if sample_name in samples:
            handler.warning(f"Sample {sample_name} already exists in the ESV table. Skipping duplicate.")
            continue
        keep_sample = False
    ...
    with open(snakemake.output.esv_sequence_fasta_file, 'w') as fasta:
        with open(snakemake.output[0], 'w') as f:
            ...
            for i, (seq_id, d) in enumerate(sorted(hash_seq.items(), key=lambda x: x[1]['total'], reverse=True), start=1):
                f.write(f"ESV_{i}\t{seq_id}\t")
                f.write("\t".join([hast_sample[seq_id][sample] if sample in hast_sample[seq_id] else "0" for sample in samples]))
                f.write(f"\t{d['seq']}\n")
                fasta.write(f">{seq_id}\n{d['seq']}\n")  # write out only unique sequences

    df = pl.read_csv(snakemake.output[0], separator="\t")
    # read tsv report
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=config['cpu_cores']) as executor:
        results = list(executor.map(read_report, snakemake.input.remove_chimera_reports))
    ...
    summary_report_df = summary_report_df.with_columns(
        (pl.col("merged_reads") / pl.col("read_pairs") * 100)
        .fill_null(0)
    ...
    summary_report_df = summary_report_df.with_columns(
        (pl.col("primer_trimmed_reads") / pl.col("merged_reads") * 100)
        .fill_null(0)
    ...
    summary_report_df = summary_report_df.with_columns(
        (pl.col("passed_quality_filtering") / pl.col("primer_trimmed_reads") * 100)
        .fill_null(0)
    ...
    summary_report_df = summary_report_df.with_columns(
        (pl.col("dereplicated_reads") / pl.col("passed_quality_filtering") * 100)
        .fill_null(0)
    ...
    summary_report_df = summary_report_df.with_columns(
        (pl.col("reads_passed_denoising") / pl.col("dereplicated_reads") * 100)
        .fill_null(0)
    ...
    col_order = [
        "Sample", "read_pairs", "merged_reads", "merged_percent",
        "primer_trimmed_reads", "trimmed_percent",
        "passed_quality_filtering", "passed_filtering_percent",
        "dereplicated_reads", "dereplicated_percent",
        "reads_passed_denoising", "denoised_percent",
        "chimeric_reads", "borderline_reads", "ESVs"
    ]
    ...
    summary_report_df = summary_report_df.with_columns(
        pl.col("n_esv").cast(pl.Int64)
    )
    summary_report_df.write_csv(
        snakemake.output[1],
        separator="\t",
        float_precision=2
    )
    ...
    if config['make_json_reports'] is True:
        ...
        with open(os.path.join(report_path, f"{project_name}_table_6_metabarcoding_sample.json"), "w") as f:
    ...
    # construct table 8: ESV sequence
        ...
        with open(os.path.join(report_path, f"{project_name}_table_8_ESV_sequence.json"), "w") as f:
    ...
    # construct table 7: ESV
        ...
        esv_table_long = esv_table_long.with_columns(
            pl.col("abundance").cast(pl.Int64)
        )
        ...
        with open(os.path.join(report_path, f"{project_name}_table_7_ESV.json"), "w") as f:
...
#  construct the metabarcoding_run.json
    ...
    with open(os.path.join(report_path, f"{project_name}_table_5_metabarcoding_run.json"), "w") as metabarcoding_run:
