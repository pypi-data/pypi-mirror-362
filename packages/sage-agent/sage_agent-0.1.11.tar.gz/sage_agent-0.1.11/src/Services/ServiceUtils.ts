...
export class ServiceUtils {
        ...
        if (ServiceUtils.isToolUseMessage(currentMessage)) {
          ...
          if (isPreviousToolUse) {
            ...
            if (toolUseId === toolResultId) {
              ...
              console.log(
                'Removing unmatched user tool_result message:',
                JSON.stringify(currentMessage)
              );
            }
          }
  ...
  /**
   * Adds normalized messages to conversation history, avoiding duplicates
   * @param conversationHistory The current conversation history
   * @param normalizedNewMessages The new messages to add
   */
  static addMessagesToHistory(
    conversationHistory: any[],
    normalizedNewMessages: any[]
  ): void {
  ...
  /**
   * Creates a tool result message and adds it to conversation history
   * @param conversationHistory The current conversation history
   * @param assistantMessage The assistant message with tool use
   * @param toolResult The tool result to add
   */
