...
export class DiffApprovalDialog {
  ...
  public async showDialog(
    ...
    this.diffCells = allDiffs
      .filter(diff => !notebookPath || diff.notebookPath === notebookPath)
      .map(diff => ({
        ...diff,
        displaySummary:
   ...
   */
  private createDiffCellItem(diffCell: IPendingDiff): HTMLElement {
    ...
    runButton.innerHTML = `<svg width="15" height="16" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M3.75 2.75L11.25 8L3.75 13.25V2.75Z" fill="#22C55E" stroke="#22C55E" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>`;
    ...
    this.diffCells.forEach(diffCell => {
      const subscription = diffStateService
        ...
        .subscribe(stateChange => {
          if (stateChange) {
            this.updateCellUI(stateChange.cellId, stateChange.approved);
          }
        });
   ...
   */
  private updateCellUI(cellId: string, approved: boolean | undefined): void {
    ...
    if (cellElement) {
      ...
      // Get the current diff state to check for "run" decision
      const currentState = diffStateService.getCurrentState();
    ...
    if (buttons) {
      ...
      if (isRunDecision) {
        buttons.runButton.style.opacity = '0.6';
        buttons.approveButton.style.opacity = '0';
        buttons.rejectButton.style.opacity = '0';
      } else if (approved === true) {
        buttons.approveButton.style.opacity = '0.6';
        buttons.rejectButton.style.opacity = '0';
        buttons.runButton.style.opacity = '0';
      } else if (approved === false) {
        buttons.rejectButton.style.opacity = '0.6';
        buttons.approveButton.style.opacity = '0';
        buttons.runButton.style.opacity = '0';
      } else {
        buttons.approveButton.style.opacity = '1';
        buttons.rejectButton.style.opacity = '1';
        buttons.runButton.style.opacity = '1';
      }
    }
  }
  ...
  public async runCell(trackingId: string): Promise<void> {
    if (this.callbacks) {
      ...
      if (notebookTools) {
        ...
        diffStateService.updateDiffStateResult(trackingId, res);
      }
  ...
  private showSpinnerForCell(cellId: string, hoverButtons: HTMLElement): void {
    ...
    let spinner = hoverButtons.querySelector('.sage-ai-diff-spinner') as HTMLElement;
    if (!spinner) {
  ...
  private hideSpinnerForCell(cellId: string, hoverButtons: HTMLElement): void {
    ...
    // Hide spinner
    const spinner = hoverButtons.querySelector('.sage-ai-diff-spinner') as HTMLElement;
    if (spinner) {
  ...
  private createSpinnerElement(): HTMLElement {
    ...
    if (!document.querySelector('#sage-ai-spinner-style')) {
      const style = document.createElement('style');
      style.id = 'sage-ai-spinner-style';
      style.textContent = `
        @keyframes spin {
