...
export class StateDBCachingService {
  ...
  private static readonly NAMESPACE = 'sage-agent';
  ...
  public static async getValue<T>(key: string, defaultValue: T): Promise<T> {
    if (!StateDBCachingService.stateDB) {
      console.warn('[StateDBCachingService] State DB not initialized, using default value');
      return defaultValue;
    }
    ...
    try {
      ...
      console.warn(`[StateDBCachingService] Failed to get value '${key}':`, error);
      return defaultValue;
    }
  }
   ...
   */
  public static async setValue(key: string, value: PartialJSONValue): Promise<void> {
    if (!StateDBCachingService.stateDB) {
      console.warn('[StateDBCachingService] State DB not initialized, cannot set value');
      return;
    }
    ...
    try {
      ...
      console.error(`[StateDBCachingService] Failed to set value '${key}':`, error);
    }
  }
  ...
  public static async removeValue(key: string): Promise<void> {
    if (!StateDBCachingService.stateDB) {
      console.warn('[StateDBCachingService] State DB not initialized, cannot remove value');
      return;
    }
    ...
    try {
      ...
      console.error(`[StateDBCachingService] Failed to remove value '${key}':`, error);
    }
  }
   ...
   */
  public static async getObjectValue<T>(key: string, defaultValue: T): Promise<T> {
  ...
  public static async listKeys(): Promise<string[]> {
    ...
    try {
      const allKeys = await StateDBCachingService.stateDB.list(StateDBCachingService.NAMESPACE);
      return allKeys.ids.map(id => id.replace(`${StateDBCachingService.NAMESPACE}:`, ''));
    } catch (error) {
