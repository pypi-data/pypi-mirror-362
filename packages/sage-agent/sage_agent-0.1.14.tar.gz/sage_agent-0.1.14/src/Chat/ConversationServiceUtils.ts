...
export class ConversationServiceUtils {
  ...
  ): { preparedMessages: any[]; tools: any[] } {
    console.log(
      `[ConversationService] Processing conversation for notebook: ${context.notebookPath || 'unknown'}`
    );
  ...
  static async sendMessageWithStreaming(
    ...
    return await context.chatService.sendMessage(
      ...
      async (error, attempt) => {
        ...
        context.messageComponent.addErrorMessage(
          `API request failed: ${error.message}. Retrying in 5 seconds... (Attempt ${attempt})`
        );

        context.loadingManager.updateLoadingIndicator('Waiting to retry...');
      },
      ...
      context.notebookPath as string,
      createErrorMessage
    );
  }
  ...
  static handleToolUseStreaming(
    ...
    if (toolUse.type === 'tool_use_delta') {
      ConversationServiceUtils.handleToolUseDelta(
        context,
        toolUse,
        streamingState
      );
  ...
  /**
   * Step 3a: Handle tool use delta (streaming input)
   */
  static handleToolUseDelta(
    context: ConversationContext,
    toolUse: any,
    streamingState: StreamingState
  ): void {
    ...
    if (match && match[1]) {
      try {
        ...
        // JSON not well-formed yet, ignore and wait for next delta
      }
    }
  }
  ...
  static handleToolUseStop(
    ...
    if (toolUse.name === 'notebook-edit_plan') {
      context.toolService.notebookTools?.edit_plan({
  ...
  static async processSingleToolCall(
    ...
    const toolResult = isStreamToolCall
      ? streamingState.streamingToolCall!.toolResult
      : await ConversationServiceUtils.processToolCall(context, {
          id: content.id,
          name: toolName,
          input: toolArgs
        });
    ...
    context.chatService.addToolResult(assistantMessage, toolResult);
  ...
  static async handleCodeExecution(
    ...
    if (hasApprovalDecisions) {
      ...
      context.messageComponent.addToolResult(
        ...
        content.id,
        toolResult.content,
        { cell_id: cellId, notebook_path: context.notebookPath }
      );
        ...
        content: [
          {
            ...
            input: { cell_id: cellId, notebook_path: context.notebookPath }
          }
        ]
      };
      context.chatService.addToolResult(assistantMessage, toolResult);
  ...
  static trackActionForUndo(
    ...
    if (toolName === 'notebook-add_cell' && toolResult && toolResult.content) {
      try {
        const cellId = toolResult.content.replace(/"/g, '').trim();
        context.actionHistory.addAction(
          ActionType.ADD_CELL,
          { cellId, ...toolArgs },
    ...
    } else if (toolName === 'notebook-edit_cell' && actionBeforeExecution) {
      context.actionHistory.addAction(
        ActionType.EDIT_CELL,
        actionBeforeExecution,
        `Edited cell ${toolArgs.cell_id.substring(0, 8)}...`
      );
    } else if (toolName === 'notebook-remove_cells' && actionBeforeExecution) {
      context.actionHistory.addAction(
  ...
  static async handlePendingDiffsAfterToolCalls(
    ...
    if (
      hasToolCalls &&
      context.diffManager &&
      context.diffManager.hasPendingDiffs()
    ) {
      context.isActiveToolExecution = true;
      ...
      const llmStateDisplay =
        AppStateService.getChatContainerSafe()?.chatWidget.llmStateDisplay;
      if (llmStateDisplay) {
        llmStateDisplay.showPendingDiffs(context.notebookPath);
      }
  ...
  static async checkPendingDiffs(
    ...
    context.diffManager.startDiffApprovalProcess(
      ...
      true,
      false,
      context.notebookPath || undefined
    );
    ...
    context.messageComponent.scrollToBottom();
  ...
  static async checkExecutedCells(
    ...
    if (executedCells.length === 0) {
      ...
      context.messageComponent.addToolResult(
        ...
        contentId,
        toolResult.content,
        { cell_id: requestedCellId, notebook_path: context.notebookPath }
      );
