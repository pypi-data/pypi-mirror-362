...
export class CachingService {
  ...
  private static readonly PLUGIN_ID = 'sage-agent:plugin';
  ...
  public static async getSetting<T>(key: string, defaultValue: T): Promise<T> {
    if (!CachingService.settingsRegistry) {
      console.warn('[CachingService] Settings registry not initialized, using default value');
      return defaultValue;
    }

    try {
      const settings = await CachingService.settingsRegistry.load(CachingService.PLUGIN_ID);
   ...
   */
  public static async setSetting(key: string, value: PartialJSONValue): Promise<void> {
    if (!CachingService.settingsRegistry) {
      console.warn('[CachingService] Settings registry not initialized, cannot set setting');
      return;
    }
  ...
  public static async removeSetting(key: string): Promise<void> {
    if (!CachingService.settingsRegistry) {
      console.warn('[CachingService] Settings registry not initialized, cannot remove setting');
      return;
    }
    ...
    try {
      ...
      console.error(`[CachingService] Failed to remove setting '${key}':`, error);
    }
  }
   ...
   */
  public static async getBooleanSetting(key: string, defaultValue: boolean = false): Promise<boolean> {
   ...
   */
  public static async setBooleanSetting(key: string, value: boolean): Promise<void> {
   ...
   */
  public static async getStringSetting(key: string, defaultValue: string = ''): Promise<string> {
    return CachingService.getSetting(key, defaultValue);
  }
   ...
   */
  public static async setStringSetting(key: string, value: string): Promise<void> {
    return CachingService.setSetting(key, value);
  }
   ...
   */
  public static async getObjectSetting<T>(key: string, defaultValue: T): Promise<T> {
    return CachingService.getSetting(key, defaultValue);
  }
   ...
   */
  public static async setObjectSetting<T>(key: string, value: T): Promise<void> {
