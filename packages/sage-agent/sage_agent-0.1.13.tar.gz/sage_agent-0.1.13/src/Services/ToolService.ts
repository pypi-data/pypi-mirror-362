...
export class ToolService {
  ...
  // Add current active notebook path
  ...
  public updateNotebookPath(oldPath: string, newPath: string): void {
    const currentContext = this.notebookContexts.get(oldPath) || {
    ...
    this.notebookContexts.set(newPath, currentContext);
  ...
  /**
   * Set the current active notebook path context
   * @param notebookPath Path to the notebook being used for context
   */
  setCurrentNotebookPath(notebookPath: string | null): void {
    if (notebookPath === this.currentNotebookPath) {
      return; // No change needed
    }

    console.log(`[ToolService] Setting current notebook path: ${notebookPath}`);
    this.currentNotebookPath = notebookPath;

    // If we have context for this notebook, restore it
    if (notebookPath && this.notebookContexts.has(notebookPath)) {
      ...
      console.log(
        `[ToolService] Restored context for notebook: ${notebookPath}`,
        { kernel: this.kernelInfo, notebook: this.notebookInfo }
      );
    } else if (notebookPath) {
      // If this is a new notebook path, initialize with current notebook tracker
      this.handleNotebookChanged();
      // Store the new context
      this.storeNotebookContext(notebookPath);
    }
  }

  /**
   * Get the current notebook path
   * @returns The current notebook path or null if none
   */
  ...
  private handleNotebookChanged(): void {
    ...
    if (kernelConnection) {
      ...
      console.log(
        'Updated kernel info from notebook tracker:',
        this.kernelInfo
      );
    }

    console.log(
      'Updated notebook info from notebook tracker:',
      this.notebookInfo
    );

    // If we have a current notebook path, store this updated context
    if (this.currentNotebookPath) {
      this.storeNotebookContext(this.currentNotebookPath);
    }
  }
  ...
  /**
   * Check if a tool can be handled locally by NotebookTools or FilesystemTools
  ...
  /**
   * Execute a tool locally using NotebookTools or FilesystemTools
  ...
  async executeTool(
    ...
    // Before refreshing info, capture the notebook path from the tool arguments
    // or use the current notebook path
    const notebookPath =
      toolCall.input?.notebook_path || this.currentNotebookPath;

    // If the tool specifies a notebook path different from current, switch context
    if (notebookPath && notebookPath !== this.currentNotebookPath) {
      this.setCurrentNotebookPath(notebookPath);
    }
    ...
    if (this.canHandleToolLocally(toolName)) {
      try {
        ...
        if (includeContextInfo) {
          ...
          // Use the provided notebook path or current notebook path
          const contextNotebookPath =
            toolArguments.notebook_path || this.currentNotebookPath;
          if (contextNotebookPath) {
            toolArguments.notebook_path = contextNotebookPath;
          }
