...
export class AnthropicStreamHandler {
  ...
  /**
   * Creates a message stream with the Anthropic API
   */
  static async createStream(params: StreamCreationParams): Promise<any> {
    const {
      client,
      modelName,
      messages,
    ...
    const systemMessages: Anthropic.Beta.Messages.BetaTextBlockParam[] = [
      {
        text: systemPrompt,
        type: 'text',
        cache_control: {
  ...
  static async processStream(
    ...
    try {
      ...
      this.updateConversationHistory(finalMessage, conversationHistory);
  ...
  private static handleToolUseStart(
    ...
    config.errorLogger?.({
      message: 'Tool call being generated',
      toolUseInfo: contentBlock
    });
   ...
   */
  private static flushDeltaBuffer(context: any, config: StreamHandlerConfig) {
    for (let i = context.messageContent.length - 1; i >= 0; i--) {
      if (context.messageContent[i].type === 'tool_use') {
        config.onToolUse!({
          type: 'tool_use_delta',
  ...
  /**
   * Updates conversation history with the final message
   */
  private static updateConversationHistory(
    finalMessage: any,
    conversationHistory: any[]
  ) {
    if (
      finalMessage?.content?.[0] &&
      (finalMessage.content[0] as any).text !== undefined
    ) {
      const assistantMessage = {
        role: 'assistant',
        content: (finalMessage.content[0] as any).text
      };
   ...
   */
  private static createCancelledResult(): StreamResult {
    return {
      cancelled: true,
      role: 'assistant',
      content: []
