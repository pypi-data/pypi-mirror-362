...
export class CodeConfirmationDialog {
  ...
  constructor(
    ...
    onActivateAutoRunMode: () => void
  ) {
    this.chatHistory = chatHistory;
    this.messageComponent = messageComponent;
    this.onActivateAutoRunMode = onActivateAutoRunMode;
  }
  ...
  private executeApproval(): void {
    ...
    if (
      this.currentAutoRunSelect &&
      this.currentAutoRunSelect.value === 'dont-ask'
    ) {
      this.onActivateAutoRunMode();
    }
  ...
  private cleanup(): void {
    ...
    this.currentAutoRunSelect = null;
  ...
  public async showConfirmation(
    ...
    return new Promise<boolean>(resolve => {
      ...
      heading.innerHTML = `${codeIcon} Sage is trying to run ${cellId || 'cell'}`;
      heading.className = 'sage-ai-code-confirmation-heading';
      this.confirmationContainer.appendChild(heading);

      // Auto run mode checkbox
      const autoRunModeCheckboxContainer = document.createElement('div');
      autoRunModeCheckboxContainer.className =
        'sage-ai-auto-run-mode-checkbox-container';

      // Create the select element
      const autoRunModeSelect = document.createElement('select');
      autoRunModeSelect.style.border = '0';
      autoRunModeSelect.style.backgroundColor = 'transparent';
      autoRunModeSelect.style.fontSize = 'var(--jp-ui-font-size1)';
      autoRunModeSelect.style.color = 'var(--jp-ui-font-color2)';
      autoRunModeSelect.id = 'sage-ai-auto-run-mode-select';
      autoRunModeSelect.className = 'sage-ai-auto-run-mode-checkbox';

      // Store reference to the select element for external control
      this.currentAutoRunSelect = autoRunModeSelect;

      // Create the options
      const askEveryTimeOption = document.createElement('option');
      askEveryTimeOption.value = 'ask';
      askEveryTimeOption.textContent = 'Ask every time';

      const dontAskOption = document.createElement('option');
      dontAskOption.value = 'dont-ask';
      dontAskOption.textContent = "Don't ask every time";

      // Set default selection
      autoRunModeSelect.value = 'ask';

      // Add options to select
      autoRunModeSelect.appendChild(askEveryTimeOption);
      autoRunModeSelect.appendChild(dontAskOption);

      autoRunModeCheckboxContainer.appendChild(autoRunModeSelect);
      ...
      let modifierIcon = isMac ? cmdIcon : ctrlIcon;
      ...
      bottomContainer.appendChild(autoRunModeCheckboxContainer);
