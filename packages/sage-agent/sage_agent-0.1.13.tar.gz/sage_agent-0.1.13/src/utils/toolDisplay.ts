...
> = {
  'codebase-list_repos': 'Sage is listing repos...',
  'notebook-wait_user_reply': '',
  'notebook-edit_plan': 'Sage is editing the plan...',
  'notebook-add_cell': 'Sage is adding a new cell...',
  'notebook-edit_cell': 'Sage is editing a cell...',
  'notebook-get_cell_info': 'Sage is getting cell info...',
  'notebook-remove_cells': 'Sage is removing cell...',
  'notebook-run_cell': 'Sage is running a cell...',
  'notebook-read_cells': 'Sage is reading cell...',
  'filesystem-delete_dataset': 'Sage is deleting dataset...',
  'filesystem-list_datasets': 'Sage is listing datasets...',
  'filesystem-save_dataset': ctx => {
    if (!ctx.filepath) {
      return 'Sage is saving dataset...';
    }
    return `Sage is saving dataset ${codeWrapper(ctx.filepath, true)}...`;
  },
  'filesystem-read_dataset': ctx => {
    if (!ctx.filepath) {
      return 'Sage is reading dataset...';
    }
    return `Sage is reading dataset ${codeWrapper(ctx.filepath, true)}...`;
  },
  'web-download_dataset': ctx => {
    const hasTickers = ctx.tickers && ctx.tickers?.length;
    if (hasTickers && ctx.period && ctx.interval) {
      return `Sage is downloading ${ctx.tickers.map(codeWrapper).join(', ')} dataset in the web for the period of ${codeWrapper(ctx.period)} and interval of ${codeWrapper(ctx.interval)}...`;
    }

    if (hasTickers && ctx.period) {
      return `Sage is downloading ${ctx.tickers.map(codeWrapper).join(', ')} dataset in the web for the period of ${codeWrapper(ctx.period)}...`;
    }

    if (hasTickers && ctx.interval) {
      return `Sage is downloading ${ctx.tickers.map(codeWrapper).join(', ')} dataset in the web for the interval of ${codeWrapper(ctx.interval)}...`;
    }

    if (hasTickers) {
      return `Sage is downloading ${ctx.tickers.map(codeWrapper).join(', ')} dataset in the web...`;
    }

    return `Sage is downloading dataset in the web...`;
  },
  'web-search_dataset': ctx => {
    if (!ctx.queries || ctx.queries.length === 0) {
      return 'Sage is searching for dataset in the web...';
    }
    return `Sage is searching for ${ctx.queries.map(codeWrapper).join(', ')} dataset in the web...`;
  }
};
