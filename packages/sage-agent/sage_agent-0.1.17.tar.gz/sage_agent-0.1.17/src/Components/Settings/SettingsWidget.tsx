...
export class SettingsWidget extends ReactWidget {
  ...
  private async loadTokenModeSetting(): Promise<void> {
    const cached = await CachingService.getBooleanSetting(SETTING_KEYS.SAGE_TOKEN_MODE, false);
   ...
   */
  private async loadSetting(key: string, defaultValue: string): Promise<string> {
    ...
    if (!claudeApiKey || claudeApiKey.trim() === '') {
      try {
        ...
        if (optionalEnv.api_key) {
          ...
          console.log('[SettingsWidget] Caching API key from optional_env.json to settings registry');
      ...
      console.log('[SettingsWidget] API key already exists in settings registry, not loading from optional_env.json');
    }
