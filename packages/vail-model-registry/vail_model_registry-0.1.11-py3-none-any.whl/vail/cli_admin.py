...
def build_source_info(
    checkpoint: str, source_type: str, requires_auth: bool = False
) -> dict:
    """
    ...
    if source_type == "safetensor":
        ...
        # For gguf, we would typically need a file path or URL
        # For now, we'll assume it's available via HuggingFace
        # TODO: Update based on finalized GGUF fingerprint implementation
        source_info = {
            "source_type": "gguf_file",
            "source_identifier": json.dumps(
                {"url": f"https://huggingface.co/{checkpoint}"}
            ),
            "requires_auth": requires_auth,
        }
    else:
...
def handle_add_model(parsed_args):
    ...
    try:
        ...
        source_info = build_source_info(
            parsed_args.checkpoint, parsed_args.source_type, requires_auth
        )
    except Exception as e:
