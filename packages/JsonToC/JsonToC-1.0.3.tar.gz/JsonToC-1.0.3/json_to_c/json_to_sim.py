...
class JsonToC():
    ...
    def create_group_struct(self, i, group_struct_name):
        ...
        for j in self.group_map[i]:
            ...
            if 'group_reg' in elem:
                ...
                c_data += self.create_c_struct(j, name, addr)
        c_data +=  '};\n\n'
        return c_data
            ...
            if i.endswith('group_reg'):
                ...
                if pre_dpath != '':
                    ...
                    if addr_diff > 0:
                        ...
                        reseve_count += 1
                #c_data += '    unsigned r' + name.upper() + '; //' + addr + '\n'
                c_data += self.create_c_struct(i, name, addr)
            pre_dpath = i
        c_data +=  '};\n\n'
        c_data_list.append(c_data)
        return c_data_list

    def create_c_struct(self, js_path, name, addr):
        c_data = '    strcut {\n'

        fields = self.get_delem(self.js_dict, js_path, key = 'Fields' )
        ...
        for field in fields:
            fname = field['name']
            if fname == 'reserved':
                fname = 'reserved' + str(reserve_count)
                reserve_count += 1
            frange = field['range']
            ...
            c_data += '        unsigned int ' + fname + ' :' + str(end - start + 1) + ';\n'
            c_data += '\n'
        c_data = c_data[:-1]
        c_data += '    } ' + name.upper() + '; //' + addr + '\n\n'
        return c_data
    ...
    def replace_reg_with_sim(self, text):
        # 定义一个替换函数，根据匹配的内容返回相应的替换值
        ...
        # 使用正则表达式匹配 'reg' 或 'REG'，并调用替换函数
        pattern = r'\breg\b|(?<=_)reg(?=_)|(?<=_)reg\b|\breg(?=_)'
    ...
    def main(self):
        ...
        output_name = self.replace_reg_with_sim(self.file_name)
        with open(output_name + '.h' , 'w') as fd:
            for data_block in c_data_list:
                #fd.write(data_block)
                fd.write(self.replace_reg_with_sim(data_block))
