...
class QatarIDInfo(BaseModel):
    ...
    nationality: str = Field(..., description="Nationality in ISO 3166-1 alpha-3 format (e.g., 'PAK' 'QAT', 'SYR', 'PHL')", example="SYR")
    ...
    occupation_en: str = Field(..., description="Occupation in English")



def get_qatar_id_info_llm(ocr_text):
    function_spec = convert_pydantic_to_openai_function2(QatarIDInfo)
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Extract the relevant info from this OCR text: {ocr_text}"
            }
        ],
    ...
    # Step 4: Handle both possible cases
    message = response["choices"][0]["message"]

    if "function_call" in message:
        # ✅ Case 1: Model called the function
        args_json = message["function_call"]["arguments"]
        args = json.loads(args_json)
        try:
            return QatarIDInfo(**args)

        except Exception as e:
            print("❌ Invalid function arguments:", e)
            return {'error':'covered_photo', 'error_details':'Could not find name in given document'}
    else:
        # ❌ Case 2: Model just responded normally
        print("⚠️ Model did not call the function. Content:")
...
def qatar_front_id_extraction(client, image_data, front_id_text, front_id_text_description):
    ...
    try:
        result = get_qatar_id_info_llm(front_id_text_description)
        ...
        if isinstance(result,dict):
            ...
            'occupation_en' : result.occupation_en
            }
