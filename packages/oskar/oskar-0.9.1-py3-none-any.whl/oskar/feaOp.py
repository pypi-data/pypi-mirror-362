#!/usr/bin/env python
# -*- coding: utf-8 -*-
r"""Expression operators for finite element approximations.
"""
import numpy as np
import pygimli as pg

from . op import OP, Constant, findInstance, hasInstance, hasOP
from . elementMats import dotE, createEMap, trE, createE, symE
...
class FEAOP(OP):
    ...
    def __getitem__(self, idx):
        """Get item from FEAOP."""
        pg.warning('FEAOP.__getitem__ in use?')
    ...
    def findMesh(self, op=None):
        ...
        if hasattr(op.a, 'mesh'):
            if op.a.mesh is not None and op.a.mesh.nodeCount() > 0:
                return op.a.mesh

        if hasattr(op.b, 'mesh'):
            if op.b.mesh is not None and op.b.mesh.nodeCount() > 0:
                return op.b.mesh
    ...
    def eval(self, *args, **kwargs):
        ...
        from . elasticity import ConstitutiveMatrix, asVoigtMapping
        ...
        with pg.tictoc('FEAop.eval'):
            ...
            if len(spaces) != 1:
                if self.op == '*':
                    if isinstance(p, pg.core.Boundary):
                        if self.b.op == 'norm':
                            pg._y()
            ...
            def _evalForCell(c, u, p=None, op=None, **kwargs):
                """
                    TODO:
                        * c can be cellSet if eval p is on a node, currently
                        only the mean of op[c] is chooses, maybe harmonic
                        or weighted mean can me useful .. test and check!
                """
                ...
                if isinstance(op, ConstitutiveMatrix):
                    return op
                ...
                if op.b is None:
                    ...
                    if isinstance(a, ConstitutiveMatrix):
                        return a @ asVoigtMapping(b)
            ...
            def _evalBoundary(b, u):
                """Evaluate for boundary."""
                pass
            ...
            if isinstance(p, pg.core.MeshEntity):
                with pg.tictoc('eval.forcell [ent]'):
                    return _evalForCell(p, uA, **kwargs)

            if p is None:
                p = self.mesh.cells()
                with pg.tictoc('eval.forcell [c, ]'):
    ...
    def findElementMatAndScale(self, a, vLevel=0, **kwargs):
        ...
        if isinstance(a, list) and not pg.isPos(a) and not pg.isPosList(a):
            ## [f, a, b] .. decide the mult itself after know what to need
            #check_in_use
            if pg.core.deepDebug() == -1:
                pg._g(f'{" "*8*vLevel}->ret({a}): {None, a, 1}')
            return None, a, 1
        ...
        from . feaSpace import FEASpace
        from . feaSolution import FEASolution, SolutionGrad, SolutionDiv

        if isinstance(a, FEASpace):
            if pg.core.deepDebug() == -1:
                pg._g(f'{" "*8*vLevel}->ret ({a}): {a.uMat(order=order), None, 1}')
        ...
        if isinstance(a, FEAOP):
            if a.op == 'grad' and isinstance(a.a, FEASpace):
                ...
                if a._solutionGrad is True:
                    if pg.core.deepDebug() == -1:
                        pg._g(f'{" "*8*vLevel}->ret ({a}): {None, "SolutionGrad", 1}')
                ...
                if a._solutionGrad is True:
                    if pg.core.deepDebug() == -1:
                        pg._g(f'{" "*8*vLevel}->ret ({a}): {None, "SolutionDiv", 1}')
            ...
            if a.op == 'neg':
                ...
                with pg.tictoc(f'create sym'):
                    pg.core.sym(aa, aas)
                aa = aas

            elif a.op == 'tr':
                aas = createEMap(name=f'tr({aa})', space=aa.space)
                with pg.tictoc(f'create tr'):
    ...
    def assembleLinearForm(self, RHS, onBoundaries=False,
        ...
        with tictoc('assemble linear form'):
            ...
            ### special case for dirac impules on Linear form
            ...
            from . elasticity import ElasticityTensor
            ...
            if isinstance(self.a, ConstantSpace):
                ...
                with tictoc('integrate'):
                    ...
                    if isinstance(f, (pg.core.stdVectorMatrixVector, pg.core.stdVectorRDenseMatrixVector)) and \
                        len(f) == a.size():
                        ### f is matrix per quadrature -- Refactor!

                        ### Check and fix first probably from wrong: s * grad(v)
                        if a.mats()[0].nCoeff() == 1 and a.mats()[0].cols() == 1:
                    ...
                    elif isinstance(f, ElasticityTensor) \
                        ...
                        elif isinstance(f[0], ElasticityTensor) \
                            or pg.isMatrix(f[0]):
    ...
    def assembleBilinearForm(self, LHS=None, onBoundaries=False,
        ...
        with tictoc('assemble bilinear form'):
            ...
            with tictoc('create maps'):
                a, fa, siga = self.findElementMatAndScale(feaop.a, **kwargs)
                b, fb, sigb = self.findElementMatAndScale(feaop.b, **kwargs)
            ...
            if isinstance(a, OP):
                ...
                with pg.tictoc('preeval a'):
                    a = a.eval(dim=b.pMat(0).cols())

            if isinstance(b, OP):
                # for stuff like c*(a+b)
                with pg.tictoc('preeval b'):
            ...
            if useMul is True:
                ...
                def _eMapImult(a, fa, fa2):
                    if not pg.isScalar(fa2, 1.0):
                        ...
                        if isinstance(fa, (np.ndarray, list)) and pg.isMatrix(fa[0]):
                ...
                af = _eMapImult(a, fa, fa2)
                bf = _eMapImult(b, fb, fb2)

                if pg.core.deepDebug() == -1:
                    pg.info('bilinear dot (useMul): '
                            f'{self} : {fa} : {fb} : {a} : {b}')
                ...
                with tictoc('integrate .2'):
                    ...
                    if pg.isPosList(f) and len(f) > 3:
                        ...
                        if pg.isScalar(f) and 1: # allways cache, TODO: check
                            ###
    ...
    def assembleWithMats(self, onBoundaries=False, LHS=None, RHS=None,
        ...
        with pg.tictoc(key='assemble.mats'):
            ...
            li, bi = self.findForms(cache=True)
    ...
    def assemble(self, onBoundaries=False, core=False, useMats=False, **kwargs):
        """Assemble FEAOP into a lineare system of equations.
        """
        #### implement and clean
    ...
    def apply(self, entID, op=None, entity='cell', debug=False, forEval=False,
        ...
        * ..
        ...
        if 0 and (debug or pg.core.deepDebug() == -1):
            pg._g('*'*60)
            pg._g(f'** apply() : {op}')
        ...
        if any([pg.isScalar(op.a),
                ...
                hasattr(op.a, '__iter__') and len(op.a) == self.mesh.cellCount(),
        ...
        if op.a is None:
            ...
            a = op.a
        elif isinstance(op.a, ConstantSpace) or \
            isinstance(op.a, FEASolution) or \
            isinstance(op.a, FEASolutionOP) or \
            isinstance(op.a, FEAFunction):
            #pg.critical('in use?')
        ...
        if op.b is None:
            ...
            return _applyParam(a, op.op,
                               op.b[getattr(self.mesh, entity)(entID).marker()])
        elif isinstance(op.b, ElasticityTensor) or pg.isMatrix(op.b):
            if pg.core.deepDebug() == -1:
                pg._b('op.b = C', op.b)
            a.mulR = op.b
            return a
            ...
            if len(op.b) == self.mesh.cellCount() \
                and (isinstance(op.b[0], ElasticityTensor) \
...
class Dirac(FEAOP):
    ...
    """
    def __init__(self, rs=None, t0=None, space=None, **kwargs):
        """
        """
    ...
    def __str__(self):
        """String representation of Dirac operator."""
...
def findForms(op, sign='+', parentOP=None,
              scale=None, vLevel=0):
    ...
    * Bilinear form is: `[sign, space-function, scale-function]`
    ...
    with pg.tictoc(key='find forms'):
        ...
        def _findForms(op, li, bi, sign='+', scale=None, vLevel=0):
            """Helper function to find forms.
            """
            #op.dump()
            def _countAssemblings(a):
                """Count number of assemblings in the operator."""
                N = 0
                if hasattr(a, 'op') and a.op == 'identity':
                    return 0

                if hasattr(a, 'a'):
                    N += _countAssemblings(a.a)
                if hasattr(a, 'b'):
                    N += _countAssemblings(a.b)
            ...
            if pg.core.deepDebug() == -1:
                ...
                pg._g(f'# {" "*8*vLevel} Find forms for op: {op}, '
                        ...
                        f'asm_count {_countAssemblings(op)}')
            ...
            if pg.isScalar(op, 0.0):
            ...
            elif op.op == None and _needsAssembling(op):
                ## single u
                ...
                if _needsAssembling(op.a) is True and \
                    ...
                    _needsAssembling(op.a) is True and not _needsAssembling(op.b):
                    # LF: u*Scale * B or Scale*u * B
                    _findForms(op.a, li, bi, sign, scale=_addScale(scale, op.b),
                                vLevel=vLevel+1)


                elif isinstance(op.a, OP) and _needsAssembling(op.a) is True and \
                (pg.isScalar(op.b) or op.b is None or not _needsAssembling(op.b)):
                    ## (u*u)|u * 1.0
                    # pg._y(op.a)
                    # pg._y(scale)
                    _findForms(op.a, li, bi, sign, scale=_addScale(scale, op.b),
                    ...
                    # li.append([sign, op])
                elif isinstance(op.b, OP) and _needsAssembling(op.b) is True and \
                (pg.isScalar(op.a) or op.a is None or not _needsAssembling(op.a)):
                    ## 1.0 * (u*u)|u
                    # pg._r(op.b)
                    # pg._r(f'scale: {scale}')
        ...
        # li2 = []
        # bi2 = []
        # _findForms(op, li2, bi2) # no costs

        # if pg.core.deepDebug() == -1:
        #     print('bilinear forms:', bi2)
        #     print('linear forms:', li2)
        ...
        ## Linear for should look like [sign, f(FEASpace)*scale, extra scalar scale]
        ## move wrong scales to the right side, and the rescale the FEASpace
        if 1:
            for i, l in enumerate(li):
                #l[1].dump()
                if not hasFEASpace(l[1]):
                    ## TODO should this happen? checkme
                    continue
                ...
                if hasOP(l[1], 'identity') or hasOP(l[2], 'identity'):
                    continue
                ...
                if 1:
                    def _tokenize(e):
                        ...
                        if pg.isScalar(e) or isinstance(e, (pg.core.RMatrix, ParameterDict)):
                            #pg._r(e)
                            return None, None, e
                        ...
                        elif isinstance(e, (OP, list, np.ndarray,
                                            pg.Vector, pg.Pos)):
                            #pg._y(e)
        ...
        # remove NULL linear forms if they are ar more than one li
        if len(li) > 1:
