...
from oskar.feaOp import findForms, FEAOP
...
from oskar.elasticity import createElasticityTensor, asNoMapping, asVoigtMapping
...
class TestFiniteElementBasics(TestCollection):
    ...
    def test_ElementMatrices(self):
        """Test element matrices and compare to the different implementation
        from pygimli.
        """
        def _testU(c, orders):
        ...
        def _testdUdU(c, orders):
            ...
            for order in orders:
                ...
                C = createElasticityTensor(E=1, nu=0.1, dim=c.dim(),
                                           voigtNotation=True)
    ...
    def testTracedAssemblingLF(self):
        ...
        def _test(dim, full_tensor_but_2d=False, elastic=True):
            pg.info(f'testTracedAssemblingLF: dim={dim}, '
                    f'full_tensor_but_2d={full_tensor_but_2d}, '
                    f'elastic={elastic}')
            ...
            C = createElasticityTensor(lam=lam, mu=mu, dim=mesh.dim())

            full_tensor_but_2d = False

            if full_tensor_but_2d is True:
                ## full tensor but 2d mesh
                C3 = createElasticityTensor(lam=lam, mu=mu, dim=3)
                lmu = 3*lam + 2*mu
                C = C3
            ...
            for c in s.mesh.cells():
                ...
                if s.elastic is True:
                    if C.shape == (6, 6) and c.dim() == 2:
                        # 3D elasticity tensor for 2D problem
                        CDim = 3
        ...
        _test(dim=2, full_tensor_but_2d=False)
        _test(dim=2, full_tensor_but_2d=True)
        _test(dim=3, full_tensor_but_2d=True)
        _test(dim=2, full_tensor_but_2d=False, elastic=False)
        _test(dim=2, full_tensor_but_2d=True, elastic=False)
        _test(dim=3, full_tensor_but_2d=True, elastic=False)
...
class TestFEAOperators(TestCollection):
    ...
    def test_FEA_Dirac(self):
        r"""Test :math:`\int s* dirac()*f dx`

        Linear form integration with special dirac operator.
        """
        ms = createTestMeshs()
...
class TestFEAExpressions(TestCollection):
    ...
    def test_FEA_Advection_Expressions(self):
        ...
        for mesh in createTestMeshs()[2:]:
            ...
            for vel in [vel0,
                ...
                self.assertEqual((vel0*grad(u)).apply(0), (vel*grad(u)).apply(0))
    ...
    def test_FEA_Elastic_Expressions(self):
        ...
        # L = grad(v)*sigmaE(E0)
        # L = grad(v)*(lam*tr(E0) + 2.0*mu*E0)
        # L = grad(v)*pg.core.RMatrix(E0)
        # L = lam*tr(eps(v))*I(v)

        # # # L = grad(v)*(lam*tr(eps(v))*I + 2.0*mu*eps(v)-alpha*(3*lam + 2*mu)*T)
        # # # L = v*div(sigma(vF))
    ...
    def test_FEA_Elastic_Expressions_Mapping(self):
        """Test for Vector space expression for elastic properties with mapping.
        """
        def eps(v):
            return sym(grad(v))
        ...
        C0 = createElasticityTensor(lam=lam, mu=mu, dim=mesh.dim(),
                                        voigtNotation=True)
        C03 = createElasticityTensor(lam=lam, mu=mu, dim=3,
                                        voigtNotation=True)
        ...
        #L = lambda v: grad(v) * I(v)
        # lambda v: grad(v) * alpha*T*I(v),
        # lambda v: grad(v) * (alpha*T*I(v)),
        # lambda v: grad(v) * (alpha*T)*I(v),
        # RT = L(v)
        # pg._g(RT.assemble(core=False))

        # R = L(vE).assemble(core=False)
        # pg._g(R); print(R==RT)
        # # R = L(vE).assemble(core=True)
        # # pg._y(R); print(R==RT)
        # R = L(vE).assemble(useMats=True)
        # pg._r(R); print(R==RT)
        ...
        LR = grad(v) * (2*mu + 2*lam)* alpha*T*I(v)
        # LR = grad(v) * sigmaT(v, 1, (2*mu + 3*lam)* alpha)
        # LR = grad(v) * sigma(v)
        RT = _testExp(LR)
        pg._g(RT)
        ...
        # L = grad(vE) * (C03*eps(vE) - C03*alpha*I(vE))
        # L = grad(vE) * C03*eps(vE)
        ...
        # ## TODO!!
        # def sigmaEff2(v, T):
        #     return sigma2(v) - (alpha*(3*lam + 2*mu)*T*I(v))

        # def sigmaEff1(v, T):
        #     return C*eps(v) - C*alpha*T*I(v) # wannehave
        #     return C*(eps(v) - alpha*T*I(v)) # wannehave

        # TODO
        # L = grad(v2)*sigmaEff2(v2, T)
        # R = L.assemble()
        # pg._g(R)
        # L = grad(v1) * (C*eps(v1) - C*alpha*T*I(v1)) # wannehave
        # # L = grad(v1) * (C*(eps(v1) - alpha*T*I(v1))) # wannehave
        # # L = grad(v1) * C*(eps(v1) - alpha*T*I(v1)) # wannehave
        # R = L.assemble()
        # pg._y(R)

        #LR = grad(v2)*(alpha*(2*lam + 2*mu)*I(v2)) # 2D elasticity tensor
        #LR = grad(v2)*((2*lam + 2*mu)*I(v2))  # 2D elasticity tensor
        #LR = grad(v2)*((3*lam + 2*mu)*I(v2))  # 3D elasticity tensor
        #LR = grad(v2)*(alpha*(3*lam + 2*mu)*T*I(v2)) # final wannehave /perquad
        # R = LR.assemble()
        # _testExp(LR)
        # pg._g(R)

        # L = grad(v1) * (C*alpha*I(v1)) # OKAY // const
        # L = grad(v1) * (C*alpha*I(v1)) # wannehave // per cell
        # L = grad(v1) * (C*alpha*T*I(v1)) # wannehave // per quad
        #L = grad(v1) * (C*I(v1)) # wannehave
        #L = grad(v1) * C*alpha*I(v1) # wannehave
        #L = grad(v1) * ((C*alpha)*I(v1)) # wannehave
        #L = grad(v1) * (C*alpha*T*I(v1)) # wannehave
        # L = grad(v1) * (C*(eps(v1) - alpha*T*I(v1))) # wannehave
        # L = grad(v1) * C*(eps(v1) - alpha*T*I(v1)) # wannehave
        ...
        # _testExp(L, LR)
        ##
        # L = grad(v2)*(2.0*mu*eps(v2) + (lam*tr(eps(v2)) - alpha * T) * I(v2)) #fails
        # LF, BF = findForms(L)

        # L = grad(v2)*(-0.3*T*I(v2))
        # L.dump()
        # L = LF[0][1]
        # L.dump()
        # print(LF)
        # RR = L.assemble(core=False)
        # pg._g(RR)
        # RR = L.assemble(core=True)
        # pg._y(RR)
        # RR = L.assemble(useMats=True)
        # pg._r(RR)
        ...
        for i, mesh in enumerate(createTestMeshs()[2:]):
            ...
            C0 = createElasticityTensor(lam=lam, mu=mu, dim=v.mesh.dim(),
                                         voigtNotation=True)
            C03 = createElasticityTensor(lam=lam, mu=mu, dim=3,
                                         voigtNotation=True)
            ...
            try:
                ...
                ## test more complex expressions with elasticity tensor
                for C in [C0,
                          cC
                          ]:
                    LR = grad(v) * sigma(v)
                    ...
                    _testExp(LR)
                    _testExp(grad(vE) * (C*eps(vE) - C*alpha*I(vE)), LR)
                    ...
                    ## 2D but 3D elasticity tensor
                    if mesh.dim() == 2:
                        LR = grad(v) * (alpha*(2*mu + 3*lam))*I(v)
                        _testExp(LR)
                        _testExp(grad(vE) * (C03*alpha*I(vE)), LR)

                        LR = grad(v) * sigmaT(v, 1, (2*mu + 3*lam)* alpha)
                        _testExp(LR)
                        _testExp(grad(vE) * (C03*eps(vE) - C03*alpha*I(vE)), LR)
                        #_testExp(grad(vE) * C03*(eps(vE) - alpha*I(vE)), LR) ## FIXME

                        LR = grad(v) * sigmaT(v, T, (2*mu + 3*lam)* alpha)
                        _testExp(LR)
                        #_testExp(grad(vE) * (C03*eps(vE) - C03*alpha*T*I(vE)), LR) ## FIXME
                        # _testExp(grad(vE) * C03*(eps(vE) - alpha*T*I(vE)), LR)  ## FIXME
...
class TestFEAEval(TestCollection):
    ...
    def test_FEASolution_Eval(self):
        ...
        self.assertEqual(pg.interpolate(mesh, L(uS(mN)), mN), K.eval(mN), atol=1e-16)
        self.assertEqual(pg.interpolate(mesh, L(uS(mN)), mC), K.eval(mC), atol=1e-16)
    ...
    def test_FEA_Elastic_Expression_Eval3(self):
        ...
        v1 = VectorSpace(m, p=2, elastic=True)
        vS1 = FEASolution(v1, vF(v1.mesh))
        ...
        dim = 3
        # C = createElasticityTensor(lam=lam, mu=mu, dim=dim,
        #                               voigtNotation=True)
        # print('C:', C)
        ...
        # pg._g(sigma(vS)(pt, dim=dim))
        # pg._y(sigma1(vS)(pt, dim=dim))

        # print(asNoMapping(sigma1(vS1)(pt, dim=dim)))

        # self.assertEqual(sigma(vS)(pt, dim=dim),
        #                  asNoMapping(sigma1(vS1)(pt, dim=dim)))

        # self.assertEqual(asVoigtMapping(sigma(vS)(pt, dim=dim)),
        #                  sigma1(vS1)(pt, dim=dim))
        ...
        for i, m in enumerate(createTestMeshs()[2:]):
            ...
            for pt in [
                ...
                try:
                    ...
                    pg._y(f'sigma1(vS1)(pt, dim=dim): {sigma1(vS1)(pt, dim=dim)}')
