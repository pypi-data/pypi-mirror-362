#!/usr/bin/env python
# -*- coding: utf-8 -*-
r"""Classes to hold and manage finite element solutions.
"""
import copy
...
def _showSolution(u, mesh=None, **kwargs):
    ...
    if isVecField(v_):
        ## TODO: refactor with FEAFunction
        if not noAbs:
...
class FEASolution(OP):
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        """ ufunc to communicate with numpy.
        """
    ...
    def qpInterpolationMatrix(self, p):
        ...
        if not ha in self._qpInterpolationMatrixCache:
...
class FEASolutionOP(OP):
    ...
    def show(self, **kwargs):
        ...
        **kwargs forwarded to pg.show().
        ...
        -------
        ax, cBar:
            Return axe and colorbar from pg.show.
        """
        return _showSolution(self, **kwargs)
    ...
    def eval(self, *args, **kwargs):
        ...
        if case == 1 or (hasattr(self, 'interpolationOrder') and \
            ...
            if hasattr(self.b, 'mesh') and not 'mesh' in kwargs:
                # give mesh for cell values
                a = _get(self.a, p, mesh=self.b.mesh)
            else:
                a = _get(self.a, p)

            if hasattr(self.a, 'mesh') and not 'mesh' in kwargs:
                # give mesh for cell values
