...
from .op import OP
from .utils import call, __newFieldOrder__
...
def _fillMatX(E):
    ...
    for i in range(len(E._mat)):
        try:
            E.setMatXI(i, E._mat[i])
        except RuntimeError as e:
            print(E)
            print(E._mat[i])
            E.setMatXI(i, np.ascontiguousarray(E._mat[i]))
...
def copyE(A, rows=None, cols=None, matX=None, transpose=False, scale=True):
...
def duE(c, scale=1.0, order=2, nCoeff=1, dofPerCoeff=0, dofOffset=0,
        ...
        core=False):
    """ Create a gradient ElementMatrix.
    """
    if core is True:
        ## should ne be here
        pg.criticle('implement me')

    # print('duE', c, scale, order, nCoeff, dofPerCoeff, dofOffset)
    if nCoeff > 1 and dofPerCoeff == 0:
        pg.error('number of coefficents > 1 but no dofPerCoeff given')
...
def trE(A):
    """ Return trace for ElementMatrix as I(v) * trace(E)
    """
    if pg.isMatrix(A):
        return np.trace(A)
...
def mulE(A, f, c=1.0, core=False, **kwargs):
    ...
    if sum(A.colIDs()) > 0:
        ...
        print(f'f:{f}')
        print(f'c:{c}')
        pg.critical("Mult E only non bilinear Elementmatrices")

    ### simple elementwise multiplication for two ElementMatrices without
    # bilinear span
    if isinstance(f, pg.core.ElementMatrix):
        ...
        if pg.isScalar(f.mulR):
            if f.mulR != 1.0:
                E.mulR *= f.mulR

        if isinstance(f.mulR, ElasticityTensor) or pg.isMatrix(f.mulR):
            ## E * f(C) * I(v)
            if f.entity().dim() == 2:
                if f.mulR.shape == (6,6):
                    if pg.core.deepDebug() == -1:
    ...
    if isinstance(f, list):
        ...
        ###
        if any([callable(fi) for fi in f]):
    ...
    if isinstance(f, OP):
        ...
        if pg.isScalar(f):
            if pg.core.deepDebug() == -1:
                pg._y(f'f is global scalar per cell')
            ...
            if pg.core.deepDebug() == -1:
                pg._y(f'f is pos per quadrature')
            ...
            ###
            if pg.core.deepDebug() == -1:
                pg._b(f'f is scalar per quadrature')
            ...
            if pg.core.deepDebug() == -1:
                pg._b(f'f is scalar per node per scalar space')
            ...
            if pg.core.deepDebug() == -1:
                pg._b(f'f is scalar per node per vector space')
            ...
            print(f'c: {c}')
            print(f'f: {type(f)} {f}')
            pg.critical("Can't interprete f")
...
def dotE(A, B, c=1.0, core=False, **kwargs):
    ...
    if pg.core.deepDebug() == -1:
        ...
        pg._g(f'*'*60)
        try:
            pg._g(f'A: {type(A)} mulR: {A.mulR} div:{A.isDiv()}, order:{A.order()} \n{A}\n')
        except BaseException:
            try:
                pg._g(f'A: {type(A)} mulR: {A.mulR} \n{A}\n')
            except:
                pg._g(f'A: {type(A)} \n{A}\n')

        try:
            pg._g(f'B: {type(B)} mulR: {B.mulR} div:{B.isDiv()}, order:{B.order()} \n{B}\n')
        except BaseException:
            try:
                pg._g(f'B: {type(B)} mulR: {B.mulR}\n{B}\n')
            except:
                pg._g(f'B: {type(B)} \n{B}\n')
    ...
    if pg.isScalar(c):
    ...
    elif isinstance(c, ElasticityTensor) or pg.isMatrix(c) \
        and (c.shape == (6,6) and A.entity().dim() == 2):
        ### C is 3D elasticity tensor but A is 2D space
        if pg.core.deepDebug() == -1:
        ...
        pg.critical("Can't interprete c", c)
