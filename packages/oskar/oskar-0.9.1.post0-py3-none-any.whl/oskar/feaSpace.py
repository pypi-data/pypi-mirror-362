...
class FEASpace():
    ...
    def __hash__(self):
        ...
        """
        return hash(self.mesh) ^ self.order
    ...
    def nodeIdx(self, boundaryMarker):
        """Return all node indices (unique sorted)
            for all boundaries by marker. """
        m = self.mesh
        ids = []
...
class ConstantSpace(FEASpace):
    ...
    def split(self, u, skipHistory=False, time=None):
        """ Split the values from the global vector u and create a FEASolution.
        """
        return u[self.dofs]
...
class ScalarSpace(FEASpace):
    ...
    def split(self, u, skipHistory=False, time=None):
        ...
        if self._solution is None or skipHistory is True:
...
class VectorSpace(FEASpace):
    ...
    def values(self, v):
        ...
        if v.ndim == 1:
            ...
            self._valuesRaw = np.array(self._values.flatten())#.reshape(1, self.dof).T
    ...
    def split(self, u, skipHistory=False, time=None):
        ...
        if self._solution is None or skipHistory is True:
            ...
            # TODO solution holds an own copy of this space for evaluation (clunky, refactor me!)
            ...
            self._solution = FEASolution(self, values=None, name=self.__str__())
...
def TaylorHood(mesh, order:int=4):
    ...
    The Mixed space consist of a VectorSpace with quadratic base function and a ScalarSpace with linear base function.
