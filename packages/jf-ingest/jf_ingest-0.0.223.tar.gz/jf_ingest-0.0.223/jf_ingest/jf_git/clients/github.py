...
class GithubClient:
    ...
    def get_pr_authors(
    ...
    ) -> dict[int, dict[str, Union[str, int, None]]]:
        """
        ...
        pr_authors: dict[int, dict[str, Union[str, int, None]]] = {}
        ...
        for number_batch in batch_iterable(pr_ids, 100):
            ...
            for _, pr_data in result['data']['organization']['repo'].items():
                ...
                if pr_actor:
                    ...
                    auth_name = pr_actor.get('name')
                    if isinstance(auth_name, str) and auth_name.startswith('GitHubArchive\\'):
                        auth_name = auth_name.split('\\', 1)[1]
