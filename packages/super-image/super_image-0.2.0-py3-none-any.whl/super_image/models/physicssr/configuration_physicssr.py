...
class PhysicssrConfig(PretrainedConfig):
    """
    This is the configuration class to store the configuration of a :class:`~super_image.PhysicssrModel`.
    It is used to instantiate the model according to the specified arguments, defining the model architecture.
    Instantiating a configuration with the defaults will yield a similar
    configuration to that of the [PhysicsSR](https://huggingface.co/eugenesiow/physicssr) architecture.
    ...
    Examples:
        ```python
        from super_image import PhysicssrModel, PhysicssrConfig
        ...
        ```
    """
    model_type = 'PHYSICS_SR'

    def __init__(self, scale: int = None, n_resblocks=32, n_feats=256, n_colors=3, rgb_range=255,
                 rgb_mean=DIV2K_RGB_MEAN, rgb_std=DIV2K_RGB_STD, res_scale=0.1, data_parallel=False, **kwargs):
        """
        Args:
