...
class SmsrConfig(PretrainedConfig):
    """
    This is the configuration class to store the configuration of a :class:`~super_image.SmsrModel`.
    It is used to instantiate the model according to the specified arguments, defining the model architecture.
    Instantiating a configuration with the defaults will yield a similar
    configuration to that of the [SMSR](https://huggingface.co/eugenesiow/smsr) architecture.
    ...
    Examples:
        ```python
        from super_image import SmsrModel, SmsrConfig
