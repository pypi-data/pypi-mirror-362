...
def create_layer(
    ...
    response = requests.post(
        ...
        json={
            ...
            "dynamic_compute_version": version("earthdaily-earthone-dynamic-compute"),
        },
        timeout=60,
    )
...
def create_mosaic(
    ...
    resampler : eo.catalog.ResampleAlgorithm
        default: eo.catalog.ResampleAlgorithm.NEAR. Optional resampling algorithm to use when mosaicking.
        eo.catalog.ResampleAlgorithm.BILINEAR option is also available.
...
def select_scenes(
    ...
    """
    Select scenes based on date, from a product in the EarthOne catalog.
...
def filter_data(stack_graft: Dict, encoded_filter_func: str) -> Dict:
    ...
    encoded_filter_func: str
        Encoded predicate, either a json serialized eo.catalog.properties.OpExpression,
        or a base64 encoded function. The function must take a eo.catalog.Image as input
        and return a bool.
...
def compute_aoi(
    graft: Dict, aoi: eo.geo.AOI, layer_id: str = None, **kwargs
) -> np.ma.MaskedArray:
    ...
    if isinstance(
        aoi,
        (
            earthdaily.earthone.core.common.geo.geocontext.AOI,
            earthdaily.earthone.core.common.geo.geocontext.DLTile,
            earthdaily.earthone.core.common.geo.geocontext.XYZTile,
        ),
    ):
        aoi = eo.geo.AOI(
...
def _geocontext_from_latlon(lat: float, lon: float) -> eo.geo.AOI:
    """
