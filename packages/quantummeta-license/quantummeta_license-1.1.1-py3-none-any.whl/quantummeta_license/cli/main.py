...
# Server commands subgroup
server_app = typer.Typer(help="License server commands")
...
def start(
    ...
    try:
        ...
        rprint(f"[green]🚀 Starting QuantumMeta License Server...[/green]")
        rprint(f"[cyan]Server: http://{host}:{port}[/cyan]")
        rprint(f"[cyan]Admin Dashboard: http://{host}:{port}/admin[/cyan]")
        rprint(f"[cyan]API Documentation: http://{host}:{port}/docs[/cyan]")
        
        uvicorn.run(
            "quantummeta_license.server.main:app",
            host=host,
            port=port,
            reload=reload,
            log_level="info"
        ...
        rprint("[red]❌ Server dependencies not installed[/red]")
        rprint("[yellow]Install with: pip install quantummeta-license[server][/yellow]")
        raise typer.Exit(1)
    except Exception as e:
...
@server_app.command()
def setup_email():
    """Setup email configuration for the license server."""
    try:
        import subprocess
        import sys
        
        rprint("[green]🔧 Setting up email configuration...[/green]")
        ...
        if result.returncode == 0:
            rprint("[green]✅ Email setup completed successfully![/green]")
        else:
            rprint("[red]❌ Email setup failed[/red]")
            raise typer.Exit(1)
            
    except Exception as e:
        rprint(f"[red]❌ Error setting up email: {e}[/red]")
        raise typer.Exit(1)
