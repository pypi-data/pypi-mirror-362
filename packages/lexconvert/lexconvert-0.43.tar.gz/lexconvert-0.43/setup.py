from setuptools import setup, find_packages;setup(name='lexconvert',version='0.43',entry_points={'console_scripts':['lexconvert=lexconvert.__init__:main']},license='Apache 2',platforms='any',url='http://ssb22.user.srcf.net/lexconvert/',author='Silas S. Brown',author_email='ssb22@cam.ac.uk',description='Convert phoneme codes and lexicon formats for English speech synths',long_description=r'''# lexconvert
...
E.g.: `python -m lexconvert` `--phones` unicode-ipa This is a test sentence.  
...
E.g.: `python -m lexconvert` `--ruby` unicode-ipa This is a test sentence.  
...
E.g.: `python -m lexconvert` `--try` festival h @0 l ou1  
 or: `python -m lexconvert` `--try` unicode-ipa `'\u02c8\u0279\u026adn\u0329'` (for Unicode put `'\uNNNN'` or UTF-8)
...
e.g. for Debian festlex-oald package: `python -m lexconvert` `--festival-dictionary-to-espeak` /usr/share/festival/dicts/oald/all.scm  
or if you can't install the Debian package, try downloading http://ftp.debian.org/debian/pool/non-free/f/festlex-oald/festlex-oald_1.4.0.orig.tar.gz, unpack it into /tmp, and do: `python -m lexconvert` `--festival-dictionary-to-espeak` /tmp/festival/lib/dicts/oald/oald-0.4.out  
