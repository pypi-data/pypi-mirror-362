...
class Commit:
    ...
    def format(self, url: str) -> str:
        ...
        if match := RE_PR_NUMBER.search(msg):
            ...
            msg += f" ([{self.sha}]({url}/commit/{self.sha}))"
        return msg
...
Sections = dict[str, list[Formattable]]
...
class Release:
    ...
    def __init__(
        self, version: str, sections: Sections, date: str = ""
    ) -> None:
    ...
    def format(self, url: str) -> str:
        ...
        output.append("")
        for category, commits in sorted(self.sections.items()):
            if commits:
                output.append(f"### {category}")
                output.append("")
                output.extend(c.format(url) for c in commits)
                output.append("")
        return "\n".join(output)
...
def parse_changelog(path: str) -> Changelog:
    ...
    for line in read_all_lines(path):
        ...
        if line and version and sections and section in sections:
            sections[section].append(Entry(line))
...
def classify_commits(commits: list[Commit]) -> Sections:
