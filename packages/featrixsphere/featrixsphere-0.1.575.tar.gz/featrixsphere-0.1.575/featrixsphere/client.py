        ...
        try:
            ...
            if single_predictor.get('available'):
                ...
                try:
                    ...
                    # Look for single predictor training metrics, not ES training metrics
                    single_predictor_path = single_predictor.get('path', '')
                    if debug:
                        print(f"🔍 Debug: single_predictor_path = {single_predictor_path}")
                    
                    # Extract the job directory from the single predictor path to find its training metrics
                    if single_predictor_path:
                        import os
                        job_dir = os.path.dirname(single_predictor_path)
                        training_metrics_path = os.path.join(job_dir, 'training_metrics.json')
                        if debug:
                            print(f"🔍 Debug: looking for training metrics at: {training_metrics_path}")
                        
                        # Try to load the single predictor training metrics directly
                        try:
                            import requests
                            import json
                            
                            # Get training metrics from the specific single predictor job
                            response = requests.get(f"{self.base_url}/file", params={'path': training_metrics_path})
                            if response.status_code == 200:
                                sp_metrics = response.json()
                                if debug:
                                    print(f"🔍 Debug: loaded single predictor metrics keys = {list(sp_metrics.keys())}")
                                
                                target_column = sp_metrics.get('target_column')
                                if debug:
                                    print(f"🔍 Debug: extracted target_column from SP metrics = {target_column}")
                                    
                                if target_column:
                                    # Create metadata from single predictor metrics
                                    metadata = {
                                        'epochs': sp_metrics.get('args', {}).get('n_epochs'),
                                        'target_column_type': sp_metrics.get('target_column_type'),
                                        'final_metrics': sp_metrics.get('final_metrics', {}),
                                        'training_info': sp_metrics.get('training_info', []),
                                ...
                                if debug:
                                    print(f"❌ Failed to load single predictor metrics: {response.status_code}")
                        except Exception as metrics_e:
                            if debug:
                                print(f"❌ Error loading single predictor metrics: {metrics_e}")
                            # Fallback: try the old way with ES training metrics
                            training_metrics = models.get('training_metrics', {})
                            if training_metrics.get('available'):
                                ...
                                if target_column:
                                    ...
                                    if debug:
                                        print(f"✅ Fallback: Added single predictor for target_column: {target_column}")
