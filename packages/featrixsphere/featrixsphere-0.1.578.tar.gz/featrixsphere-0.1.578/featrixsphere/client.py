    ...
    def _get_available_predictors(self, session_id: str, debug: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if single_predictor.get('available'):
                # Try to get single predictor training metrics (API has fallback to search job directories)
                try:
                    if debug:
                        print(f"🔍 Debug: single_predictor available, trying to get training metrics...")
                    
                    # Use the training metrics endpoint which has fallback logic for single predictors
                    metrics_data = self.get_training_metrics(session_id)
                    if debug:
                    ...
                    if target_column:
                        ...
                        if debug:
                            print(f"❌ No target_column found in training metrics - checking if this is ES metrics instead of SP metrics")
                            print(f"   This might indicate the API is returning ES training metrics instead of single predictor metrics")
