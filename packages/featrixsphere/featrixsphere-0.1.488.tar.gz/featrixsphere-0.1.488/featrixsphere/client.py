...
class FeatrixSphereClient:
    ...
    def _create_clean_evolution_plot(self, epoch_projections, session_id, latest_epoch, 
        ...
        """Create a clean, functional evolution plot with working categorical controls and training progress."""
        try:
            import plotly.graph_objects as go
            from plotly.subplots import make_subplots
            import pandas as pd
            ...
            all_data = []
            epochs = sorted([v.get('epoch', 0) for v in epoch_projections.values() if v.get('coords')])
            
            for frame_idx, epoch in enumerate(epochs):
                for proj_data in epoch_projections.values():
                    if proj_data.get('epoch', 0) == epoch:
                        coords = proj_data.get('coords', [])
                        if coords:
            ...
            # Create subplots with 3D + training progress charts
            fig = make_subplots(
                rows=2, cols=2,
                ...
                specs=[[{"type": "scatter3d", "rowspan": 2}, {"type": "xy"}],
                       [None, {"type": "xy"}]],
                subplot_titles=("🎯 3D Embedding Space Evolution", 
                              f"📊 Training Loss (Epoch {latest_epoch})",
                              f"📈 Progress: Frame {len(epochs)} | ES: {es_epoch_count} | SP: {sp_epoch_count}")
            )
            ...
            for frame_idx in range(len(epochs)):
                ...
                scatter_trace = go.Scatter3d(
                    ...
                    customdata=[epochs[frame_idx]] * len(frame_data)
                )
                
                frames.append(go.Frame(data=[scatter_trace], name=str(frame_idx)))
            ...
            # Create comprehensive animation controls with frame-by-frame navigation
            ...
            fig.update_layout(
                ...
                # REMOVE plotly's default controls to prevent duplicates
                showlegend=True,  # Enable legend for training loss charts
                legend=dict(x=0.75, y=0.95, bgcolor='rgba(255,255,255,0.8)'),
                margin=dict(l=0, r=0, t=60, b=50),  # Increased bottom margin for slider
                height=600,
                title=None,  # Remove default title to prevent clutter
                annotations=[
                    dict(
                        text="💡 Use slider below for frame-by-frame navigation",
                        xref="paper", yref="paper",
                        x=0.5, y=-0.08,
    ...
    def _add_training_progress_charts(self, fig, training_metrics, latest_epoch, es_epoch_count, sp_epoch_count):
        """Add training progress charts to the subplot figure."""
        try:
            import plotly.graph_objects as go
            ...
            # TOP RIGHT: Training Loss Chart
            if loss_history:
                # Filter data up to current epoch
                epochs = [e.get('epoch', 0) for e in loss_history if e.get('epoch', 0) <= latest_epoch]
                train_losses = [e.get('loss', 0) for e in loss_history if e.get('epoch', 0) <= latest_epoch]
                val_losses = [e.get('validation_loss', 0) for e in loss_history if e.get('epoch', 0) <= latest_epoch]
                ...
                if epochs:
                    ...
                    if epochs:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                x=[latest_epoch], y=[current_train_loss],
                                mode='markers',
                        ...
                        fig.add_trace(
                            go.Scatter(
                                x=[latest_epoch], y=[current_val_loss],
                                mode='markers',
                                marker=dict(color='#ff7f0e', size=8, symbol='circle'),
                                showlegend=False
                            ),
                            row=1, col=2
                        )
            
            # BOTTOM RIGHT: Progress Bar Chart
            categories = []
            values = []
            colors = []
            ...
            print(f"⚠️ Error adding training progress charts: {e}")
