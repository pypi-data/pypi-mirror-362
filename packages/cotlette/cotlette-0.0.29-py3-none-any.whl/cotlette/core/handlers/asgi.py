...
from cotlette.core.exceptions import RequestAborted, RequestDataTooBig
from cotlette.core.handlers import base
from cotlette.http import (
    FileResponse,
    HttpRequest,
    HttpResponse,
...
from cotlette.urls import set_script_prefix
from cotlette.utils.functional import cached_property

logger = logging.getLogger("cotlette.request")
...
class ASGIHandler(base.BaseHandler):
    ...
    async def __call__(self, scope, receive, send):
        ...
        if scope["type"] != "http":
            raise ValueError(
                "Cotlette can only handle ASGI/HTTP connections, not %s." % scope["type"]
            )
