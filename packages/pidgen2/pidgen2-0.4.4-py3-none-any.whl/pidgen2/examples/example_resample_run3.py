...
files = [
  ("root://eoslhcb.cern.ch//eos/lhcb/grid/prod/lhcb/anaprod/lhcb/MC/2024/MC.ROOT/00284237/0000/00284237_00000001_1.mc.root", "b2dk_pidgen", "2024_WithUT_block1_v0-MagUp-Pi"),
]

# Name of the input tree
# Could also include ROOT directory, e.g. "Dir/Tree"
input_tree = "Bu2D0pi_D02Kpi/DecayTree"
...
etavar = None
pvar   = "P"
## Could also use eta variable instead of P
#etavar = "ETA"
#pvar   = None

ntrvar = "nLongTracks"  # In Run3 (but not in Runs 1/2 !) the multiplicity proxy is the number of _long tracks_
...
scale = (1, 1, 1.1)  # List of scale factors for each dimension of input data, to increase track multiplicity by 10%

# List of (kernel, seeds) combinations for raw template smearing
kernels = [
  ('default',    [0, 1, 2]),      # Default kernel width, no template sampling 
                                  # and two "bootstrapped" templates with seeds 1,2
  ('syst1',      [0]),            # Wider "syst1" kernel, no template sampling 
  ((2, 2, 2, 2), [0])             # Kernel with custom widths (2,2,2,2) bins in PID,Pt,Eta,Ntracks 
...
config = {
  'Bach' :    {
            "PROBNN_K"    : ("pi_Dstar2Dpi", "PROBNN_K", kernels),
            "PROBNN_PI"   : ("pi_Dstar2Dpi", "PROBNN_PI", kernels),
            "PID_K"       : ("pi_Dstar2Dpi", "PID_K", kernels),
           },
}
