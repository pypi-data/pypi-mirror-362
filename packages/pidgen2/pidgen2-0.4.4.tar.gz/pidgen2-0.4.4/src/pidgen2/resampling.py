...
from .plotting import plot, plot_distr2d, plot_distr1d, set_lhcb_style, plot_hist2d, plot_hist1d, plot_hist1d_comparison
...
def truncate_middle(s, n):
  ...
  """
  if isinstance(s, tuple) : s = s[0]  # Only leave main filename if separate sweight file
  if len(s) <= n : return s
  n_2 = int(n) // 2 - 3
  n_1 = n - n_2 - 3
...
def cache_calib_tuple(sample, trees, expressions, branches, verbose = False, cachedir = None, cut = None) : 
  """ 
  ...
  for i,filename in enumerate(sample) : 
    ...
    if isinstance(filename, tuple) : 
      ...
      main_filename, sweight_filename = filename
    else : 
      main_filename = filename
      sweight_filename = None

    sys.stdout.write(f"\rReading file ({i+1}/{len(sample)}) {truncate_middle(main_filename, 80)}")
    sys.stdout.flush()

    if cachedir : 
      calib_cache_filename = cachedir + "/" + main_filename.split("/")[-1]
      file = uproot.recreate(calib_cache_filename, compression=uproot.ZLIB(4))

    for tree in trees : 
      if sweight_filename is None :  # Run1/2-like calibration with sWeights included in main file
        try : 
          for t in uproot.iterate(main_filename + ":" + tree, library = "pd", expressions = branches, cut = cut) :
      ...
      else : # Run3 calibration where sweights are in a separate file
        main_ds = uproot.concatenate(main_filename + ":" + tree, library = "pd", expressions = branches[:-1])
        sw_ds = uproot.concatenate(sweight_filename + ":" + tree, library = "pd", expressions = branches[-1:])
        arr = np.stack([ main_ds.eval(b).values for b in expressions[:-1] ] + \
                       [ sw_ds[branches[-1]].values ], axis = 1)
        if cut : 
          # Apply cut to the combination
          cut_ds = uproot.concatenate(main_filename + ":" + tree, library = "pd", expressions = cut).values[:,0]
          arr = arr[cut_ds]
...
def read_calib_file(filename, trees, expressions, branches, cachedir = None, cut = None, cache_action = None) : 
  ...
  datasets = []
  if cachedir : 
    cache_filename = cachedir + "/" + main_filename.split("/")[-1]
...
def calib_transform(x, config, variable) :
  ...
  Args: 
    x: 2D numpy array in the format returned by read_calib_file function. 
  ...
  for i,(s,t) in enumerate(zip(config["smear"], transform_sample)) : 
    ...
    if s : 
      xs = x[:,i+1] + s[0] + (s[1]-s[0])*np.random.uniform(size = x.shape[0])
    else : 
      xs = x[:,i+1]
    arr += [ t(xs) ]
...
def subst_probe_prefix(name, prefix) : 
  if name.startswith("probe_") : 
    return name.replace("probe_", prefix + "_")
  if name.startswith("{prefix}_") : 
    return name.format(prefix = prefix)
  return name
...
def store_to_cache(variable, config, max_files = 0, verbose = False, cachedir = None) : 
  ...
  track_prefix = config.get("prefix", "probe")
  expressions = [ subst_probe_prefix(variable["expression"], track_prefix) ] + config["branches"]
  if "branches" in variable :
    calib_branches = [ subst_probe_prefix(v, track_prefix) for v in variable["branches"]] + config["branches"]
  else : 
  ...
  if max_files == 0 : 
    n_files = cache_calib_tuple(sample, trees, expressions, calib_branches, verbose = verbose, cachedir = calib_cache_dirname, cut = cut)
  else : 
    n_files = cache_calib_tuple(sample[:max_files], trees, expressions, calib_branches, verbose = verbose, cachedir = calib_cache_dirname, cut = cut)
  if verbose >= 0 :
    print(f"Read {n_files} calibration subsamples from remote storage and stored to cache.")
...
def create_template_multipass(variable, config, kernels = None, use_calib_cache = False, control_plots = False, 
  ...
  np.random.seed(1)  # To make variable transformation of the calibration sample (calib_transform) deterministic
  ...
  if control_plots : 
    ...
    if len(ranges) == 4 : 
      ...
      data_1dproj = {
      0 : [np.sum(counts, (1,2,3)), edges[0]],
      1 : [np.sum(counts, (0,2,3)), edges[1]],
      2 : [np.sum(counts, (0,1,3)), edges[2]],
      3 : [np.sum(counts, (0,1,2)), edges[3]],
      }
      ...
      smooth_1dproj = {
      0 : [np.sum(smooth_counts, (1,2,3)), edges[0]],
      1 : [np.sum(smooth_counts, (0,2,3)), edges[1]],
      2 : [np.sum(smooth_counts, (0,1,3)), edges[2]],
      3 : [np.sum(smooth_counts, (0,1,2)), edges[3]],
      }
      ...
      data_1dslices = {
      0 : [counts[:,n2,n3,n4], edges[0]], 
      1 : [counts[n1,:,n3,n4], edges[1]], 
      2 : [counts[n1,n2,:,n4], edges[2]], 
      3 : [counts[n1,n2,n3,:], edges[3]], 
      }
      ...
      smooth_1dslices = {
      0 : [smooth_counts[:,n2,n3,n4], edges[0]], 
      1 : [smooth_counts[n1,:,n3,n4], edges[1]], 
      2 : [smooth_counts[n1,n2,:,n4], edges[2]], 
      3 : [smooth_counts[n1,n2,n3,:], edges[3]], 
      }
    ...
    for i in smooth_1dproj.keys() : 

      if verbose : print(f"Plots for 1D projection {names[i]}")

      with plot(f"{names[i]}_data_1dproj", prefix) as (fig, ax) : 
        plot_hist1d_comparison(data_1dproj[i], smooth_1dproj[i], ax = ax, label = "Normalised " + labels[i], log = False, title = "Projection")

      with plot(f"{names[i]}_data_1dslice", prefix) as (fig, ax) : 
        plot_hist1d_comparison(data_1dslices[i], smooth_1dslices[i], ax = ax, label = "Normalised " + labels[i], log = False, title = "Data slice")
...
def resample_data(data, config, variable, template, chunk_size = 50000, verbose = False) : 
  ...
  while not stop : 
    ...
    rnd = np.random.uniform(size = (end_index-start_index, ))
