...
def read_jsonfile(json_location: str) -> str:
    ...
    json_rdd = spark.sparkContext.wholeTextFiles(json_location)
...
def create_jsonpath_list(json_path: str, json_data: str) -> list:
    """
...
def increase_level(level: int, previous_level: int, json_path: list, key: str, final: int, inPath: int) -> int:
    """
...
def create_new_key_col(parent_key: str, i: int, item: any) -> tuple:
    """
...
def add_fields_to_df(df, Datestring: str, SourceConfigId: int, TableName: str, DeltaTableName: str, pk_columns: list):
    """
    ...
    df = df.withColumn('PrimaryKeyColumn', fc.concat_ws("_", *[fc.col(c).cast("string") for c in pk_columns]))
...
def clean_dataset(dataset):
    ...
    cleaned_dict = []  # Initialize a list to hold the cleaned dictionaries.
...
def load_json_to_df(json_location: str, DeltaTableName: str, FolderName: str, SourceConfigId: int, TableName: str, json_search_path: str, pk_columns: str):
    ...
    df = spark.createDataFrame(standardized_dataset)
