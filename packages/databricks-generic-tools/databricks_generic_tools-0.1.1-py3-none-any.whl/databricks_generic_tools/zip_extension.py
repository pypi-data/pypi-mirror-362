...
def extract_and_store_from_zipfile(LocFrom: str, LocToUnzipped: str, ZipFileNameFilter: str) -> str:
    ...
    zip_file_data = spark_e.read.format("binaryFile").load(LocFrom).select("content").collect()[0][0]
    ...
    with zipfile.ZipFile(io.BytesIO(zip_file_data), 'r') as zip_ref:
        ...
        localFilePath = f"/tmp/unzipped_files/{datetime.now(timezone.utc)}"
        ...
        dbutils_e.fs.cp(f"file:{localFilePath}/{fileName}", f"{LocToUnzipped}/{fileName}")
