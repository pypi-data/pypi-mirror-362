...
from db2_hj3415.valuation import connection, get_all_codes_sync, pretty_print, red as db_red, mil as db_mil, \
...
async def generate_data(col: str, target: str, client: AsyncIOMotorClient) -> T:
    generator = GENERATOR_MAP.get(col)
    if not generator:
        raise ValueError(f"지원하지 않는 컬렉션: {col}")
    data = await generator(target, client)
    pretty_print(data)
    return data


async def save_data(col: str, data: T, client: AsyncIOMotorClient):
...
async def generate_many_data(col: str, targets: list[str], client: AsyncIOMotorClient) -> dict[str, T]:
    generator = GENERATOR_MAP.get(col)
    if not generator:
        raise ValueError(f"지원하지 않는 컬렉션: {col}")
    ...
    for target in targets:
        results[target] = await generator(target, client)

    pretty_print(results)
    return results


async def save_many_data(col: str, many_data: dict[str, T], client: AsyncIOMotorClient):
...
def handle_save_many_command(col: str, targets: list[str]):
    ...
    async def main():
        try:
            many_data = await generate_many_data(col, valid_targets, client)
...
def handle_save_command(col: str, target: str):
    ...
    async def main():
        try:
            data = await generate_data(col, target, client)
            await save_data(col, data, client)
        finally:
            client.close()
