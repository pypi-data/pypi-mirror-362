...
from .utils import get_raw_data, timeseries_to_dataframe, show_graph_prophet, redis_cached
...
def nbeats_forecast(ticker: str, refresh: bool = False):
    ...
    # 1) 각 TimeSeries → DataFrame
    actual_df = actual_ts.to_dataframe()
    actual_df.columns = ["actual"]

    fcst_mean_df = fcst_mean_ts.to_dataframe()
    fcst_mean_df.columns = ["forecast"]

    lower_df = lower_ts.to_dataframe()
    lower_df.columns = ["lower"]

    upper_df = upper_ts.to_dataframe()
    upper_df.columns = ["upper"]

    # 2) index 기준 outer merge
    merged = (
        actual_df
        .merge(fcst_mean_df, how="outer", left_index=True, right_index=True)
        .merge(lower_df, how="outer", left_index=True, right_index=True)
        .merge(upper_df, how="outer", left_index=True, right_index=True)
        .sort_index()
    )

    # 3) 반올림 & NaN → None
    merged = merged.round(2).where(~merged.isna())
...
def run_prophet_forecast(ticker: str):
    ...
    show_graph_prophet(series_scaler_dict, forecast_series)
