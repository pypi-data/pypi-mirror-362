...
async def prepare_c1034_data(col: Literal['c103','c104'], code: str, client: AsyncIOMotorClient) -> dict[str, pd.DataFrame]:
    match col:
        case 'c103':
            data = await c103.get_latest(code, client, as_type='dataframe')
        case 'c104':
            data = await c104.get_latest(code, client, as_type='dataframe')
        case _:
