...
def main():
    ...
    cache_subparsers = cache_parser.add_subparsers(dest='cache_type', help='cache 종류: corp, mi')

    # ───── cache corp ─────
    cache_corp_parser = cache_subparsers.add_parser('corp', help='기업 코드 캐시')
    cache_corp_parser.add_argument('engine', type=str, help="이름: prophet, nbeats")
    cache_corp_parser.add_argument('targets', nargs='*', help="종목코드 (예: 005930, 000660, all)")

    # ───── cache me ─────
    cache_me_parser = cache_subparsers.add_parser('mi', help='시장지표 캐시')
    cache_me_parser.add_argument('engine', type=str, help="이름: prophet, nbeats")
    cache_me_parser.add_argument('targets', nargs='*', help="all 만 가능")
