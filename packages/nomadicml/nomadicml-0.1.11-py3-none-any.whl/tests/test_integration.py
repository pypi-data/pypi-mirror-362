...
def assert_has_string_field(d, field):
    ...
    assert isinstance(d[field], str), f"Field {field} is not a string"

def assert_has_number_field(d, field):
    assert field in d, f"Missing field: {field}"
    assert isinstance(d[field], (float, int)), f"Field {field} is not a number"
...
def test_rapid_review():
    ...
    response = client.upload(video_url)
        ...
        for analysis in analyses:
            ...
            for event in analysis['events']:
                ...
                try:
                    ...
                    print(f"label field format is wrong (known issue so we don't fail): {e}")
                    print(event['label'])
                    continue
...
def test_edge_case():
    ...
    try:
        ...
        print(analysis)
        assert_has_string_field(analysis, 'video_id')
        assert list(analysis.keys()) == ['video_id', 'mode', 'status', 'events']
