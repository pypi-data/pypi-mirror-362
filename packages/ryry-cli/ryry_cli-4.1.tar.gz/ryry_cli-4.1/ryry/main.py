...
def widget_status(stdscr, idx):
    ...
    for it in widget_map:
        ...
        timeout = widget_map[it].get("timeout", 600)
        end_args = ""
        if is_block:
            end_args = "[X]"
        ...
        real_stdsrc(idx, 0, scr_str(f'{f"[{name}*{max_task_number} V{version}] {it}{end_args} OOT:{timeout}s".ljust(maxJust)}'.ljust(ll*3-2)))
        idx+=1
