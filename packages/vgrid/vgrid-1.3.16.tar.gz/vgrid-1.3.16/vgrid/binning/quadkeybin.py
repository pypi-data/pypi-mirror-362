...
def quadkey_bin(data, resolution, stats, category=None, numeric_field=None, lat_col="lat", lon_col="lon", **kwargs):
    ...
    for _, row in tqdm(gdf.iterrows(), total=len(gdf), desc="Binning points"):
        ...
        if geom.geom_type == "Point":
            quadkey_id = latlon2quadkey(geom.y, geom.x, resolution)
            append_stats_value(quadkey_bins, quadkey_id, props, stats, category, numeric_field)
        elif geom.geom_type == "MultiPoint":
            for p in geom.geoms:
                quadkey_id = latlon2quadkey(p.y, p.x, resolution)
                append_stats_value(quadkey_bins, quadkey_id, props, stats, category, numeric_field)
    records = []
