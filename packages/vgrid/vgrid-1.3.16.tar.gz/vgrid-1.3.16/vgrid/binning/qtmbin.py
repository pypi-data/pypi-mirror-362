...
def qtm_bin(data, resolution, stats, category=None, numeric_field=None, lat_col="lat", lon_col="lon", **kwargs):
    ...
    for _, row in tqdm(gdf.iterrows(), total=len(gdf), desc="Binning points"):
        ...
        if geom.geom_type == "Point":
            qtm_id = qtm.latlon_to_qtm_id(geom.y, geom.x, resolution)
            append_stats_value(qtm_bins, qtm_id, props, stats, category, numeric_field)
        elif geom.geom_type == "MultiPoint":
            for p in geom.geoms:
                qtm_id = qtm.latlon_to_qtm_id(p.y, p.x, resolution)
                append_stats_value(qtm_bins, qtm_id, props, stats, category, numeric_field)
    records = []
...
def qtmbin(
    ...
    **kwargs,
    ...
    result_gdf = qtm_bin(data, resolution, stats, category, numeric_field, **kwargs)
