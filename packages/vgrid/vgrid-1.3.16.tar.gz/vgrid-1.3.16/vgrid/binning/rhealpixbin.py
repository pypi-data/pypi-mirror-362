...
def rhealpix_bin(data, resolution, stats, category=None, numeric_field=None, lat_col="lat", lon_col="lon", **kwargs):
    ...
    for _, row in tqdm(gdf.iterrows(), total=len(gdf), desc="Binning points"):
        ...
        if geom.geom_type == "Point":
            rhealpix_id = latlon2rhealpix(geom.y, geom.x, resolution)
            append_stats_value(rhealpix_bins, rhealpix_id, props, stats, category, numeric_field)
        elif geom.geom_type == "MultiPoint":
            for p in geom.geoms:
                rhealpix_id = latlon2rhealpix(p.y, p.x, resolution)
                append_stats_value(rhealpix_bins, rhealpix_id, props, stats, category, numeric_field)
    records = []
...
def rhealpixbin(
    ...
    **kwargs,
    ...
    result_gdf = rhealpix_bin(data, resolution, stats, category, numeric_field, **kwargs)
