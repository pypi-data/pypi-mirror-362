...
def grid_id_to_corner_coord(gid, level, shift=False):
    ...
    level_x_i = np.array(
        [int(id[1]) if len(id) != 6 else int(id[3:]) for id in id_elements], dtype=int
    )
    level_y_i = np.array(
        [int(id[0]) if len(id) != 6 else int(id[0:3]) for id in id_elements], dtype=int
    )
    ...
    corner_x = np.array(
        [level_x_i[i] * levels_specs[i]["x_length"] for i in range(level_x_i.shape[0])]
    ).sum()

    corner_y = np.array(
        [level_y_i[i] * levels_specs[i]["y_length"] for i in range(level_y_i.shape[0])]
    ).sum()
...
def easedggs_grid_bounds(bounds, source_crs, level):
    ...
    lower_left, upper_left, lower_right, upper_right = chain(
        product([bounds[0], bounds[2]], [bounds[1], bounds[3]])
    )
    # print(f'conrner passed in: {corners}')
    line_segments = list(
        pairwise_circle([upper_left, upper_right, lower_right, lower_left])
    )
    ...
    if source_crs != ease_crs:
        ...
        tranform_coords = Transformer.from_crs(
            source_crs, ease_crs, always_xy=True
        ).transform
        line_segments = [
            [tranform_coords(c[0], c[1]) for c in seg] for seg in line_segments
        ]
    ...
    bound_box = GeoSeries([Point(c[0], c[1]) for c in line_segments], crs=ease_crs)
    ...
    bound_box = GeoSeries(
        [
            Point(bound_box[0], bound_box[3]),  # upper_left
            Point(bound_box[2], bound_box[1]),
        ],  # lower_right
        crs=ease_crs,
    )
    ...
    ease_xs = np.array(
        [shift_range_grid_multiple(ease_xs[i], axis="x") for i in range(len(ease_xs))]
    )
    ease_ys = np.array(
        [shift_range_grid_multiple(ease_ys[i], axis="y") for i in range(len(ease_ys))]
    )
...
def easedggs_align_check(in_raster, level):
    ...
    with rasterio.open(in_raster, "r") as src:
        ...
        if src.crs != "EPSG: 6933":
            results["Projection is EASE-Grid 2.0 (EPSG:6933):"] = str(
                "Fail. Supplied CRS is " + str(src.crs)
            )
            test_count -= 1
        ...
        if not epsilon_check(xw, easedggs_t.xoff):
            results[
                "The X dimension of the cell corners matches a EASE-DGGS corner:"
            ] = str("Fail. Difference = " + str(round(abs(xw - easedggs_t.xoff), 5)))
            test_count -= 1

        if not epsilon_check(yw, easedggs_t.yoff):
            results[
                "The Y dimension of the cell corners matches a EASE-DGGS corner:"
