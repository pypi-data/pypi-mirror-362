...
# import nanosurf.lib.devices.i2c.chip_AD7172
...
class ADC_Module(ADC.Chip_AD7172): 
    ...
    def setup_analog_mode(self, differential: bool):
        ...
        if self.differential:
            ...
            print ("ADC is set to differential mode")
        else:
            self.setup_single_end_mode()
            print ("ADC is set to single ended mode")
    ...
    def read_multiple_channel(self, channel: int) -> int:
        if self.differential:
    ...
    # def GPIO_ON(GPIO:int): #must also be defined in chip_AD7172
        
    # def GPIO_OFF(GPIO:int):
