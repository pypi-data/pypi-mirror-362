...
class VMFModule(nsf.frameworks.qt_app.ModuleBase):
    ...
    def __init__(self, app: nsf.frameworks.qt_app.ApplicationBase):
        ...
        self.list_of_known_sample_holder_configurations = ["2mm Gap","4mm Gap", "6mm Gap", "8mm Gap", "10mm Gap", "User"]
    ...
    def start_connect_to_vmf_controller(self):
        if self.worker_thread is not None:
            ai_port_in_use = 1 if self.app.settings._is_in_setup_mode else -1
            auto_setup = False if self.app.settings._is_in_setup_mode else True
            self.worker_thread.start_worker(command=vmf_worker.VFMWorkerCmd.Connect, call_setup=auto_setup, ai_port=ai_port_in_use)
    ...
    def is_referenced(self) -> bool:
        return self.worker_thread.vmf_controller.reference_data.is_reference_move_done
    
    def get_min_max_field(self) -> typing.Tuple[float, float]:
        return (self.worker_thread.vmf_controller.reference_data.field_min, self.worker_thread.vmf_controller.reference_data.field_max)
    ...
    def start_store_current_calibration_to_sample_holder(self):
        if self.worker_thread is not None:
        ...
        try:
            if self.worker_thread is not None:
                if self.worker_thread.vmf_controller is not None:
                    ...
                    self.worker_thread.vmf_controller.clear_referenced()
        except IndexError:
