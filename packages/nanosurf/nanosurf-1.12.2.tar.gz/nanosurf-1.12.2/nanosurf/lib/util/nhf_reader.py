...
class _CalNode():
    ...
    """
    def __init__(self, uuid:str, sink_name:str, unit:str, map_type:_CalMapType, map_par:list[float], calc_sink_from_source:bool=True, source_name:str= None):
        self.uuid = uuid
...
class NHFDataset():
    def __init__(self, parent:'NHFSegment', h5_dataset:h5py.Dataset, delay_load_of_data_from_file:bool=True) -> None:
        ...
        if delay_load_of_data_from_file:
            ...
            self.dataset:np.ndarray = self.h5_dataset
        else:
            self.h5_dataset = self._convert_h5py_dataset_to_numpy_array(self.h5_dataset)
    ...
    def _need_conversion_to_physical_unit(self, target_signal_id:str=None) -> bool:
        """ Returns True if data conversion to target signal is not yet done. If target is omitted default signal is used. 
        """
    ...
    def get_default_signal(self) -> str:
    ...
    def calc_dataset_by_calibration_graph(self, calibration_graph:list[int]):
        cal_nodes = self.parent_segment._file_hdl.cal_group.calibration_nodes
    ...
    def _build_calibration_graph(self, target_signal_id:str) -> list[int]:
        ...
        try:
            source_signal_id = self.attribute["signal_id"]
            calibration_graph = self.parent_segment._file_hdl.cal_group.build_calibration_graph(source_signal_id, target_signal_id)
    ...
    def _get_calib_node_by_index(self, node_index:int) -> _CalNode:
        return self.parent_segment._file_hdl.cal_group.calibration_nodes[node_index]

    def _find_calibration_nodes_by_uuid(self, uuid:str) -> list[_CalNode]:
        all_nodes = self.parent_segment._file_hdl.cal_group.calibration_nodes
        found_nodes = [node for node in all_nodes if node.uuid == uuid]
        return found_nodes
    
    def _load_dataset_without_conversion(self, handle_nan:bool=False):
        self.dataset = self._convert_h5py_dataset_to_numpy_array(self.h5_dataset)
    ...
    def _load_dataset_with_physical_unit(self, target_signal_id:str = None) -> None:
    ...
    def _create_unit_to_signal_map(self) -> dict[str,str]:
        ...
        file_version = self.parent_segment._file_hdl.version()
...
class NHFSegment():
    ...
    def channel_signals(self, ch:typing.Union[str, int]) -> set[str]:
        nhf_dataset = None
    ...
    def channel_units(self, ch:typing.Union[str, int]) -> set[str]:
        nhf_dataset = None
        if isinstance(ch, str):
            nhf_dataset = self.channel[ch]
        elif isinstance(ch, int):
    ...
    def read_channel(self, ch:typing.Union[str, int], as_matrix:bool=False, with_signal:str=None, with_unit:str=None) -> NHFDataset:
        """ Loads a specified data channel by name or index.
            The data provided by physical units as specified in the files calibration information.
    ...
    def _read_segment_structure(self):
        ...
        if dataset_names:
            for dataset_id, dataset_name in dataset_names.items():
                ...
                self.channel[dataset_name] = NHFDataset(self,dataset_data)
...
class NHFMeasurement(NHFSegment):
    ...
    def _read_measurement_structure(self):
        ...
        self.h5_datasets = _get_unidentified_sub_dataset(self._hdf_group, segment_names)
...
class NHFFileReader():
    ...
    def _read_file_structure(self):    
        ...
        self.group = _get_unidentified_sub_groups(self._file_id, known_groups)
        self.h5_datasets = _get_unidentified_sub_dataset(self._file_id, known_groups)

    def _read_calibration_group(self) -> NHFCalibrationGroup:
        calib_group = None
        ...
        for group_name, group_type in  group_type_names.items():  
            if group_type == "calibration":
                calib_group = NHFCalibrationGroup(self, self._file_id[group_name], group_name)
                break
        return calib_group
