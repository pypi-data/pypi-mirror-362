...
class CalibrateModule(nsf.frameworks.qt_app.ModuleBase):
    ...
    def do_analysis(self):
        ...
        uncal_adc_values_in_V = self.worker_thread.uncal_measurement
        
        # ref_channel = nsf.SciChannel(ref_field_in_T, unit = "T", name = "FM302 Teslameter")
        # adc_channel = nsf.SciChannel(uncal_adc_values_in_V, unit = "V", name = "intern hall sensor")
        # my_measurement = nsf.SciStream(adc_channel, channels=1)
        # my_measurement.set_channel(0,ref_channel)
        # nsf.plot.plot(my_measurement)
        ...
        if fit_degree == 1: 
            ...
            print("error fit degree")

        # offset = -1.0*offset/slope
        # slope = 1.0 / slope
        ...
        # adc_calib_channel = nsf.SciChannel(calib_adc_values_in_T, unit = "T", name = "intern hall sensor")
        # my_measurement = nsf.SciStream(adc_channel, channels=2)
        # my_measurement.set_channel(0,ref_channel)
        # my_measurement.set_channel(1,adc_calib_channel)
        # nsf.plot.plot(my_measurement, individual_plots=True)
