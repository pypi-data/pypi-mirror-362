...
class SetupScreen(nsf.frameworks.qt_app.ModuleScreen):
    ...
    def do_setup_screen(self, module: 'setup_module.SetupModule'):
        ...
        self.button_load_calibration = nsf.gui.NSFPushButton("Load Settings from EEPROM")
        ...
        self.layout_left.addWidget(NSFHLine())
        self.layout_left.addSpacing(10)
        self.layout_left.addWidget(self.button_load_calibration)
    ...
    def bind_gui_elements(self):
        ...
        self.button_load_calibration.clicked_event.connect(self.on_button_load_setup_clicked)    
    ...
    def start_stop_button_state(self, wait: bool = False, stop_state: bool = False):
        if wait:
            self.button_load_calibration.setEnabled(False)
            self.button_save_calibration.setEnabled(False)
            self.button_init_ctrl_setup.setEnabled(False)
            self.button_init_holder_setup.setEnabled(False)
        else:
            self.button_load_calibration.setEnabled(True)
            self.button_save_calibration.setEnabled(True)
            self.button_init_ctrl_setup.setEnabled(True)
            self.button_init_holder_setup.setEnabled(True)
    ...
    def on_button_load_setup_clicked(self):
