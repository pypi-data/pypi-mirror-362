    ...
    def do_work(self):
        ...
        if self.connect_to_controller():
            ...
            for field_step in range(self.par_field_steps):
                ...
                current_field = self.par_start_field + field_step*self.par_step_field
                ...
                if do_capture_measurement:
                    mask = f"{self.par_target_file_mask}_{nsf.SciVal(current_field,'T')}_[INDEX]".replace(" ", "_")
                ...
                for image_no in range(self.par_images_per_step):
                    ...
                    while self.spm_scan.IsScanning and not self.is_stop_request_pending():
                        ...
                        self.sig_tick.emit()
                        self.send_info_message(f"Measuring at {nsf.SciVal(current_field,'T')}, Image: {image_no+1}. Scan line: {self.spm_scan.Currentline}")
