...
class StdHSpacer(QtWidgets.QSpacerItem):
    def __init__(self, minimal_width: int = 200):
        super().__init__(minimal_width, 1, QtWidgets.QSizePolicy.MinimumExpanding, QtWidgets.QSizePolicy.Fixed)
...
class Screen(nsf.frameworks.qt_app.ModuleScreen):
    ...
    def do_setup_screen(self, worker: worker.Worker):
        self.worker = worker
        ...
        self.layout_left.addSpacerItem(StdHSpacer(minimal_width=300))
    ...
    def bind_gui_elements(self):
        nsf.gui.connect_to_property(self.combo_tip_mode_id, self.worker.settings.tip_mode)
