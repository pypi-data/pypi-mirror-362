...
class CalibrateWorker(nsf.frameworks.qt_app.nsf_thread.NSFBackgroundWorker):
    ...
    def do_work(self):
        ...
        if self._FM302COM != "" or sim_mode:
            ...
            uncal_config = VMFSampleHolderController.VMFConfiguration()
            uncal_config.cal_values = [0.0, 1.0, 0.0]
            uncal_config.name = active_config_backup.name
            self.vmf_module.worker_thread.vmf_controller._set_active_config(uncal_config)
            ...
            for pos_index in range(self.par_num_steps):
                ...
                self.uncal_measurement.append(cur_measured_value)
    ...
    def read_FM302(self) -> float:
        """read the magnetic field from the external USB Teslameter"""
        field = 0
