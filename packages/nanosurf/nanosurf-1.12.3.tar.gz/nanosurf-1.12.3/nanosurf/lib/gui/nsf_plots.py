...
class NSFChart(pg.GraphicsLayoutWidget):
    ...
    def set_data_point_symbols(self, symbols: Union[str, List[str]], layer:int=0):
        self.plot_layers[layer].setSymbol(symbols)

    def plot_data(self, y: Union[list, np.ndarray], x: Union[list, np.ndarray, None] = None, layer_index: int = 0, max_index:int=None):
    ...
    def set_marker(self, x: Union[float, list[float]], y: Union[float, list[float]]):
