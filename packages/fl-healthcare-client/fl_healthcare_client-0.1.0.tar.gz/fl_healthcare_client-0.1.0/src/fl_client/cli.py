...
@app.command()
def sync(
    url_or_path: str = typer.Argument(..., help="Supabase storage path or URL to download global model from"),
    ...
    config_file: Optional[str] = typer.Option(None, "--config", help="Supabase configuration file"),
    require_auth: bool = typer.Option(False, "--require-auth", help="Require Firebase authentication"),
    firebase_config: Optional[str] = typer.Option(None, "--firebase-config", help="Firebase configuration file")
):
    """
    Download the latest global model from Supabase Storage or HTTP URL.

    Examples:
        # Download from Supabase Storage
        python cli.py sync models/global/latest_model.pkl --require-auth

        # Download from HTTP URL (fallback)
        python cli.py sync https://server.com/global_model.pkl
    """
    print_banner()
    ...
    try:
        ...
        auth_token = None
        if require_auth or not url_or_path.startswith(('http://', 'https://')):
            auth_session = get_authenticated_session(firebase_config, require_auth)
            if auth_session and auth_session.is_authenticated():
                auth_token = auth_session.get_auth_token()
                console.print(f"[green]🔐 Authenticated as: {auth_session.get_user_info()['email']}[/green]")
            elif require_auth:
                console.print("[red]❌ Authentication required but failed[/red]")
                raise typer.Exit(1)

        # Download global model
        success = download_model(url_or_path, model, client_id, config_file, auth_token)
...
def upload(
    ...
    server_url_or_path: str = typer.Argument(..., help="Supabase storage path or server endpoint URL for uploading"),
    client_id: Optional[str] = typer.Option("client_001", "--client-id", "-c", help="Client identifier"),
    ...
    config_file: Optional[str] = typer.Option(None, "--config", help="Supabase configuration file"),
    require_auth: bool = typer.Option(True, "--require-auth/--no-auth", help="Require Firebase authentication"),
    firebase_config: Optional[str] = typer.Option(None, "--firebase-config", help="Firebase configuration file"),
    metadata: Optional[str] = typer.Option(None, "--metadata", help="JSON metadata to include with upload")
):
    """
    Upload local model weights to Supabase Storage or server endpoint.

    Examples:
        # Upload to Supabase Storage
        python cli.py upload ./models/local_model.pkl models/uploads --round 5 --require-auth

        # Upload to HTTP server (fallback)
        python cli.py upload ./models/local_model.pkl https://server.com/upload --round 5 --no-auth
    """
    print_banner()
    console.print(f"[bold yellow]📤 Uploading Local Model[/bold yellow]")
    console.print(f"Model: {model}")
    console.print(f"Destination: {server_url_or_path}")
    console.print(f"Client ID: {client_id}")
    ...
    try:
        ...
        # Handle authentication if required
        auth_token = None
        if require_auth or not server_url_or_path.startswith(('http://', 'https://')):
            auth_session = get_authenticated_session(firebase_config, require_auth)
            if auth_session and auth_session.is_authenticated():
                auth_token = auth_session.get_auth_token()
                console.print(f"[green]🔐 Authenticated as: {auth_session.get_user_info()['email']}[/green]")
        ...
        # Upload model
        success = upload_model(model, server_url_or_path, round_num, client_id, config_file, auth_token, upload_metadata)
...
def info():
    ...
    info_text = """
    [bold blue]Federated Learning Client Information[/bold blue]

    [cyan]Available Commands:[/cyan]
    • [green]train[/green]        - Train MLP model locally on CSV data
    • [green]evaluate[/green]     - Evaluate trained model on test dataset
    • [green]sync[/green]         - Download global model from Supabase Storage or HTTP URL
    • [green]upload[/green]       - Upload local model weights to Supabase Storage or server
    • [green]full-sync[/green]    - Complete sync cycle with server
    • [green]compare[/green]      - Compare multiple models on test data
    • [green]login[/green]        - Authenticate with Firebase
    • [green]logout[/green]       - Clear saved authentication
    • [green]setup-config[/green] - Create example configuration files
    ...
    [cyan]Storage & Authentication:[/cyan]
    • Supabase Storage for global model storage
    • Firebase Authentication for secure access
    • Signed URLs for secure downloads
    • Automatic fallback to HTTP endpoints

    [cyan]Example Workflow:[/cyan]
    1. python cli.py setup-config  # Create configuration files
    2. python cli.py login         # Authenticate with Firebase
    3. python cli.py train ./data/client1.csv --max-iter 1000
    4. python cli.py sync models/global/latest_model.pkl --require-auth
    5. python cli.py upload ./models/local_model.pkl models/uploads --round 5

    For detailed help on any command, use: python cli.py [COMMAND] --help
    """
