...
from agilix_api_fr8train.factories.api import build_api_connection
...
from agilix_api_fr8train.utils import converter
...
class Api:
    ...
    class Domains:
        ...
        def list_domains(
            ...
            params = {
                "domainid": domain_id if domain_id else self._conn.home_domain_id,
                "includedescendantdomains": include_descendant_domains,
                "limit": limit,
            }

            response = self._conn.get("listdomains", params=params)

            return response.get("response", {}).get("domains", {}).get("domain", [])

        def create_domains(self, domain_list: list[CreateDomainDefinition]) -> list:
            payload = converter.create_domain_list_to_payload(domain_list)
