...
class ReaperSession:
    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, 'instance'): cls.instance = super(ReaperSession, cls).__new__(cls)
    ...
    def reaper_pod_from_cluster(state: ReplState, update_state = True) -> str:
    ...
    def reaper_spec(state: ReplState, update_state=False):
        if reaper := ReaperSession.reaper_pod_from_cluster(state, update_state=update_state):
            user, pw = state.user_pass(secret_path='reaper.secret')
            local_port = Config().get('reaper.port-forward.local-port', 9001)

            return {
                'pod': reaper,
                'exec': f'kubectl exec -it {reaper} -n {state.namespace} -- bash',
                'forward': f'kubectl port-forward pods/{reaper} -n {state.namespace} {local_port}:8080',
