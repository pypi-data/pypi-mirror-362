...
class Bash(Command):
    ...
    def exec_with_dir(self, state: ReplState, args: list[str]):
        ...
        if state.pod:
            rs = [cassandra_pod_exec(state.pod, state.namespace, command, show_out=not session_just_created)]
        elif state.statefulset:
            rs = cassandra_nodes_exec(state.statefulset, state.namespace, command, action='bash', show_out=not session_just_created)
