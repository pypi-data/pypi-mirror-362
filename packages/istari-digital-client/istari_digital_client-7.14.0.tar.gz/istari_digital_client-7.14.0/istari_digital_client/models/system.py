...
class System(BaseModel, ClientHaving, Shareable, Archivable):
    ...
    def get_json_file_contents_by_snapshot(self, snapshot: Optional[Union[Snapshot | str]] = None) -> Dict[str, Any]:
        """
        Retrieves the contents of all JSON files for a given snapshot.

        If no snapshot ID is provided, the baseline_tagged_snapshot_id will be used.

        param snapshot: The Snapshot object or snapshot ID string to retrieve JSON files from.
        ...
        if snapshot is None:
            ...
            snapshot = self.client.get_snapshot(snapshot_id=snapshot)
        elif not isinstance(snapshot, Snapshot):
            raise TypeError("snapshot must be an instance of Snapshot or a snapshot ID string.")
    ...
    def get_json_file_contents_by_snapshot_tag(self, snapshot_tag: SnapshotTag) -> Dict[str, Any]:
        ...
        return self.get_json_file_contents_by_snapshot(snapshot=snapshot_tag.snapshot_id)
