...
class RepositoryBase(Repository_Base[TEntity, TKey]):
    ...
    def _build_sql_query(
        ...
        Examples:
            ...
            ```python
            # Nested field access with dot notation (simple equality)
            query, params = self._build_sql_query({
                "address.city": "Seattle"
            })
            # Result: "SELECT * FROM c WHERE EXISTS(
            #   SELECT VALUE p FROM p IN c.address WHERE p.city = @param0
            # )"
            ...
            # Result: "SELECT * FROM c WHERE EXISTS(
            #   SELECT VALUE p FROM p IN c.tags WHERE CONTAINS(p.name, @param0)
            # )"
            ...
            # Result: "SELECT * FROM c WHERE EXISTS(
            #   SELECT VALUE p FROM p IN c.products WHERE p.price >= @param0
            # )"
            ...
            # Result: "SELECT * FROM c WHERE EXISTS(
            #   SELECT VALUE p FROM p IN c.friends WHERE p.age IN (@param0_0, @param0_1, @param0_2)
            # )"
            ...
            # Result: "SELECT * FROM c WHERE EXISTS(
            #   SELECT VALUE p FROM p IN c.contacts WHERE STARTSWITH(p.email, @param0)
            # )"
            ...
            # Result: "SELECT * FROM c WHERE EXISTS(
            #   SELECT VALUE p FROM p IN c.items WHERE IS_DEFINED(p.discountPrice)
            # )"
            ```
        ...
        Array Operations with Dot Notation:
            When using dot notation (e.g., "friends.age"), the query builder automatically
            generates EXISTS subqueries to match elements within arrays:

            - "friends.age": 25 → EXISTS(SELECT VALUE p FROM p IN c.friends WHERE p.age = 25)
            - "items.price": {"$gte": 100} → EXISTS(SELECT VALUE p FROM p IN c.items WHERE p.price >= 100)
            - "tags.name": {"$contains": "urgent"} → EXISTS(SELECT VALUE p FROM p IN c.tags WHERE CONTAINS(p.name, "urgent"))
            - "contacts.type": {"$in": ["email", "phone"]} → EXISTS(SELECT VALUE p FROM p IN c.contacts WHERE p.type IN ("email", "phone"))
        ...
        def process_condition(field, condition, is_nested=False):
            ...
            if isinstance(condition, dict):
                for operator, value in condition.items():
                    ...
                    # Check if this is an array/nested field operation (dot notation)
                    is_array_operation = "." in field

                    if is_array_operation:
                        # Split field path for array access
                        field_parts = field.split(".", 1)
                        array_field = field_parts[0]
                        element_field = field_parts[1]

                    if operator == "$contains":
                        if is_array_operation:
                            # Generate EXISTS query for array element matching
                            conditions.append(
                                f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE CONTAINS(p.{element_field}, @{param_name}))"
                            )
                        ...
                        if is_array_operation:
                            ...
                            conditions.append(
                                f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE STARTSWITH(p.{element_field}, @{param_name}))"
                            )
                            parameters[f"@{param_name}"] = value
                        else:
                        ...
                        if is_array_operation:
                            ...
                            conditions.append(
                                f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE ENDSWITH(p.{element_field}, @{param_name}))"
                            )
                            parameters[f"@{param_name}"] = value
                        else:
                        ...
                        if is_array_operation:
                            ...
                            conditions.append(
                                f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE p.{element_field} = @{param_name})"
                            )
                            parameters[f"@{param_name}"] = value
                        else:
                        ...
                        if is_array_operation:
                            ...
                            conditions.append(
                                f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE p.{element_field} IN ({', '.join(param_names)}))"
                            )
                            param_counter += 1
                        else:
                        ...
                        if is_array_operation:
                            ...
                            conditions.append(
                                f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE p.{element_field} NOT IN ({', '.join(param_names)}))"
                            )
                            param_counter += 1
                        else:
                            param_names = []
                        ...
                        if is_array_operation:
                            ...
                            if value:
                                conditions.append(
                                    f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE IS_DEFINED(p.{element_field}))"
                                )
                            else:
                                conditions.append(
                                    f"EXISTS(SELECT VALUE p FROM p IN c.{array_field} WHERE NOT IS_DEFINED(p.{element_field}))"
                                )
                        else:
                            if value:
