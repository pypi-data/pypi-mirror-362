...
class XlsxReportGenerator(ReportGenerator):
    ...
    def open_workbook(self, workbook_name=None):
        ...
        self.wb_format_datetime_format = wb.add_format({'num_format': 'dd/mm/yyyy hh:mm:ss'})
    ...
    def write_header_block(self, sheet) -> tuple[int, int]:
        ...
        if self.is_logo_available():
            ...
            sheet.insert_image(self.logo_cell, self.logo_filename, {'x_scale': 0.5, 'y_scale': 0.5})
            next_row = self.header_logo_y_offset

        sheet.write(next_row, index_col, self.report_name, self.get_title_format())
        next_row += 1
        sheet.write(next_row, index_col, "Auto-generated by Doover", self.get_subtitle_format())
        next_row += 2

        # Write time period details.
        sheet.write(next_row, index_col, "Start : ", self.get_bold_format())
        sheet.write(next_row, index_col + 2, self.localized_period_from.strftime('%d/%m/%y %H:%M:%S'))
        next_row += 1
        sheet.write(next_row, index_col, "End : ", self.get_bold_format())
        sheet.write(next_row, index_col + 2, self.localized_period_to.strftime('%d/%m/%y %H:%M:%S'))
    ...
    def process_message(self, m, series, value_cache):
        ...
        for i, s in enumerate(series):
            try:
                ...
                if temp_obj and 'currentValue' in temp_obj and temp_obj['currentValue'] is not None:
                ...
                if value is not None:
                    ...
                    if s['name'] == "lastPacket" and raw_value in ["Flow", "Calibration"]:
                        return {}
    ...
    def write_data_sheet(self, sheet):
        ...
        col_counter = 1
        for s in self.data_series:
            sheet.write(header_row, index_col + col_counter, s['display_name'], self.get_bold_format())
            col_counter += 1

        data_start_row = header_row + 2
        # Determine the number of rows (assuming all series share the same timestamps).
        num_rows = len(self.data_series[0]["timestamp_strs"]) if self.data_series and self.data_series[0]["timestamp_strs"] else 0

        # Write each row of data.
        for i in range(num_rows):
            # Write the timestamp from the first series.
            sheet.write(data_start_row + i, index_col, self.data_series[0]["timestamp_strs"][i], self.get_datetime_format())
        ...
        data_range = {
            ...
            'data_columns': 1 + len(self.data_series),  # Column 0 for Time plus one per series.
    ...
    def write_summary_sheet(self, sheet, data_range):
        ...
        self.write_extra_summary(sheet, data_range, next_row=header_row + self.summary_chart_height)
    ...
    def get_summary_chart_series(self, data_range):
        ...
        for col in range(1, data_range['data_columns']):
            ...
            series_name_ref = self.get_series_name_reference('Data', col, data_range['header_row'])
