...
class ReportGeneratorDooverDataMixin:
    ...
    def get_data_object(payload_dict, data_object_name):
        ...
        for key in reported_flattened:
            parts = key.split(".")
            if parts and parts[-1] == "name" and reported_flattened[key].lower() == data_object_name.lower():
                flat_object_key = ".".join(parts[:-1])
                break
            elif len(parts) > 2 and parts[-2].lower() == data_object_name.lower():
                flat_object_key = ".".join(parts[:-1])
                break
            elif len(parts) > 3 and parts[-3].lower() == data_object_name.lower() and parts[-2] == "currentValue":
    ...
    def get_available_dataseries(payload_dict):
        ...
        for key, value in reported_flattened.items():
            if ".type" in key and value == "uiVariable":
                ...
                if var_type_key in reported_flattened and reported_flattened[var_type_key] == "time":
        ...
        for series_name in series_names:
            result.append(
                ReportGeneratorDooverDataMixin.get_data_object(payload_dict, series_name)
            )
        return result
    ...
    def retrieve_data(self, period_from, period_to, agent_id):
        ...
        """
        curr_server_time = int(
            self.get_current_server_time(agent_id)
        )
        local_time = int(datetime.utcnow().replace(tzinfo=timezone.utc).timestamp())
        time_diff = curr_server_time - local_time

        start_secs = copy.copy(period_from).replace(tzinfo=self.for_timezone).timestamp() + time_diff
        end_secs = copy.copy(period_to).replace(tzinfo=self.for_timezone).timestamp() + time_diff
        ...
        for day in range(floor(num_days)):
            ...
            msgs.extend(
                self._get_data_for_window(
                    temp_start, temp_end, agent_id
                )
            )
        ...
        if remainder > 0:
            ...
            msgs.extend(
                self._get_data_for_window(
                    end_secs - remainder_secs, end_secs, agent_id
                )
            )
        return msgs