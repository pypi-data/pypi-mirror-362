...
# from ..docker.device_agent.device_agent import DeviceAgentInterface ## Causes circular import error
...
class UIManager:
    def __init__(
            self,
            agent_id: str = None,
            client=None,  # client: Union[Client, DeviceAgentInterface] = None, # Circular import
    ...
    def _add_interaction(self, interaction: Interaction):
        ...
        if name in self._interactions:
            ...
            if (hasattr(self._interactions[name], "_current_value") and
                    self._interactions[name]._current_value is not NotSet):
    ...
    def update_variable(self, variable_name: str, value: Any, critical: bool = False) -> bool:
        ...
        if not (element and isinstance(element, Variable)):
            log.info(f"PyDoover: Tried to update variable '{variable_name}' that doesn't exist.")
            return False
    ...
    def publish_to_channel(self, channel_name: str, data: dict[str, Any], record_log: bool = True,
                           timestamp: Optional[datetime] = None, **kwargs):
        return self._publish_to_channel(channel_name, data, record_log=record_log, timestamp=timestamp, **kwargs)

    def _publish_to_channel(self, channel_name: str, data: dict[str, Any], record_log: bool = True,
                            timestamp: Optional[datetime] = None, **kwargs):
    ...
    async def _publish_to_channel_async(self, channel_name: str, data: dict[str, Any], record_log: bool = True,
                                        timestamp: Optional[datetime] = None, **kwargs):
        if isinstance(self.client, Client):
    ...
    def clear_ui(self):
        ...
        self._publish_to_channel("ui_state", {"state": None}, record_log=False)
