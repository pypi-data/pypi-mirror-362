...
class GRPCInterface:
    ...
    def __init__(self, uri: str, is_async: bool = False, timeout: int = 7):
        self.uri = uri
    ...
    def process_response(self, stub_call: str, response, *args, **kwargs):
        if response is None:
            logging.warning(f"Error processing response for {self.__class__.__name__}.{stub_call}: response was None")
            return None
        ...
        if response.response_header.success is False:
            ...
            message = getattr(response.response_header, "message", None) or getattr(response.response_header, "response_message", None)
            log.warning(
