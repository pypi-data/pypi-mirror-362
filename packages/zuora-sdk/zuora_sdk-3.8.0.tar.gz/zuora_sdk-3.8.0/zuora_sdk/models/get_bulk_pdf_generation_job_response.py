...
class GetBulkPdfGenerationJobResponse(BaseModel):
    ...
    failed_documents: Optional[List[DocumentIdList]] = Field(default=None, description="Array of Objects where each object contains billing document type and their ids which failed to execute", alias="failedDocuments")
    skipped_documents: Optional[List[DocumentIdList]] = Field(default=None, description="Array of Objects where each object contains billing document type and their ids which failed to execute", alias="skippedDocuments")
    ...
    __properties: ClassVar[List[str]] = ["jobId", "jobName", "status", "stepStatus", "fileUrls", "failedDocuments", "skippedDocuments", "createdOn", "createdBy", "success"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in failed_documents (list)
        _items = []
        if self.failed_documents:
            for _item_failed_documents in self.failed_documents:
                if _item_failed_documents:
                    _items.append(_item_failed_documents.to_dict())
            _dict['failedDocuments'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in skipped_documents (list)
        _items = []
        if self.skipped_documents:
            for _item_skipped_documents in self.skipped_documents:
                if _item_skipped_documents:
                    _items.append(_item_skipped_documents.to_dict())
            _dict['skippedDocuments'] = _items
    ...
    def to_alias_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "failedDocuments": [DocumentIdList.from_dict(_item) for _item in obj["failedDocuments"]] if obj.get("failedDocuments") is not None else None,
            "skippedDocuments": [DocumentIdList.from_dict(_item) for _item in obj["skippedDocuments"]] if obj.get("skippedDocuments") is not None else None,
