...
class CreateCCReferenceTransactionPaymentMethod(BaseModel):
    """
    ...
    token_id: Annotated[str, Field(min_length=1, strict=True, max_length=255)] = Field(description="A gateway unique identifier that replaces sensitive payment method data or represents a gateway's unique customer profile. `tokenId` is required for the CC Reference Transaction payment method.   When `tokenId` is used to represent a customer profile, `secondTokenId` is conditionally required for representing the underlying tokenized payment method.   The values for the `tokenId` and `secondTokenId` fields differ for gateways. For more information, see the Knowledge Center article specific to each gateway that supports the CC Reference Transaction payment method.   **Note:** When creating an ACH payment method, if you need to pass in tokenized information, use the `mandateId` instead of `tokenId` field.", alias="tokenId")
    ...
    __properties: ClassVar[List[str]] = ["accountKey", "paymentMethodNumber", "paymentGatewayNumber", "screeningAmount", "authGateway", "gatewayOptions", "ipAddress", "makeDefault", "processingOptions", "skipValidation", "type", "currencyCode", "tokenize", "tokens", "cardHolderInfo", "cardNumber", "creditCardMaskNumber", "expirationMonth", "expirationYear", "mandateInfo", "secondTokenId", "tokenId", "cardType"]
