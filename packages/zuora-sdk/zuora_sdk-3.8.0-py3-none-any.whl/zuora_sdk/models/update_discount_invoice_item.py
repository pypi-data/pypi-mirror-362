...
class UpdateDiscountInvoiceItem(BaseModel):
    ...
    booking_reference: Optional[StrictStr] = Field(default=None, description="The booking reference of the discount item. **Note**: This field is only available only if id of parent invoice item is null and you have Taxation enabled. ", alias="bookingReference")
    ...
    tax_items: Optional[List[CreateTaxationItem]] = Field(default=None, description="Container for taxation items. The maximum number of taxation items is 5. **Note**: This field is only available only if id of parent invoice item is null and you have Taxation enabled. ", alias="taxItems")
