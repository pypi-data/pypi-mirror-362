...
class SpinConv(Model):
    ...
    """
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, stride: int = 1, padding=None, stimuli=nn.Hardswish, pool_kernel_size: int = 2, pool_padding: int = 0):
        if padding is None:
            padding = kernel_size // 2
...
if __name__ == '__main__':
    ...
    x = torch.randn(sample_num, 3, 32, 32)
    ...
    class SimTrainProcess(TrainningProcess):
        ...
        def init_model(self, record: dict = None):
            model = Model(
                SpinConv(3, 3, kernel_size=3, stride=2, stimuli=nn.Hardswish, pool_kernel_size=2),
                manipulators.Flatten(1),
                FullConnectedLayer(3 * 8 * 8, 5, stimuli=nn.Hardswish()), 
            )
    ...
    training_process = SimTrainProcess(sfs.path_of('spin_conv').ensure(), device='cuda:0', val_index=0)
