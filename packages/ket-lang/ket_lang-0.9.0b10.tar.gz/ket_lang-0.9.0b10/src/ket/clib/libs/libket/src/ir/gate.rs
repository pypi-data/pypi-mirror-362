...
impl QuantumGate {
    ...
    pub(crate) fn decomposition_list(&self, control_size: usize) -> Vec<Algorithm> {
        if std::env::var("KET_FORCE_DECOMPOSE_ALGORITHM").unwrap_or("0".to_string()) == "1" {
                ...
                .as_str()
            {
                ...
                other => panic!("undefined decomposition algorithm: {}. Are you sure that you want to use this configuration?", other),
            };
            return match self {
    ...
    fn decompose_x(
        ...
        match &schema.algorithm {
            ...
            _ => panic!("Invalid Decomposition {:?} for Pauli Gate", schema),
        }
    }
