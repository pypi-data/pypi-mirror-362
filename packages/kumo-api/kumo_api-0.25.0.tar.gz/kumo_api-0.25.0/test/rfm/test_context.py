...
from kumoapi.rfm.context import Subgraph, Table
...
def test_zero_column() -> None:
    context = Context(
        task_type=TaskType.BINARY_CLASSIFICATION,
        entity_table_names=('USERS', ),
        subgraph=Subgraph(
            anchor_time=np.array([0, 0]),
            table_dict={
                'USERS':
                Table(
        ...
        y_train=pd.Series([False, True]),
        y_test=None,
    )

    out = Context.from_bytes(context.serialize())
    pd.testing.assert_frame_equal(
        context.subgraph.table_dict['USERS'].df,
        out.subgraph.table_dict['USERS'].df,
    )
    print(out)
