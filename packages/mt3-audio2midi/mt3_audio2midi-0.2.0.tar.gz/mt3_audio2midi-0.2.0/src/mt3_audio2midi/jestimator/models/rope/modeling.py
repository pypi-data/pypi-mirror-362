...
"""Rope encoder implemented in Flax."""
import dataclasses
import math
...
from mt3_audio2midi.jestimator.modeling import global_kwargs, sparse_xe_with_logits, normalize_loss_by_size, truncated_normal_initializer, Dropout  # pylint: disable=g-multiple-import
...
class EncoderBlock(nn.Module):
  ...
  def __call__(
    ...
    rotaries = embedding.generate_fixed_pos_embedding(self.dim, self.max_length)
    for i, layer in enumerate(self.layers):
      att_tgt = None if attention_targets is None else attention_targets[i]
      x = layer(x, attention_target=att_tgt, rotaries=rotaries)
      if return_all_hidden:
        all_hidden += (x,)
...
class AttentionLayer(nn.Module):
  ...
  def __call__(
    ...
    if use_rotary and rotaries is not None:
      ...
      sin, cos = rotaries
      query, key = embedding.apply_rotary_embedding(query, key, cos, sin)
