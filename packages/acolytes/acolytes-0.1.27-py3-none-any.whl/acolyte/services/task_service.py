...
class TaskService:
    ...
    async def get_task_full_context(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.db.execute_async(
                """
                    SELECT c.*, c.timestamp as created_at FROM sessions c
                    JOIN task_sessions ts ON c.session_id = ts.session_id
                    WHERE ts.task_id = ?
    ...
    async def find_active_task(self, user_id: str = "default") -> Optional[TaskCheckpoint]:
        ...
        try:
            ...
            logger.info("[TRACE] Created TaskCheckpoint from DB row")
            ...
            logger.info("[TRACE] Exception in find_active_task")
            return None
    ...
    def _build_timeline(self, sessions: List[Dict], decisions: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for session in sessions:
            timeline.append(
                {
                    "type": "session",
                    "timestamp": session.get("created_at", session.get("timestamp")),
                    "title": f"Session: {session['id'][:8]}",
                    "summary": session.get("content", "")[:100],
                }
            )
    ...
    def _extract_key_files(self, sessions: List[Dict]) -> List[str]:
        ...
        for session in sessions:
            summary = session.get("content", "")  # El campo real es content, no summary
