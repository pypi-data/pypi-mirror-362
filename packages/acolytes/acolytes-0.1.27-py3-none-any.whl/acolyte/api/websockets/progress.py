        ...
        if isinstance(event, ProgressEvent) and hasattr(event, 'task_id'):
            ...
            if event.task_id == task_id:
                ...
                try:
                    ...
                    try:
                        event_queue.get_nowait()  # Descartar el más viejo
                        event_queue.put_nowait(event)  # Agregar el nuevo
                        logger.warning("Event queue full, discarded oldest event", task_id=task_id)
                    except Exception as e:
                        logger.warning(
                            "Event queue full and could not discard old events",
                            task_id=task_id,
                            error=str(e),
                        )
