...
-- Main sessions table (renamed from conversations for clarity)
...
-- CREATE INDEX idx_session_id ON conversations(session_id); -- Removed: redundant, session_id is already PRIMARY KEY
CREATE INDEX IF NOT EXISTS idx_timestamp ON sessions(timestamp);
CREATE INDEX IF NOT EXISTS idx_role ON sessions(role);
CREATE INDEX IF NOT EXISTS idx_task ON sessions(task_id);
...
LEFT JOIN sessions c ON ts.session_id = c.session_id
GROUP BY t.id;
...
CREATE INDEX IF NOT EXISTS idx_technical_decisions_type ON technical_decisions(decision_type);
CREATE INDEX IF NOT EXISTS idx_technical_decisions_task ON technical_decisions(task_id);
CREATE INDEX IF NOT EXISTS idx_technical_decisions_impact ON technical_decisions(impact_level);
...
CREATE INDEX IF NOT EXISTS idx_graph_edges_type ON code_graph_edges(relation_type);
CREATE INDEX IF NOT EXISTS idx_graph_edges_strength ON code_graph_edges(strength);
...
-- ==============================================================
-- JOB STATES - Unified table for long-running processes
-- ==============================================================
-- For indexing, reindexing, dream analysis, and future jobs
-- Replaces the scattered checkpointing in runtime_state
...
CREATE TABLE IF NOT EXISTS job_states (
    ...
    current_item TEXT,  -- Current file/step being processed
    metadata TEXT DEFAULT '{}',  -- JSON with job-specific details
    error_message TEXT,
...
CREATE TRIGGER IF NOT EXISTS update_job_states_timestamp
AFTER UPDATE ON job_states
BEGIN
    UPDATE job_states SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;

-- Table for system versions
...
-- Changes made:
-- 1. sessions table (renamed from conversations) with session_id as PRIMARY KEY
-- 2. IDs use lower(hex(...)) for Python compatibility
-- 3. Added optional fields in sessions: task_id, related_sessions, total_tokens
...
-- 8. NEW: job_states table for unified job/process tracking (indexing, reindexing, dream)
-- 9. NEW: system_versions table for system version tracking
