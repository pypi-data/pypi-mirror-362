...
from acolyte.core.utils.datetime_utils import utc_now, parse_iso_datetime, ensure_utc
        ...
        for result in results:
            ...
            if chunk.metadata.last_modified:
                ...
                if isinstance(chunk.metadata.last_modified, str):
                    ...
                    # Use centralized ensure_utc to handle timezone
                    modified_dt = ensure_utc(chunk.metadata.last_modified)
                    if chunk.metadata.last_modified.tzinfo is None:
                        logger.info("[TRACE] Datetime without timezone in rerank")
