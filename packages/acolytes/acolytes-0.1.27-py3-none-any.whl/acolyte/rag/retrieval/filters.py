...
from acolyte.core.utils.datetime_utils import parse_iso_datetime, ensure_utc, format_iso
...
class SearchFilters:
    ...
    def filter_by_date(
        ...
        # Ensure start and end dates are in UTC if provided
        if start:
            ...
            if chunk.metadata.last_modified:
                ...
                if isinstance(chunk.metadata.last_modified, str):
                    try:
                        ...
                        logger.info("[TRACE] Date parsing failed")
                        continue
                    if chunk_date is None:
                        # Skip this chunk if parsing returns None
                        logger.info("[TRACE] parse_iso_datetime returned None")
                        continue
                elif hasattr(chunk.metadata.last_modified, 'tzinfo'):
                    # It's a datetime object, ensure it's in UTC
                    chunk_date = ensure_utc(chunk.metadata.last_modified)
                else:
                    # Skip this chunk if we can't parse the date
                    logger.info("[TRACE] Unknown date type for last_modified")
                    continue
    ...
    def to_weaviate_where(self, filters: Dict[str, Any]) -> Optional[Dict]:
        ...
        if 'chunk_types' in filters and filters['chunk_types']:
            ...
            if len(chunk_type_operands) == 1:
                ...
                logger.info("[TRACE] Multiple chunk types in filter")
            ...
            operands.append(
                {
                    ...
                    "valueDate": (
                        format_iso(filters['date_from'])
                        if hasattr(filters['date_from'], 'tzinfo')
            ...
            operands.append(
                {
                    ...
                    "valueDate": (
                        format_iso(filters['date_to'])
                        if hasattr(filters['date_to'], 'tzinfo')
