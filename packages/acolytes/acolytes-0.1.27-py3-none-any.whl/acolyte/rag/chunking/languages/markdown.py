...
class MarkdownChunker(BaseChunker):
    ...
    def _create_chunk_from_node(
        ...
        if node.type == 'fenced_code_block':
            logger.info("[TRACE] markdown fenced_code_block")
        ...
        if not chunk:
            logger.info("[TRACE] markdown no chunk created")
            return None
    ...
    def _create_section_chunk(self, section_node, lines, file_path, processed_ranges):
        ...
        if any(
            start_line <= s <= end_line for ranges in processed_ranges.values() for (s, e) in ranges
        ):
            return None
        ...
        # Mark as processed
        if 'section' not in processed_ranges:
            processed_ranges['section'] = set()
        processed_ranges['section'].add((start_line, end_line))
        ...
        if heading_node.type == 'atx_heading':
            ...
            for child in heading_node.children:
                if child.type == 'atx_h1_marker':
                    ...
                    logger.info("[TRACE] markdown heading content extraction")
            ...
            for child in heading_node.children:
                if child.type == 'setext_h1_underline':
                    ...
                    logger.info("[TRACE] markdown setext h2 underline")
                    level = 2
            ...
            if child.type == 'info_string':
                ...
                if info:
                    ...
                    if 'executable' in info or 'runnable' in info:
                        logger.info("[TRACE] markdown executable code block")
                        metadata['is_executable'] = True
                    if 'output' in info or 'result' in info:
                        logger.info("[TRACE] markdown output code block")
