...
class ChunkerFactory:
    ...
    def create(cls, file_path: str, content: Optional[str] = None) -> BaseChunker:
        ...
        if content and '\x00' in content[:1000]:
            raise ValueError("Binary file not supported: " + file_path)

        language = cls.detect_language(file_path, content)
        logger.debug("Detected language", language=language, file_path=file_path)
        ...
        if chunker:
            logger.info(
                "Using chunker", chunker_class=chunker.__class__.__name__, file_path=file_path
            )
            return chunker
        else:
            # Fallback to DefaultChunker for unknown languages
            logger.info("Using DefaultChunker (fallback)", file_path=file_path)
    ...
    def _get_language_chunker(cls, language: str) -> Optional[BaseChunker]:
        ...
        try:
            ...
            if not issubclass(chunker_class, BaseChunker):
                logger.error("Not a BaseChunker subclass", class_name=class_name)
                return None
            ...
            for method in required_methods:
                if not hasattr(chunker, method) or not callable(getattr(chunker, method)):
                    logger.error("Missing required method", class_name=class_name, method=method)
                    return None
            ...
            if 'Set' in str(e) and language == 'python':
                ...
                logger.info("[TRACE] Python chunker typing issue fallback")
