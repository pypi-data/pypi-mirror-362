...
def warn_if_gpu_missing(expected_gpu: bool, detected_gpu: bool):
    """Warn the user if a GPU is expected but not detected."""
    if expected_gpu and not detected_gpu:
        logger.warning("GPU not detected, falling back to CPU mode.")
        logger.info("This will significantly impact model performance.")
