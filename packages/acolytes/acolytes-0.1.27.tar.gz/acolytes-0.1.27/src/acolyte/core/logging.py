...
class AsyncLogger:
    ...
    def _ensure_handler(self):
        ...
        if log_path != self._current_log_path or self._handler_id is None:
            ...
            loguru_logger.add(
                sys.stderr,
                format="<green>{time:HH:mm:ss}</green> | <level>{level: <8}</level> | <cyan>{name}</cyan>:<cyan>{function: <20}</cyan> | <level>{message}</level>",
                level="DEBUG" if self.debug_mode else "INFO",
                colorize=True,
            )

            # Add file handler with our format (without colors)
            self._handler_id = loguru_logger.add(
                str(log_path),  # Convert to string for cross-platform compatibility
                format="{time:HH:mm:ss} | {level: <8} | {name}:{function} | {message}",
                rotation="10 MB",
                compression="zip",
                enqueue=True,  # Key for async
