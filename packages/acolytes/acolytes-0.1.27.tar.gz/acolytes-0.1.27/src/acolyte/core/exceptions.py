...
class VersionIncompatibilityError(AcolyteError):
    """
    Error cuando las versiones almacenadas no coinciden con las actuales.

    Se usa cuando los modelos de chunking, embeddings o esquemas de Weaviate
    han cambiado y requieren migración o re-indexación.
    """

    def __init__(self, stored_versions: Dict[str, str], current_versions: Dict[str, str]) -> None:
        message = (
            f"Version incompatibility detected. "
            f"Stored: {stored_versions}, Current: {current_versions}"
        )
        super().__init__(
            message=message,
            code="VersionIncompatibilityError",
            context={
                "stored_versions": stored_versions,
                "current_versions": current_versions,
                "differences": {
                    k: {"stored": stored_versions.get(k), "current": current_versions.get(k)}
                    for k in set(stored_versions) | set(current_versions)
                    if stored_versions.get(k) != current_versions.get(k)
                },
            },
        )

        # Add helpful suggestions
        self.add_suggestion("Run 'acolyte migrate' to update the database schema")
        self.add_suggestion("Or run 'acolyte index --full' to reindex with new versions")
        self.add_suggestion("Check the migration guide for breaking changes")
...
def from_exception(exc: AcolyteError) -> ErrorResponse:
    ...
    error_type_map = {
        ...
        "VersionIncompatibilityError": ErrorType.CONFIGURATION,  # Version mismatch = config issue
    }
