...
class TypeScriptChunker(BaseChunker, SecurityAnalysisMixin, PatternDetectionMixin):
    ...
    def _get_tree_sitter_language(self) -> Any:
        ...
        # Default to TypeScript, will be overridden in chunk()
    ...
    async def chunk(self, content: str, file_path: str) -> List[Chunk]:
        ...
        # Check file extension and set up appropriate parser
        if ext == 'tsx':
            ...
            self._language = Language(tsjs.language())  # JSX uses JS parser
        else:
        ...
        # Initialize parser with selected language
        ...
        if '<' in text and '>' in text:
            ...
            if start < end:
                ...
                if generics_text and not any(op in generics_text for op in ['=', '(', ')']):
                    logger.info("[TRACE] typescript generics extraction")
    ...
    def _check_eval_usage(self, node, text: str) -> Optional[Dict[str, Any]]:
    ...
    def _check_unsafe_regex(self, node, text: str) -> Optional[Dict[str, Any]]:
        """Check for unsafe regex patterns."""
        if 'new RegExp' in text and 'escape' not in text:
            logger.info("[TRACE] typescript unsafe regex detected")
            return {
                'type': 'unsafe_regex',
                'severity': 'medium',
    ...
    def _check_innerHTML(self, node, text: str) -> Optional[Dict[str, Any]]:
            ...
            if 'require(' in text:
                ...
                for req in requires:
                    if not req.startswith('.'):
                        logger.info("[TRACE] typescript CommonJS require dependency")
