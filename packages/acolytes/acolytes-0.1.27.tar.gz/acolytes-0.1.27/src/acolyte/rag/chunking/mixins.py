...
class ComplexityMixin:
    ...
    def _analyze_complexity_recursive(
        ...
        if depth > max_depth:
            logger.info("[TRACE] Max recursion depth reached in complexity analysis")
            return
...
class TodoExtractionMixin:
    ...
    def _extract_todos(self, node) -> List[Dict[str, Any]]:
        ...
        """
        todos = []
        self._find_todos_recursive(node, todos, depth=0)
        return todos

    def _find_todos_recursive(
        self, node, todos: List[Dict[str, Any]], depth: int = 0, max_depth: int = 1000
    ):
        """Recursively find TODO comments in AST with depth protection."""
        ...
        for child in node.children:
            self._find_todos_recursive(child, todos, depth + 1, max_depth)
...
class SecurityAnalysisMixin:
    ...
    def _detect_security_issues(self, node) -> List[Dict[str, Any]]:
        ...
        """
        issues = []
        self._analyze_security_recursive(node, issues, depth=0)
        return issues

    def _analyze_security_recursive(
        self, node, issues: List[Dict[str, Any]], depth: int = 0, max_depth: int = 1000
    ):
        """Recursively analyze nodes for security issues with depth protection."""
        # Prevent infinite recursion
        if depth > max_depth:
            return
        ...
        for child in node.children:
            self._analyze_security_recursive(child, issues, depth + 1, max_depth)
    ...
    def _detect_patterns(
        ...
        if not metadata:
            if hasattr(self, '_calculate_complexity'):
                ...
                logger.info("[TRACE] No _calculate_complexity method, using defaults")
...
class DependencyAnalysisMixin:
    ...
    def _is_internal_dependency(self, dep: str) -> bool:
        ...
        if hasattr(self, 'config'):
            ...
            logger.info("[TRACE] No config available, using default project name")
