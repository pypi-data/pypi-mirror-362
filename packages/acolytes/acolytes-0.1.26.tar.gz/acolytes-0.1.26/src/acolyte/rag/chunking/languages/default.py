...
class DefaultChunker(BaseChunker):
    ...
    def __init__(self, language: str = 'unknown'):
        ...
        self._parser = None
        # Keep the language - don't override with None
    ...
    def _get_tree_sitter_language(self) -> Any:
        ...
        logger.info("[TRACE] default._get_tree_sitter_language called")
        return None
    ...
    def _get_import_node_types(self) -> List[str]:
        ...
        logger.info("[TRACE] default._get_import_node_types called")
        return []
    ...
    async def chunk(self, content: str, file_path: str) -> List[Chunk]:
        ...
        logger.info("Using enhanced default chunker", file_path=file_path, language=self._language)
        ...
        if last_chunk_end < len(lines):
            logger.info("[TRACE] default chunking remaining content")
    ...
    def _extract_structural_chunks(
        ...
        while i < len(lines):
            ...
            if func_match:
                logger.info("[TRACE] default function pattern matched")
                ...
                for j in range(i - 1, -1, -1):
                    if lines[j].strip():
                        if self._is_decorator_or_comment(lines[j].strip()):
                            ...
                            logger.info("[TRACE] default decorator found above")
                        break
                if not found_decorator_above:
                    logger.info("[TRACE] default no decorator above")
                    break
                continue

            # Include comments and decorators
            if self._is_decorator_or_comment(line):
                logger.info("[TRACE] default decorator/comment included")
                start_idx = i
            else:
                break
    ...
    def _detect_patterns_heuristic(self, content: str) -> Dict[str, List[str]]:
        ...
        # Anti-patterns
        if loc > 100:
            logger.info("[TRACE] default long block anti-pattern")
        ...
        if max_indent > 20:  # 5 levels with 4-space indent
            logger.info("[TRACE] default deep nesting anti-pattern")
    ...
    def _chunk_remaining_content(
        ...
        while lines and not lines[0].strip():
            logger.info("[TRACE] default removing empty line at start")
            lines.pop(0)
            start_offset += 1
        while lines and not lines[-1].strip():
            logger.info("[TRACE] default removing empty line at end")
            lines.pop()
    ...
    def _guess_chunk_type(self, content: str) -> ChunkType:
        ...
        # Check for function patterns first
        for line in content.split('\n'):
            line = line.strip()
            if line:
                # Check function patterns
                for pattern, _ in self.FUNCTION_PATTERNS:
                    if re.match(pattern, line):
                        # Distinguish between method (indented) and function
                        if self._get_indentation(line) > 0:
                            return ChunkType.METHOD
                        else:
                            return ChunkType.FUNCTION

                # Check class patterns
                for pattern, _ in self.CLASS_PATTERNS:
                    if re.match(pattern, line):
                        return ChunkType.CLASS  # Check for test patterns
    ...
    def _create_chunk(
        ...
        if start_line < 1:
            ...
            logger.info("[TRACE] default invalid start_line")
            start_line = 1
        if end_line < start_line:
            logger.warning(
                "Invalid end_line < start_line, adjusting", end_line=end_line, start_line=start_line
            )
            logger.info("[TRACE] default invalid end_line")
            end_line = start_line
    ...
    def _is_comment_node(self, node: Any) -> bool:
        ...
        logger.info("[TRACE] default._is_comment_node called")
        return False
