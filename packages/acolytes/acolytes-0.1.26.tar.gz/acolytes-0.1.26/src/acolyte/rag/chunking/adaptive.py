...
class AdaptiveChunker:
    ...
    async def chunk(self, content: str, file_path: str) -> List[Chunk]:
        ...
        if not hasattr(self._current_chunker, 'parser') or self._current_chunker.parser is None:
            logger.warning(f"No tree-sitter parser available for {language}, using default chunker")
            from .languages.default import DefaultChunker

            self._current_chunker = DefaultChunker()
            return await self._current_chunker.chunk(content, file_path)

        # CRITICAL FIX: Add timeout to tree-sitter parsing to prevent infinite hangs on problematic files
        try:
            loop = asyncio.get_running_loop()
            tree = await asyncio.wait_for(
                loop.run_in_executor(
                    None,
                    lambda: self._current_chunker.parser.parse(bytes(content, 'utf8')),  # type: ignore
                ),
                timeout=15.0,  # 15 seconds max for tree-sitter parsing
            )
        except asyncio.TimeoutError:
            logger.warning(f"Tree-sitter parsing timeout for {file_path}, using default chunker")
            from .languages.default import DefaultChunker
