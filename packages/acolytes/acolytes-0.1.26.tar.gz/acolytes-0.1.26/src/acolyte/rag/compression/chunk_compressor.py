...
class ContextualCompressor:
    ...
    def compress_chunks(
        ...
        if not chunks:
            ...
            logger.info("[TRACE] Empty chunks list in compress_chunks")
        ...
        if token_budget <= 0:
            logger.error("Invalid token_budget", token_budget=token_budget)
            logger.info("[TRACE] Invalid token_budget in compress_chunks")
            raise ValueError("token_budget must be positive, got " + str(token_budget))
        ...
        for relevance, chunk in scored_chunks:
            ...
            if elapsed_ms > self.early_stop_ms:
                logger.warning(
                    ...
                    processed=len(compressed_chunks),
                    total=len(chunks),
                )
                break
            ...
            if current_tokens + compressed_tokens <= token_budget:
                ...
                if relevance > self.relevance_thresholds["recompress"]:
                    ...
                    if current_tokens + aggressive_tokens <= token_budget:
                        ...
                        logger.debug(
                            "Chunk skipped: won't fit",
                            relevance=round(relevance, 2),
                            tokens_needed=aggressive_tokens,
                        )
                        logger.info("[TRACE] Chunk won't fit even with aggressive compression")
                else:
                    # Low relevance, skip
                    logger.debug(
                        "Chunk skipped: low relevance score", relevance=round(relevance, 2)
                    )
                    logger.info("[TRACE] Chunk skipped due to low relevance")
        ...
        logger.info(
            ...
            compressed_tokens=current_tokens,
            tokens_saved=tokens_saved,
            savings_percentage=round((1 - compression_ratio) * 100, 1),
    ...
    def _compress_chunk(self, chunk: Chunk, relevance: float, target_ratio: float) -> Chunk:
        ...
        try:
            ...
            logger.error("Failed to get compression strategy", error=str(e))
            logger.info("[TRACE] Failed to get compression strategy")
        ...
        try:
            ...
            logger.error(
                "Compression failed for chunk",
                chunk_id=chunk.id,
                error=str(e),
                action="Using original content",
            )
            logger.info("[TRACE] Compression strategy failed")
            return chunk
...
class CompressionMetrics:
    ...
    def log_summary(self) -> None:
        ...
        logger.info(
            "CompressionMetrics summary",
            total_compressions=summary['total_compressions'],
            tokens_saved=summary['total_tokens_saved'],
            avg_time_ms=round(summary['average_time_ms'], 1),
            p95_ms=round(summary['p95_time_ms'], 1),
            p99_ms=round(summary['p99_time_ms'], 1),
            avg_savings_pct=round(summary['average_savings_percentage'], 1),
        )

        # Log métricas base de Core (usando composición)
        base_metrics = self.collector.get_metrics()
        if base_metrics:
            logger.debug("Core base metrics", metrics=base_metrics)
