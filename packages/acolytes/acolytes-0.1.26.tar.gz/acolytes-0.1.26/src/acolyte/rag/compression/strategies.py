            ...
            for line in lines[:10]:  # Check first 10 lines
                ...
                if stripped and not stripped.startswith(("#", "//", "/*")):
                    ...
                    if len(result_lines) >= 3:
                        break
            if result_lines:
                logger.info("[TRACE] No function matches found, returning first lines")
                return "\n".join(result_lines) + "\n# ..."
    ...
    def _extract_relevant_section(self, content: str) -> str:
        ...
        for i, header_match in enumerate(headers):
            ...
            if "Section" in header_text or "section" in header_text:
                ...
                logger.info("[TRACE] Found relevant section header")
        ...
        if relevant_sections:
            ...
            if len(result) > self.section_preview_chars:
                ...
                logger.info("[TRACE] Truncating relevant sections to preview limit")
            return result
...
class ConfigCompressionStrategy(CompressionStrategy):
    ...
    def _extract_relevant_section(self, content: str) -> str:
        ...
        longest = max(sections, key=len)
        if len(longest) > 1000:
            logger.info("[TRACE] Truncating long config section")
...
class DataCompressionStrategy(CompressionStrategy):
    ...
    def _extract_schema(self, content: str) -> str:
        ...
        if create_stmts:
            logger.info("[TRACE] Found CREATE TABLE statements in data file")
    ...
    def _extract_metadata(self, content: str) -> str:
        ...
        if lines and "," in lines[0]:
            ...
            logger.info("[TRACE] Detected SQL format in metadata extraction")
        else:
    ...
    def compress(self, content: str, relevance_score: float, target_ratio: float = 0.5) -> str:
        ...
        if relevance_score > 0.7:
            ...
            if len(lines) > self.max_lines_preview:
                ...
                logger.info("[TRACE] Applying conservative compression for unknown file type")
                return "\n".join(result)
            return content
