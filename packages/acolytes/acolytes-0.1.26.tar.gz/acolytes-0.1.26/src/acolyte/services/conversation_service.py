...
class ConversationService:
    ...
    async def create_session(self, initial_message: str) -> str:
        ...
        - NO requiere task_id (puede ser None)
        ...
        try:
            ...
            # Crear entrada inicial de sesión en sessions
            # Usamos la tabla sessions adaptada para resúmenes de sesión
    ...
    async def save_conversation_turn(
        ...
        Args:
            ...
            task_id: ID de tarea asociada (opcional)
        ...
        try:
            ...
            async def db_operation_with_counter() -> Any:
                ...
                attempt_count += 1
                return await self.db.execute_async(
                    "SELECT total_tokens, content FROM sessions WHERE session_id = ? AND role = 'system'",
                    (session_id,),
                    FetchType.ONE,
                )
            ...
            # Solo actualizamos el resumen acumulado en sessions
    ...
    async def get_session_context(
        ...
        try:
            ...
            result = await self.db.execute_async(
                "SELECT * FROM sessions WHERE session_id = ? AND role = 'system'",
                (session_id,),
                FetchType.ONE,
            )
            ...
            # Los resúmenes están en el campo content de sessions
    ...
    async def get_last_session(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(result.data, list) and result.data:
                logger.warning("[TRACE] get_last_session returned list instead of dict")
    ...
    async def complete_session(self, session_id: str) -> None:
        ...
        try:
            ...
            logger.warning("[TRACE] complete_session error handling")
    ...
    async def _search_by_keywords(
        ...
        try:
            ...
            if not keywords:
                logger.warning("[TRACE] No keywords extracted from query")
                return []
            ...
            if task_id:
                where_clause = f"{where_clause} AND task_id = ?"
                params.append(task_id)

            query_sql = f"""
                SELECT session_id as id, content as summary, timestamp as created_at, task_id
                FROM sessions
                WHERE {where_clause}
                ORDER BY timestamp DESC
                LIMIT ?
            ...
            logger.warning("[TRACE] _search_by_keywords error handling")
    ...
    async def _search_by_keywords_typed(
        ...
        try:
            ...
            logger.warning("[TRACE] _search_by_keywords_typed error handling")
    ...
    async def _handle_cache_invalidation(self, event: Event) -> None:
        ...
        if hasattr(self, "_session_cache") and self._session_cache is not None:
            logger.warning("[TRACE] _session_cache clearing")
