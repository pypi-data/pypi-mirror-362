...
class UniXcoderEmbeddings:
    ...
    def __init__(self, metrics: Optional[MetricsProvider] = None):
        ...
        # Lock to prevent concurrent model loading from multiple workers
        ...
        logger.info("UniXcoderEmbeddings initialized with lazy loading")
        ...
        # Device state will be loaded lazily when needed
        self._device_str = None  # Store device string for lazy loading

        # Load device state (async, non-blocking)
    ...
    def _load_device_state(self):
        ...
        Improved version that handles event loops better.
        """
        logger.debug("Starting _load_device_state()")

        async def load():
            try:
                logger.debug("Attempting to load device state from runtime_state")
                device_str = await self._runtime_state.get("embeddings.device")
                if device_str and device_str in ["cuda", "cpu"]:
                    ...
                    logger.debug("No device state found in runtime_state")
            except Exception as e:
                # Not critical if it fails
                logger.debug(
                    "Could not load device state", error=str(e), error_type=type(e).__name__
                )

        # Better handling of event loops
        try:
            loop = asyncio.get_running_loop()
            logger.debug("Found running event loop, creating task")
            # If there's a loop, create a task but don't block
            task = loop.create_task(load())
            # Store task reference to avoid garbage collection
            self._device_load_task = task
        except RuntimeError as e:
            # No running loop, that's OK - will load lazily when needed
            logger.debug("No running event loop for device state load", error=str(e))
            self._device_str = None
            self._device_load_task = None  # Mark as attempted to avoid repeated calls
    ...
    async def _save_device_state(self, device_str: str):
        ...
        """
        try:
            success = await self._runtime_state.set("embeddings.device", device_str)
            if success:
                logger.info("Device state saved in runtime_state", device=device_str)
            else:
                logger.warning("Failed to save device state", device=device_str)
        except Exception as e:
            # Not critical if it fails
        ...
        if not hasattr(self, '_torch'):
            try:
                ...
                error = ExternalServiceError(
                    "PyTorch is required for embeddings",
    ...
    def transformers(self):
        ...
        if not hasattr(self, '_transformers'):
            try:
                ...
                error = ExternalServiceError(
                    "Transformers is required for embeddings",
    ...
    def device(self) -> 'torch.device':
        ...
        if self._device is None:
            with self._device_lock:
                ...
                if self._device is None:
                    # Lazy load device state if not done yet
                    if self._device_str is None and not hasattr(self, '_device_load_task'):
                        self._load_device_state()

                    # Get torch via property (lazy loads if needed)
                    torch_module = self.torch

                    device_config = self.config.get("embeddings.device", "auto")

                    # Use loaded device state if available and matches config
                    if self._device_str and device_config == "auto":
                        self._device = torch_module.device(self._device_str)
    ...
    def _load_model(self):
        ...
        with self._model_lock:
            # Double-check: another thread might have loaded while waiting for lock
            if self._model is not None:
                return
            ...
            download_timeout = self.config.get(
                "embeddings.download_timeout", 300
            )  # 5 minutes default
            logger.info("Using download timeout", timeout=download_timeout)

            # Check CUDA availability before attempting to load
            torch_module = self.torch  # Lazy load torch
            if self.device.type == "cuda" and not torch_module.cuda.is_available():
                logger.warning("CUDA requested but not available, changing to CPU")
                self._device = torch_module.device("cpu")
                # Persist the change for future restarts
                try:
                    asyncio.create_task(self._save_device_state("cpu"))
                except RuntimeError:
                    # If no active event loop, try to save state properly
                    try:
                        # Check if there's already a running event loop
                        try:
                            loop = asyncio.get_running_loop()
                            # If we're already in an event loop, use it
                            asyncio.run_coroutine_threadsafe(self._save_device_state("cpu"), loop)
                        except RuntimeError:
                            # No running loop, create a new one
                            asyncio.run(self._save_device_state("cpu"))
                    except Exception:
            ...
            try:
                ...
                try:
                    ...
                    logger.info("Loading tokenizer from cache (offline mode)...")

                    # Use local_files_only to avoid network calls and timeout issues
                    ...
                    logger.error(
                        "Tokenizer loading failed", error_type=type(e).__name__, error=str(e)
                    )
                    error = ExternalServiceError(
                        f"Could not load tokenizer {self.model_name}",
                        context={
                    ...
                    error.add_suggestion("Run 'acolyte install' to download the model")
                    error.add_suggestion("Check that the model files exist in cache")
                    error.add_suggestion(
                        f"Try manually: transformers-cli download {self.model_name}"
                    )
                    logger.error("Error loading tokenizer", error=error.to_dict())
                    raise error

                # Try loading model
                try:
                    # Lazy load transformers
                    AutoModel = self.transformers.AutoModel
                    logger.info("Loading model from cache (offline mode)...")
                    ...
                    error.add_suggestion("Check that model files exist in cache")
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        asyncio.run_coroutine_threadsafe(
                                            self._save_device_state("cpu"), loop
                                        )
                                    except RuntimeError:
                                        # No running loop, create a new one
                                        asyncio.run(self._save_device_state("cpu"))
                ...
                if self.metrics and hasattr(self.metrics, 'record_operation'):
                    self.metrics.record_operation("model_load", load_time)

                # Log available memory if is CUDA
                if self.device.type == "cuda":
                    memory_info = torch_module.cuda.get_device_properties(0)
                    memory_used = torch_module.cuda.memory_allocated(0) / 1024**3  # GB
                    memory_total = memory_info.total_memory / 1024**3  # GB
                    logger.info(
                        "GPU memory usage",
                        gpu_name=memory_info.name,
                ...
                error_details = {
                    ...
                    "device": str(self.device),
                    "error_type": type(e).__name__,
                    "error_message": str(e)[:1000],  # Limit very long messages but keep detail
                }
                logger.error("Detailed error information", extra=error_details)
                # Use proper logging instead of print for traceback
                logger.error("Full traceback for debugging", exc_info=True)

                error = ExternalServiceError(
                    "Unexpected error loading model",
                    context={**error_details, "traceback": traceback_str},
                )
                error.add_suggestion("Check logs for more details")
                error.add_suggestion("Report this error if persists")
                logger.error("Unanticipated error", extra={"error_details": error.to_dict()})
                raise error
    ...
    def _prepare_plain_text(
        self, text: Union[str, object], context: Optional[RichCodeContext]
    ) -> str:
    ...
    def encode(
        ...
        if cached is not None:
            if self.metrics and hasattr(self.metrics, 'record_cache_hit'):
                self.metrics.record_cache_hit()
            return cached
        if self.metrics and hasattr(self.metrics, 'record_cache_miss'):
        ...
        # Generate embedding
        torch_module = self.torch  # Get torch via property
    ...
    def _estimate_tokens(self, text: Union[str, Chunk]) -> int:
        ...
        if isinstance(text, Chunk):
            ...
            text_content = str(text)  # Always convert to string
        ...
        if not isinstance(text_content, str):
            ...
            return 100  # Default safe estimate
        ...
        if len(text_content) > 1000:
            word_count = int(word_count * (len(text_content) / 1000))
    ...
    def _encode_batch_token_limited(
        ...
        for i, (text, ctx) in enumerate(zip(texts, contexts)):
            ...
            if estimated_tokens > max_tokens_per_batch:
                logger.warning(
                    "Individual text exceeds token limit, processing it only",
    ...
    def encode_with_rerank(
        ...
        if len(initial_candidates) <= top_k:
            logger.debug(
                "Few candidates, returning without re-ranking",
        ...
        logger.debug(
            ...
            initial_retrieval=initial_top_k,
    ...
    def get_metrics_summary(self) -> EmbeddingsMetricsSummaryDict:
        ...
        if (
            self.metrics
            and hasattr(self.metrics, 'get_p95_latency')
            and hasattr(self.metrics, 'get_cache_hit_rate')
        ):
            base_info.update(
                {
