...
def index(path: str, partial: bool, progress: bool, verbose: bool, resume: Optional[str]):
    ...
    # --- Validación de versiones crítica antes de indexar ---
    try:
        from acolyte.install.database import DatabaseInitializer

        # project_id y global_dir ya están disponibles
        dbi = DatabaseInitializer(
            project_path=project_path, project_id=project_id, global_dir=manager.global_dir
        )
        import asyncio
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Mostrar resumen estructurado si está presente
            if 'report' in result:
                click.echo("\nIndexing summary:")
...
def doctor(fix: bool, project: str, clean_sqlite: bool):
    ...
    run_doctor(fix=fix, project=project, clean_sqlite=clean_sqlite)
...
async def monitor_indexing_progress(
    ...
    # Use the new unified progress monitor
    ...
    try:
        ...
        monitor.display_final_stats(final_stats, elapsed)
...
# Keep the old implementation as fallback for compatibility
