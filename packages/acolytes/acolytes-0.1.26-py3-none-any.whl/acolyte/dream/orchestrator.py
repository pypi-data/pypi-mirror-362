...
# Lazy import - analyzer loads heavy modules
...
class DreamOrchestrator:
    ...
    def __init__(self, weaviate_client: Optional[Any] = None) -> None:
        ...
        # Lazy load analyzer (imports heavy modules)
        self._analyzer = None
    ...
    @property
    def analyzer(self):
        """Lazy load analyzer to avoid importing heavy modules at startup."""
        if self._analyzer is None:
            from .analyzer import DreamAnalyzer

            self._analyzer = DreamAnalyzer(weaviate_client=self.weaviate_client)
    ...
    def _generate_suggestion(self, fatigue_level: float, is_emergency: bool) -> str:
        ...
        if is_emergency:
            ...
            logger.info(
                "[TRACE] Generating message for deep project analysis without query or focus areas"
            )
            return (
