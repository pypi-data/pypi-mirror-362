...
class QueryAnalyzer:
    ...
    def _is_specific_query(
        ...
        if any(entities.values()):
            logger.info("[TRACE] Query classified as specific due to entities")
            return True
    ...
    def clear_cache(self) -> None:
        ...
        logger.info("[TRACE] Query analysis cache cleared")
    ...
    def calculate_relevance(self, chunk: Chunk, context: QueryContext) -> float:
        ...
        for func in context.entities.get("functions", []):
            ...
            if func.lower() in chunk_lower:
                ...
                logger.info("[TRACE] Function matched in chunk content")
                break
            ...
            if (
                ...
                score += 0.3
                function_matched = True
                logger.info("[TRACE] Function matched in metadata name")
                break
        ...
        if context.query_tokens:
            ...
            if relevant_query_tokens and relevant_chunk_tokens:
                ...
                logger.info("[TRACE] Token overlap scoring applied")
        ...
        for entity_type, entities in context.entities.items():
            ...
            for entity in entities:
                if entity.lower() in chunk_lower:
                    entity_matches += 1
                    logger.info("[TRACE] Entity match found in chunk")

        score += min(entity_matches * 0.1, 0.3)
        if entity_matches > 0:
            logger.info("[TRACE] Entity matches added to score")
    ...
    def _calculate_line_proximity_score(
        ...
        if not metadata.start_line:
            logger.info("[TRACE] No start_line in metadata for line proximity")
            return 0.0
            ...
            try:
                ...
                for target in target_lines:
                    if chunk_start <= target <= chunk_end:
                        ...
                        logger.info("[TRACE] Line within chunk boundaries")
                    else:
                        # Calculate proximity
                        distance = min(abs(target - chunk_start), abs(target - chunk_end))
                        if distance <= 5:
                            best_score = max(best_score, 0.3)
                            logger.info("[TRACE] Line within 5 lines of chunk")
                        elif distance <= 10:
                            best_score = max(best_score, 0.2)
                            logger.info("[TRACE] Line within 10 lines of chunk")
                        elif distance <= 20:
                            best_score = max(best_score, 0.1)
                            logger.info("[TRACE] Line within 20 lines of chunk")

            except ValueError:
                logger.info("[TRACE] ValueError parsing line number")
                continue
