...
class PythonChunker(
    ...
    def _get_tree_sitter_language(self) -> Any:
        ...
        logger.info("[TRACE] python._get_tree_sitter_language called")
    ...
    def _determine_function_type(self, func_node, decorated_node=None) -> ChunkType:
        ...
        if name and name.startswith('__') and name.endswith('__'):
            if name == '__init__':
                logger.info("[TRACE] python constructor detected")
                return ChunkType.CONSTRUCTOR
            # Other magic methods are still methods
            logger.info("[TRACE] python magic method detected")
        ...
        if name and (name.startswith('test_') or name.startswith('Test')):
            logger.info("[TRACE] python test function detected")
        ...
        if decorated_node:
            ...
            if any('@property' in d for d in decorators):
                logger.info("[TRACE] python property decorator detected")
                return ChunkType.PROPERTY
            if any('@staticmethod' in d or '@classmethod' in d for d in decorators):
                logger.info("[TRACE] python static/classmethod detected")
                return ChunkType.METHOD
    ...
    def _detect_language_patterns(
        ...
        if re.search(r'def\s+\w+\s*\([^)]*=\s*(\[\]|\{\})', content):
            logger.info("[TRACE] python mutable default argument")
    ...
    def _calculate_quality_metrics(self, node) -> Dict[str, Any]:
        ...
        if 'assert' in content:
            logger.info("[TRACE] python assertions detected")
    ...
    def _extract_function_metadata(self, func_node) -> Dict[str, Any]:
        ...
        if func_name:
            if func_name.startswith('__') and not func_name.endswith('__'):
                logger.info("[TRACE] python private method")
        ...
        for child in func_node.children:
            if child.type == 'type':
                logger.info("[TRACE] python function return type")
        ...
        if func_node.parent and func_node.parent.type == 'decorated_definition':
            ...
            if any('@abstractmethod' in d for d in decorators):
                logger.info("[TRACE] python abstract method")
                ...
                if param.type == 'identifier':
                    ...
                    for child in param.children:
                        if child.type == 'identifier':
                            ...
                            logger.info("[TRACE] python parameter type annotation")
                    ...
                    if child.type == 'call':
                        ...
                        for c in child.children:
                            if c.type == 'identifier':
                                ...
                                logger.info("[TRACE] python exception found in raise")
                                break
            for child in n.children:
                find_raises(child)
            ...
            if child.type == 'block':
                for stmt in child.children:
                    ...
                    if '__slots__' in stmt.text.decode('utf8'):
                        logger.info("[TRACE] python __slots__ detected")
    ...
    def _is_comment_node(self, node) -> bool:
        ...
        # Solo log en desarrollo, no en tests
        # if not os.getenv("PYTEST_CURRENT_TEST"):
        #     logger.debug("[TRACE] python._is_comment_node called")
