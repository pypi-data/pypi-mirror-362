...
@shared_task(bind=True)
def bulk_update_task_optimized(self, serializer_class_path: str, updates_list: list[dict], user_id: int | None = None):
    """
    Optimized Celery task for bulk updating of model instances.
    ...
    # Initialize progress tracking in Redis
    BulkOperationCache.set_task_progress(task_id, 0, len(updates_list), "Starting optimized bulk update...")
    ...
    try:
        ...
        logger.info(
            "Optimized bulk update task %s completed: %s updated, %s errors",
            task_id,
            result.success_count,
            result.error_count,
        )
        ...
        logger.exception("Optimized bulk update task %s failed", task_id)
        result.add_error(0, f"Task failed: {e!s}")
        BulkOperationCache.set_task_result(task_id, result.to_dict())
...
@shared_task(bind=True)
def bulk_delete_task_optimized(self, model_class_path: str, ids_list: list[int], user_id: int | None = None):
    """
    Optimized Celery task for bulk deletion of model instances.
    ...
    # Initialize progress tracking in Redis
    BulkOperationCache.set_task_progress(task_id, 0, len(ids_list), "Starting optimized bulk delete...")
    ...
    try:
        ...
        logger.info(
            "Optimized bulk delete task %s completed: %s deleted, %s errors",
            task_id,
            result.success_count,
            result.error_count,
        )
        ...
        logger.exception("Optimized bulk delete task %s failed", task_id)
        result.add_error(0, f"Task failed: {e!s}")
        BulkOperationCache.set_task_result(task_id, result.to_dict())
...
@shared_task(bind=True)
def bulk_get_task_optimized(self, model_class_path: str, serializer_class_path: str, query_params: dict, user_id: int | None = None):
    """
    Optimized Celery task for bulk retrieval of model instances.
    ...
    # Initialize progress tracking in Redis
    BulkOperationCache.set_task_progress(task_id, 0, 1, "Starting optimized bulk retrieval...")
    ...
    try:
        ...
        logger.info(
            "Optimized bulk get task %s completed: %s records retrieved",
            task_id,
            len(serialized_results)
        )
        ...
        logger.exception("Optimized bulk get task %s failed", task_id)
        error_result = {
            "task_id": task_id,
            "operation_type": "bulk_get",
            "success": False,
