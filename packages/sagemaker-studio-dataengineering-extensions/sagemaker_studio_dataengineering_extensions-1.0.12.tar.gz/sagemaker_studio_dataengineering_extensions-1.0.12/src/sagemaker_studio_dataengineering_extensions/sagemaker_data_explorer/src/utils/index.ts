...
function getDefaultConnectionName(connections: Connection[], connectionType: ConnectionType) {
  ...
  if (connectionType === ConnectionType.SPARK && oldDefaultSparkConnectionExists) {
    return OLD_DEFAULT_SPARK_CONNECTION_NAME;
  }
  return DEFAULT_CONNECTION_NAME[connectionType as string];
}
...
export function getSQLQuery(connections: Connection[], connectionType: ConnectionType, nodeData: NodeData) {
  ...
  let notebookQuery = connectionType === ConnectionType.SPARK ? `%%pyspark ${connectionName}\n` : `%%sql ${connectionName}\n`;
  if (path) {
    ...
    switch (connectionType) {
      ...
      case ConnectionType.SPARK:
        if (isAwsDataCatalog) {
          notebookQuery += `spark.sql("select * from \`spark_catalog\`.\`${databaseName}\`.\`${table}\` limit 10")`;
        } else {
          const catalogChildName = catalogChild ? `_${catalogChild}` : '';
          notebookQuery += `spark.sql("select * from \`${catalogName}${catalogChildName}\`.\`${databaseName}\`.\`${table}\` limit 10")`;
        }
        break;
    }
  }
