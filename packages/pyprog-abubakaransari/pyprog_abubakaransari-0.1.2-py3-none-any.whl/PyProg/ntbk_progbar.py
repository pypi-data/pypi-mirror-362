        ...
        if self.paint == "bar-by-bar":
            ...
            for i in range(self.length):
                ...
                if i < filled_len:
                    ...
                    bar_html += f'<span style="{self.space_style}">{self.space}</span>'
            content = (
            ...
            bar_html += f'<span style="{self.space_style}">{self.space * (self.length - filled_len)}</span>'
            content = (
                f'<span style="color:{label_color}">{processed_lf} </span>'
                f'{bar_html}'
                f'<span style="color:{right_color}"> {processed_rt}</span>'
            ...
            for i in range(filled_len):
                bar_char = bar_seq[i % len(bar_seq)]
                bar_html += bar_char
            bar_html += self.space * (self.length - filled_len)
...
def simple(iterable, total=None, args=None, **kwargs):
    ...
    >>> from PyProg.notebook_progress_bar import simple, time
...
class NotebookProgressBar(Bar):
    ...
    Example
    -------
    >>> from notebook_prog_Bar import NotebookProgressBar
    >>> from utils import gradient_colors
    >>> import time
    >>> 
