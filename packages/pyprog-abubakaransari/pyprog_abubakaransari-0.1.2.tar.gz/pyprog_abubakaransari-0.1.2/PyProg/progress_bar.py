...
class PBAR:
    ...
    def build_output(self):
        ...
        if self.paint == "bar-by-bar":
            bar_str = ''.join(
                self.get_color(i) + bar_seq[i % len(bar_seq)] if i < filled_len else Style.RESET_ALL + self.space
            ...
            bar_str = ''.join(bar_seq[i % len(bar_seq)] for i in range(filled_len))
            bar_str = color + bar_str + Style.RESET_ALL + (self.space * (self.length - filled_len))
            output = f"\r{left_txt_color}{left_text}{reset}{bar_str}{reset}{right_txt_color}{right_text}{Style.RESET_ALL}"

        else:
            bar_str = ''.join(bar_seq[i % len(bar_seq)] for i in range(filled_len))
            bar_str += self.space * (self.length - filled_len)
            output = f"\r{left_txt_color}{left_text}{reset}{bar_str}{reset}{right_txt_color}{right_text}{Style.RESET_ALL}"
