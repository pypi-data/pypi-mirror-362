...
class TextFormatter:
    ...
    def format(self, analysis: Dict[str, Any], **kwargs) -> str:
        ...
        for i, ax in enumerate(axes):
            ...
            title_info = f"title={ax_info.get('title')}" if ax_info.get('title') else "no_title"
            ...
            plot_types_str = ', '.join([
                f"{pt.get('type', '').lower()}" + (f" (label={pt.get('label')})" if pt.get('label') else "")
            ...
            if curve_points_to_show:
                ...
                if len(curve_points_to_show) > 10:
                    lines.append(f"    ... and {len(curve_points_to_show) - 10} more points")
...
class SemanticFormatter:
    ...
    def format(self, analysis: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if 'plot_description' not in semantic_analysis:
            ...
            title = semantic_analysis.get('title') or semantic_analysis.get('figure_info', {}).get('title', 'No Title')
            figure_type = semantic_analysis.get('figure_type') or semantic_analysis.get('figure_info', {}).get('figure_type', 'Unknown')
            axes = semantic_analysis.get('axes') or semantic_analysis.get('axes_info') or []
            ...
            if axes:
                ...
                for i, ax in enumerate(axes):
                    ...
                    if plot_types:
                        plot_types_str = ', '.join([pt.get('type', 'unknown') for pt in plot_types])
