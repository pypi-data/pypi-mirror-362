...
def generate_table_name(entity_name: str = None,
    ...
    if ml_table_name:
        ...
        table_name = None
    return add_env_to_suffix(table_name)
...
def generate_view_name(table_name: Optional[str]) -> Optional[str]:
    ...
    return add_env_to_suffix(view_name)
...
def add_env_to_suffix(name: Optional[str]) -> Optional[str]:
    ...
    match name:
        case "profile_table" | "contact_table" | "user_table":
            if not name.endswith(f"_{env_name}"):
                name += f"_{env_name}"
        case _:
            pass  # don't append for any other names
...
def replace_view_with_table(view_table_name: str, select_clause_value: str = None) -> str:
    ...
    # if table_name in table_columns and all(  # if all required columns from the view present in the table.
