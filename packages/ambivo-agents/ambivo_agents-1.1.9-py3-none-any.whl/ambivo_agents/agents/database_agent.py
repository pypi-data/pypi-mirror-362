...
Always prioritize data security and provide clear, formatted results."""
        ...
        self._unsafe_patterns = [
            ...
            r'\balter\s+table\b', r'\bcreate\s+table\b', r'\bcreate\s+database\b',
            r'--', r'/\*', r'\*/', r'\bexec\b', r'\bexecute\b'
        ]

    async def process_message(self, message: Union[str, AgentMessage], execution_context: Optional[ExecutionContext] = None, **kwargs) -> Union[str, AgentMessage]:
        ...
        try:
            ...
            context = "\n".join([f"{h.get('sender', h.get('role', 'unknown'))}: {h.get('content', h.get('message', ''))}" for h in history[-3:]])
            ...
            if return_agent_message:
                return AgentMessage(
                    id=str(uuid.uuid4()),
                    sender_id=self.agent_id,
                    recipient_id=message.sender_id if isinstance(message, AgentMessage) else "unknown",
                    content=error_msg,
                    message_type=MessageType.AGENT_RESPONSE,
                    session_id=message.session_id if isinstance(message, AgentMessage) else "unknown",
                    conversation_id=message.conversation_id if isinstance(message, AgentMessage) else "unknown",
    ...
    async def _is_connection_request(self, message: str) -> bool:
        ...
        strong_indicators = [
            "connect to", "connect to database", "setup connection", 
            "establish connection", "database connection"
        ]
        
        # Connection patterns with database info
        connection_patterns = [
            "localhost:", "127.0.0.1:", "mongodb://", 
            "mysql:", "postgresql:", "host:", "port:", "username:", "password:"
        ]
        ...
        if "connect" in message_lower and any(pattern in message_lower for pattern in connection_patterns):
            return True
    ...
    async def _is_schema_request(self, message: str) -> bool:
        ...
        schema_keywords = [
            "schema", "tables", "columns", "describe", "structure",
            "show tables", "table info", "database structure"
        ]
    ...
    async def _is_query_request(self, message: str) -> bool:
        ...
        # SQL query keywords
        sql_keywords = [
            "select", "query", "sql", "rows", "show me", "get", "fetch"
        ]
        
        # MongoDB query keywords
        mongodb_keywords = [
            "find", "find()", "findOne", "count", "distinct", "aggregate",
            "use ", "db.", "show collections", "show dbs", "show databases"
        ]
        
        # General query keywords
        general_keywords = [
            "search", "sum", "average", "data", "records"
        ]
        
        return any(keyword in message_lower for keyword in sql_keywords + mongodb_keywords + general_keywords)
    ...
    async def _handle_connection_request(self, message_content: str) -> str:
        ...
        try:
            ...
            llm_response = await self.llm_service.generate_response(
                connection_prompt, context={"system_message": "You are a database connection parser. Return only valid JSON."}
            )
            ...
            try:
                ...
                if json_match:
                    ...
                    if "mysql" in message_content.lower() and "localhost:3306" in message_content.lower():
    ...
    async def _handle_query_request(self, message_content: str, context: str) -> str:
        ...
        try:
            ...
            if self._current_db_config.db_type == DatabaseType.MONGODB:
                ...
                if (message_content.lower().startswith('use ') or 
                    message_content.lower().startswith('db.') or
                    message_content.lower() in ['show dbs', 'show databases', 'show collections']):
                    ...
                    llm_response = await self.llm_service.generate_response(
                        query_prompt, context={"system_message": "You are a MongoDB query generator. Return only the query or UNSAFE_QUERY."}
                    )
                ...
                llm_response = await self.llm_service.generate_response(
                    query_prompt, context={"system_message": "You are a SQL query generator. Return only the query or UNSAFE_QUERY."}
                )
                query = llm_response.strip()
            ...
            if query == "UNSAFE_QUERY" or not query or query.lower() in ['plaintext', 'unsafe_query', 'no query', 'null', 'none']:
                self.logger.warning(f"LLM returned invalid query '{query}' for message: {message_content}")
                ...
                if self._current_db_config.db_type == DatabaseType.MONGODB:
                    ...
                    if any(word in message_lower for word in ["faculty", "researcher", "professor", "academic", "distribution", "department"]):
                        query = "db.faculty.find().limit(10)"
                        self.logger.info(f"Using MongoDB faculty fallback query: {query}")
                    elif any(word in message_lower for word in ["publication", "paper", "research", "citation", "venue", "h-index"]):
                        query = "db.publications.find().limit(10)"
                        self.logger.info(f"Using MongoDB publication fallback query: {query}")
                    elif any(word in message_lower for word in ["university", "institution", "school"]):
            ...
            if analytics_requested and result.success and result.data and self.enable_analytics_handoff:
                ...
                if handoff_result["success"]:
                    ...
                    await self.add_to_conversation_history(
                        f"ANALYTICS_HANDOFF:{handoff_result['csv_path']}", 
                        "system"
                    )
    ...
    async def _is_analytics_requested(self, message: str) -> bool:
        ...
        message_lower = message.lower()
        analytics_keywords = [
            "analyze", "visualization", "chart", "graph", "plot", "analytics", 
            "visualize", "dashboard", "trend", "pattern", "export", "csv",
            "statistics", "summary", "insights", "correlation"
        ]
    ...
    def _is_mongodb_query_safe(self, query_lower: str) -> bool:
        ...
        mongodb_unsafe_patterns = [
            ...
            r'\$where', r'eval\(', r'function\s*\(', r'javascript',
            # Database/collection destruction
            r'dropDatabase\s*\(', r'\.drop\s*\(', 
            # Data modification/deletion (only block in strict mode)
            r'\.remove\s*\(', r'\.deleteOne\s*\(', r'\.deleteMany\s*\(',
            r'\.insertOne\s*\(', r'\.insertMany\s*\(', r'\.insert\s*\(',
            r'\.updateOne\s*\(', r'\.updateMany\s*\(', r'\.update\s*\(',
            r'\.replaceOne\s*\(', r'\.save\s*\(',
            # Index operations that modify structure
            r'\.createIndex\s*\(', r'\.dropIndex\s*\(', r'\.reIndex\s*\(',
            # Collection creation/modification
            r'\.createCollection\s*\(', r'\.renameCollection\s*\('
        ]
    ...
    async def _execute_mongodb_query(self, query: str) -> QueryResult:
        ...
        try:
            ...
            if query_lower in ['show dbs', 'show databases']:
                ...
                return QueryResult(success=False, error=f"Unsupported MongoDB query format: {query}")
    ...
    async def _execute_mongodb_shell_command(self, query: str, connection) -> QueryResult:
        ...
        try:
            ...
            if query_clean.startswith('db.'):
                ...
                # Get database
                if not self._current_db_config.database:
                    return QueryResult(success=False, error="No database selected. Use 'use <database>' first.")
                ...
                if operation == 'find':
                    ...
                    data = [{f"distinct_{field}": value} for value in distinct_values[:self.max_result_rows]]
                    ...
                    try:
                        ...
                        return QueryResult(success=False, error=f"Aggregate execution failed: {str(e)}")
                ...
                # Add support for other common read operations
                    ...
                    try:
                        ...
                        return QueryResult(success=False, error=f"Operation '{operation}' failed: {str(e)}")
    ...
    async def _format_query_result(self, result: QueryResult) -> str:
        ...
        output = []
        output.append(f"✅ Query Results ({result.row_count} rows, {result.execution_time_ms:.1f}ms):")
        output.append("")
        ...
        try:
            ...
            if not self._current_db_config.database:
                ...
                if databases:
                    ...
                    output.append("")
                    output.append("💡 Use 'use <database_name>' to select a database and see its collections.")
                else:
    ...
    def _generate_analytics_suggestions(self, result: QueryResult) -> List[str]:
        ...
        if result.columns:
            ...
            for col in result.columns:
                col_lower = col.lower()
                if any(word in col_lower for word in ['count', 'sum', 'total', 'amount', 'price', 'value', 'quantity']):
            ...
            if numeric_columns:
                ...
                if len(numeric_columns) > 1:
                    suggestions.append(f"Analyze correlation between: {', '.join(numeric_columns[:2])}")
            
            if date_columns and numeric_columns:
                suggestions.append(f"Time series analysis of {numeric_columns[0]} over {date_columns[0]}")
