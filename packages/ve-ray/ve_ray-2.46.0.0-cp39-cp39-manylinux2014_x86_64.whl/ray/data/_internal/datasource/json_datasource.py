...
class JSONDatasource(FileBasedDatasource):
    ...
    def __init__(
        ...
        **file_based_datasource_kwargs,
        ...
        # Check if pyarrow >= 20.0.0 which has the `open_json` function
        # using `open_json` for streaming read JSON files.
        try:
            from pyarrow.json import open_json

            self.using_pyarrow_open_json = True
        except ImportError:
            self.using_pyarrow_open_json = False

    def _read_with_pyarrow_open_json(self, f: "pyarrow.NativeFile", path: str):
    ...
    def _read_stream(self, f: "pyarrow.NativeFile", path: str):
        ...
        if self.is_jsonl:
            ...
            try:
                if self.using_pyarrow_open_json:
                    yield from self._read_with_pyarrow_open_json(f, path)
                else:
                    buffer: pa.lib.Buffer = f.read_buffer()
