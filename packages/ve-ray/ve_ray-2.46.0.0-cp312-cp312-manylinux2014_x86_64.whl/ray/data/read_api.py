...
#implement a simplified ray read client with minimal requirement
@PublicAPI
def read_lance_table(
    ...
    *,
    ...
    Examples:
        >>> import ray
        >>> ds = ray.data.read_lance_table( # doctest: +SKIP
        ...     tbl_identifier="db_name.tbl_name",
        ...     columns=["image", "label"],
        ...     filter="label = 2 AND text IS NOT NULL",
        ... )

    Args:
        tbl_identifier: The URI of the Lance dataset to read from. Local file paths, S3, and GCS
            are supported.
        columns: The columns to read. By default, all columns are read.
        filter: Read returns only the rows matching the filter. By default, no
            filter is applied.
    ...
    from lance.catalog import Catalog, CatalogFactory
    from lance.catalog.utils import Utils

    # Initialize the LasCatalog object
    catalog = CatalogFactory.create(storage_options)
    uri = catalog.convert_table_to_dataset_uri(tbl_identifier)
    tbl_location = Utils.rename_scheme(uri)
...
def read_kafka(
    ...
    *,
    ...
    """
    Create a :class:`~ray.data.Dataset` from a Kafka topic.

    This is a streaming API, so the number of rows is not known in advance.

    Examples:
        >>> import ray
        >>> ds = ray.data.read_kafka( # doctest: +SKIP
    ...
    Args:
        ...
        group_id: The consumer group ID to use. If not provided, a random group ID
            will be used. This is a convenience parameter that sets the "group.id"
            in `consumer_config`. If "group.id" is already present in
            `consumer_config`, this parameter must match it if provided.
        consumer_config: A dictionary of Kafka consumer configuration options.
            For more information, see `librdkafka configuration properties
        ...
        concurrency: The maximum number of Ray tasks to run concurrently. Set this
            ...
            concurrency is dynamically decided based on the available resources.
        override_num_blocks: Override the number of output blocks from all read tasks.
            This corresponds to the number of read tasks, which is also the number of
            Kafka topic partitions to read from. By default, this is the number of
            partitions of the topic.

    Returns:
        A :class:`~ray.data.Dataset` producing records read from the Kafka topic.
        Each record is a dictionary with the following fields: 'key', 'value', 'topic',
        'partition', 'offset', 'timestamp', and 'headers'.
    """  # noqa: E501
