...
class ProcessServer():
    ...
    def run_process(self, sidpy_dset, process_name, **args):
        ...
        #if the process worked, we should received a hdf5 file. If not, then we should receive just an error message
        try:
            ...
            #there was an error
            error_file_path = 'error_output.txt'

            with open(error_file_path, 'wb') as file:
                for chunk in chunks:
                    file.write(chunk)

            # Step 2: Read the error file as a normal text file
