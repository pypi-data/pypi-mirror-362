...
import inspect
from ..utilities.utils import logger, make_be_dset, progress_bar
from . import AECypher
...
class AEBE():
    def __init__(self, exe_path = "G:\My Drive\AE\PyAE\BEPyAE 022823 01\BEPyAE.exe", client = "BEPyAE.Application", 
                 pyae_vi = "\BE_PyAE_01.vi", pyscanner_vi = "\FPGA PyScanner\FPGA_PyScanner_01.vi", 
                 log_file = "logfile.p", backend = 'cypher', backend_path = r"C:\AsylumResearch\v18\Igor Pro Folder\Igor.exe") -> None:
        """
        ...
        Args:
            ...
            backend (str): Default = 'cypher'. Others will be supported in due course.
            backend_path (str): Default = C:\AsylumResearch\v18\Igor Pro Folder\Igor.exe. This is the .exe file of the backend you are using.
        """ 
        ...
        if self.backend.lower() == 'cypher':
            self.ae_cypher = AECypher.AECypher(exe_path = self.backend_path)
    ...
    @logger
    def make_be_dataset(self, file_name, pfm_imgstack, channel_imgstack, complex_spectra,
        ...
        # Retrieve BE pulse parameters
        be_parms = self.VI.getcontrolvalue('BE_pulse_control_cluster')
        vec_dc = np.asarray(self.VI.getcontrolvalue("BEPS_VS_indicator_cluster")[0])

        if self.backend == 'cypher':
            ar_metadata = self.ae_cypher.Get_MasterVariables()
        else:
            ar_metadata = {}
        
        output = make_be_dset(file_name, pfm_imgstack, channel_imgstack, complex_spectra,
                start_x, finish_x, start_y, finish_y, coordinates, beps, be_parms=be_parms,vec_dc = vec_dc,
    ...
    @logger
    def raster_scan(self, file_name = "BEPFM", raster_parms_dict = None, pixel_line_ratio = 1,
        ...
        scan_line_array = np.linspace(raster_parms_list[3], raster_parms_list[5], int(raster_parms_list[1]*pixel_line_ratio))
        ...
        if progress_on:
            bar_progress = progress_bar(int(raster_parms_list[1]))  
        ...
        be_pulse_parms = np.array(self.VI.getcontrolvalue('BE_pulse_control_cluster'))
        center_freq = be_pulse_parms[0]
        bandwidth = be_pulse_parms[1]
        freq_min = center_freq - bandwidth/2
        freq_max = center_freq + bandwidth/2
        num_points = np.array(line_cx_spectra).shape[1]
        metadata = self.ae_cypher.Get_MasterVariables()

        freq_vector = np.linspace(freq_min, freq_max, num_points)
        dset_imgs, dset_chns, complex_spectra = make_be_dset(file_name = file_name, 
                                                             ...
                                                             freq_vector = freq_vector, metadata=metadata, be_parms=be_pulse_parms)
    ...
    def do_beps_grid (self, beps_parms_dict = None, beps_grid_parms_dict = None, file_name = "BEPS_grid", 
        ...
        # make a progress bar
        if progress_on:
            bar_progress = progress_bar(max_value = len(pixels_x))
        ...
        freq_min = center_freq - bandwidth/2
        freq_max = center_freq + bandwidth/2
        num_points = np.array(cpx_spectra).shape[1]
        freq_vector = np.linspace(freq_min, freq_max, num_points)
        metadata = self.ae_cypher.Get_MasterVariables()
        num_cycles = np.array(self.VI.getcontrolvalue('Initialize_BEPS_VS_control_cluster'))[4]
        beps_parms = {'Cycles':num_cycles , 'Vdc':vs, 'x_points': len(pixel_x),'y_points':len(pixel_y) }
        vdc, beps_qf, beps_chs, beps_cs = make_be_dset(file_name = file_name,
                                                       ...
                                                       beps = True, metadata=metadata, be_parms = be_pulse_parms,
                                                       beps_parms = beps_parms)
    ...
    def do_beps_specific (self, beps_parms_dict = None, file_name = "BEPS_spec", 
        ...
        num_points = np.array(cpx_spectra).shape[1]
        freq_vector = np.linspace(freq_min, freq_max, num_points)
        vdc = np.array(self.VI.getcontrolvalue('BEPS_VS_indicator_cluster')[0])
        number_of_cycles = self.VI.getcontrolvalue('Initialize_BEPS_VS_control_cluster')[4]
        beps_parms = {}
        beps_parms['Vdc'] = vdc
        beps_parms['Cycles'] = number_of_cycles
        beps_parms['rest'] = np.array(self.VI.getcontrolvalue('Initialize_BEPS_VS_control_cluster'))
        beps_parms = {'Cycles':number_of_cycles , 'Vdc':vs, 'x_points': 1, 'y_points':1 }
        vdc, beps_qf, beps_chs, beps_cs = make_be_dset(file_name = file_name,
                                                       pfm_imgstack = np.asarray(grid_beps_quick_fit),
                                                       channel_imgstack = np.asarray(grid_beps_chns),
                                                       complex_spectra = np.asarray(grid_beps_cpx_spectra),
                                                       ...
                                                       finish_y = 1, 
                                                       coordinates = coordinates_final,
                                                       beps = True,beps_parms= beps_parms,
                                                       be_parms = be_pulse_parms,
                                                       freq_vector=freq_vector)
    ...
    def fpga_spiral_scan_BE(self, be_parms_dict = None, do_create_be_waveform = True,
        ...
        #################################################################################################
            if spiral_reconstruction == True:
                ...
                # BE reconstruction
                hf["BE Line/recontructions"] = rescontructions
                ...
                be_results = {"daq_wave": daq_wave, "complex_spectrogram": complex_spectrogram, 
                              ...
                              "reconstructions": rescontructions}
