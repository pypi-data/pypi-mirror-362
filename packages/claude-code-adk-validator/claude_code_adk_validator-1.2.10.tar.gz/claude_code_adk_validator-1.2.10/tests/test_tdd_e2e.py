...
class TestTDDEndToEnd:
    ...
    def setup_method(self):
        ...
        # Handle case where current directory might not exist in CI
        try:
            self.original_cwd = os.getcwd()
        except (FileNotFoundError, OSError):
            # Fallback to a known directory
            self.original_cwd = Path(__file__).parent.parent.absolute()
    ...
    def teardown_method(self):
        ...
        try:
            ...
            # If original cwd doesn't exist, just continue
            pass
        shutil.rmtree(self.test_dir)
        
        # Clean up test results file
        original_data_dir = Path(self.original_cwd) / ".claude/claude-code-adk-validator/data"
        original_test_file = original_data_dir / "test.json"
        if original_test_file.exists():
            original_test_file.unlink()
    ...
    def create_test_results(self, test_modules: list):
        """Create test results file in the validator's expected location."""
        # Create test results in the original directory where validator runs
        original_data_dir = Path(self.original_cwd) / ".claude/claude-code-adk-validator/data"
...
def test_subtract():
    assert subtract(5, 3) == 2""",
        ...
        assert "TDD" in result["stderr"] and ("multiple" in result["stderr"].lower() or "two test" in result["stderr"].lower())
        assert "ONE" in result["stderr"] or "one test at a time" in result["stderr"].lower()
    ...
    return a + b""",
        ...
        # Should mention tests are passing or no tests found
        assert "test" in result["stderr"].lower() and ("passing" in result["stderr"] or "pytest" in result["stderr"])
...
import pytest''',
        ...
        response = json.loads(result["stdout"])
        assert response["continue"] is True
        assert response["decision"] == "approve"  # Warnings are implemented as approvals
        assert "no test functions" in response["reason"]
        assert "(with recommendations)" in response["reason"]
...
def sample_data():
    return {"key": "value"}""",
        ...
        assert response["continue"] is True
        assert response["decision"] == "approve"
        assert "Non-programming file" in response["reason"] or "conftest.py" in response["reason"]
