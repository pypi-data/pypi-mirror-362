...
def validate_hook_input(stage: Optional[str] = None) -> None:
    ...
    try:
        ...
        if response.decision == "block":
            # For blocking: write JSON to stdout AND comprehensive response to stderr, exit with code 2
            claude_json = response.to_claude_json()
            print(json.dumps(claude_json))  # JSON to stdout for tests
            print(
                response.format_for_stderr(), file=sys.stderr
            )  # Detailed info to stderr
            sys.exit(2)
        else:
...
def run_full_validation(
    ...
    if api_key:
        ...
        # Return highest priority warning or approval
        # Check warnings in order of priority (security > tdd > file)
        # Note: warnings are implemented as approve with educational content
        if (
            hasattr(security_response, "metadata")
            and security_response.metadata.validation_stage
            == ValidationStage.TOOL_ENFORCEMENT
        ):
            return security_response
        if security_response.reason and "recommendation" in security_response.reason:
            return security_response
        if tdd_response.reason and "recommendation" in tdd_response.reason:
            return tdd_response
        if file_response.reason and "recommendation" in file_response.reason:
            return file_response

        # If no warnings, return the last checked response
        return file_response
