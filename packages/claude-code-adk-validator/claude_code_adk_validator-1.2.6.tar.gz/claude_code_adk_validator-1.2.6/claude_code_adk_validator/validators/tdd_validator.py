...
class TDDValidator:
    ...
    def get_pytest_results(self) -> Optional[str]:
        ...
        try:
            ...
            print(
                "Unable to load test results. QUICK FIX:",
                file=sys.stderr,
            )
            print(
                "1. Install: uv add --dev pytest-json-report",
                file=sys.stderr,
            )
            print(
                "2. Run: uv run pytest --json-report --json-report-file=.pytest_results.json",
                file=sys.stderr,
            )
            print(
                "OR bypass: export DISABLE_TDD_VALIDATION=true",
                file=sys.stderr,
            )
            return None
    ...
    def generate_fallback_educational_content(
        ...
        if is_test:
            ...
            return {
                ...
                "educational_notes": [
                    "QUICK FIX - Enable pytest-json-report:",
                    "1. Install: uv add --dev pytest-json-report",
                    "2. Run test: uv run pytest --json-report --json-report-file=.pytest_results.json",
                    "3. Now you can write implementation code!",
                    "",
                    "OR temporarily bypass: export DISABLE_TDD_VALIDATION=true",
                    "",
...
- setup_instructions: ALWAYS include detailed pytest setup instructions including:
  * QUICK START: uv add --dev pytest-json-report
  * Run test: uv run pytest --json-report --json-report-file=.pytest_results.json
  * The validator will read .pytest_results.json automatically
  * Alternative: export DISABLE_TDD_VALIDATION=true to bypass temporarily
  * Or use the validator's plugin: uv add --dev claude-code-adk-validator
...
Make your response educational, comprehensive, and actionable. Focus on helping the developer understand not just what to do, but why it matters and how to do it correctly."""
            ...
            if not response or not hasattr(response, "text") or not response.text:
                ...
                print(
                    "TDD Analysis: LLM returned no response. QUICK FIX:",
                    file=sys.stderr,
                )
                ...
                print("3. This creates a file the validator can read", file=sys.stderr)
                print("", file=sys.stderr)
                print("OR bypass: export DISABLE_TDD_VALIDATION=true", file=sys.stderr)
        ...
        if self.is_test_file(file_path):
            ...
            if not test_output:
                ...
                enhanced_educational_notes = [
                    "QUICK START - Use pytest-json-report (Recommended):",
                    "",
                    "1. Install pytest-json-report in your project:",
                    "   uv add --dev pytest-json-report",
                    "",
                    "2. Run your failing test to generate output:",
                    "   uv run pytest --json-report --json-report-file=.pytest_results.json",
                    "",
                    "3. The validator will now accept your Write operations!",
                    "",
                    "ALTERNATIVE OPTIONS:",
                    "",
                    "Option A - Bypass TDD validation temporarily:",
                    "   export DISABLE_TDD_VALIDATION=true",
                    "",
                    "Option B - Create a mock test result:",
                    "   mkdir -p .claude/claude-code-adk-validator/data",
                    "   echo '{\"testModules\": [{\"moduleId\": \"test.py\", \"tests\": [{\"name\": \"test_failing\", \"state\": \"failed\"}]}]}' > .claude/claude-code-adk-validator/data/test.json",
                    "",
                    "Option C - Use the validator's pytest plugin:",
                    ...
                    "       config.pluginmanager.register(reporter, 'claude_code_adk_validator_reporter')",
                    "",
