...
class BasicComparisonReport(object):
    """
    ...
    def report_sub_phases_table_downlink_capacity(self, dv, sub_phases):
        """
        ...
        sub_phases_header = ['sub-phase', 'period', 'Total [Gbits]', "XB [Gbits]", "KA [Gbits]", 'Total [Gbits]'
            ,'Description']
    ...
    def report_sub_phases_table_generated_dv(self, dv, sub_phases):
        """
    ...
    def report_summary_table(self, dfs, generated_dv=False, downlink_capacity=True, total_downlink=False):
        """
        ...
        if downlink_capacity:
            ...
            metrics.append(['Total Downlink capacity'] + values + [sum(values)])
            values = [round(dfs[k].get_total_downlink('XB_LINK'), 2) for k in sorted(dfs.keys())]
            metrics.append(['Total Downlink capacity XB_LINK'] + values + [sum(values)])
            values = [round(dfs[k].get_total_downlink('KAB_LINK'), 2) for k in sorted(dfs.keys())]
            metrics.append(['Total Downlink capacity KAB_LINK'] + values + [sum(values)])
        if total_downlink:
            values = [round(dfs[k].get_total_ssmm_accum(), 2) for k in sorted(dfs.keys())]
            metrics.append(['Actual Total Downlink to ground'] + values + [sum(values)])
            values = [round(dfs[k].get_x_band_accum(), 2) for k in sorted(dfs.keys())]
            metrics.append(['Actual Total Downlink to ground XB_LINK'] + values + [sum(values)])
            values = [round(dfs[k].get_ka_band_accum(), 2) for k in sorted(dfs.keys())]
            metrics.append(['Actual Total Downlink to ground KAB_LINK'] + values + [sum(values)])
