# This is inspired by Prefect https://github.com/PrefectHQ/prefect/blob/main/src/prefect/utilities/services.py
...
class SimpleCounter:
    service_name: str
    print_interval: int = 3600
    _value: int = 0
    _last_checkpoint: float = field(default_factory=time.time)

    def incr(self, by: int = 1):
        self._value += by
        duration = time.time() - self._last_checkpoint
        if duration > self.print_interval:
            logger.info(f"Service {self.service_name} has run {self._value} times in the last {duration:.0f} seconds.")
            self.reset()
...
async def critical_service_loop(
    ...
    show_stats_interval: Optional[int] = 3600,
):
    """
    ...
    Args:
        ...
        show_stats_interval: show stats every `show_stats_interval` seconds
    """
    ...
    workload_display_name = workload.__name__ if hasattr(workload, "__name__") else workload
    loop_counter = SimpleCounter(service_name=workload_display_name, print_interval=show_stats_interval)
    logger.info(f"Starting service loop for {workload_display_name!r}")
    ...
    while True:
        try:
            ...
            logger.debug(f"Run of {workload!r} failed with TransportError", exc_info=exc)
            ...
            if exc.response.status_code >= 500:
                ...
                track_record.append(False)
                failures.append((exc, sys.exc_info()[-1]))
                logger.debug(f"Run of {workload!r} failed with HTTPStatusError", exc_info=exc)
            else:
                raise
        ...
        if not any(track_record):
            ...
            printer(f"\nFailed the last {consecutive} attempts. " "Please check your environment and configuration.")
        ...
        if jitter_range is not None:
            ...
            lower_bound = interval * (1 - jitter_range)
            upper_bound = interval * (1 + jitter_range)
