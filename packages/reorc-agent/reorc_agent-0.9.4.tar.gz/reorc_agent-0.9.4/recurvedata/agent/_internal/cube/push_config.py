...
class CubePushConfigService:
    ...
    async def download_cacert_pem(self):
        """Download cacert.pem file asynchronously"""
    ...
    async def fetch_updated_env_file(self, payload: CubePushConfigPayload, existing_env_file: str) -> dict:
        ...
        try:
            ...
            if env_content:
                ...
                logger.info(f"{self.log_prefix} Changes detected - ClickHouse: {has_clickhouse_changes}, General: {has_general_env_changes}")
    ...
    async def push_cube_config(self, payload: CubePushConfigPayload) -> tuple[bool, str | None]:
        """Main processing function for cube push config service."""
        logger.info(f"{self.log_prefix} Processing cube push config for env_id: {payload.env_id}, project_id: {payload.project_id}")
        ...
        try:
            ...
            if payload.project_id:
                ...
                if restart_needed:
                    logger.info(f"{self.log_prefix} Restarting cube api container, waiting for 10 seconds before validation")
            ...
            return False, f"Error processing cube push config for env_id {payload.env_id}: {e}"
