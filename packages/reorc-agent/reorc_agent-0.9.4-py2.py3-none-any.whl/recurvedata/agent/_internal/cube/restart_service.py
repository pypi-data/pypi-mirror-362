...
class CubeRestartService:
    ...
    async def restart_cube_service(self, payload: CubeRestartServicePayload) -> tuple[bool, str | None]:
        ...
        try:
            ...
            if is_connection_valid:
                ...
                return False, "Cube restart completed but connection validation failed"

        except Exception as e:
            logger.error(f"{self.log_prefix} Error processing cube restart for env_id {payload.env_id}: {e}")
            return False, f"Error processing cube restart for env_id {payload.env_id}: {e}"
