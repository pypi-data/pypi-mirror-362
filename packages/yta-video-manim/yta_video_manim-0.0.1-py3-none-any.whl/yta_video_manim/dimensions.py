# This was previously using 3 constants, and one of those was
# 'MOVIEPY_SCENE_DEFAULT_SIZE'. It is the same as the
# DEFAULT_SCENE_SIZE but maybe it was like that for something...
from yta_video_manim.constants import DEFAULT_SCENE_SIZE, DEFAULT_MANIM_SCENE_SIZE
from manim import *
...
class ManimXGenerator:
    ...
    def mobject_fitting_screen(
        ...
        **kwargs
        ...
        if ManimDimensions.manim_width_to_width(mobject.width) > DEFAULT_SCENE_SIZE[0]:
            mobject.scale_to_fit_width(ManimDimensions.width_to_manim_width(DEFAULT_SCENE_SIZE[0]))
        if ManimDimensions.manim_height_to_height(mobject.height) > DEFAULT_SCENE_SIZE[1]:
...
def fullscreen_image(
    ...
    **kwargs
    ...
    """
    image_width_fitted = ImageMobject(filename, image_mode, **kwargs).scale_to_fit_width(ManimDimensions.width_to_manim_width(DEFAULT_SCENE_SIZE[0]))

    # We want the image that occupies the whole screen
    if ManimDimensions.manim_height_to_height(image_width_fitted.height) >= DEFAULT_SCENE_SIZE[1]:
        return image_width_fitted
    
    image_height_fitted = ImageMobject(filename, image_mode, **kwargs).scale_to_fit_height(ManimDimensions.height_to_manim_height(DEFAULT_SCENE_SIZE[1]))
...
def preprocess_image(
    ...
    """
    if ManimDimensions.manim_width_to_width(image.width) > DEFAULT_SCENE_SIZE[0]:
        image.scale_to_fit_width(ManimDimensions.width_to_manim_width(DEFAULT_SCENE_SIZE[0]))
    if ManimDimensions.manim_height_to_height(image.height) > DEFAULT_SCENE_SIZE[1]:
