...
REQUIRED_MODULES = [
    'multithreading', 'loguru', 'colorama', 'tqdm', 'bs4', 'pyfiglet',
    'requests', 'ipcalc', 'six', 'ping3', 'aiohttp', 'InquirerPy',
    'termcolor', 'tldextract', 'websocket-client', 'dnspython', 'bugscan-x',
    'inquirer', 'shutil', 'cryptography', 'queue'
]

SYSTEM_PACKAGES = ["rust", "binutils", "clang", "openssl", "openssl-tool", "make", "dnsutils",]
...
def install_system_package(package_name, package_manager):
    try:
        subprocess.run([package_manager, "install", "-y", package_name], check=True)
...
def update_system(package_manager):
    try:
        ...
        subprocess.run([package_manager, "upgrade", "-y"], check=True)
...
def install_dependencies():
    ...
    =========================================
    \033[38;5;10mAUTOMATED PACKAGE AND MODULE INSTALLATION\033[0m
    =========================================
    ...
    if input("\033[4m \033[38;5;196mFirst Time Running?\033[0m \033[38;5;208mInstall packages and modules?\033[0m (\033[38;5;46myes\033[0m/\033[38;5;196mno\033[0m):").lower() not in ['yes', 'y']:
        print("\nSkipping installation...")
        time.sleep(1)
        clear_screen()
        return

    # Install system packages on non-Windows systems
    if os.name != 'nt':
        package_manager = input("\nEnter your package manager (apt/pkg/etc): ").strip()
        
        if update_system(package_manager):
            print("\nInstalling system packages...")
            for package in SYSTEM_PACKAGES:
                install_system_package(package, package_manager)
...
def host_checker():
    ...
    def check_status(url, filename=None, not_found_filename=None):
        try:
            ...
            url_http = url
            r_http = requests.get(url_http, headers=headers, timeout=5)
            ...
            r_https = requests.get(url_https, headers=headers, timeout=5)
            ...
            if filename:
                with open(filename, 'a') as f:
                    f.write(f'{url_http} : HTTP({status_http}), Server: {server_http}, Connection: {connection_http}, IPs: IPv4: {", ".join(ipv4_addresses_http)}, IPv6: {", ".join(ipv6_addresses_http)}\n')
                    f.write(f'{url_https} : HTTPS({status_https}), Server: {server_https}, Connection: {connection_https}, IPs: IPv4: {", ".join(ipv4_addresses_https)}, IPv6: {", ".join(ipv6_addresses_https)}\n')
...
def host_proxy_checker():
    ...
    def save_result(result, output_file, save_flag):
        # Only save if the output_file is provided and if the result contains "Keep-Alive" and "SNI Used"
        if output_file and "Keep-Alive" in result and "SNI Used" in result:
            with open(output_file, 'a' if os.path.exists(output_file) else 'w') as file:
                file.write(result + "\n")
            save_flag[0] = True  # Indicate that a result has been saved

    def sni_check(domain_or_ip, https_url, output_file, save_flag):
        try:
            print(f"Performing SNI check on {domain_or_ip}...")
            response = requests.get(https_url, headers={"Host": domain_or_ip}, timeout=5)
            status_code = response.status_code
            if status_code in [200]:
                server_info = response.headers.get('Server', 'Unknown')
                connection = response.headers.get('Connection', '')
                keep_alive = "Keep-Alive" if "keep-alive" in connection.lower() else "Closed"

                result = f"{domain_or_ip} | SNI Check | Status: {status_code} | Server: {server_info} | Connection: {keep_alive}"
                print(f"\033[92mSNI check successful for {domain_or_ip} (Status: {status_code}, Server: {server_info}, Connection: {keep_alive})\033[0m")
                save_result(result, output_file, save_flag)
            else:
                print(f"\033[93mSNI check for {domain_or_ip} received status code: {status_code}\033[0m")
        except RequestException:
            print(f"\033[91mSNI check failed for {domain_or_ip}\033[0m")
            sni_check(domain_or_ip)

    def proxy_request(proxy_host, http_url, https_url, output_file, save_flag):
        proxies = {"http": f"http://{proxy_host}", "https": f"http://{proxy_host}"}

        def try_request(url, port, use_sni=False):
            try:
                headers = {"Host": proxy_host} if use_sni else {}  # Set SNI header if needed
                response = requests.get(url, proxies={"http": proxies["http"]}, headers=headers, timeout=5)
                status_code = response.status_code
                if status_code in [200]:
                    server_info = response.headers.get('Server', 'Unknown')
                    connection = response.headers.get('Connection', '')
                    ...
                    sni_note = "SNI Used" if use_sni else "No Sni"  # Add SNI indicator to result
                    result = f"{proxy_host} | Use As Proxy | {sni_note} | Port {port} | Status: {status_code} | Server: {server_info} | Connection: {keep_alive}"
                    
                    print(f"\033[92mRequest on port {port} successful for {url} via {proxy_host} {sni_note} "
                        f"(Status: {status_code}, Server: {server_info}, Connection: {keep_alive})\033[0m")
                    
                    save_result(result, output_file, save_flag)
                else:
                    print(f"Proxy Request via port {port} received status code: {status_code}")
            except LocationParseError as e:
                print("\033[91mFailed to parse URL: {url}. Error: \033[0m")
            except (RequestException, ProxyError) as e:
                print(f"\033[91mError connecting to {url} on port {port}. Error: \033[0m")
            except Exception as e:  # Catch any other exceptions
                print("\033[91mAn unexpected error occurred: \033[0m")

        # Try HTTP and HTTPS via proxy (without and with SNI)
        try_request(http_url, 80),
        try_request(http_url, 80, use_sni=True),
        try_request(https_url, 443),
        try_request(https_url, 443, use_sni=True)  # HTTPS with SNI

    def process_proxies(proxies, http_url, https_url, output_file):
        save_flag = [False]  # A list to pass the flag by reference (lists are mutable)
        
        with ThreadPoolExecutor(max_workers=20) as executor:
            list(tqdm(executor.map(lambda proxy: proxy_request(proxy, http_url, https_url, output_file, save_flag), proxies), desc="Processing proxies", unit="proxy"))

        # After processing, notify the user whether results were saved or not
        if output_file:
            if save_flag[0]:
                print(f"\033[92mResults saved to {output_file}\033[0m")
            else:
                print("\033[93mI could not find anything, please try again.\033[0m")

    def validate_url(url):
        url = re.sub(r'^\.{1,}', '', url)  # Remove any leading dots
        return url
    ...
    def host_proxy_checker_main():
        input_file = input("Enter txt file with proxy URLs, domains, or CIDRs: ")
        try:
            with open(input_file, 'r') as file:
                proxies = []
                for line in file:
                    entry = line.strip()
                    if "/" in entry:  # Check if CIDR is provided
                        proxies.extend(generate_ips_from_cidr(entry))
                    else:
                        proxies.append(entry)

            if not proxies:
                print("Error: No valid proxies, domains, or IPs found in the file.")
                return

            http_url = input("Enter the isp domain to check: ")
            http_url = validate_url(http_url)
            http_url = http_url if http_url.startswith("http") else "http://" + http_url
            https_url = "https://" + http_url.lstrip("http://")

            output_file = input("Enter Output .txt: ") if input("Do you want to save the results? (y/n): ").strip().lower() == 'y' else None
            process_proxies(proxies, http_url, https_url, output_file)

        except FileNotFoundError:
            print(f"Error: File {input_file} not found.")

    host_proxy_checker_main()
...
def web_crawler():
    ...
    # Set a semaphore for limiting concurrent requests per URL
    concurrency_limit = 10

    # Load visited URLs from the output file if it exists
    ...
    async def fetch_url(session, url):
        if url in visited_urls or (start_domain and urlparse(url).netloc != start_domain):
            return False
        async with asyncio.Semaphore(concurrency_limit):  # Limit to 10 concurrent requests per main URL
            try:
                headers = {
                ...
                async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    ...
                    if response_code == 200:
                        ...
                        for link in soup.find_all('a', href=True):
                            href = link['href']
                            next_url = urljoin(url, href).split('#')[0]  # Remove fragment identifiers
                            if next_url not in found_urls:
                                found_urls.add(next_url)
                                tasks.append(fetch_url(session, next_url))
                                new_urls_found = True

                        # Execute concurrent requests within the main URL's scope
                        if tasks:
                            await asyncio.gather(*tasks)
        ...
        async with aiohttp.ClientSession() as session:
            for url in url_list:
                ...
                if not parsed_url.scheme:
                    url = 'https://' + url.strip()  # Default to https if no scheme is present
                elif not parsed_url.scheme in ["http", "https"]:
                    print(f"Invalid URL scheme for {url}, skipping...")
                    continue
                global start_domain
                start_domain = urlparse(url).netloc
                print(f"Processing domain: {start_domain}")
                new_urls_found = await fetch_url(session, url)
                if not new_urls_found:
                    save_output()  # Save if no new URLs are found for this domain
    ...
    async def web_crawler_main():
        ...
        # Final save after completion if there are any new URLs left
        save_output()
...
def hacker_target():
    ...
    def hacker_target_main():
        target = input("Enter the domain to scan: ").strip()
        ...
        while True:
            clear_screen()
            print(f"\n[+] Select an option for further scanning: ({target})")
            print("[1] Host Search")
            print("[2] Reverse DNS")
            print("[3] DNS Lookup")
...
def stat():
    ...
    def create_combined_output_file() -> str:
        """Create single output file with timestamp for all results."""
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f"combined_scan_{timestamp}.txt"
        os.makedirs('scans', exist_ok=True)
        return os.path.join('scans', filename)
    ...
    def attempt_connection(ip, output_file_name):
        ...
        for port in ports:
            try:
                ...
                if status_code in [200, 401, 402, 403, 404, 405, 500]:
...
def websocket_scanner_old():
    ...
    \t#####################################################################
    \t#                                                                   #          
    \t#   KING'S WEBSOCKET SCANNER                                        #          
    \t#                                                                   #          
    \t#  This script is used to scan a range of IP addresses              #
    \t#  to find ones that are running a server with the cloudflare       #
    \t#  string in the Server field of the HTTP header.                   #
    \t#  It does this by creating a list of IP addresses using the        #
    \t#  ipaddress module and then using multiprocessing to create a      #
    \t#  specified number of threads to scan the addresses concurrently.  # 
    \t#                                                                   #
    \t#  Built for speed (using threading)                                #
    \t#                                                                   #
    \t#####################################################################
...
def open_port_checker():
    ...
    def scan_port(target, port, timeout=0.5):
        ...
        try:
            if port == 443:  # SSL/TLS port example
                context = ssl.create_default_context()
    ...
    def scan_ports_threaded(targets, ports, num_threads=10, timeout=0.5):
        """Scan ports for multiple targets using multiple threads, processing them in chunks."""
        results_dict = {}
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
    ...
    def print_results(results_dict, ports):
        """Print results in a table format with ports as columns and domains/IPs as rows."""
        headers = ['Domain/IP'] + [str(port) for port in ports]
        separator_line = "+" + "+".join(["-" * len(header) for header in headers]) + "+" 
        header_line = "|" + "|".join([f" {header} " for header in headers]) + "|"

        print(separator_line)
        print(header_line)
        print(separator_line.replace("-", "="))  # Double separator below headers

        rows = []
        for target, results in results_dict.items():
            open_ports = {port: status for port, status in results.items() if status == "Open"}
            if open_ports:
                row = [target] + [f"{port} {results[port]}" if results[port] == "Open" else "Closed" for port in ports]
                row_line = "|" + "|".join([f" {cell} " for cell in row]) + "|"
                rows.append(row_line)
                print(row_line)
                print(separator_line)

        return "\n".join([separator_line, header_line, separator_line.replace("-", "=")] + rows + [separator_line])

    def save_to_file(filename, data, results_dict, ports):
        """Save data to a file, including only targets with open ports."""
        with open(filename, "w") as file:
            headers = ['Domain/IP'] + [str(port) for port in ports]
            separator_line = "+" + "+".join(["-" * len(header) for header in headers]) + "+"
            header_line = "|" + "|".join([f" {header} " for header in headers]) + "|"
            ...
            file.write(separator_line.replace("-", "=") + "\n")

            for target, results in results_dict.items():
                open_ports = {port: status for port, status in results.items() if status == "Open"}
                if open_ports:
                    row = [target] + [f"{port} Open" if results[port] == "Open" else "Closed" for port in ports]
                    row_line = "|" + "|".join([f" {cell} " for cell in row]) + "|"
                    file.write(row_line + "\n")
                    file.write(separator_line + "\n")
    ...
    def open_port_checker_main():
        ports = [80, 8080, 443, 21, 22, 53, 67, 68, 123, 161, 162, 500, 520, 514, 5353, 4500, 1900, 5000, 3000]
        ...
        try:
            ...
            if input_type == "1":  # Single domain
                target = input("Enter the domain or IP: ")
                num_threads = int(input("Enter the number of threads: "))
                results_dict = scan_ports_threaded([target], ports, num_threads)
                ...
                num_threads = int(input("Enter the number of threads: "))
                results_dict = scan_ports_threaded(targets, ports, num_threads)

            table_str = print_results(results_dict, ports)

            save_file = input("Do you want to save the results to a file? (Y/N): ")
            if save_file.lower() == "y":
                filename = input("Enter the filename to save the results: ")
                save_to_file(filename, table_str, results_dict, ports)
...
def dork_scanner():
    ...
    generate_ascii_banner("DORK", "SCANNER")
...
def file_proccessing():
    ...
    def extract_domains(lines, output_file):
        ...
        domains = list(set(domains))  # Remove duplicate domains

        if domains:  # Only write if there are domains to save
            with open(output_file, 'w') as out_file:
                for domain in domains:
                    clean_domain = domain.lstrip('*.')  # Remove '*' and '.' from the start
                    out_file.write(f"{clean_domain}\n")
    ...
    input_filename = input("Enter the name of the file to be processed: ")
...
def teamerror():
        ...
        def decode_vmess_file(input_file, output_file):
            try:
                ...
                for encoded_data in file_content.splitlines():
                    decoded_v2ray_data = decode_v2ray(encoded_data.strip())
        ...
        def decode_v2ray(encoded_data):
            for protocol_prefix in ["vmess://", "vless://", "trojan://"]:
                if encoded_data.startswith(protocol_prefix):
                    ...
                    break  # Exit the loop once we find a matching prefix

            try:
                # Ensure we properly handle padding for base64 decoding
                ...
                print(f"Failed to decode: {encoded_data}. Error: {e}")
                return None
    ...
    def script003():
        ...
        def split_and_decode_vless(file_name):
            ...
            try:
                ...
                print("Decoded VLESS configurations saved to:", output_decoded)
    ...
    def script005():
        ...
        def a4():
            ...
            try:
                ...
                print(CYAN + f"Total connected VMess URLs: {connected_count}" + ENDC)
                ...
                if save_file == 'yes':
                    ...
                    print(CYAN + f"Working VMess URLs saved to '{output_file_path}'." + ENDC)

            except FileNotFoundError:
                print(FAIL + f"File '{file_path}' not found in the current directory. Please provide a valid file name." + ENDC)
            except Exception as e:
                print(FAIL + "An error occurred:" + ENDC)
    ...
    print("3.""\033[95mDecode VLESS/ Replace all host/ip\033\033[0m""  4.)""\033[33mRe-encode VMESS !!\033[0m")
    ...
    if choice == '0':
        time.sleep(1)
        os.system('cls' if os.name == 'nt' else 'clear')
        Configs_V2ray_menu()  # Return to the main menu
    elif choice == '':
        clear_screen()
        Configs_V2ray_menu() 
    elif choice == '1':
