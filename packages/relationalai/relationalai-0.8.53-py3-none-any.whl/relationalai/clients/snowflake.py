...
Otherwise, remove it from your '{profile}' configuration profile.
    ...
    def poll_use_index(
        self, app_name: str, sources: Iterable[str], model: str, engine_name: str, program_span_id: str | None, headers: Dict | None = None,
    ):
        ...
        def check_ready(spinner):
            ...
            use_index_id = f"{model}_{execution_id}"
            ...
            with debugging.span("wait", counter=counter, use_index_id=use_index_id) as span:
    ...
    def _exec_async_v2(
        ...
        with debugging.span("transaction", **query_attrs_dict) as txn_span:
            with debugging.span("create_v2", **query_attrs_dict) as create_span:
                request_headers['user-agent'] = f"pyrel/{__version__}"
                use_graph_index = self.config.get("use_graph_index", USE_GRAPH_INDEX)
