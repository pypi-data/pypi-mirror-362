...
def span_code(span:SpanNode):
    if span.type == "rule":
        ...
        if toggles["lqp"]:
            code_with_header("lqp", span.end_attrs["lqp"], language="lqp")
        if toggles["sql"]:
            code_with_header("sql", span.end_attrs["sql"], language="sql")
...
@ui.refreshable
def details():
    with ui.column().style("gap:2px; margin-left: 220px;"):
...
@ui.refreshable
def sidebar():
    with ui.column().style("min-width: 250px; max-width: 300px; margin-top: 50px; height: calc(100vh - 60px); overflow-x: hidden; overflow-y: auto; position: fixed; top: 0; left: 0; z-index: 1000;"):
        for root in current_json_objects:
            span_ui(root)
