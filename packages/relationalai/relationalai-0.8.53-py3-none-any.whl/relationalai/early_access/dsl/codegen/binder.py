...
from relationalai.early_access.dsl.bindings.common import Binding, IdentifierConceptBinding, RoleBinding, PartitioningConceptBinding, \
    BindableTable, BindableAttribute, ReferentConceptBinding
...
class Binder:
    def __init__(self, model, reasoner: OntologyReasoner):
        self._model = model
        ...
        self._filtering_subtype_bindings: OrderedSet[PartitioningConceptBinding] = OrderedSet()
    ...
    def is_filtering_binding(binding: Binding) -> bool:
        """
        Returns True if the binding is a FilteringConceptBinding or has filter_by.
        """
        return isinstance(binding, PartitioningConceptBinding) or binding.filter_by is not None

    def subtype_bindings(self) -> OrderedSet[Union[IdentifierConceptBinding, ReferentConceptBinding]]:
        """
        Returns the subtype bindings.
        """
        return self._subtype_bindings

    def filtering_subtype_bindings(self) -> OrderedSet[PartitioningConceptBinding]:
        """
        Returns the filtering subtype bindings.
        """
        return self._filtering_subtype_bindings
    ...
    def lookup_binding_role(self, binding: Binding):
        if isinstance(binding, RoleBinding):
            ...
            role = binding.role
        elif isinstance(binding, (IdentifierConceptBinding, PartitioningConceptBinding, ReferentConceptBinding)):
    ...
    def lookup_subtype_reference_binding(self, binding: IdentifierConceptBinding):
        """
    ...
    def lookup_subtype_reference_binding_by_source(self, binding: Union[IdentifierConceptBinding, PartitioningConceptBinding]):
        """
        Look up a constructor binding referenced by the subtype binding.
        """
        ctor_role = self._reasoner.subtype_unique_ref_scheme_role(binding.entity_type)
    ...
    def lookup_bound_identifier_constraint_by_source(self, concept: qb.Concept, source: BindableTable):
        ...
        if len(bound_constraints) != len(identifier_constraints):
            raise ValueError(f"Cannot look up bound identifier constraints for {concept} from {source}, "
                             f"expected {len(identifier_constraints)} but found {len(bound_constraints)} "
                             f"constraints")
    ...
    def _classify_binding(self, binding: Binding):
        ...
        elif isinstance(binding, PartitioningConceptBinding):
            self._process_filtering_subtype_binding(binding)
        else:
            raise ValueError(f"Unknown binding type: {type(binding)}")
    ...
    def _process_filtering_subtype_binding(self, binding: PartitioningConceptBinding):
        """
        ...
        RoleValueConstraint.
        """
        self._filtering_subtype_bindings.add(binding)
        # TODO: add to self._role_bindings

    def _classify_binding_by_type(self, binding: Binding, player: qb.Concept):
        if isinstance(binding, PartitioningConceptBinding):
            # FilteringSubtypeBinding is neither an entity nor a value type binding.
            return
