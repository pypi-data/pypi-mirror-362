...
def find_keys(item: Any, keys:OrderedSet[Any]|None = None, enable_primitive_key: bool = False) -> OrderedSet[Any]:
    ...
    if isinstance(item, (list, tuple)):
        ...
        # find last role's key if it is 1-to-many
        if item.is_many():
            find_keys(item._field_refs[-1], keys)
    elif isinstance(item, Concept):
...
class Relationship(Producer):
    ...
    def __init__(self, madlib:str, parent:Producer|None=None, short_name:str="", model:Model|None=None, fields:list[Field]|None=None, is_many:bool|None=None, ir_relation:ir.Relation|None=None):
        ...
        if isinstance(item, ConceptExpression):
            ...
            if isinstance(item, ConceptMember):
                ...
                if types.is_value_type(op_type):
                    inner_type, type_box_type = out.type, op_type
                    if inner_type == type_box_type:
                        return out

                    if (inner_type, type_box_type) in self.box_type_relations:
                        box_type = self.box_type_relations[(inner_type, type_box_type)]
                    else:
                        box_type = f.relation(
                            "box_type",
                            [f.field("a", inner_type), f.field("b", type_box_type)],
                            annos=[builtins.type_box_annotation]
                        )
                        self.box_type_relations[(inner_type, type_box_type)] = box_type
                    self.relations[item] = box_type
                    new_out = f.var(to_name(item._op), op_type)
                    ctx.add(f.lookup(box_type, [out, new_out], annos=[builtins.type_box_annotation]))
                    out = new_out
