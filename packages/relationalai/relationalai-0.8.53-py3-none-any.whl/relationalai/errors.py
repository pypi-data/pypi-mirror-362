...
class UnknownSourceWarning(RAIWarning):
    def __init__(self, source_fqns: list[str], role: str | None = None):
        ...
        if len(source_fqns) == 1:
            msg = textwrap.fill(
                f"Unable to access source '{source_fqns[0]}'. Ensure the source's fully qualified name is spelled correctly and that the role your program is executing with has permission to access it.",
                78,
            )
        else:
        ...
        if role:
            role_note = textwrap.fill(
                f"Your current primary role '{role}' must be granted the required privileges. "
                f"Due to a Snowflake secondary role issue, the RAI Python API only uses the primary role. "
                f"You may have access in other clients if privileges are granted through secondary roles, "
                f"but the RAI Python API will fail unless the primary role has the necessary permissions.",
                78,
            )
            msg += "\n\n" + role_note
...
class EngineNameValidationException(RAIException):
    ...
    def format_message(self):
        ...
        return "\n".join([
            f"We tried to auto-create engine [yellow]{self.engine_name}[/yellow] but it failed. Engine name requirements are:"
            "\n",
            f"[yellow]{ENGINE_NAME_ERROR}"
        ])
...
def handle_missing_integration(e: Exception):
    ...
    if in_warehouse and "NameResolutionError" in str(e):
...
[yellow]Please check your configuration file and make sure all required values are set.
"""
...
class SnowflakeAppMissingException(RAIException):
    ...
    def format_message(self):
        base = textwrap.dedent(f"""
            ...
            If it's installed under a different name, run 'rai init' on the command line to set the app name.
        """)

        if self.role:
            role_note = textwrap.dedent(f"""
                If the app is installed but access is denied, check your current primary role: '{self.role}'.
                Due to a Snowflake secondary role issue, the RAI Python API currently only allows access via the primary role.
                You may have access in other clients if privileges are granted through secondary roles,
                but the RAI Python API will fail unless the primary role has the required permissions.
            """)
        else:
            role_note = textwrap.dedent("""
...
class SnowflakeImportMissingException(RAIException):
    ...
    def format_message(self):
        ...
        return self.message_for_environment(
            message,
            f"rai imports:stream --source {self.import_name} --model {self.model_name}",
