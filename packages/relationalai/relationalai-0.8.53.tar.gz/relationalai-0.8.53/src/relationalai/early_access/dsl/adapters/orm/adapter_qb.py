...
class ORMAdapterQB:
    ...
    def _add_inclusive_role_constraints(self, model):
        ...
        for rc in self._parser.inclusive_role_constraints():
            constraint_roles = []
            for ro in rc.roles:
                constraint_roles.append(self._role_id_to_role_object(model, roles[ro]))
    ...
    def _add_exclusive_role_constraints(self, model):
        ...
        for rc in self._parser.exclusive_role_constraints():
            constraint_sequences = [self._role_id_to_role_object(model, roles[ro]) for role_list in rc.roles for ro in role_list]
            model.exclusive_roles(*constraint_sequences)
    ...
    def _add_ring_constraints(self, model):
        ...
        for rc in self._parser.ring_constraints().values():
            ...
            for ro in rc.roles:
                constraint_roles.append(self._role_id_to_role_object(model, roles[ro]))
            for rt in rc.ring_types:
                constraint_types.append(self._ring_type_mapping.get(rt))
    ...
    def _add_role_subset_constraints(self, model):
        ...
        for rsc in self._parser.role_subset_constraints().values():
            constraint_roles = [[self._role_id_to_role_object(model, roles[ro]) for ro in ro_list] for ro_list in rsc.roles]
    ...
    def _add_equality_constraints(self, model):
        ...
        for ec in self._parser.equality_constraints().values():
            constraint_roles = [[self._role_id_to_role_object(model, roles[ro]) for ro in ro_list] for ro_list in ec.roles]
