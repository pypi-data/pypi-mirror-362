...
class TestSQLModelFeatures(tb.SQLModelTestCase):
    SCHEMA = os.path.join(
    ...
    def setUpClass(cls):
        ...
        from feat import default, other
        from feat.other import nested
        cls.sm = default
        cls.sm_o = other
        cls.sm_on = nested
    ...
    def test_sqlmodel_module_01(self):
        vals = self.sess.exec(select(self.sm_o.Branch))
        self.assertEqual(
        ...
        vals = self.sess.exec(select(self.sm_on.Leaf))
        self.assertEqual(
            {r.num for r in vals},
            {10, 20, 30},
        )

        vals = self.sess.exec(select(self.sm.Theme))
        self.assertEqual(
            {
    ...
    def test_sqlmodel_module_02(self):
        val = self.sess.exec(
            select(self.sm.Theme).filter_by(color='orange')
        ).one()
        self.assertEqual(
        ...
        br = self.sess.exec(
            select(self.sm_o.Branch).filter_by(val='small')
        ).one()
    ...
    def test_sqlmodel_bklink_01(self):
        # test backlink name collisions
        foo = self.sess.exec(
            select(self.sm.Foo).filter_by(name='foo')
        ).one()
        oof = self.sess.exec(
            select(self.sm.Foo).filter_by(name='oof')
        ).one()
